...
\lstdefinelanguage{TXL}
{morekeywords={all,assert,attr,by,comments,compounds,construct,deconstruct,define,each,end,export,external,function,import,
include,keys,list,match,not,...,],[,',opt,push,pop,redefine,repeat,replace,rule,see,skipping,tokens,where},
 sensitive=true, morecomment=[l]{\%}, morecomment=[l]{\#}, identifierstyle=\color{cyan},
 identifierstyle=\ttfamily, upquote=true,
 morecomment=[s]{\%\{}{\}\%}, morecomment=[s]{\%\(}{\)\%},morestring=[b]{"},
}

\lstdefinelanguage{diff}{sensitive=true, morecomment=[l]{<}, morecomment=[l]{>}, 
 morecomment=[s]{\%\{}{\}\%}, morecomment=[s]{\%\(}{\)\%},morestring=[b]{"},
}
...
\lstset{stringstyle=
\color{stringcolor}
\ttfamily,  identifierstyle=\color{cyan}}
...
\lstinputlisting[language=java,numbers=left,caption={\tt cat -n HelloWorld.java},label=listing:1]{code/HelloWorld-1.java}
\lstinputlisting[language=java,numbers=left,caption={\tt cat -n HelloWorld-2.java},label=listing:2]{code/HelloWorld-2.java}
\lstinputlisting[language=diff,identifierstyle=,caption={\tt diff -w HelloWorld.java HelloWorld-2.java},label=listing:3]{code/HelloWorld-2.diff}
\lstinputlisting[language=diff,identifierstyle=,caption={\tt ldiff.pl -w -o diff HelloWorld.java HelloWorld-2.java},label=listing:ldiff]{code/HelloWorld-2.ldiff}
...
\lstinputlisting[language=TXL,identifierstyle=,numbers=none,caption={\tt cat -n java.grm},label=listing:java0]{code/java.rules0.grm}
\lstinputlisting[language=TXL,identifierstyle=,numbers=left,numbersep=3pt,caption={\tt cat -n java.annotated.grm},label=listing:java1]{code/java.rules1.grm}
...
\lstinputlisting[language=TXL,identifierstyle=,numbers=left,numbersep=2pt,caption={\tt cat -n norm.grm},label=listing:extend]{code/norm.grm}
...
\lstinputlisting[language=TXL,identifierstyle=,numbers=left,numbersep=2pt,caption={\tt cat -n norm.Txl},label=listing:normalise]{code/norm.Txl}
...
\lstinputlisting[language=TXL,identifierstyle=,numbers=left,caption={\tt cat -n java.Txl},label=listing:output]{code/java.Txl}
...
\lstinputlisting[language=java,numbers=left,caption={\tt mct HelloWorld.java},label=listing:result5]{code/HelloWorld-2.norm}
\lstinputlisting[language=java,numbers=left,caption={\tt cat HelloWorld.java HelloWorld-2.java > HelloWorld-2.pair \\ mct HelloWorld-2.pair java.Txl},label=listing:result4]{code/HelloWorld-2.mct}
...
%\begin{ldiff}
%\tiny $\Delta_2=${\tt ldiff} & \tiny  Commit & \tiny  $\Delta_2$ File(J)  & \tiny  \tiny $\Delta_2$ File(L) & \tiny 
%$\Delta_2$ File(M) \\
%               & \tiny  & \tiny  $\Delta_2$ LOC(J) & \tiny  $\Delta_2$ LOC(L) & \tiny  $\Delta_2$ LOC(M) \\ 
%\end{ldiff}
\hline\hline
...
Loh and Kim~\cite{Loh:2010:LPD:1810295.1810348} present the LSDiff tool which automatically identifies structural changes as logic rules. 

Although our implementation is based on TXL~\cite{txl}, the concepts and implementations could be replaced using other generic transformation systems/ language engineering tools such as ADL/SDL\cite{klint05tosem}. 

\cite{wenzel08icse, wenzel08icsm}

\cite{degenais08icse}

\section{Conclusions}\label{sec:conclude}

In this paper, we presented a declarative approach to specify changes in programs that are relevant to different programming tasks. In particular, we demonstrated its use in extracting the changes at the API levels for evolving Java programs and hardware specifications. We show that four elementary annotations can account for all light-weight syntactical normalisations by default, and can be customized for more advanced semantical changes through user-defined functions. The generic specification approach is declarative and already works on several programming languages and domain-specific languages. The automated tool was implemented on top of a generic transformation system, TXL, and the results of detecting relevant differences in the three programs are evaluated and compared with other {\tt diff} utilities such as line-based {\tt diff}, showing more precise detection and acceptable scalability and time performance. The tool and results can be downloaded from the \href{http://sead1.open.ac.uk/mct}{http://sead1.open.ac.uk/mct}. 
