...
Using four elementary extensions to program language grammars (Ignore, Order, Prefer and Keep), developers can specify, with limited effort, what types of changes are {\em meaningful}, or relevant to their tasks. The algorithms for generating the normalisation and clone removal transformations distill the meaningful differences automatically. Our tool has been evaluated on a benchmark of programs to compare with similar techniques.
\end{abstract}
...
\lstinputlisting[numbers=left,caption={\tt cat -n HelloWorld-1.java},label=listing:1]{code/HelloWorld-1.java}
\lstinputlisting[numbers=left,caption={\tt cat -n HelloWorld-2.java},label=listing:2]{code/HelloWorld-2.java}
\lstinputlisting[language=,identifierstyle=,caption={\tt diff -w HelloWorld.java HelloWorld2.java},label=listing:3]{code/HelloWorld-2.diff}
\lstinputlisting[language=,identifierstyle=,caption={\tt ldiff.pl -w -o diff HelloWorld-1.java HelloWorld-2.java},label=listing:ldiff]{code/HelloWorld-2.ldiff}
...
\lstinputlisting[language=,identifierstyle=,numbers=left,caption={\tt cat -n java.annotated.grm},label=listing:java1]{code/java.rules1.grm}
