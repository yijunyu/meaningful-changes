...
public class FileFiniteStateMachineModel extends FileFiniteStateMachineBaseModel {
    ...
    public void unmarshal(InputStream stream) throws Exception {
        ...
        org.ofsm.impl.schema.FiniteStateMachine defMachine = 
            (org.ofsm.impl.schema.FiniteStateMachine)unmarshaller.unmarshal(stream);
        //org.ofsm.impl.schema.DefinitionsType defs = defMachine.getDefinitions();
    ...
    private void unmarshalStates(org.ofsm.impl.schema.States states) throws ClassNotFoundException, 
                                                                        InstantiationException, 
                                                                        IllegalAccessException {
    ...
    private org.ofsm.impl.schema.EventType marshalEvent(EventInfo eventInfo) {
        org.ofsm.impl.schema.EventType eventDef = 
                objFactory.createEventType();                              
        eventDef.setName(eventInfo.getEvent().getName());
        if(eventInfo.getClassName()!=null && eventInfo.getClassName().length() > 0)
            eventDef.setClassName(eventInfo.getClassName());
        return eventDef;
    }
    ...
    private org.ofsm.impl.schema.StateType marshalState(StateInfo stateInfo) {
        ...
        if(stateInfo.getClassName()!=null && stateInfo.getClassName().length() > 0)        
            stateDef.setClassName(stateInfo.getClassName());
        return stateDef;
    }
    ...
    private void marshalTransition(StateTransition trans, Transition transDef) {
        if(trans.getFromState()!=null)
            transDef.setFromState(trans.getFromState().getName());
        if(trans.getToState()!=null)
            transDef.setToState(trans.getToState().getName());
        if(trans.getEvent()!=null)
            transDef.setWhenEvent(trans.getEvent().getName());
