...
void setpcmark () {
    int i;
    xfmark_T * fm;
    if (global_busy || listcmd_busy || cmdmod.keepjumps) return;
    curwin -> w_prev_pcmark = curwin -> w_pcmark;
    curwin -> w_pcmark = curwin -> w_cursor;
    if (++curwin->w_jumplistlen > 100) {
        curwin -> w_jumplistlen = 100;
        vim_free (curwin -> w_jumplist [0].fname);
        for (i = 1; i < 100; ++i) curwin -> w_jumplist [i - 1] = curwin -> w_jumplist [i];
    }
...
void checkpcmark () {
    if (curwin -> w_prev_pcmark.lnum != 0 && ((((curwin -> w_pcmark).lnum == (curwin -> w_cursor).lnum) && ((curwin -> w_pcmark).col == (curwin -> w_cursor).
      col) && ((curwin -> w_pcmark).coladd == (curwin -> w_cursor).coladd)) || curwin -> w_pcmark.lnum == 0)) {
    ...
    int count;
{
    ...
    xfmark_T * jmp;
    cleanup_jumplist ();
    if (curwin -> w_jumplistlen == 0) return (pos_T *) ((void *) 0);
    for (;;) {
        if (curwin -> w_jumplistidx + count < 0 || curwin -> w_jumplistidx + count >= curwin -> w_jumplistlen) return (pos_T *) ((void *) 0);
        if (curwin -> w_jumplistidx == curwin -> w_jumplistlen) {
            setpcmark ();
            --curwin->w_jumplistidx;
            if (curwin -> w_jumplistidx + count < 0) return (pos_T *) ((void *) 0);
        }
        curwin -> w_jumplistidx += count;
        jmp = curwin -> w_jumplist + curwin -> w_jumplistidx;
        if (jmp -> fmark.fnum == 0) fname2fnum (jmp);
        if (jmp -> fmark.fnum != curbuf -> b_fnum) {
            if (buflist_findnr (jmp -> fmark.fnum) == ((void *) 0)) {
                count += count < 0 ? -1 : 1;
                continue;
            }
            if (buflist_getfile (jmp -> fmark.fnum, jmp -> fmark.mark.lnum, 0, 0) == 0) return (pos_T *) ((void *) 0);
            curwin -> w_cursor = jmp -> fmark.mark;
            pos = (pos_T *) -1;
        }
        else pos = &(jmp->fmark.mark);
        return pos;
    }
}
...
{
    int n;
    if (curbuf -> b_changelistlen == 0) return (pos_T *) ((void *) 0);
    n = curwin -> w_changelistidx;
    if (n + count < 0) {
        if (n == 0) return (pos_T *) ((void *) 0);
        n = 0;
    }
    else if (n + count >= curbuf -> b_changelistlen) {
        if (n == curbuf -> b_changelistlen - 1) return (pos_T *) ((void *) 0);
...
pos_T * getmark (c, changefile)
    ...
    int changefile;
{
    return getmark_fnum (c, changefile, ((void *) 0));
}

pos_T * getmark_fnum (c, changefile, fnum)
    int c;
    int changefile;
    int * fnum;
{
    ...
    if (c > '~');
    ...
    else if (c == '"') posp = &(curbuf->b_last_cursor);
    else if (c == '^') posp = &(curbuf->b_last_insert);
    else if (c == '.') posp = &(curbuf->b_last_change);
    else if (c == '[') posp = &(curbuf->b_op_start);
    else if (c == ']') posp = &(curbuf->b_op_end);
    else if (c == '{' || c == '}') {
        pos_T pos;
        oparg_T oa;
        ...
        pos = curwin -> w_cursor;
        listcmd_busy = 1;
        if (findpar (&oa.inclusive, c == '}' ? 1 : (-1), 1L, '\000', 0)) {
        ...
        if ((c == '<') == (((*startp).lnum != (*endp).lnum) ? (*startp).lnum < (*endp).lnum : (*startp).col != (*endp).col ? (*startp).col < (*endp).col : 
          (*startp).coladd < (*endp).coladd)) posp = startp;
        else posp = endp;
        if (curbuf -> b_visual.vi_mode == 'V') {
            pos_copy = *posp;
            posp = &pos_copy;
            if (c == '<') pos_copy.col = 0;
            else pos_copy.col = (0x7fffffffL);
            pos_copy.coladd = 0;
        }
    }
    else if (((c) < 0x7f && islower (c))) {
        posp = &(curbuf->b_namedm[c-'a']);
    }
    else if (((c) < 0x7f && isupper (c)) || ((c) >= '0' && (c) <= '9')) {
        if (((c) >= '0' && (c) <= '9')) c = c - '0' + ('z' - 'a' + 1);
        else c -= 'A';
        posp = &(namedfm[c].fmark.mark);
        if (namedfm [c].fmark.fnum == 0) fname2fnum (&namedfm[c]);
        if (fnum != ((void *) 0)) *fnum = namedfm [c].fmark.fnum;
        else if (namedfm [c].fmark.fnum != curbuf -> b_fnum) {
            posp = &pos_copy;
            if (namedfm [c].fmark.mark.lnum != 0 && changefile && namedfm [c].fmark.fnum) {
                if (buflist_getfile (namedfm [c].fmark.fnum, (linenr_T) 1, 0x01, 0) == 1) {
                    curwin -> w_cursor = namedfm [c].fmark.mark;
                    return (pos_T *) -1;
                }
                pos_copy.lnum = -1;
            }
            else pos_copy.lnum = 0;
        }
    }
    return posp;
}
    ...
    int begin_line;
{
    ...
    pos_T pos;
    pos = *startpos;
    if (dir == (-1) && begin_line) pos.col = 0;
    else if (dir == 1 && begin_line) pos.col = (0x7fffffffL);
    for (i = 0; i < ('z' - 'a' + 1); i ++) {
        if (curbuf -> b_namedm [i].lnum > 0) {
            if (dir == 1) {
                if ((result == ((void *) 0) || (((curbuf -> b_namedm [i]).lnum != (*result).lnum) ? (curbuf -> b_namedm [i]).lnum < (*result).lnum : (curbuf ->
                  b_namedm [i]).col != (*result).col ? (curbuf -> b_namedm [i]).col < (*result).col : (curbuf -> b_namedm [i]).coladd < (*result).coladd)) && (
                  ((pos).lnum != (curbuf -> b_namedm [i]).lnum) ? (pos).lnum < (curbuf -> b_namedm [i]).lnum : (pos).col != (curbuf -> b_namedm [i]).col ? (pos
                  ).col < (curbuf -> b_namedm [i]).col : (pos).coladd < (curbuf -> b_namedm [i]).coladd)) result = &curbuf->b_namedm[i];
            }
            else {
                if ((result == ((void *) 0) || (((*result).lnum != (curbuf -> b_namedm [i]).lnum) ? (*result).lnum < (curbuf -> b_namedm [i]).lnum : (*result).
                  col != (curbuf -> b_namedm [i]).col ? (*result).col < (curbuf -> b_namedm [i]).col : (*result).coladd < (curbuf -> b_namedm [i]).coladd)) && (
                  ((curbuf -> b_namedm [i]).lnum != (pos).lnum) ? (curbuf -> b_namedm [i]).lnum < (pos).lnum : (curbuf -> b_namedm [i]).col != (pos).col ? (
                  curbuf -> b_namedm [i]).col < (pos).col : (curbuf -> b_namedm [i]).coladd < (pos).coladd)) result = &curbuf->b_namedm[i];
            }
        }
    }
    return result;
    ...
    xfmark_T * fm;
{
    ...
    if (fm -> fname != ((void *) 0)) {
        if (fm -> fname [0] == '~' && (fm -> fname [1] == '/')) {
            ...
            vim_strncpy (NameBuff + len, fm -> fname + 2, 1024 - len - 1);
        }
        else vim_strncpy (NameBuff, fm -> fname, 1024 - 1);
    ...
    buf_T * buf;
{
    ...
    int i;
    win_T * wp;
    if (buf -> b_ffname == ((void *) 0)) return;
    name = home_replace_save (buf, buf -> b_ffname);
    if (name == ((void *) 0)) return;
    for (i = 0; i < ('z' - 'a' + 1) + 10; ++i) fmarks_check_one (&namedfm[i], name, buf);
    for (wp = firstwin; wp != ((void *) 0); wp = wp -> w_next) {
        for (i = 0; i < wp -> w_jumplistlen; ++i) fmarks_check_one (&wp->w_jumplist[i], name, buf);
    }
    vim_free (name);
}
...
static void fmarks_check_one (fm, name, buf)
    ...
    buf_T * buf;
{
    if (fm -> fmark.fnum == 0 && fm -> fname != ((void *) 0) && strcasecmp ((char *) (((name))), (char *) (((fm -> fname)))) == 0) {
...
{
    if (pos == ((void *) 0)) {
        emsg ((char_u *) (((char *) (e_umark))));
        return 0;
    }
    if (pos -> lnum <= 0) {
        if (pos -> lnum == 0) emsg ((char_u *) (((char *) (e_marknotset))));
        return 0;
    }
    if (pos -> lnum > curbuf -> b_ml.ml_line_count) {
        emsg ((char_u *) (((char *) (e_markinval))));
        return 0;
    }
    return 1;
}
...
{
    static int i = -1;
    if (i == -1) for (i = 0; i < ('z' - 'a' + 1) + 1; i ++) {
        namedfm [i].fmark.mark.lnum = 0;
        namedfm [i].fname = ((void *) 0);
    }
    for (i = 0; i < ('z' - 'a' + 1); i ++) buf -> b_namedm [i].lnum = 0;
...
char_u * fm_getname (fmark, lead_len)
    ...
    int lead_len;
{
    if (fmark -> fnum == curbuf -> b_fnum) return mark_line (&(fmark->mark), lead_len);
    ...
    int lead_len;
{
    char_u * s, * p;
    int len;
    if (mp -> lnum == 0 || mp -> lnum > curbuf -> b_ml.ml_line_count) return vim_strsave ((char_u *) "-invalid-");
    ...
    exarg_T * eap;
{
    ...
    for (i = 0; i < ('z' - 'a' + 1); ++i) show_one_mark (i + 'a', arg, &curbuf->b_namedm[i], ((void *) 0), 1);
    for (i = 0; i < ('z' - 'a' + 1) + 10; ++i) {
        if (namedfm [i].fmark.fnum != 0) name = fm_getname (&namedfm[i].fmark, 15);
        else name = namedfm [i].fname;
        if (name != ((void *) 0)) {
            show_one_mark (i >= ('z' - 'a' + 1) ? i - ('z' - 'a' + 1) + '0' : i + 'A', arg, &namedfm[i].fmark.mark, name, namedfm [i].fmark.fnum == curbuf ->
              b_fnum);
            if (namedfm [i].fmark.fnum != 0) vim_free (name);
        }
    }
    ...
    int current;
{
    ...
    int mustfree = 0;
    if (c == -1) {
        if (did_title) did_title = 0;
        else {
            if (arg == ((void *) 0)) msg ((char_u *) (((char *) ("No marks set"))));
            else emsg2 ((char_u *) (((char *) ("E283: No marks matching \"%s\""))), (char_u *) (arg));
        }
    }
    else if (!got_int && (arg == ((void *) 0) || vim_strchr (arg, c) != ((void *) 0)) && p -> lnum != 0) {
        if (!did_title) {
        ...
        msg_putchar ('\n');
        if (!got_int) {
            __builtin___sprintf_chk ((char *) IObuff, 0, __builtin_object_size ((char *) IObuff, 1 > 1), " %c %6ld %4d ", c, p -> lnum, p -> col);
            msg_outtrans (IObuff);
    ...
    exarg_T * eap;
{
    ...
    int digit;
    int n;
    if (*eap->arg == '\000' && eap -> forceit) clrallmarks (curbuf);
    else if (eap -> forceit) emsg ((char_u *) (((char *) (e_invarg))));
    else if (*eap->arg == '\000') emsg ((char_u *) (((char *) (e_argreq))));
    else {
        for (p = eap -> arg; *p != '\000'; ++p) {
            lower = ((*p) < 0x7f && islower (*p));
            digit = ((*p) >= '0' && (*p) <= '9');
            if (lower || digit || ((*p) < 0x7f && isupper (*p))) {
                if (p [1] == '-') {
                    from = *p;
                    to = p [2];
                    if (!(lower?((p[2])<0x7f&&islower(p[2])):(digit?((p[2])>='0'&&(p[2])<='9'):((p[2])<0x7f&&isupper(p[2])))) || to < from) {
                ...
                for (i = from; i <= to; ++i) {
                    if (lower) curbuf -> b_namedm [i - 'a'].lnum = 0;
                    else {
                        if (digit) n = i - '0' + ('z' - 'a' + 1);
                        else n = i - 'A';
    ...
    exarg_T * eap __attribute__ ((unused));
{
    ...
    for (i = 0; i < curwin -> w_jumplistlen && !got_int; ++i) {
        if (curwin -> w_jumplist [i].fmark.mark.lnum != 0) {
            if (curwin -> w_jumplist [i].fmark.fnum == 0) fname2fnum (&curwin->w_jumplist[i]);
            ...
            __builtin___sprintf_chk ((char *) IObuff, 0, __builtin_object_size ((char *) IObuff, 1 > 1), "%c %2d %5ld %4d ", i == curwin -> w_jumplistidx ? '>'
              : ' ', i > curwin -> w_jumplistidx ? i - curwin -> w_jumplistidx : curwin -> w_jumplistidx - i, curwin -> w_jumplist [i].fmark.mark.lnum, curwin
              -> w_jumplist [i].fmark.mark.col);
            msg_outtrans (IObuff);
            msg_outtrans_attr (name, curwin -> w_jumplist [i].fmark.fnum == curbuf -> b_fnum ? highlight_attr [(int) (HLF_D)] : 0);
            vim_free (name);
            ui_breakcheck ();
        }
        out_flush ();
    }
    if (curwin -> w_jumplistidx == curwin -> w_jumplistlen) msg_puts ((char_u *) ("\n>"));
}
    ...
    exarg_T * eap __attribute__ ((unused));
{
    ...
    for (i = 0; i < curbuf -> b_changelistlen && !got_int; ++i) {
        if (curbuf -> b_changelist [i].lnum != 0) {
            msg_putchar ('\n');
            if (got_int) break;
            __builtin___sprintf_chk ((char *) IObuff, 0, __builtin_object_size ((char *) IObuff, 1 > 1), "%c %3d %5ld %4d ", i == curwin -> w_changelistidx ?
              '>' : ' ', i > curwin -> w_changelistidx ? i - curwin -> w_changelistidx : curwin -> w_changelistidx - i, (long) curbuf -> b_changelist [i].lnum,
    ...
    if (curwin -> w_changelistidx == curbuf -> b_changelistlen) msg_puts ((char_u *) ("\n>"));
}
    ...
    long amount_after;
{
    ...
    win_T * win;
    tabpage_T * tab;
    if (line2 < line1 && amount_after == 0L) return;
    if (!cmdmod.lockmarks) {
        for (i = 0; i < ('z' - 'a' + 1); i ++) {
            {
                lp = &(curbuf->b_namedm[i].lnum);
                if (*lp >= line1 && *lp <= line2) {
                    if (amount == (0x7fffffffL)) *lp = 0;
                    else *lp += amount;
                }
                else if (amount_after && *lp > line2) *lp += amount_after;
            };
            if (namedfm [i].fmark.fnum == fnum) {
                lp = &(namedfm[i].fmark.mark.lnum);
                if (*lp >= line1 && *lp <= line2) {
                    if (amount == (0x7fffffffL)) *lp = line1;
                    else *lp += amount;
                }
                else if (amount_after && *lp > line2) *lp += amount_after;
            };
        ...
        for ((tab) = first_tabpage; (tab) != ((void *) 0); (tab) = (tab) -> tp_next) for ((win) = ((tab) == curtab) ? firstwin : (tab) -> tp_firstwin; (win); (
          win) = (win) -> w_next) qf_mark_adjust (win, line1, line2, amount, amount_after);
    ...
    for ((tab) = first_tabpage; (tab) != ((void *) 0); (tab) = (tab) -> tp_next) for ((win) = ((tab) == curtab) ? firstwin : (tab) -> tp_firstwin; (win); (win)
      = (win) -> w_next) {
        if (!cmdmod.lockmarks) for (i = 0; i < win -> w_jumplistlen; ++i) if (win -> w_jumplist [i].fmark.fnum == fnum) {
            lp = &(win->w_jumplist[i].fmark.mark.lnum);
            if (*lp >= line1 && *lp <= line2) {
                if (amount == (0x7fffffffL)) *lp = line1;
                else *lp += amount;
            }
        ...
        if (win -> w_buffer == curbuf) {
            if (!cmdmod.lockmarks) for (i = 0; i < win -> w_tagstacklen; i ++) if (win -> w_tagstack [i].fmark.fnum == fnum) {
            ...
            if (win != curwin) {
                if (win -> w_topline >= line1 && win -> w_topline <= line2) {
                    if (amount == (0x7fffffffL)) {
                        if (line1 <= 1) win -> w_topline = 1;
                        else win -> w_topline = line1 - 1;
                    }
                    else win -> w_topline += amount;
                    win -> w_topfill = 0;
                }
                ...
                if (win -> w_cursor.lnum >= line1 && win -> w_cursor.lnum <= line2) {
                    if (amount == (0x7fffffffL)) {
                        if (line1 <= 1) win -> w_cursor.lnum = 1;
                        else win -> w_cursor.lnum = line1 - 1;
                        win -> w_cursor.col = 0;
                    }
                    else win -> w_cursor.lnum += amount;
                }
                else if (amount_after && win -> w_cursor.lnum > line2) win -> w_cursor.lnum += amount_after;
            }
    ...
    long col_amount;
{
    ...
    win_T * win;
    pos_T * posp;
    if ((col_amount == 0L && lnum_amount == 0L) || cmdmod.lockmarks) return;
    for (i = 0; i < ('z' - 'a' + 1); i ++) {
        {
            posp = &(curbuf->b_namedm[i]);
            if (posp -> lnum == lnum && posp -> col >= mincol) {
                posp -> lnum += lnum_amount;
                if (col_amount < 0 && posp -> col <= (colnr_T) - col_amount) posp -> col = 0;
                else posp -> col += col_amount;
            }
        };
        if (namedfm [i].fmark.fnum == fnum) {
            posp = &(namedfm[i].fmark.mark);
            if (posp -> lnum == lnum && posp -> col >= mincol) {
                posp -> lnum += lnum_amount;
                if (col_amount < 0 && posp -> col <= (colnr_T) - col_amount) posp -> col = 0;
                else posp -> col += col_amount;
    ...
    for (win = firstwin; win != ((void *) 0); win = win -> w_next) {
        for (i = 0; i < win -> w_jumplistlen; ++i) if (win -> w_jumplist [i].fmark.fnum == fnum) {
            posp = &(win->w_jumplist[i].fmark.mark);
            if (posp -> lnum == lnum && posp -> col >= mincol) {
                posp -> lnum += lnum_amount;
                if (col_amount < 0 && posp -> col <= (colnr_T) - col_amount) posp -> col = 0;
                else posp -> col += col_amount;
...
static void cleanup_jumplist () {
    ...
    for (from = 0; from < curwin -> w_jumplistlen; ++from) {
        if (curwin -> w_jumplistidx == from) curwin -> w_jumplistidx = to;
        for (i = from + 1; i < curwin -> w_jumplistlen; ++i) if (curwin -> w_jumplist [i].fmark.fnum == curwin -> w_jumplist [from].fmark.fnum && curwin ->
          w_jumplist [from].fmark.fnum != 0 && curwin -> w_jumplist [i].fmark.mark.lnum == curwin -> w_jumplist [from].fmark.mark.lnum) break;
        if (i >= curwin -> w_jumplistlen) curwin -> w_jumplist [to ++] = curwin -> w_jumplist [from];
        else vim_free (curwin -> w_jumplist [from].fname);
    }
    if (curwin -> w_jumplistidx == curwin -> w_jumplistlen) curwin -> w_jumplistidx = to;
    ...
    win_T * to;
{
    ...
    for (i = 0; i < from -> w_jumplistlen; ++i) {
        ...
        if (from -> w_jumplist [i].fname != ((void *) 0)) to -> w_jumplist [i].fname = vim_strsave (from -> w_jumplist [i].fname);
    }
...
{
    int i;
    for (i = 0; i < wp -> w_jumplistlen; ++i) vim_free (wp -> w_jumplist [i].fname);
}
    ...
    int force;
{
    ...
    int i;
    str = virp -> vir_line + 1;
    if (*str <= 127 && ((*virp->vir_line == '\'' && (((*str) >= '0' && (*str) <= '9') || isupper (*str))) || (*virp->vir_line == '-' && *str == '\''))) {
        if (*str == '\'') {
            if (curwin -> w_jumplistlen == 100) fm = ((void *) 0);
            else {
                for (i = curwin -> w_jumplistlen; i > 0; --i) curwin -> w_jumplist [i] = curwin -> w_jumplist [i - 1];
                ++curwin->w_jumplistidx;
        ...
        else if (((*str) >= '0' && (*str) <= '9')) fm = &namedfm[*str - '0' + ('z' - 'a' + 1)];
        else fm = &namedfm[*str - 'A'];
        if (fm != ((void *) 0) && (fm -> fmark.mark.lnum == 0 || force)) {
            ...
            str = skipwhite (str);
            vim_free (fm -> fname);
            fm -> fname = viminfo_readstring (virp, (int) (str - virp -> vir_line), 0);
        }
    }
    ...
    FILE * fp;
{
    ...
    buf_T * buf;
    xfmark_T * fm;
    if (get_viminfo_parameter ('f') == 0) return;
    fputs (((char *) ("\n# File marks:\n")), fp);
    if (curbuf -> b_ffname != ((void *) 0) && !removable(curbuf->b_ffname)) {
        name = buflist_nr2name (curbuf -> b_fnum, 1, 0);
        for (i = ('z' - 'a' + 1); i < ('z' - 'a' + 1) + 10 - 1; ++i) if (namedfm [i].fmark.mark.lnum == curwin -> w_cursor.lnum && (namedfm [i].fname == ((void
          *) 0) ? namedfm [i].fmark.fnum == curbuf -> b_fnum : (name != ((void *) 0) && strcmp ((char *) (name), (char *) (namedfm [i].fname)) == 0))) break;
        vim_free (name);
        vim_free (namedfm [i].fname);
        for (; i > ('z' - 'a' + 1); --i) namedfm [i] = namedfm [i - 1];
        namedfm [('z' - 'a' + 1)].fmark.mark = curwin -> w_cursor;
        namedfm [('z' - 'a' + 1)].fmark.fnum = curbuf -> b_fnum;
        namedfm [('z' - 'a' + 1)].fname = ((void *) 0);
    }
    for (i = 0; i < ('z' - 'a' + 1) + 10; i ++) write_one_filemark (fp, &namedfm[i], '\'', i < ('z' - 'a' + 1) ? i + 'A' : i - ('z' - 'a' + 1) + '0');
    fputs (((char *) ("\n# Jumplist (newest first):\n")), fp);
    setpcmark ();
    cleanup_jumplist ();
    for (fm = &curwin->w_jumplist[curwin->w_jumplistlen-1]; fm >= &curwin->w_jumplist[0]; --fm) {
        if (fm -> fmark.fnum == 0 || ((buf = buflist_findnr (fm -> fmark.fnum)) != ((void *) 0) && !removable(buf->b_ffname))) write_one_filemark (fp, fm, '-',
          '\'');
    }
}
...
{
    char_u * name;
    if (fm -> fmark.mark.lnum == 0) return;
    if (fm -> fmark.fnum != 0) name = buflist_nr2name (fm -> fmark.fnum, 1, 0);
    else name = fm -> fname;
    ...
    if (fm -> fmark.fnum != 0) vim_free (name);
}
    ...
    if (name != ((void *) 0)) {
        for (p = p_viminfo; *p;) {
            ...
            if (part [0] == 'r') {
                ...
                if (strncasecmp ((char *) ((part + 1)), (char *) ((name)), (size_t) ((n))) == 0) {
                    retval = 1;
                    break;
                }
            }
    ...
    FILE * fp_out;
{
    ...
    for ((tp) = first_tabpage; (tp) != ((void *) 0); (tp) = (tp) -> tp_next) for ((win) = ((tp) == curtab) ? firstwin : (tp) -> tp_firstwin; (win); (win) = (
      win) -> w_next) set_last_cursor (win);
    fputs (((char *) ("\n# History of marks within files (newest to oldest):\n")), fp_out);
    count = 0;
    for (buf = firstbuf; buf != ((void *) 0); buf = buf -> b_next) {
        if (buf -> b_marks_read) {
            if (buf -> b_last_cursor.lnum != 0) is_mark_set = 1;
            else {
                is_mark_set = 0;
                for (i = 0; i < ('z' - 'a' + 1); i ++) if (buf -> b_namedm [i].lnum != 0) {
                    is_mark_set = 1;
                    break;
                }
            }
            if (is_mark_set && buf -> b_ffname != ((void *) 0) && buf -> b_ffname [0] != '\000' && !removable(buf->b_ffname)) {
                ...
                for (i = 0; i < buf -> b_changelistlen; ++i) write_one_mark (fp_out, '+', &buf->b_changelist[i]);
                for (i = 0; i < ('z' - 'a' + 1); i ++) write_one_mark (fp_out, 'a' + i, &buf->b_namedm[i]);
                count ++;
            }
        }
    }
...
static void write_one_mark (fp_out, c, pos)
    ...
    pos_T * pos;
{
    if (pos -> lnum != 0) fprintf (fp_out, "\t%c\t%ld\t%d\n", c, (long) pos -> lnum, (int) pos -> col);
}
    ...
    int flags;
{
    ...
    pos_T pos;
    list_T * list = ((void *) 0);
    if ((name_buf = alloc (512)) == ((void *) 0)) return;
    * name_buf = '\000';
    if (fp_out == ((void *) 0) && (flags & (8 | 4))) {
        list = list_alloc ();
        if (list != ((void *) 0)) set_vim_var_list (54, list);
    }
    num_marked_files = get_viminfo_parameter ('\'');
    while (!eof && (count < num_marked_files || fp_out == ((void *) 0))) {
        if (line [0] != '>') {
            if (line [0] != '\n' && line [0] != '\r' && line [0] != '#') {
                if (viminfo_error ("E576: ", ((char *) ("Missing '>'")), line)) break;
            }
        ...
        while (p != str && (*p == '\000' || vim_isspace (*p))) p --;
        if (*p) p ++;
        * p = '\000';
        if (list != ((void *) 0)) list_append_string (list, str, -1);
        load_marks = copy_marks_out = 0;
        if (fp_out == ((void *) 0)) {
            if ((flags & 2) && curbuf -> b_ffname != ((void *) 0)) {
                if (*name_buf == '\000') home_replace (((void *) 0), curbuf -> b_ffname, name_buf, 512, 1);
                if (strcasecmp ((char *) (((str))), (char *) (((name_buf)))) == 0) load_marks = 1;
            }
        }
        else {
            for (buf = firstbuf; buf != ((void *) 0); buf = buf -> b_next) if (buf -> b_ffname != ((void *) 0)) {
                home_replace (((void *) 0), buf -> b_ffname, name_buf, 512, 1);
                if (strcasecmp ((char *) (((str))), (char *) (((name_buf)))) == 0) break;
            }
            ...
            if (load_marks) {
                if (line [1] != '\000') {
                    ...
                    case '.' :
                        ...
                        break;
                    case '+' :
                        if (curbuf -> b_changelistlen == 100) ((__builtin_object_size ((char *) (curbuf -> b_changelist), 0) != (size_t) - 1) ?
                          __builtin___memmove_chk ((char *) (curbuf -> b_changelist), (char *) (curbuf -> b_changelist + 1), sizeof (pos_T) * (100 - 1),
                          __builtin_object_size ((char *) (curbuf -> b_changelist), 0)) : __inline_memmove_chk ((char *) (curbuf -> b_changelist), (char *) (
                          curbuf -> b_changelist + 1), sizeof (pos_T) * (100 - 1)));
                        else ++curbuf->b_changelistlen;
                        curbuf -> b_changelist [curbuf -> b_changelistlen - 1] = pos;
                        break;
                    default :
                        if ((i = line [1] - 'a') >= 0 && i < ('z' - 'a' + 1)) curbuf -> b_namedm [i] = pos;
                    }
                }
            }
            else if (copy_marks_out) fputs ((char *) line, fp_out);
        }
        if (load_marks) {
            win_T * wp;
            for (wp = firstwin; wp != ((void *) 0); wp = wp -> w_next) {
