    ...
    int flags;
{
    ...
    if ((mfp = (memfile_T *)alloc((unsigned)sizeof(memfile_T))) == ((void *)0))
 return ((void *)0);
...
# 177 "memfile.c"
    if (mfp->mf_fd >= 0
     && fstat(((mfp->mf_fd)), ((&stf))) == 0
     && stf.st_blksize >= 1048
     && stf.st_blksize <= 50000)
 mfp->mf_page_size = stf.st_blksize;


    if (mfp->mf_fd < 0 || (flags & (0x0400|0x0800))
    ...
    int del_file;
{
 ...
 vim_free(mf_rem_free(mfp));
    for (i = 0; i < 64; ++i)
 for (tp = mfp->mf_trans[i]; tp != ((void *)0); tp = tpnext)
 {
     tpnext = tp->nt_next;
     vim_free(tp);
 }
...
# 375 "memfile.c"
 ...
 {
     if (hp == ((void *)0) && (hp = mf_alloc_bhdr(mfp, page_count)) == ((void *)0))
  return ((void *)0);
 ...
 {
     if ((p = (char_u *)alloc(mfp->mf_page_size * page_count)) == ((void *)0))
  return ((void *)0);
     hp = mf_rem_free(mfp);
     hp->bh_data = p;
 }
 ...
 else
 {
    ...
    (void)((__builtin_object_size (((char *)(hp->bh_data)), 0) != (size_t) -1) ? __builtin___memset_chk (((char *)(hp->bh_data)), (0), ((size_t)mfp->mf_page_size * page_count), __builtin_object_size (((char *)(hp->bh_data)), 0)) : __inline_memset_chk (((char *)(hp->bh_data)), (0), ((size_t)mfp->mf_page_size * page_count)));
    ...
    int infile;
{
    ...
    if ((flags & 2) == 0)
 emsg((char_u *)(((char *)("E293: block was not locked"))));
    flags &= ~2;
    if (dirty)
    {
 flags |= 1;
    ...
    int flags;
{
 ...
 if (((flags & 1) || hp->bh_bnum >= 0)
  && (hp->bh_flags & 1)
  && (status == 1 || (hp->bh_bnum >= 0
      && hp->bh_bnum < mfp->mf_infile_count)))
 {
     if ((flags & 8) && hp->bh_bnum != 0)
  continue;
...
mf_rem_hash(mfp, hp)
    ...
    bhdr_T *hp;
{
    if (hp->bh_hash_prev == ((void *)0))
 mfp->mf_hash[((hp->bh_bnum) & (64 - 1))] = hp->bh_hash_next;
    else
    ...
    blocknr_T nr;
{
    ...
    for (hp = mfp->mf_hash[((nr) & (64 - 1))]; hp != ((void *)0); hp = hp->bh_hash_next)
 if (hp->bh_bnum == nr)
     break;
    return hp;
}
...
# 883 "memfile.c"
 ...
 return ((void *)0);
    ...
    if ((hp->bh_flags & 1) && mf_write(mfp, hp) == 0)
 return ((void *)0);
    ...
    bhdr_T *hp;
{
 ...
 if (mf_write_block(mfp, hp2 == ((void *)0) ? hp : hp2, offset, size) == 0)
 {
  ...
  emsg((char_u *)(((char *)("E297: Write error in swap file"))));
     did_swapwrite_msg = 1;
     return 0;
 }
 did_swapwrite_msg = 0;
    ...
    unsigned size;
{
 ...
 if (data == ((void *)0))
    ...
    if ((unsigned)write((mfp->mf_fd), (char *)(data), (size_t) (size)) != size)
 result = 0;
    ...
    bhdr_T *hp;
{
 ...
 return 1;
    ...
    if ((np = (NR_TRANS *)alloc((unsigned)sizeof(NR_TRANS))) == ((void *)0))
 return 0;
    ...
    blocknr_T old_nr;
{
    ...
    hash = ((old_nr) & (64 - 1));
    for (np = mfp->mf_trans[hash]; np != ((void *)0); np = np->nt_next)
