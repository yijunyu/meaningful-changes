    ...
    UINT32_T length;
{
 ...
 ctx->total[1]++;
    ...
    if (left && length >= fill)
    {
 ((__builtin_object_size ((void *)(ctx->buffer + left), 0) != (size_t) -1) ? __builtin___memcpy_chk ((void *)(ctx->buffer + left), (void *)input, fill, __builtin_object_size ((void *)(ctx->buffer + left), 0)) : __inline_memcpy_chk ((void *)(ctx->buffer + left), (void *)input, fill));
 ...
 input += 64;
    ...
    if (length)
 ((__builtin_object_size ((void *)(ctx->buffer + left), 0) != (size_t) -1) ? __builtin___memcpy_chk ((void *)(ctx->buffer + left), (void *)input, length, __builtin_object_size ((void *)(ctx->buffer + left), 0)) : __inline_memcpy_chk ((void *)(ctx->buffer + left), (void *)input, length));
}
    ...
    int salt_len;
{
 ...
 sha256_update(&ctx, salt, salt_len);
    ...
    for (j = 0; j < 32; j++)
 __builtin___sprintf_chk ((char *)hexit + j * 2, 0, __builtin_object_size ((char *)hexit + j * 2, 1 > 1), "%02x", sha256sum[j]);
...
sha256_self_test()
{
 ...
 if (i < 2)
 {
     hexit = sha256_bytes((char_u *)sha_self_test_msg[i],
      (int)strlen((char *)(sha_self_test_msg[i])),
      ((void *)0), 0);
     ((__builtin_object_size ((char *)(output), 0) != (size_t) -1) ? __builtin___strcpy_chk ((char *)(output), (char *)(hexit), __builtin_object_size ((char *)(output), 1 > 1)) : __inline_strcpy_chk ((char *)(output), (char *)(hexit)));
 }
 else
 {
     sha256_start(&ctx);
     ((__builtin_object_size ((buf), 0) != (size_t) -1) ? __builtin___memset_chk ((buf), ('a'), (1000), __builtin_object_size ((buf), 0)) : __inline_memset_chk ((buf), ('a'), (1000)));
     for (j = 0; j < 1000; j++)
  sha256_update(&ctx, (char_u *)buf, 1000);
     sha256_finish(&ctx, sha256sum);
     for (j = 0; j < 32; j++)
  __builtin___sprintf_chk (output + j * 2, 0, __builtin_object_size (output + j * 2, 1 > 1), "%02x", sha256sum[j]);
 }
