    ...
    xfmark_T *fm;
{
      ...
      ))
 {
     ...
     vim_strncpy(NameBuff, fm->fname, 1024 - 1);
    ...
    buf_T *buf;
{
    if (fm->fmark.fnum == 0
     && fm->fname != ((void *)0)
     && strcasecmp((char *)(((name))), (char *)(((fm->fname)))) == 0)
    {
    ...
    int current;
{
 ...
 {
     if (arg == ((void *)0))
  msg((char_u *)(((char *)("No marks set"))));
     else
  emsg2((char_u *)(((char *)("E283: No marks matching \"%s\""))), (char_u *)(arg));
 }
    }
    ...
    exarg_T *eap;
{
 ...
 for (p = eap->arg; *p != '\000'; ++p)
 {
  ...
  if (p[1] == '-')
  {
      ...
      from = *p;
      to = p[2];
      if (!(lower ? ((p[2]) < 0x7f && islower(p[2]))
    : (digit ? ((p[2]) >= '0' && (p[2]) <= '9')
        : ((p[2]) < 0x7f && isupper(p[2]))))
       || to < from)
      {
  ...
  switch (*p)
  {
      ...
      case ' ': break;
      default: emsg2((char_u *)(((char *)(e_invarg2))), (char_u *)(p));
         return;
  }
 }
    }
    ...
    exarg_T *eap __attribute__((unused));
{
 ...
 if (curwin->w_jumplist[i].fmark.mark.lnum != 0)
 {
  ...
  break;
     ...
     __builtin___sprintf_chk ((char *)IObuff, 0, __builtin_object_size ((char *)IObuff, 1 > 1), "%c %2d %5ld %4d ", i == curwin->w_jumplistidx ? '>' : ' ', i > curwin->w_jumplistidx ? i - curwin->w_jumplistidx : curwin->w_jumplistidx - i, curwin->w_jumplist[i].fmark.mark.lnum, curwin->w_jumplist[i].fmark.mark.col);
    ...
    exarg_T *eap __attribute__((unused));
{
 ...
 if (curbuf->b_changelist[i].lnum != 0)
 {
     ...
     if (got_int)
  break;
     __builtin___sprintf_chk ((char *)IObuff, 0, __builtin_object_size ((char *)IObuff, 1 > 1), "%c %3d %5ld %4d ", i == curwin->w_changelistidx ? '>' : ' ', i > curwin->w_changelistidx ? i - curwin->w_changelistidx : curwin->w_changelistidx - i, (long)curbuf->b_changelist[i].lnum, curbuf->b_changelist[i].col);
    ...
    long amount_after;
{
 ...
 for (i = 0; i < ('z' - 'a' + 1); i++)
 {
     { lp = &(curbuf->b_namedm[i].lnum); if (*lp >= line1 && *lp <= line2) { if (amount == (0x7fffffffL)) *lp = 0; else *lp += amount; } else if (amount_after && *lp > line2) *lp += amount_after; };
     if (namedfm[i].fmark.fnum == fnum)
  { lp = &(namedfm[i].fmark.mark.lnum); if (*lp >= line1 && *lp <= line2) { if (amount == (0x7fffffffL)) *lp = line1; else *lp += amount; } else if (amount_after && *lp > line2) *lp += amount_after; };
 }
 ...
 { lp = &(curbuf->b_last_insert.lnum); if (*lp >= line1 && *lp <= line2) { if (amount == (0x7fffffffL)) *lp = 0; else *lp += amount; } else if (amount_after && *lp > line2) *lp += amount_after; };


 { lp = &(curbuf->b_last_change.lnum); if (*lp >= line1 && *lp <= line2) { if (amount == (0x7fffffffL)) *lp = 0; else *lp += amount; } else if (amount_after && *lp > line2) *lp += amount_after; };



 for (i = 0; i < curbuf->b_changelistlen; ++i)
     { lp = &(curbuf->b_changelist[i].lnum); if (*lp >= line1 && *lp <= line2) { if (amount == (0x7fffffffL)) *lp = line1; else *lp += amount; } else if (amount_after && *lp > line2) *lp += amount_after; };




 { lp = &(curbuf->b_visual.vi_start.lnum); if (*lp >= line1 && *lp <= line2) { if (amount == (0x7fffffffL)) *lp = line1; else *lp += amount; } else if (amount_after && *lp > line2) *lp += amount_after; };
 { lp = &(curbuf->b_visual.vi_end.lnum); if (*lp >= line1 && *lp <= line2) { if (amount == (0x7fffffffL)) *lp = line1; else *lp += amount; } else if (amount_after && *lp > line2) *lp += amount_after; };
 ...
 for ((tab) = first_tabpage; (tab) != ((void *)0); (tab) = (tab)->tp_next) for ((win) = ((tab) == curtab) ? firstwin : (tab)->tp_firstwin; (win); (win) = (win)->w_next)
 ...
 sign_mark_adjust(line1, line2, amount, amount_after);
    ...
    { lp = &(curwin->w_pcmark.lnum); if (*lp >= line1 && *lp <= line2) { if (amount == (0x7fffffffL)) *lp = 0; else *lp += amount; } else if (amount_after && *lp > line2) *lp += amount_after; };


    { lp = &(curwin->w_prev_pcmark.lnum); if (*lp >= line1 && *lp <= line2) { if (amount == (0x7fffffffL)) *lp = 0; else *lp += amount; } else if (amount_after && *lp > line2) *lp += amount_after; };


    if (saved_cursor.lnum != 0)
 { lp = &(saved_cursor.lnum); if (*lp >= line1 && *lp <= line2) { if (amount == (0x7fffffffL)) *lp = line1; else *lp += amount; } else if (amount_after && *lp > line2) *lp += amount_after; };
 ...
 if (!cmdmod.lockmarks)
  ...
  if (win->w_jumplist[i].fmark.fnum == fnum)
      { lp = &(win->w_jumplist[i].fmark.mark.lnum); if (*lp >= line1 && *lp <= line2) { if (amount == (0x7fffffffL)) *lp = line1; else *lp += amount; } else if (amount_after && *lp > line2) *lp += amount_after; };
 ...
 if (win->w_buffer == curbuf)
 {
  ...
  for (i = 0; i < win->w_tagstacklen; i++)
   ...
   { lp = &(win->w_tagstack[i].fmark.mark.lnum); if (*lp >= line1 && *lp <= line2) { if (amount == (0x7fffffffL)) *lp = line1; else *lp += amount; } else if (amount_after && *lp > line2) *lp += amount_after; };



     if (win->w_old_cursor_lnum != 0)
     {
  { lp = &(win->w_old_cursor_lnum); if (*lp >= line1 && *lp <= line2) { if (amount == (0x7fffffffL)) *lp = line1; else *lp += amount; } else if (amount_after && *lp > line2) *lp += amount_after; };
  { lp = &(win->w_old_visual_lnum); if (*lp >= line1 && *lp <= line2) { if (amount == (0x7fffffffL)) *lp = line1; else *lp += amount; } else if (amount_after && *lp > line2) *lp += amount_after; };
     }
    ...
    long col_amount;
{
 ...
 return;
    ...
    for (i = 0; i < ('z' - 'a' + 1); i++)
    {
 { posp = &(curbuf->b_namedm[i]); if (posp->lnum == lnum && posp->col >= mincol) { posp->lnum += lnum_amount; if (col_amount < 0 && posp->col <= (colnr_T)-col_amount) posp->col = 0; else posp->col += col_amount; } };
 if (namedfm[i].fmark.fnum == fnum)
     { posp = &(namedfm[i].fmark.mark); if (posp->lnum == lnum && posp->col >= mincol) { posp->lnum += lnum_amount; if (col_amount < 0 && posp->col <= (colnr_T)-col_amount) posp->col = 0; else posp->col += col_amount; } };
    }
    for (i = ('z' - 'a' + 1); i < ('z' - 'a' + 1) + 10; i++)
    {
 if (namedfm[i].fmark.fnum == fnum)
    ...
    { posp = &(curbuf->b_last_insert); if (posp->lnum == lnum && posp->col >= mincol) { posp->lnum += lnum_amount; if (col_amount < 0 && posp->col <= (colnr_T)-col_amount) posp->col = 0; else posp->col += col_amount; } };


    { posp = &(curbuf->b_last_change); if (posp->lnum == lnum && posp->col >= mincol) { posp->lnum += lnum_amount; if (col_amount < 0 && posp->col <= (colnr_T)-col_amount) posp->col = 0; else posp->col += col_amount; } };



    for (i = 0; i < curbuf->b_changelistlen; ++i)
 { posp = &(curbuf->b_changelist[i]); if (posp->lnum == lnum && posp->col >= mincol) { posp->lnum += lnum_amount; if (col_amount < 0 && posp->col <= (colnr_T)-col_amount) posp->col = 0; else posp->col += col_amount; } };




    { posp = &(curbuf->b_visual.vi_start); if (posp->lnum == lnum && posp->col >= mincol) { posp->lnum += lnum_amount; if (col_amount < 0 && posp->col <= (colnr_T)-col_amount) posp->col = 0; else posp->col += col_amount; } };
    { posp = &(curbuf->b_visual.vi_end); if (posp->lnum == lnum && posp->col >= mincol) { posp->lnum += lnum_amount; if (col_amount < 0 && posp->col <= (colnr_T)-col_amount) posp->col = 0; else posp->col += col_amount; } };



    { posp = &(curwin->w_pcmark); if (posp->lnum == lnum && posp->col >= mincol) { posp->lnum += lnum_amount; if (col_amount < 0 && posp->col <= (colnr_T)-col_amount) posp->col = 0; else posp->col += col_amount; } };


    { posp = &(curwin->w_prev_pcmark); if (posp->lnum == lnum && posp->col >= mincol) { posp->lnum += lnum_amount; if (col_amount < 0 && posp->col <= (colnr_T)-col_amount) posp->col = 0; else posp->col += col_amount; } };


    { posp = &saved_cursor; if (posp->lnum == lnum && posp->col >= mincol) { posp->lnum += lnum_amount; if (col_amount < 0 && posp->col <= (colnr_T)-col_amount) posp->col = 0; else posp->col += col_amount; } };
 ...
 for (i = 0; i < win->w_jumplistlen; ++i)
     if (win->w_jumplist[i].fmark.fnum == fnum)
  { posp = &(win->w_jumplist[i].fmark.mark); if (posp->lnum == lnum && posp->col >= mincol) { posp->lnum += lnum_amount; if (col_amount < 0 && posp->col <= (colnr_T)-col_amount) posp->col = 0; else posp->col += col_amount; } };
 ...
 if (win->w_buffer == curbuf)
 {
     ...
     for (i = 0; i < win->w_tagstacklen; i++)
  if (win->w_tagstack[i].fmark.fnum == fnum)
      { posp = &(win->w_tagstack[i].fmark.mark); if (posp->lnum == lnum && posp->col >= mincol) { posp->lnum += lnum_amount; if (col_amount < 0 && posp->col <= (colnr_T)-col_amount) posp->col = 0; else posp->col += col_amount; } };


     if (win != curwin)
  { posp = &win->w_cursor; if (posp->lnum == lnum && posp->col >= mincol) { posp->lnum += lnum_amount; if (col_amount < 0 && posp->col <= (colnr_T)-col_amount) posp->col = 0; else posp->col += col_amount; } };
 }
    }
}
    ...
    int force;
{
     ...
     *str <= 127 &&
     ...
     ((*virp->vir_line == '\'' && (((*str) >= '0' && (*str) <= '9') || isupper(*str)))
      || (*virp->vir_line == '-' && *str == '\'')))
    {
 if (*str == '\'')
 {
    ...
    int flags;
{
 ...
 if (line[0] != '>')
 {
     if (line[0] != '\n' && line[0] != '\r' && line[0] != '#')
     {
  if (viminfo_error("E576: ", ((char *)("Missing '>'")), line))
      break;
     }
 ...
 while (p != str && (*p == '\000' || vim_isspace(*p)))
     p--;
 if (*p)
     p++;
 *p = '\000';
 ...
 if (fp_out == ((void *)0))
 {
  ...
  if (strcasecmp((char *)(((str))), (char *)(((name_buf)))) == 0)
      load_marks = 1;
     }
 }
 else
