...
gui_init_check()
{
    ...
    {
 if (result == 0)
     emsg((char_u *)(((char *)("E229: Cannot start the GUI"))));
 return result;
    }
...
gui_init()
{
 ...
 {
     if (strcmp((char *)(use_gvimrc), (char *)("NONE")) != 0
      && strcmp((char *)(use_gvimrc), (char *)("NORC")) != 0
      && do_source(use_gvimrc, 0, 0) != 1)
  emsg2((char_u *)(((char *)("E230: Cannot read from \"%s\""))), (char_u *)(use_gvimrc));
 }
 else
 {
     if (process_env((char_u *)"GVIMINIT", 0) == 0
   && do_source((char_u *)USR_GVIMRC_FILE, 1,
         2) == 0
  ...
  if ( fullpathcmp((char_u *)USR_GVIMRC_FILE,
         (char_u *)GVIMRC_FILE, 0) != 1
   )
      do_source((char_u *)GVIMRC_FILE, 1, 2);
 ...
 set_option_value((char_u *)"gfn", 0L, (char_u *)font_argument, 0);
    if (
     ...
     gui_init_font(*p_guifont == '\000' ? hl_get_font_name()
        : p_guifont, 0) == 0)
    {
    ...
    int clear_selection;
{
 ...
 if (!screen_cleared || ScreenLines == ((void *)0))
     return;
 ...
 if (aep != ((void *)0))
 {
     attr = aep->ae_attr;
     if (cfg == INVALCOLOR)
  cfg = ((attr & 0x01) ? aep->ae_u.gui.bg_color
         : aep->ae_u.gui.fg_color);
     if (cbg == INVALCOLOR)
  cbg = ((attr & 0x01) ? aep->ae_u.gui.fg_color
         : aep->ae_u.gui.bg_color);
 }
 if (cfg == INVALCOLOR)
     cfg = (attr & 0x01) ? gui.back_pixel : gui.norm_pixel;
 if (cbg == INVALCOLOR)
     cbg = (attr & 0x01) ? gui.norm_pixel : gui.back_pixel;
 attr &= ~0x01;
 if (cattr & 0x01)
 {
     cc = cbg;
    ...
    int direction;
{
 ...
 gui_mch_update();
 if (gui_mch_get_winpos(&x, &y) == 1 && (x < 0 || y < 0))
    ...
    int back;
{
 ...
 highlight_mask = gui.highlight_mask;
    ...
    if (aep != ((void *)0) && aep->ae_u.gui.font != NOFONT)
 ...
 draw_flags |= DRAW_BOLD;
    ...
    if (back != 0 && ((draw_flags & DRAW_BOLD) || (highlight_mask & 0x04)))
 return 0;
    if ((hl_mask_todo & 0x08)
    ...
    long wtime;
{
 ...
 return retval;
    ...
    if (gui_mch_wait_for_chars(p_ut) == 1)
 retval = 1;
...
gui_remove_scrollbars()
{
    ...
    {
 if (i == SBAR_BOTTOM)
     gui_mch_enable_scrollbar(&gui.bottom_sbar, 0);
 else
 {
     for (wp = firstwin; wp != ((void *)0); wp = wp->w_next)
     {
    ...
    int still_dragging;
{
 ...
 value = sb->max - sb->size + 1;
    ...
    sb->value = value;
    if ((sb->wp == ((void *)0) || sb->wp == curwin) && pum_visible())
 return;
    if (sb->wp != ((void *)0))
    {
 sb_num = 0;

 for (wp = firstwin; wp != sb->wp && wp != ((void *)0); wp = wp->w_next)
     sb_num++;
 if (wp == ((void *)0))
     return;
 bytes[0] = 0x9b;
    ...
    int force;
{
 ...
 if (size < 1 || wp->w_botline - 2 > max)
 {
     ...
     sb->height = 0;
     if (gui.which_scrollbars[SBAR_LEFT])
  gui_do_scrollbar(wp, SBAR_LEFT, 0);
     if (gui.which_scrollbars[SBAR_RIGHT])
  gui_do_scrollbar(wp, SBAR_RIGHT, 0);
     continue;
 }
...
gui_do_scroll()
{
    ...
    for (wp = firstwin, i = 0; i < current_scrollbar; wp = ((wp)->w_next), i++)
 if (wp == ((void *)0))
     break;
    if (wp == ((void *)0))
 ...
 pum_redraw();
    ...
    return (wp == curwin && !(((curwin->w_cursor).lnum == (old_cursor).lnum) && ((curwin->w_cursor).col == (old_cursor).col) && ((curwin->w_cursor).coladd == (old_cursor).coladd)));
}
    static colnr_T
    ...
    int force;
{
 ...
 return;
    ...
    if (!force && curwin->w_onebuf_opt.wo_wrap && gui.prev_wrap)
 return;
    ...
    char_u *name;
{
 ...
 emsg2((char_u *)(((char *)("E254: Cannot allocate color %s"))), (char_u *)(name));
    return t;
}
    ...
    int y;
{
 ...
 fill_mouse_coord(st + 4,
  ...
  wp->w_wincol == 0 ? -1 : wp->w_wincol + 10000,
...
gui_mouse_correct()
{
 ...
 wp = xy2win(x, y);
    ...
    {
 validate_cline_row();
 gui_mch_setmouse((int)(curwin->w_wincol + curwin->w_width) * gui.char_width - 3,
  ((curwin->w_winrow) + curwin->w_wrow) * gui.char_height
           + (gui.char_height) / 2);
    }
}
