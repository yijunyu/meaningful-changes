    ...
    char **argv;
{
    ...
    ((__builtin_object_size ((&params), 0) != (size_t) -1) ? __builtin___memset_chk ((&params), (0), (sizeof(params)), __builtin_object_size ((&params), 0)) : __inline_memset_chk ((&params), (0), (sizeof(params))));
    params.argc = argc;
    params.argv = argv;
...
# 240 "main.c"
    init_normal_cmds();
# 250 "main.c"
    if ((IObuff = alloc((1024+1))) == ((void *)0)
     || (NameBuff = alloc(1024)) == ((void *)0))
 mch_exit(0);
...
# 512 "main.c"
 ...
 if (diffopt_horizontal())
     params.window_layout = 1;
 else
     params.window_layout = 2;
    }
...
# 742 "main.c"
 ...
 read_stdin();
    ...
    if ((did_emsg || msg_didout) && *(term_strings[(int)(KS_TI)]) != '\000')
 newline_on_exit = 1;
# 760 "main.c"
    settmode(2);
    ...
    int exitval;
{
 ...
 for (tp = first_tabpage; tp != ((void *)0); tp = next_tp)
 {
     next_tp = tp->tp_next;
     for (wp = (tp == curtab)
      ? firstwin : tp->tp_firstwin; wp != ((void *)0); wp = wp->w_next)
     {
  buf = wp->w_buffer;
    ...
    mparm_T *parmp;
{
    ...
    if (strcmp((char *)(initstr), (char *)(parmp->argv[0])) == 0)
 disallow_gui = 1;
 ...
 ++initstr;
    ...
    if ((((initstr[0]) < 'A' || (initstr[0]) > 'Z') ? (initstr[0]) : (initstr[0]) + ('a' - 'A')) == 'e'
     && ((((initstr[1]) < 'A' || (initstr[1]) > 'Z') ? (initstr[1]) : (initstr[1]) + ('a' - 'A')) == 'v'
  || (((initstr[1]) < 'A' || (initstr[1]) > 'Z') ? (initstr[1]) : (initstr[1]) + ('a' - 'A')) == 'g'))
    {
 ...
 initstr += 4;
    ...
    else if (strncasecmp((char *)(initstr), (char *)("vim"), (size_t)(3)) == 0)
 initstr += 3;
 ...
 parmp->diff_mode = 1;
    ...
    {
 if (strncasecmp((char *)(initstr + 2), (char *)("im"), (size_t)(2)) == 0)
     exmode_active = 2;
 else
     exmode_active = 1;
 change_compatible(1);
...
# 1971 "main.c"
  ...
  case 's':
      ...
      {
scripterror:
   fprintf(__stderrp, "%s", (((char *)("Attempt to open script file again: \""))));
...
# 2248 "main.c"
 ...
 else
 {
  ...
  mainerr(1, (char_u *)argv[0]);
     parmp->edit_type = 1;
# 2314 "main.c"
     if (ga_grow(&global_alist.al_ga, 1) == 0
      || (p = vim_strsave((char_u *)argv[0])) == ((void *)0))
  mch_exit(2);

     if (parmp->diff_mode && mch_isdir(p) && (global_alist.al_ga.ga_len) > 0
          && !mch_isdir(alist_name(&((aentry_T *)global_alist.al_ga.ga_data)[0])))
     {
  char_u *r;
...
# 2448 "main.c"
 if (!parmp->stdout_isatty)
     fprintf(__stderrp, "%s", (((char *)("Vim: Warning: Output is not to a terminal\n"))));
 if (!input_isatty)
     fprintf(__stderrp, "%s", (((char *)("Vim: Warning: Input is not from a terminal\n"))));
 out_flush();
 if (scriptin[0] == ((void *)0))
     ui_delay(2000L, 1);
    ...
    mparm_T *parmp;
{
 ...
 else
 {
  ...
  emsg2((char_u *)(((char *)("E282: Cannot read from \"%s\""))), (char_u *)(parmp->use_vimrc));
 }
    }
    else if (!silent_mode)
    {
...
# 2900 "main.c"
 ...
 {
     if (do_source((char_u *)"$HOME/.vimrc", 1, 1) == 0
# 2911 "main.c"
  && process_env((char_u *)"EXINIT", 0) == 0
  && do_source((char_u *)"$HOME/.exrc", 0, 0) == 0)
     {
...
# 2929 "main.c"
 if (p_exrc)
 {
  ...
  secure = p_secure;
     ...
     i = 0;
     if (fullpathcmp((char_u *)"$HOME/.vimrc",
          (char_u *)".vimrc", 0) != 1
# 2949 "main.c"
      && fullpathcmp((char_u *)"$VIM/vimrc",
          (char_u *)".vimrc", 0) != 1

    )
  i = do_source((char_u *)".vimrc", 1, 1);
  ...
  if ( fullpathcmp((char_u *)"$HOME/.exrc",
          (char_u *)".exrc", 0) != 1
    ...
    char *fname;
{
    ...
    return !(stat((fname), (&s)) != 0 || s.st_uid != uid

     || lstat((fname), (&s)) != 0 || s.st_uid != uid
