...
static void find_endpos (int idx, lpos_T * startpos, lpos_T * m_endpos, lpos_T * hl_endpos, long * flagsp, lpos_T * end_endpos, int * end_idx, reg_extmatch_T *
  start_ext);
    ...
    linenr_T lnum;
{
    ...
    syn_win = wp;
    syn_stack_alloc ();
    if (syn_block -> b_sst_array == ((void *) 0)) return;
    syn_block -> b_sst_lasttick = display_tick;
    if (((&current_state) -> ga_itemsize != 0) && current_lnum < lnum && current_lnum < syn_buf -> b_ml.ml_line_count) {
        (void) syn_finish_line (0);
        if (!current_state_stored) {
            ++current_lnum;
            (void) store_current_state ();
        }
        if (current_lnum != lnum) invalidate_current_state ();
    }
    else invalidate_current_state ();
    if (((&current_state) -> ga_itemsize == 0) && syn_block -> b_sst_array != ((void *) 0)) {
        for (p = syn_block -> b_sst_first; p != ((void *) 0); p = p -> sst_next) {
            if (p -> sst_lnum > lnum) break;
            if (p -> sst_lnum <= lnum && p -> sst_change_lnum == 0) {
                last_valid = p;
                if (p -> sst_lnum >= lnum - syn_block -> b_syn_sync_minlines) last_min_valid = p;
            }
        }
        if (last_min_valid != ((void *) 0)) load_current_state (last_min_valid);
    }
    if (((&current_state) -> ga_itemsize == 0)) {
        syn_sync (wp, lnum, last_valid);
        first_stored = current_lnum + syn_block -> b_syn_sync_minlines;
    }
    else first_stored = current_lnum;
    if (syn_block -> b_sst_len <= Rows) dist = 999999;
    else dist = syn_buf -> b_ml.ml_line_count / (syn_block -> b_sst_len - Rows) + 1;
    while (current_lnum < lnum) {
        syn_start_line ();
        (void) syn_finish_line (0);
        ++current_lnum;
        if (current_lnum >= first_stored) {
            if (prev == ((void *) 0)) prev = syn_stack_find_entry (current_lnum - 1);
            if (prev == ((void *) 0)) sp = syn_block -> b_sst_first;
            else sp = prev;
            while (sp != ((void *) 0) && sp -> sst_lnum < current_lnum) sp = sp -> sst_next;
            if (sp != ((void *) 0) && sp -> sst_lnum == current_lnum && syn_stack_equal (sp)) {
                parsed_lnum = current_lnum;
                prev = sp;
                while (sp != ((void *) 0) && sp -> sst_change_lnum <= parsed_lnum) {
                    if (sp -> sst_lnum <= lnum) prev = sp;
                    else if (sp -> sst_change_lnum == 0) break;
                    sp -> sst_change_lnum = 0;
                    sp = sp -> sst_next;
                }
                load_current_state (prev);
            }
            else if (prev == ((void *) 0) || current_lnum == lnum || current_lnum >= prev -> sst_lnum + dist) prev = store_current_state ();
        }
        line_breakcheck ();
        if (got_int) {
            current_lnum = lnum;
    ...
    synstate_T * p;
{
    ...
    if (p -> sst_stacksize > 7) {
        ...
        for (i = 0; i < gap -> ga_len; i ++) unref_extmatch (((bufstate_T *) ((gap) -> ga_data)) [i].bs_extmatch);
        ga_clear (gap);
    }
    else {
        for (i = 0; i < p -> sst_stacksize; i ++) unref_extmatch (p -> sst_union.sst_stack [i].bs_extmatch);
    }
}
...
static void clear_current_state () {
    ...
    for (i = 0; i < current_state.ga_len; i ++) unref_extmatch (sip [i].si_extmatch);
    ...
    synstate_T * last_valid;
{
    ...
    invalidate_current_state ();
    if (syn_block -> b_syn_sync_minlines > start_lnum) start_lnum = 1;
    else {
        if (syn_block -> b_syn_sync_minlines == 1) lnum = 1;
        else if (syn_block -> b_syn_sync_minlines < 10) lnum = syn_block -> b_syn_sync_minlines * 2;
        else lnum = syn_block -> b_syn_sync_minlines * 3 / 2;
        if (syn_block -> b_syn_sync_maxlines != 0 && lnum > syn_block -> b_syn_sync_maxlines) lnum = syn_block -> b_syn_sync_maxlines;
        if (lnum >= start_lnum) start_lnum = 1;
        else start_lnum -= lnum;
    }
    ...
    if (syn_block -> b_syn_sync_flags & 0x01) {
        ...
        for (; start_lnum > 1; --start_lnum) {
            ...
            if (*line == '\000' || *(line+strlen((char*)(line))-1) != '\\') break;
        }
        current_lnum = start_lnum;
        ...
        if (find_start_comment ((int) syn_block -> b_syn_sync_maxlines) != ((void *) 0)) {
            for (idx = syn_block -> b_syn_patterns.ga_len; --idx >= 0;) if (((synpat_T *) ((syn_block) -> b_syn_patterns.ga_data)) [idx].sp_syn.id == syn_block
              -> b_syn_sync_id && ((synpat_T *) ((syn_block) -> b_syn_patterns.ga_data)) [idx].sp_type == 2) {
                validate_current_state ();
                if (push_current_state (idx) == 1) update_si_attr (current_state.ga_len - 1);
                break;
            }
        }
        ...
        if (syn_block -> b_syn_sync_maxlines != 0 && start_lnum > syn_block -> b_syn_sync_maxlines) break_lnum = start_lnum - syn_block -> b_syn_sync_maxlines;
        else break_lnum = 0;
        ...
        while (--lnum > break_lnum) {
            ...
            if (lnum > 1 && syn_match_linecont (lnum - 1)) continue;
            validate_current_state ();
                ...
                for (;;) {
                    ...
                    if (had_sync_point && current_state.ga_len) {
                        ...
                        if (found_m_endpos.lnum > current_lnum) {
                            ...
                            if (current_lnum >= end_lnum) break;
                        }
                        else if (found_m_endpos.col > current_col) current_col = found_m_endpos.col;
                        else ++current_col;
                        prev_current_col = current_col;
                        if (syn_getcurline () [current_col] != '\000') ++current_col;
                        check_state_ends ();
            ...
            if (found_flags) {
                clear_current_state ();
                if (found_match_idx >= 0 && push_current_state (found_match_idx) == 1) update_si_attr (current_state.ga_len - 1);
                if (found_flags & 0x10) {
...
static void syn_start_line () {
    current_finished = 0;
    current_col = 0;
    if (current_state.ga_len > 0) syn_update_ends (1);
    next_match_idx = -1;
    ++current_line_id;
}
    ...
    int startofline;
{
    ...
    if (startofline) {
        for (i = 0; i < current_state.ga_len; ++i) {
            ...
            if (cur_si -> si_idx >= 0 && (((synpat_T *) ((syn_block) -> b_syn_patterns.ga_data)) [cur_si -> si_idx]).sp_type == 1 && cur_si -> si_m_endpos.lnum
              < current_lnum) {
    ...
    if (keepend_level >= 0) for (; i > keepend_level; --i) if (((stateitem_T *) (current_state.ga_data)) [i].si_flags & 0x4000) break;
    seen_keepend = 0;
    for (; i < current_state.ga_len; ++i) {
        cur_si = &((stateitem_T*)(current_state.ga_data))[i];
        if ((cur_si -> si_flags & 0x400) || (seen_keepend && !startofline) || (i == current_state.ga_len - 1 && startofline)) {
            cur_si -> si_h_startpos.col = 0;
            cur_si -> si_h_startpos.lnum = current_lnum;
            if (!(cur_si->si_flags&0x8000)) update_si_end (cur_si, (int) current_col, !startofline);
            if (!startofline && (cur_si -> si_flags & 0x400)) seen_keepend = 1;
        }
    }
    check_keepend ();
    check_state_ends ();
    ...
    synblock_T * block;
{
    ...
    if (block -> b_sst_array != ((void *) 0)) {
        for (p = block -> b_sst_first; p != ((void *) 0); p = p -> sst_next) clear_syn_state (p);
    ...
    synblock_T * block;
{
    ...
    for (wp = firstwin; wp != ((void *) 0); wp = wp -> w_next) {
        if (wp -> w_s == block && foldmethodIsSyntax (wp)) foldUpdateAll (wp);
    }
}
...
static void syn_stack_alloc () {
    ...
    if (syn_block -> b_sst_len > len * 2 || syn_block -> b_sst_len < len) {
        ...
        if (syn_block -> b_sst_array != ((void *) 0)) {
            ...
            if (len < syn_block -> b_sst_len - syn_block -> b_sst_freecount + 2) len = syn_block -> b_sst_len - syn_block -> b_sst_freecount + 2;
        }
        sstp = (synstate_T *) alloc_clear ((unsigned) (len * sizeof (synstate_T)));
        if (sstp == ((void *) 0)) return;
        to = sstp - 1;
        if (syn_block -> b_sst_array != ((void *) 0)) {
            for (from = syn_block -> b_sst_first; from != ((void *) 0); from = from -> sst_next) {
                ++to;
                * to = *from;
                to -> sst_next = to + 1;
            }
        ...
        while (++to < sstp + len) to -> sst_next = to + 1;
    ...
    buf_T * buf;
{
    ...
    for (wp = firstwin; wp != ((void *) 0); wp = wp -> w_next) {
        if ((wp -> w_buffer == buf) && (wp -> w_s != &buf->b_s)) syn_stack_apply_changes_block (wp -> w_s, buf);
    }
}
    ...
    buf_T * buf;
{
    ...
    linenr_T n;
    if (block -> b_sst_array == ((void *) 0)) return;
    prev = ((void *) 0);
    for (p = block -> b_sst_first; p != ((void *) 0);) {
        if (p -> sst_lnum + block -> b_syn_sync_linebreaks > buf -> b_mod_top) {
            ...
            if (n <= buf -> b_mod_bot) {
                np = p -> sst_next;
                if (prev == ((void *) 0)) block -> b_sst_first = np;
                else prev -> sst_next = np;
                syn_stack_free_entry (block, p);
                p = np;
                continue;
            }
            if (p -> sst_change_lnum != 0 && p -> sst_change_lnum > buf -> b_mod_top) {
                if (p -> sst_change_lnum + buf -> b_mod_xlines > buf -> b_mod_top) p -> sst_change_lnum += buf -> b_mod_xlines;
                else p -> sst_change_lnum = buf -> b_mod_top;
            }
            if (p -> sst_change_lnum == 0 || p -> sst_change_lnum < buf -> b_mod_bot) p -> sst_change_lnum = buf -> b_mod_bot;
            p -> sst_lnum = n;
        }
        prev = p;
        p = p -> sst_next;
...
static int syn_stack_cleanup () {
    ...
    int dist;
    int retval = 0;
    if (syn_block -> b_sst_array == ((void *) 0) || syn_block -> b_sst_first == ((void *) 0)) return retval;
    if (syn_block -> b_sst_len <= Rows) dist = 999999;
    else dist = syn_buf -> b_ml.ml_line_count / (syn_block -> b_sst_len - Rows) + 1;
    ...
    for (p = prev -> sst_next; p != ((void *) 0); prev = p, p = p -> sst_next) {
        if (prev -> sst_lnum + dist > p -> sst_lnum) {
            if (p -> sst_tick > syn_block -> b_sst_lasttick) {
                if (!above || p -> sst_tick < tick) tick = p -> sst_tick;
                above = 1;
            }
            else if (!above && p -> sst_tick < tick) tick = p -> sst_tick;
        }
    }
    prev = syn_block -> b_sst_first;
    for (p = prev -> sst_next; p != ((void *) 0); prev = p, p = p -> sst_next) {
...
static synstate_T * store_current_state () {
    ...
    for (i = current_state.ga_len - 1; i >= 0; --i) {
        cur_si = &((stateitem_T*)(current_state.ga_data))[i];
        if (cur_si -> si_h_startpos.lnum >= current_lnum || cur_si -> si_m_endpos.lnum >= current_lnum || cur_si -> si_h_endpos.lnum >= current_lnum || (cur_si
          -> si_end_idx && cur_si -> si_eoe_pos.lnum >= current_lnum)) break;
    }
    if (i >= 0) {
        if (sp != ((void *) 0)) {
            if (syn_block -> b_sst_first == sp) syn_block -> b_sst_first = sp -> sst_next;
            else {
                for (p = syn_block -> b_sst_first; p != ((void *) 0); p = p -> sst_next) if (p -> sst_next == sp) break;
                if (p != ((void *) 0)) p -> sst_next = sp -> sst_next;
            }
        ...
        if (syn_block -> b_sst_freecount == 0) sp = ((void *) 0);
        else {
    ...
    if (sp != ((void *) 0)) {
        ...
        if (current_state.ga_len > 7) {
            ...
            if (ga_grow (&sp->sst_union.sst_ga, current_state.ga_len) == 0) sp -> sst_stacksize = 0;
            else sp -> sst_union.sst_ga.ga_len = current_state.ga_len;
            bp = ((bufstate_T *) ((&(sp->sst_union.sst_ga)) -> ga_data));
        }
        else bp = sp -> sst_union.sst_stack;
    ...
    synstate_T * from;
{
    ...
    validate_current_state ();
    keepend_level = -1;
    if (from -> sst_stacksize && ga_grow (&current_state, from -> sst_stacksize) != 0) {
        if (from -> sst_stacksize > 7) bp = ((bufstate_T *) ((&(from->sst_union.sst_ga)) -> ga_data));
        else bp = from -> sst_union.sst_stack;
        for (i = 0; i < from -> sst_stacksize; ++i) {
            ...
            if (keepend_level < 0 && (((stateitem_T *) (current_state.ga_data)) [i].si_flags & 0x400)) keepend_level = i;
            ((stateitem_T *) (current_state.ga_data)) [i].si_ends = 0;
            ((stateitem_T *) (current_state.ga_data)) [i].si_m_lnum = 0;
            if (((stateitem_T *) (current_state.ga_data)) [i].si_idx >= 0) ((stateitem_T *) (current_state.ga_data)) [i].si_next_list = (((synpat_T *) ((
              syn_block) -> b_syn_patterns.ga_data)) [((stateitem_T *) (current_state.ga_data)) [i].si_idx]).sp_next_list;
            else ((stateitem_T *) (current_state.ga_data)) [i].si_next_list = ((void *) 0);
            update_si_attr (i);
        }
    ...
    synstate_T * sp;
{
    ...
    if (sp -> sst_stacksize == current_state.ga_len && sp -> sst_next_list == current_next_list) {
        if (sp -> sst_stacksize > 7) bp = ((bufstate_T *) ((&(sp->sst_union.sst_ga)) -> ga_data));
        else bp = sp -> sst_union.sst_stack;
        for (i = current_state.ga_len; --i >= 0;) {
            if (bp [i].bs_idx != ((stateitem_T *) (current_state.ga_data)) [i].si_idx) break;
            if (bp [i].bs_extmatch != ((stateitem_T *) (current_state.ga_data)) [i].si_extmatch) {
                ...
                if (bsx == ((void *) 0) || six == ((void *) 0)) break;
                for (j = 0; j < 10; ++j) {
                    if (bsx -> matches [j] != six -> matches [j]) {
                        if (bsx -> matches [j] == ((void *) 0) || six -> matches [j] == ((void *) 0)) break;
                        if ((((synpat_T *) ((syn_block) -> b_syn_patterns.ga_data)) [((stateitem_T *) (current_state.ga_data)) [i].si_idx]).sp_ic ? strcasecmp (
                          (char *) ((bsx -> matches [j])), (char *) ((six -> matches [j]))) != 0 : strcmp ((char *) (bsx -> matches [j]), (char *) (six ->
                          matches [j])) != 0) break;
                    }
                }
                if (j != 10) break;
    ...
    linenr_T lnum;
{
    ...
    if (sp != ((void *) 0) && sp -> sst_lnum < lnum) sp = sp -> sst_next;
    if (sp != ((void *) 0) && sp -> sst_change_lnum != 0) sp -> sst_change_lnum = lnum;
}
    ...
    linenr_T lnum;
{
    ...
    if (((&current_state) -> ga_itemsize != 0) && lnum == current_lnum + 1) {
        ...
        if (sp != ((void *) 0) && sp -> sst_lnum == lnum) {
            (void) syn_finish_line (0);
            if (syn_stack_equal (sp)) retval = 0;
            ++current_lnum;
            (void) store_current_state ();
        }
    }
    ...
    if (!current_finished) {
        while (!current_finished) {
            ...
            if (syncing && current_state.ga_len) {
                cur_si = &((stateitem_T*)(current_state.ga_data))[current_state.ga_len-1];
                if (cur_si -> si_idx >= 0 && (((synpat_T *) ((syn_block) -> b_syn_patterns.ga_data)) [cur_si -> si_idx].sp_flags & (0x10 | 0x20))) return 1;
                prev_current_col = current_col;
                if (syn_getcurline () [current_col] != '\000') ++current_col;
                check_state_ends ();
                current_col = prev_current_col;
...
{
    int attr = 0;
    if (can_spell != ((void *) 0)) *can_spell = syn_block -> b_syn_spell == 0 ? (syn_block -> b_spell_cluster_id == 0) : (syn_block -> b_syn_spell == 1);
    if (syn_block -> b_sst_array == ((void *) 0)) return 0;
    ...
    if (((&current_state) -> ga_itemsize == 0)) validate_current_state ();
    while (current_col <= col) {
        attr = syn_current_attr (0, 1, can_spell, current_col == col ? keep_state : 0);
        ++current_col;
    }
    return attr;
}
    ...
    int keep_state;
{
    ...
    if (line [current_col] == '\000' && current_col != 0) {
        if (next_match_idx >= 0 && next_match_col >= (int) current_col && next_match_col != (0x7fffffffL)) (void) push_next_match (((void *) 0));
        current_finished = 1;
    ...
    do {
        ...
        keep_next_list = 0;
        syn_id = 0;
        if (current_state.ga_len) cur_si = &((stateitem_T*)(current_state.ga_data))[current_state.ga_len-1];
        else cur_si = ((void *) 0);
        if (syn_block -> b_syn_containedin || cur_si == ((void *) 0) || cur_si -> si_cont_list != ((void *) 0)) {
            if (do_keywords) {
                line = syn_getcurline ();
                if (vim_iswordc_buf (line + current_col, syn_buf) && (current_col == 0 || !vim_iswordc_buf(line+current_col-1,syn_buf))) {
                    ...
                    if (syn_id != 0) {
                        if (push_current_state (-1) == 1) {
                            ...
                            else cur_si -> si_attr = syn_id2attr (syn_id);
                            cur_si -> si_cont_list = ((void *) 0);
                            cur_si -> si_next_list = next_list;
                            check_keepend ();
                        }
                        else vim_free (next_list);
                    }
                }
            }
            if (syn_id == 0 && syn_block -> b_syn_patterns.ga_len) {
                if (next_match_idx < 0 || next_match_col < (int) current_col) {
                    ...
                    for (idx = syn_block -> b_syn_patterns.ga_len; --idx >= 0;) {
                        ...
                        if (spp -> sp_syncing == syncing && (displaying || !(spp->sp_flags&0x1000)) && (spp -> sp_type == 1 || spp -> sp_type == 2) && (
                          current_next_list != ((void *) 0) ? in_id_list (((void *) 0), current_next_list, &spp->sp_syn, 0) : (cur_si == ((void *) 0) ? 
                          !(spp->sp_flags&0x01) : in_id_list (cur_si, cur_si -> si_cont_list, &spp->sp_syn, spp -> sp_flags & 0x01)))) {
                            if (spp -> sp_line_id == current_line_id && spp -> sp_startcol >= next_match_col) continue;
                            spp -> sp_line_id = current_line_id;
                            lc_col = current_col - spp -> sp_offsets [6];
                            if (lc_col < 0) lc_col = 0;
                            regmatch.rmm_ic = spp -> sp_ic;
                            regmatch.regprog = spp -> sp_prog;
                            if (!syn_regexec(&regmatch, current_lnum, (colnr_T) lc_col)) {
                                spp -> sp_startcol = (0x7fffffffL);
                                continue;
                            }
                            syn_add_start_off (&pos, &regmatch, spp, 0, -1);
                            ...
                            if (startcol >= next_match_col) continue;
                            ...
                            syn_add_start_off (&hl_startpos, &regmatch, spp, 2, -1);
                            syn_add_end_off (&eos_pos, &regmatch, spp, 4, 0);
                            ...
                            if (spp -> sp_type == 2 && (spp -> sp_flags & 0x04)) {
                                ...
                                if (endpos.lnum == 0) continue;
                            }
                            else if (spp -> sp_type == 1) {
                                syn_add_end_off (&hl_endpos, &regmatch, spp, 3, 0);
                                syn_add_end_off (&endpos, &regmatch, spp, 1, 0);
                                if (endpos.lnum == current_lnum && (int) endpos.col + syncing < startcol) {
                                    if (regmatch.startpos [0].col == regmatch.endpos [0].col) try_next_column = 1;
                                    continue;
                                }
                            }
                            if (hl_startpos.lnum == current_lnum && (int) hl_startpos.col < startcol) hl_startpos.col = startcol;
                ...
                if (next_match_idx >= 0 && next_match_col == (int) current_col) {
                    ...
                    if (next_match_m_endpos.lnum == current_lnum && next_match_m_endpos.col == current_col && lspp -> sp_next_list != ((void *) 0)) {
                    ...
                    else cur_si = push_next_match (cur_si);
                    found_match = 1;
                }
            }
        }
        if (current_next_list != ((void *) 0) && !keep_next_list) {
            if (!found_match) {
                line = syn_getcurline ();
                if (((current_next_flags & 0x100) && ((line [current_col]) == ' ' || (line [current_col]) == '\t')) || ((current_next_flags & 0x200) && *line ==
                  '\000')) break;
            }
            current_next_list = ((void *) 0);
            next_match_idx = -1;
            if (!zero_width_next_list) found_match = 1;
        }
    }
    while (found_match);
    current_attr = 0;
    ...
    if (cur_si != ((void *) 0)) {
        for (idx = current_state.ga_len - 1; idx >= 0; --idx) {
            ...
            if ((current_lnum > sip -> si_h_startpos.lnum || (current_lnum == sip -> si_h_startpos.lnum && current_col >= sip -> si_h_startpos.col)) && (sip ->
              si_h_endpos.lnum == 0 || current_lnum < sip -> si_h_endpos.lnum || (current_lnum == sip -> si_h_endpos.lnum && current_col < sip -> si_h_endpos.
              col))) {
        ...
        if (can_spell != ((void *) 0)) {
            ...
            if (syn_block -> b_spell_cluster_id == 0) {
                if (syn_block -> b_nospell_cluster_id == 0 || current_trans_id == 0) *can_spell = (syn_block -> b_syn_spell != 2);
                else {
                    sps.inc_tag = 0;
                ...
                if (current_trans_id == 0) *can_spell = (syn_block -> b_syn_spell == 1);
                else {
                    sps.inc_tag = 0;
                    ...
                    * can_spell = in_id_list (sip, sip -> si_cont_list, &sps, 0);
                    if (syn_block -> b_nospell_cluster_id != 0) {
                        sps.id = syn_block -> b_nospell_cluster_id;
                        if (in_id_list (sip, sip -> si_cont_list, &sps, 0)) *can_spell = 0;
                    }
                }
            }
        }
        if (!syncing && !keep_state) {
            check_state_ends ();
            if (current_state.ga_len > 0 && syn_getcurline () [current_col] != '\000') {
                ++current_col;
                check_state_ends ();
                --current_col;
            }
    ...
    else if (can_spell != ((void *) 0)) *can_spell = syn_block -> b_syn_spell == 0 ? (syn_block -> b_spell_cluster_id == 0) : (syn_block -> b_syn_spell == 1);
    if (current_next_list != ((void *) 0) && syn_getcurline () [current_col + 1] == '\000' && !(current_next_flags&(0x80|0x200))) current_next_list = ((void *)
      0);
    if (zero_width_next_ga.ga_len > 0) ga_clear (&zero_width_next_ga);
...
{
    int i;
    for (i = current_state.ga_len; --i >= 0;) if (((stateitem_T *) (current_state.ga_data)) [i].si_m_startcol == (int) current_col && ((stateitem_T *) (
      current_state.ga_data)) [i].si_m_lnum == (int) current_lnum && ((stateitem_T *) (current_state.ga_data)) [i].si_idx == idx) return 1;
    for (i = gap -> ga_len; --i >= 0;) if (((int *) (gap -> ga_data)) [i] == idx) return 1;
    return 0;
}
    ...
    stateitem_T * cur_si;
{
    ...
    if (push_current_state (next_match_idx) == 1) {
        ...
        if (keepend_level < 0 && (cur_si -> si_flags & 0x400)) keepend_level = current_state.ga_len - 1;
        check_keepend ();
        update_si_attr (current_state.ga_len - 1);
        if (spp -> sp_type == 2 && spp -> sp_syn_match_id != 0 && push_current_state (next_match_idx) == 1) {
            cur_si = &((stateitem_T*)(current_state.ga_data))[current_state.ga_len-1];
            cur_si -> si_h_startpos = next_match_h_startpos;
            cur_si -> si_m_startcol = current_col;
            cur_si -> si_m_lnum = current_lnum;
...
static void check_state_ends () {
    ...
    cur_si = &((stateitem_T*)(current_state.ga_data))[current_state.ga_len-1];
    for (;;) {
        if (cur_si -> si_ends && (cur_si -> si_m_endpos.lnum < current_lnum || (cur_si -> si_m_endpos.lnum == current_lnum && cur_si -> si_m_endpos.col <=
          current_col))) {
            if (cur_si -> si_end_idx && (cur_si -> si_eoe_pos.lnum > current_lnum || (cur_si -> si_eoe_pos.lnum == current_lnum && cur_si -> si_eoe_pos.col >
              current_col))) {
                ...
                if (!(current_next_flags&(0x80|0x200)) && syn_getcurline () [current_col] == '\000') current_next_list = ((void *) 0);
                if (cur_si -> si_flags & 0x4000) had_extend = 1;
                pop_current_state ();
                if (current_state.ga_len == 0) break;
                if (had_extend && keepend_level >= 0) {
                    syn_update_ends (0);
                    if (current_state.ga_len == 0) break;
                }
                cur_si = &((stateitem_T*)(current_state.ga_data))[current_state.ga_len-1];
                if (cur_si -> si_idx >= 0 && ((synpat_T *) ((syn_block) -> b_syn_patterns.ga_data)) [cur_si -> si_idx].sp_type == 2 && 
                  !(cur_si->si_flags&(0x40|0x400))) {
                    update_si_end (cur_si, (int) current_col, 1);
                    check_keepend ();
                    if ((current_next_flags & 0x08) && keepend_level < 0 && syn_getcurline () [current_col] == '\000') break;
                }
            }
        }
        else break;
    ...
    int idx;
{
    ...
    synpat_T * spp;
    if (sip -> si_idx < 0) return;
    spp = &(((synpat_T*)((syn_block)->b_syn_patterns.ga_data))[sip->si_idx]);
    if (sip -> si_flags & 0x40) sip -> si_id = spp -> sp_syn_match_id;
    else sip -> si_id = spp -> sp_syn.id;
    sip -> si_attr = syn_id2attr (sip -> si_id);
    sip -> si_trans_id = sip -> si_id;
    if (sip -> si_flags & 0x40) sip -> si_cont_list = ((void *) 0);
    else sip -> si_cont_list = spp -> sp_cont_list;
    if ((spp -> sp_flags & 0x02) && !(sip->si_flags&0x40)) {
        if (idx == 0) {
            sip -> si_attr = 0;
            sip -> si_trans_id = 0;
            if (sip -> si_cont_list == ((void *) 0)) sip -> si_cont_list = (short *) -1;
        }
        else {
...
static void check_keepend () {
    ...
    lpos_T maxpos_h;
    stateitem_T * sip;
    if (keepend_level < 0) return;
    for (i = current_state.ga_len - 1; i > keepend_level; --i) if (((stateitem_T *) (current_state.ga_data)) [i].si_flags & 0x4000) break;
    maxpos.lnum = 0;
    maxpos.col = 0;
    maxpos_h.lnum = 0;
    maxpos_h.col = 0;
    for (; i < current_state.ga_len; ++i) {
        ...
        if (sip -> si_ends && (sip -> si_flags & 0x400)) {
            ...
            if (maxpos_h.lnum == 0 || maxpos_h.lnum > sip -> si_h_endpos.lnum || (maxpos_h.lnum == sip -> si_h_endpos.lnum && maxpos_h.col > sip -> si_h_endpos
              .col)) maxpos_h = sip -> si_h_endpos;
        }
    }
}
    ...
    int force;
{
    ...
    int end_idx;
    if (sip -> si_idx < 0) return;
    if (!force && sip -> si_m_endpos.lnum >= current_lnum) return;
    end_idx = 0;
...
static int push_current_state (idx)
    int idx;
{
    if (ga_grow (&current_state, 1) == 0) return 0;
    ((__builtin_object_size ((&((stateitem_T*)(current_state.ga_data))[current_state.ga_len]), 0) != (size_t) - 1) ? __builtin___memset_chk 
      ((&((stateitem_T*)(current_state.ga_data))[current_state.ga_len]), (0), (sizeof (stateitem_T)), __builtin_object_size 
      ((&((stateitem_T*)(current_state.ga_data))[current_state.ga_len]), 0)) : __inline_memset_chk 
      ((&((stateitem_T*)(current_state.ga_data))[current_state.ga_len]), (0), (sizeof (stateitem_T))));
...
static void pop_current_state () {
    ...
    next_match_idx = -1;
    if (keepend_level >= current_state.ga_len) keepend_level = -1;
}

static void find_endpos (idx, startpos, m_endpos, hl_endpos, flagsp, end_endpos, end_idx, start_ext)
    int idx;
    lpos_T * startpos;
    lpos_T * m_endpos;
    lpos_T * hl_endpos;
    ...
    reg_extmatch_T * start_ext;
{
    ...
    for (;;) {
        ...
        if (spp_skip != ((void *) 0)) {
            ...
            if (syn_regexec (&regmatch, startpos -> lnum, lc_col) && regmatch.startpos [0].col <= best_regmatch.startpos [0].col) {
                syn_add_end_off (&pos, &regmatch, spp_skip, 1, 1);
                if (pos.lnum > startpos -> lnum) break;
                line = ml_get_buf (syn_buf, startpos -> lnum, 0);
                if (pos.col <= matchcol) ++matchcol;
                else if (pos.col <= regmatch.endpos [0].col) matchcol = pos.col;
                else for (matchcol = regmatch.endpos [0].col; line [matchcol] != '\000' && matchcol < pos.col; ++matchcol);
                if (line [matchcol] == '\000') break;
                continue;
            }
        }
        spp = &(((synpat_T*)((syn_block)->b_syn_patterns.ga_data))[best_idx]);
        syn_add_end_off (m_endpos, &best_regmatch, spp, 1, 1);
        if (m_endpos -> lnum == startpos -> lnum && m_endpos -> col < startpos -> col) m_endpos -> col = startpos -> col;
        syn_add_end_off (end_endpos, &best_regmatch, spp, 3, 1);
        if (end_endpos -> lnum == startpos -> lnum && end_endpos -> col < startpos -> col) end_endpos -> col = startpos -> col;
        ...
        if (spp -> sp_syn_match_id != spp -> sp_syn.id && spp -> sp_syn_match_id != 0) {
            * end_idx = best_idx;
            ...
            if (hl_endpos -> lnum == startpos -> lnum && hl_endpos -> col < startpos -> col) hl_endpos -> col = startpos -> col;
...
static void limit_pos (pos, limit)
    ...
    lpos_T * limit;
{
    if (pos -> lnum > limit -> lnum) *pos = *limit;
    else if (pos -> lnum == limit -> lnum && pos -> col > limit -> col) pos -> col = limit -> col;
}
...
static void limit_pos_zero (pos, limit)
    ...
    lpos_T * limit;
{
    if (pos -> lnum == 0) *pos = *limit;
    else limit_pos (pos, limit);
}
    ...
    int extra;
{
    ...
    if (result -> lnum > syn_buf -> b_ml.ml_line_count) col = 0;
    else if (off != 0) {
...
static int syn_regexec (rmp, lnum, col)
    regmmatch_T * rmp;
    linenr_T lnum;
    colnr_T col;
{
...
static int check_keyword_id (line, startcol, endcolp, flagsp, next_listp, cur_si, ccharp)
    char_u * line;
    int startcol;
    int * endcolp;
    long * flagsp;
    ...
    int * ccharp __attribute__ ((unused));
{
    ...
    if (kwlen > 80) return 0;
    vim_strncpy (keyword, kwp, kwlen);
    for (round = 1; round <= 2; ++round) {
        ht = round == 1 ? &syn_block->b_keywtab : &syn_block->b_keywtab_ic;
        if (ht -> ht_used == 0) continue;
        if (round == 2) (void) str_foldcase (kwp, kwlen, keyword, 80 + 1);
        hi = hash_find (ht, keyword);
        if (!((hi)->hi_key==((void*)0)||(hi)->hi_key==&hash_removed)) for (kp = ((keyentry_T *) (((hi) -> hi_key) - (dumkey.keyword - (char_u *) &dumkey))); kp
          != ((void *) 0); kp = kp -> ke_next) {
            if (current_next_list != 0 ? in_id_list (((void *) 0), current_next_list, &kp->k_syn, 0) : (cur_si == ((void *) 0) ? !(kp->flags&0x01) : in_id_list
              (cur_si, cur_si -> si_cont_list, &kp->k_syn, kp -> flags & 0x01))) {
    ...
    int syncing __attribute__ ((unused));
{
    ...
    if (strncasecmp ((char *) (arg), (char *) ("match"), (size_t) (5)) == 0 && next - arg == 5) curwin -> w_s -> b_syn_ic = 0;
    else if (strncasecmp ((char *) (arg), (char *) ("ignore"), (size_t) (6)) == 0 && next - arg == 6) curwin -> w_s -> b_syn_ic = 1;
    else emsg2 ((char_u *) (((char *) ("E390: Illegal argument: %s"))), (char_u *) (arg));
}
    ...
    int syncing __attribute__ ((unused));
{
    ...
    if (eap -> skip) return;
    next = skiptowhite (arg);
    if (strncasecmp ((char *) (arg), (char *) ("toplevel"), (size_t) (8)) == 0 && next - arg == 8) curwin -> w_s -> b_syn_spell = 1;
    else if (strncasecmp ((char *) (arg), (char *) ("notoplevel"), (size_t) (10)) == 0 && next - arg == 10) curwin -> w_s -> b_syn_spell = 2;
    else if (strncasecmp ((char *) (arg), (char *) ("default"), (size_t) (7)) == 0 && next - arg == 7) curwin -> w_s -> b_syn_spell = 0;
    else emsg2 ((char_u *) (((char *) ("E390: Illegal argument: %s"))), (char_u *) (arg));
}
    ...
    synblock_T * block;
{
    ...
    for (i = block -> b_syn_patterns.ga_len; --i >= 0;) syn_clear_pattern (block, i);
    ga_clear (&block->b_syn_patterns);
    for (i = block -> b_syn_clusters.ga_len; --i >= 0;) syn_clear_cluster (block, i);
...
static void syntax_sync_clear () {
    int i;
    for (i = curwin -> w_s -> b_syn_patterns.ga_len; --i >= 0;) if (((synpat_T *) ((curwin -> w_s) -> b_syn_patterns.ga_data)) [i].sp_syncing)
    ...
    int idx;
{
    ...
    if (spp -> sp_flags & 0x2000) --block->b_syn_folditems;
    syn_clear_pattern (block, idx);
    ((__builtin_object_size ((char *) (spp), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (spp), (char *) (spp + 1), sizeof (synpat_T) * (block ->
      b_syn_patterns.ga_len - idx - 1), __builtin_object_size ((char *) (spp), 0)) : __inline_memmove_chk ((char *) (spp), (char *) (spp + 1), sizeof (synpat_T
      ) * (block -> b_syn_patterns.ga_len - idx - 1)));
    ...
    int syncing;
{
    ...
    eap -> nextcmd = find_nextcmd (arg);
    if (eap -> skip) return;
    if (curwin -> w_s -> b_syn_topgrp != 0) return;
    if (ends_excmd (*arg)) {
        if (syncing) syntax_sync_clear ();
        else {
            syntax_clear (curwin -> w_s);
            if (curwin -> w_s == &curwin->w_buffer->b_s) do_unlet ((char_u *) "b:current_syntax", 1);
    ...
    int syncing;
{
    ...
    for (idx = curwin -> w_s -> b_syn_patterns.ga_len; --idx >= 0;) {
        ...
        if (spp -> sp_syn.id != id || spp -> sp_syncing != syncing) continue;
    ...
    char * name;
{
    ...
    eap -> nextcmd = check_nextcmd (eap -> arg);
    if (!eap->skip) {
        ((__builtin_object_size ((char *) (buf), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (buf), (char *) ("so "), __builtin_object_size ((char *
          ) (buf), 1 > 1)) : __inline_strcpy_chk ((char *) (buf), (char *) ("so ")));
    ...
    int syncing;
{
    ...
    if (syncing) {
        if (curwin -> w_s -> b_syn_sync_flags & 0x01) {
            ...
            if (curwin -> w_s -> b_syn_sync_minlines == 0) msg_puts ((char_u *) (((char *) ("no syncing"))));
            else {
    ...
    if (ends_excmd (*arg)) {
        for (id = 1; id <= highlight_ga.ga_len && !got_int; ++id) syn_list_one (id, syncing, 0);
        for (id = 0; id < curwin -> w_s -> b_syn_clusters.ga_len && !got_int; ++id) syn_list_cluster (id);
    }
    else {
        while (!ends_excmd(*arg) && !got_int) {
            ...
            if (*arg == '@') {
                id = syn_scl_namen2id (arg + 1, (int) (arg_end - arg - 1));
                if (id == 0) emsg2 ((char_u *) (((char *) ("E392: No such syntax cluster: %s"))), (char_u *) (arg));
                else syn_list_cluster (id - 25000);
            }
            else {
                id = syn_namen2id (arg, (int) (arg_end - arg));
                if (id == 0) emsg2 ((char_u *) (((char *) (e_nogroup))), (char_u *) (arg));
                else syn_list_one (id, syncing, 1);
            }
            arg = skipwhite (arg_end);
        }
    }
...
static void syn_lines_msg () {
    if (curwin -> w_s -> b_syn_sync_maxlines > 0 || curwin -> w_s -> b_syn_sync_minlines > 0) {
        ...
        if (curwin -> w_s -> b_syn_sync_minlines > 0) {
            ...
            msg_outnum (curwin -> w_s -> b_syn_sync_minlines);
            if (curwin -> w_s -> b_syn_sync_maxlines) msg_puts ((char_u *) (", "));
        }
    ...
    int link_only;
{
    ...
    for (idx = 0; idx < curwin -> w_s -> b_syn_patterns.ga_len && !got_int; ++idx) {
        ...
        if (spp -> sp_type == 1) {
            ...
            while (((synpat_T *) ((curwin -> w_s) -> b_syn_patterns.ga_data)) [idx].sp_type == 2) put_pattern ("start", '=', 
              &((synpat_T*)((curwin->w_s)->b_syn_patterns.ga_data))[idx++], attr);
            if (((synpat_T *) ((curwin -> w_s) -> b_syn_patterns.ga_data)) [idx].sp_type == 4) put_pattern ("skip", '=', 
              &((synpat_T*)((curwin->w_s)->b_syn_patterns.ga_data))[idx++], attr);
            while (idx < curwin -> w_s -> b_syn_patterns.ga_len && ((synpat_T *) ((curwin -> w_s) -> b_syn_patterns.ga_data)) [idx].sp_type == 3) put_pattern (
              "end", '=', &((synpat_T*)((curwin->w_s)->b_syn_patterns.ga_data))[idx++], attr);
            --idx;
            msg_putchar (' ');
        }
        syn_list_flags (namelist1, spp -> sp_flags, attr);
        if (spp -> sp_cont_list != ((void *) 0)) put_id_list ((char_u *) "contains", spp -> sp_cont_list, attr);
        if (spp -> sp_syn.cont_in_list != ((void *) 0)) put_id_list ((char_u *) "containedin", spp -> sp_syn.cont_in_list, attr);
        ...
        if (spp -> sp_flags & (0x10 | 0x20)) {
            if (spp -> sp_flags & 0x10) msg_puts_attr ((char_u *) "grouphere", attr);
            else msg_puts_attr ((char_u *) "groupthere", attr);
            msg_putchar (' ');
            if (spp -> sp_sync_idx >= 0) msg_outtrans (((struct hl_group *) ((highlight_ga.ga_data))) [((synpat_T *) ((curwin -> w_s) -> b_syn_patterns.ga_data
              )) [spp -> sp_sync_idx].sp_syn.id - 1].sg_name);
            else msg_puts ((char_u *) ("NONE"));
            msg_putchar (' ');
        }
    }
...
static void syn_list_flags (nl, flags, attr)
    struct name_list * nl;
    int flags;
    int attr;
{
    int i;
    for (i = 0; nl [i].flag != 0; ++i) if (flags & nl [i].flag) {
    ...
    int id;
{
    ...
    if (msg_col >= endcol) endcol = msg_col + 1;
    if (Columns <= endcol) endcol = Columns - 1;
    msg_advance (endcol);
    if (((syn_cluster_T *) ((curwin -> w_s) -> b_syn_clusters.ga_data)) [id].scl_list != ((void *) 0)) {
        put_id_list ((char_u *) "cluster", ((syn_cluster_T *) ((curwin -> w_s) -> b_syn_clusters.ga_data)) [id].scl_list, highlight_attr [(int) (HLF_D)]);
    }
    else {
    ...
    int attr;
{
    ...
    for (p = list; *p; ++p) {
        if (*p >= 10000 && *p < 15000) {
            if (p [1]) msg_puts ((char_u *) ("ALLBUT"));
            else msg_puts ((char_u *) ("ALL"));
        }
        ...
        else msg_outtrans (((struct hl_group *) ((highlight_ga.ga_data))) [*p - 1].sg_name);
        if (p [1]) msg_putchar (',');
    }
    msg_putchar (' ');
}
    ...
    int attr;
{
    ...
    if (last_matchgroup != spp -> sp_syn_match_id) {
        ...
        msg_putchar ('=');
        if (last_matchgroup == 0) msg_outtrans ((char_u *) "NONE");
        else msg_outtrans (((struct hl_group *) ((highlight_ga.ga_data))) [last_matchgroup - 1].sg_name);
        msg_putchar (' ');
    }
    msg_puts_attr ((char_u *) s, attr);
    msg_putchar (c);
    for (i = 0; vim_strchr (spp -> sp_pattern, sepchars [i]) != ((void *) 0);) if (sepchars [++i] == '\000') {
        i = 0;
        break;
    }
    ...
    for (i = 0; i < 7; ++i) {
        ...
        if (spp -> sp_off_flags & (mask + (mask << 7))) {
            if (!first) msg_putchar (',');
            msg_puts ((char_u *) spo_name_tab [i]);
            n = spp -> sp_offsets [i];
            if (i != 6) {
                if (spp -> sp_off_flags & mask) msg_putchar ('s');
                else msg_putchar ('e');
                if (n > 0) msg_putchar ('+');
            }
            if (n || i == 6) msg_outnum (n);
            first = 0;
        }
    }
    msg_putchar (' ');
    ...
    for (hi = ht -> ht_array; todo > 0 && !got_int; ++hi) {
        if (!((hi)->hi_key==((void*)0)||(hi)->hi_key==&hash_removed)) {
            ...
            for (kp = ((keyentry_T *) (((hi) -> hi_key) - (dumkey.keyword - (char_u *) &dumkey))); kp != ((void *) 0) && !got_int; kp = kp -> ke_next) {
                if (kp -> k_syn.id == id) {
                    if (prev_contained != (kp -> flags & 0x01) || prev_skipnl != (kp -> flags & 0x80) || prev_skipwhite != (kp -> flags & 0x100) ||
                      prev_skipempty != (kp -> flags & 0x200) || prev_cont_in_list != kp -> k_syn.cont_in_list || prev_next_list != kp -> next_list) outlen =
                      9999;
                    else outlen = (int) strlen ((char *) (kp -> keyword));
        ...
        if (!((hi)->hi_key==((void*)0)||(hi)->hi_key==&hash_removed)) {
            ...
            for (kp = ((keyentry_T *) (((hi) -> hi_key) - (dumkey.keyword - (char_u *) &dumkey))); kp != ((void *) 0);) {
                if (kp -> k_syn.id == id) {
                    kp_next = kp -> ke_next;
                    if (kp_prev == ((void *) 0)) {
                        if (kp_next == ((void *) 0)) hash_remove (ht, hi);
                        else hi -> hi_key = ((kp_next) -> keyword);
                    }
                    else kp_prev -> ke_next = kp_next;
    ...
    int conceal_char;
{
    ...
    if (curwin -> w_s -> b_syn_ic) name_ic = str_foldcase (name, (int) strlen ((char *) (name)), name_folded, 80 + 1);
    else name_ic = name;
    kp = (keyentry_T *) alloc ((int) (sizeof (keyentry_T) + strlen ((char *) (name_ic))));
    if (kp == ((void *) 0)) return;
    ((__builtin_object_size ((char *) (kp -> keyword), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (kp -> keyword), (char *) (name_ic),
      __builtin_object_size ((char *) (kp -> keyword), 1 > 1)) : __inline_strcpy_chk ((char *) (kp -> keyword), (char *) (name_ic)));
    kp -> k_syn.id = id;
    kp -> k_syn.inc_tag = current_syn_inc_tag;
    kp -> flags = flags;
    kp -> k_char = conceal_char;
    kp -> k_syn.cont_in_list = copy_id_list (cont_in_list);
    if (cont_in_list != ((void *) 0)) curwin -> w_s -> b_syn_containedin = 1;
    kp -> next_list = copy_id_list (next_list);
    if (curwin -> w_s -> b_syn_ic) ht = &curwin->w_s->b_keywtab_ic;
    else ht = &curwin->w_s->b_keywtab;
    ...
    char_u * * name_end;
{
    ...
    * name_end = skiptowhite (arg);
    ...
    if (ends_excmd (*arg) || *rest == '\000') return ((void *) 0);
    return rest;
}
    ...
    int * conceal_char __attribute__ ((unused));
{
    ...
    for (;;) {
        if (strchr (first_letters, *arg) == ((void *) 0)) break;
        for (fidx = sizeof (flagtab) / sizeof (struct flag); --fidx >= 0;) {
            p = flagtab [fidx].name;
            for (i = 0, len = 0; p [i] != '\000'; i += 2, ++len) if (arg [len] != p [i] && arg [len] != p [i + 1]) break;
            if (p [i] == '\000' && (((arg [len]) == ' ' || (arg [len]) == '\t') || (flagtab [fidx].argtype > 0 ? arg [len] == '=' : ends_excmd (arg [len])))) {
                if (opt -> keyword && (flagtab [fidx].flags == 0x1000 || flagtab [fidx].flags == 0x2000 || flagtab [fidx].flags == 0x4000)) fidx = -1;
                break;
            }
        }
        if (fidx < 0) break;
        if (flagtab [fidx].argtype == 1) {
            ...
            if (get_id_list (&arg, 8, &opt->cont_list) == 0) return ((void *) 0);
        }
        else if (flagtab [fidx].argtype == 2) {
            if (get_id_list (&arg, 11, &opt->cont_in_list) == 0) return ((void *) 0);
        }
        else if (flagtab [fidx].argtype == 3) {
            if (get_id_list (&arg, 9, &opt->next_list) == 0) return ((void *) 0);
        }
            ...
            if (flagtab [fidx].flags == 0x10 || flagtab [fidx].flags == 0x20) {
                ...
                gname_start = arg;
                arg = skiptowhite (arg);
                if (gname_start == arg) return ((void *) 0);
                gname = vim_strnsave (gname_start, (int) (arg - gname_start));
                if (gname == ((void *) 0)) return ((void *) 0);
                if (strcmp ((char *) (gname), (char *) ("NONE")) == 0) *opt->sync_idx = -2;
                else {
                    syn_id = syn_name2id (gname);
                    for (i = curwin -> w_s -> b_syn_patterns.ga_len; --i >= 0;) if (((synpat_T *) ((curwin -> w_s) -> b_syn_patterns.ga_data)) [i].sp_syn.id ==
                      syn_id && ((synpat_T *) ((curwin -> w_s) -> b_syn_patterns.ga_data)) [i].sp_type == 2) {
                        *opt->sync_idx = i;
                        break;
                    }
                    if (i < 0) {
            ...
            else if (flagtab [fidx].flags == 0x2000 && foldmethodIsSyntax (curwin)) foldUpdateAll (curwin);
        }
    }
    return arg;
}
...
static void syn_incl_toplevel (id, flagsp)
    ...
    int * flagsp;
{
    if ((*flagsp & 0x01) || curwin -> w_s -> b_syn_topgrp == 0) return;
    *flagsp |= 0x01;
    if (curwin -> w_s -> b_syn_topgrp >= 25000) {
        ...
        if (grp_list != ((void *) 0)) {
            grp_list [0] = id;
            grp_list [1] = 0;
            syn_combine_list (&((syn_cluster_T*)((curwin->w_s)->b_syn_clusters.ga_data))[tlg_id].scl_list, &grp_list, 2);
        }
    }
}
    ...
    int syncing __attribute__ ((unused));
{
    ...
    if (*eap->arg == '<' || *eap->arg == '$' || mch_isFullName (eap -> arg)) {
        ...
        if (expand_filename (eap, syn_cmdlinep, &errormsg) == 0) {
            if (errormsg != ((void *) 0)) emsg ((char_u *) (errormsg));
            return;
        }
    }
    ...
    if (source ? do_source (eap -> arg, 0, 0) == 0 : source_runtime (eap -> arg, 1) == 0) emsg2 ((char_u *) (((char *) (e_notopen))), (char_u *) (eap -> arg));
    ...
    int syncing __attribute__ ((unused));
{
    ...
    if (rest != ((void *) 0)) {
        ...
        if (keyword_copy != ((void *) 0)) {
            ...
            for (; rest != ((void *) 0) && !ends_excmd(*rest); rest = skipwhite (rest)) {
                ...
                if (rest == ((void *) 0) || ends_excmd (*rest)) break;
                while (*rest != '\000' && !((*rest) == ' ' || (*rest) == '\t')) {
                    if (*rest == '\\' && rest [1] != '\000') ++rest;
                    *p++ = *rest++;
                }
                *p++ = '\000';
                ++cnt;
    ...
    if (rest != ((void *) 0)) eap -> nextcmd = check_nextcmd (rest);
    ...
    int syncing;
{
    ...
    rest = get_syn_options (rest, &syn_opt_arg, &conceal_char);
    init_syn_patterns ();
    ((__builtin_object_size ((&item), 0) != (size_t) - 1) ? __builtin___memset_chk ((&item), (0), (sizeof (item)), __builtin_object_size ((&item), 0)) :
      __inline_memset_chk ((&item), (0), (sizeof (item))));
    rest = get_syn_pattern (rest, &item);
    if (vim_regcomp_had_eol () && !(syn_opt_arg.flags&0x800)) syn_opt_arg.flags |= 0x08;
    rest = get_syn_options (rest, &syn_opt_arg, &conceal_char);
    if (rest != ((void *) 0)) {
        eap -> nextcmd = check_nextcmd (rest);
        if (!ends_excmd(*rest) || eap -> skip) rest = ((void *) 0);
        else if (ga_grow (&curwin->w_s->b_syn_patterns, 1) != 0 && (syn_id = syn_check_group (arg, (int) (group_name_end - arg))) != 0) {
            syn_incl_toplevel (syn_id, &syn_opt_arg.flags);
            ...
            ((synpat_T *) ((curwin -> w_s) -> b_syn_patterns.ga_data)) [idx].sp_syn.cont_in_list = syn_opt_arg.cont_in_list;
            if (syn_opt_arg.cont_in_list != ((void *) 0)) curwin -> w_s -> b_syn_containedin = 1;
            ((synpat_T *) ((curwin -> w_s) -> b_syn_patterns.ga_data)) [idx].sp_next_list = syn_opt_arg.next_list;
            ++curwin->w_s->b_syn_patterns.ga_len;
            if (syn_opt_arg.flags & (0x10 | 0x20)) curwin -> w_s -> b_syn_sync_flags |= 0x02;
            if (syn_opt_arg.flags & 0x2000) ++curwin->w_s->b_syn_folditems;
            redraw_curbuf_later (35);
            syn_stack_free_all (curwin -> w_s);
            return;
        }
    ...
    vim_free (syn_opt_arg.cont_in_list);
    vim_free (syn_opt_arg.next_list);
    if (rest == ((void *) 0)) emsg2 ((char_u *) (((char *) (e_invarg2))), (char_u *) (arg));
}
    ...
    int syncing;
{
    ...
    while (rest != ((void *) 0) && !ends_excmd(*rest)) {
        ...
        if (rest == ((void *) 0) || ends_excmd (*rest)) break;
        key_end = rest;
        while (*key_end && !((*key_end) == ' ' || (*key_end) == '\t') && *key_end != '=') ++key_end;
        vim_free (key);
        ...
        if (strcmp ((char *) (key), (char *) ("MATCHGROUP")) == 0) item = 3;
        else if (strcmp ((char *) (key), (char *) ("START")) == 0) item = 0;
        else if (strcmp ((char *) (key), (char *) ("END")) == 0) item = 2;
        ...
        if (item == 3) {
            p = skiptowhite (rest);
            if ((p - rest == 4 && strncmp ((char *) (rest), (char *) ("NONE"), (size_t) (4)) == 0) || eap -> skip) matchgroup_id = 0;
            else {
            ...
            if (item == 0) reg_do_extmatch = 1;
            else if (item == 1 || item == 2) reg_do_extmatch = 2;
            rest = get_syn_pattern (rest, ppp -> pp_synp);
            reg_do_extmatch = 0;
            if (item == 2 && vim_regcomp_had_eol () && !(syn_opt_arg.flags&0x800)) ppp -> pp_synp -> sp_flags |= 0x08;
            ppp -> pp_matchgroup_id = matchgroup_id;
            ++pat_count;
        }
    }
    vim_free (key);
    if (illegal || not_enough) rest = ((void *) 0);
    if (rest != ((void *) 0) && (pat_ptrs [0] == ((void *) 0) || pat_ptrs [2] == ((void *) 0))) {
        not_enough = 1;
        rest = ((void *) 0);
    }
    if (rest != ((void *) 0)) {
        eap -> nextcmd = check_nextcmd (rest);
        if (!ends_excmd(*rest) || eap -> skip) rest = ((void *) 0);
        else if (ga_grow (&(curwin->w_s->b_syn_patterns), pat_count) != 0 && (syn_id = syn_check_group (arg, (int) (group_name_end - arg))) != 0) {
            syn_incl_toplevel (syn_id, &syn_opt_arg.flags);
            idx = curwin -> w_s -> b_syn_patterns.ga_len;
            for (item = 0; item <= 2; ++item) {
                for (ppp = pat_ptrs [item]; ppp != ((void *) 0); ppp = ppp -> pp_next) {
                    ...
                    ((synpat_T *) ((curwin -> w_s) -> b_syn_patterns.ga_data)) [idx].sp_syncing = syncing;
                    ((synpat_T *) ((curwin -> w_s) -> b_syn_patterns.ga_data)) [idx].sp_type = (item == 0) ? 2 : (item == 1) ? 4 : 3;
                    ((synpat_T *) ((curwin -> w_s) -> b_syn_patterns.ga_data)) [idx].sp_flags |= syn_opt_arg.flags;
                    ((synpat_T *) ((curwin -> w_s) -> b_syn_patterns.ga_data)) [idx].sp_syn.id = syn_id;
                    ((synpat_T *) ((curwin -> w_s) -> b_syn_patterns.ga_data)) [idx].sp_syn.inc_tag = current_syn_inc_tag;
                    ((synpat_T *) ((curwin -> w_s) -> b_syn_patterns.ga_data)) [idx].sp_syn_match_id = ppp -> pp_matchgroup_id;
                    if (item == 0) {
                        ((synpat_T *) ((curwin -> w_s) -> b_syn_patterns.ga_data)) [idx].sp_cont_list = syn_opt_arg.cont_list;
                        ((synpat_T *) ((curwin -> w_s) -> b_syn_patterns.ga_data)) [idx].sp_syn.cont_in_list = syn_opt_arg.cont_in_list;
                        if (syn_opt_arg.cont_in_list != ((void *) 0)) curwin -> w_s -> b_syn_containedin = 1;
    ...
    for (item = 0; item <= 2; ++item) for (ppp = pat_ptrs [item]; ppp != ((void *) 0); ppp = ppp_next) {
        if (!success) {
    ...
    if (!success) {
        ...
        vim_free (syn_opt_arg.cont_in_list);
        vim_free (syn_opt_arg.next_list);
        if (not_enough) emsg2 ((char_u *) (((char *) ("E399: Not enough arguments: syntax region %s"))), (char_u *) (arg));
        else if (illegal || rest == ((void *) 0)) emsg2 ((char_u *) (((char *) (e_invarg2))), (char_u *) (arg));
    }
}

static int syn_compare_stub (v1, v2) const
    void * v1;
    const void * v2;
{
    const short * s1 = v1;
    ...
    int list_op;
{
    ...
    int count;
    int round;
    if (*clstr2 == ((void *) 0)) return;
    if (*clstr1 == ((void *) 0) || list_op == 1) {
        if (list_op == 1) vim_free (*clstr1);
        if (list_op == 1 || list_op == 2) *clstr1 = *clstr2;
        else vim_free (*clstr2);
        return;
    }
    for (g1 = *clstr1; *g1; g1 ++) ++count1;
    for (g2 = *clstr2; *g2; g2 ++) ++count2;
    ...
    for (round = 1; round <= 2; round ++) {
        ...
        while (*g1 && *g2) {
            if (*g1 < *g2) {
                if (round == 2) clstr [count] = *g1;
                count ++;
                g1 ++;
                continue;
            }
            if (list_op == 2) {
                if (round == 2) clstr [count] = *g2;
                count ++;
            }
            if (*g1 == *g2) g1 ++;
            g2 ++;
        }
        for (; *g1; g1 ++, count ++) if (round == 2) clstr [count] = *g1;
        if (list_op == 2) for (; *g2; g2 ++, count ++) if (round == 2) clstr [count] = *g2;
        if (round == 1) {
            if (count == 0) {
                clstr = ((void *) 0);
                break;
    ...
    char_u * name;
{
    ...
    if (name_u == ((void *) 0)) return 0;
    for (i = curwin -> w_s -> b_syn_clusters.ga_len; --i >= 0;) if (((syn_cluster_T *) ((curwin -> w_s) -> b_syn_clusters.ga_data)) [i].scl_name_u != ((void *)
      0) && strcmp ((char *) (name_u), (char *) (((syn_cluster_T *) ((curwin -> w_s) -> b_syn_clusters.ga_data)) [i].scl_name_u)) == 0) break;
    vim_free (name_u);
    ...
    int len;
{
    ...
    id = syn_scl_name2id (name);
    if (id == 0) id = syn_add_cluster (name);
    else vim_free (name);
    return id;
}
    ...
    char_u * name;
{
    ...
    ((__builtin_object_size ((&(((syn_cluster_T*)((curwin->w_s)->b_syn_clusters.ga_data))[len])), 0) != (size_t) - 1) ? __builtin___memset_chk 
      ((&(((syn_cluster_T*)((curwin->w_s)->b_syn_clusters.ga_data))[len])), (0), (sizeof (syn_cluster_T)), __builtin_object_size 
      ((&(((syn_cluster_T*)((curwin->w_s)->b_syn_clusters.ga_data))[len])), 0)) : __inline_memset_chk 
    ...
    if (strcasecmp ((char *) (name), (char *) ("Spell")) == 0) curwin -> w_s -> b_spell_cluster_id = len + 25000;
    if (strcasecmp ((char *) (name), (char *) ("NoSpell")) == 0) curwin -> w_s -> b_nospell_cluster_id = len + 25000;
    return len + 25000;
}
    ...
    int syncing __attribute__ ((unused));
{
    ...
    rest = get_group_name (arg, &group_name_end);
    if (rest != ((void *) 0)) {
        scl_id = syn_check_cluster (arg, (int) (group_name_end - arg)) - 25000;
        for (;;) {
            if (strncasecmp ((char *) (rest), (char *) ("add"), (size_t) (3)) == 0 && (((rest [3]) == ' ' || (rest [3]) == '\t') || rest [3] == '=')) {
                opt_len = 3;
                list_op = 2;
            }
            else if (strncasecmp ((char *) (rest), (char *) ("remove"), (size_t) (6)) == 0 && (((rest [6]) == ' ' || (rest [6]) == '\t') || rest [6] == '=')) {
                opt_len = 6;
                list_op = 3;
            }
            else if (strncasecmp ((char *) (rest), (char *) ("contains"), (size_t) (8)) == 0 && (((rest [8]) == ' ' || (rest [8]) == '\t') || rest [8] == '='))
              {
                opt_len = 8;
                list_op = 1;
            }
            ...
            syn_combine_list (&((syn_cluster_T*)((curwin->w_s)->b_syn_clusters.ga_data))[scl_id].scl_list, &clstr_list, list_op);
            got_clstr = 1;
        }
        if (got_clstr) {
            redraw_curbuf_later (35);
    ...
    if (!got_clstr) emsg ((char_u *) (((char *) ("E400: No cluster specified"))));
    if (rest == ((void *) 0) || !ends_excmd(*rest)) emsg2 ((char_u *) (((char *) (e_invarg2))), (char_u *) (arg));
}
    ...
    synpat_T * ci;
{
    ...
    int idx;
    char_u * cpo_save;
    if (arg == ((void *) 0) || arg [1] == '\000' || arg [2] == '\000') return ((void *) 0);
    end = skip_regexp (arg + 1, *arg, 1, ((void *) 0));
    if (*end != *arg) {
        emsg2 ((char_u *) (((char *) ("E401: Pattern delimiter not found: %s"))), (char_u *) (arg));
        return ((void *) 0);
    }
    if ((ci -> sp_pattern = vim_strnsave (arg + 1, (int) (end - arg - 1))) == ((void *) 0)) return ((void *) 0);
    cpo_save = p_cpo;
    p_cpo = (char_u *) "";
    ci -> sp_prog = vim_regcomp (ci -> sp_pattern, 1);
    p_cpo = cpo_save;
    if (ci -> sp_prog == ((void *) 0)) return ((void *) 0);
    ci -> sp_ic = curwin -> w_s -> b_syn_ic;
    ++end;
    do {
        for (idx = 7; --idx >= 0;) if (strncmp ((char *) (end), (char *) (spo_name_tab [idx]), (size_t) (3)) == 0) break;
        if (idx >= 0) {
            p = &(ci->sp_offsets[idx]);
            if (idx != 6) switch (end [3]) {
            case 's' :
                break;
            case 'b' :
                break;
    ...
    int syncing __attribute__ ((unused));
{
    ...
    while (!ends_excmd(*arg_start)) {
        ...
        if (strcmp ((char *) (key), (char *) ("CCOMMENT")) == 0) {
            if (!eap->skip) curwin -> w_s -> b_syn_sync_flags |= 0x01;
            if (!ends_excmd(*next_arg)) {
                arg_end = skiptowhite (next_arg);
                if (!eap->skip) curwin -> w_s -> b_syn_sync_id = syn_check_group (next_arg, (int) (arg_end - next_arg));
                next_arg = skipwhite (arg_end);
            }
            else if (!eap->skip) curwin -> w_s -> b_syn_sync_id = syn_name2id ((char_u *) "Comment");
        }
        else if (strncmp ((char *) (key), (char *) ("LINES"), (size_t) (5)) == 0 || strncmp ((char *) (key), (char *) ("MINLINES"), (size_t) (8)) == 0 ||
          strncmp ((char *) (key), (char *) ("MAXLINES"), (size_t) (8)) == 0 || strncmp ((char *) (key), (char *) ("LINEBREAKS"), (size_t) (10)) == 0) {
            if (key [4] == 'S') arg_end = key + 6;
            else if (key [0] == 'L') arg_end = key + 11;
            else arg_end = key + 9;
            ...
            n = getdigits (&arg_end);
            if (!eap->skip) {
                if (key [4] == 'B') curwin -> w_s -> b_syn_sync_linebreaks = n;
                else if (key [1] == 'A') curwin -> w_s -> b_syn_sync_maxlines = n;
                else curwin -> w_s -> b_syn_sync_minlines = n;
            }
        }
            ...
            if (!eap->skip) {
                if ((curwin -> w_s -> b_syn_linecont_pat = vim_strnsave (next_arg + 1, (int) (arg_end - next_arg - 1))) == ((void *) 0)) {
                    finished = 1;
                    break;
                }
                ...
                cpo_save = p_cpo;
                p_cpo = (char_u *) "";
                curwin -> w_s -> b_syn_linecont_prog = vim_regcomp (curwin -> w_s -> b_syn_linecont_pat, 1);
                p_cpo = cpo_save;
            ...
            eap -> arg = next_arg;
            if (strcmp ((char *) (key), (char *) ("MATCH")) == 0) syn_cmd_match (eap, 1);
            else if (strcmp ((char *) (key), (char *) ("REGION")) == 0) syn_cmd_region (eap, 1);
            else if (strcmp ((char *) (key), (char *) ("CLEAR")) == 0) syn_cmd_clear (eap, 1);
            else illegal = 1;
            finished = 1;
            break;
        }
    ...
    vim_free (key);
    if (illegal) emsg2 ((char_u *) (((char *) ("E404: Illegal arguments: %s"))), (char_u *) (arg_start));
    else if (!finished) {
    ...
    short * * list;
{
    ...
    for (round = 1; round <= 2; ++round) {
        ...
        while (!ends_excmd(*p)) {
            ...
            if (strcmp ((char *) (name + 1), (char *) ("ALLBUT")) == 0 || strcmp ((char *) (name + 1), (char *) ("ALL")) == 0 || strcmp ((char *) (name + 1), (
              char *) ("TOP")) == 0 || strcmp ((char *) (name + 1), (char *) ("CONTAINED")) == 0) {
                ...
                if (name [1] == 'A') id = 10000;
                else if (name [1] == 'T') id = 15000;
                else id = 20000;
                ...
                if ((char_u *) strpbrk ((char *) (name + 1), (char *) ((char_u *) "\\.*^$~[")) == ((void *) 0)) id = syn_check_group (name + 1, (int) (end - p)
                  );
                else {
                    * name = '^';
                    ((__builtin_object_size ((char *) (name), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (name), (char *) ("$"),
                      __builtin_object_size ((char *) (name), 1 > 1)) : __inline_strcat_chk ((char *) (name), (char *) ("$")));
    ...
    * arg = p;
    ...
    if (*list == ((void *) 0)) *list = retval;
    else vim_free (retval);
    return 1;
}
    ...
    short * list;
{
    ...
    if (retval != ((void *) 0)) ((__builtin_object_size ((char *) (retval), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (retval), (char *) (list), (
      size_t) len, __builtin_object_size ((char *) (retval), 0)) : __inline_memmove_chk ((char *) (retval), (char *) (list), (size_t) len));
    return retval;
}
    ...
    int contained;
{
    ...
    static int depth = 0;
    int r;
    if (cur_si != ((void *) 0) && ssp -> cont_in_list != ((void *) 0) && !(cur_si->si_flags&0x40)) {
        while ((cur_si -> si_flags & 0x10000) && cur_si > (stateitem_T *) (current_state.ga_data)) --cur_si;
        if (cur_si -> si_idx >= 0 && in_id_list (((void *) 0), ssp -> cont_in_list, 
          &(((synpat_T*)((syn_block)->b_syn_patterns.ga_data))[cur_si->si_idx].sp_syn), ((synpat_T *) ((syn_block) -> b_syn_patterns.ga_data)) [cur_si -> si_idx
          ].sp_flags & 0x01)) return 1;
    }
    if (list == ((void *) 0)) return 0;
    if (list == (short *) -1) return !contained;
    item = *list;
    if (item >= 10000 && item < 25000) {
        if (item < 15000) {
            if (item - 10000 != ssp -> inc_tag) return 0;
        }
        else if (item < 20000) {
            if (item - 15000 != ssp -> inc_tag || contained) return 0;
        }
        else {
            if (item - 20000 != ssp -> inc_tag || !contained) return 0;
        }
        item = *++list;
        retval = 0;
    }
    ...
    exarg_T * eap;
{
    ...
    if (curwin -> w_s == &curwin->w_buffer->b_s) {
        ...
        ((__builtin_object_size (curwin -> w_s, 0) != (size_t) - 1) ? __builtin___memset_chk (curwin -> w_s, 0, sizeof (synblock_T), __builtin_object_size (
          curwin -> w_s, 0)) : __inline_memset_chk (curwin -> w_s, 0, sizeof (synblock_T)));
    ...
    if (old_value != ((void *) 0)) old_value = vim_strsave (old_value);
    apply_autocmds (EVENT_SYNTAX, eap -> arg, curbuf -> b_fname, 1, curbuf);
    new_value = get_var_value ((char_u *) "b:current_syntax");
    if (new_value != ((void *) 0)) set_internal_string_var ((char_u *) "w:current_syntax", new_value);
    if (old_value == ((void *) 0)) do_unlet ((char_u *) "b:current_syntax", 1);
    else {
...
int syntax_present (win)
    win_T * win;
{
    return (win -> w_s -> b_syn_patterns.ga_len != 0 || win -> w_s -> b_syn_clusters.ga_len != 0 || win -> w_s -> b_keywtab.ht_used > 0 || win -> w_s ->
      b_keywtab_ic.ht_used > 0);
}

static enum {EXP_SUBCMD, EXP_CASE} expand_what;
    ...
    char_u * arg;
{
    ...
    if (*arg != '\000') {
        ...
        if (*p != '\000') {
            ...
            if (*skiptowhite(xp->xp_pattern) != '\000') xp -> xp_context = 0;
            else if (strncasecmp ((char *) (arg), (char *) ("case"), (size_t) (p - arg)) == 0) expand_what = EXP_CASE;
            else if (strncasecmp ((char *) (arg), (char *) ("keyword"), (size_t) (p - arg)) == 0 || strncasecmp ((char *) (arg), (char *) ("region"), (size_t) (
              p - arg)) == 0 || strncasecmp ((char *) (arg), (char *) ("match"), (size_t) (p - arg)) == 0 || strncasecmp ((char *) (arg), (char *) ("list"), (
              size_t) (p - arg)) == 0) xp -> xp_context = 13;
            else xp -> xp_context = 0;
        }
    }
}
...
char_u * get_syntax_name (xp, idx)
    ...
    int idx;
{
    if (expand_what == EXP_SUBCMD) return (char_u *) subcommands [idx].name;
...
int syn_get_id (wp, lnum, col, trans, spellp, keep_state)
    ...
    int keep_state;
{
    if (wp -> w_buffer != syn_buf || lnum != current_lnum || col < current_col) syntax_start (wp, lnum);
    ...
    long lnum;
{
    ...
    if (wp -> w_s -> b_syn_folditems != 0) {
        ...
        for (i = 0; i < current_state.ga_len; ++i) if (((stateitem_T *) (current_state.ga_data)) [i].si_flags & 0x2000) ++level;
    }
...
{"ErrorMsg term=standout ctermbg=DarkRed ctermfg=White guibg=Red guifg=White", "IncSearch term=reverse cterm=reverse gui=reverse",
      "ModeMsg term=bold cterm=bold gui=bold", "NonText term=bold ctermfg=Blue gui=bold guifg=Blue",
      "StatusLine term=reverse,bold cterm=reverse,bold gui=reverse,bold", "StatusLineNC term=reverse cterm=reverse gui=reverse",
      "VertSplit term=reverse cterm=reverse gui=reverse", "VisualNOS term=underline,bold cterm=underline,bold gui=underline,bold",
      "DiffText term=reverse cterm=bold ctermbg=Red gui=bold guibg=Red", "PmenuThumb cterm=reverse gui=reverse", "PmenuSbar ctermbg=Grey guibg=Grey",
      "TabLineSel term=bold cterm=bold gui=bold", "TabLineFill term=reverse cterm=reverse gui=reverse", ((void *) 0)};
static char * (highlight_init_light []) =
{"Directory term=bold ctermfg=DarkBlue guifg=Blue", "LineNr term=underline ctermfg=Brown guifg=Brown",
      "MoreMsg term=bold ctermfg=DarkGreen gui=bold guifg=SeaGreen", "Question term=standout ctermfg=DarkGreen gui=bold guifg=SeaGreen",
      "Search term=reverse ctermbg=Yellow ctermfg=NONE guibg=Yellow guifg=NONE", "SpellBad term=reverse ctermbg=LightRed guisp=Red gui=undercurl",
      "SpellCap term=reverse ctermbg=LightBlue guisp=Blue gui=undercurl", "SpellRare term=reverse ctermbg=LightMagenta guisp=Magenta gui=undercurl",
      "SpellLocal term=underline ctermbg=Cyan guisp=DarkCyan gui=undercurl", "Pmenu ctermbg=LightMagenta guibg=LightMagenta",
      "PmenuSel ctermbg=LightGrey guibg=Grey", "SpecialKey term=bold ctermfg=DarkBlue guifg=Blue", "Title term=bold ctermfg=DarkMagenta gui=bold guifg=Magenta"
      , "WarningMsg term=standout ctermfg=DarkRed guifg=Red", "WildMenu term=standout ctermbg=Yellow ctermfg=Black guibg=Yellow guifg=Black",
      "Folded term=standout ctermbg=Grey ctermfg=DarkBlue guibg=LightGrey guifg=DarkBlue",
      "FoldColumn term=standout ctermbg=Grey ctermfg=DarkBlue guibg=Grey guifg=DarkBlue",
      "SignColumn term=standout ctermbg=Grey ctermfg=DarkBlue guibg=Grey guifg=DarkBlue", "Visual term=reverse guibg=LightGrey",
      "DiffAdd term=bold ctermbg=LightBlue guibg=LightBlue", "DiffChange term=bold ctermbg=LightMagenta guibg=LightMagenta",
      "DiffDelete term=bold ctermfg=Blue ctermbg=LightCyan gui=bold guifg=Blue guibg=LightCyan",
      "TabLine term=underline cterm=underline ctermfg=black ctermbg=LightGrey gui=underline guibg=LightGrey",
      "CursorColumn term=reverse ctermbg=LightGrey guibg=Grey90", "CursorLine term=underline cterm=underline guibg=Grey90",
      "ColorColumn term=reverse ctermbg=LightRed guibg=LightRed", "MatchParen term=reverse ctermbg=Cyan guibg=Cyan", ((void *) 0)};
static char * (highlight_init_dark []) =
{"Directory term=bold ctermfg=LightCyan guifg=Cyan", "LineNr term=underline ctermfg=Yellow guifg=Yellow",
      "MoreMsg term=bold ctermfg=LightGreen gui=bold guifg=SeaGreen", "Question term=standout ctermfg=LightGreen gui=bold guifg=Green",
      "Search term=reverse ctermbg=Yellow ctermfg=Black guibg=Yellow guifg=Black", "SpecialKey term=bold ctermfg=LightBlue guifg=Cyan",
      "SpellBad term=reverse ctermbg=Red guisp=Red gui=undercurl", "SpellCap term=reverse ctermbg=Blue guisp=Blue gui=undercurl",
      "SpellRare term=reverse ctermbg=Magenta guisp=Magenta gui=undercurl", "SpellLocal term=underline ctermbg=Cyan guisp=Cyan gui=undercurl",
      "Pmenu ctermbg=Magenta guibg=Magenta", "PmenuSel ctermbg=DarkGrey guibg=DarkGrey", "Title term=bold ctermfg=LightMagenta gui=bold guifg=Magenta",
      "WarningMsg term=standout ctermfg=LightRed guifg=Red", "WildMenu term=standout ctermbg=Yellow ctermfg=Black guibg=Yellow guifg=Black",
      "Folded term=standout ctermbg=DarkGrey ctermfg=Cyan guibg=DarkGrey guifg=Cyan",
      "FoldColumn term=standout ctermbg=DarkGrey ctermfg=Cyan guibg=Grey guifg=Cyan",
      "SignColumn term=standout ctermbg=DarkGrey ctermfg=Cyan guibg=Grey guifg=Cyan", "Visual term=reverse guibg=DarkGrey",
      "DiffAdd term=bold ctermbg=DarkBlue guibg=DarkBlue", "DiffChange term=bold ctermbg=DarkMagenta guibg=DarkMagenta",
      "DiffDelete term=bold ctermfg=Blue ctermbg=DarkCyan gui=bold guifg=Blue guibg=DarkCyan",
      "TabLine term=underline cterm=underline ctermfg=white ctermbg=DarkGrey gui=underline guibg=DarkGrey",
      "CursorColumn term=reverse ctermbg=DarkGrey guibg=Grey40", "CursorLine term=underline cterm=underline guibg=Grey40",
      "ColorColumn term=reverse ctermbg=DarkRed guibg=DarkRed", "MatchParen term=reverse ctermbg=DarkCyan guibg=DarkCyan", ((void *) 0)};
    ...
    int reset;
{
    ...
    if (p != ((void *) 0) && load_colors (p) == 1) return;
    if (both) {
        had_both = 1;
        pp = highlight_init_both;
        for (i = 0; pp [i] != ((void *) 0); ++i) do_highlight ((char_u *) pp [i], reset, 1);
    }
    else if (!had_both) return;
    if (*p_bg == 'l') pp = highlight_init_light;
    else pp = highlight_init_dark;
    for (i = 0; pp [i] != ((void *) 0); ++i) do_highlight ((char_u *) pp [i], reset, 1);
    if (t_colors > 8) do_highlight ((char_u *) (*p_bg == 'l' ? "Visual cterm=NONE ctermbg=LightGrey" : "Visual cterm=NONE ctermbg=DarkGrey"), 0, 1);
    else {
        do_highlight ((char_u *) "Visual cterm=reverse ctermbg=NONE", 0, 1);
        if (*p_bg == 'l') do_highlight ((char_u *) "Search ctermfg=black", 0, 1);
    }
    if (get_var_value ((char_u *) "g:syntax_on") != ((void *) 0)) {
        static int recursive = 0;
        if (recursive >= 5) emsg ((char_u *) (((char *) ("E679: recursive loop loading syncolor.vim"))));
        else {
            ++recursive;
    ...
    int init;
{
    ...
    if (ends_excmd (*line)) {
        for (i = 1; i <= highlight_ga.ga_len && !got_int; ++i) highlight_list_one ((int) i);
        return;
    }
    ...
    if (strncmp ((char *) (line), (char *) ("clear"), (size_t) (name_end - line)) == 0) doclear = 1;
    if (strncmp ((char *) (line), (char *) ("link"), (size_t) (name_end - line)) == 0) dolink = 1;
    if (!doclear && !dolink && ends_excmd (*linep)) {
        id = syn_namen2id (line, (int) (name_end - line));
        if (id == 0) emsg2 ((char_u *) (((char *) ("E411: highlight group not found: %s"))), (char_u *) (line));
        else highlight_list_one (id);
        return;
    }
    if (dolink) {
        ...
        if (strncmp ((char *) (to_start), (char *) ("NONE"), (size_t) (4)) == 0) to_id = 0;
        else to_id = syn_check_group (to_start, (int) (to_end - to_start));
        if (from_id > 0 && (!init || ((struct hl_group *) ((highlight_ga.ga_data))) [from_id - 1].sg_set == 0)) {
            if (to_id > 0 && !forceit && !init && hl_has_settings (from_id - 1, dodefault)) {
                if (sourcing_name == ((void *) 0) && !dodefault) emsg ((char_u *) (((char *) ("E414: group has settings, highlight link ignored"))));
            }
            else {
                if (!init) ((struct hl_group *) ((highlight_ga.ga_data))) [from_id - 1].sg_set |= 8;
    ...
    if (doclear) {
        ...
        if (ends_excmd (*line)) {
            ...
            for (idx = 0; idx < highlight_ga.ga_len; ++idx) highlight_clear (idx);
            init_highlight (1, 1);
            highlight_changed ();
            redraw_later_clear ();
            return;
    ...
    if (id == 0) return;
    idx = id - 1;
    if (dodefault && hl_has_settings (idx, 1)) return;
    if (strcmp ((char *) (((struct hl_group *) ((highlight_ga.ga_data))) [idx].sg_name_u), (char *) ("NORMAL")) == 0) is_normal_group = 1;
    if (doclear || (forceit && init)) {
        highlight_clear (idx);
        if (!doclear) ((struct hl_group *) ((highlight_ga.ga_data))) [idx].sg_set = 0;
    }
    if (!doclear) while (!ends_excmd(*linep)) {
        key_start = linep;
        if (*linep == '=') {
        ...
        while (*linep && !((*linep) == ' ' || (*linep) == '\t') && *linep != '=') ++linep;
        vim_free (key);
        ...
        if (strcmp ((char *) (key), (char *) ("NONE")) == 0) {
            if (!init || ((struct hl_group *) ((highlight_ga.ga_data))) [idx].sg_set == 0) {
                if (!init) ((struct hl_group *) ((highlight_ga.ga_data))) [idx].sg_set |= 1 + 2 + 4;
                highlight_clear (idx);
            }
            continue;
        }
        ...
        if (*linep == '\'') ++linep;
        if (strcmp ((char *) (key), (char *) ("TERM")) == 0 || strcmp ((char *) (key), (char *) ("CTERM")) == 0 || strcmp ((char *) (key), (char *) ("GUI")) ==
          0) {
            attr = 0;
            off = 0;
            while (arg [off] != '\000') {
            ...
            if (error) break;
            if (*key == 'T') {
                if (!init || !(((structhl_group*)((highlight_ga.ga_data)))[idx].sg_set&1)) {
                    if (!init) ((struct hl_group *) ((highlight_ga.ga_data))) [idx].sg_set |= 1;
                    ((struct hl_group *) ((highlight_ga.ga_data))) [idx].sg_term = attr;
                }
            }
            else if (*key == 'C') {
                if (!init || !(((structhl_group*)((highlight_ga.ga_data)))[idx].sg_set&2)) {
                ...
                if (!init || !(((structhl_group*)((highlight_ga.ga_data)))[idx].sg_set&4)) {
            ...
            if (!init || !(((structhl_group*)((highlight_ga.ga_data)))[idx].sg_set&2)) {
                ...
                if (((*arg) >= '0' && (*arg) <= '9')) color = atoi ((char *) arg);
                else if (strcasecmp ((char *) (arg), (char *) ("fg")) == 0) {
                    if (cterm_normal_fg_color) color = cterm_normal_fg_color - 1;
                    else {
                    ...
                    if (cterm_normal_bg_color > 0) color = cterm_normal_bg_color - 1;
                    else {
                    ...
                    {"Black", "DarkBlue", "DarkGreen", "DarkCyan", "DarkRed", "DarkMagenta", "Brown", "DarkYellow", "Gray", "Grey", "LightGray", "LightGrey",
                          "DarkGray", "DarkGrey", "Blue", "LightBlue", "Green", "LightGreen", "Cyan", "LightCyan", "Red", "LightRed", "Magenta", "LightMagenta"
                    ...
                    {0, 4, 2, 6, 1, 5, 130, 130, 248, 248, 7, 7, 242, 242, 12, 81, 10, 121, 14, 159, 9, 224, 13, 225, 11, 229, 15, -1};
                    static int color_numbers_8 [28] =
                    {0, 4, 2, 6, 1, 5, 3, 3, 7, 7, 7, 7, 0 + 8, 0 + 8, 4 + 8, 4 + 8, 2 + 8, 2 + 8, 6 + 8, 6 + 8, 1 + 8, 1 + 8, 5 + 8, 5 + 8, 3 + 8, 3 + 8, 7 + 8
                          , -1};
                    off = (((*arg) < 'a' || (*arg) > 'z') ? (*arg) : (*arg) - ('a' - 'A'));
                    for (i = (sizeof (color_names) / sizeof (char *)); --i >= 0;) if (off == color_names [i] [0] && strcasecmp ((char *) (arg + 1), (char *) (
                      color_names [i] + 1)) == 0) break;
                    if (i < 0) {
                    ...
                    if (color >= 0) {
                        if (t_colors == 8) {
                            ...
                            if (key [5] == 'F') {
                                ...
                                else ((struct hl_group *) ((highlight_ga.ga_data))) [idx].sg_cterm &= ~0x02;
                            }
                            color &= 7;
                        }
                        else if (t_colors == 16 || t_colors == 88 || t_colors == 256) {
                            if (*(term_strings[(int)(KS_CAF)]) != '\000') p = (term_strings [(int) (KS_CAF)]);
                            else p = (term_strings [(int) (KS_CSF)]);
                            if (*p != '\000' && *(p+strlen((char*)(p))-1) == 'm') switch (t_colors) {
                            case 16 :
                                color = color_numbers_8 [i];
                                break;
                            case 88 :
                ...
                if (key [5] == 'F') {
                    ...
                    if (is_normal_group) {
                        ...
                        {
                            must_redraw = 50;
                            if (termcap_active && color >= 0) term_fg_color (color);
                        }
                    }
                }
                else {
                    ...
                    if (is_normal_group) {
                        cterm_normal_bg_color = color + 1;
                        {
                            must_redraw = 50;
                            if (color >= 0) {
                                if (termcap_active) term_bg_color (color);
                                if (t_colors < 16) i = (color == 0 || color == 4);
                                else i = (color < 7 || color == 8);
                                if (i != (*p_bg == 'd')) set_option_value ((char_u *) "bg", 0L, i ? (char_u *) "dark" : (char_u *) "light", 0);
                            }
                        }
                    }
                }
            ...
            if (!init || !(((structhl_group*)((highlight_ga.ga_data)))[idx].sg_set&4)) {
                ...
                if (strcmp ((char *) (arg), (char *) ("NONE"))) ((struct hl_group *) ((highlight_ga.ga_data))) [idx].sg_gui_fg_name = vim_strsave (arg);
                else ((struct hl_group *) ((highlight_ga.ga_data))) [idx].sg_gui_fg_name = ((void *) 0);
            }
        }
            ...
            if (!init || !(((structhl_group*)((highlight_ga.ga_data)))[idx].sg_set&4)) {
                ...
                if (strcmp ((char *) (arg), (char *) ("NONE")) != 0) ((struct hl_group *) ((highlight_ga.ga_data))) [idx].sg_gui_bg_name = vim_strsave (arg);
                else ((struct hl_group *) ((highlight_ga.ga_data))) [idx].sg_gui_bg_name = ((void *) 0);
            }
        }
            ...
            if (!init || !(((structhl_group*)((highlight_ga.ga_data)))[idx].sg_set&4)) {
                ...
                if (strcmp ((char *) (arg), (char *) ("NONE")) != 0) ((struct hl_group *) ((highlight_ga.ga_data))) [idx].sg_gui_sp_name = vim_strsave (arg);
                else ((struct hl_group *) ((highlight_ga.ga_data))) [idx].sg_gui_sp_name = ((void *) 0);
            }
        }
            ...
            if (strncmp ((char *) (arg), (char *) ("t_"), (size_t) (2)) == 0) {
                ...
                while (arg [off] != '\000') {
                    ...
                    vim_free (tname);
                    if (p == ((void *) 0)) p = (char_u *) "";
                    if ((int) (strlen ((char *) (buf)) + strlen ((char *) (p))) >= 99) {
                        emsg2 ((char_u *) (((char *) ("E422: terminal code too long: %s"))), (char_u *) (arg));
                        error = 1;
                        break;
                    }
                    ((__builtin_object_size ((char *) (buf), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (buf), (char *) (p), __builtin_object_size (
                      (char *) (buf), 1 > 1)) : __inline_strcat_chk ((char *) (buf), (char *) (p)));
                    off += len;
                    if (arg [off] == ',') ++off;
                }
            }
            ...
            if (error) break;
            if (strcmp ((char *) (buf), (char *) ("NONE")) == 0) p = ((void *) 0);
            else p = vim_strsave (buf);
            if (key [2] == 'A') {
        ...
        if (!init || !(((structhl_group*)((highlight_ga.ga_data)))[idx].sg_set&8)) ((struct hl_group *) ((highlight_ga.ga_data))) [idx].sg_link = 0;
        linep = skipwhite (linep);
    }
    if (error && idx == highlight_ga.ga_len) syn_unadd_group ();
    else {
        if (is_normal_group) {
            ((struct hl_group *) ((highlight_ga.ga_data))) [idx].sg_term_attr = 0;
            ((struct hl_group *) ((highlight_ga.ga_data))) [idx].sg_cterm_attr = 0;
        }
        else set_hl_attr (idx);
...
static int hl_has_settings (idx, check_link)
    ...
    int check_link;
{
    return (((struct hl_group *) ((highlight_ga.ga_data))) [idx].sg_term_attr != 0 || ((struct hl_group *) ((highlight_ga.ga_data))) [idx].sg_cterm_attr != 0 ||
    ...
    int idx;
{
    ...
    if (((struct hl_group *) ((highlight_ga.ga_data))) [idx].sg_link == 0) ((struct hl_group *) ((highlight_ga.ga_data))) [idx].sg_scriptID = 0;
}

static garray_T term_attr_table =
{0, 0, 0, 0, ((void *) 0)};
static garray_T cterm_attr_table =
{0, 0, 0, 0, ((void *) 0)};
    ...
    attrentry_T * aep;
{
    ...
    for (i = 0; i < table -> ga_len; ++i) {
        ...
        if (aep -> ae_attr == taep -> ae_attr && ((table == &term_attr_table && (aep -> ae_u.term.start == ((void *) 0)) == (taep -> ae_u.term.start == ((void *
          ) 0)) && (aep -> ae_u.term.start == ((void *) 0) || strcmp ((char *) (aep -> ae_u.term.start), (char *) (taep -> ae_u.term.start)) == 0) && (aep ->
          ae_u.term.stop == ((void *) 0)) == (taep -> ae_u.term.stop == ((void *) 0)) && (aep -> ae_u.term.stop == ((void *) 0) || strcmp ((char *) (aep -> ae_u
          .term.stop), (char *) (taep -> ae_u.term.stop)) == 0)) || (table == &cterm_attr_table && aep -> ae_u.cterm.fg_color == taep -> ae_u.cterm.fg_color &&
          aep -> ae_u.cterm.bg_color == taep -> ae_u.cterm.bg_color))) return i + (0x3f + 1);
    }
    if (table -> ga_len + (0x3f + 1) > 65535) {
        ...
        clear_hl_tables ();
        must_redraw = 50;
        for (i = 0; i < highlight_ga.ga_len; ++i) set_hl_attr (i);
        recursive = 0;
    }
    if (ga_grow (table, 1) == 0) return 0;
    taep = &(((attrentry_T*)table->ga_data)[table->ga_len]);
    ((__builtin_object_size ((taep), 0) != (size_t) - 1) ? __builtin___memset_chk ((taep), (0), (sizeof (attrentry_T)), __builtin_object_size ((taep), 0)) :
      __inline_memset_chk ((taep), (0), (sizeof (attrentry_T))));
    taep -> ae_attr = aep -> ae_attr;
    if (table == &term_attr_table) {
        if (aep -> ae_u.term.start == ((void *) 0)) taep -> ae_u.term.start = ((void *) 0);
        else taep -> ae_u.term.start = vim_strsave (aep -> ae_u.term.start);
        if (aep -> ae_u.term.stop == ((void *) 0)) taep -> ae_u.term.stop = ((void *) 0);
        else taep -> ae_u.term.stop = vim_strsave (aep -> ae_u.term.stop);
    }
    ...
    int prim_attr;
{
    ...
    attrentry_T new_en;
    if (char_attr == 0) return prim_attr;
    if (char_attr <= 0x3f && prim_attr <= 0x3f) return char_attr | prim_attr;
    if (t_colors > 1) {
        if (char_attr > 0x3f) char_aep = syn_cterm_attr2entry (char_attr);
        if (char_aep != ((void *) 0)) new_en = *char_aep;
        else {
            ((__builtin_object_size ((&new_en), 0) != (size_t) - 1) ? __builtin___memset_chk ((&new_en), (0), (sizeof (new_en)), __builtin_object_size 
              ((&new_en), 0)) : __inline_memset_chk ((&new_en), (0), (sizeof (new_en))));
            if (char_attr <= 0x3f) new_en.ae_attr = char_attr;
        }
        if (prim_attr <= 0x3f) new_en.ae_attr |= prim_attr;
        else {
            spell_aep = syn_cterm_attr2entry (prim_attr);
            if (spell_aep != ((void *) 0)) {
                new_en.ae_attr |= spell_aep -> ae_attr;
                if (spell_aep -> ae_u.cterm.fg_color > 0) new_en.ae_u.cterm.fg_color = spell_aep -> ae_u.cterm.fg_color;
                if (spell_aep -> ae_u.cterm.bg_color > 0) new_en.ae_u.cterm.bg_color = spell_aep -> ae_u.cterm.bg_color;
            }
        }
        return get_attr_entry (&cterm_attr_table, &new_en);
    }
    if (char_attr > 0x3f) char_aep = syn_term_attr2entry (char_attr);
    if (char_aep != ((void *) 0)) new_en = *char_aep;
    else {
        ((__builtin_object_size ((&new_en), 0) != (size_t) - 1) ? __builtin___memset_chk ((&new_en), (0), (sizeof (new_en)), __builtin_object_size ((&new_en), 0
          )) : __inline_memset_chk ((&new_en), (0), (sizeof (new_en))));
        if (char_attr <= 0x3f) new_en.ae_attr = char_attr;
    }
    if (prim_attr <= 0x3f) new_en.ae_attr |= prim_attr;
    else {
...
{
    attrentry_T * aep;
    if (t_colors > 1) aep = syn_cterm_attr2entry (attr);
    else aep = syn_term_attr2entry (attr);
    if (aep == ((void *) 0)) return 0;
    return aep -> ae_attr;
}
...
{
    attr -= (0x3f + 1);
    if (attr >= term_attr_table.ga_len) return ((void *) 0);
...
{
    attr -= (0x3f + 1);
    if (attr >= cterm_attr_table.ga_len) return ((void *) 0);
    ...
    int id;
{
    struct hl_group * sgp;
    int didh = 0;
    sgp = &((structhl_group*)((highlight_ga.ga_data)))[id-1];
    didh = highlight_list_arg (id, didh, 1, sgp -> sg_term, ((void *) 0), "term");
    didh = highlight_list_arg (id, didh, 2, 0, sgp -> sg_start, "start");
    didh = highlight_list_arg (id, didh, 2, 0, sgp -> sg_stop, "stop");
    didh = highlight_list_arg (id, didh, 1, sgp -> sg_cterm, ((void *) 0), "cterm");
    didh = highlight_list_arg (id, didh, 3, sgp -> sg_cterm_fg, ((void *) 0), "ctermfg");
    didh = highlight_list_arg (id, didh, 3, sgp -> sg_cterm_bg, ((void *) 0), "ctermbg");
    didh = highlight_list_arg (id, didh, 1, sgp -> sg_gui, ((void *) 0), "gui");
    didh = highlight_list_arg (id, didh, 2, 0, sgp -> sg_gui_fg_name, "guifg");
    didh = highlight_list_arg (id, didh, 2, 0, sgp -> sg_gui_bg_name, "guibg");
    didh = highlight_list_arg (id, didh, 2, 0, sgp -> sg_gui_sp_name, "guisp");
    ...
    if (!didh) highlight_list_arg (id, didh, 2, 0, (char_u *) "cleared", "");
    if (p_verbose > 0) last_set_msg (sgp -> sg_scriptID);
}
    ...
    char * name;
{
    ...
    if (type == 2 ? (sarg != ((void *) 0)) : (iarg != 0)) {
        ts = buf;
        if (type == 3) __builtin___sprintf_chk ((char *) buf, 0, __builtin_object_size ((char *) buf, 1 > 1), "%d", iarg - 1);
        else if (type == 2) ts = sarg;
        else {
            buf [0] = '\000';
            for (i = 0; hl_attr_table [i] != 0; ++i) {
                if (iarg & hl_attr_table [i]) {
                    if (buf [0] != '\000') ((__builtin_object_size ((char *) (buf), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (buf), (char *) (","
                      ), __builtin_object_size ((char *) (buf), 1 > 1)) : __inline_strcat_chk ((char *) (buf), (char *) (",")));
                    ((__builtin_object_size ((char *) (buf), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (buf), (char *) (hl_name_table [i]),
                      __builtin_object_size ((char *) (buf), 1 > 1)) : __inline_strcat_chk ((char *) (buf), (char *) (hl_name_table [i])));
...
{
    int attr;
    if (id <= 0 || id > highlight_ga.ga_len) return ((void *) 0);
    if (modec == 'g') attr = ((struct hl_group *) ((highlight_ga.ga_data))) [id - 1].sg_gui;
    else if (modec == 'c') attr = ((struct hl_group *) ((highlight_ga.ga_data))) [id - 1].sg_cterm;
    else attr = ((struct hl_group *) ((highlight_ga.ga_data))) [id - 1].sg_term;
    if (attr & flag) return (char_u *) "1";
    return ((void *) 0);
}
    ...
    int modec;
{
    ...
    int font = 0;
    if (id <= 0 || id > highlight_ga.ga_len) return ((void *) 0);
    if ((((what [0]) < 'A' || (what [0]) > 'Z') ? (what [0]) : (what [0]) + ('a' - 'A')) == 'f' && (((what [1]) < 'A' || (what [1]) > 'Z') ? (what [1]) : (what
      [1]) + ('a' - 'A')) == 'g') fg = 1;
    else if ((((what [0]) < 'A' || (what [0]) > 'Z') ? (what [0]) : (what [0]) + ('a' - 'A')) == 'f' && (((what [1]) < 'A' || (what [1]) > 'Z') ? (what [1]) : (
      what [1]) + ('a' - 'A')) == 'o' && (((what [2]) < 'A' || (what [2]) > 'Z') ? (what [2]) : (what [2]) + ('a' - 'A')) == 'n' && (((what [3]) < 'A' || (what
      [3]) > 'Z') ? (what [3]) : (what [3]) + ('a' - 'A')) == 't') font = 1;
    else if ((((what [0]) < 'A' || (what [0]) > 'Z') ? (what [0]) : (what [0]) + ('a' - 'A')) == 's' && (((what [1]) < 'A' || (what [1]) > 'Z') ? (what [1]) : (
      what [1]) + ('a' - 'A')) == 'p') sp = 1;
    else if (!((((what[0])<'A'||(what[0])>'Z')?(what[0]):(what[0])+('a'-'A'))=='b'&&(((what[1])<'A'||(what[1])>'Z')?(what[1]):(what[1])+('a'-'A'))=='g')) return
      ((void *) 0);
    if (modec == 'g') {
        if (fg) return (((struct hl_group *) ((highlight_ga.ga_data))) [id - 1].sg_gui_fg_name);
        if (sp) return (((struct hl_group *) ((highlight_ga.ga_data))) [id - 1].sg_gui_sp_name);
        return (((struct hl_group *) ((highlight_ga.ga_data))) [id - 1].sg_gui_bg_name);
    }
    if (font || sp) return ((void *) 0);
    if (modec == 'c') {
        if (fg) n = ((struct hl_group *) ((highlight_ga.ga_data))) [id - 1].sg_cterm_fg - 1;
        else n = ((struct hl_group *) ((highlight_ga.ga_data))) [id - 1].sg_cterm_bg - 1;
    ...
    int idx;
{
    ...
    if (sgp -> sg_name_u != ((void *) 0) && strcmp ((char *) (sgp -> sg_name_u), (char *) ("NORMAL")) == 0) return;
    if (sgp -> sg_start == ((void *) 0) && sgp -> sg_stop == ((void *) 0)) sgp -> sg_term_attr = sgp -> sg_term;
    else {
    ...
    if (sgp -> sg_cterm_fg == 0 && sgp -> sg_cterm_bg == 0) sgp -> sg_cterm_attr = sgp -> sg_cterm;
    else {
    ...
    char_u * name;
{
    ...
    vim_strup (name_u);
    for (i = highlight_ga.ga_len; --i >= 0;) if (((struct hl_group *) ((highlight_ga.ga_data))) [i].sg_name_u != ((void *) 0) && strcmp ((char *) (name_u), (
      char *) (((struct hl_group *) ((highlight_ga.ga_data))) [i].sg_name_u)) == 0) break;
    return i + 1;
}
...
char_u * syn_id2name (id)
    int id;
{
    if (id <= 0 || id > highlight_ga.ga_len) return (char_u *) "";
    ...
    char_u * name;
{
    ...
    ((__builtin_object_size ((&(((structhl_group*)((highlight_ga.ga_data)))[highlight_ga.ga_len])), 0) != (size_t) - 1) ? __builtin___memset_chk 
      ((&(((structhl_group*)((highlight_ga.ga_data)))[highlight_ga.ga_len])), (0), (sizeof (struct hl_group)), __builtin_object_size 
      ((&(((structhl_group*)((highlight_ga.ga_data)))[highlight_ga.ga_len])), 0)) : __inline_memset_chk 
      ((&(((structhl_group*)((highlight_ga.ga_data)))[highlight_ga.ga_len])), (0), (sizeof (struct hl_group))));
    ...
    int hl_id;
{
    ...
    sgp = &((structhl_group*)((highlight_ga.ga_data)))[hl_id-1];
    if (t_colors > 1) attr = sgp -> sg_cterm_attr;
    else attr = sgp -> sg_term_attr;
    return attr;
}
    ...
    int hl_id;
{
    int count;
    struct hl_group * sgp;
    if (hl_id > highlight_ga.ga_len || hl_id < 1) return 0;
    for (count = 100; --count >= 0;) {
        sgp = &((structhl_group*)((highlight_ga.ga_data)))[hl_id-1];
        if (sgp -> sg_link == 0 || sgp -> sg_link > highlight_ga.ga_len) break;
        hl_id = sgp -> sg_link;
    }
    return hl_id;
}
...
int highlight_changed () {
    ...
    int id_S = -1;
    int hlcnt;
    static int hl_flags [HLF_COUNT] =
    {'8', '@', 'd', 'e', 'h', 'i', 'l', 'm', 'M', 'n', 'r', 's', 'S', 'c', 't', 'v', 'V', 'w', 'W', 'f', 'F', 'A', 'C', 'D', 'T', '-', '>', 'B', 'P', 'R', 'L',
          '+', '=', 'x', 'X', '*', '#', '_', '!', '.', 'o'};
    need_highlight_changed = 0;
    for (hlf = 0; hlf < (int) HLF_COUNT; ++hlf) highlight_attr [hlf] = 0;
    for (i = 0; i < 2; ++i) {
        if (i) p = p_hl;
        else p = get_highlight_default ();
        if (p == ((void *) 0)) continue;
        while (*p) {
            for (hlf = 0; hlf < (int) HLF_COUNT; ++hlf) if (hl_flags [hlf] == *p) break;
            ++p;
            if (hlf == (int) HLF_COUNT || *p == '\000') return 0;
            attr = 0;
            for (; *p && *p != ','; ++p) {
                if (((*p) == ' ' || (*p) == '\t')) continue;
                if (attr > 0x3f) return 0;
                switch (*p) {
                case 'b' :
                    attr |= 0x02;
                    break;
                ...
                case ':' :
                    ++p;
                    if (attr || *p == '\000') return 0;
                    end = vim_strchr (p, ',');
                    if (end == ((void *) 0)) end = p + strlen ((char *) (p));
                    id = syn_check_group (p, (int) (end - p));
                    if (id == 0) return 0;
                    attr = syn_id2attr (id);
                    p = end - 1;
                    if (hlf == (int) HLF_SNC) id_SNC = syn_get_final_id (id);
                    else if (hlf == (int) HLF_S) id_S = syn_get_final_id (id);
                    break;
                default :
                    return 0;
                }
    ...
    if (ga_grow (&highlight_ga, 10) == 0) return 0;
    hlcnt = highlight_ga.ga_len;
    if (id_S == 0) {
        ((__builtin_object_size ((&((structhl_group*)((highlight_ga.ga_data)))[hlcnt+9]), 0) != (size_t) - 1) ? __builtin___memset_chk 
          ((&((structhl_group*)((highlight_ga.ga_data)))[hlcnt+9]), (0), (sizeof (struct hl_group)), __builtin_object_size 
          ((&((structhl_group*)((highlight_ga.ga_data)))[hlcnt+9]), 0)) : __inline_memset_chk ((&((structhl_group*)((highlight_ga.ga_data)))[hlcnt+9]), (0), (
    ...
    for (i = 0; i < 9; i ++) {
        ...
        if (id == 0) {
            ...
            if (id_SNC == 0) {
                ((__builtin_object_size ((&hlt[hlcnt+i]), 0) != (size_t) - 1) ? __builtin___memset_chk ((&hlt[hlcnt+i]), (0), (sizeof (struct hl_group)),
                  __builtin_object_size ((&hlt[hlcnt+i]), 0)) : __inline_memset_chk ((&hlt[hlcnt+i]), (0), (sizeof (struct hl_group))));
            ...
            else ((__builtin_object_size ((char *) (&hlt[hlcnt+i]), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (&hlt[hlcnt+i]), (char *) 
              (&hlt[id_SNC-1]), sizeof (struct hl_group), __builtin_object_size ((char *) (&hlt[hlcnt+i]), 0)) : __inline_memmove_chk ((char *) (&hlt[hlcnt+i])
              , (char *) (&hlt[id_SNC-1]), sizeof (struct hl_group)));
            hlt [hlcnt + i].sg_link = 0;
            hlt [hlcnt + i].sg_term ^= hlt [id - 1].sg_term ^ hlt [id_S - 1].sg_term;
            if (hlt [id - 1].sg_start != hlt [id_S - 1].sg_start) hlt [hlcnt + i].sg_start = hlt [id - 1].sg_start;
            if (hlt [id - 1].sg_stop != hlt [id_S - 1].sg_stop) hlt [hlcnt + i].sg_stop = hlt [id - 1].sg_stop;
            hlt [hlcnt + i].sg_cterm ^= hlt [id - 1].sg_cterm ^ hlt [id_S - 1].sg_cterm;
            if (hlt [id - 1].sg_cterm_fg != hlt [id_S - 1].sg_cterm_fg) hlt [hlcnt + i].sg_cterm_fg = hlt [id - 1].sg_cterm_fg;
            if (hlt [id - 1].sg_cterm_bg != hlt [id_S - 1].sg_cterm_bg) hlt [hlcnt + i].sg_cterm_bg = hlt [id - 1].sg_cterm_bg;
    ...
    char_u * arg;
{
    ...
    if (*arg != '\000') {
        ...
        if (*p != '\000') {
            ...
            if (*p != '\000') {
                include_link = 0;
                if (arg [1] == 'i' && arg [0] == 'N') highlight_list ();
                if (strncmp ((char *) ("link"), (char *) (arg), (size_t) (p - arg)) == 0 || strncmp ((char *) ("clear"), (char *) (arg), (size_t) (p - arg)) ==
                  0) {
                    xp -> xp_pattern = skipwhite (p);
...
static void highlight_list () {
    int i;
    for (i = 10; --i >= 0;) highlight_list_two (i, highlight_attr [(int) (HLF_D)]);
    for (i = 40; --i >= 0;) highlight_list_two (99, 0);
}
...
char_u * get_highlight_name (xp, idx)
    ...
    int idx;
{
    if (idx == highlight_ga.ga_len && include_none != 0) return (char_u *) "none";
    if (idx == highlight_ga.ga_len + include_none && include_default != 0) return (char_u *) "default";
    if (idx == highlight_ga.ga_len + include_none + include_default && include_link != 0) return (char_u *) "link";
    if (idx == highlight_ga.ga_len + include_none + include_default + 1 && include_link != 0) return (char_u *) "clear";
    if (idx < 0 || idx >= highlight_ga.ga_len) return ((void *) 0);
