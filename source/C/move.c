    ...
    win_T * wp;
{
    ...
    for (; lnum <= wp -> w_buffer -> b_ml.ml_line_count; ++lnum) {
        ...
        else if (lnum == wp -> w_topline) n = plines_win_nofill (wp, lnum, 1) + wp -> w_topfill;
        else n = plines_win (wp, lnum, 1);
...
void update_topline_redraw () {
    update_topline ();
    if (must_redraw) update_screen (0);
}
...
void update_topline () {
    ...
    if (curwin -> w_valid & 0x80) return;
    if (mouse_dragging > 0) p_so = mouse_dragging - 1;
    old_topline = curwin -> w_topline;
    old_topfill = curwin -> w_topfill;
    if ((curbuf -> b_ml.ml_line_count == 1 && *ml_get((linenr_T)1) == '\000')) {
        if (curwin -> w_topline != 1) redraw_later (40);
        curwin -> w_topline = 1;
        curwin -> w_botline = 2;
        ...
        if (curwin -> w_topline > 1) {
            if (curwin -> w_cursor.lnum < curwin -> w_topline) check_topline = 1;
            else if (check_top_offset ()) check_topline = 1;
        }
        if (!check_topline && curwin -> w_topfill > diff_check_fill (curwin, curwin -> w_topline)) check_topline = 1;
        if (check_topline) {
            ...
            if (hasAnyFolding (curwin)) {
                ...
                for (lnum = curwin -> w_cursor.lnum; lnum < curwin -> w_topline + p_so; ++lnum) {
                    ++n;
                    if (lnum >= curbuf -> b_ml.ml_line_count || n >= halfheight) break;
                    (void) hasFolding (lnum, ((void *) 0), &lnum);
                }
            }
            else n = curwin -> w_topline + p_so - curwin -> w_cursor.lnum;
            if (n >= halfheight) scroll_cursor_halfway (0);
            else {
    ...
    if (check_botline) {
        if (!(curwin->w_valid&0x40)) validate_botline ();
        if (curwin -> w_botline <= curbuf -> b_ml.ml_line_count) {
            if (curwin -> w_cursor.lnum < curwin -> w_botline) {
                if (((long) curwin -> w_cursor.lnum >= (long) curwin -> w_botline - p_so || hasAnyFolding (curwin))) {
                    ...
                    if (n >= p_so) check_botline = 0;
                }
                else check_botline = 0;
            }
            if (check_botline) {
                if (hasAnyFolding (curwin)) {
                    line_count = 0;
                ...
                else line_count = curwin -> w_cursor.lnum - curwin -> w_botline + 1 + p_so;
                if (line_count <= curwin -> w_height + 1) scroll_cursor_bot (scrolljump_value (), 0);
                else scroll_cursor_halfway (0);
            }
        }
    }
    curwin -> w_valid |= 0x80;
    if (curwin -> w_topline != old_topline || curwin -> w_topfill != old_topfill) {
        ...
        else redraw_later (10);
        if (curwin -> w_cursor.lnum == curwin -> w_topline) validate_cursor ();
    }
    p_so = save_so;
}

static int scrolljump_value () {
    if (p_sj >= 0) return (int) p_sj;
...
static int check_top_offset () {
    ...
    if (curwin -> w_cursor.lnum < curwin -> w_topline + p_so || hasAnyFolding (curwin)) {
        ...
        while (n < p_so) {
            topline_back (&loff);
            if (loff.lnum < curwin -> w_topline || (loff.lnum == curwin -> w_topline && loff.fill > 0)) break;
            n += loff.height;
        }
        if (n < p_so) return 1;
    }
    ...
    win_T * wp;
{
    if (wp -> w_cursor.lnum != wp -> w_valid_cursor.lnum) {
        wp -> w_valid &= ~(0x01|0x02|0x04|0x08|0x10|0x80);
...
void changed_cline_bef_curs () {
    curwin -> w_valid &= ~(0x01|0x02|0x04|0x08|0x80);
}

void changed_cline_bef_curs_win (wp)
    win_T * wp;
{
    wp -> w_valid &= ~(0x01|0x02|0x04|0x08|0x80);
}

void changed_line_abv_curs () {
    curwin -> w_valid &= ~(0x01|0x02|0x04|0x10|0x08|0x80);
}

void changed_line_abv_curs_win (wp)
    win_T * wp;
{
    wp -> w_valid &= ~(0x01|0x02|0x04|0x10|0x08|0x80);
}

void validate_botline () {
    if (!(curwin->w_valid&0x20)) comp_botline (curwin);
}

static void validate_botline_win (wp)
    win_T * wp;
{
    if (!(wp->w_valid&0x20)) comp_botline (wp);
}
...
void validate_cursor () {
    check_cursor_moved (curwin);
    if ((curwin -> w_valid & (0x02 | 0x01)) != (0x02 | 0x01)) curs_columns (1);
}
    ...
    int do_botline;
{
    ...
    for (lnum = wp -> w_topline; lnum < wp -> w_cursor.lnum; ++i) {
        ...
        if (!all_invalid && i < wp -> w_lines_valid) {
            if (wp -> w_lines [i].wl_lnum < lnum || !wp->w_lines[i].wl_valid) continue;
            if (wp -> w_lines [i].wl_lnum == lnum) {
                if (!wp->w_buffer->b_mod_set || wp -> w_lines [i].wl_lastlnum < wp -> w_cursor.lnum || wp -> w_buffer -> b_mod_top > wp -> w_lines [i].
                  wl_lastlnum + 1) valid = 1;
            }
            else if (wp -> w_lines [i].wl_lnum > lnum) --i;
        }
        if (valid && (lnum != wp -> w_topline || !wp->w_onebuf_opt.wo_diff)) {
            lnum = wp -> w_lines [i].wl_lastlnum + 1;
            if (lnum > wp -> w_cursor.lnum) break;
            ...
            else if (lnum == wp -> w_topline) wp -> w_cline_row += plines_win_nofill (wp, lnum ++, 1) + wp -> w_topfill;
            else wp -> w_cline_row += plines_win (wp, lnum ++, 1);
        }
    }
    check_cursor_moved (wp);
    if (!(wp->w_valid&0x08)) {
        if (all_invalid || i == wp -> w_lines_valid || (i < wp -> w_lines_valid && (!wp->w_lines[i].wl_valid || wp -> w_lines [i].wl_lnum != wp -> w_cursor.
          lnum))) {
            if (wp -> w_cursor.lnum == wp -> w_topline) wp -> w_cline_height = plines_win_nofill (wp, wp -> w_cursor.lnum, 1) + wp -> w_topfill;
            else wp -> w_cline_height = plines_win (wp, wp -> w_cursor.lnum, 1);
            wp -> w_cline_folded = hasFoldingWin (wp, wp -> w_cursor.lnum, ((void *) 0), ((void *) 0), 1, ((void *) 0));
        }
    ...
    wp -> w_valid |= 0x10 | 0x08;
    if (do_botline && all_invalid) validate_botline_win (wp);
}
    ...
    win_T * wp;
{
    ...
    if (!(wp->w_valid&0x04)) {
        ...
        wp -> w_valid |= 0x04;
        if (wp -> w_onebuf_opt.wo_cuc && !pum_visible()) redraw_win_later (wp, 35);
    }
}
...
static void validate_cheight () {
    ...
    if (!(curwin->w_valid&0x08)) {
        if (curwin -> w_cursor.lnum == curwin -> w_topline) curwin -> w_cline_height = plines_nofill (curwin -> w_cursor.lnum) + curwin -> w_topfill;
...
void validate_cursor_col () {
    ...
    if (!(curwin->w_valid&0x02)) {
        ...
        if (curwin -> w_onebuf_opt.wo_wrap && col >= (colnr_T) (curwin -> w_width) && width > 0) col -= ((col - (curwin -> w_width)) / width + 1) * width;
        if (col > (int) curwin -> w_leftcol) col -= curwin -> w_leftcol;
        else col = 0;
        curwin -> w_wcol = col;
        curwin -> w_valid |= 0x02;
    }
...
int win_col_off (wp)
    win_T * wp;
{
    return (((wp -> w_onebuf_opt.wo_nu || wp -> w_onebuf_opt.wo_rnu) ? number_width (wp) + 1 : 0) + (cmdwin_type == 0 || wp != curwin ? 0 : 1) + wp ->
      w_onebuf_opt.wo_fdc + (netbeans_active () || wp -> w_buffer -> b_signlist != ((void *) 0) ? 2 : 0));
}
...
int win_col_off2 (wp)
    win_T * wp;
{
    if ((wp -> w_onebuf_opt.wo_nu || wp -> w_onebuf_opt.wo_rnu) && vim_strchr (p_cpo, 'n') != ((void *) 0)) return number_width (wp) + 1;
    return 0;
}
    ...
    int scroll;
{
    ...
    update_topline ();
    if (!(curwin->w_valid&0x10)) curs_rows (curwin, 0);
    if (curwin -> w_cline_folded) startcol = curwin -> w_virtcol = endcol = curwin -> w_leftcol;
    else getvvcol (curwin, &curwin->w_cursor, &startcol, &(curwin->w_virtcol), &endcol);
    if (startcol > dollar_vcol) dollar_vcol = 0;
    ...
    if (textwidth <= 0) {
        ...
        if (curwin -> w_wcol >= (curwin -> w_width)) {
            ...
            curwin -> w_wrow += n;
            if (*p_sbr && *ml_get_cursor() == '\000' && curwin -> w_wcol == (int) vim_strsize (p_sbr)) curwin -> w_wcol = 0;
        }
    }
    else if (scroll && !curwin->w_cline_folded) {
        off_left = (int) startcol - (int) curwin -> w_leftcol - p_siso;
        off_right = (int) endcol - (int) (curwin -> w_leftcol + (curwin -> w_width) - p_siso) + 1;
        if (off_left < 0 || off_right > 0) {
            if (off_left < 0) diff = -off_left;
            else diff = off_right;
            if (p_ss == 0 || diff >= textwidth / 2 || off_right >= off_left) new_leftcol = curwin -> w_wcol - extra - textwidth / 2;
            else {
                if (diff < p_ss) diff = p_ss;
                if (off_left < 0) new_leftcol = curwin -> w_leftcol - diff;
                else new_leftcol = curwin -> w_leftcol + diff;
            }
            if (new_leftcol < 0) new_leftcol = 0;
    ...
    else if (curwin -> w_wcol > (int) curwin -> w_leftcol) curwin -> w_wcol -= curwin -> w_leftcol;
    else curwin -> w_wcol = 0;
    if (curwin -> w_cursor.lnum == curwin -> w_topline) curwin -> w_wrow += curwin -> w_topfill;
    else curwin -> w_wrow += diff_check_fill (curwin, curwin -> w_cursor.lnum);
    prev_skipcol = curwin -> w_skipcol;
    p_lines = 0;
    if ((curwin -> w_wrow >= curwin -> w_height || ((prev_skipcol > 0 || curwin -> w_wrow + p_so >= curwin -> w_height) && (p_lines = plines_win_nofill (curwin
      , curwin -> w_cursor.lnum, 0)) - 1 >= curwin -> w_height)) && curwin -> w_height != 0 && curwin -> w_cursor.lnum == curwin -> w_topline && width > 0 &&
      curwin -> w_width != 0) {
        extra = 0;
        if (curwin -> w_skipcol + p_so * width > curwin -> w_virtcol) extra = 1;
        if (p_lines == 0) p_lines = plines_win (curwin, curwin -> w_cursor.lnum, 0);
        --p_lines;
        if (p_lines > curwin -> w_wrow + p_so) n = curwin -> w_wrow + p_so;
        else n = p_lines;
        if ((colnr_T) n >= curwin -> w_height + curwin -> w_skipcol / width) extra += 2;
        if (extra == 3 || p_lines < p_so * 2) {
            n = curwin -> w_virtcol / width;
            if (n > curwin -> w_height / 2) n -= curwin -> w_height / 2;
            else n = 0;
            if (n > p_lines - curwin -> w_height + 1) n = p_lines - curwin -> w_height + 1;
            curwin -> w_skipcol = n * width;
        }
        else if (extra == 1) {
            extra = (curwin -> w_skipcol + p_so * width - curwin -> w_virtcol + width - 1) / width;
            if (extra > 0) {
                if ((colnr_T) (extra * width) > curwin -> w_skipcol) extra = curwin -> w_skipcol / width;
                curwin -> w_skipcol -= extra * width;
            }
        }
        else if (extra == 2) {
            endcol = (n - curwin -> w_height + 1) * width;
            while (endcol > curwin -> w_virtcol) endcol -= width;
            if (endcol > curwin -> w_skipcol) curwin -> w_skipcol = endcol;
        }
        ...
        if (extra > 0) win_ins_lines (curwin, 0, extra, 0, 0);
        else if (extra < 0) win_del_lines (curwin, 0, -extra, 0, 0);
    }
    else curwin -> w_skipcol = 0;
    if (prev_skipcol != curwin -> w_skipcol) redraw_later (40);
    if (((curwin -> w_valid & 0x01) == 0 && (curwin -> w_onebuf_opt.wo_rnu || curwin -> w_onebuf_opt.wo_cul)) || (curwin -> w_onebuf_opt.wo_cuc && (curwin ->
      w_valid & 0x04) == 0)) if (!pum_visible()) redraw_later (35);
    curwin -> w_valid |= 0x02 | 0x01 | 0x04;
}

void scrolldown (line_count, byfold)
    long line_count;
    int byfold __attribute__ ((unused));
{
    long done = 0;
    int wrow;
    int moved = 0;
    ...
    while (line_count -- > 0) {
        if (curwin -> w_topfill < diff_check (curwin, curwin -> w_topline) && curwin -> w_topfill < curwin -> w_height - 1) {
            ...
            if (curwin -> w_topline == 1) break;
            --curwin->w_topline;
            curwin -> w_topfill = 0;
            if (hasFolding (curwin -> w_topline, &first, ((void *) 0))) {
                ++done;
                if (!byfold) line_count -= curwin -> w_topline - first - 1;
    ...
    if (curwin -> w_cursor.lnum == curwin -> w_topline) curwin -> w_cline_row = 0;
    ...
    while (wrow >= curwin -> w_height && curwin -> w_cursor.lnum > 1) {
        if (hasFolding (curwin -> w_cursor.lnum, &first, ((void *) 0))) {
            --wrow;
            if (first == 1) curwin -> w_cursor.lnum = 1;
            else curwin -> w_cursor.lnum = first - 1;
        }
        else wrow -= plines (curwin -> w_cursor.lnum --);
        curwin -> w_valid &= ~(0x01|0x02|0x08|0x10|0x04);
        moved = 1;
    }
    if (moved) {
        foldAdjustCursor ();
...
{
    linenr_T lnum;
    if ((byfold && hasAnyFolding (curwin)) || curwin -> w_onebuf_opt.wo_diff) {
        lnum = curwin -> w_topline;
        while (line_count --) {
            if (curwin -> w_topfill > 0) --curwin->w_topfill;
            else {
                if (byfold) (void) hasFolding (lnum, ((void *) 0), &lnum);
    ...
    if (curwin -> w_botline > curbuf -> b_ml.ml_line_count + 1) curwin -> w_botline = curbuf -> b_ml.ml_line_count + 1;
    check_topfill (curwin, 0);
    if (hasAnyFolding (curwin)) (void) hasFolding (curwin -> w_topline, &curwin->w_topline, ((void *) 0));
    ...
    if (wp -> w_topfill > 0) {
        ...
        if (wp -> w_topfill + n > wp -> w_height) {
            if (down && wp -> w_topline > 1) {
                ...
                if (wp -> w_topfill < 0) wp -> w_topfill = 0;
            }
        }
    }
}
...
static void max_topfill () {
    ...
    if (n >= curwin -> w_height) curwin -> w_topfill = 0;
    else {
        curwin -> w_topfill = diff_check_fill (curwin, curwin -> w_topline);
        if (curwin -> w_topfill + n > curwin -> w_height) curwin -> w_topfill = curwin -> w_height - n;
    }
}
...
void scrolldown_clamp () {
    ...
    if (curwin -> w_topline <= 1 && !can_fill) return;
    validate_cursor ();
...
void scrollup_clamp () {
    int start_row;
    if (curwin -> w_topline == curbuf -> b_ml.ml_line_count && curwin -> w_topfill == 0) return;
    validate_cursor ();
    ...
    lineoff_T * lp;
{
    if (lp -> fill < diff_check_fill (curwin, lp -> lnum)) {
        ...
        --lp->lnum;
        lp -> fill = 0;
        if (lp -> lnum < 1) lp -> height = (0x7fffffffL);
        else if (hasFolding (lp -> lnum, &lp->lnum, ((void *) 0))) lp -> height = 1;
        else {
    ...
    lineoff_T * lp;
{
    if (lp -> fill < diff_check_fill (curwin, lp -> lnum + 1)) {
        ...
        ++lp->lnum;
        lp -> fill = 0;
        if (lp -> lnum > curbuf -> b_ml.ml_line_count) lp -> height = (0x7fffffffL);
        else if (hasFolding (lp -> lnum, ((void *) 0), &lp->lnum)) lp -> height = 1;
        else {
            lp -> height = plines_nofill (lp -> lnum);
        }
    }
    ...
    int always;
{
    ...
    int off = p_so;
    if (mouse_dragging > 0) off = mouse_dragging - 1;
    validate_cheight ();
    used = curwin -> w_cline_height;
    if (curwin -> w_cursor.lnum < curwin -> w_topline) scrolled = used;
    ...
    extra += i;
    while (top > 0) {
        if (hasFolding (top, &top, ((void *) 0))) i = 1;
        else i = plines (top);
        used += i;
        if (extra + i <= off && bot < curbuf -> b_ml.ml_line_count) {
            if (hasFolding (bot, ((void *) 0), &bot)) ++used;
            else used += plines (bot);
        }
        if (used > curwin -> w_height) break;
        if (top < curwin -> w_topline) scrolled += i;
        if ((new_topline >= curwin -> w_topline || scrolled > min_scroll) && extra >= off) break;
        extra += i;
        new_topline = top;
        --top;
        ++bot;
    }
    if (used > curwin -> w_height) scroll_cursor_halfway (0);
    else {
        if (new_topline < curwin -> w_topline || always) curwin -> w_topline = new_topline;
        if (curwin -> w_topline > curwin -> w_cursor.lnum) curwin -> w_topline = curwin -> w_cursor.lnum;
        curwin -> w_topfill = diff_check_fill (curwin, curwin -> w_topline);
        if (curwin -> w_topfill > 0 && extra > off) {
            curwin -> w_topfill -= extra - off;
            if (curwin -> w_topfill < 0) curwin -> w_topfill = 0;
        }
        check_topfill (curwin, 0);
        if (curwin -> w_topline != old_topline || curwin -> w_topfill != old_topfill) curwin -> w_valid &= ~(0x01|0x10|0x20|0x40);
        curwin -> w_valid |= 0x80;
    }
}
...
{
    wp -> w_filler_rows = 0;
    if (used == 0) wp -> w_empty_rows = 0;
    else {
        wp -> w_empty_rows = wp -> w_height - used;
        if (wp -> w_botline <= wp -> w_buffer -> b_ml.ml_line_count) {
            wp -> w_filler_rows = diff_check_fill (wp, wp -> w_botline);
            if (wp -> w_empty_rows > wp -> w_filler_rows) wp -> w_empty_rows -= wp -> w_filler_rows;
            else {
    ...
    int set_topbot;
{
    ...
    if (set_topbot) {
        ...
        for (curwin -> w_topline = curwin -> w_botline; curwin -> w_topline > 1; curwin -> w_topline = loff.lnum) {
            ...
            topline_back (&loff);
            if (loff.height == (0x7fffffffL) || used + loff.height > curwin -> w_height) break;
            used += loff.height;
            curwin -> w_topfill = loff.fill;
        }
        set_empty_rows (curwin, used);
        curwin -> w_valid |= 0x20 | 0x40;
        if (curwin -> w_topline != old_topline || curwin -> w_topfill != old_topfill) curwin -> w_valid &= ~(0x01|0x10);
    }
    else validate_botline ();
    used = plines_nofill (cln);
    if (cln >= curwin -> w_botline) {
        scrolled = used;
        if (cln == curwin -> w_botline) scrolled -= curwin -> w_empty_rows;
    }
    ...
    while (loff.lnum > 1) {
        if ((((scrolled <= 0 || scrolled >= min_scroll) && extra >= (mouse_dragging > 0 ? mouse_dragging - 1 : p_so)) || boff.lnum + 1 > curbuf -> b_ml.
          ml_line_count) && loff.lnum <= curwin -> w_botline && (loff.lnum < curwin -> w_botline || loff.fill >= fill_below_window)) break;
        topline_back (&loff);
        if (loff.height == (0x7fffffffL)) used = (0x7fffffffL);
        else used += loff.height;
        if (used > curwin -> w_height) break;
        if (loff.lnum >= curwin -> w_botline && (loff.lnum > curwin -> w_botline || loff.fill <= fill_below_window)) {
            scrolled += loff.height;
            if (loff.lnum == curwin -> w_botline && boff.fill == 0) scrolled -= curwin -> w_empty_rows;
        }
        if (boff.lnum < curbuf -> b_ml.ml_line_count) {
            ...
            if (extra < (mouse_dragging > 0 ? mouse_dragging - 1 : p_so) || scrolled < min_scroll) {
                ...
                if (boff.lnum >= curwin -> w_botline || (boff.lnum + 1 == curwin -> w_botline && boff.fill > curwin -> w_filler_rows)) {
                    ...
                    if (boff.lnum == curwin -> w_botline && boff.fill == 0) scrolled -= curwin -> w_empty_rows;
                }
            }
        }
    }
    if (scrolled <= 0) line_count = 0;
    else if (used > curwin -> w_height) line_count = used;
    else {
        line_count = 0;
    ...
    if (line_count >= curwin -> w_height && line_count > min_scroll) scroll_cursor_halfway (0);
    else scrollup (line_count, 1);
    ...
    int atend;
{
    ...
    if (!hasFolding(topline,&curwin->w_topline, ((void *) 0))) curwin -> w_topline = topline;
...
void cursor_correct () {
    ...
    if (curwin -> w_topline == 1) {
        ...
        if (below_wanted > max_off) below_wanted = max_off;
    }
    validate_botline ();
    if (curwin -> w_botline == curbuf -> b_ml.ml_line_count + 1 && mouse_dragging == 0) {
        below_wanted = 0;
        max_off = (curwin -> w_height - 1) / 2;
        if (above_wanted > max_off) above_wanted = max_off;
    }
    cln = curwin -> w_cursor.lnum;
    if (cln >= curwin -> w_topline + above_wanted && cln < curwin -> w_botline - below_wanted && !hasAnyFolding(curwin)) return;
    ...
    while ((above < above_wanted || below < below_wanted) && topline < botline) {
        if (below < below_wanted && (below <= above || above >= above_wanted)) {
            if (hasFolding (botline, &botline, ((void *) 0))) ++below;
            else below += plines (botline);
            --botline;
        }
        if (above < above_wanted && (above < below || below >= below_wanted)) {
            if (hasFolding (topline, ((void *) 0), &topline)) ++above;
            else above += plines_nofill (topline);
            if (topline < botline) above += diff_check_fill (curwin, topline + 1);
            ++topline;
        }
    }
    if (topline == botline || botline == 0) curwin -> w_cursor.lnum = topline;
    else if (topline > botline) curwin -> w_cursor.lnum = botline;
    else {
        if (cln < topline && curwin -> w_topline > 1) {
            curwin -> w_cursor.lnum = topline;
            curwin -> w_valid &= ~(0x01|0x02|0x08|0x10);
        }
    ...
    long count;
{
    ...
    for (; count > 0; --count) {
        validate_botline ();
        if (dir == 1 ? ((curwin -> w_topline >= curbuf -> b_ml.ml_line_count - p_so) && curwin -> w_botline > curbuf -> b_ml.ml_line_count) : (curwin ->
          w_topline == 1 && curwin -> w_topfill == diff_check_fill (curwin, curwin -> w_topline))) {
            beep_flush ();
            retval = 0;
            break;
        }
        ...
        if (dir == 1) {
            if (firstwin == lastwin && p_window > 0 && p_window < Rows - 1) {
                if (p_window <= 2) ++curwin->w_topline;
                ...
                check_topfill (curwin, 0);
                curwin -> w_cursor.lnum = curwin -> w_topline;
                curwin -> w_valid &= ~(0x02|0x08|0x01|0x10|0x20|0x40);
            }
        }
        else {
            if (curwin -> w_topline == 1) {
            ...
            while (n <= curwin -> w_height && loff.lnum >= 1) {
                topline_back (&loff);
                if (loff.height == (0x7fffffffL)) n = (0x7fffffffL);
                else n += loff.height;
            }
            if (loff.lnum < 1) {
                curwin -> w_topline = 1;
                ...
                if (loff.lnum >= curwin -> w_topline && (loff.lnum > curwin -> w_topline || loff.fill >= curwin -> w_topfill)) {
                    ...
                    if (curwin -> w_topfill < diff_check_fill (curwin, curwin -> w_topline)) max_topfill ();
                    ...
                    curwin -> w_valid &= ~(0x02|0x08|0x01|0x10);
                }
                else {
                    curwin -> w_topline = loff.lnum;
                    curwin -> w_topfill = loff.fill;
    ...
    int dir;
{
    ...
    if (lp -> fill > 0) lp -> height = 1;
    else lp -> height = plines_nofill (lp -> lnum);
    h1 = lp -> height;
    if (h1 > min_height) return;
    loff0 = *lp;
    if (dir > 0) botline_forw (lp);
    ...
    h4 = lp -> height;
    if (h4 + h3 + h2 > min_height || h3 + h2 + h1 > min_height) *lp = loff1;
    else *lp = loff2;
    return;
}
    ...
    linenr_T Prenum;
{
    ...
    int n;
    int room;
    if (Prenum) curwin -> w_onebuf_opt.wo_scr = (Prenum > curwin -> w_height) ? curwin -> w_height : Prenum;
    n = (curwin -> w_onebuf_opt.wo_scr <= curwin -> w_height) ? curwin -> w_onebuf_opt.wo_scr : curwin -> w_height;
    validate_botline ();
    ...
    if (flag) {
        while (n > 0 && curwin -> w_botline <= curbuf -> b_ml.ml_line_count) {
            ...
            scrolled += i;
            if (curwin -> w_onebuf_opt.wo_diff) curwin -> w_valid &= ~(0x20|0x40);
            else {
                room += i;
                do {
        ...
        while (n > 0 && curwin -> w_topline > 1) {
            ...
            curwin -> w_valid &= ~(0x10|0x01|0x20|0x40);
            scrolled += i;
        ...
        if (n > 0) {
            if (curwin -> w_cursor.lnum <= (linenr_T) n) curwin -> w_cursor.lnum = 1;
...
void do_check_cursorbind () {
    ...
    for (curwin = firstwin; curwin; curwin = curwin -> w_next) {
        ...
        if (curwin != old_curwin && curwin -> w_onebuf_opt.wo_crb) {
            if (curwin -> w_onebuf_opt.wo_diff) curwin -> w_cursor.lnum = diff_get_corresponding_line (old_curbuf, line, curbuf, curwin -> w_cursor.lnum);
