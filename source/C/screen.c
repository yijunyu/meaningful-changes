...
static int screen_cur_row, screen_cur_col;
    ...
    int type;
{
    win_T * wp;
    for (wp = firstwin; wp != ((void *) 0); wp = wp -> w_next) {
        if (wp -> w_buffer == buf) redraw_win_later (wp, type);
    }
}

void redrawWinline (lnum, invalid)
    linenr_T lnum;
    int invalid __attribute__ ((unused));
{
    int i;
    if (curwin -> w_redraw_top == 0 || curwin -> w_redraw_top > lnum) curwin -> w_redraw_top = lnum;
    if (curwin -> w_redraw_bot == 0 || curwin -> w_redraw_bot < lnum) curwin -> w_redraw_bot = lnum;
    redraw_later (10);
    if (invalid) {
        i = find_wl_entry (curwin, lnum);
        if (i >= 0) curwin -> w_lines [i].wl_valid = 0;
    }
}
    ...
    int type;
{
    ...
    int did_one;
    if (!screen_valid(1)) return;
    if (must_redraw) {
        if (type < must_redraw) type = must_redraw;
        must_redraw = 0;
    }
    if (curwin -> w_lines_valid == 0 && type < 40) type = 40;
    if (!redrawing() || updating_screen) {
        redraw_later (type);
        must_redraw = type;
        if (type > 25) curwin -> w_lines_valid = 0;
        return;
    }
    updating_screen = 1;
    ++display_tick;
    if (msg_scrolled) {
        clear_cmdline = 1;
        if (msg_scrolled > Rows - 5) type = 50;
        else if (type != 50) {
            check_for_delay (0);
            if (screen_ins_lines (0, 0, msg_scrolled, (int) Rows, ((void *) 0)) == 0) type = 50;
            for (wp = firstwin; wp != ((void *) 0); wp = wp -> w_next) {
                if ((wp -> w_winrow) < msg_scrolled) {
                    if ((wp -> w_winrow) + wp -> w_height > msg_scrolled && wp -> w_redr_type < 30 && wp -> w_lines_valid > 0 && wp -> w_topline == wp ->
                      w_lines [0].wl_lnum) {
                        ...
                        wp -> w_redr_type = 40;
                        if ((wp -> w_winrow) + wp -> w_height + (wp -> w_status_height) <= msg_scrolled) wp -> w_redr_status = 1;
                    }
                }
            }
            redraw_cmdline = 1;
    ...
    if (clear_cmdline) check_for_delay (0);
    if (curwin -> w_redr_type < 40 && curwin -> w_nrwidth != ((curwin -> w_onebuf_opt.wo_nu || curwin -> w_onebuf_opt.wo_rnu) ? number_width (curwin) : 0))
      curwin -> w_redr_type = 40;
    if (type == 20) update_curswant ();
    if (curwin -> w_redr_type < type && !((type==10&&curwin->w_lines[0].wl_valid&&curwin->w_topfill==curwin->w_old_topfill&&curwin->w_botfill==curwin->
      w_old_botfill&&curwin->w_topline==curwin->w_lines[0].wl_lnum)||(type==20&&VIsual_active&&curwin->w_old_cursor_lnum==curwin->w_cursor.lnum&&curwin->
      w_old_visual_mode==VIsual_mode&&(curwin->w_valid&0x04)&&curwin->w_old_curswant==curwin->w_curswant))) curwin -> w_redr_type = type;
    if (redraw_tabline || type >= 40) draw_tabline ();
    for (wp = firstwin; wp != ((void *) 0); wp = wp -> w_next) {
        if (wp -> w_buffer -> b_mod_set) {
            win_T * wwp;
            for (wwp = firstwin; wwp != wp; wwp = wwp -> w_next) if (wwp -> w_buffer == wp -> w_buffer) break;
            if (wwp == wp && syntax_present (wp)) syn_stack_apply_changes (wp -> w_buffer);
        }
    }
    did_one = 0;
    ...
    for (wp = firstwin; wp != ((void *) 0); wp = wp -> w_next) {
        if (wp -> w_redr_type != 0) {
            ...
            if (!did_one) {
                did_one = 1;
                start_search_hl ();
                if (clip_star.available && clip_isautosel ()) clip_update_selection ();
            }
            win_update (wp);
        }
        if (wp -> w_redr_status) {
    ...
    end_search_hl ();
    for (wp = firstwin; wp != ((void *) 0); wp = wp -> w_next) wp -> w_buffer -> b_mod_set = 0;
    updating_screen = 0;
    if (clear_cmdline || redraw_cmdline) showmode ();
    if (!did_intro && (curbuf -> b_ml.ml_line_count == 1 && *ml_get((linenr_T)1) == '\000') && curbuf -> b_fname == ((void *) 0) && firstwin -> w_next == ((
      void *) 0) && vim_strchr (p_shm, 'I') == ((void *) 0)) intro_message (0);
    did_intro = 1;
}
...
static void update_finish () {
    if (redraw_cmdline) showmode ();
    end_search_hl ();
    updating_screen = 0;
}

void update_debug_sign (buf, lnum)
    buf_T * buf;
    linenr_T lnum;
{
    win_T * wp;
    ...
    for (wp = firstwin; wp != ((void *) 0); wp = wp -> w_next) {
        if (buf != ((void *) 0) && lnum > 0) {
            if (wp -> w_buffer == buf && lnum >= wp -> w_topline && lnum < wp -> w_botline) {
                if (wp -> w_redraw_top == 0 || wp -> w_redraw_top > lnum) wp -> w_redraw_top = lnum;
                if (wp -> w_redraw_bot == 0 || wp -> w_redraw_bot < lnum) wp -> w_redraw_bot = lnum;
                redraw_win_later (wp, 10);
            }
        }
        else redraw_win_later (wp, 10);
        if (wp -> w_redr_type != 0) doit = 1;
    }
    if (!doit || updating_screen) return;
    update_prepare ();
    for (wp = firstwin; wp; wp = wp -> w_next) {
        if (wp -> w_redr_type != 0) win_update (wp);
        if (wp -> w_redr_status) win_redr_status (wp);
    }
    update_finish ();
}
    ...
    win_T * wp;
{
    ...
    if (wp -> w_nrwidth != i) {
        ...
        if (wp -> w_redraw_bot != 0) mod_bot = wp -> w_redraw_bot + 1;
        else mod_bot = 0;
        wp -> w_redraw_top = 0;
        wp -> w_redraw_bot = 0;
        if (buf -> b_mod_set) {
            if (mod_top == 0 || mod_top > buf -> b_mod_top) {
                ...
                if (syntax_present (wp)) {
                    ...
                    if (mod_top < 1) mod_top = 1;
                }
            }
            if (mod_bot == 0 || mod_bot < buf -> b_mod_bot) mod_bot = buf -> b_mod_bot;
            if (search_hl.rm.regprog != ((void *) 0) && re_multiline (search_hl.rm.regprog)) top_to_mod = 1;
            else {
                cur = wp -> w_match_head;
                while (cur != ((void *) 0)) {
                    if (cur -> match.regprog != ((void *) 0) && re_multiline (cur -> match.regprog)) {
                        top_to_mod = 1;
                        break;
                    }
                    cur = cur -> next;
        ...
        if (mod_top != 0 && hasAnyFolding (wp)) {
            ...
            for (i = 0; i < wp -> w_lines_valid; ++i) if (wp -> w_lines [i].wl_valid) {
                if (wp -> w_lines [i].wl_lastlnum < mod_top) lnumt = wp -> w_lines [i].wl_lastlnum + 1;
                if (lnumb == (0x7fffffffL) && wp -> w_lines [i].wl_lnum >= mod_bot) {
                    lnumb = wp -> w_lines [i].wl_lnum;
                    if (wp -> w_onebuf_opt.wo_fdc > 0) ++lnumb;
                }
            }
            (void) hasFoldingWin (wp, mod_top, &mod_top, ((void *) 0), 1, ((void *) 0));
            if (mod_top > lnumt) mod_top = lnumt;
            --mod_bot;
            (void) hasFoldingWin (wp, mod_bot, ((void *) 0), &mod_bot, 1, ((void *) 0));
            ++mod_bot;
            if (mod_bot < lnumb) mod_bot = lnumb;
        }
        if (mod_top != 0 && mod_top < wp -> w_topline) {
            if (mod_bot > wp -> w_topline) mod_top = wp -> w_topline;
            else if (syntax_present (wp)) top_end = 1;
        }
        if (mod_top != 0 && buf -> b_mod_xlines != 0 && wp -> w_onebuf_opt.wo_nu) mod_bot = (0x7fffffffL);
    }
    if (type == 30) {
        j = 0;
        ...
        if (top_end == 0) type = 40;
        else type = 10;
    }
    if (screen_cleared) screen_cleared = 2;
    if ((type == 10 || type == 35 || type == 20 || type == 25) && !wp->w_botfill && !wp->w_old_botfill) {
        if (mod_top != 0 && wp -> w_topline == mod_top) {
        }
        else if (wp -> w_lines [0].wl_valid && (wp -> w_topline < wp -> w_lines [0].wl_lnum || (wp -> w_topline == wp -> w_lines [0].wl_lnum && wp -> w_topfill
          > wp -> w_old_topfill))) {
            if (hasAnyFolding (wp)) {
                linenr_T ln;
                j = 0;
                for (ln = wp -> w_topline; ln < wp -> w_lines [0].wl_lnum; ++ln) {
                    ++j;
                    if (j >= wp -> w_height - 2) break;
                    (void) hasFoldingWin (wp, ln, ((void *) 0), &ln, 1, ((void *) 0));
                }
            }
            else j = wp -> w_lines [0].wl_lnum - wp -> w_topline;
            if (j < wp -> w_height - 2) {
                i = plines_m_win (wp, wp -> w_topline, wp -> w_lines [0].wl_lnum - 1);
                if (wp -> w_lines [0].wl_lnum != wp -> w_topline) i += diff_check_fill (wp, wp -> w_lines [0].wl_lnum) - wp -> w_old_topfill;
                if (i < wp -> w_height - 2) {
                    if (i > 0) check_for_delay (0);
                    if (win_ins_lines (wp, 0, i, 0, wp == firstwin) == 1) {
                        if (wp -> w_lines_valid != 0) {
                            top_end = i;
                            scrolled_down = 1;
                            if ((wp -> w_lines_valid += j) > wp -> w_height) wp -> w_lines_valid = wp -> w_height;
                            for (idx = wp -> w_lines_valid; idx - j >= 0; idx --) wp -> w_lines [idx] = wp -> w_lines [idx - j];
                            while (idx >= 0) wp -> w_lines [idx --].wl_valid = 0;
                        }
                    }
                    else mid_start = 0;
                }
            ...
            if (j == -1) {
                ...
                if (wp -> w_lines [0].wl_lnum == wp -> w_topline) row += wp -> w_old_topfill;
                else row += diff_check_fill (wp, wp -> w_topline);
                row -= wp -> w_topfill;
                if (row > 0) {
                    check_for_delay (0);
                    if (win_del_lines (wp, 0, row, 0, wp == firstwin) == 1) bot_start = wp -> w_height - row;
                    else mid_start = 0;
                }
                if ((row == 0 || bot_start < 999) && wp -> w_lines_valid != 0) {
                    ...
                    if (wp -> w_onebuf_opt.wo_diff && bot_start > 0) wp -> w_lines [0].wl_size = plines_win_nofill (wp, wp -> w_topline, 1) + wp -> w_topfill;
                }
            }
        }
        if (mid_start == 0) {
            mid_end = wp -> w_height;
            if (lastwin == firstwin) {
                if (screen_cleared != 1) screenclear ();
                if (redraw_tabline) draw_tabline ();
            }
        }
        if (screen_cleared == 1) must_redraw = 0;
    }
    else {
        mid_start = 0;
        mid_end = wp -> w_height;
    ...
    if ((VIsual_active && buf == curwin -> w_buffer) || (wp -> w_old_cursor_lnum != 0 && type != 40)) {
        linenr_T from, to;
        if (VIsual_active) {
            if (VIsual_active && (VIsual_mode != wp -> w_old_visual_mode || type == 25)) {
                ...
                if (wp -> w_old_cursor_lnum < from) from = wp -> w_old_cursor_lnum;
                if (wp -> w_old_cursor_lnum > to) to = wp -> w_old_cursor_lnum;
                if (wp -> w_old_visual_lnum < from) from = wp -> w_old_visual_lnum;
                if (wp -> w_old_visual_lnum > to) to = wp -> w_old_visual_lnum;
            }
            else {
                ...
                if (VIsual.lnum != wp -> w_old_visual_lnum || VIsual.col != wp -> w_old_visual_col) {
                    if (wp -> w_old_visual_lnum < from && wp -> w_old_visual_lnum != 0) from = wp -> w_old_visual_lnum;
                    if (wp -> w_old_visual_lnum > to) to = wp -> w_old_visual_lnum;
                    if (VIsual.lnum < from) from = VIsual.lnum;
                    if (VIsual.lnum > to) to = VIsual.lnum;
                }
            }
            if (VIsual_mode == 22) {
                colnr_T fromc, toc;
                ...
                ++toc;
                if (curwin -> w_curswant == (0x7fffffffL)) toc = (0x7fffffffL);
                if (fromc != wp -> w_old_cursor_fcol || toc != wp -> w_old_cursor_lcol) {
                    if (from > VIsual.lnum) from = VIsual.lnum;
                    if (to < VIsual.lnum) to = VIsual.lnum;
                }
        ...
        if (from < wp -> w_topline) from = wp -> w_topline;
        if (wp -> w_valid & 0x20) {
            if (from >= wp -> w_botline) from = wp -> w_botline - 1;
            if (to >= wp -> w_botline) to = wp -> w_botline - 1;
        }
        if (mid_start > 0) {
            lnum = wp -> w_topline;
            idx = 0;
            srow = 0;
            if (scrolled_down) mid_start = top_end;
            else mid_start = 0;
            while (lnum < from && idx < wp -> w_lines_valid) {
                if (wp -> w_lines [idx].wl_valid) mid_start += wp -> w_lines [idx].wl_size;
                else if (!scrolled_down) srow += wp -> w_lines [idx].wl_size;
                ++idx;
                if (idx < wp -> w_lines_valid && wp -> w_lines [idx].wl_valid) lnum = wp -> w_lines [idx].wl_lnum;
                else ++lnum;
            }
            srow += mid_start;
            mid_end = wp -> w_height;
    ...
    for (;;) {
        ...
        srow = row;
        if (row < top_end || (row >= mid_start && row < mid_end) || top_to_mod || idx >= wp -> w_lines_valid || (row + wp -> w_lines [idx].wl_size > bot_start)
          || (mod_top != 0 && (lnum == mod_top || (lnum >= mod_top && (lnum < mod_bot || did_update == 3 || (did_update == 2 && syntax_present (wp) && ((
          foldmethodIsSyntax (wp) && hasAnyFolding (wp)) || syntax_check_changed (lnum)))))))) {
            if (lnum == mod_top) top_to_mod = 0;
            if (lnum == mod_top && mod_bot != (0x7fffffffL) && !(dollar_vcol!=0&&mod_bot==mod_top+1)) {
                int old_rows = 0;
                int new_rows = 0;
                int xtra_rows;
                linenr_T l;
                for (i = idx; i < wp -> w_lines_valid; ++i) {
                    if (wp -> w_lines [i].wl_valid && wp -> w_lines [i].wl_lnum == mod_bot) break;
                    old_rows += wp -> w_lines [i].wl_size;
                    if (wp -> w_lines [i].wl_valid && wp -> w_lines [i].wl_lastlnum + 1 == mod_bot) {
                        ++i;
                        while (i < wp -> w_lines_valid && !wp->w_lines[i].wl_valid) old_rows += wp -> w_lines [i ++].wl_size;
                        break;
                    }
                }
                if (i >= wp -> w_lines_valid) {
                    ...
                    for (l = lnum; l < mod_bot; ++l) {
                        if (hasFoldingWin (wp, l, ((void *) 0), &l, 1, ((void *) 0))) ++new_rows;
                        else if (l == wp -> w_topline) new_rows += plines_win_nofill (wp, l, 1) + wp -> w_topfill;
                        else new_rows += plines_win (wp, l, 1);
                        ++j;
                    ...
                    if (xtra_rows < 0) {
                        if (row - xtra_rows >= wp -> w_height - 2) mod_bot = (0x7fffffffL);
                        else {
                            check_for_delay (0);
                            if (win_del_lines (wp, row, -xtra_rows, 0, 0) == 0) mod_bot = (0x7fffffffL);
                            else bot_start = wp -> w_height + xtra_rows;
                        }
                    }
                    else if (xtra_rows > 0) {
                        if (row + xtra_rows >= wp -> w_height - 2) mod_bot = (0x7fffffffL);
                            ...
                            check_for_delay (0);
                            if (win_ins_lines (wp, row + old_rows, xtra_rows, 0, 0) == 0) mod_bot = (0x7fffffffL);
                            else if (top_end > row + old_rows) top_end += xtra_rows;
                        }
                    }
                    if (mod_bot != (0x7fffffffL) && i != j) {
                        if (j < i) {
                            int x = row + new_rows;
                            for (;;) {
                                if (i >= wp -> w_lines_valid) {
                            ...
                            if (bot_start > x) bot_start = x;
                        }
                        else {
                            j -= i;
                            wp -> w_lines_valid += j;
                            if (wp -> w_lines_valid > wp -> w_height) wp -> w_lines_valid = wp -> w_height;
                            for (i = wp -> w_lines_valid; i - j >= idx; --i) wp -> w_lines [i] = wp -> w_lines [i - j];
                            while (i >= idx) {
            ...
            else if (idx < wp -> w_lines_valid && wp -> w_lines [idx].wl_valid && wp -> w_lines [idx].wl_lnum == lnum && lnum > wp -> w_topline && 
              !(dy_flags&0x001) && srow + wp -> w_lines [idx].wl_size > wp -> w_height && diff_check_fill (wp, lnum) == 0) {
                row = wp -> w_height + 1;
            }
            else {
                prepare_search_hl (wp, lnum);
                if (syntax_last_parsed != 0 && syntax_last_parsed + 1 < lnum && syntax_present (wp)) syntax_end_parsing (syntax_last_parsed + 1);
            ...
            if (row > wp -> w_height) {
                if (dollar_vcol == 0) wp -> w_lines [idx].wl_size = plines_win (wp, lnum, 1);
                ++idx;
                break;
            }
            if (dollar_vcol == 0) wp -> w_lines [idx].wl_size = row - srow;
            ++idx;
            lnum += fold_count + 1;
        }
        else {
    ...
    if (idx > wp -> w_lines_valid) wp -> w_lines_valid = idx;
    if (syntax_last_parsed != 0 && syntax_present (wp)) syntax_end_parsing (syntax_last_parsed + 1);
    wp -> w_empty_rows = 0;
    wp -> w_filler_rows = 0;
    if (!eof && !didline) {
        if (lnum == wp -> w_topline) {
            ...
            screen_fill ((wp -> w_winrow) + wp -> w_height - 1, (wp -> w_winrow) + wp -> w_height, (int) (wp -> w_wincol + wp -> w_width) - 3, (int) (wp ->
              w_wincol + wp -> w_width), '@', '@', highlight_attr [(int) (HLF_AT)]);
        ...
        if (eof) {
            ...
            if (j > 0 && !wp->w_botfill) {
                if (char2cells (fill_diff) > 1) i = '-';
                else i = fill_diff;
                if (row + j > wp -> w_height) j = wp -> w_height - row;
                win_draw_end (wp, i, i, row, row + (int) j, HLF_DED);
                row += j;
            }
        }
        else if (dollar_vcol == 0) wp -> w_botline = lnum;
    ...
    if (!got_int) got_int = save_got_int;
}

static int draw_signcolumn (win_T * wp);

static int draw_signcolumn (wp)
    win_T * wp;
{
    return (wp -> w_buffer -> b_signlist != ((void *) 0) || netbeans_active ());
}
    ...
    hlf_T hl;
{
    int n = 0;
    {
        if (cmdwin_type != 0 && wp == curwin) {
            n = 1;
            if (n > wp -> w_width) n = wp -> w_width;
            screen_fill ((wp -> w_winrow) + row, (wp -> w_winrow) + endrow, (wp -> w_wincol), (int) (wp -> w_wincol) + n, cmdwin_type, ' ', highlight_attr [(
              int) (HLF_AT)]);
        }
        if (wp -> w_onebuf_opt.wo_fdc > 0) {
            int nn = n + wp -> w_onebuf_opt.wo_fdc;
            if (nn > (wp -> w_width)) nn = (wp -> w_width);
            screen_fill ((wp -> w_winrow) + row, (wp -> w_winrow) + endrow, (wp -> w_wincol) + n, (int) (wp -> w_wincol) + nn, ' ', ' ', highlight_attr [(int) (
              HLF_FC)]);
            n = nn;
        }
        ...
        screen_fill ((wp -> w_winrow) + row, (wp -> w_winrow) + endrow, (wp -> w_wincol) + n, (int) (wp -> w_wincol + wp -> w_width), c1, c2, highlight_attr [(
          int) (hl)]);
    }
...
static int advance_color_col (vcol, color_cols)
    ...
    int * * color_cols;
{
    while (**color_cols >= 0 && vcol > **color_cols) ++*color_cols;
    return (**color_cols >= 0);
}
    ...
    int row;
{
    ...
    if (fdc > (wp -> w_width) - col) fdc = (wp -> w_width) - col;
    if (fdc > 0) {
        fill_foldcolumn (buf, wp, 1, lnum);
        copy_text_attr (off + col, buf, fdc, highlight_attr [(int) (HLF_FC)]);
        col += fdc;
    }
    for (ri = 0; ri < (wp -> w_width) - col; ++ri) ScreenAttrs [off + (col) + ri] = highlight_attr [(int) (HLF_FL)];
    if (draw_signcolumn (wp)) {
        len = (wp -> w_width) - col;
        if (len > 0) {
            if (len > 2) len = 2;
    ...
    if (wp -> w_onebuf_opt.wo_nu || wp -> w_onebuf_opt.wo_rnu) {
        ...
        if (len > 0) {
            ...
            long num;
            if (len > w + 1) len = w + 1;
            if (wp -> w_onebuf_opt.wo_nu) num = (long) lnum;
            else num = (long) abs ((int) get_cursor_rel_lnum (wp, lnum));
            __builtin___sprintf_chk ((char *) buf, 0, __builtin_object_size ((char *) buf, 1 > 1), "%*ld ", w, num);
    ...
    txtcol = col;
    {
        ...
        if (len > (wp -> w_width) - col) len = (wp -> w_width) - col;
        if (len > 0) {
            ((__builtin_object_size ((char *) (current_ScreenLine + col), 0) != (size_t) - 1) ? __builtin___strncpy_chk ((char *) (current_ScreenLine + col), (
              char *) (text), (size_t) (len), __builtin_object_size ((char *) (current_ScreenLine + col), 1 > 1)) : __inline_strncpy_chk ((char *) (
              current_ScreenLine + col), (char *) (text), (size_t) (len)));
            col += len;
        }
    }
    ...
    if (text != buf) vim_free (text);
    if (VIsual_active && wp -> w_buffer == curwin -> w_buffer) {
        if (((((curwin -> w_cursor).lnum != (VIsual).lnum) ? (curwin -> w_cursor).lnum < (VIsual).lnum : (curwin -> w_cursor).col != (VIsual).col ? (curwin ->
          w_cursor).col < (VIsual).col : (curwin -> w_cursor).coladd < (VIsual).coladd) || (((curwin -> w_cursor).lnum == (VIsual).lnum) && ((curwin -> w_cursor
          ).col == (VIsual).col) && ((curwin -> w_cursor).coladd == (VIsual).coladd)))) {
            top = &curwin->w_cursor;
            bot = &VIsual;
        }
        else {
        ...
        if (lnum >= top -> lnum && lnume <= bot -> lnum && (VIsual_mode != 'v' || ((lnum > top -> lnum || (lnum == top -> lnum && top -> col == 0)) && (lnume <
          bot -> lnum || (lnume == bot -> lnum && (bot -> col - (*p_sel == 'e')) >= (colnr_T) strlen ((char *) (ml_get_buf (wp -> w_buffer, lnume, 0)))))))) {
            if (VIsual_mode == 22) {
                if (wp -> w_old_cursor_fcol + txtcol < (colnr_T) (wp -> w_width)) {
                    if (wp -> w_old_cursor_lcol + txtcol < (colnr_T) (wp -> w_width)) len = wp -> w_old_cursor_lcol;
                    else len = (wp -> w_width) - txtcol;
                    for (ri = 0; ri < len - (int) wp -> w_old_cursor_fcol; ++ri) ScreenAttrs [off + (wp -> w_old_cursor_fcol + txtcol) + ri] = highlight_attr [
                      (int) (HLF_V)];
                }
            }
            else {
                for (ri = 0; ri < (wp -> w_width) - txtcol; ++ri) ScreenAttrs [off + (txtcol) + ri] = highlight_attr [(int) (HLF_V)];
            }
        }
    }
    if (wp -> w_onebuf_opt.wo_cuc) {
        txtcol += wp -> w_virtcol;
        if (wp -> w_onebuf_opt.wo_wrap) txtcol -= wp -> w_skipcol;
        else txtcol -= wp -> w_leftcol;
        if (txtcol >= 0 && txtcol < (wp -> w_width)) ScreenAttrs [off + txtcol] = hl_combine_attr (ScreenAttrs [off + txtcol], highlight_attr [(int) (HLF_CUC)]
          );
    }
    screen_line ((row + (wp -> w_winrow)), ((wp -> w_wincol)), ((int) (wp -> w_width)), ((int) (wp -> w_width)));
...
{
    int i;
    ((__builtin_object_size ((char *) (ScreenLines + off), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (ScreenLines + off), (char *) (buf), (size_t
      ) len, __builtin_object_size ((char *) (ScreenLines + off), 0)) : __inline_memmove_chk ((char *) (ScreenLines + off), (char *) (buf), (size_t) len));
    for (i = 0; i < len; ++i) ScreenAttrs [off + i] = attr;
}
    ...
    linenr_T lnum;
{
    ...
    int first_level;
    int empty;
    copy_spaces (p, (size_t) wp -> w_onebuf_opt.wo_fdc);
    level = win_foldinfo.fi_level;
    if (level > 0) {
        empty = (wp -> w_onebuf_opt.wo_fdc == 1) ? 0 : 1;
        first_level = level - wp -> w_onebuf_opt.wo_fdc - closed + 1 + empty;
        if (first_level < 1) first_level = 1;
        for (i = 0; i + empty < wp -> w_onebuf_opt.wo_fdc; ++i) {
            if (win_foldinfo.fi_lnum == lnum && first_level + i >= win_foldinfo.fi_low_level) p [i] = '-';
            else if (first_level == 1) p [i] = '|';
            else if (first_level + i <= 9) p [i] = '0' + first_level + i;
            else p [i] = '>';
            if (first_level + i == level) break;
        }
    }
    if (closed) p [i >= wp -> w_onebuf_opt.wo_fdc ? i - 1 : i] = '+';
}
...
static int win_line (wp, lnum, startrow, endrow, nochange)
    ...
    int startrow;
    int endrow;
    int nochange __attribute__ ((unused));
{
    int col;
    unsigned off;
    int c = 0;
    ...
    int change_start = (0x7fffffffL);
    int change_end = -1;
    colnr_T trailcol = (0x7fffffffL);
    ...
    if (syntax_present (wp) && !wp->w_s->b_syn_error) {
        ...
        if (did_emsg) wp -> w_s -> b_syn_error = 1;
        else {
    ...
    if (color_cols != ((void *) 0)) draw_color_col = advance_color_col ((vcol), &color_cols);
    if (wp -> w_onebuf_opt.wo_spell && *wp->w_s->b_p_spl != '\000' && wp -> w_s -> b_langp.ga_len > 0 && *(char**)(wp->w_s->b_langp.ga_data) != ((void *) 0)) {
        has_spell = 1;
        extra_check = 1;
        nextline [150] = '\000';
        ...
        if (lnum == checked_lnum) cur_checked_col = checked_col;
        checked_lnum = 0;
        if (lnum != capcol_lnum) cap_col = -1;
        if (lnum == 1) cap_col = 0;
        capcol_lnum = 0;
    }
    fromcol = -10;
    ...
    if (VIsual_active && wp -> w_buffer == curwin -> w_buffer) {
        ...
        if (VIsual_mode == 22) {
            ...
            if (lnum > top -> lnum && lnum <= bot -> lnum) fromcol = 0;
            else if (lnum == top -> lnum) {
                if (VIsual_mode == 'V') fromcol = 0;
                else {
                    getvvcol (wp, top, (colnr_T *) &fromcol, ((void *) 0), ((void *) 0));
                    if (gchar_pos (top) == '\000') tocol = fromcol + 1;
                }
            }
            if (VIsual_mode != 'V' && lnum == bot -> lnum) {
                ...
                else if (bot -> col == (0x7fffffffL)) tocol = (0x7fffffffL);
                else {
                    pos = *bot;
                    if (*p_sel == 'e') getvvcol (wp, &pos, (colnr_T *) &tocol, ((void *) 0), ((void *) 0));
                    else {
        ...
        if (!highlight_match && lnum == curwin -> w_cursor.lnum && wp == curwin) noinvcur = 1;
        if (fromcol >= 0) {
            area_highlighting = 1;
        ...
        if (lnum == curwin -> w_cursor.lnum) getvcol (curwin, &(curwin->w_cursor), (colnr_T *) &fromcol, ((void *) 0), ((void *) 0));
        else fromcol = 0;
    ...
    if (filler_lines < 0) {
        if (filler_lines == -1) {
            if (diff_find_change (wp, lnum, &change_start, &change_end)) diff_hlf = HLF_ADD;
            else if (change_start == 0) diff_hlf = HLF_TXD;
            else diff_hlf = HLF_CHD;
        }
        else diff_hlf = HLF_ADD;
        filler_lines = 0;
        area_highlighting = 1;
    }
    if (lnum == wp -> w_topline) filler_lines = wp -> w_topfill;
    filler_todo = filler_lines;
    v = buf_getsigntype (wp -> w_buffer, lnum, 1);
    if (v != 0) line_attr = sign_get_attr ((int) v, 1);
    if (bt_quickfix (wp -> w_buffer) && qf_current_entry (wp) == lnum) line_attr = highlight_attr [(int) (HLF_L)];
    if (line_attr != 0) area_highlighting = 1;
    line = ml_get_buf (wp -> w_buffer, lnum, 0);
    ptr = line;
    if (has_spell) {
        if (cap_col == 0) cap_col = (int) (skipwhite (line) - line);
        if (nextline [150] == '\000') {
            nextlinecol = (0x7fffffffL);
            nextline_idx = 0;
        }
        else {
            ...
            if (v < 150) {
                nextlinecol = 0;
                ((__builtin_object_size ((char *) (nextline), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (nextline), (char *) (line), (size_t) v,
                  __builtin_object_size ((char *) (nextline), 0)) : __inline_memmove_chk ((char *) (nextline), (char *) (line), (size_t) v));
                ((__builtin_object_size ((char *) ((nextline + v)), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) ((nextline + v)), (char *) ((
                  nextline + 150)), strlen ((char *) (nextline + 150)) + 1, __builtin_object_size ((char *) ((nextline + v)), 0)) : __inline_memmove_chk ((char
                  *) ((nextline + v)), (char *) ((nextline + 150)), strlen ((char *) (nextline + 150)) + 1));
                nextline_idx = v + 1;
            }
            else {
                nextlinecol = v - 150;
                ((__builtin_object_size ((char *) (nextline), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (nextline), (char *) (line + nextlinecol)
                  , 150, __builtin_object_size ((char *) (nextline), 0)) : __inline_memmove_chk ((char *) (nextline), (char *) (line + nextlinecol), 150));
                nextline_idx = 150 + 1;
            }
        }
    }
    if (wp -> w_onebuf_opt.wo_list && lcs_trail) {
        ...
        while (trailcol > (colnr_T) 0 && ((ptr [trailcol - 1]) == ' ' || (ptr [trailcol - 1]) == '\t')) --trailcol;
        trailcol += (colnr_T) (ptr - line);
        extra_check = 1;
    }
    if (wp -> w_onebuf_opt.wo_wrap) v = wp -> w_skipcol;
    else v = wp -> w_leftcol;
    if (v > 0) {
        while (vcol < v && *ptr != '\000') {
            c = win_lbr_chartabsize (wp, ptr, (colnr_T) vcol, ((void *) 0));
            vcol += c;
            ++ptr;
        }
        if (vcol < v && (wp -> w_onebuf_opt.wo_cuc || draw_color_col || virtual_active () || (VIsual_active && wp -> w_buffer == curwin -> w_buffer))) {
            vcol = v;
        }
        if (vcol > v) {
            vcol -= c;
        ...
        if (tocol <= vcol) fromcol = 0;
        else if (fromcol >= 0 && fromcol < vcol) fromcol = vcol;
        if (wp -> w_onebuf_opt.wo_wrap) need_showbreak = 1;
        if (has_spell) {
            int len;
            ...
            if (len == 0 || (int) wp -> w_cursor.col > ptr - line) {
                ...
                if (spell_hlf != HLF_COUNT) spell_attr = highlight_attr [spell_hlf];
            }
            wp -> w_cursor = pos;
            if (has_syntax) syntax_start (wp, lnum);
        }
    }
    if (fromcol >= 0) {
        if (noinvcur) {
            ...
            else if ((colnr_T) fromcol < wp -> w_virtcol) fromcol_prev = wp -> w_virtcol;
        }
    ...
    while (cur != ((void *) 0) || shl_flag == 0) {
        ...
        shl -> startcol = (0x7fffffffL);
        shl -> endcol = (0x7fffffffL);
        shl -> attr_cur = 0;
        if (shl -> rm.regprog != ((void *) 0)) {
            ...
            if (shl -> lnum != 0 && shl -> lnum <= lnum) {
                if (shl -> lnum == lnum) shl -> startcol = shl -> rm.startpos [0].col;
                else shl -> startcol = 0;
                if (lnum == shl -> lnum + shl -> rm.endpos [0].lnum - shl -> rm.startpos [0].lnum) shl -> endcol = shl -> rm.endpos [0].col;
                else shl -> endcol = (0x7fffffffL);
        ...
        if (shl != &search_hl && cur != ((void *) 0)) cur = cur -> next;
    }
    if (wp -> w_onebuf_opt.wo_cul && lnum == wp -> w_cursor.lnum && !VIsual_active) {
    ...
    for (;;) {
        if (draw_state != 0 + 1 + 1 + 1 + 1 + 1 + 1) {
            ...
            if (draw_state == 0 + 1 + 1 + 1 + 1 - 1 && n_extra == 0) {
                ...
                if ((wp -> w_onebuf_opt.wo_nu || wp -> w_onebuf_opt.wo_rnu) && (row == startrow + filler_lines || vim_strchr (p_cpo, 'n') == ((void *) 0))) {
                    if (row == startrow + filler_lines) {
                        long num;
                        if (wp -> w_onebuf_opt.wo_nu) num = (long) lnum;
                        else num = (long) abs ((int) get_cursor_rel_lnum (wp, lnum));
                        __builtin___sprintf_chk ((char *) extra, 0, __builtin_object_size ((char *) extra, 1 > 1), "%*ld ", number_width (wp), num);
                        if (wp -> w_skipcol > 0) for (p_extra = extra; *p_extra == ' '; ++p_extra) *p_extra = '-';
                        p_extra = extra;
                        c_extra = '\000';
                    }
                    else c_extra = ' ';
                    ...
                    if (wp -> w_onebuf_opt.wo_cul && lnum == wp -> w_cursor.lnum) char_attr = hl_combine_attr (highlight_attr [(int) (HLF_CUL)], char_attr);
                }
            }
            if (draw_state == 0 + 1 + 1 + 1 + 1 + 1 - 1 && n_extra == 0) {
                draw_state = 0 + 1 + 1 + 1 + 1 + 1;
                if (filler_todo > 0) {
                    if (char2cells (fill_diff) > 1) c_extra = '-';
                    else c_extra = fill_diff;
        ...
        if (dollar_vcol != 0 && wp == curwin && lnum == wp -> w_cursor.lnum && vcol >= (long) wp -> w_virtcol && filler_todo <= 0) {
            ...
            if (wp -> w_onebuf_opt.wo_cuc) row = wp -> w_cline_row + wp -> w_cline_height;
            else row = wp -> w_height;
            break;
        }
        if (draw_state == 0 + 1 + 1 + 1 + 1 + 1 + 1 && area_highlighting) {
            if (vcol == fromcol || ((int) vcol_prev == fromcol_prev && vcol_prev < vcol && vcol < tocol)) area_attr = attr;
            else if (area_attr != 0 && (vcol == tocol || (noinvcur && (colnr_T) vcol == wp -> w_virtcol))) area_attr = 0;
            if (!n_extra) {
                v = (long) (ptr - line);
                cur = wp -> w_match_head;
                shl_flag = 0;
                while (cur != ((void *) 0) || shl_flag == 0) {
                    if (shl_flag == 0 && ((cur != ((void *) 0) && cur -> priority > 0) || cur == ((void *) 0))) {
                        shl = &search_hl;
                        shl_flag = 1;
                    }
                    else shl = &cur->hl;
                    while (shl -> rm.regprog != ((void *) 0)) {
                        if (shl -> startcol != (0x7fffffffL) && v >= (long) shl -> startcol && v < (long) shl -> endcol) {
                            shl -> attr_cur = shl -> attr;
                        }
                            ...
                            if (shl -> lnum == lnum) {
                                ...
                                if (shl -> rm.endpos [0].lnum == 0) shl -> endcol = shl -> rm.endpos [0].col;
                                else shl -> endcol = (0x7fffffffL);
                                if (shl -> startcol == shl -> endcol) {
                                    ++shl->endcol;
                                }
                ...
                while (cur != ((void *) 0) || shl_flag == 0) {
                    ...
                    if (shl -> attr_cur != 0) search_attr = shl -> attr_cur;
                    if (shl != &search_hl && cur != ((void *) 0)) cur = cur -> next;
                }
            }
            if (diff_hlf != (hlf_T) 0) {
                if (diff_hlf == HLF_CHD && ptr - line >= change_start && n_extra == 0) diff_hlf = HLF_TXD;
                if (diff_hlf == HLF_TXD && ptr - line > change_end && n_extra == 0) diff_hlf = HLF_CHD;
                line_attr = highlight_attr [(int) (diff_hlf)];
            }
            attr_pri = 1;
            if (area_attr != 0) char_attr = area_attr;
            else if (search_attr != 0) char_attr = search_attr;
            else if (line_attr != 0 && ((fromcol == -10 && tocol == (0x7fffffffL)) || vcol < fromcol || vcol_prev < fromcol_prev || vcol >= tocol)) char_attr =
              line_attr;
            else {
                attr_pri = 0;
                if (has_syntax) char_attr = syntax_attr;
                else char_attr = 0;
            }
        }
        if (n_extra > 0) {
            ...
            c = *ptr;
            ++ptr;
            if (wp -> w_onebuf_opt.wo_list && (c == 160) && lcs_nbsp) {
                c = lcs_nbsp;
            ...
            if (extra_check) {
                ...
                if (has_syntax && v > 0) {
                    save_did_emsg = did_emsg;
                    did_emsg = 0;
                    syntax_attr = get_syntax_attr ((colnr_T) v - 1, has_spell ? &can_spell : ((void *) 0), 0);
                    if (did_emsg) {
                    ...
                    line = ml_get_buf (wp -> w_buffer, lnum, 0);
                    ptr = line + v;
                    if (!attr_pri) char_attr = syntax_attr;
                    else char_attr = hl_combine_attr (syntax_attr, char_attr);
                }
                if (has_spell && v >= word_end && v > cur_checked_col) {
                    ...
                    if (c != 0 && (!has_syntax || can_spell)) {
                        ...
                        hlf_T spell_hlf = HLF_COUNT;
                        prev_ptr = ptr - 1;
                        if ((prev_ptr - line) - nextlinecol >= 0) p = nextline + (prev_ptr - line) - nextlinecol;
                        else p = prev_ptr;
                        cap_col -= (int) (prev_ptr - line);
                        len = spell_check (wp, p, &spell_hlf, &cap_col, nochange);
                        word_end = v + len;
                        if (spell_hlf != HLF_COUNT && (State & 0x10) != 0 && wp -> w_cursor.lnum == lnum && wp -> w_cursor.col >= (colnr_T) (prev_ptr - line) &&
                        ...
                        if (cap_col > 0) {
                            ...
                            else cap_col += (int) (prev_ptr - line);
                        }
                    }
                }
                if (spell_attr != 0) {
                    if (!attr_pri) char_attr = hl_combine_attr (char_attr, spell_attr);
                    else char_attr = hl_combine_attr (spell_attr, char_attr);
                }
                if (wp -> w_onebuf_opt.wo_lbr && (breakat_flags [(char_u) (c)]) && !(breakat_flags[(char_u)(*ptr)]) && !wp->w_onebuf_opt.wo_list) {
                    n_extra = win_lbr_chartabsize (wp, ptr - (1), (colnr_T) vcol, ((void *) 0)) - 1;
                    c_extra = ' ';
                    if (((c) == ' ' || (c) == '\t')) c = ' ';
                }
                if (trailcol != (0x7fffffffL) && ptr > line + trailcol && c == ' ') {
                    c = lcs_trail;
                    if (!attr_pri) {
                        n_attr = 1;
                        extra_attr = highlight_attr [(int) (HLF_8)];
            ...
            if (!(chartab[c&0xff]&0x10)) {
                ...
                else if (c == '\000' && ((wp -> w_onebuf_opt.wo_list && lcs_eol > 0) || ((fromcol >= 0 || fromcol_prev >= 0) && tocol > vcol && VIsual_mode !=
                  22 && ((col < (wp -> w_width))) && !(noinvcur&&lnum==wp->w_cursor.lnum&&(colnr_T)vcol==wp->w_virtcol))) && lcs_eol_one >= 0) {
                    if (diff_hlf == (hlf_T) 0 && line_attr == 0) {
                        if (area_highlighting && virtual_active () && tocol != (0x7fffffffL) && vcol < tocol) n_extra = 0;
                        else {
                            p_extra = at_end_str;
                            n_extra = 1;
                            c_extra = '\000';
                    ...
                    if (wp -> w_onebuf_opt.wo_list) c = lcs_eol;
                    else c = ' ';
                    lcs_eol_one = -1;
                    --ptr;
                    if (!attr_pri) {
                ...
                else if (VIsual_active && (VIsual_mode == 22 || VIsual_mode == 'v') && virtual_active () && tocol != (0x7fffffffL) && vcol < tocol && ((col < (
                  wp -> w_width)))) {
                    c = ' ';
                    --ptr;
                }
                else if ((diff_hlf != (hlf_T) 0 || line_attr != 0) && ((col < (wp -> w_width)))) {
                    c = ' ';
                    --ptr;
                    ++did_line_attr;
                    if (line_attr != 0 && char_attr == search_attr && col > 0) char_attr = line_attr;
                    if (diff_hlf == HLF_TXD) {
                        diff_hlf = HLF_CHD;
                        if (attr == 0 || char_attr != attr) char_attr = highlight_attr [(int) (diff_hlf)];
                    }
                }
            }
        }
        if (n_attr > 0 && draw_state == 0 + 1 + 1 + 1 + 1 + 1 + 1 && !attr_pri) char_attr = extra_attr;
        if (lcs_prec_todo != '\000' && (wp -> w_onebuf_opt.wo_wrap ? wp -> w_skipcol > 0 : wp -> w_leftcol > 0) && filler_todo <= 0 && draw_state > 0 + 1 + 1 +
          1 + 1 && c != '\000') {
            c = lcs_prec;
            lcs_prec_todo = '\000';
            if (!attr_pri) {
        ...
        if (c == '\000' || did_line_attr == 1) {
            ...
            if ((long) (wp -> w_onebuf_opt.wo_wrap ? wp -> w_skipcol : wp -> w_leftcol) > prevcol) ++prevcol;
            prevcol_hl_flag = 0;
            if (prevcol == (long) search_hl.startcol) prevcol_hl_flag = 1;
            else {
                cur = wp -> w_match_head;
                while (cur != ((void *) 0)) {
            ...
            if (lcs_eol == lcs_eol_one && ((area_attr != 0 && vcol == fromcol && (VIsual_mode != 22 || lnum == VIsual.lnum || lnum == curwin -> w_cursor.lnum)
              && c == '\000') || (prevcol_hl_flag == 1 && did_line_attr <= 1))) {
                int n = 0;
                {
                    if (col >= (wp -> w_width)) n = -1;
                }
                if (n != 0) {
                    off += n;
                    col += n;
                ...
                if (area_attr == 0) {
                    ...
                    while (cur != ((void *) 0) || shl_flag == 0) {
                        ...
                        if ((ptr - line) - 1 == (long) shl -> startcol) char_attr = shl -> attr;
                        if (shl != &search_hl && cur != ((void *) 0)) cur = cur -> next;
                    }
                }
        ...
        if (c == '\000') {
            ...
            if (vcol < v + col - win_col_off (wp)) vcol = v + col - win_col_off (wp);
            if (draw_color_col) draw_color_col = advance_color_col ((vcol), &color_cols);
            if (((wp -> w_onebuf_opt.wo_cuc && (int) wp -> w_virtcol >= (vcol) - eol_hl_off && (int) wp -> w_virtcol < (wp -> w_width) * (row - startrow + 1) +
              v && lnum != wp -> w_cursor.lnum) || draw_color_col)) {
                int rightmost_vcol = 0;
                int i;
                if (wp -> w_onebuf_opt.wo_cuc) rightmost_vcol = wp -> w_virtcol;
                if (draw_color_col) for (i = 0; color_cols [i] >= 0; ++i) if (rightmost_vcol < color_cols [i]) rightmost_vcol = color_cols [i];
                while (col < (wp -> w_width)) {
                    ScreenLines [off] = ' ';
                    ++col;
                    if (draw_color_col) draw_color_col = advance_color_col ((vcol), &color_cols);
                    if (wp -> w_onebuf_opt.wo_cuc && (vcol) == (long) wp -> w_virtcol) ScreenAttrs [off ++] = highlight_attr [(int) (HLF_CUC)];
                    else if (draw_color_col && (vcol) == *color_cols) ScreenAttrs [off ++] = highlight_attr [(int) (HLF_MC)];
                    else ScreenAttrs [off ++] = 0;
                    if ((vcol) >= rightmost_vcol) break;
                    ++vcol;
                }
            }
            screen_line ((screen_row), ((wp -> w_wincol)), (col), ((int) (wp -> w_width)));
        ...
        if (lcs_ext && !wp->w_onebuf_opt.wo_wrap && filler_todo <= 0 && (col == (wp -> w_width) - 1) && (*ptr != '\000' || (wp -> w_onebuf_opt.wo_list &&
          lcs_eol_one > 0) || (n_extra && (c_extra != '\000' || *p_extra != '\000')))) {
            c = lcs_ext;
            char_attr = highlight_attr [(int) (HLF_AT)];
        }
        if (draw_color_col) draw_color_col = advance_color_col ((vcol), &color_cols);
        vcol_save_attr = -1;
        if (draw_state == 0 + 1 + 1 + 1 + 1 + 1 + 1 && !lnum_in_visual_area) {
            if (wp -> w_onebuf_opt.wo_cuc && (vcol) == (long) wp -> w_virtcol && lnum != wp -> w_cursor.lnum) {
        ...
        vcol_prev = vcol;
        if (draw_state < 0 + 1 + 1 + 1 + 1 + 1 + 1 || n_skip <= 0) {
            ScreenLines [off] = c;
            ScreenAttrs [off] = char_attr;
            {
                ++off;
        ...
        if (draw_state > 0 + 1 + 1 + 1 + 1 && filler_todo <= 0) ++vcol;
        if (vcol_save_attr >= 0) char_attr = vcol_save_attr;
        if (draw_state > 0 + 1 + 1 + 1 + 1 && n_attr3 > 0 && --n_attr3 == 0) char_attr = saved_attr3;
        if (n_attr > 0 && draw_state == 0 + 1 + 1 + 1 + 1 + 1 + 1 && --n_attr == 0) char_attr = saved_attr2;
        if (((col >= (wp -> w_width))) && (*ptr != '\000' || filler_todo > 0 || (wp -> w_onebuf_opt.wo_list && lcs_eol != '\000' && p_extra != at_end_str) || (
          n_extra != 0 && (c_extra != '\000' || *p_extra != '\000')))) {
            screen_line ((screen_row), ((wp -> w_wincol)), (col), ((int) (wp -> w_width)));
            ++row;
            ++screen_row;
            if ((!wp->w_onebuf_opt.wo_wrap && filler_todo <= 0) || lcs_eol_one == -1) break;
            if (draw_state != 0 + 1 + 1 + 1 + 1 + 1 + 1 && filler_todo <= 0) {
                win_draw_end (wp, '@', ' ', row, wp -> w_height, HLF_AT);
                draw_vsep_win (wp, row);
                row = endrow;
            }
            ...
            if (screen_cur_row == screen_row - 1 && filler_todo <= 0 && (wp -> w_width) == Columns) {
                ...
                if (p_tf) {
                    if (screen_cur_col != (wp -> w_width)) screen_char (LineOffset [screen_row - 1] + (unsigned) Columns - 1, screen_row - 1, (int) (Columns - 1
                      ));
            ...
            if (filler_todo <= 0) need_showbreak = 1;
            --filler_todo;
            if (filler_todo == 0 && wp -> w_botfill) break;
        }
    }
...
static int char_needs_redraw (off_from, off_to, cols)
    ...
    int cols;
{
    if (cols > 0 && ((ScreenLines [off_from] != ScreenLines [off_to] || ScreenAttrs [off_from] != ScreenAttrs [off_to]))) return 1;
    return 0;
}
    ...
    int clear_width;
{
    ...
    while (col < endcol) {
        ...
        redraw_next = force || char_needs_redraw (off_from + 1, off_to + 1, endcol - col - 1);
        if (redraw_this) {
            if (p_wiv && !force && ScreenAttrs [off_to] != 0 && ScreenAttrs [off_from] != ScreenAttrs [off_to]) {
                windgoto (row, col + coloff);
                out_str ((term_strings [(int) (KS_CE)]));
                screen_start ();
                force = 1;
                redraw_next = 1;
            ...
            if (term_is_xterm) {
                ...
                if (hl > 0x3f) hl = syn_attr2attr (hl);
                if (hl & 0x02) redraw_next = 1;
            }
            ...
            else if (screen_attr != 0) screen_stop_highlight ();
        }
        off_to += 1;
        off_from += 1;
        col += 1;
    ...
    if (clear_width > 0) {
        ...
        else LineWraps [row] = 0;
    }
}

void status_redraw_all () {
    win_T * wp;
    for (wp = firstwin; wp; wp = wp -> w_next) if (wp -> w_status_height) {
        wp -> w_redr_status = 1;
        redraw_later (10);
    }
}
...
void status_redraw_curbuf () {
    win_T * wp;
    for (wp = firstwin; wp; wp = wp -> w_next) if (wp -> w_status_height != 0 && wp -> w_buffer == curbuf) {
        wp -> w_redr_status = 1;
        redraw_later (10);
    }
}
...
void redraw_statuslines () {
    win_T * wp;
    for (wp = firstwin; wp; wp = wp -> w_next) if (wp -> w_redr_status) win_redr_status (wp);
    if (redraw_tabline) draw_tabline ();
}
...
void win_redraw_last_status (frp)
    frame_T * frp;
{
    if (frp -> fr_layout == 0) frp -> fr_win -> w_redr_status = 1;
    else if (frp -> fr_layout == 1) {
        for (frp = frp -> fr_child; frp != ((void *) 0); frp = frp -> fr_next) win_redraw_last_status (frp);
    }
    else {
        frp = frp -> fr_child;
        while (frp -> fr_next != ((void *) 0)) frp = frp -> fr_next;
    ...
    int row;
{
    ...
    if (wp -> w_vsep_width) {
        c = fillchar_vsep (&hl);
        screen_fill ((wp -> w_winrow) + row, (wp -> w_winrow) + wp -> w_height, (wp -> w_wincol + wp -> w_width), (wp -> w_wincol + wp -> w_width) + 1, c, ' ',
          hl);
    }
}
...
{
    int len = 0;
    int emenu = (xp -> xp_context == 11 || xp -> xp_context == 21);
    if (emenu && menu_is_separator (s)) return 1;
    while (*s != '\000') {
...
static int skip_status_match_char (xp, s)
    ...
    char_u * s;
{
    if ((rem_backslash (s) && xp -> xp_context != 8) || ((xp -> xp_context == 11 || xp -> xp_context == 21) && (s [0] == '\t' || (s [0] == '\\' && s [1] !=
      '\000')))) {
        if (xp -> xp_shell && csh_like_shell () && s [1] == '\\' && s [2] == '!') return 2;
        return 1;
    }
    return 0;
}
    ...
    int showtail;
{
    ...
    if (match == 0) first_match = 0;
    else if (match < first_match) {
        first_match = match;
        add_left = 1;
    }
    else {
        for (i = first_match; i < match; ++i) clen += status_match_len (xp, (showtail ? sm_gettail (matches [i]) : matches [i])) + 2;
        if (first_match > 0) clen += 2;
        if ((long) clen > Columns) {
            first_match = match;
            clen = 2;
            for (i = match; i < num_matches; ++i) {
                clen += status_match_len (xp, (showtail ? sm_gettail (matches [i]) : matches [i])) + 2;
                if ((long) clen >= Columns) break;
            }
            if (i == num_matches) add_left = 1;
        }
    }
    if (add_left) while (first_match > 0) {
    ...
    if (first_match == 0) {
        * buf = '\000';
        ...
        ((__builtin_object_size ((char *) (buf), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (buf), (char *) ("< "), __builtin_object_size ((char *)
          (buf), 1 > 1)) : __inline_strcpy_chk ((char *) (buf), (char *) ("< ")));
        len = 2;
    }
    clen = len;
    i = first_match;
    while ((long) (clen + status_match_len (xp, (showtail ? sm_gettail (matches [i]) : matches [i])) + 2) < Columns) {
        ...
        emenu = (xp -> xp_context == 11 || xp -> xp_context == 21);
        if (emenu && menu_is_separator (s)) {
            ((__builtin_object_size ((char *) (buf + len), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (buf + len), (char *) (transchar ('|')),
              __builtin_object_size ((char *) (buf + len), 1 > 1)) : __inline_strcpy_chk ((char *) (buf + len), (char *) (transchar ('|'))));
            ...
            clen += ptr2cells (s);
            {
                ((__builtin_object_size ((char *) (buf + len), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (buf + len), (char *) (transchar (*s)),
                  __builtin_object_size ((char *) (buf + len), 1 > 1)) : __inline_strcpy_chk ((char *) (buf + len), (char *) (transchar (*s))));
                len += (int) strlen ((char *) (buf + len));
            }
        }
        if (i == match) selend = buf + len;
        *(buf+len++) = ' ';
        *(buf+len++) = ' ';
        clen += 2;
        ...
        if (wild_menu_showing == 0) {
            if (msg_scrolled > 0) {
                if (cmdline_row == Rows - 1) {
                    screen_del_lines (0, 0, 1, (int) Rows, 1, ((void *) 0));
                    ++msg_scrolled;
                }
                else {
                    ++cmdline_row;
    ...
    win_T * wp;
{
    ...
    if (wp -> w_status_height == 0) {
        ...
        if (wp -> w_buffer -> b_help || wp -> w_onebuf_opt.wo_pvw || bufIsChanged (wp -> w_buffer) || wp -> w_buffer -> b_p_ro) *(p+len++) = ' ';
        if (wp -> w_buffer -> b_help) {
            ((__builtin_object_size ((char *) (p + len), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (p + len), (char *) (((char *) ("[Help]"))),
              __builtin_object_size ((char *) (p + len), 1 > 1)) : __inline_strcpy_chk ((char *) (p + len), (char *) (((char *) ("[Help]")))));
            len += (int) strlen ((char *) (p + len));
        }
        if (wp -> w_onebuf_opt.wo_pvw) {
            ((__builtin_object_size ((char *) (p + len), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (p + len), (char *) (((char *) ("[Preview]"))),
              __builtin_object_size ((char *) (p + len), 1 > 1)) : __inline_strcpy_chk ((char *) (p + len), (char *) (((char *) ("[Preview]")))));
            len += (int) strlen ((char *) (p + len));
        }
        if (bufIsChanged (wp -> w_buffer)) {
            ((__builtin_object_size ((char *) (p + len), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (p + len), (char *) ("[+]"),
              __builtin_object_size ((char *) (p + len), 1 > 1)) : __inline_strcpy_chk ((char *) (p + len), (char *) ("[+]")));
            len += 3;
        }
        if (wp -> w_buffer -> b_p_ro) {
            ((__builtin_object_size ((char *) (p + len), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (p + len), (char *) ("[RO]"),
              __builtin_object_size ((char *) (p + len), 1 > 1)) : __inline_strcpy_chk ((char *) (p + len), (char *) ("[RO]")));
            len += 4;
        }
        this_ru_col = ru_col - (Columns - (wp -> w_width));
        if (this_ru_col < ((wp -> w_width) + 1) / 2) this_ru_col = ((wp -> w_width) + 1) / 2;
        if (this_ru_col <= 1) {
            p = (char_u *) "<";
            len = 1;
        }
        ...
        if (get_keymap_str (wp, NameBuff, 1024) && (int) (this_ru_col - len) > (int) (strlen ((char *) (NameBuff)) + 1)) screen_puts (NameBuff, row, (int) (
          this_ru_col - strlen ((char *) (NameBuff)) - 1 + (wp -> w_wincol)), attr);
        win_redr_ruler (wp, 1);
    }
    if (wp -> w_vsep_width != 0 && wp -> w_status_height != 0 && redrawing ()) {
        if (stl_connected (wp)) fillchar = fillchar_status (&attr, wp == curwin);
        else fillchar = fillchar_vsep (&attr);
        screen_putchar (fillchar, (wp -> w_winrow) + wp -> w_height, (wp -> w_wincol + wp -> w_width), attr);
    }
    busy = 0;
}
    ...
    win_T * wp;
{
    ...
    if (called_emsg) {
        set_string_option_direct ((char_u *) "statusline", -1, (char_u *) "", 1 | (*wp->w_onebuf_opt.wo_stl != '\000' ? 4 : 2), -5);
    }
    ...
    win_T * wp;
{
    ...
    while (fr -> fr_parent != ((void *) 0)) {
        if (fr -> fr_parent -> fr_layout == 2) {
            if (fr -> fr_next != ((void *) 0)) break;
        }
        else {
            if (fr -> fr_next != ((void *) 0)) return 1;
        }
        fr = fr -> fr_parent;
    }
    return 0;
...
{
    char_u * p;
    if (wp -> w_buffer -> b_p_iminsert != 1) return 0;
    {
        ...
        curbuf = wp -> w_buffer;
        curwin = wp;
        ((__builtin_object_size ((char *) (buf), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (buf), (char *) ("b:keymap_name"), __builtin_object_size
          ((char *) (buf), 1 > 1)) : __inline_strcpy_chk ((char *) (buf), (char *) ("b:keymap_name")));
        ++emsg_skip;
        ...
        if ((int) (strlen ((char *) (p)) + 3) < len) __builtin___sprintf_chk ((char *) buf, 0, __builtin_object_size ((char *) buf, 1 > 1), "<%s>", p);
        else buf [0] = '\000';
        vim_free (s);
    }
    return buf [0] != '\000';
    ...
    int draw_ruler;
{
    ...
    if (wp == ((void *) 0)) {
        ...
        if (draw_ruler) {
            ...
            if (*stl == '%') {
                if (*++stl == '-') stl ++;
                if (atoi ((char *) stl)) while (((*stl) >= '0' && (*stl) <= '9')) stl ++;
                if (*stl++ != '(') stl = p_ruf;
            }
            col = ru_col - (Columns - (wp -> w_width));
            if (col < ((wp -> w_width) + 1) / 2) col = ((wp -> w_width) + 1) / 2;
            ...
            if (*wp->w_onebuf_opt.wo_stl != '\000') stl = wp -> w_onebuf_opt.wo_stl;
            else stl = p_stl;
            use_sandbox = was_set_insecurely ((char_u *) "statusline", *wp->w_onebuf_opt.wo_stl == '\000' ? 0 : 4);
        }
        col += (wp -> w_wincol);
    }
    if (maxwidth <= 0) return;
    stl = vim_strsave (stl);
    width = build_stl_str_hl (wp == ((void *) 0) ? curwin : wp, buf, sizeof (buf), stl, use_sandbox, fillchar, maxwidth, hltab, tabtab);
    vim_free (stl);
    ...
    for (n = 0; hltab [n].start != ((void *) 0); n ++) {
        ...
        p = hltab [n].start;
        if (hltab [n].userhl == 0) curattr = attr;
        else if (hltab [n].userhl < 0) curattr = syn_id2attr (-hltab[n].userhl);
        else if (wp != ((void *) 0) && wp != curwin && wp -> w_status_height != 0) curattr = highlight_stlnc [hltab [n].userhl - 1];
        else curattr = highlight_user [hltab [n].userhl - 1];
    }
    ...
    if (wp == ((void *) 0)) {
        ...
        for (n = 0; tabtab [n].start != ((void *) 0); n ++) {
            ...
            while (col < len) TabPageIdxs [col ++] = fillchar;
            p = tabtab [n].start;
            fillchar = tabtab [n].userhl;
        }
        while (col < Columns) TabPageIdxs [col ++] = fillchar;
    }
}
    ...
    int attr;
{
    ...
    int need_redraw;
    if (ScreenLines == ((void *) 0) || row >= screen_Rows) return;
    off = LineOffset [row] + col;
    while (col < screen_Columns && (len < 0 || (int) (ptr - text) < len) && *ptr != '\000') {
        c = *ptr;
        ...
        if (need_redraw || force_redraw_this) {
            if (need_redraw && ScreenLines [off] != ' ' && (term_is_xterm)) {
                ...
                if (n > 0x3f) n = syn_attr2attr (n);
                if (n & 0x02) force_redraw_next = 1;
            }
            ScreenLines [off] = c;
    ...
    win_T * wp;
{
    ...
    while (cur != ((void *) 0)) {
        cur -> hl.rm = cur -> match;
        if (cur -> hlg_id == 0) cur -> hl.attr = 0;
        else cur -> hl.attr = syn_id2attr (cur -> hlg_id);
    ...
    linenr_T lnum;
{
    ...
    while (cur != ((void *) 0) || shl_flag == 0) {
        ...
        if (shl -> rm.regprog != ((void *) 0) && shl -> lnum == 0 && re_multiline (shl -> rm.regprog)) {
            if (shl -> first_lnum == 0) {
                for (shl -> first_lnum = lnum; shl -> first_lnum > wp -> w_topline; --shl->first_lnum) if (hasFoldingWin (wp, shl -> first_lnum - 1, ((void *) 0
                  ), ((void *) 0), 1, ((void *) 0))) break;
            }
            n = 0;
    ...
    colnr_T mincol;
{
    ...
    if (shl -> lnum != 0) {
        ...
        if (lnum > l) shl -> lnum = 0;
        else if (lnum < l || shl -> rm.endpos [0].col > mincol) return;
    }
    called_emsg = 0;
    for (;;) {
        ...
        if (shl -> lnum == 0) matchcol = 0;
        else if (vim_strchr (p_cpo, 'c') == ((void *) 0) || (shl -> rm.endpos [0].lnum == 0 && shl -> rm.endpos [0].col <= shl -> rm.startpos [0].col)) {
            char_u * ml;
        ...
        else matchcol = shl -> rm.endpos [0].col;
        shl -> lnum = lnum;
    ...
    if (full_screen) {
        {
            if (attr > 0x3f) {
                ...
                if (aep == ((void *) 0)) attr = 0;
                else attr = aep -> ae_attr;
            }
            if ((attr & 0x02) && (term_strings [(int) (KS_MD)]) != ((void *) 0)) out_str ((term_strings [(int) (KS_MD)]));
            else if (aep != ((void *) 0) && t_colors > 1 && aep -> ae_u.cterm.fg_color && cterm_normal_fg_bold) out_str ((term_strings [(int) (KS_ME)]));
            if ((attr & 0x20) && (term_strings [(int) (KS_SO)]) != ((void *) 0)) out_str ((term_strings [(int) (KS_SO)]));
            if ((attr & (0x08 | 0x10)) && (term_strings [(int) (KS_US)]) != ((void *) 0)) out_str ((term_strings [(int) (KS_US)]));
            if ((attr & 0x04) && (term_strings [(int) (KS_CZH)]) != ((void *) 0)) out_str ((term_strings [(int) (KS_CZH)]));
            if ((attr & 0x01) && (term_strings [(int) (KS_MR)]) != ((void *) 0)) out_str ((term_strings [(int) (KS_MR)]));
            if (aep != ((void *) 0)) {
                if (t_colors > 1) {
                    if (aep -> ae_u.cterm.fg_color) term_fg_color (aep -> ae_u.cterm.fg_color - 1);
                    if (aep -> ae_u.cterm.bg_color) term_bg_color (aep -> ae_u.cterm.bg_color - 1);
                }
                else {
                    if (aep -> ae_u.term.start != ((void *) 0)) out_str (aep -> ae_u.term.start);
                }
            }
        }
    }
    ...
    if (screen_attr != 0) {
        {
            if (screen_attr > 0x3f) {
                ...
                if (t_colors > 1) {
                    ...
                    if (aep != ((void *) 0) && (aep -> ae_u.cterm.fg_color || aep -> ae_u.cterm.bg_color)) do_ME = 1;
                }
                else {
                    aep = syn_term_attr2entry (screen_attr);
                    if (aep != ((void *) 0) && aep -> ae_u.term.stop != ((void *) 0)) {
                        if (strcmp ((char *) (aep -> ae_u.term.stop), (char *) ((term_strings [(int) (KS_ME)]))) == 0) do_ME = 1;
                        else out_str (aep -> ae_u.term.stop);
                    }
                }
                if (aep == ((void *) 0)) screen_attr = 0;
                else screen_attr = aep -> ae_attr;
            }
            if (screen_attr & 0x20) {
                if (strcmp ((char *) ((term_strings [(int) (KS_SE)])), (char *) ((term_strings [(int) (KS_ME)]))) == 0) do_ME = 1;
                else out_str ((term_strings [(int) (KS_SE)]));
            }
            if (screen_attr & (0x08 | 0x10)) {
                if (strcmp ((char *) ((term_strings [(int) (KS_UE)])), (char *) ((term_strings [(int) (KS_ME)]))) == 0) do_ME = 1;
                else out_str ((term_strings [(int) (KS_UE)]));
            }
            if (screen_attr & 0x04) {
                if (strcmp ((char *) ((term_strings [(int) (KS_CZR)])), (char *) ((term_strings [(int) (KS_ME)]))) == 0) do_ME = 1;
                else out_str ((term_strings [(int) (KS_CZR)]));
            }
            if (do_ME || (screen_attr & (0x02 | 0x01))) out_str ((term_strings [(int) (KS_ME)]));
            if (t_colors > 1) {
                if (cterm_normal_fg_color != 0) term_fg_color (cterm_normal_fg_color - 1);
                if (cterm_normal_bg_color != 0) term_bg_color (cterm_normal_bg_color - 1);
                if (cterm_normal_fg_bold) out_str ((term_strings [(int) (KS_MD)]));
            }
        }
    }
    screen_attr = 0;
...
void reset_cterm_colors () {
    if (t_colors > 1) {
        if (cterm_normal_fg_color > 0 || cterm_normal_bg_color > 0) {
            out_str ((term_strings [(int) (KS_OP)]));
            screen_attr = -1;
        }
...
{
    int attr;
    if (row >= screen_Rows || col >= screen_Columns) return;
    if (row == screen_Rows - 1 && col == screen_Columns - 1) {
        ScreenAttrs [off] = (sattr_T) - 1;
        return;
    }
    if (screen_char_attr != 0) attr = screen_char_attr;
    else attr = ScreenAttrs [off];
    if (screen_attr != attr) screen_stop_highlight ();
    windgoto (row, col);
    if (screen_attr != attr) screen_start_highlight (attr);
    {
    ...
    int invert;
{
    int r, c;
    int off;
    if (ScreenLines == ((void *) 0)) return;
    if (invert) screen_char_attr = 0x01;
    ...
    win_T * wp;
{
    ...
    screen_draw_rectangle (row, col, end - row, width, 0);
}
    ...
    int attr;
{
    ...
    int norm_term;
    int force_next = 0;
    if (end_row > screen_Rows) end_row = screen_Rows;
    if (end_col > screen_Columns) end_col = screen_Columns;
    if (ScreenLines == ((void *) 0) || start_row >= end_row || start_col >= end_col) return;
    norm_term = (t_colors <= 1);
    for (row = start_row; row < end_row; ++row) {
        did_delete = 0;
        if (c2 == ' ' && end_col == Columns && can_clear ((term_strings [(int) (KS_CE)])) && (attr == 0 || (norm_term && attr <= 0x3f && ((attr & ~(0x02|0x04))
          == 0)))) {
            col = start_col;
            if (c1 != ' ') ++col;
            off = LineOffset [row] + col;
            end_off = LineOffset [row] + end_col;
            while (off < end_off && ScreenLines [off] == ' ' && ScreenAttrs [off] == 0) ++off;
            if (off < end_off) {
        ...
        for (col = start_col; col < end_col; ++col) {
            if (ScreenLines [off] != c || ScreenAttrs [off] != attr || force_next) {
                if (term_is_xterm) {
                    if (ScreenLines [off] != ' ' && (ScreenAttrs [off] > 0x3f || ScreenAttrs [off] & 0x02)) force_next = 1;
                    else force_next = 0;
                }
                ScreenLines [off] = c;
                ScreenAttrs [off] = attr;
                if (!did_delete || c != ' ') screen_char (off, row, col);
            }
            ++off;
            if (col == start_col) {
                if (did_delete) break;
        ...
        if (end_col == Columns) LineWraps [row] = 0;
        if (row == Rows - 1) {
            redraw_cmdline = 1;
            if (c1 == ' ' && c2 == ' ') clear_cmdline = 0;
            if (start_col == 0) mode_displayed = 0;
        }
    }
}
    ...
    int check_msg_scroll;
{
    if ((emsg_on_display || (check_msg_scroll && msg_scroll)) && !did_wait_return && emsg_silent == 0) {
        ...
        ui_delay (1000L, 1);
        emsg_on_display = 0;
        if (check_msg_scroll) msg_scroll = 0;
    }
}

int screen_valid (clear)
    int clear;
{
    screenalloc (clear);
    ...
    int clear;
{
    ...
    int retry_count = 0;
    retry : if ((ScreenLines != ((void *) 0) && Rows == screen_Rows && Columns == screen_Columns) || Rows == 0 || Columns == 0 || (!full_screen && ScreenLines
      == ((void *) 0))) return;
    if (entered) return;
    entered = 1;
    ++RedrawingDisabled;
    ...
    for ((tp) = first_tabpage; (tp) != ((void *) 0); (tp) = (tp) -> tp_next) for ((wp) = ((tp) == curtab) ? firstwin : (tp) -> tp_firstwin; (wp); (wp) = (wp) ->
      w_next) win_free_lsize (wp);
    if (aucmd_win != ((void *) 0)) win_free_lsize (aucmd_win);
    new_ScreenLines = (schar_T *) lalloc ((long_u) ((Rows + 1) * Columns * sizeof (schar_T)), 0);
    new_ScreenAttrs = (sattr_T *) lalloc ((long_u) ((Rows + 1) * Columns * sizeof (sattr_T)), 0);
    new_LineOffset = (unsigned *) lalloc ((long_u) (Rows * sizeof (unsigned)), 0);
    new_LineWraps = (char_u *) lalloc ((long_u) (Rows * sizeof (char_u)), 0);
    new_TabPageIdxs = (short *) lalloc ((long_u) (Columns * sizeof (short)), 0);
    for ((tp) = first_tabpage; (tp) != ((void *) 0); (tp) = (tp) -> tp_next) for ((wp) = ((tp) == curtab) ? firstwin : (tp) -> tp_firstwin; (wp); (wp) = (wp) ->
      w_next) {
        if (win_alloc_lines (wp) == 0) {
            outofmem = 1;
            goto give_up;
        }
    }
    if (aucmd_win != ((void *) 0) && aucmd_win -> w_lines == ((void *) 0) && win_alloc_lines (aucmd_win) == 0) outofmem = 1;
    give_up : if (new_ScreenLines == ((void *) 0) || new_ScreenAttrs == ((void *) 0) || new_LineOffset == ((void *) 0) || new_LineWraps == ((void *) 0) ||
      new_TabPageIdxs == ((void *) 0) || outofmem) {
        ...
        for (new_row = 0; new_row < Rows; ++new_row) {
            ...
            if (!clear) {
                (void) ((__builtin_object_size ((new_ScreenLines + new_row * Columns), 0) != (size_t) - 1) ? __builtin___memset_chk ((new_ScreenLines + new_row
                  * Columns), (' '), ((size_t) Columns * sizeof (schar_T)), __builtin_object_size ((new_ScreenLines + new_row * Columns), 0)) :
                  __inline_memset_chk ((new_ScreenLines + new_row * Columns), (' '), ((size_t) Columns * sizeof (schar_T))));
                (void) ((__builtin_object_size ((new_ScreenAttrs + new_row * Columns), 0) != (size_t) - 1) ? __builtin___memset_chk ((new_ScreenAttrs + new_row
                  * Columns), (0), ((size_t) Columns * sizeof (sattr_T)), __builtin_object_size ((new_ScreenAttrs + new_row * Columns), 0)) :
                  __inline_memset_chk ((new_ScreenAttrs + new_row * Columns), (0), ((size_t) Columns * sizeof (sattr_T))));
                old_row = new_row + (screen_Rows - Rows);
                if (old_row >= 0 && ScreenLines != ((void *) 0)) {
                    if (screen_Columns < Columns) len = screen_Columns;
                    else len = Columns;
                    ((__builtin_object_size ((char *) (new_ScreenLines + new_LineOffset [new_row]), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (
                      new_ScreenLines + new_LineOffset [new_row]), (char *) (ScreenLines + LineOffset [old_row]), (size_t) len * sizeof (schar_T),
                      __builtin_object_size ((char *) (new_ScreenLines + new_LineOffset [new_row]), 0)) : __inline_memmove_chk ((char *) (new_ScreenLines +
                      new_LineOffset [new_row]), (char *) (ScreenLines + LineOffset [old_row]), (size_t) len * sizeof (schar_T)));
                    ((__builtin_object_size ((char *) (new_ScreenAttrs + new_LineOffset [new_row]), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (
                      new_ScreenAttrs + new_LineOffset [new_row]), (char *) (ScreenAttrs + LineOffset [old_row]), (size_t) len * sizeof (sattr_T),
                      __builtin_object_size ((char *) (new_ScreenAttrs + new_LineOffset [new_row]), 0)) : __inline_memmove_chk ((char *) (new_ScreenAttrs +
                      new_LineOffset [new_row]), (char *) (ScreenAttrs + LineOffset [old_row]), (size_t) len * sizeof (sattr_T)));
                }
            }
        }
...
static void screenclear2 () {
    int i;
    if (starting == 2 || ScreenLines == ((void *) 0)) return;
    screen_attr = -1;
    screen_stop_highlight ();
    ...
    if (can_clear ((term_strings [(int) (KS_CL)]))) {
        out_str ((term_strings [(int) (KS_CL)]));
        clear_cmdline = 0;
        mode_displayed = 0;
    }
    else {
        for (i = 0; i < Rows; ++i) lineinvalid (LineOffset [i], (int) Columns);
        clear_cmdline = 1;
    }
    screen_cleared = 1;
    ...
    redraw_cmdline = 1;
    redraw_tabline = 1;
    if (must_redraw == 50) must_redraw = 40;
    compute_cmdrow ();
    msg_row = cmdline_row;
    msg_col = 0;
    screen_start ();
...
static void lineclear (off, width)
    ...
    int width;
{
    (void) ((__builtin_object_size ((ScreenLines + off), 0) != (size_t) - 1) ? __builtin___memset_chk ((ScreenLines + off), (' '), ((size_t) width * sizeof (
      schar_T)), __builtin_object_size ((ScreenLines + off), 0)) : __inline_memset_chk ((ScreenLines + off), (' '), ((size_t) width * sizeof (schar_T))));
    (void) ((__builtin_object_size ((ScreenAttrs + off), 0) != (size_t) - 1) ? __builtin___memset_chk ((ScreenAttrs + off), (0), ((size_t) width * sizeof (
      sattr_T)), __builtin_object_size ((ScreenAttrs + off), 0)) : __inline_memset_chk ((ScreenAttrs + off), (0), ((size_t) width * sizeof (sattr_T))));
}
...
static void lineinvalid (off, width)
    ...
    int width;
{
    (void) ((__builtin_object_size ((ScreenAttrs + off), 0) != (size_t) - 1) ? __builtin___memset_chk ((ScreenAttrs + off), (-1), ((size_t) width * sizeof (
      sattr_T)), __builtin_object_size ((ScreenAttrs + off), 0)) : __inline_memset_chk ((ScreenAttrs + off), (-1), ((size_t) width * sizeof (sattr_T))));
}
    ...
    win_T * wp;
{
    ...
    ((__builtin_object_size ((char *) (ScreenLines + off_to), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (ScreenLines + off_to), (char *) (
      ScreenLines + off_from), wp -> w_width * sizeof (schar_T), __builtin_object_size ((char *) (ScreenLines + off_to), 0)) : __inline_memmove_chk ((char *) (
      ScreenLines + off_to), (char *) (ScreenLines + off_from), wp -> w_width * sizeof (schar_T)));
    ((__builtin_object_size ((char *) (ScreenAttrs + off_to), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (ScreenAttrs + off_to), (char *) (
      ScreenAttrs + off_from), wp -> w_width * sizeof (sattr_T), __builtin_object_size ((char *) (ScreenAttrs + off_to), 0)) : __inline_memmove_chk ((char *) (
      ScreenAttrs + off_to), (char *) (ScreenAttrs + off_from), wp -> w_width * sizeof (sattr_T)));
}

int can_clear (p)
    char_u * p;
{
    return (*p != '\000' && (t_colors <= 1 || cterm_normal_bg_color == 0 || *(term_strings[(int)(KS_UT)]) != '\000'));
}
    ...
    int col;
{
    ...
    if (col != screen_cur_col || row != screen_cur_row) {
        ...
        if (col >= screen_Columns) col = screen_Columns - 1;
        if (screen_attr && *(term_strings[(int)(KS_MS)]) == '\000') noinvcurs = 5;
        else noinvcurs = 0;
        ...
        if (row >= screen_cur_row && screen_cur_col < Columns) {
            ...
            if (row == screen_cur_row && col < screen_cur_col) {
                if (*(term_strings[(int)(KS_LE)])) bs = (term_strings [(int) (KS_LE)]);
                else bs = (term_strings [(int) (KS_BC)]);
                if (*bs) cost = (screen_cur_col - col) * (int) strlen ((char *) (bs));
                else cost = 999;
                if (col + 1 < cost) {
                    plan = 2;
                    wouldbe_col = 0;
            ...
            if (cost < goto_cost) {
                ...
                if (i > 0) {
                    if ((term_strings [(int) (KS_ND)]) [0] != '\000' && (term_strings [(int) (KS_ND)]) [1] == '\000') {
                        while (i -- > 0) out_char (*(term_strings[(int)(KS_ND)]));
                    }
                    else {
                        int off;
                        off = LineOffset [row] + screen_cur_col;
                        while (i -- > 0) {
                            if (ScreenAttrs [off] != screen_attr) screen_stop_highlight ();
                            out_char (ScreenLines [off]);
                            ++off;
                        }
                    }
        ...
        if (cost >= goto_cost) {
            if (noinvcurs) screen_stop_highlight ();
            if (row == screen_cur_row && (col > screen_cur_col) && *(term_strings[(int)(KS_CRI)]) != '\000') term_cursor_right (col - screen_cur_col);
            else term_windgoto (row, col);
        }
        screen_cur_row = row;
        screen_cur_col = col;
    }
    ...
    int mayclear;
{
    ...
    int lastrow;
    int retval;
    if (invalid) wp -> w_lines_valid = 0;
    if (wp -> w_height < 5) return 0;
    if (line_count > wp -> w_height - row) line_count = wp -> w_height - row;
    retval = win_do_lines (wp, row, line_count, mayclear, 0);
    if (retval != 2) return retval;
    did_delete = 0;
    if (wp -> w_next != ((void *) 0) || wp -> w_status_height) {
        if (screen_del_lines (0, (wp -> w_winrow) + wp -> w_height - line_count, line_count, (int) Rows, 0, ((void *) 0)) == 1) did_delete = 1;
        else if (wp -> w_next) return 0;
    }
    if (!did_delete) {
        wp -> w_redr_status = 1;
        redraw_cmdline = 1;
    ...
    if (screen_ins_lines (0, (wp -> w_winrow) + row, line_count, (int) Rows, ((void *) 0)) == 0) {
        if (did_delete) {
            wp -> w_redr_status = 1;
    ...
    int mayclear;
{
    ...
    if (retval != 2) return retval;
    if (screen_del_lines (0, (wp -> w_winrow) + row, line_count, (int) Rows, 0, ((void *) 0)) == 0) return 0;
    if (wp -> w_next || wp -> w_status_height || cmdline_row < Rows - 1) {
        if (screen_ins_lines (0, (wp -> w_winrow) + wp -> w_height - line_count, line_count, (int) Rows, ((void *) 0)) == 0) {
            wp -> w_redr_status = 1;
            win_rest_invalid (wp -> w_next);
        }
    }
    else redraw_cmdline = 1;
    return 1;
}
...
{
    int retval;
    if (!redrawing() || line_count <= 0) return 0;
    if (mayclear && Rows - line_count < 5 && wp -> w_width == Columns) {
        screenclear ();
        return 0;
    }
    if (row + line_count >= wp -> w_height) {
        screen_fill ((wp -> w_winrow) + row, (wp -> w_winrow) + wp -> w_height, (wp -> w_wincol), (int) (wp -> w_wincol + wp -> w_width), ' ', ' ', 0);
        return 1;
    }
    clear_cmdline = 1;
    if (scroll_region || (wp -> w_width) != Columns) {
        if (scroll_region && (wp -> w_width == Columns || *(term_strings[(int)(KS_CSV)]) != '\000')) scroll_region_set (wp, row);
        if (del) retval = screen_del_lines ((wp -> w_winrow) + row, 0, line_count, wp -> w_height - row, 0, wp);
        else retval = screen_ins_lines ((wp -> w_winrow) + row, 0, line_count, wp -> w_height - row, wp);
        if (scroll_region && (wp -> w_width == Columns || *(term_strings[(int)(KS_CSV)]) != '\000')) scroll_region_reset ();
        return retval;
    }
    if (wp -> w_next != ((void *) 0) && p_tf) return 0;
    return 2;
}
    ...
    win_T * wp;
{
    ...
    int type;
    int result_empty;
    int can_ce = can_clear ((term_strings [(int) (KS_CE)]));
    if (!screen_valid(1) || line_count <= 0 || line_count > p_ttyscroll) return 0;
    result_empty = (row + line_count >= end);
    if (wp != ((void *) 0) && wp -> w_width != Columns && *(term_strings[(int)(KS_CSV)]) == '\000') type = 9;
    else if (can_clear ((term_strings [(int) (KS_CD)])) && result_empty) type = 8;
    else if (*(term_strings[(int)(KS_CAL)]) != '\000' && (line_count > 1 || *(term_strings[(int)(KS_AL)]) == '\000')) type = 1;
    else if (*(term_strings[(int)(KS_CDL)]) != '\000' && result_empty && (line_count > 1 || !can_ce)) type = 2;
    else if (*(term_strings[(int)(KS_AL)]) != '\000') type = 3;
    else if (can_ce && result_empty) type = 4;
    else if (*(term_strings[(int)(KS_DL)]) != '\000' && result_empty) type = 5;
    else if (*(term_strings[(int)(KS_SR)]) != '\000' && row == 0 && (*(term_strings[(int)(KS_DA)]) == '\000' || can_ce)) type = 6;
    else return 0;
    if (type == 8 || type == 2 || type == 4 || type == 5) return screen_del_lines (off, row, line_count, end, 0, wp);
    if (*(term_strings[(int)(KS_DB)])) screen_del_lines (off, end - line_count, line_count, end, 0, wp);
    if (off + row > 0 || (wp != ((void *) 0) && wp -> w_width != Columns)) clip_clear_selection ();
    else clip_scroll_selection (-line_count);
    if (*(term_strings[(int)(KS_CCS)]) != '\000') cursor_row = row;
    else cursor_row = row + off;
    row += off;
    end += off;
    for (i = 0; i < line_count; ++i) {
        if (wp != ((void *) 0) && wp -> w_width != Columns) {
            j = end - 1 - i;
            while ((j -= line_count) >= row) linecopy (j + line_count, j, wp);
            j += line_count;
            if (can_clear ((char_u *) " ")) lineclear (LineOffset [j] + wp -> w_wincol, wp -> w_width);
            else lineinvalid (LineOffset [j] + wp -> w_wincol, wp -> w_width);
            LineWraps [j] = 0;
        }
        else {
            j = end - 1 - i;
            ...
            if (can_clear ((char_u *) " ")) lineclear (temp, (int) Columns);
            else lineinvalid (temp, (int) Columns);
        }
    }
    screen_stop_highlight ();
    windgoto (cursor_row, 0);
    if (type == 9) redraw_block (row, end, wp);
    else if (type == 1) {
        ...
        for (i = 0; i < line_count; i ++) {
            if (type == 3) {
                if (i && cursor_row != 0) windgoto (cursor_row, 0);
                out_str ((term_strings [(int) (KS_AL)]));
            }
            else out_str ((term_strings [(int) (KS_SR)]));
            screen_start ();
        }
    }
    if (type == 6 && *(term_strings[(int)(KS_DA)])) {
        for (i = 0; i < line_count; ++i) {
            windgoto (off + i, 0);
            out_str ((term_strings [(int) (KS_CE)]));
            screen_start ();
    ...
    win_T * wp __attribute__ ((unused));
{
    ...
    int can_delete;
    int type;
    if (!screen_valid(1) || line_count <= 0 || (!force && line_count > p_ttyscroll)) return 0;
    result_empty = row + line_count >= end;
    can_delete = (*(term_strings[(int)(KS_DB)]) == '\000' || can_clear ((term_strings [(int) (KS_CE)])));
    if (wp != ((void *) 0) && wp -> w_width != Columns && *(term_strings[(int)(KS_CSV)]) == '\000') type = 9;
    else if (can_clear ((term_strings [(int) (KS_CD)])) && result_empty) type = 8;
    else if (row == 0 && (line_count == 1 || *(term_strings[(int)(KS_CDL)]) == '\000')) type = 7;
    else if (*(term_strings[(int)(KS_CDL)]) != '\000' && line_count > 1 && can_delete) type = 2;
    else if (can_clear ((term_strings [(int) (KS_CE)])) && result_empty && (wp == ((void *) 0) || wp -> w_width == Columns)) type = 4;
    else if (*(term_strings[(int)(KS_DL)]) != '\000' && can_delete) type = 5;
    else if (*(term_strings[(int)(KS_CDL)]) != '\000' && can_delete) type = 2;
    else return 0;
    if (off + row > 0 || (wp != ((void *) 0) && wp -> w_width != Columns)) clip_clear_selection ();
    else clip_scroll_selection (line_count);
    if (*(term_strings[(int)(KS_CCS)]) != '\000') {
        cursor_row = row;
        cursor_end = end;
    }
    else {
    ...
    for (i = 0; i < line_count; ++i) {
        if (wp != ((void *) 0) && wp -> w_width != Columns) {
            j = row + i;
            while ((j += line_count) <= end - 1) linecopy (j - line_count, j, wp);
            j -= line_count;
            if (can_clear ((char_u *) " ")) lineclear (LineOffset [j] + wp -> w_wincol, wp -> w_width);
            else lineinvalid (LineOffset [j] + wp -> w_wincol, wp -> w_width);
            LineWraps [j] = 0;
    ...
    if (type == 9) redraw_block (row, end, wp);
        ...
        windgoto (cursor_row, 0);
        out_str ((term_strings [(int) (KS_CD)]));
        screen_start ();
    }
    else if (type == 2) {
        windgoto (cursor_row, 0);
        ...
        for (i = line_count; --i >= 0;) out_char ('\n');
    }
    else {
        for (i = line_count; --i >= 0;) {
            if (type == 5) {
                windgoto (cursor_row, 0);
                out_str ((term_strings [(int) (KS_DL)]));
            }
            else {
    ...
    if (*(term_strings[(int)(KS_DB)]) && (type == 5 || type == 2)) {
...
int showmode () {
    ...
    int nwr_save;
    int sub_attr;
    do_mode = ((p_smd && msg_silent == 0) && ((State & 0x10) || restart_edit || VIsual_active));
    if (do_mode || Recording) {
        ...
        if (clear_cmdline && cmdline_row < Rows - 1) msg_clr_cmdline ();
        msg_pos_mode ();
        cursor_off ();
        ...
        if (do_mode) {
            ...
            if (edit_submode != ((void *) 0)) {
                ...
                if (edit_submode_extra != ((void *) 0)) length -= vim_strsize (edit_submode_extra);
                if (length > 0) {
                    if (edit_submode_pre != ((void *) 0)) length -= vim_strsize (edit_submode_pre);
                    if (length - vim_strsize (edit_submode) > 0) {
                        if (edit_submode_pre != ((void *) 0)) msg_puts_attr (edit_submode_pre, attr);
                        msg_puts_attr (edit_submode, attr);
                    }
                    if (edit_submode_extra != ((void *) 0)) {
                        msg_puts_attr ((char_u *) (" "), (attr));
                        if ((int) edit_submode_highl < (int) HLF_COUNT) sub_attr = highlight_attr [(int) (edit_submode_highl)];
                        else sub_attr = attr;
                ...
                if (State & 0x80) msg_puts_attr ((char_u *) (((char *) (" VREPLACE"))), (attr));
                else if (State & 0x40) msg_puts_attr ((char_u *) (((char *) (" REPLACE"))), (attr));
                else if (State & 0x10) {
                    msg_puts_attr ((char_u *) (((char *) (" INSERT"))), (attr));
                }
                else if (restart_edit == 'I') msg_puts_attr ((char_u *) (((char *) (" (insert)"))), (attr));
                else if (restart_edit == 'R') msg_puts_attr ((char_u *) (((char *) (" (replace)"))), (attr));
                else if (restart_edit == 'V') msg_puts_attr ((char_u *) (((char *) (" (vreplace)"))), (attr));
                if ((State & 0x10) && p_paste) msg_puts_attr ((char_u *) (((char *) (" (paste)"))), (attr));
                if (VIsual_active) {
                    char * p;
                    switch ((VIsual_select ? 4 : 0) + (VIsual_mode == 22) * 2 + (VIsual_mode == 'V')) {
                    case 0 :
                        p = " VISUAL";
                        break;
                    case 1 :
                    ...
                    msg_puts_attr ((char_u *) (((char *) (p))), (attr));
                }
                msg_puts_attr ((char_u *) (" --"), (attr));
            }
            need_clear = 1;
        }
        if (Recording && edit_submode == ((void *) 0)) {
            msg_puts_attr ((char_u *) (((char *) ("recording"))), (attr));
            need_clear = 1;
        }
        mode_displayed = 1;
        if (need_clear || clear_cmdline) msg_clr_eos ();
        msg_didout = 0;
        length = msg_col;
        msg_col = 0;
        need_wait_return = nwr_save;
    }
    else if (clear_cmdline && msg_silent == 0) msg_clr_cmdline ();
    if (VIsual_active) clear_showcmd ();
    if (redrawing () && lastwin -> w_status_height == 0) win_redr_ruler (lastwin, 1);
    redraw_cmdline = 0;
    clear_cmdline = 0;
    return length;
}
...
void unshowmode (force)
    int force;
{
    if (!redrawing() || (!force && char_avail () && !KeyTyped)) redraw_cmdline = 1;
    else {
        msg_pos_mode ();
        if (Recording) msg_puts_attr ((char_u *) (((char *) ("recording"))), (highlight_attr [(int) (HLF_CM)]));
        msg_clr_eos ();
    }
}
...
static void draw_tabline () {
    ...
    redraw_tabline = 0;
    if (tabline_height () < 1) return;
    for (scol = 0; scol < Columns; ++scol) TabPageIdxs [scol] = 0;
    if (*p_tal != '\000') {
        int save_called_emsg = called_emsg;
        called_emsg = 0;
        win_redr_custom (((void *) 0), 0);
        if (called_emsg) set_string_option_direct ((char_u *) "tabline", -1, (char_u *) "", 1, -5);
        called_emsg |= save_called_emsg;
    }
    else {
        for (tp = first_tabpage; tp != ((void *) 0); tp = tp -> tp_next) ++tabcount;
        tabwidth = (Columns - 1 + tabcount / 2) / tabcount;
        if (tabwidth < 6) tabwidth = 6;
        attr = attr_nosel;
        tabcount = 0;
        scol = 0;
        for (tp = first_tabpage; tp != ((void *) 0) && col < Columns - 4; tp = tp -> tp_next) {
            scol = col;
            if (tp -> tp_topframe == topframe) attr = attr_sel;
            if (use_sep_chars && col > 0) screen_putchar ('|', 0, col ++, attr);
            if (tp -> tp_topframe != topframe) attr = attr_nosel;
            ...
            modified = 0;
            for (wincount = 0; wp != ((void *) 0); wp = wp -> w_next, ++wincount) if (bufIsChanged (wp -> w_buffer)) modified = 1;
            if (modified || wincount > 1) {
                if (wincount > 1) {
                    vim_snprintf ((char *) NameBuff, 1024, "%d", wincount);
                    len = (int) strlen ((char *) (NameBuff));
                    if (col + len >= Columns - 3) break;
                    screen_puts_len (NameBuff, len, 0, col, hl_combine_attr (attr, highlight_attr [(int) (HLF_T)]));
                    col += len;
                }
                if (modified) screen_puts_len ((char_u *) "+", 1, 0, col ++, attr);
                screen_putchar (' ', 0, col ++, attr);
            }
            ...
            if (room > 0) {
                ...
                if (len > Columns - col - 1) len = Columns - col - 1;
                screen_puts_len (p, (int) strlen ((char *) (p)), 0, col, attr);
                col += len;
            }
            screen_putchar (' ', 0, col ++, attr);
            ++tabcount;
            while (scol < col) TabPageIdxs [scol ++] = tabcount;
        }
        if (use_sep_chars) c = '_';
        else c = ' ';
...
void get_trans_bufname (buf)
    buf_T * buf;
{
    if (buf_spname (buf) != ((void *) 0)) ((__builtin_object_size ((char *) (NameBuff), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (NameBuff), (
      char *) (buf_spname (buf)), __builtin_object_size ((char *) (NameBuff), 1 > 1)) : __inline_strcpy_chk ((char *) (NameBuff), (char *) (buf_spname (buf))))
      ;
    else home_replace (buf, buf -> b_fname, NameBuff, 1024, 1);
    ...
    int is_curwin;
{
    ...
    if (*attr != 0 && ((highlight_attr [(int) (HLF_S)] != highlight_attr [(int) (HLF_SNC)] || !is_curwin || firstwin == lastwin) || (fill_stl != fill_stlnc)))
      return fill;
    if (is_curwin) return '^';
    return '=';
}
    ...
    int * attr;
{
    * attr = highlight_attr [(int) (HLF_C)];
    if (*attr == 0 && fill_vert == ' ') return '|';
    else return fill_vert;
}
    ...
    int always;
{
    ...
    else win_redr_ruler (curwin, always);
    if (need_maketitle || (p_icon && (stl_syntax & 1)) || (p_title && (stl_syntax & 2))) maketitle ();
    if (redraw_tabline) draw_tabline ();
}
    ...
    int always;
{
    ...
    int width = Columns;
    if (!p_ru) return;
    if (wp -> w_cursor.lnum > wp -> w_buffer -> b_ml.ml_line_count) return;
    if (wp == lastwin && lastwin -> w_status_height == 0) if (edit_submode != ((void *) 0)) return;
    if (pum_visible ()) return;
    if (*p_ruf) {
        int save_called_emsg = called_emsg;
        called_emsg = 0;
        win_redr_custom (wp, 1);
        if (called_emsg) set_string_option_direct ((char_u *) "rulerformat", -1, (char_u *) "", 1, -5);
        called_emsg |= save_called_emsg;
        return;
    }
    if (!(State&0x10) && *ml_get_buf(wp->w_buffer,wp->w_cursor.lnum,0) == '\000') empty_line = 1;
    validate_virtcol_win (wp);
    if (redraw_cmdline || always || wp -> w_cursor.lnum != wp -> w_ru_cursor.lnum || wp -> w_cursor.col != wp -> w_ru_cursor.col || wp -> w_virtcol != wp ->
      w_ru_virtcol || wp -> w_cursor.coladd != wp -> w_ru_cursor.coladd || wp -> w_topline != wp -> w_ru_topline || wp -> w_buffer -> b_ml.ml_line_count != wp
      -> w_ru_line_count || wp -> w_topfill != wp -> w_ru_topfill || empty_line != wp -> w_ru_empty) {
        cursor_off ();
        if (wp -> w_status_height) {
            row = (wp -> w_winrow) + wp -> w_height;
            fillchar = fillchar_status (&attr, wp == curwin);
        ...
        vim_snprintf ((char *) buffer, 70, "%ld,", (wp -> w_buffer -> b_ml.ml_flags & 1) ? 0L : (long) (wp -> w_cursor.lnum));
        len = strlen ((char *) (buffer));
        col_print (buffer + len, 70 - len, empty_line ? 0 : (int) wp -> w_cursor.col + 1, (int) virtcol + 1);
        i = (int) strlen ((char *) (buffer));
        get_rel_pos (wp, buffer + i + 1, 70 - i - 1);
        o = i + vim_strsize (buffer + i + 1);
        if (wp -> w_status_height == 0) ++o;
        this_ru_col = ru_col - (Columns - width);
        if (this_ru_col < 0) this_ru_col = 0;
        if (this_ru_col < (width + 1) / 2) this_ru_col = (width + 1) / 2;
        ...
        if (this_ru_col + (int) strlen ((char *) (buffer)) > width) buffer [width - this_ru_col] = '\000';
        screen_puts (buffer, row, this_ru_col + off, attr);
        i = redraw_cmdline;
        screen_fill (row, row + 1, this_ru_col + off + (int) strlen ((char *) (buffer)), (int) (off + width), fillchar, fillchar, attr);
        redraw_cmdline = i;
    ...
    win_T * wp;
{
    int n;
    linenr_T lnum;
    if (wp -> w_onebuf_opt.wo_nu) lnum = wp -> w_buffer -> b_ml.ml_line_count;
    else lnum = wp -> w_height;
    if (lnum == wp -> w_nrwidth_line_count) return wp -> w_nrwidth_width;
    ...
    while (lnum > 0);
    if (n < wp -> w_onebuf_opt.wo_nuw - 1) n = wp -> w_onebuf_opt.wo_nuw - 1;
