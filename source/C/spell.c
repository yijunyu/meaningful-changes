...
typedef enum {STATE_START = 0, STATE_NOPREFIX, STATE_SPLITUNDO, STATE_ENDNUL, STATE_PLAIN, STATE_DEL, STATE_INS_PREP, STATE_INS, STATE_SWAP, STATE_UNSWAP,
  STATE_SWAP3, STATE_UNSWAP3, STATE_UNROT3L, STATE_UNROT3R, STATE_REP_INI, STATE_REP, STATE_REP_UNDO, STATE_FINAL} state_T;
    ...
    int docount;
{
    ...
    if (*ptr <= ' ') return 1;
    if (wp -> w_s -> b_langp.ga_len == 0) return 1;
    ((__builtin_object_size ((&mi), 0) != (size_t) - 1) ? __builtin___memset_chk ((&mi), (0), (sizeof (matchinf_T)), __builtin_object_size ((&mi), 0)) :
      __inline_memset_chk ((&mi), (0), (sizeof (matchinf_T))));
    if (*ptr >= '0' && *ptr <= '9') {
        if (*ptr == '0' && (ptr [1] == 'x' || ptr [1] == 'X')) mi.mi_end = skiphex (ptr + 2);
    ...
    if (spell_iswordp (mi.mi_fend, wp)) {
        ...
        if (capcol != ((void *) 0) && *capcol == 0 && wp -> w_s -> b_cap_prog != ((void *) 0)) {
            c = ((int) *(ptr));
            if (!((c)<256?spelltab.st_isu[c]:0)) wrongcaplen = (int) (mi.mi_fend - ptr);
        }
    }
    if (capcol != ((void *) 0)) *capcol = -1;
    mi.mi_end = mi.mi_fend;
    mi.mi_capflags = 0;
    mi.mi_cend = ((void *) 0);
    mi.mi_win = wp;
    if (*mi.mi_fend != '\000') ++mi.mi_fend;
    (void) spell_casefold (ptr, (int) (mi.mi_fend - ptr), mi.mi_fword, 250 + 1);
    ...
    for (lpi = 0; lpi < wp -> w_s -> b_langp.ga_len; ++lpi) {
        ...
        if (mi.mi_lp -> lp_slang -> sl_fidxs == ((void *) 0)) continue;
        find_word (&mi, 0);
        find_word (&mi, 1);
        find_prefix (&mi, 0);
        if (mi.mi_lp -> lp_slang -> sl_nobreak && mi.mi_result == 3 && mi.mi_result2 != 3) {
    ...
    if (mi.mi_result != 0) {
        if (nrlen > 0) {
            if (mi.mi_result == 3 || mi.mi_result == -1) return nrlen;
        }
            ...
            if (capcol != ((void *) 0) && wp -> w_s -> b_cap_prog != ((void *) 0)) {
                ...
                regmatch.rm_ic = 0;
                if (vim_regexec (&regmatch, ptr, 0)) *capcol = (int) (regmatch.endp [0] - ptr);
            }
            return 1;
        }
        else if (mi.mi_end == ptr) ++mi.mi_end;
        else if (mi.mi_result == 3 && (((langp_T *) (wp -> w_s -> b_langp).ga_data) + (0)) -> lp_slang -> sl_nobreak) {
            char_u * p, * fp;
        ...
        if (mi.mi_result == 3 || mi.mi_result == -1) *attrp = HLF_SPB;
        else if (mi.mi_result == 1) *attrp = HLF_SPR;
        else *attrp = HLF_SPL;
    }
    ...
    int mode;
{
    ...
    if (byts == ((void *) 0)) return;
    for (;;) {
        if (flen <= 0 && *mip->mi_fend != '\000') flen = fold_more (mip);
        len = byts [arridx ++];
        if (byts [arridx] == 0) {
            if (endidxcnt == 250) {
        ...
        while (lo < hi) {
            m = (lo + hi) / 2;
            if (byts [m] > c) hi = m - 1;
            else if (byts [m] < c) lo = m + 1;
            else {
                lo = hi = m;
                break;
            }
        }
        if (hi < lo || byts [lo] != c) break;
        arridx = idxs [lo];
        ++wlen;
        --flen;
        if (c == ' ') {
            for (;;) {
                if (flen <= 0 && *mip->mi_fend != '\000') flen = fold_more (mip);
                if (ptr [wlen] != ' ' && ptr [wlen] != '\011') break;
                ++wlen;
                --flen;
            }
        }
    ...
    while (endidxcnt > 0) {
        ...
        if (spell_iswordp (ptr + wlen, mip -> mi_win)) {
            if (slang -> sl_compprog == ((void *) 0) && !slang->sl_nobreak) continue;
            word_ends = 0;
        }
        else word_ends = 1;
        prefix_found = 0;
        ...
        for (len = byts [arridx - 1]; len > 0 && byts [arridx] == 0; --len, ++arridx) {
            ...
            if (mode == 0) {
                ...
                if (mip -> mi_capflags == 0x80 || !spell_valid_case(mip->mi_capflags,flags)) continue;
            }
            else if (mode == 2 && !prefix_found) {
                c = valid_word_prefix (mip -> mi_prefcnt, mip -> mi_prefarridx, flags, mip -> mi_word + mip -> mi_prefixlen, slang, 0);
                if (c == 0) continue;
                if (c & (0x01 << 24)) flags |= 0x08;
                prefix_found = 1;
            }
            if (slang -> sl_nobreak) {
                if ((mode == 3 || mode == 4) && (flags & 0x10) == 0) {
                    mip -> mi_result = 0;
                    break;
                }
            }
            else if ((mode == 3 || mode == 4 || !word_ends)) {
                if (((unsigned) flags >> 24) == 0 || wlen - mip -> mi_compoff < slang -> sl_compminlen) continue;
                if (!word_ends && mip -> mi_complen + mip -> mi_compextra + 2 > slang -> sl_compmax && slang -> sl_compsylmax == 250) continue;
                if (mip -> mi_complen > 0 && (flags & 0x1000)) continue;
                if (!word_ends && (flags & 0x2000)) continue;
                if (!byte_in_str(mip->mi_complen==0?slang->sl_compstartflags:slang->sl_compallflags,((unsigned)flags>>24))) continue;
                if (match_checkcompoundpattern (ptr, wlen, &slang->sl_comppat)) continue;
                if (mode == 3) {
                    int capflags;
                    p = mip -> mi_word + mip -> mi_compoff;
                    capflags = captype (p, mip -> mi_word + wlen);
                    if (capflags == 0x80 || (capflags == 0x04 && (flags & 0x40) != 0)) continue;
                    if (capflags != 0x04) {
                        --p;
                        if (spell_iswordp_nmw (p) ? capflags == 0x02 : (flags & 0x02) != 0 && capflags != 0x02) continue;
                    }
                }
                ...
                if (word_ends) {
                    ...
                    if (slang -> sl_compsylmax < 250) {
                        if (ptr == mip -> mi_word) (void) spell_casefold (ptr, wlen, fword, 250);
                        else vim_strncpy (fword, ptr, endlen [endidxcnt]);
                    }
                    if (!can_compound(slang,fword,mip->mi_compflags)) continue;
                }
                else if (slang -> sl_comprules != ((void *) 0) && !match_compoundrule(slang,mip->mi_compflags)) continue;
            }
            else if (flags & 0x0200) continue;
            nobreak_result = 0;
            if (!word_ends) {
                ...
                c = mip -> mi_compoff;
                ++mip->mi_complen;
                if (flags & 0x0800) ++mip->mi_compextra;
                for (lpi = 0; lpi < mip -> mi_win -> w_s -> b_langp.ga_len; ++lpi) {
                    if (slang -> sl_nobreak) {
                        mip -> mi_lp = (((langp_T *) (mip -> mi_win -> w_s -> b_langp).ga_data) + (lpi));
                        if (mip -> mi_lp -> lp_slang -> sl_fidxs == ((void *) 0) || !mip->mi_lp->lp_slang->sl_nobreak) continue;
                    }
                    find_word (mip, 3);
                    if (!slang->sl_nobreak || mip -> mi_result == 3) {
                        mip -> mi_compoff = wlen;
                ...
                if (slang -> sl_nobreak) {
                    ...
                    if (mip -> mi_result == 0) break;
                    continue;
                }
            }
            if (flags & 0x10) res = -1;
            else if (flags & 0x01) {
                if ((mip -> mi_lp -> lp_region & (flags >> 16)) != 0) res = 0;
                else res = 2;
            }
            else if (flags & 0x08) res = 1;
            else res = 0;
            if (nobreak_result == 3) {
                ...
                else if (mip -> mi_result2 == res && mip -> mi_end2 < mip -> mi_word + wlen) mip -> mi_end2 = mip -> mi_word + wlen;
            }
            else if (mip -> mi_result > res) {
                mip -> mi_result = res;
                mip -> mi_end = mip -> mi_word + wlen;
            }
            else if (mip -> mi_result == res && mip -> mi_end < mip -> mi_word + wlen) mip -> mi_end = mip -> mi_word + wlen;
            if (mip -> mi_result == 0) break;
        }
        if (mip -> mi_result == 0) break;
    }
    ...
    garray_T * gap;
{
    ...
    for (i = 0; i + 1 < gap -> ga_len; i += 2) {
        ...
        if (strncmp ((char *) (ptr + wlen), (char *) (p), (size_t) (strlen ((char *) (p)))) == 0) {
            ...
            if (len <= wlen && strncmp ((char *) (ptr + wlen - len), (char *) (p), (size_t) (len)) == 0) return 1;
        }
    }
    return 0;
}
    ...
    char_u * flags;
{
    regmatch_T regmatch;
    char_u * p;
    if (slang -> sl_compprog == ((void *) 0)) return 0;
    p = flags;
    regmatch.regprog = slang -> sl_compprog;
    regmatch.rm_ic = 0;
    if (!vim_regexec(&regmatch, p, 0)) return 0;
    if (slang -> sl_compsylmax < 250 && count_syllables (slang, word) > slang -> sl_compsylmax) return (int) strlen ((char *) (flags)) < slang -> sl_compmax;
    return 1;
}
...
static int can_be_compound (sp, slang, compflags, flag)
    ...
    int flag;
{
    if (!byte_in_str(sp->ts_complen==sp->ts_compsplit?slang->sl_compstartflags:slang->sl_compallflags,flag)) return 0;
    ...
    int cond_req;
{
    ...
    for (prefcnt = totprefcnt - 1; prefcnt >= 0; --prefcnt) {
        ...
        if (prefid != (pidx & 0xff)) continue;
        if ((flags & 0x0100) && (pidx & (0x02 << 24))) continue;
        rp = slang -> sl_prefprog [((unsigned) pidx >> 8) & 0xffff];
        if (rp != ((void *) 0)) {
            regmatch.regprog = rp;
            regmatch.rm_ic = 0;
            if (!vim_regexec(&regmatch, word, 0)) continue;
        }
        else if (cond_req) continue;
        return pidx;
    }
    return 0;
}
    ...
    int mode;
{
    ...
    for (;;) {
        ...
        if (byts [arridx] == 0) {
            ...
            if (mode == 3) mip -> mi_prefixlen += mip -> mi_compoff;
            find_word (mip, 2);
            if (len == 0) break;
        }
        if (ptr [wlen] == '\000') break;
    ...
    matchinf_T * mip;
{
    ...
    if (*mip->mi_fend != '\000') ++mip->mi_fend;
    (void) spell_casefold (p, (int) (mip -> mi_fend - p), mip -> mi_fword + mip -> mi_fwordlen, 250 - mip -> mi_fwordlen);
...
static int spell_valid_case (wordflags, treeflags)
    ...
    int treeflags;
{
    return ((wordflags == 0x04 && (treeflags & 0x40) == 0) || ((treeflags & (0x04 | 0x80)) == 0 && ((treeflags & 0x02) == 0 || (wordflags & 0x02) != 0)));
}
    ...
    hlf_T * attrp;
{
    ...
    int found_one = 0;
    int wrapped = 0;
    if (no_spell_checking (wp)) return 0;
    ...
    while (!got_int) {
        ...
        if (lnum == 1) capcol = 0;
        if (capcol == 0) capcol = (int) (skipwhite (line) - line);
        else if (curline && wp == curwin) {
            col = (int) (skipwhite (line) - line);
            if (check_need_cap (lnum, col)) capcol = col;
            line = ml_get_buf (wp -> w_buffer, lnum, 0);
        }
        ((__builtin_object_size ((char *) (buf), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (buf), (char *) (line), __builtin_object_size ((char *)
          (buf), 1 > 1)) : __inline_strcpy_chk ((char *) (buf), (char *) (line)));
        if (lnum < wp -> w_buffer -> b_ml.ml_line_count) spell_cat_line (buf + strlen ((char *) (buf)), ml_get_buf (wp -> w_buffer, lnum + 1, 0), 250);
        p = buf + skip;
        endp = buf + len;
        while (p < endp) {
            if (dir == (-1) && lnum == wp -> w_cursor.lnum && !wrapped && (colnr_T) (p - buf) >= wp -> w_cursor.col) break;
            attr = HLF_COUNT;
            len = spell_check (wp, p, &attr, &capcol, 0);
            if (attr != HLF_COUNT) {
                if (allwords || attr == HLF_SPB) {
                    if (dir == (-1) || lnum != wp -> w_cursor.lnum || (lnum == wp -> w_cursor.lnum && (wrapped || (colnr_T) (curline ? p - buf + len : p - buf)
                      > wp -> w_cursor.col))) {
                        if (has_syntax) {
                            col = (int) (p - buf);
                            (void) syn_get_id (wp, lnum, (colnr_T) col, 0, &can_spell, 0);
                            if (!can_spell) attr = HLF_COUNT;
                        }
                        else can_spell = 1;
                        if (can_spell) {
                            found_one = 1;
                            ...
                            if (dir == 1) {
                                ...
                                vim_free (buf);
                                if (attrp != ((void *) 0)) *attrp = attr;
                                return len;
                            }
                            else if (curline) found_pos.col += len;
                            found_len = len;
                        }
                    }
                    else found_one = 1;
        ...
        if (curline) break;
        if (dir == (-1)) {
            if (lnum == wp -> w_cursor.lnum && wrapped) break;
            if (lnum > 1) --lnum;
            else if (!p_ws) break;
            else {
                lnum = wp -> w_buffer -> b_ml.ml_line_count;
                wrapped = 1;
                if (!shortmess('s')) give_warning ((char_u *) ((char *) (top_bot_msg)), 1);
            }
            capcol = -1;
        }
        else {
            if (lnum < wp -> w_buffer -> b_ml.ml_line_count) ++lnum;
            else if (!p_ws) break;
            else {
                lnum = 1;
                wrapped = 1;
                if (!shortmess('s')) give_warning ((char_u *) ((char *) (bot_top_msg)), 1);
            }
            if (lnum == wp -> w_cursor.lnum && (!found_one || wrapped)) break;
            if (attr == HLF_COUNT) skip = (int) (p - endp);
            else skip = 0;
            --capcol;
            if (*skipwhite(line) == '\000') capcol = 0;
        }
        line_breakcheck ();
    }
    vim_free (buf);
    ...
    int maxlen;
{
    ...
    int n;
    p = skipwhite (line);
    while (vim_strchr ((char_u *) "*#/\"\t", *p) != ((void *) 0)) p = skipwhite (p + 1);
    if (*p != '\000') {
        n = (int) (p - line) + 1;
        if (n < maxlen - 1) {
            ((__builtin_object_size ((buf), 0) != (size_t) - 1) ? __builtin___memset_chk ((buf), (' '), (n), __builtin_object_size ((buf), 0)) :
              __inline_memset_chk ((buf), (' '), (n)));
    ...
    char_u * lang;
{
    ...
    spelload_T sl;
    int round;
    ((__builtin_object_size ((char *) (sl.sl_lang), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (sl.sl_lang), (char *) (lang), __builtin_object_size
      ((char *) (sl.sl_lang), 1 > 1)) : __inline_strcpy_chk ((char *) (sl.sl_lang), (char *) (lang)));
    sl.sl_slang = ((void *) 0);
    sl.sl_nobreak = 0;
    for (round = 1; round <= 2; ++round) {
        ...
        if (r == 0 && *sl.sl_lang != '\000') {
            ...
            r = do_in_runtimepath (fname_enc, 0, spell_load_cb, &sl);
            if (r == 0 && *sl.sl_lang != '\000' && round == 1 && apply_autocmds (EVENT_SPELLFILEMISSING, lang, curbuf -> b_fname, 0, curbuf)) continue;
            break;
        }
        break;
    }
    if (r == 0) {
        smsg ((char_u *) ((char *) ("Warning: Cannot find word list \"%s.%s.spl\" or \"%s.ascii.spl\"")), lang, spell_enc (), lang);
    }
    else if (sl.sl_slang != ((void *) 0)) {
        ((__builtin_object_size ((char *) (fname_enc + strlen ((char *) (fname_enc)) - 3), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (fname_enc +
          strlen ((char *) (fname_enc)) - 3), (char *) ("add.spl"), __builtin_object_size ((char *) (fname_enc + strlen ((char *) (fname_enc)) - 3), 1 > 1)) :
          __inline_strcpy_chk ((char *) (fname_enc + strlen ((char *) (fname_enc)) - 3), (char *) ("add.spl")));
...
static void int_wordlist_spl (fname)
    char_u * fname;
{
    vim_snprintf ((char *) fname, 1024, "%s.%s.spl", int_wordlist, spell_enc ());
}
    ...
    char_u * lang;
{
    ...
    if (lp != ((void *) 0)) {
        if (lang != ((void *) 0)) lp -> sl_name = vim_strsave (lang);
    ...
    slang_T * lp;
{
    ...
    gap = &lp->sl_sal;
    if (lp -> sl_sofo) {
        if (gap -> ga_data != ((void *) 0)) for (i = 0; i < gap -> ga_len; ++i) vim_free (((int * *) gap -> ga_data) [i]);
    }
    ...
    ga_clear (gap);
    for (i = 0; i < lp -> sl_prefixcnt; ++i) vim_free (lp -> sl_prefprog [i]);
    lp -> sl_prefixcnt = 0;
    ...
    void * cookie;
{
    ...
    if (slang != ((void *) 0)) {
        if (slp -> sl_nobreak && slang -> sl_add) slang -> sl_nobreak = 1;
        else if (slang -> sl_nobreak) slp -> sl_nobreak = 1;
        slp -> sl_slang = slang;
    }
}
    ...
    int silent;
{
    ...
    if (fd == ((void *) 0)) {
        if (!silent) emsg2 ((char_u *) (((char *) (e_notopen))), (char_u *) (fname));
        else if (p_verbose > 2) {
            verbose_enter ();
    ...
    if (old_lp == ((void *) 0)) {
        lp = slang_alloc (lang);
        if (lp == ((void *) 0)) goto endFAIL;
        lp -> sl_fname = vim_strsave (fname);
        if (lp -> sl_fname == ((void *) 0)) goto endFAIL;
        lp -> sl_add = strstr ((char *) gettail (fname), ".add.") != ((void *) 0);
    }
    else lp = old_lp;
    sourcing_name = fname;
    sourcing_lnum = 0;
    for (i = 0; i < 8; ++i) buf [i] = getc (fd);
    ...
    for (;;) {
        ...
        c = getc (fd);
        len = get4c (fd);
        if (len < 0) goto truncerr;
        res = 0;
        switch (n) {
        case 15 :
            lp -> sl_info = read_string (fd, len);
            if (lp -> sl_info == ((void *) 0)) goto endFAIL;
            break;
        case 0 :
        ...
        case 2 :
            ...
            if (lp -> sl_midword == ((void *) 0)) goto endFAIL;
            break;
        case 3 :
        ...
        case 9 :
            ...
            if (lp -> sl_syllable == ((void *) 0)) goto endFAIL;
            if (init_syl_tab (lp) == 0) goto endFAIL;
            break;
        default :
            if (c & 1) {
            ...
            while (--len >= 0) if (getc (fd) < 0) goto truncerr;
            break;
        }
        someerror : if (res == -2) {
            emsg ((char_u *) (((char *) (e_format))));
        ...
        if (res == -1) {
            truncerr : emsg ((char_u *) (((char *) (e_spell_trunc))));
            goto endFAIL;
        }
        if (res == -3) goto endFAIL;
    }
    res = spell_read_tree (fd, &lp->sl_fbyts, &lp->sl_fidxs, 0, 0);
    if (res != 0) goto someerror;
    res = spell_read_tree (fd, &lp->sl_kbyts, &lp->sl_kidxs, 0, 0);
    if (res != 0) goto someerror;
    res = spell_read_tree (fd, &lp->sl_pbyts, &lp->sl_pidxs, 1, lp -> sl_prefixcnt);
    if (res != 0) goto someerror;
    ...
    goto endOK;
    endFAIL : if (lang != ((void *) 0)) *lang = '\000';
    if (lp != ((void *) 0) && old_lp == ((void *) 0)) slang_free (lp);
    lp = ((void *) 0);
    endOK : if (fd != ((void *) 0)) fclose (fd);
    ...
    int * cntp;
{
    ...
    int i;
    char_u * str;
    for (i = 0; i < cnt_bytes; ++i) cnt = (cnt << 8) + getc (fd);
    if (cnt < 0) {
        * cntp = -1;
        return ((void *) 0);
    }
    ...
    int len;
{
    int i;
    if (len > 16) return -2;
    for (i = 0; i < len; ++i) lp -> sl_regions [i] = getc (fd);
    ...
    FILE * fd;
{
    ...
    if (flagslen < 0) return flagslen;
    fol = read_cnt_string (fd, 2, &follen);
    if (follen < 0) {
        vim_free (flags);
        return follen;
    }
    if (flags != ((void *) 0) && fol != ((void *) 0)) set_spell_charflags (flags, flagslen, fol);
    vim_free (flags);
    vim_free (fol);
    if ((flags == ((void *) 0)) != (fol == ((void *) 0))) return -2;
    return 0;
}
    ...
    slang_T * lp;
{
    ...
    if (lp -> sl_prefprog == ((void *) 0)) return -3;
    lp -> sl_prefixcnt = cnt;
    for (i = 0; i < cnt; ++i) {
        n = getc (fd);
        if (n < 0 || n >= 250) return -2;
        if (n > 0) {
            buf [0] = '^';
            p = buf + 1;
            while (n -- > 0) *p++ = getc (fd);
    ...
    short * first;
{
    ...
    for (i = 0; i < gap -> ga_len; ++i) {
        ...
        if (first [*ftp->ft_from] == -1) first [*ftp->ft_from] = i;
    }
    return 0;
}
    ...
    slang_T * slang;
{
    ...
    slang -> sl_sofo = 0;
    i = getc (fd);
    if (i & 1) slang -> sl_followup = 1;
    if (i & 2) slang -> sl_collapse = 1;
    if (i & 4) slang -> sl_rem_accents = 1;
    cnt = get2c (fd);
    if (cnt < 0) return -1;
    gap = &slang->sl_sal;
    ga_init2 (gap, sizeof (salitem_T), 10);
    if (ga_grow (gap, cnt + 1) == 0) return -3;
    for (; gap -> ga_len < cnt; ++gap->ga_len) {
        smp = &((salitem_T*)gap->ga_data)[gap->ga_len];
        ccnt = getc (fd);
        if (ccnt < 0) return -1;
        if ((p = alloc (ccnt + 2)) == ((void *) 0)) return -3;
        smp -> sm_lead = p;
        for (i = 0; i < ccnt; ++i) {
            c = getc (fd);
            if (vim_strchr ((char_u *) "0123456789(-<^$", c) != ((void *) 0)) break;
            *p++ = c;
        }
        ...
        *p++ = '\000';
        ...
        else smp -> sm_oneof = ((void *) 0);
        smp -> sm_rules = p;
        if (i < ccnt) *p++ = c;
        for (++i; i < ccnt; ++i) *p++ = getc (fd);
        *p++ = '\000';
    ...
    int count;
{
    ...
    if (((hi) -> hi_key == ((void *) 0) || (hi) -> hi_key == &hash_removed)) {
        ...
        if (wc == ((void *) 0)) return;
        ((__builtin_object_size ((char *) (wc -> wc_word), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (wc -> wc_word), (char *) (p),
          __builtin_object_size ((char *) (wc -> wc_word), 1 > 1)) : __inline_strcpy_chk ((char *) (wc -> wc_word), (char *) (p)));
        wc -> wc_count = count;
        hash_add_item (&lp->sl_wordcount, hi, wc -> wc_word, hash);
    }
    else {
        wc = ((wordcount_T *) ((hi) -> hi_key - (unsigned) (dumwc.wc_word - (char_u *) &dumwc)));
        if ((wc -> wc_count += count) < (unsigned) count) wc -> wc_count = 0xffff;
    }
}
    ...
    int split;
{
    ...
    if (!((hi)->hi_key==((void*)0)||(hi)->hi_key==&hash_removed)) {
        wc = ((wordcount_T *) ((hi) -> hi_key - (unsigned) (dumwc.wc_word - (char_u *) &dumwc)));
        if (wc -> wc_count < 10) bonus = 30;
        else if (wc -> wc_count < 100) bonus = 40;
        else bonus = 50;
        if (split) newscore = score - bonus / 2;
        else newscore = score - bonus;
    ...
    slang_T * slang;
{
    ...
    if (from != ((void *) 0) && to != ((void *) 0)) res = set_sofo (slang, from, to);
    else if (from != ((void *) 0) || to != ((void *) 0)) res = -2;
    else res = 0;
    vim_free (from);
    vim_free (to);
    return res;
    ...
    int len;
{
    ...
    *pp++ = '(';
    ...
    while (todo -- > 0) {
        ...
        if (vim_strchr ((char_u *) "+*[]/", c) == ((void *) 0) && !byte_in_str(slang->sl_compallflags,c)) {
            *ap++ = c;
            * ap = '\000';
        }
        if (atstart != 0) {
    ...
    *pp++ = '$';
    * pp = '\000';
    if (crp != ((void *) 0)) *crp = '\000';
    slang -> sl_compprog = vim_regcomp (pat, 1 + 2 + 4);
    vim_free (pat);
    if (slang -> sl_compprog == ((void *) 0)) return -2;
    return 0;
}
...
{
    char_u * p;
    for (p = str; *p != '\000'; ++p) if (*p == n) return 1;
    return 0;
}
    ...
    slang_T * slang;
{
    ...
    while (p != ((void *) 0)) {
        *p++ = '\000';
        ...
        s = p;
        p = vim_strchr (p, '/');
        if (p == ((void *) 0)) l = (int) strlen ((char *) (s));
        else l = (int) (p - s);
        if (l >= 30) return -2;
        if (ga_grow (&slang->sl_syl_items, 1) == 0) return -3;
    ...
    char_u * word;
{
    ...
    syl_item_T * syl;
    int c;
    if (slang -> sl_syllable == ((void *) 0)) return 0;
    for (p = word; *p != '\000'; p += len) {
        ...
        for (i = 0; i < slang -> sl_syl_items.ga_len; ++i) {
            ...
            if (syl -> sy_len > len && strncmp ((char *) (p), (char *) (syl -> sy_chars), (size_t) (syl -> sy_len)) == 0) len = syl -> sy_len;
        }
        if (len != 0) {
            ++cnt;
            skip = 0;
            ...
            c = *p;
            len = 1;
            if (vim_strchr (slang -> sl_syllable, c) == ((void *) 0)) skip = 0;
            else if (!skip) {
                ++cnt;
                skip = 1;
            }
    ...
    char_u * to;
{
    int i;
    {
        if (strlen ((char *) (from)) != strlen ((char *) (to))) return -2;
        for (i = 0; to [i] != '\000'; ++i) lp -> sl_sal_first [from [i]] = to [i];
        lp -> sl_sal.ga_len = 1;
    }
    return 0;
}
...
static idx_T read_tree_node (fd, byts, idxs, maxidx, startidx, prefixtree, maxprefcondnr)
    FILE * fd;
    char_u * byts;
    idx_T * idxs;
    int maxidx;
    ...
    int maxprefcondnr;
{
    ...
    len = getc (fd);
    if (len <= 0) return -1;
    if (startidx + len >= maxidx) return -2;
    byts [idx ++] = len;
    for (i = 1; i <= len; ++i) {
        c = getc (fd);
        if (c < 0) return -1;
        if (c <= 3) {
            if (c == 0 && !prefixtree) {
                ...
                if (prefixtree) {
                    ...
                    c |= getc (fd);
                    n = get2c (fd);
                    if (n >= maxprefcondnr) return -2;
                    c |= (n << 8);
                }
                else {
                    c2 = c;
                    c = getc (fd);
                    if (c2 == 3) c = (getc (fd) << 8) + c;
                    if (c & 0x01) c = (getc (fd) << 16) + c;
                    if (c & 0x20) c = (getc (fd) << 24) + c;
                }
                idxs [idx] = c;
                c = 0;
            }
    ...
    for (i = 1; i <= len; ++i) if (byts [startidx + i] != 0) {
        if (idxs [startidx + i] & 0x8000000) idxs [startidx + i] &= ~0x8000000;
        else {
            idxs [startidx + i] = idx;
    ...
    win_T * wp;
{
    ...
    char_u * spl_copy;
    if (recursive) return ((void *) 0);
    recursive = 1;
    ga_init2 (&ga, sizeof (langp_T), 2);
    clear_midword (wp);
    spl_copy = vim_strsave (wp -> w_s -> b_p_spl);
    if (spl_copy == ((void *) 0)) goto theend;
    for (splp = spl_copy; *splp != '\000';) {
        ...
        if (len > 4 && strcasecmp ((char *) (((lang + len - 4))), (char *) (((".spl")))) == 0) {
            ...
            if (p != ((void *) 0) && ((p [1]) < 0x7f && isalpha (p [1])) && ((p [2]) < 0x7f && isalpha (p [2])) && !((p[3])<0x7f&&isalpha(p[3]))) {
                vim_strncpy (region_cp, p + 1, 2);
                ((__builtin_object_size ((char *) (p), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (p), (char *) (p + 3), len - (p - lang) - 2,
                  __builtin_object_size ((char *) (p), 0)) : __inline_memmove_chk ((char *) (p), (char *) (p + 3), len - (p - lang) - 2));
                len -= 3;
                region = region_cp;
            }
            else dont_use_region = 1;
            for (slang = first_lang; slang != ((void *) 0); slang = slang -> sl_next) if (fullpathcmp (lang, slang -> sl_fname, 0) == 1) break;
        }
        else {
            filename = 0;
            ...
            for (slang = first_lang; slang != ((void *) 0); slang = slang -> sl_next) if (strcasecmp ((char *) (lang), (char *) (slang -> sl_name)) == 0) break
              ;
        }
        if (region != ((void *) 0)) {
            if (use_region != ((void *) 0) && strcmp ((char *) (region), (char *) (use_region)) != 0) dont_use_region = 1;
            use_region = region;
        }
        if (slang == ((void *) 0)) {
            if (filename) (void) spell_load_file (lang, lang, ((void *) 0), 0);
            else {
        ...
        for (slang = first_lang; slang != ((void *) 0); slang = slang -> sl_next) if (filename ? fullpathcmp (lang, slang -> sl_fname, 0) == 1 : strcasecmp ((
          char *) (lang), (char *) (slang -> sl_name)) == 0) {
            region_mask = 0xff;
            if (!filename && region != ((void *) 0)) {
                c = find_region (slang -> sl_regions, region);
                if (c == 0xff) {
                    if (slang -> sl_add) {
                        if (*slang->sl_regions != '\000') region_mask = 0;
                    }
                    else smsg ((char_u *) ((char *) ("Warning: region %s not supported")), region);
                }
    ...
    for (round = 0; round == 0 || *spf != '\000'; ++round) {
        if (round == 0) {
            if (int_wordlist == ((void *) 0)) continue;
            int_wordlist_spl (spf_name);
        }
        else {
            copy_option_part (&spf, spf_name, 1024 - 5, ",");
            ((__builtin_object_size ((char *) (spf_name), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (spf_name), (char *) (".spl"),
              __builtin_object_size ((char *) (spf_name), 1 > 1)) : __inline_strcat_chk ((char *) (spf_name), (char *) (".spl")));
            for (c = 0; c < ga.ga_len; ++c) {
                p = (((langp_T *) (ga).ga_data) + (c)) -> lp_slang -> sl_fname;
                if (p != ((void *) 0) && fullpathcmp (spf_name, p, 0) == 1) break;
            }
            if (c < ga.ga_len) continue;
        }
        for (slang = first_lang; slang != ((void *) 0); slang = slang -> sl_next) if (fullpathcmp (spf_name, slang -> sl_fname, 0) == 1) break;
        if (slang == ((void *) 0)) {
            if (round == 0) ((__builtin_object_size ((char *) (lang), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (lang), (char *) (
              "internal wordlist"), __builtin_object_size ((char *) (lang), 1 > 1)) : __inline_strcpy_chk ((char *) (lang), (char *) ("internal wordlist")));
            else {
            ...
            if (slang != ((void *) 0) && nobreak) slang -> sl_nobreak = 1;
        }
        if (slang != ((void *) 0) && ga_grow (&ga, 1) == 1) {
            region_mask = 0xff;
            if (use_region != ((void *) 0) && !dont_use_region) {
                c = find_region (slang -> sl_regions, use_region);
                if (c != 0xff) region_mask = 1 << c;
                else if (*slang->sl_regions != '\000') region_mask = 0;
            }
            if (region_mask != 0) {
                (((langp_T *) (ga).ga_data) + (ga.ga_len)) -> lp_slang = slang;
    ...
    for (i = 0; i < ga.ga_len; ++i) {
        ...
        if (lp -> lp_slang -> sl_sal.ga_len > 0) lp -> lp_sallang = lp -> lp_slang;
        else for (j = 0; j < ga.ga_len; ++j) {
            lp2 = (((langp_T *) (ga).ga_data) + (j));
            if (lp2 -> lp_slang -> sl_sal.ga_len > 0 && strncmp ((char *) (lp -> lp_slang -> sl_name), (char *) (lp2 -> lp_slang -> sl_name), (size_t) (2)) == 0
              ) {
                lp -> lp_sallang = lp2 -> lp_slang;
                break;
            }
        }
        if (lp -> lp_slang -> sl_rep.ga_len > 0) lp -> lp_replang = lp -> lp_slang;
        else for (j = 0; j < ga.ga_len; ++j) {
            lp2 = (((langp_T *) (ga).ga_data) + (j));
            if (lp2 -> lp_slang -> sl_rep.ga_len > 0 && strncmp ((char *) (lp -> lp_slang -> sl_name), (char *) (lp2 -> lp_slang -> sl_name), (size_t) (2)) == 0
              ) {
...
static void clear_midword (wp)
    win_T * wp;
{
    ((__builtin_object_size ((wp -> w_s -> b_spell_ismw), 0) != (size_t) - 1) ? __builtin___memset_chk ((wp -> w_s -> b_spell_ismw), (0), (256),
      __builtin_object_size ((wp -> w_s -> b_spell_ismw), 0)) : __inline_memset_chk ((wp -> w_s -> b_spell_ismw), (0), (256)));
}
...
{
    char_u * p;
    if (lp -> sl_midword == ((void *) 0)) return;
    for (p = lp -> sl_midword; *p != '\000';) wp -> w_s -> b_spell_ismw [*p++] = 1;
}
    ...
    char_u * region;
{
    int i;
    for (i = 0;; i += 2) {
        if (rp [i] == '\000') return 0xff;
        if (rp [i] == region [0] && rp [i + 1] == region [1]) break;
    }
    return i / 2;
}
    ...
    char_u * end;
{
    ...
    int allcap;
    int past_second = 0;
    for (p = word; !spell_iswordp_nmw(p); ++p) if (end == ((void *) 0) ? *p == '\000' : p >= end) return 0;
    c = *p++;
    firstcap = allcap = ((c) < 256 ? spelltab.st_isu [c] : 0);
    for (; end == ((void *) 0) ? *p != '\000' : p < end; ++p) if (spell_iswordp_nmw (p)) {
        c = ((int) *(p));
        if (!((c)<256?spelltab.st_isu[c]:0)) {
            if (past_second && allcap) return 0x80;
            allcap = 0;
        }
        else if (!allcap) return 0x80;
        past_second = 1;
    }
    if (allcap) return 0x04;
    if (firstcap) return 0x02;
    return 0;
}
    ...
    char_u * end;
{
    ...
    if (flags & 0x80) {
        ...
        if (u > l && u > 2) flags |= 0x04;
        else if (first) flags |= 0x02;
        if (u >= 2 && l >= 2) flags |= 0x20;
    }
    return flags;
}
    ...
    int added_word;
{
    ...
    for (slang = first_lang; slang != ((void *) 0); slang = slang -> sl_next) {
        if (fullpathcmp (fname, slang -> sl_fname, 0) == 1) {
            slang_clear (slang);
            if (spell_load_file (fname, ((void *) 0), slang, 0) == ((void *) 0)) slang_clear (slang);
            redraw_all_later (35);
            didit = 1;
        }
    }
    if (added_word && !didit) did_set_spelllang (curwin);
}
...
static int store_aff_word (spellinfo_T * spin, char_u * word, char_u * afflist, afffile_T * affile, hashtab_T * ht, hashtab_T * xht, int condit, int flags,
  char_u * pfxlist, int pfxlen);
    ...
    char_u * fname;
{
    ...
    while (!vim_fgets(rline,500,fd) && !got_int) {
        ...
        for (p = line;;) {
            ...
            if (itemcnt == 30) break;
            items [itemcnt ++] = p;
            if (itemcnt == 2 && spell_info_item (items [0])) while (*p >= ' ' || *p == '\011') ++p;
            else while (*p > ' ') ++p;
            if (*p == '\000') break;
            *p++ = '\000';
        }
        if (itemcnt > 0) {
            if (is_aff_rule (items, itemcnt, "SET", 2) && aff -> af_enc == ((void *) 0)) {
            ...
            else if (is_aff_rule (items, itemcnt, "FLAG", 2) && aff -> af_flagtype == 0) {
                if (strcmp ((char *) (items [1]), (char *) ("long")) == 0) aff -> af_flagtype = 1;
                else if (strcmp ((char *) (items [1]), (char *) ("num")) == 0) aff -> af_flagtype = 3;
                else if (strcmp ((char *) (items [1]), (char *) ("caplong")) == 0) aff -> af_flagtype = 2;
                else smsg ((char_u *) ((char *) ("Invalid value for FLAG in %s line %d: %s")), fname, lnum, items [1]);
                if (aff -> af_rare != 0 || aff -> af_keepcase != 0 || aff -> af_bad != 0 || aff -> af_needaffix != 0 || aff -> af_circumfix != 0 || aff ->
                  af_needcomp != 0 || aff -> af_comproot != 0 || aff -> af_nosuggest != 0 || compflags != ((void *) 0) || aff -> af_suff.ht_used > 0 || aff ->
                  af_pref.ht_used > 0) smsg ((char_u *) ((char *) ("FLAG after using flags in %s line %d: %s")), fname, lnum, items [1]);
            }
            else if (spell_info_item (items [0])) {
                p = (char_u *) getroom (spin, (spin -> si_info == ((void *) 0) ? 0 : strlen ((char *) (spin -> si_info))) + strlen ((char *) (items [0])) +
                  strlen ((char *) (items [1])) + 3, 0);
                if (p != ((void *) 0)) {
                    if (spin -> si_info != ((void *) 0)) {
                        ((__builtin_object_size ((char *) (p), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (p), (char *) (spin -> si_info),
                          __builtin_object_size ((char *) (p), 1 > 1)) : __inline_strcpy_chk ((char *) (p), (char *) (spin -> si_info)));
                    ...
                    ((__builtin_object_size ((char *) (p), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (p), (char *) (items [0]),
                      __builtin_object_size ((char *) (p), 1 > 1)) : __inline_strcat_chk ((char *) (p), (char *) (items [0])));
                    ((__builtin_object_size ((char *) (p), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (p), (char *) (" "), __builtin_object_size ((
                      char *) (p), 1 > 1)) : __inline_strcat_chk ((char *) (p), (char *) (" ")));
                    ((__builtin_object_size ((char *) (p), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (p), (char *) (items [1]),
                      __builtin_object_size ((char *) (p), 1 > 1)) : __inline_strcat_chk ((char *) (p), (char *) (items [1])));
                    spin -> si_info = p;
                }
            }
                ...
                if (aff -> af_pref.ht_used > 0) smsg ((char_u *) ((char *) ("Defining COMPOUNDFORBIDFLAG after PFX item may give wrong results in %s line %d"))
                  , fname, lnum);
            }
                ...
                if (aff -> af_pref.ht_used > 0) smsg ((char_u *) ((char *) ("Defining COMPOUNDPERMITFLAG after PFX item may give wrong results in %s line %d"))
                  , fname, lnum);
            }
            else if (is_aff_rule (items, itemcnt, "COMPOUNDFLAG", 2) && compflags == ((void *) 0)) {
                p = getroom (spin, strlen ((char *) (items [1])) + 2, 0);
                if (p != ((void *) 0)) {
                    ((__builtin_object_size ((char *) (p), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (p), (char *) (items [1]),
                      __builtin_object_size ((char *) (p), 1 > 1)) : __inline_strcpy_chk ((char *) (p), (char *) (items [1])));
                    ((__builtin_object_size ((char *) (p), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (p), (char *) ("+"), __builtin_object_size ((
                      char *) (p), 1 > 1)) : __inline_strcat_chk ((char *) (p), (char *) ("+")));
                    compflags = p;
                }
                ...
                if (atoi ((char *) items [1]) == 0) smsg ((char_u *) ((char *) ("Wrong COMPOUNDRULES value in %s line %d: %s")), fname, lnum, items [1]);
            }
            else if (is_aff_rule (items, itemcnt, "COMPOUNDRULE", 2)) {
                l = (int) strlen ((char *) (items [1])) + 1;
                if (compflags != ((void *) 0)) l += (int) strlen ((char *) (compflags)) + 1;
                p = getroom (spin, l, 0);
                if (p != ((void *) 0)) {
                    if (compflags != ((void *) 0)) {
                        ((__builtin_object_size ((char *) (p), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (p), (char *) (compflags),
                          __builtin_object_size ((char *) (p), 1 > 1)) : __inline_strcpy_chk ((char *) (p), (char *) (compflags)));
                        ((__builtin_object_size ((char *) (p), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (p), (char *) ("/"), __builtin_object_size
                          ((char *) (p), 1 > 1)) : __inline_strcat_chk ((char *) (p), (char *) ("/")));
                    }
                    ((__builtin_object_size ((char *) (p), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (p), (char *) (items [1]),
                ...
                if (compmax == 0) smsg ((char_u *) ((char *) ("Wrong COMPOUNDWORDMAX value in %s line %d: %s")), fname, lnum, items [1]);
            }
            else if (is_aff_rule (items, itemcnt, "COMPOUNDMIN", 2) && compminlen == 0) {
                compminlen = atoi ((char *) items [1]);
                if (compminlen == 0) smsg ((char_u *) ((char *) ("Wrong COMPOUNDMIN value in %s line %d: %s")), fname, lnum, items [1]);
            }
            else if (is_aff_rule (items, itemcnt, "COMPOUNDSYLMAX", 2) && compsylmax == 0) {
                compsylmax = atoi ((char *) items [1]);
                if (compsylmax == 0) smsg ((char_u *) ((char *) ("Wrong COMPOUNDSYLMAX value in %s line %d: %s")), fname, lnum, items [1]);
            }
                ...
                if (atoi ((char *) items [1]) == 0) smsg ((char_u *) ((char *) ("Wrong CHECKCOMPOUNDPATTERN value in %s line %d: %s")), fname, lnum, items [1])
                  ;
            }
            else if (is_aff_rule (items, itemcnt, "CHECKCOMPOUNDPATTERN", 3)) {
                garray_T * gap = &spin->si_comppat;
                int i;
                for (i = 0; i < gap -> ga_len - 1; i += 2) if (strcmp ((char *) (((char_u * *) (gap -> ga_data)) [i]), (char *) (items [1])) == 0 && strcmp ((
                  char *) (((char_u * *) (gap -> ga_data)) [i + 1]), (char *) (items [2])) == 0) break;
            ...
            else if (is_aff_rule (items, itemcnt, "SYLLABLE", 2) && syllable == ((void *) 0)) {
            ...
            else if ((strcmp ((char *) (items [0]), (char *) ("PFX")) == 0 || strcmp ((char *) (items [0]), (char *) ("SFX")) == 0) && aff_todo == 0 && itemcnt
              >= 4) {
                int lasti = 4;
                char_u key [7];
                if (*items[0] == 'P') tp = &aff->af_pref;
                else tp = &aff->af_suff;
                vim_strncpy (key, items [1], 7 - 1);
                hi = hash_find (tp, key);
                if (!((hi)->hi_key==((void*)0)||(hi)->hi_key==&hash_removed)) {
                    cur_aff = ((affheader_T *) (hi) -> hi_key);
                    if (cur_aff -> ah_combine != (*items[2] == 'Y')) smsg ((char_u *) ((char *) (
                      "Different combining flag in continued affix block in %s line %d: %s")), fname, lnum, items [1]);
                    if (!cur_aff->ah_follows) smsg ((char_u *) ((char *) ("Duplicate affix in %s line %d: %s")), fname, lnum, items [1]);
                }
                else {
                    cur_aff = (affheader_T *) getroom (spin, sizeof (affheader_T), 1);
                    if (cur_aff == ((void *) 0)) break;
                    cur_aff -> ah_flag = affitem2flag (aff -> af_flagtype, items [1], fname, lnum);
                    if (cur_aff -> ah_flag == 0 || strlen ((char *) (items [1])) >= 7) break;
                    if (cur_aff -> ah_flag == aff -> af_bad || cur_aff -> ah_flag == aff -> af_rare || cur_aff -> ah_flag == aff -> af_keepcase || cur_aff ->
                      ah_flag == aff -> af_needaffix || cur_aff -> ah_flag == aff -> af_circumfix || cur_aff -> ah_flag == aff -> af_nosuggest || cur_aff ->
                      ah_flag == aff -> af_needcomp || cur_aff -> ah_flag == aff -> af_comproot) smsg ((char_u *) ((char *) (
                      "Affix also used for BAD/RARE/KEEPCASE/NEEDAFFIX/NEEDCOMPOUND/NOSUGGEST in %s line %d: %s")), fname, lnum, items [1]);
                    ((__builtin_object_size ((char *) (cur_aff -> ah_key), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (cur_aff -> ah_key), (char *)
                      (items [1]), __builtin_object_size ((char *) (cur_aff -> ah_key), 1 > 1)) : __inline_strcpy_chk ((char *) (cur_aff -> ah_key), (char *) (
                      items [1])));
                ...
                else cur_aff -> ah_follows = 0;
                if (itemcnt > lasti && *items[lasti] != '#') smsg ((char_u *) ((char *) (e_afftrailing)), fname, lnum, items [lasti]);
                if (strcmp ((char *) (items [2]), (char *) ("Y")) != 0 && strcmp ((char *) (items [2]), (char *) ("N")) != 0) smsg ((char_u *) ((char *) (
                  "Expected Y or N in %s line %d: %s")), fname, lnum, items [2]);
                if (*items[0] == 'P' && aff -> af_pfxpostpone) {
                    ...
                    else did_postpone_prefix = 1;
                }
                aff_todo = atoi ((char *) items [3]);
            }
            else if ((strcmp ((char *) (items [0]), (char *) ("PFX")) == 0 || strcmp ((char *) (items [0]), (char *) ("SFX")) == 0) && aff_todo > 0 && strcmp (
              (char *) (cur_aff -> ah_key), (char *) (items [1])) == 0 && itemcnt >= 5) {
                affentry_T * aff_entry;
                int upper = 0;
                int lasti = 5;
                if (itemcnt > lasti && *items[lasti] != '#' && (strcmp ((char *) (items [lasti]), (char *) ("-")) != 0 || itemcnt != lasti + 1)) smsg ((char_u *
                  ) ((char *) (e_afftrailing)), fname, lnum, items [lasti]);
                --aff_todo;
                aff_entry = (affentry_T *) getroom (spin, sizeof (affentry_T), 1);
                if (aff_entry == ((void *) 0)) break;
                if (strcmp ((char *) (items [2]), (char *) ("0")) != 0) aff_entry -> ae_chop = getroom_save (spin, items [2]);
                ...
                if (!spin->si_ascii || !(has_non_ascii(aff_entry->ae_chop)||has_non_ascii(aff_entry->ae_add))) {
                    ...
                    if (strcmp ((char *) (items [4]), (char *) (".")) != 0) {
                        ...
                        if (*items[0] == 'P') __builtin___sprintf_chk ((char *) buf, 0, __builtin_object_size ((char *) buf, 1 > 1), "^%s", items [4]);
                        else __builtin___sprintf_chk ((char *) buf, 0, __builtin_object_size ((char *) buf, 1 > 1), "%s$", items [4]);
                        aff_entry -> ae_prog = vim_regcomp (buf, 1 + 2 + 4);
                        if (aff_entry -> ae_prog == ((void *) 0)) smsg ((char_u *) ((char *) ("Broken condition in %s line %d: %s")), fname, lnum, items [4]);
                    }
                    if (*items[0] == 'P' && aff -> af_pfxpostpone && aff_entry -> ae_flags == ((void *) 0)) {
                        if (aff_entry -> ae_chop != ((void *) 0) && aff_entry -> ae_add != ((void *) 0) && aff_entry -> ae_chop [1] == '\000') {
                            int c, c_up;
                            c = ((int) *(aff_entry->ae_chop));
                            c_up = ((c) < 256 ? (int) spelltab.st_upper [c] : (c));
                            if (c_up != c && (aff_entry -> ae_cond == ((void *) 0) || ((int) *(aff_entry->ae_cond)) == c)) {
                                p = aff_entry -> ae_add + strlen ((char *) (aff_entry -> ae_add));
                                --p;
                                if (((int) *(p)) == c_up) {
                                    upper = 1;
                        ...
                        if (aff_entry -> ae_chop == ((void *) 0) && aff_entry -> ae_flags == ((void *) 0)) {
                            int idx;
                            char_u * * pp;
                            int n;
                            for (idx = spin -> si_prefcond.ga_len - 1; idx >= 0; --idx) {
                                p = ((char_u * *) spin -> si_prefcond.ga_data) [idx];
                                if (str_equal (p, aff_entry -> ae_cond)) break;
                            }
                            if (idx < 0 && ga_grow (&spin->si_prefcond, 1) == 1) {
                                idx = spin -> si_prefcond.ga_len ++;
                                pp = ((char_u * *) spin -> si_prefcond.ga_data) + idx;
                                if (aff_entry -> ae_cond == ((void *) 0)) *pp = ((void *) 0);
                                else *pp = getroom_save (spin, aff_entry -> ae_cond);
                            }
                            if (aff_entry -> ae_add == ((void *) 0)) p = (char_u *) "";
                            else p = aff_entry -> ae_add;
                            n = -256;
                            if (!cur_aff->ah_combine) n |= 0x02;
                            if (upper) n |= 0x04;
                            if (aff_entry -> ae_comppermit) n |= 0x08;
                            if (aff_entry -> ae_compforbid) n |= 0x10;
                ...
                ;
                if (!isdigit(*items[1])) smsg ((char_u *) ((char *) ("Expected REP(SAL) count in %s line %d")), fname, lnum);
            }
            else if ((strcmp ((char *) (items [0]), (char *) ("REP")) == 0 || strcmp ((char *) (items [0]), (char *) ("REPSAL")) == 0) && itemcnt >= 3) {
                if (itemcnt > 3 && items [3] [0] != '#') smsg ((char_u *) ((char *) (e_afftrailing)), fname, lnum, items [3]);
                if (items [0] [3] == 'S' ? do_repsal : do_rep) {
                    for (p = items [1]; *p != '\000'; ++p) if (*p == '_') *p = ' ';
                    for (p = items [2]; *p != '\000'; ++p) if (*p == '_') *p = ' ';
                ...
                if (!found_map) {
                    found_map = 1;
                    if (!isdigit(*items[1])) smsg ((char_u *) ((char *) ("Expected MAP count in %s line %d")), fname, lnum);
                }
                else if (do_mapline) {
                    int c;
                    for (p = items [1]; *p != '\000';) {
                        c = *p++;
                        if ((spin -> si_map.ga_len > 0 && vim_strchr (spin -> si_map.ga_data, c) != ((void *) 0)) || vim_strchr (p, c) != ((void *) 0)) smsg ((
                          char_u *) ((char *) ("Duplicate character in MAP in %s line %d")), fname, lnum);
                    }
                ...
                if (do_sal) {
                    if (strcmp ((char *) (items [1]), (char *) ("followup")) == 0) spin -> si_followup = sal_to_bool (items [2]);
                    else if (strcmp ((char *) (items [1]), (char *) ("collapse_result")) == 0) spin -> si_collapse = sal_to_bool (items [2]);
                    else if (strcmp ((char *) (items [1]), (char *) ("remove_accents")) == 0) spin -> si_rem_accents = sal_to_bool (items [2]);
                    else add_fromto (spin, &spin->si_sal, items [1], strcmp ((char *) (items [2]), (char *) ("_")) == 0 ? (char_u *) "" : items [2]);
                }
            }
            else if (is_aff_rule (items, itemcnt, "SOFOFROM", 2) && sofofrom == ((void *) 0)) {
                sofofrom = getroom_save (spin, items [1]);
            }
            else if (is_aff_rule (items, itemcnt, "SOFOTO", 2) && sofoto == ((void *) 0)) {
                sofoto = getroom_save (spin, items [1]);
            }
            else if (strcmp ((char *) (items [0]), (char *) ("COMMON")) == 0) {
                int i;
                for (i = 1; i < itemcnt; ++i) {
                    if (((hash_find (&spin->si_commonwords, items [i])) -> hi_key == ((void *) 0) || (hash_find (&spin->si_commonwords, items [i])) -> hi_key ==
                      &hash_removed)) {
            ...
            else smsg ((char_u *) ((char *) ("Unrecognized or duplicate item in %s line %d: %s")), fname, lnum, items [0]);
        }
    }
    if (fol != ((void *) 0) || low != ((void *) 0) || upp != ((void *) 0)) {
        ...
        if (!spin->si_ascii) {
            if (fol == ((void *) 0) || low == ((void *) 0) || upp == ((void *) 0)) smsg ((char_u *) ((char *) ("Missing FOL/LOW/UPP line in %s")), fname);
    ...
    if (compsylmax != 0) {
        if (syllable == ((void *) 0)) smsg ((char_u *) ((char *) ("COMPOUNDSYLMAX used without SYLLABLE")));
    ...
    if (compflags != ((void *) 0)) process_compflags (spin, aff, compflags);
    if (spin -> si_newcompID < spin -> si_newprefID) {
        if (spin -> si_newcompID == 127 || spin -> si_newcompID == 255) msg ((char_u *) (((char *) ("Too many postponed prefixes"))));
        else if (spin -> si_newprefID == 0 || spin -> si_newprefID == 127) msg ((char_u *) (((char *) ("Too many compound flags"))));
        else msg ((char_u *) (((char *) ("Too many postponed prefixes and/or compound flags"))));
    }
    if (syllable != ((void *) 0)) {
    ...
    if (sofofrom != ((void *) 0) || sofoto != ((void *) 0)) {
        if (sofofrom == ((void *) 0) || sofoto == ((void *) 0)) smsg ((char_u *) ((char *) ("Missing SOFO%s line in %s")), sofofrom == ((void *) 0) ? "FROM" :
          "TO", fname);
        else if (spin -> si_sal.ga_len > 0) smsg ((char_u *) ((char *) ("Both SAL and SOFO lines in %s")), fname);
        else {
...
static int is_aff_rule (items, itemcnt, rulename, mincount)
    ...
    int mincount;
{
    return (strcmp ((char *) (items [0]), (char *) (rulename)) == 0 && (itemcnt == mincount || (itemcnt > mincount && items [mincount] [0] == '#')));
}
    ...
    affentry_T * entry;
{
    ...
    char_u * prevp;
    unsigned flag;
    if (entry -> ae_flags != ((void *) 0) && (affile -> af_compforbid != 0 || affile -> af_comppermit != 0)) {
        for (p = entry -> ae_flags; *p != '\000';) {
            prevp = p;
            flag = get_affitem (affile -> af_flagtype, &p);
            if (flag == affile -> af_comppermit || flag == affile -> af_compforbid) {
                ((__builtin_object_size ((char *) ((prevp)), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) ((prevp)), (char *) ((p)), strlen ((char *
                  ) (p)) + 1, __builtin_object_size ((char *) ((prevp)), 0)) : __inline_memmove_chk ((char *) ((prevp)), (char *) ((p)), strlen ((char *) (p)) +
                  1));
                p = prevp;
                if (flag == affile -> af_comppermit) entry -> ae_comppermit = 1;
                else entry -> ae_compforbid = 1;
            }
            if (affile -> af_flagtype == 3 && *p == ',') ++p;
        }
        if (*entry->ae_flags == '\000') entry -> ae_flags = ((void *) 0);
    }
}
...
static int spell_info_item (s)
    char_u * s;
{
    return strcmp ((char *) (s), (char *) ("NAME")) == 0 || strcmp ((char *) (s), (char *) ("HOME")) == 0 || strcmp ((char *) (s), (char *) ("VERSION")) == 0 ||
      strcmp ((char *) (s), (char *) ("AUTHOR")) == 0 || strcmp ((char *) (s), (char *) ("EMAIL")) == 0 || strcmp ((char *) (s), (char *) ("COPYRIGHT")) == 0;
}
    ...
    int lnum;
{
    ...
    if (res == 0) {
        if (flagtype == 3) smsg ((char_u *) ((char *) ("Flag is not a number in %s line %d: %s")), fname, lnum, item);
        else smsg ((char_u *) ((char *) ("Illegal flag in %s line %d: %s")), fname, lnum, item);
    }
    if (*p != '\000') {
    ...
    char_u * * pp;
{
    ...
    if (flagtype == 3) {
        ...
        res = *(*pp) ++;
        if (flagtype == 1 || (flagtype == 2 && res >= 'A' && res <= 'Z')) {
            if (**pp == '\000') return 0;
            res = *(*pp) ++ + (res << 16);
        }
    }
    ...
    char_u * compflags;
{
    ...
    if (spin -> si_compflags != ((void *) 0)) len += (int) strlen ((char *) (spin -> si_compflags)) + 1;
    p = getroom (spin, len, 0);
    if (p == ((void *) 0)) return;
    if (spin -> si_compflags != ((void *) 0)) {
        ((__builtin_object_size ((char *) (p), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (p), (char *) (spin -> si_compflags),
          __builtin_object_size ((char *) (p), 1 > 1)) : __inline_strcpy_chk ((char *) (p), (char *) (spin -> si_compflags)));
        ((__builtin_object_size ((char *) (p), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (p), (char *) ("/"), __builtin_object_size ((char *) (p),
          1 > 1)) : __inline_strcat_chk ((char *) (p), (char *) ("/")));
    }
    spin -> si_compflags = p;
    tp = p + strlen ((char *) (p));
    for (p = compflags; *p != '\000';) {
        if (vim_strchr ((char_u *) "/*+[]", *p) != ((void *) 0)) *tp++ = *p++;
        else {
            prevp = p;
            ...
            if (flag != 0) {
                ...
                if (!((hi)->hi_key==((void*)0)||(hi)->hi_key==&hash_removed)) id = ((compitem_T *) (hi) -> hi_key) -> ci_newID;
                else {
                    ci = (compitem_T *) getroom (spin, sizeof (compitem_T), 1);
                    if (ci == ((void *) 0)) break;
                    ((__builtin_object_size ((char *) (ci -> ci_key), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (ci -> ci_key), (char *) (key),
                      __builtin_object_size ((char *) (ci -> ci_key), 1 > 1)) : __inline_strcpy_chk ((char *) (ci -> ci_key), (char *) (key)));
                    ci -> ci_flag = flag;
                    do {
                        check_renumber (spin);
    ...
    unsigned flag;
{
    ...
    case 1 :
        for (p = afflist; *p != '\000';) {
            n = *p++;
            if ((flagtype == 1 || (n >= 'A' && n <= 'Z')) && *p != '\000') n = *p++ + (n << 16);
            if (n == flag) return 1;
        }
        break;
    case 3 :
...
static void aff_check_number (spinval, affval, name)
    ...
    char * name;
{
    if (spinval != 0 && spinval != affval) smsg ((char_u *) ((char *) ("%s value differs from what is used in another .aff file")), name);
}
...
static void aff_check_string (spinval, affval, name)
    ...
    char * name;
{
    if (spinval != ((void *) 0) && strcmp ((char *) (spinval), (char *) (affval)) != 0) smsg ((char_u *) ((char *) (
...
static int str_equal (s1, s2)
    ...
    char_u * s2;
{
    if (s1 == ((void *) 0) || s2 == ((void *) 0)) return s1 == s2;
...
static int has_non_ascii (s)
    char_u * s;
{
    char_u * p;
    if (s != ((void *) 0)) for (p = s; *p != '\000'; ++p) if (*p >= 128) return 1;
    return 0;
}
    ...
    for (ht = &aff->af_pref;; ht = &aff->af_suff) {
        ...
        for (hi = ht -> ht_array; todo > 0; ++hi) {
            if (!((hi)->hi_key==((void*)0)||(hi)->hi_key==&hash_removed)) {
                ...
                for (ae = ah -> ah_first; ae != ((void *) 0); ae = ae -> ae_next) vim_free (ae -> ae_prog);
            }
        }
        if (ht == &aff->af_suff) break;
    }
    ...
    afffile_T * affile;
{
    ...
    hash_init (&ht);
    vim_snprintf ((char *) IObuff, (1024 + 1), ((char *) ("Reading dictionary file %s ...")), fname);
    spell_message (spin, IObuff);
    spin -> si_msg_count = 999999;
    (void) vim_fgets (line, 500, fd);
    if (!vim_isdigit(*skipwhite(line))) emsg2 ((char_u *) (((char *) ("E760: No word count in %s"))), (char_u *) (fname));
    while (!vim_fgets(line,500,fd) && !got_int) {
        ...
        afflist = ((void *) 0);
        for (p = w; *p != '\000'; ++p) {
            if (*p == '\\' && (p [1] == '\\' || p [1] == '/')) ((__builtin_object_size ((char *) ((p)), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *)
              ((p)), (char *) ((p + 1)), strlen ((char *) (p + 1)) + 1, __builtin_object_size ((char *) ((p)), 0)) : __inline_memmove_chk ((char *) ((p)), (
              char *) ((p + 1)), strlen ((char *) (p + 1)) + 1));
            else if (*p == '/') {
                * p = '\000';
                afflist = p + 1;
                break;
        ...
        if (spin -> si_verbose && spin -> si_msg_count > 10000) {
            spin -> si_msg_count = 0;
            vim_snprintf ((char *) message, sizeof (message), ((char *) ("line %6d, word %6d - %s")), lnum, spin -> si_foldwcount + spin -> si_keepwcount, w);
            msg_start ();
        ...
        if (!((hi)->hi_key==((void*)0)||(hi)->hi_key==&hash_removed)) {
            if (p_verbose > 0) smsg ((char_u *) ((char *) ("Duplicate word in %s line %d: %s")), fname, lnum, dw);
            else if (duplicate == 0) smsg ((char_u *) ((char *) ("First duplicate word in %s line %d: %s")), fname, lnum, dw);
            ++duplicate;
        }
        ...
        if (afflist != ((void *) 0)) {
            ...
            if (affile -> af_needaffix != 0 && flag_in_afflist (affile -> af_flagtype, afflist, affile -> af_needaffix)) need_affix = 1;
            if (affile -> af_pfxpostpone) pfxlen = get_pfxlist (affile, afflist, store_afflist);
            if (spin -> si_compflags != ((void *) 0)) get_compflags (affile, afflist, store_afflist + pfxlen);
        }
        if (store_word (spin, dw, flags, spin -> si_region, store_afflist, need_affix) == 0) retval = 0;
        if (afflist != ((void *) 0)) {
            if (store_aff_word (spin, dw, afflist, affile, &affile->af_suff, &affile->af_pref, 4, flags, store_afflist, pfxlen) == 0) retval = 0;
            if (store_aff_word (spin, dw, afflist, affile, &affile->af_pref, ((void *) 0), 4, flags, store_afflist, pfxlen) == 0) retval = 0;
        }
        vim_free (pc);
    }
    if (duplicate > 0) smsg ((char_u *) ((char *) ("%d duplicate word(s) in %s")), duplicate, fname);
    if (spin -> si_ascii && non_ascii > 0) smsg ((char_u *) ((char *) ("Ignored %d word(s) with non-ASCII characters in %s")), non_ascii, fname);
    hash_clear (&ht);
    fclose (fd);
    return retval;
}
...
{
    int flags = 0;
    if (affile -> af_keepcase != 0 && flag_in_afflist (affile -> af_flagtype, afflist, affile -> af_keepcase)) flags |= 0x80 | 0x40;
    if (affile -> af_rare != 0 && flag_in_afflist (affile -> af_flagtype, afflist, affile -> af_rare)) flags |= 0x08;
    if (affile -> af_bad != 0 && flag_in_afflist (affile -> af_flagtype, afflist, affile -> af_bad)) flags |= 0x10;
    if (affile -> af_needcomp != 0 && flag_in_afflist (affile -> af_flagtype, afflist, affile -> af_needcomp)) flags |= 0x0200;
    if (affile -> af_comproot != 0 && flag_in_afflist (affile -> af_flagtype, afflist, affile -> af_comproot)) flags |= 0x0800;
    if (affile -> af_nosuggest != 0 && flag_in_afflist (affile -> af_flagtype, afflist, affile -> af_nosuggest)) flags |= 0x0400;
    return flags;
}
    ...
    char_u * store_afflist;
{
    ...
    for (p = afflist; *p != '\000';) {
        ...
        if (get_affitem (affile -> af_flagtype, &p) != 0) {
            ...
            if (!((hi)->hi_key==((void*)0)||(hi)->hi_key==&hash_removed)) store_afflist [cnt ++] = ((compitem_T *) (hi) -> hi_key) -> ci_newID;
        }
        if (affile -> af_flagtype == 3 && *p == ',') ++p;
    }
    store_afflist [cnt] = '\000';
...
static int store_aff_word (spin, word, afflist, affile, ht, xht, condit, flags, pfxlist, pfxlen)
    spellinfo_T * spin;
    char_u * word;
    char_u * afflist;
    afffile_T * affile;
    ...
    int pfxlen;
{
    ...
    for (hi = ht -> ht_array; todo > 0 && retval == 1; ++hi) {
        if (!((hi)->hi_key==((void*)0)||(hi)->hi_key==&hash_removed)) {
            --todo;
            ah = ((affheader_T *) (hi) -> hi_key);
            if (((condit & 1) == 0 || ah -> ah_combine) && flag_in_afflist (affile -> af_flagtype, afflist, ah -> ah_flag)) {
                for (ae = ah -> ah_first; ae != ((void *) 0); ae = ae -> ae_next) {
                    regmatch.regprog = ae -> ae_prog;
                    regmatch.rm_ic = 0;
                    if ((xht != ((void *) 0) || !affile->af_pfxpostpone || ae -> ae_chop != ((void *) 0) || ae -> ae_flags != ((void *) 0)) && (ae -> ae_chop ==
                      ((void *) 0) || strlen ((char *) (ae -> ae_chop)) < wordlen) && (ae -> ae_prog == ((void *) 0) || vim_regexec (&regmatch, word, (colnr_T)
                      0)) && (((condit & 2) == 0) == ((condit & 8) == 0 || ae -> ae_flags == ((void *) 0) || 
                      !flag_in_afflist(affile->af_flagtype,ae->ae_flags,affile->af_circumfix)))) {
                        if (xht == ((void *) 0)) {
                            if (ae -> ae_add == ((void *) 0)) *newword = '\000';
                            else ((__builtin_object_size ((char *) (newword), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (newword), (char *) (ae ->
                              ae_add), __builtin_object_size ((char *) (newword), 1 > 1)) : __inline_strcpy_chk ((char *) (newword), (char *) (ae -> ae_add)));
                            p = word;
                            if (ae -> ae_chop != ((void *) 0)) {
                                p += strlen ((char *) (ae -> ae_chop));
                            }
                            ((__builtin_object_size ((char *) (newword), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (newword), (char *) (p),
                              __builtin_object_size ((char *) (newword), 1 > 1)) : __inline_strcat_chk ((char *) (newword), (char *) (p)));
                        }
                        else {
                            ((__builtin_object_size ((char *) (newword), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (newword), (char *) (word),
                              __builtin_object_size ((char *) (newword), 1 > 1)) : __inline_strcpy_chk ((char *) (newword), (char *) (word)));
                            if (ae -> ae_chop != ((void *) 0)) {
                            ...
                            if (ae -> ae_add != ((void *) 0)) ((__builtin_object_size ((char *) (newword), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *
                              ) (newword), (char *) (ae -> ae_add), __builtin_object_size ((char *) (newword), 1 > 1)) : __inline_strcat_chk ((char *) (newword
                              ), (char *) (ae -> ae_add)));
                        }
                        use_flags = flags;
                        use_pfxlist = pfxlist;
                        ...
                        if (ae -> ae_flags != ((void *) 0)) {
                            ...
                            if (affile -> af_needaffix != 0 && flag_in_afflist (affile -> af_flagtype, ae -> ae_flags, affile -> af_needaffix)) need_affix = 1;
                            if (affile -> af_circumfix != 0 && flag_in_afflist (affile -> af_flagtype, ae -> ae_flags, affile -> af_circumfix)) {
                                use_condit |= 2;
                                if ((condit & 2) == 0) need_affix = 1;
                            }
                            if (affile -> af_pfxpostpone || spin -> si_compflags != ((void *) 0)) {
                                if (affile -> af_pfxpostpone) use_pfxlen = get_pfxlist (affile, ae -> ae_flags, store_afflist);
                                else use_pfxlen = 0;
                                use_pfxlist = store_afflist;
                                for (i = 0; i < pfxlen; ++i) {
                                    for (j = 0; j < use_pfxlen; ++j) if (pfxlist [i] == use_pfxlist [j]) break;
                                    if (j == use_pfxlen) use_pfxlist [use_pfxlen ++] = pfxlist [i];
                                }
                                if (spin -> si_compflags != ((void *) 0)) get_compflags (affile, ae -> ae_flags, use_pfxlist + use_pfxlen);
                                for (i = pfxlen; pfxlist [i] != '\000'; ++i) {
                                    for (j = use_pfxlen; use_pfxlist [j] != '\000'; ++j) if (pfxlist [i] == use_pfxlist [j]) break;
                        ...
                        if (spin -> si_prefroot != ((void *) 0) && spin -> si_prefroot -> wn_sibling != ((void *) 0)) {
                            use_flags |= 0x0100;
                            if (!ah->ah_combine && use_pfxlist != ((void *) 0)) use_pfxlist += use_pfxlen;
                        }
                        if (spin -> si_compflags != ((void *) 0) && !ae->ae_comppermit) {
                            if (xht != ((void *) 0)) use_flags |= 0x2000;
                            else use_flags |= 0x1000;
                        }
                        if (store_word (spin, newword, use_flags, spin -> si_region, use_pfxlist, need_affix) == 0) retval = 0;
                        if ((condit & 4) && ae -> ae_flags != ((void *) 0)) if (store_aff_word (spin, newword, ae -> ae_flags, affile, &affile->af_suff, xht,
                          use_condit & (xht == ((void *) 0) ? ~0 : ~4), use_flags, use_pfxlist, pfxlen) == 0) retval = 0;
                        if (xht != ((void *) 0) && ah -> ah_combine) {
                            if (store_aff_word (spin, newword, afflist, affile, xht, ((void *) 0), use_condit, use_flags, use_pfxlist, pfxlen) == 0 || (ae ->
                              ae_flags != ((void *) 0) && store_aff_word (spin, newword, ae -> ae_flags, affile, xht, ((void *) 0), use_condit, use_flags,
                              use_pfxlist, pfxlen) == 0)) retval = 0;
                        }
                    }
                }
            }
    ...
    char_u * fname;
{
    ...
    while (!vim_fgets(rline,500,fd) && !got_int) {
        ...
        if (*line == '/') {
            ...
            if (strncmp ((char *) (line), (char *) ("encoding="), (size_t) (9)) == 0) {
                if (spin -> si_conv.vc_type != 0) smsg ((char_u *) ((char *) ("Duplicate /encoding= line ignored in %s line %d: %s")), fname, lnum, line - 1);
                else if (did_word) smsg ((char_u *) ((char *) ("/encoding= line after word ignored in %s line %d: %s")), fname, lnum, line - 1);
                else {
                    smsg ((char_u *) ((char *) ("Conversion in %s not supported")), fname);
                }
                continue;
            ...
            if (strncmp ((char *) (line), (char *) ("regions="), (size_t) (8)) == 0) {
                if (spin -> si_region_count > 1) smsg ((char_u *) ((char *) ("Duplicate /regions= line ignored in %s line %d: %s")), fname, lnum, line);
                else {
                    line += 8;
                    if (strlen ((char *) (line)) > 16) smsg ((char_u *) ((char *) ("Too many regions in %s line %d: %s")), fname, lnum, line);
                    else {
                        spin -> si_region_count = (int) strlen ((char *) (line)) / 2;
                        ((__builtin_object_size ((char *) (spin -> si_region_name), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (spin ->
                          si_region_name), (char *) (line), __builtin_object_size ((char *) (spin -> si_region_name), 1 > 1)) : __inline_strcpy_chk ((char *) (
                          spin -> si_region_name), (char *) (line)));
                        spin -> si_region = (1 << spin -> si_region_count) - 1;
                    }
                }
                continue;
            }
            smsg ((char_u *) ((char *) ("/ line ignored in %s line %d: %s")), fname, lnum, line - 1);
            continue;
        }
        flags = 0;
        ...
        if (p != ((void *) 0)) {
            *p++ = '\000';
            while (*p != '\000') {
                if (*p == '=') flags |= 0x80 | 0x40;
                else if (*p == '!') flags |= 0x10;
                else if (*p == '?') flags |= 0x08;
                else if (((*p) >= '0' && (*p) <= '9')) {
                    if ((flags & 0x01) == 0) regionmask = 0;
                    flags |= 0x01;
                    l = *p - '0';
                    if (l > spin -> si_region_count) {
                        smsg ((char_u *) ((char *) ("Invalid region nr in %s line %d: %s")), fname, lnum, p);
                        break;
                    }
                    regionmask |= 1 << (l - 1);
                }
                else {
                    smsg ((char_u *) ((char *) ("Unrecognized flags in %s line %d: %s")), fname, lnum, p);
                    break;
                }
                ++p;
            }
    ...
    if (spin -> si_ascii && non_ascii > 0) {
        vim_snprintf ((char *) IObuff, (1024 + 1), ((char *) ("Ignored %d words with non-ASCII characters")), non_ascii);
        spell_message (spin, IObuff);
    }
    return retval;
}
    ...
    int align;
{
    ...
    if (align && bl != ((void *) 0)) bl -> sb_used = (bl -> sb_used + sizeof (char *) -1) & ~(sizeof(char*)-1);
    ...
    char_u * s;
{
    ...
    if (sc != ((void *) 0)) ((__builtin_object_size ((char *) (sc), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (sc), (char *) (s),
      __builtin_object_size ((char *) (sc), 1 > 1)) : __inline_strcpy_chk ((char *) (sc), (char *) (s)));
    return sc;
}
    ...
    int need_affix;
{
    ...
    for (p = pfxlist; res == 1; ++p) {
        if (!need_affix || (p != ((void *) 0) && *p != '\000')) res = tree_add_word (spin, foldword, spin -> si_foldroot, ct | flags, region, p == ((void *) 0)
          ? 0 : *p);
        if (p == ((void *) 0) || *p == '\000') break;
    }
    ++spin->si_foldwcount;
    if (res == 1 && (ct == 0x80 || (flags & 0x80))) {
        for (p = pfxlist; res == 1; ++p) {
            if (!need_affix || (p != ((void *) 0) && *p != '\000')) res = tree_add_word (spin, word, spin -> si_keeproot, flags, region, p == ((void *) 0) ? 0 :
              *p);
            if (p == ((void *) 0) || *p == '\000') break;
        }
        ++spin->si_keepwcount;
    ...
    int affixID;
{
    ...
    for (i = 0;; ++i) {
        if (node != ((void *) 0) && node -> wn_refs > 1) {
            ...
            for (copyp = node; copyp != ((void *) 0); copyp = copyp -> wn_sibling) {
                ...
                if (np == ((void *) 0)) return 0;
                np -> wn_child = copyp -> wn_child;
                if (np -> wn_child != ((void *) 0)) ++np->wn_child->wn_refs;
                ...
                np -> wn_refs = 1;
                if (copyprev != ((void *) 0)) *copyprev = np;
                copyprev = &np->wn_sibling;
                if (copyp == node) node = np;
            }
        }
        while (node != ((void *) 0) && (node -> wn_byte < word [i] || (node -> wn_byte == '\000' && (flags < 0 ? node -> wn_affixID < (unsigned) affixID : (
          node -> wn_flags < (unsigned) (flags & 0xffff) || (node -> wn_flags == (flags & 0xffff) && (spin -> si_sugtree ? (node -> wn_region & 0xffff) < region
          : node -> wn_affixID < (unsigned) affixID))))))) {
            prev = &node->wn_sibling;
            node = *prev;
        }
        if (node == ((void *) 0) || node -> wn_byte != word [i] || (word [i] == '\000' && (flags < 0 || spin -> si_sugtree || node -> wn_flags != (flags &
          0xffff) || node -> wn_affixID != affixID))) {
            np = get_wordnode (spin);
            if (np == ((void *) 0)) return 0;
            np -> wn_byte = word [i];
            if (node == ((void *) 0)) np -> wn_refs = 1;
            else {
                np -> wn_refs = node -> wn_refs;
                node -> wn_refs = 1;
            }
            if (prev != ((void *) 0)) *prev = np;
            np -> wn_sibling = node;
            node = np;
        }
        if (word [i] == '\000') {
    ...
    if (spin -> si_compress_cnt > 1) {
        if (--spin->si_compress_cnt == 1) spin -> si_blocks_cnt += compress_inc;
    }
    if (spin -> si_compress_cnt == 1 ? spin -> si_free_count < 250 : spin -> si_blocks_cnt >= compress_start) {
        ...
        if (affixID >= 0) wordtree_compress (spin, spin -> si_keeproot);
    }
    return 1;
}
...
int spell_check_msm () {
    ...
    if (*p != '\000') return 0;
    if (start == 0 || incr == 0 || added == 0 || incr > start) return 0;
...
{
    wordnode_T * n;
    if (spin -> si_first_free == ((void *) 0)) n = (wordnode_T *) getroom (spin, sizeof (wordnode_T), 1);
    else {
        n = spin -> si_first_free;
        spin -> si_first_free = n -> wn_child;
        ((__builtin_object_size ((n), 0) != (size_t) - 1) ? __builtin___memset_chk ((n), (0), (sizeof (wordnode_T)), __builtin_object_size ((n), 0)) :
          __inline_memset_chk ((n), (0), (sizeof (wordnode_T))));
        --spin->si_free_count;
    }
    return n;
}
    ...
    wordnode_T * node;
{
    ...
    if (--node->wn_refs == 0) {
        for (np = node; np != ((void *) 0); np = np -> wn_sibling) {
            if (np -> wn_child != ((void *) 0)) cnt += deref_wordnode (spin, np -> wn_child);
    ...
    wordnode_T * root;
{
    ...
    if (root -> wn_sibling != ((void *) 0)) {
        ...
        if (spin -> si_verbose || p_verbose > 2) {
            if (tot > 1000000) perc = (tot - n) / (tot / 100);
            else if (tot == 0) perc = 0;
            else perc = (tot - n) * 100 / tot;
            vim_snprintf ((char *) IObuff, (1024 + 1), ((char *) ("Compressed %d of %d nodes; %d (%d%%) remaining")), n, tot, tot - n, perc);
            spell_message (spin, IObuff);
        }
        hash_clear (&ht);
    }
    ...
    int * tot;
{
    ...
    for (np = node; np != ((void *) 0) && !got_int; np = np -> wn_sibling) {
        ...
        if ((child = np -> wn_child) != ((void *) 0)) {
            ...
            if (!((hi)->hi_key==((void*)0)||(hi)->hi_key==&hash_removed)) {
                for (tp = (wordnode_T *) ((hi) -> hi_key); tp != ((void *) 0); tp = tp -> wn_u2.next) if (node_equal (child, tp)) {
                    ++tp->wn_refs;
    ...
    *tot += len + 1;
    ...
    nr = 0;
    for (np = node; np != ((void *) 0); np = np -> wn_sibling) {
        if (np -> wn_byte == '\000') n = np -> wn_flags + (np -> wn_region << 8) + (np -> wn_affixID << 16);
    ...
    wordnode_T * n2;
{
    wordnode_T * p1;
    wordnode_T * p2;
    for (p1 = n1, p2 = n2; p1 != ((void *) 0) && p2 != ((void *) 0); p1 = p1 -> wn_sibling, p2 = p2 -> wn_sibling) if (p1 -> wn_byte != p2 -> wn_byte || (p1 ->
      wn_byte == '\000' ? (p1 -> wn_flags != p2 -> wn_flags || p1 -> wn_region != p2 -> wn_region || p1 -> wn_affixID != p2 -> wn_affixID) : (p1 -> wn_child !=
      p2 -> wn_child))) break;
...
static int rep_compare (s1, s2) const
    void * s1;
    const void * s2;
{
    ...
    char_u * fname;
{
    ...
    if (!spin->si_ascii && !spin->si_add) {
        ...
        for (i = 128; i < 256; ++i) {
            flags = 0;
            if (spelltab.st_isw [i]) flags |= 0x01;
            if (spelltab.st_isu [i]) flags |= 0x02;
            fputc (flags, fd);
        }
    ...
    for (round = 1; round <= 3; ++round) {
        if (round == 1) gap = &spin->si_rep;
        else if (round == 2) {
            if (spin -> si_sofofr != ((void *) 0) && spin -> si_sofoto != ((void *) 0)) continue;
            gap = &spin->si_sal;
        }
        else gap = &spin->si_repsal;
        if (gap -> ga_len == 0) continue;
        if (round != 2) qsort (gap -> ga_data, (size_t) gap -> ga_len, sizeof (fromto_T), rep_compare);
        ...
        if (round == 2) {
            i = 0;
            if (spin -> si_followup) i |= 1;
            if (spin -> si_collapse) i |= 2;
            if (spin -> si_rem_accents) i |= 4;
            putc (i, fd);
        }
        ...
        for (i = 0; i < gap -> ga_len; ++i) {
            ...
            for (rr = 1; rr <= 2; ++rr) {
                ...
                l = (int) strlen ((char *) (p));
                putc (l, fd);
                if (l > 0) fwv &= fwrite (p, l, (size_t) 1, fd);
            }
        }
    }
    ...
    if (spin -> si_commonwords.ht_used > 0) {
        ...
        for (round = 1; round <= 2; ++round) {
            ...
            for (hi = spin -> si_commonwords.ht_array; todo > 0; ++hi) if (!((hi)->hi_key==((void*)0)||(hi)->hi_key==&hash_removed)) {
                l = (int) strlen ((char *) (hi -> hi_key)) + 1;
                len += l;
                if (round == 2) fwv &= fwrite (hi -> hi_key, (size_t) l, (size_t) 1, fd);
                --todo;
            }
            if (round == 1) put_bytes (fd, (long_u) len, 4);
        }
    }
    ...
    if (!spin->si_nosugfile && (spin -> si_sal.ga_len > 0 || (spin -> si_sofofr != ((void *) 0) && spin -> si_sofoto != ((void *) 0)))) {
        putc (11, fd);
        putc (0, fd);
    ...
    if (spin -> si_compflags != ((void *) 0)) {
        ...
        for (i = 0; i < spin -> si_comppat.ga_len; ++i) l += (int) strlen ((char *) (((char_u * *) (spin -> si_comppat.ga_data)) [i])) + 1;
        ...
        fwv &= fwrite (spin -> si_compflags, (size_t) strlen ((char *) (spin -> si_compflags)), (size_t) 1, fd);
    }
    if (spin -> si_nobreak) {
        putc (10, fd);
        putc (0, fd);
    ...
    spin -> si_memtot = 0;
    for (round = 1; round <= 3; ++round) {
        if (round == 1) tree = spin -> si_foldroot -> wn_sibling;
        else if (round == 2) tree = spin -> si_keeproot -> wn_sibling;
    ...
    if (putc (0, fd) == (-1)) retval = 0;
    theend : if (fclose (fd) == (-1)) retval = 0;
    if (fwv != (size_t) 1) retval = 0;
    if (retval == 0) emsg ((char_u *) (((char *) (e_write))));
    return retval;
}
...
{
    wordnode_T * np;
    if (node != ((void *) 0)) for (np = node; np != ((void *) 0); np = np -> wn_sibling) {
        np -> wn_u1.index = 0;
        np -> wn_u2.wnode = ((void *) 0);
        if (np -> wn_byte != '\000') clear_node (np -> wn_child);
    }
}
    ...
    int prefixtree;
{
    ...
    for (np = node; np != ((void *) 0); np = np -> wn_sibling) ++siblingcount;
    if (fd != ((void *) 0)) putc (siblingcount, fd);
    for (np = node; np != ((void *) 0); np = np -> wn_sibling) {
        if (np -> wn_byte == 0) {
            if (fd != ((void *) 0)) {
                if (prefixtree) {
                    if (np -> wn_flags == (short_u) - 256) putc (0, fd);
                    else {
                        putc (2, fd);
                        putc (np -> wn_flags, fd);
                    }
                    ...
                    flags = np -> wn_flags;
                    if (regionmask != 0 && np -> wn_region != regionmask) flags |= 0x01;
                    if (np -> wn_affixID != 0) flags |= 0x20;
                    if (flags == 0) {
                        putc (0, fd);
                    }
                    else {
                        ...
                        if (flags & 0x01) putc (np -> wn_region, fd);
                        if (flags & 0x20) putc (np -> wn_affixID, fd);
                    }
                }
            }
        }
            ...
            else if (np -> wn_child -> wn_u2.wnode == ((void *) 0)) np -> wn_child -> wn_u2.wnode = node;
            if (fd != ((void *) 0)) if (putc (np -> wn_byte, fd) == (-1)) {
    ...
    for (np = node; np != ((void *) 0); np = np -> wn_sibling) if (np -> wn_byte != 0 && np -> wn_child -> wn_u2.wnode == node) newindex = put_node (fd, np ->
      wn_child, newindex, regionmask, prefixtree);
    return newindex;
}
    ...
    char_u * wfname;
{
    ...
    slang_T * slang;
    int free_slang = 0;
    for (slang = first_lang; slang != ((void *) 0); slang = slang -> sl_next) if (fullpathcmp (wfname, slang -> sl_fname, 0) == 1) break;
    if (slang == ((void *) 0)) {
        spell_message (spin, (char_u *) ((char *) ("Reading back spell file...")));
        slang = spell_load_file (wfname, ((void *) 0), ((void *) 0), 0);
        if (slang == ((void *) 0)) return;
        free_slang = 1;
    }
    ...
    if (sug_filltree (spin, slang) == 0) goto theend;
    if (sug_maketable (spin) == 0) goto theend;
    smsg ((char_u *) ((char *) ("Number of words after soundfolding: %ld")), (long) spin -> si_spellbuf -> b_ml.ml_line_count);
    spell_message (spin, (char_u *) ((char *) (msg_compressing)));
    wordtree_compress (spin, spin -> si_foldroot);
    ((__builtin_object_size ((char *) (fname), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (fname), (char *) (wfname), __builtin_object_size ((char *
      ) (fname), 1 > 1)) : __inline_strcpy_chk ((char *) (fname), (char *) (wfname)));
    len = (int) strlen ((char *) (fname));
    fname [len - 2] = 'u';
    fname [len - 1] = 'g';
    sug_write (spin, fname);
    theend : if (free_slang) slang_free (slang);
    ...
    slang_T * slang;
{
    ...
    if (spin -> si_foldroot == ((void *) 0)) return 0;
    spin -> si_sugtree = 1;
    byts = slang -> sl_fbyts;
    idxs = slang -> sl_fidxs;
    arridx [0] = 0;
    ...
    while (depth >= 0 && !got_int) {
        if (curi [depth] > byts [arridx [depth]]) {
            ...
            if (depth > 0) wordcount [depth - 1] += wordcount [depth];
            --depth;
            line_breakcheck ();
        }
        else {
            ...
            if (c == 0) {
                ...
                if (tree_add_word (spin, tsalword, spin -> si_foldroot, words_done >> 16, words_done & 0xffff, 0) == 0) return 0;
                ++words_done;
                ++wordcount[depth];
                spin -> si_blocks_cnt = 0;
                while (byts [n + 1] == 0) {
    ...
    spellinfo_T * spin;
{
    ...
    if (spin -> si_spellbuf == ((void *) 0)) return 0;
    ga_init2 (&ga, 1, 100);
    if (sug_filltable (spin, spin -> si_foldroot -> wn_sibling, 0, &ga) == -1) res = 0;
    ga_clear (&ga);
    return res;
}
    ...
    garray_T * gap;
{
    ...
    for (p = node; p != ((void *) 0); p = p -> wn_sibling) {
        if (p -> wn_byte == '\000') {
            ...
            if (ml_append_buf (spin -> si_spellbuf, (linenr_T) wordnr, gap -> ga_data, gap -> ga_len, 1) == 0) return -1;
            ++wordnr;
            while (p -> wn_sibling != ((void *) 0) && p -> wn_sibling -> wn_byte == '\000') p -> wn_sibling = p -> wn_sibling -> wn_sibling;
            p -> wn_flags = 0;
            p -> wn_region = 0;
        }
        else {
            wordnr = sug_filltable (spin, p -> wn_child, wordnr, gap);
            if (wordnr == -1) return -1;
        }
    }
    return wordnr;
}
    ...
    char_u * fname;
{
    ...
    vim_snprintf ((char *) IObuff, (1024 + 1), ((char *) ("Writing suggestion file %s ...")), fname);
    spell_message (spin, IObuff);
    ...
    if (putc (0, fd) == (-1)) emsg ((char_u *) (((char *) (e_write))));
    vim_snprintf ((char *) IObuff, (1024 + 1), ((char *) ("Estimated runtime memory use: %d bytes")), spin -> si_memtot);
    spell_message (spin, IObuff);
    theend : fclose (fd);
}
...
static void mkspell (fcount, fnames, ascii, overwrite, added_word)
    int fcount;
    char_u * * fnames;
    int ascii;
    int overwrite;
    int added_word;
{
    ...
    int error = 0;
    spellinfo_T spin;
    ((__builtin_object_size ((&spin), 0) != (size_t) - 1) ? __builtin___memset_chk ((&spin), (0), (sizeof (spin)), __builtin_object_size ((&spin), 0)) :
      __inline_memset_chk ((&spin), (0), (sizeof (spin))));
    ...
    if (fcount >= 1) {
        ...
        if (fcount == 1 && len > 4 && strcmp ((char *) (fnames [0] + len - 4), (char *) (".add")) == 0) {
            ...
            innames = &fnames[0];
            incount = 1;
            vim_snprintf ((char *) wfname, sizeof (wfname), "%s.%s.spl", fnames [0], spin.si_ascii ? (char_u *) "ascii" : spell_enc ());
        }
        else if (len > 4 && strcmp ((char *) (fnames [0] + len - 4), (char *) (".spl")) == 0) {
            vim_strncpy (wfname, fnames [0], sizeof (wfname) - 1);
        }
        else vim_snprintf ((char *) wfname, sizeof (wfname), "%s.%s.spl", fnames [0], spin.si_ascii ? (char_u *) "ascii" : spell_enc ());
        if (strstr ((char *) gettail (wfname), ".ascii.") != ((void *) 0)) spin.si_ascii = 1;
        if (strstr ((char *) gettail (wfname), ".add.") != ((void *) 0)) spin.si_add = 1;
    }
    if (incount <= 0) emsg ((char_u *) (((char *) (e_invarg))));
    else if (vim_strchr (gettail (wfname), '_') != ((void *) 0)) emsg ((char_u *) (((char *) ("E751: Output file name must not have region name"))));
    else if (incount > 8) emsg ((char_u *) (((char *) ("E754: Only up to 8 regions supported"))));
    else {
        ...
        for (i = 0; i < incount; ++i) {
            ...
            if (incount > 1) {
                ...
                if (strlen ((char *) (gettail (innames [i]))) < 5 || innames [i] [len - 3] != '_') {
                    emsg2 ((char_u *) (((char *) ("E755: Invalid region in %s"))), (char_u *) (innames [i]));
                    return;
                }
                spin.si_region_name [i * 2] = (((innames [i] [len - 2]) < 'A' || (innames [i] [len - 2]) > 'Z') ? (innames [i] [len - 2]) : (innames [i] [len -
                  2]) + ('a' - 'A'));
                spin.si_region_name [i * 2 + 1] = (((innames [i] [len - 1]) < 'A' || (innames [i] [len - 1]) > 'Z') ? (innames [i] [len - 1]) : (innames [i] [
                  len - 1]) + ('a' - 'A'));
            }
        }
        ...
        if (spin.si_foldroot == ((void *) 0) || spin.si_keeproot == ((void *) 0) || spin.si_prefroot == ((void *) 0)) {
            free_blocks (spin.si_blocks);
            return;
        }
        if (!spin.si_add) spin.si_clear_chartab = 1;
        for (i = 0; i < incount && !error; ++i) {
            ...
            if (stat (((char *) fname), (&st)) >= 0) {
                ...
                if (afile [i] == ((void *) 0)) error = 1;
                else {
                    vim_snprintf ((char *) fname, sizeof (fname), "%s.dic", innames [i]);
                    if (spell_read_dic (&spin, fname, afile [i]) == 0) error = 1;
                }
            }
            else {
                if (spell_read_wordfile (&spin, innames [i]) == 0) error = 1;
            }
        }
        if (spin.si_compflags != ((void *) 0) && spin.si_nobreak) msg ((char_u *) (((char *) ("Warning: both compounding and NOBREAK specified"))));
        if (!error && !got_int) {
            spell_message (&spin, (char_u *) ((char *) (msg_compressing)));
            wordtree_compress (&spin, spin.si_foldroot);
        ...
        if (!error && !got_int) {
            vim_snprintf ((char *) IObuff, (1024 + 1), ((char *) ("Writing spell file %s ...")), wfname);
            spell_message (&spin, IObuff);
            error = write_vim_spell (&spin, wfname) == 0;
            spell_message (&spin, (char_u *) ((char *) ("Done!")));
            vim_snprintf ((char *) IObuff, (1024 + 1), ((char *) ("Estimated runtime memory use: %d bytes")), spin.si_memtot);
            spell_message (&spin, IObuff);
            if (!error) spell_reload_one (wfname, added_word);
        }
        ...
        for (i = 0; i < incount; ++i) if (afile [i] != ((void *) 0)) spell_free_aff (afile [i]);
        free_blocks (spin.si_blocks);
        if (spin.si_sugtime != 0 && !error && !got_int) spell_make_sugfile (&spin, wfname);
    }
}
...
{
    if (spin -> si_verbose || p_verbose > 2) {
        if (!spin->si_verbose) verbose_enter ();
        msg ((char_u *) (str));
        out_flush ();
        if (!spin->si_verbose) verbose_leave ();
    }
}
...
void ex_spell (eap)
    exarg_T * eap;
{
    spell_add_word (eap -> arg, (int) strlen ((char *) (eap -> arg)), eap -> cmdidx == CMD_spellwrong, eap -> forceit ? 0 : (int) eap -> line2, eap -> cmdidx ==
      CMD_spellundo);
}
    ...
    int undo;
{
    ...
    if (idx == 0) {
        ...
        if (buf != ((void *) 0) && buf -> b_ml.ml_mfp == ((void *) 0)) buf = ((void *) 0);
    ...
    if (bad || undo) {
        ...
        if (fd != ((void *) 0)) {
            while (!vim_fgets(line,250*2,fd)) {
                fpos = fpos_next;
                fpos_next = ftell (fd);
                if (strncmp ((char *) (word), (char *) (line), (size_t) (len)) == 0 && (line [len] == '/' || line [len] < ' ')) {
                    fclose (fd);
                    fd = fopen (((char *) fname), ("r+"));
                    if (fd == ((void *) 0)) break;
    ...
    if (!undo) {
        ...
        if (fd == ((void *) 0)) emsg2 ((char_u *) (((char *) (e_notopen))), (char_u *) (fname));
        else {
            if (bad) fprintf (fd, "%.*s/!\n", len, word);
            else fprintf (fd, "%.*s\n", len, word);
            fclose (fd);
    ...
    if (fd != ((void *) 0)) {
        ...
        if (buf != ((void *) 0)) buf_reload (buf, buf -> b_orig_mode);
        redraw_all_later (35);
    }
}
...
static void init_spellfile () {
    ...
    if (*curwin->w_s->b_p_spl != '\000' && curwin -> w_s -> b_langp.ga_len > 0) {
        for (lend = curwin -> w_s -> b_p_spl; *lend != '\000' && vim_strchr ((char_u *) ",._", *lend) == ((void *) 0); ++lend) if (vim_ispathsep (*lend)) {
            aspath = 1;
            lstart = lend + 1;
        }
        rtp = p_rtp;
        while (*rtp != '\000') {
            if (aspath) vim_strncpy (buf, curbuf -> b_s.b_p_spl, lstart - curbuf -> b_s.b_p_spl - 1);
            else copy_option_part (&rtp, buf, 1024, ",");
            if (filewritable (buf) == 2) {
                if (aspath) vim_strncpy (buf, curbuf -> b_s.b_p_spl, lend - curbuf -> b_s.b_p_spl);
                else {
                    l = (int) strlen ((char *) (buf));
                    vim_snprintf ((char *) buf + l, 1024 - l, "/spell");
                    if (!filewritable(buf) != 2) mkdir ((char *) (buf), 0755);
                    l = (int) strlen ((char *) (buf));
                    vim_snprintf ((char *) buf + l, 1024 - l, "/%.*s", (int) (lend - lstart), lstart);
                }
                l = (int) strlen ((char *) (buf));
                fname = (((langp_T *) (curwin -> w_s -> b_langp).ga_data) + (0)) -> lp_slang -> sl_fname;
                vim_snprintf ((char *) buf + l, 1024 - l, ".%s.add", fname != ((void *) 0) && strstr ((char *) gettail (fname), ".ascii.") != ((void *) 0) ? (
...
{
    int i;
    ((__builtin_object_size ((sp -> st_isw), 0) != (size_t) - 1) ? __builtin___memset_chk ((sp -> st_isw), (0), (sizeof (sp -> st_isw)), __builtin_object_size (
      (sp -> st_isw), 0)) : __inline_memset_chk ((sp -> st_isw), (0), (sizeof (sp -> st_isw))));
    ((__builtin_object_size ((sp -> st_isu), 0) != (size_t) - 1) ? __builtin___memset_chk ((sp -> st_isu), (0), (sizeof (sp -> st_isu)), __builtin_object_size (
      (sp -> st_isu), 0)) : __inline_memset_chk ((sp -> st_isu), (0), (sizeof (sp -> st_isu))));
    for (i = 0; i < 256; ++i) {
        sp -> st_fold [i] = i;
        sp -> st_upper [i] = i;
    }
    for (i = '0'; i <= '9'; ++i) sp -> st_isw [i] = 1;
    for (i = 'A'; i <= 'Z'; ++i) {
        sp -> st_isw [i] = 1;
        sp -> st_isu [i] = 1;
        sp -> st_fold [i] = i + 0x20;
    ...
    char_u * upp;
{
    ...
    while (*pf != '\000') {
        ...
        l = *pl++;
        u = *pu++;
        if (f < 256) new_st.st_isw [f] = 1;
        if (l < 256) new_st.st_isw [l] = 1;
        if (u < 256) new_st.st_isw [u] = 1;
        if (l < 256 && l != f) {
            if (f >= 256) {
    ...
    char_u * fol;
{
    ...
    for (i = 0; i < 128; ++i) {
        ...
        if (*p != '\000') {
            ...
            if (i + 128 != c && new_st.st_isu [i + 128] && c < 256) new_st.st_upper [c] = i + 128;
        }
    }
    ...
    spelltab_T * new_st;
{
    ...
    if (did_set_spelltab) {
        for (i = 0; i < 256; ++i) {
            if (spelltab.st_isw [i] != new_st -> st_isw [i] || spelltab.st_isu [i] != new_st -> st_isu [i] || spelltab.st_fold [i] != new_st -> st_fold [i] ||
...
static int spell_iswordp_nmw (p)
    char_u * p;
{
    ...
    garray_T * gap;
{
    ...
    int totlen;
    size_t x = 1;
    if (fd != ((void *) 0)) put_bytes (fd, (long_u) gap -> ga_len, 2);
    totlen = 2 + gap -> ga_len;
    for (i = 0; i < gap -> ga_len; ++i) {
        p = ((char_u * *) gap -> ga_data) [i];
        if (p != ((void *) 0)) {
    ...
    int buflen;
{
    ...
    {
        for (i = 0; i < len; ++i) buf [i] = spelltab.st_fold [str [i]];
        buf [i] = '\000';
    }
    return 1;
}
...
int spell_check_sps () {
    ...
    for (p = p_sps; *p != '\000';) {
        ...
        if (((*buf) >= '0' && (*buf) <= '9')) {
            ...
            if (*s != '\000' && !((*s) >= '0' && (*s) <= '9')) f = -1;
        }
        else if (strcmp ((char *) (buf), (char *) ("best")) == 0) f = 1;
        else if (strcmp ((char *) (buf), (char *) ("fast")) == 0) f = 2;
        else if (strcmp ((char *) (buf), (char *) ("double")) == 0) f = 4;
        else if (strncmp ((char *) (buf), (char *) ("expr:"), (size_t) (5)) != 0 && strncmp ((char *) (buf), (char *) ("file:"), (size_t) (5)) != 0) f = -1;
    ...
    int count;
{
    ...
    if (no_spell_checking (curwin)) return;
    if (VIsual_active) {
        ...
        if (badlen < 0) badlen = -badlen;
        else curwin -> w_cursor.col = VIsual.col;
        ++badlen;
        end_visual_mode ();
    }
    else if (spell_move_to (curwin, 1, 1, 1, ((void *) 0)) == 0 || curwin -> w_cursor.col > prev_cursor.col) {
        curwin -> w_cursor = prev_cursor;
        line = ml_get_curline ();
        p = line + curwin -> w_cursor.col;
        while (p > line && spell_iswordp_nmw (p)) --p;
        while (*p != '\000' && !spell_iswordp_nmw(p)) ++p;
    ...
    if (line == ((void *) 0)) goto skip;
    if (sps_limit > (int) Rows - 2) limit = (int) Rows - 2;
    else limit = sps_limit;
    spell_find_suggest (line + curwin -> w_cursor.col, badlen, &sug, limit, 1, need_cap, 1);
    if (sug.su_ga.ga_len == 0) msg ((char_u *) (((char *) ("Sorry, no suggestions"))));
    else if (count > 0) {
        if (count > sug.su_ga.ga_len) smsg ((char_u *) ((char *) ("Sorry, only %ld suggestions")), (long) sug.su_ga.ga_len);
    }
    else {
        vim_free (repl_from);
        repl_from = ((void *) 0);
        ...
        msg_row = Rows - 1;
        lines_left = Rows;
        vim_snprintf ((char *) IObuff, (1024 + 1), ((char *) ("Change \"%.*s\" to:")), sug.su_badlen, sug.su_badptr);
        msg_puts (IObuff);
        msg_clr_eos ();
        msg_putchar ('\n');
        msg_scroll = 1;
        for (i = 0; i < sug.su_ga.ga_len; ++i) {
            ...
            ((__builtin_object_size ((char *) (wcopy), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (wcopy), (char *) (stp -> st_word),
              __builtin_object_size ((char *) (wcopy), 1 > 1)) : __inline_strcpy_chk ((char *) (wcopy), (char *) (stp -> st_word)));
            if (sug.su_badlen > stp -> st_orglen) vim_strncpy (wcopy + stp -> st_wordlen, sug.su_badptr + stp -> st_orglen, sug.su_badlen - stp -> st_orglen);
            vim_snprintf ((char *) IObuff, (1024 + 1), "%2d", i + 1);
            msg_puts (IObuff);
            vim_snprintf ((char *) IObuff, (1024 + 1), " \"%s\"", wcopy);
            msg_puts (IObuff);
            if (sug.su_badlen < stp -> st_orglen) {
                vim_snprintf ((char *) IObuff, (1024 + 1), ((char *) (" < \"%.*s\"")), stp -> st_orglen, sug.su_badptr);
                msg_puts (IObuff);
            }
            if (p_verbose > 0) {
                if (sps_flags & (4 | 1)) vim_snprintf ((char *) IObuff, (1024 + 1), " (%s%d - %d)", stp -> st_salscore ? "s " : "", stp -> st_score, stp ->
                  st_altscore);
                else vim_snprintf ((char *) IObuff, (1024 + 1), " (%d)", stp -> st_score);
                msg_advance (30);
                msg_puts (IObuff);
            }
            msg_putchar ('\n');
    ...
    if (selected > 0 && selected <= sug.su_ga.ga_len && u_save_cursor () == 1) {
        ...
        if (sug.su_badlen > stp -> st_orglen) {
            ...
            vim_snprintf ((char *) IObuff, (1024 + 1), "%s%.*s", stp -> st_word, sug.su_badlen - stp -> st_orglen, sug.su_badptr + stp -> st_orglen);
        ...
        p = alloc ((unsigned) strlen ((char *) (line)) - stp -> st_orglen + stp -> st_wordlen + 1);
        if (p != ((void *) 0)) {
            c = (int) (sug.su_badptr - line);
            ((__builtin_object_size ((char *) (p), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (p), (char *) (line), c, __builtin_object_size ((
              char *) (p), 0)) : __inline_memmove_chk ((char *) (p), (char *) (line), c));
            ((__builtin_object_size ((char *) (p + c), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (p + c), (char *) (stp -> st_word),
              __builtin_object_size ((char *) (p + c), 1 > 1)) : __inline_strcpy_chk ((char *) (p + c), (char *) (stp -> st_word)));
            ((__builtin_object_size ((char *) (p), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (p), (char *) (sug.su_badptr + stp -> st_orglen),
              __builtin_object_size ((char *) (p), 1 > 1)) : __inline_strcat_chk ((char *) (p), (char *) (sug.su_badptr + stp -> st_orglen)));
    ...
    else curwin -> w_cursor = prev_cursor;
    ...
    colnr_T col;
{
    ...
    colnr_T endcol;
    regmatch_T regmatch;
    if (curwin -> w_s -> b_cap_prog == ((void *) 0)) return 0;
    line = ml_get_curline ();
    endcol = 0;
    if ((int) (skipwhite (line) - line) >= (int) col) {
        if (lnum == 1) need_cap = 1;
        else {
            line = ml_get (lnum - 1);
            if (*skipwhite(line) == '\000') need_cap = 1;
            else {
    ...
    if (endcol > 0) {
        ...
        for (;;) {
            --p;
            if (p == line || spell_iswordp_nmw (p)) break;
    ...
    exarg_T * eap __attribute__ ((unused));
{
    ...
    while (!got_int) {
        if (do_search (((void *) 0), '/', frompat, 1L, 0x400, ((void *) 0)) == 0 || u_save_cursor () == 0) break;
        line = ml_get_curline ();
        if (addlen <= 0 || strncmp ((char *) (line + curwin -> w_cursor.col), (char *) (repl_to), (size_t) (strlen ((char *) (repl_to)))) != 0) {
            ...
            if (p == ((void *) 0)) break;
            ((__builtin_object_size ((char *) (p), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (p), (char *) (line), curwin -> w_cursor.col,
              __builtin_object_size ((char *) (p), 0)) : __inline_memmove_chk ((char *) (p), (char *) (line), curwin -> w_cursor.col));
            ((__builtin_object_size ((char *) (p + curwin -> w_cursor.col), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (p + curwin -> w_cursor.col)
              , (char *) (repl_to), __builtin_object_size ((char *) (p + curwin -> w_cursor.col), 1 > 1)) : __inline_strcpy_chk ((char *) (p + curwin ->
              w_cursor.col), (char *) (repl_to)));
            ((__builtin_object_size ((char *) (p), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (p), (char *) (line + curwin -> w_cursor.col + strlen
              ((char *) (repl_from))), __builtin_object_size ((char *) (p), 1 > 1)) : __inline_strcat_chk ((char *) (p), (char *) (line + curwin -> w_cursor.
              col + strlen ((char *) (repl_from)))));
            ml_replace (curwin -> w_cursor.lnum, p, 0);
    ...
    curwin -> w_cursor = pos;
    vim_free (frompat);
    if (sub_nsubs == 0) emsg2 ((char_u *) (((char *) ("E753: Not found: %s"))), (char_u *) (repl_from));
    else do_sub_msg (0);
}
    ...
    int interactive;
{
    ...
    if (ga_grow (gap, sug.su_ga.ga_len) == 1) {
        for (i = 0; i < sug.su_ga.ga_len; ++i) {
            stp = &(((suggest_T*)(sug.su_ga).ga_data)[i]);
            wcopy = alloc (stp -> st_wordlen + (unsigned) strlen ((char *) (sug.su_badptr + stp -> st_orglen)) + 1);
            if (wcopy == ((void *) 0)) break;
            ((__builtin_object_size ((char *) (wcopy), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (wcopy), (char *) (stp -> st_word),
              __builtin_object_size ((char *) (wcopy), 1 > 1)) : __inline_strcpy_chk ((char *) (wcopy), (char *) (stp -> st_word)));
            ((__builtin_object_size ((char *) (wcopy + stp -> st_wordlen), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (wcopy + stp -> st_wordlen), (
              char *) (sug.su_badptr + stp -> st_orglen), __builtin_object_size ((char *) (wcopy + stp -> st_wordlen), 1 > 1)) : __inline_strcpy_chk ((char *) (
              wcopy + stp -> st_wordlen), (char *) (sug.su_badptr + stp -> st_orglen)));
...
static void spell_find_suggest (badptr, badlen, su, maxcount, banbadword, need_cap, interactive)
    char_u * badptr;
    int badlen;
    suginfo_T * su;
    int maxcount;
    ...
    int interactive;
{
    ...
    int i;
    langp_T * lp;
    ((__builtin_object_size ((su), 0) != (size_t) - 1) ? __builtin___memset_chk ((su), (0), (sizeof (suginfo_T)), __builtin_object_size ((su), 0)) :
      __inline_memset_chk ((su), (0), (sizeof (suginfo_T))));
    ga_init2 (&su->su_ga, (int) sizeof (suggest_T), 10);
    ga_init2 (&su->su_sga, (int) sizeof (suggest_T), 10);
    if (*badptr == '\000') return;
    hash_init (&su->su_banned);
    su -> su_badptr = badptr;
    if (badlen != 0) su -> su_badlen = badlen;
    else su -> su_badlen = spell_check (curwin, su -> su_badptr, &attr, ((void *) 0), 0);
    su -> su_maxcount = maxcount;
    su -> su_maxscore = 350;
    if (su -> su_badlen >= 250) su -> su_badlen = 250 - 1;
    vim_strncpy (su -> su_badword, su -> su_badptr, su -> su_badlen);
    (void) spell_casefold (su -> su_badptr, su -> su_badlen, su -> su_fbadword, 250);
    su -> su_badflags = badword_captype (su -> su_badptr, su -> su_badptr + su -> su_badlen);
    if (need_cap) su -> su_badflags |= 0x02;
    ...
    if (su -> su_sallang != ((void *) 0)) spell_soundfold (su -> su_sallang, su -> su_fbadword, 1, su -> su_sal_badword);
    c = ((int) *(su->su_badptr));
    if (!((c)<256?spelltab.st_isu[c]:0) && attr == HLF_COUNT) {
        make_case_word (su -> su_badword, buf, 0x02);
        add_suggestion (su, &su->su_ga, buf, su -> su_badlen, 52, 0, 1, su -> su_sallang, 0);
    }
    if (banbadword) add_banned (su, su -> su_badword);
    sps_copy = vim_strsave (p_sps);
    if (sps_copy == ((void *) 0)) return;
    for (p = sps_copy; *p != '\000';) {
        ...
        else if (strncmp ((char *) (buf), (char *) ("file:"), (size_t) (5)) == 0) spell_suggest_file (su, buf + 5);
        else {
            spell_suggest_intern (su, interactive);
            if (sps_flags & 4) do_combine = 1;
        }
    }
    vim_free (sps_copy);
    if (do_combine) score_combine (su);
}
    ...
    char_u * expr;
{
    ...
    if (list != ((void *) 0)) {
        for (li = list -> lv_first; li != ((void *) 0); li = li -> li_next) if (li -> li_tv.v_type == 4) {
            score = get_spellword (li -> li_tv.vval.v_list, &p);
            if (score >= 0 && score <= su -> su_maxscore) add_suggestion (su, &su->su_ga, p, su -> su_badlen, score, 0, 1, su -> su_sallang, 0);
        }
        list_unref (list);
    }
    ...
    while (!vim_fgets(line,250*2,fd) && !got_int) {
        ...
        *p++ = '\000';
        if (strcasecmp ((char *) (su -> su_badword), (char *) (line)) == 0) {
            ...
            add_suggestion (su, &su->su_ga, p, su -> su_badlen, 30, 0, 1, su -> su_sallang, 0);
        }
    }
    fclose (fd);
    check_suggestions (su, &su->su_ga);
    ...
    int interactive;
{
    ...
    if (sps_flags & 4) score_comp_sal (su);
    if ((sps_flags & 2) == 0) {
        if (sps_flags & 1) rescore_suggestions (su);
...
static void suggest_load_files () {
    ...
    for (lpi = 0; lpi < curwin -> w_s -> b_langp.ga_len; ++lpi) {
        ...
        if (slang -> sl_sugtime != 0 && !slang->sl_sugloaded) {
            ...
            if (dotp == ((void *) 0) || strcasecmp ((char *) (((dotp))), (char *) (((".spl")))) != 0) continue;
            ((__builtin_object_size ((char *) (dotp), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (dotp), (char *) (".sug"), __builtin_object_size (
              (char *) (dotp), 1 > 1)) : __inline_strcpy_chk ((char *) (dotp), (char *) (".sug")));
            fd = fopen (((char *) slang -> sl_fname), ("r"));
            if (fd == ((void *) 0)) goto nextone;
            for (i = 0; i < 6; ++i) buf [i] = getc (fd);
            ...
            if (spell_read_tree (fd, &slang->sl_sbyts, &slang->sl_sidxs, 0, 0) != 0) {
                someerror : emsg2 ((char_u *) (((char *) ("E782: error while reading .sug file: %s"))), (char_u *) (slang -> sl_fname));
                slang_clear_sug (slang);
                goto nextone;
            }
            slang -> sl_sugbuf = open_spellbuf ();
            if (slang -> sl_sugbuf == ((void *) 0)) goto someerror;
            wcount = get4c (fd);
            if (wcount < 0) goto someerror;
            ga_init2 (&ga, 1, 100);
            for (wordnr = 0; wordnr < wcount; ++wordnr) {
                ga.ga_len = 0;
                for (;;) {
                    c = getc (fd);
                    if (c < 0 || ga_grow (&ga, 1) == 0) goto someerror;
                    ((char_u *) ga.ga_data) [ga.ga_len ++] = c;
                    if (c == '\000') break;
                }
                if (ml_append_buf (slang -> sl_sugbuf, (linenr_T) wordnr, ga.ga_data, ga.ga_len, 1) == 0) goto someerror;
            }
            ga_clear (&ga);
            tree_count_words (slang -> sl_fbyts, slang -> sl_fidxs);
            tree_count_words (slang -> sl_sbyts, slang -> sl_sidxs);
            nextone : if (fd != ((void *) 0)) fclose (fd);
            ((__builtin_object_size ((char *) (dotp), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (dotp), (char *) (".spl"), __builtin_object_size (
              (char *) (dotp), 1 > 1)) : __inline_strcpy_chk ((char *) (dotp), (char *) (".spl")));
        }
    }
}
...
{
    int i;
    for (i = 0; i < su -> su_ga.ga_len; ++i) vim_free ((((suggest_T *) (su -> su_ga).ga_data) [i]).st_word);
    ga_clear (&su->su_ga);
    for (i = 0; i < su -> su_sga.ga_len; ++i) vim_free ((((suggest_T *) (su -> su_sga).ga_data) [i]).st_word);
    ga_clear (&su->su_sga);
    ...
    int upper;
{
    ...
    p = word;
    c = *p++;
    if (upper) c = ((c) < 256 ? (int) spelltab.st_upper [c] : (c));
    else c = ((c) < 256 ? (int) spelltab.st_fold [c] : (c));
    {
        l = 1;
        wcopy [0] = c;
    }
    ...
    suginfo_T * su;
{
    ...
    if (strlen ((char *) (p)) == len && strncmp ((char *) (su -> su_fbadword), (char *) (p), (size_t) (len)) == 0) {
        ...
        add_suggestion (su, &su->su_ga, word, su -> su_badlen, ((3 * 65 + 0) / 4), 0, 1, su -> su_sallang, 0);
    }
}
    ...
    suginfo_T * su;
{
    ...
    int lpi;
    langp_T * lp;
    ((__builtin_object_size ((char *) (fword), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (fword), (char *) (su -> su_fbadword),
      __builtin_object_size ((char *) (fword), 1 > 1)) : __inline_strcpy_chk ((char *) (fword), (char *) (su -> su_fbadword)));
    n = (int) strlen ((char *) (fword));
    p = su -> su_badptr + su -> su_badlen;
    (void) spell_casefold (p, (int) strlen ((char *) (p)), fword + n, 250 - n);
    for (lpi = 0; lpi < curwin -> w_s -> b_langp.ga_len; ++lpi) {
        lp = (((langp_T *) (curwin -> w_s -> b_langp).ga_data) + (lpi));
        if (lp -> lp_slang -> sl_fbyts == ((void *) 0)) continue;
    ...
    int soundfold;
{
    ...
    depth = 0;
    sp = &stack[0];
    ((__builtin_object_size ((sp), 0) != (size_t) - 1) ? __builtin___memset_chk ((sp), (0), (sizeof (trystate_T)), __builtin_object_size ((sp), 0)) :
      __inline_memset_chk ((sp), (0), (sizeof (trystate_T))));
    sp -> ts_curi = 1;
    if (soundfold) {
    ...
    while (depth >= 0 && !got_int) {
        ...
        case STATE_NOPREFIX :
            ...
            if (flags & 0x0400) break;
            fword_ends = (fword [sp -> ts_fidx] == '\000' || (soundfold ? ((fword [sp -> ts_fidx]) == ' ' || (fword [sp -> ts_fidx]) == '\t') : 
              !spell_iswordp(fword+sp->ts_fidx,curwin)));
            tword [sp -> ts_twordlen] = '\000';
            if (sp -> ts_prefixdepth <= 0xfd && (sp -> ts_flags & 1) == 0) {
                n = stack [sp -> ts_prefixdepth].ts_arridx;
                len = pbyts [n ++];
                for (c = 0; c < len && pbyts [n + c] == 0; ++c);
                if (c > 0) {
                    c = valid_word_prefix (c, n, flags, tword + sp -> ts_splitoff, slang, 0);
                    if (c == 0) break;
                    if (c & (0x01 << 24)) flags |= 0x08;
                    sp -> ts_flags |= 1;
                }
            }
            if (sp -> ts_complen == sp -> ts_compsplit && fword_ends && (flags & 0x0200)) goodword_ends = 0;
            else goodword_ends = 1;
            p = ((void *) 0);
            compound_ok = 1;
            if (sp -> ts_complen > sp -> ts_compsplit) {
                if (slang -> sl_nobreak) {
                    if (sp -> ts_fidx - sp -> ts_splitfidx == sp -> ts_twordlen - sp -> ts_splitoff && strncmp ((char *) (fword + sp -> ts_splitfidx), (char *)
                      (tword + sp -> ts_splitoff), (size_t) (sp -> ts_fidx - sp -> ts_splitfidx)) == 0) {
                        preword [sp -> ts_prewordlen] = '\000';
                        newscore = score_wordcount_adj (slang, sp -> ts_score, preword + sp -> ts_prewordlen, sp -> ts_prewordlen > 0);
                        if (newscore <= su -> su_maxscore) add_suggestion (su, &su->su_ga, preword, sp -> ts_splitfidx - repextra, newscore, 0, 0, lp ->
                          lp_sallang, 0);
                        break;
                    }
                }
                else {
                    if (((unsigned) flags >> 24) == 0 || sp -> ts_twordlen - sp -> ts_splitoff < slang -> sl_compminlen) break;
                    compflags [sp -> ts_complen] = ((unsigned) flags >> 24);
                    compflags [sp -> ts_complen + 1] = '\000';
                    vim_strncpy (preword + sp -> ts_prewordlen, tword + sp -> ts_splitoff, sp -> ts_twordlen - sp -> ts_splitoff);
                    if (match_checkcompoundpattern (preword, sp -> ts_prewordlen, &slang->sl_comppat)) compound_ok = 0;
                    if (compound_ok) {
                        p = preword;
                        while (*skiptowhite(p) != '\000') p = skipwhite (skiptowhite (p));
                        if (fword_ends && !can_compound(slang,p,compflags+sp->ts_compsplit)) compound_ok = 0;
                    }
                    p = preword + sp -> ts_prewordlen;
                    --p;
                }
            }
            if (soundfold) ((__builtin_object_size ((char *) (preword + sp -> ts_prewordlen), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (preword +
              sp -> ts_prewordlen), (char *) (tword + sp -> ts_splitoff), __builtin_object_size ((char *) (preword + sp -> ts_prewordlen), 1 > 1)) :
              __inline_strcpy_chk ((char *) (preword + sp -> ts_prewordlen), (char *) (tword + sp -> ts_splitoff)));
            else if (flags & 0x80) find_keepcap_word (slang, tword + sp -> ts_splitoff, preword + sp -> ts_prewordlen);
            else {
                c = su -> su_badflags;
                if ((c & 0x04) && su -> su_badlen == 1) c = 0x02;
                c |= flags;
                if (p != ((void *) 0) && spell_iswordp_nmw (p)) c &= ~0x02;
            ...
            if (!soundfold) {
                ...
                if ((sp -> ts_complen == sp -> ts_compsplit && (!((hash_find(&su->su_banned, preword + sp -> ts_prewordlen)) -> hi_key == ((void *) 0) || (
                  hash_find (&su->su_banned, preword + sp -> ts_prewordlen)) -> hi_key == &hash_removed))) || (!((hash_find(&su->su_banned, preword)) -> hi_key
                  == ((void *) 0) || (hash_find (&su->su_banned, preword)) -> hi_key == &hash_removed))) {
                    if (slang -> sl_compprog == ((void *) 0)) break;
                    goodword_ends = 0;
                }
            }
            newscore = 0;
            if (!soundfold) {
                if ((flags & 0x01) && (((unsigned) flags >> 16) & lp -> lp_region) == 0) newscore += 200;
                if (flags & 0x08) newscore += 180;
                if (!spell_valid_case(su->su_badflags,captype(preword+sp->ts_prewordlen,((void*)0)))) newscore += 52;
            }
            if (fword_ends && goodword_ends && sp -> ts_fidx >= sp -> ts_fidxtry && compound_ok) {
                if (soundfold) {
                    ...
                    if (!spell_iswordp(p,curwin)) {
                        ...
                        --p;
                        if (spell_iswordp (p, curwin)) newscore += 103;
                    }
                    score = score_wordcount_adj (slang, sp -> ts_score + newscore, preword + sp -> ts_prewordlen, sp -> ts_prewordlen > 0);
                    if (score <= su -> su_maxscore) {
                        add_suggestion (su, &su->su_ga, preword, sp -> ts_fidx - repextra, score, 0, 0, lp -> lp_sallang, 0);
                        if (su -> su_badflags & 0x20) {
                            c = captype (preword, ((void *) 0));
                            if (c == 0 || c == 0x04) {
                                make_case_word (tword + sp -> ts_splitoff, preword + sp -> ts_prewordlen, c == 0 ? 0x04 : 0);
                                add_suggestion (su, &su->su_ga, preword, sp -> ts_fidx - repextra, score + 52, 0, 0, lp -> lp_sallang, 0);
                            }
                        }
                    }
                }
            ...
            if ((sp -> ts_fidx >= sp -> ts_fidxtry || fword_ends)) {
                ...
                try_compound = 0;
                if (!soundfold && slang -> sl_compprog != ((void *) 0) && ((unsigned) flags >> 24) != 0 && sp -> ts_twordlen - sp -> ts_splitoff >= slang ->
                  sl_compminlen && (slang -> sl_compsylmax < 250 || sp -> ts_complen + 1 - sp -> ts_compsplit < slang -> sl_compmax) && (can_be_compound (sp,
                  slang, compflags, ((unsigned) flags >> 24)))) {
                    try_compound = 1;
                    compflags [sp -> ts_complen] = ((unsigned) flags >> 24);
                    compflags [sp -> ts_complen + 1] = '\000';
                }
                if (slang -> sl_nobreak) try_compound = 1;
                else if (!fword_ends && try_compound && (sp -> ts_flags & 2) == 0) {
                    try_compound = 0;
                    sp -> ts_flags |= 2;
                    --sp->ts_curi;
                    compflags [sp -> ts_complen] = '\000';
                ...
                if (try_split || try_compound) {
                    if (!try_compound && (!fword_ends || !goodword_ends)) {
                        if (sp -> ts_complen == sp -> ts_compsplit && (flags & 0x0200)) break;
                        p = preword;
                        while (*skiptowhite(p) != '\000') p = skipwhite (skiptowhite (p));
                        if (sp -> ts_complen > sp -> ts_compsplit && !can_compound(slang,p,compflags+sp->ts_compsplit)) break;
                        if (slang -> sl_nosplitsugs) newscore += 249;
                        else newscore += 149;
                        newscore = score_wordcount_adj (slang, newscore, preword + sp -> ts_prewordlen, 1);
                    }
                    if ((stack [depth].ts_score + (newscore) < su -> su_maxscore)) {
                        ...
                        ++depth;
                        sp = &stack[depth];
                        if (!try_compound && !fword_ends) ((__builtin_object_size ((char *) (preword), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (
                          preword), (char *) (" "), __builtin_object_size ((char *) (preword), 1 > 1)) : __inline_strcat_chk ((char *) (preword), (char *) (" "
                          )));
                        sp -> ts_prewordlen = (char_u) strlen ((char *) (preword));
                        sp -> ts_splitoff = sp -> ts_twordlen;
                        sp -> ts_splitfidx = sp -> ts_fidx;
                        if (((!try_compound && !spell_iswordp_nmw(fword+sp->ts_fidx)) || fword_ends) && fword [sp -> ts_fidx] != '\000' && goodword_ends) {
                            int l;
                            l = 1;
                            if (fword_ends) {
                                ((__builtin_object_size ((char *) (preword + sp -> ts_prewordlen), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (
                                  preword + sp -> ts_prewordlen), (char *) (fword + sp -> ts_fidx), l, __builtin_object_size ((char *) (preword + sp ->
                                  ts_prewordlen), 0)) : __inline_memmove_chk ((char *) (preword + sp -> ts_prewordlen), (char *) (fword + sp -> ts_fidx), l));
                                sp -> ts_prewordlen += l;
                                preword [sp -> ts_prewordlen] = '\000';
                            }
                            else sp -> ts_score -= 149 - 93;
                            sp -> ts_fidx += l;
                        }
                        if (try_compound) ++sp->ts_complen;
                        else sp -> ts_compsplit = sp -> ts_complen;
                        sp -> ts_prefixdepth = 0xff;
                        n = sp -> ts_fidx;
                        su -> su_badflags = badword_captype (su -> su_badptr + n, su -> su_badptr + su -> su_badlen);
                        sp -> ts_arridx = 0;
        ...
        case STATE_PLAIN :
            ...
            if (sp -> ts_curi > byts [arridx]) {
                if (sp -> ts_fidx >= sp -> ts_fidxtry) sp -> ts_state = STATE_DEL;
                else sp -> ts_state = STATE_FINAL;
            }
            else {
                arridx += sp -> ts_curi ++;
                c = byts [arridx];
                if (c == fword [sp -> ts_fidx]) newscore = 0;
                else newscore = 93;
                if ((newscore == 0 || (sp -> ts_fidx >= sp -> ts_fidxtry && ((sp -> ts_flags & 4) == 0 || c != fword [sp -> ts_delidx]))) && (stack [depth].
                  ts_score + (newscore) < su -> su_maxscore)) {
                    go_deeper (stack, depth, newscore);
                    ++depth;
                    sp = &stack[depth];
                    ++sp->ts_fidx;
                    ...
                    {
                        if (newscore != 0 && !soundfold && slang -> sl_has_map && similar_chars (slang, c, fword [sp -> ts_fidx - 1])) sp -> ts_score -= 93 - 33
                          ;
                    }
                }
            }
        ...
        case STATE_DEL :
            ...
            sp -> ts_curi = 1;
            if (soundfold && sp -> ts_fidx == 0 && fword [sp -> ts_fidx] == '*') newscore = 2 * 94 / 3;
            else newscore = 94;
            if (fword [sp -> ts_fidx] != '\000' && (stack [depth].ts_score + (newscore) < su -> su_maxscore)) {
                go_deeper (stack, depth, newscore);
                ++depth;
                ...
                {
                    ++stack[depth].ts_fidx;
                    if (fword [sp -> ts_fidx] == fword [sp -> ts_fidx + 1]) stack [depth].ts_score -= 94 - 66;
                }
                break;
            }
        case STATE_INS_PREP :
        ...
        case STATE_INS :
            ...
            n += sp -> ts_curi ++;
            c = byts [n];
            if (soundfold && sp -> ts_twordlen == 0 && c == '*') newscore = 2 * 96 / 3;
            else newscore = 96;
            if (c != fword [sp -> ts_fidx] && (stack [depth].ts_score + (newscore) < su -> su_maxscore)) {
                go_deeper (stack, depth, newscore);
                ++depth;
                sp = &stack[depth];
                tword [sp -> ts_twordlen ++] = c;
                sp -> ts_arridx = idxs [n];
                {
                    if (sp -> ts_twordlen >= 2 && tword [sp -> ts_twordlen - 2] == c) sp -> ts_score -= 96 - 67;
                }
            }
            break;
        case STATE_SWAP :
            ...
            {
                if (p [1] == '\000') c2 = '\000';
                else if (!soundfold && !spell_iswordp(p+1,curwin)) c2 = c;
                else c2 = p [1];
            }
            if (c2 == '\000') {
                sp -> ts_state = STATE_REP_INI;
            ...
            else sp -> ts_state = STATE_REP_INI;
            break;
        case STATE_UNSWAP :
            p = fword + sp -> ts_fidx;
            {
        ...
        case STATE_SWAP3 :
            p = fword + sp -> ts_fidx;
            {
                c = *p;
                c2 = p [1];
                if (!soundfold && !spell_iswordp(p+2,curwin)) c3 = c;
                else c3 = p [2];
            }
            if (c == c3 || c3 == '\000') {
                sp -> ts_state = STATE_REP_INI;
        ...
        case STATE_REP_INI :
            if ((lp -> lp_replang == ((void *) 0) && !soundfold) || sp -> ts_score + 65 >= su -> su_maxscore || sp -> ts_fidx < sp -> ts_fidxtry) {
                sp -> ts_state = STATE_FINAL;
                break;
            }
            if (soundfold) sp -> ts_curi = slang -> sl_repsal_first [fword [sp -> ts_fidx]];
            else sp -> ts_curi = lp -> lp_replang -> sl_rep_first [fword [sp -> ts_fidx]];
            if (sp -> ts_curi < 0) {
                sp -> ts_state = STATE_FINAL;
                break;
            }
        ...
        case STATE_REP :
            p = fword + sp -> ts_fidx;
            if (soundfold) gap = &slang->sl_repsal;
            else gap = &lp->lp_replang->sl_rep;
            while (sp -> ts_curi < gap -> ga_len) {
                ...
                if (strncmp ((char *) (ftp -> ft_from), (char *) (p), (size_t) (strlen ((char *) (ftp -> ft_from)))) == 0 && (stack [depth].ts_score + (65) < su
                  -> su_maxscore)) {
                    ...
                    if (fl != tl) {
                        ((__builtin_object_size ((char *) ((p + tl)), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) ((p + tl)), (char *) ((p + fl)),
                          strlen ((char *) (p + fl)) + 1, __builtin_object_size ((char *) ((p + tl)), 0)) : __inline_memmove_chk ((char *) ((p + tl)), (char *)
                          ((p + fl)), strlen ((char *) (p + fl)) + 1));
                        repextra += tl - fl;
                    }
                    ((__builtin_object_size ((char *) (p), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (p), (char *) (ftp -> ft_to), tl,
                      __builtin_object_size ((char *) (p), 0)) : __inline_memmove_chk ((char *) (p), (char *) (ftp -> ft_to), tl));
                    stack [depth].ts_fidxtry = sp -> ts_fidx + tl;
                    break;
                }
            }
            if (sp -> ts_curi >= gap -> ga_len && sp -> ts_state == STATE_REP) sp -> ts_state = STATE_FINAL;
            break;
        case STATE_REP_UNDO :
            if (soundfold) gap = &slang->sl_repsal;
            else gap = &lp->lp_replang->sl_rep;
            ...
            p = fword + sp -> ts_fidx;
            if (fl != tl) {
                ((__builtin_object_size ((char *) ((p + fl)), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) ((p + fl)), (char *) ((p + tl)), strlen (
                  (char *) (p + tl)) + 1, __builtin_object_size ((char *) ((p + fl)), 0)) : __inline_memmove_chk ((char *) ((p + fl)), (char *) ((p + tl)),
                  strlen ((char *) (p + tl)) + 1));
                repextra -= tl - fl;
            }
            ((__builtin_object_size ((char *) (p), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (p), (char *) (ftp -> ft_from), fl,
              __builtin_object_size ((char *) (p), 0)) : __inline_memmove_chk ((char *) (p), (char *) (ftp -> ft_from), fl));
            sp -> ts_state = STATE_REP;
            break;
        default :
            --depth;
    ...
    char_u * kword;
{
    ...
    while (depth >= 0) {
        if (fword [fwordidx [depth]] == '\000') {
            ...
            if (l == 0) {
                if (round [depth] == 1) {
                    ((__builtin_object_size ((char *) (kword + kwordlen [depth]), 0) != (size_t) - 1) ? __builtin___strncpy_chk ((char *) (kword + kwordlen [
                      depth]), (char *) (fword + fwordidx [depth]), (size_t) (flen), __builtin_object_size ((char *) (kword + kwordlen [depth]), 1 > 1)) :
                      __inline_strncpy_chk ((char *) (kword + kwordlen [depth]), (char *) (fword + fwordidx [depth]), (size_t) (flen)));
                    kwordlen [depth + 1] = kwordlen [depth] + flen;
                }
                else {
                    ((__builtin_object_size ((char *) (kword + kwordlen [depth]), 0) != (size_t) - 1) ? __builtin___strncpy_chk ((char *) (kword + kwordlen [
                      depth]), (char *) (uword + uwordidx [depth]), (size_t) (ulen), __builtin_object_size ((char *) (kword + kwordlen [depth]), 1 > 1)) :
                      __inline_strncpy_chk ((char *) (kword + kwordlen [depth]), (char *) (uword + uwordidx [depth]), (size_t) (ulen)));
    ...
    suginfo_T * su;
{
    ...
    int score;
    int lpi;
    if (ga_grow (&su->su_sga, su -> su_ga.ga_len) == 0) return;
    for (lpi = 0; lpi < curwin -> w_s -> b_langp.ga_len; ++lpi) {
        lp = (((langp_T *) (curwin -> w_s -> b_langp).ga_data) + (lpi));
    ...
    suginfo_T * su;
{
    ...
    for (lpi = 0; lpi < curwin -> w_s -> b_langp.ga_len; ++lpi) {
        ...
        if (lp -> lp_slang -> sl_sal.ga_len > 0) {
            ...
            for (i = 0; i < su -> su_ga.ga_len; ++i) {
                ...
                if (stp -> st_altscore == 999999) stp -> st_score = (stp -> st_score * 3 + 96 * 3) / 4;
                else stp -> st_score = (stp -> st_score * 3 + stp -> st_altscore) / 4;
                stp -> st_salscore = 0;
            }
            break;
        }
    ...
    for (i = 0; i < su -> su_sga.ga_len; ++i) {
        ...
        if (stp -> st_score == 999999) stp -> st_score = (96 * 3 * 7 + stp -> st_altscore) / 8;
        else stp -> st_score = (stp -> st_score * 7 + stp -> st_altscore) / 8;
        stp -> st_salscore = 1;
    }
    check_suggestions (su, &su->su_ga);
    (void) cleanup_suggestions (&su->su_ga, su -> su_maxscore, su -> su_maxcount);
    ...
    if (ga_grow (&ga, su -> su_ga.ga_len + su -> su_sga.ga_len) == 0) return;
    ...
    for (i = 0; i < su -> su_ga.ga_len || i < su -> su_sga.ga_len; ++i) {
        for (round = 1; round <= 2; ++round) {
            ...
            if (i < gap -> ga_len) {
                ...
                for (j = 0; j < ga.ga_len; ++j) if (strcmp ((char *) (stp [j].st_word), (char *) (p)) == 0) break;
                if (j == ga.ga_len) stp [ga.ga_len ++] = (((suggest_T *) (*gap).ga_data) [i]);
                else vim_free (p);
            }
        }
    }
    ...
    if (ga.ga_len > su -> su_maxcount) {
        for (i = su -> su_maxcount; i < ga.ga_len; ++i) vim_free (stp [i].st_word);
    ...
    char_u * badsound;
{
    ...
    if (lendiff >= 0) pbad = badsound;
    else {
        (void) spell_casefold (su -> su_badptr, stp -> st_orglen, fword, 250);
        if (((su -> su_badptr [su -> su_badlen]) == ' ' || (su -> su_badptr [su -> su_badlen]) == '\t') && *skiptowhite(stp->st_word) == '\000') for (p = fword
          ; *(p=skiptowhite(p)) != '\000';) ((__builtin_object_size ((char *) ((p)), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) ((p)), (char *) ((
          p + 1)), strlen ((char *) (p + 1)) + 1, __builtin_object_size ((char *) ((p)), 0)) : __inline_memmove_chk ((char *) ((p)), (char *) ((p + 1)), strlen
          ((char *) (p + 1)) + 1));
        spell_soundfold (slang, fword, 1, badsound2);
        pbad = badsound2;
    }
    if (lendiff > 0) {
        ((__builtin_object_size ((char *) (goodword), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (goodword), (char *) (stp -> st_word),
          __builtin_object_size ((char *) (goodword), 1 > 1)) : __inline_strcpy_chk ((char *) (goodword), (char *) (stp -> st_word)));
        vim_strncpy (goodword + stp -> st_wordlen, su -> su_badptr + su -> su_badlen - lendiff, lendiff);
        pgood = goodword;
    }
    else pgood = stp -> st_word;
...
static void suggest_try_soundalike_prep () {
    ...
    for (lpi = 0; lpi < curwin -> w_s -> b_langp.ga_len; ++lpi) {
        lp = (((langp_T *) (curwin -> w_s -> b_langp).ga_data) + (lpi));
        slang = lp -> lp_slang;
        if (slang -> sl_sal.ga_len > 0 && slang -> sl_sbyts != ((void *) 0)) hash_init (&slang->sl_sounddone);
    }
}
...
static void suggest_try_soundalike_finish () {
    ...
    for (lpi = 0; lpi < curwin -> w_s -> b_langp.ga_len; ++lpi) {
        ...
        if (slang -> sl_sal.ga_len > 0 && slang -> sl_sbyts != ((void *) 0)) {
            ...
            for (hi = slang -> sl_sounddone.ht_array; todo > 0; ++hi) if (!((hi)->hi_key==((void*)0)||(hi)->hi_key==&hash_removed)) {
    ...
    langp_T * lp;
{
    ...
    if (((hi) -> hi_key == ((void *) 0) || (hi) -> hi_key == &hash_removed)) {
        sft = (sftword_T *) alloc ((unsigned) (sizeof (sftword_T) + strlen ((char *) (goodword))));
        if (sft != ((void *) 0)) {
            sft -> sft_score = score;
            ((__builtin_object_size ((char *) (sft -> sft_word), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (sft -> sft_word), (char *) (goodword),
              __builtin_object_size ((char *) (sft -> sft_word), 1 > 1)) : __inline_strcpy_chk ((char *) (sft -> sft_word), (char *) (goodword)));
            hash_add_item (&slang->sl_sounddone, hi, sft -> sft_word, hash);
        }
    }
    else {
        sft = ((sftword_T *) ((hi) -> hi_key - (dumsft.sft_word - (char_u *) &dumsft)));
        if (score >= sft -> sft_score) return;
        sft -> sft_score = score;
    }
    ...
    while (*nrline != '\000') {
        ...
        for (;;) {
            i = 1;
            if (wordcount == orgnr && byts [n + 1] == '\000') break;
            if (byts [n + 1] == '\000') ++wordcount;
            for (; byts [n + i] == '\000'; ++i) if (i > byts [n]) {
                ((__builtin_object_size ((char *) (theword + wlen), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (theword + wlen), (char *) ("BAD"),
                  __builtin_object_size ((char *) (theword + wlen), 1 > 1)) : __inline_strcpy_chk ((char *) (theword + wlen), (char *) ("BAD")));
                goto badword;
            }
            for (; i < byts [n]; ++i) {
                wc = idxs [idxs [n + i]];
                if (wordcount + wc > orgnr) break;
                wordcount += wc;
            }
            theword [wlen ++] = byts [n + i];
            n = idxs [n + i];
        }
        badword : theword [wlen] = '\000';
        for (; i <= byts [n] && byts [n + i] == '\000'; ++i) {
            ...
            if (sps_flags & 4) {
                if (score <= su -> su_maxscore) add_suggestion (su, &su->su_sga, p, su -> su_badlen, score, 0, 0, slang, 0);
            }
            else {
                if ((flags & 0x01) && (((unsigned) flags >> 16) & lp -> lp_region) == 0) goodscore = 200;
                else goodscore = 0;
                gc = ((int) *(p));
                if (((gc) < 256 ? spelltab.st_isu [gc] : 0)) {
                    bc = ((int) *(su->su_badword));
                    if (!((bc)<256?spelltab.st_isu[bc]:0) && ((bc) < 256 ? (int) spelltab.st_fold [bc] : (bc)) != ((gc) < 256 ? (int) spelltab.st_fold [gc] : (
                      gc))) goodscore += 52 / 2;
                }
                limit = ((4 * su -> su_sfmaxscore - goodscore - score) / 3);
                if (limit > 350) goodscore += spell_edit_score (slang, su -> su_badword, p);
                else goodscore += spell_edit_score_limit (slang, su -> su_badword, p, limit);
                if (goodscore < 999999) {
                    goodscore = score_wordcount_adj (slang, goodscore, p, 0);
                    goodscore = ((3 * goodscore + score) / 4);
                    if (goodscore <= su -> su_sfmaxscore) add_suggestion (su, &su->su_ga, p, su -> su_badlen, goodscore, score, 1, slang, 1);
                }
            }
        }
    }
    ...
    char_u * word;
{
    ...
    for (;;) {
        ...
        if (byts [arridx] != c) return -1;
        arridx = idxs [arridx];
        ++wlen;
        if (c == ' ') while (ptr [wlen] == ' ' || ptr [wlen] == '\011') ++wlen;
    }
    return wordnr;
}
...
static void make_case_word (fword, cword, flags)
    ...
    int flags;
{
    if (flags & 0x04) allcap_copy (fword, cword);
    else if (flags & 0x02) onecap_copy (fword, cword, 1);
    else ((__builtin_object_size ((char *) (cword), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (cword), (char *) (fword), __builtin_object_size ((
      char *) (cword), 1 > 1)) : __inline_strcpy_chk ((char *) (cword), (char *) (fword)));
}
    ...
    char_u * map;
{
    ...
    lp -> sl_has_map = 1;
    for (i = 0; i < 256; ++i) lp -> sl_map_array [i] = 0;
    for (p = map; *p != '\000';) {
        c = *p++;
        if (c == '/') headc = 0;
        else {
...
static void add_suggestion (su, gap, goodword, badlenarg, score, altscore, had_bonus, slang, maxsf)
    suginfo_T * su;
    garray_T * gap;
    char_u * goodword;
    int badlenarg;
    ...
    int maxsf;
{
    ...
    for (;;) {
        ...
        if (goodlen <= 0 || badlen <= 0) break;
        --pgood;
        --pbad;
        if (*pgood != *pbad) break;
    }
    if (badlen == 0 && goodlen == 0) return;
    if (gap -> ga_len == 0) i = -1;
    else {
        stp = &(((suggest_T*)(*gap).ga_data) [0]);
        for (i = gap -> ga_len; --i >= 0; ++stp) if (stp -> st_wordlen == goodlen && stp -> st_orglen == badlen && strncmp ((char *) (stp -> st_word), (char *)
          (goodword), (size_t) (goodlen)) == 0) {
            if (stp -> st_slang == ((void *) 0)) stp -> st_slang = slang;
            new_sug.st_score = score;
            new_sug.st_altscore = altscore;
            new_sug.st_had_bonus = had_bonus;
            if (stp -> st_had_bonus != had_bonus) {
                if (had_bonus) rescore_one (su, stp);
                else {
    ...
    if (i < 0 && ga_grow (gap, 1) == 1) {
        ...
        if (stp -> st_word != ((void *) 0)) {
            ...
            if (gap -> ga_len > (((su) -> su_maxcount < 130 ? 150 : (su) -> su_maxcount + 20) + 50)) {
                if (maxsf) su -> su_sfmaxscore = cleanup_suggestions (gap, su -> su_sfmaxscore, ((su) -> su_maxcount < 130 ? 150 : (su) -> su_maxcount + 20));
                else {
                    i = su -> su_maxscore;
    ...
    garray_T * gap;
{
    ...
    for (i = gap -> ga_len - 1; i >= 0; --i) {
        ((__builtin_object_size ((char *) (longword), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (longword), (char *) (stp [i].st_word),
          __builtin_object_size ((char *) (longword), 1 > 1)) : __inline_strcpy_chk ((char *) (longword), (char *) (stp [i].st_word)));
        len = stp [i].st_wordlen;
        vim_strncpy (longword + len, su -> su_badptr + stp [i].st_orglen, 250 - len);
        attr = HLF_COUNT;
        (void) spell_check (curwin, longword, &attr, ((void *) 0), 0);
        if (attr != HLF_COUNT) {
            vim_free (stp [i].st_word);
            --gap->ga_len;
            if (i < gap -> ga_len) ((__builtin_object_size ((char *) (stp + i), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (stp + i), (char *) (
              stp + i + 1), sizeof (suggest_T) * (gap -> ga_len - i), __builtin_object_size ((char *) (stp + i), 0)) : __inline_memmove_chk ((char *) (stp + i)
              , (char *) (stp + i + 1), sizeof (suggest_T) * (gap -> ga_len - i)));
        }
    }
}
    ...
    char_u * word;
{
    ...
    if (((hi) -> hi_key == ((void *) 0) || (hi) -> hi_key == &hash_removed)) {
        s = vim_strsave (word);
        if (s != ((void *) 0)) hash_add_item (&su->su_banned, hi, s, hash);
    }
}
...
{
    int i;
    if (su -> su_sallang != ((void *) 0)) for (i = 0; i < su -> su_ga.ga_len; ++i) rescore_one (su, &(((suggest_T*)(su->su_ga).ga_data)[i]));
}
...
static int sug_compare (s1, s2) const
    void * s1;
    const void * s2;
{
    ...
    if (n == 0) {
        ...
        if (n == 0) n = strcasecmp ((char *) (p1 -> st_word), (char *) (p2 -> st_word));
    }
    return n;
}
    ...
    int keep;
{
    ...
    if (gap -> ga_len > keep) {
        for (i = keep; i < gap -> ga_len; ++i) vim_free (stp [i].st_word);
        gap -> ga_len = keep;
    ...
    char_u * word;
{
    ...
    char_u sound [250];
    int lpi;
    if (curwin -> w_onebuf_opt.wo_spell && *curwin->w_s->b_p_spl != '\000') for (lpi = 0; lpi < curwin -> w_s -> b_langp.ga_len; ++lpi) {
        lp = (((langp_T *) (curwin -> w_s -> b_langp).ga_data) + (lpi));
        if (lp -> lp_slang -> sl_sal.ga_len > 0) {
    ...
    char_u * res;
{
    char_u fword [250];
    char_u * word;
    if (slang -> sl_sofo) spell_soundfold_sofo (slang, inword, res);
    else {
        if (folded) word = inword;
        else {
    ...
    char_u * res;
{
    ...
    int c;
    {
        for (s = inword; (c = *s) != '\000'; ++s) {
            ...
            if (c != '\000' && (ri == 0 || res [ri - 1] != c)) res [ri ++] = c;
        }
    }
    res [ri] = '\000';
}
        ...
        while (*s != '\000') {
            if (((*s) == ' ' || (*s) == '\t')) {
                *t++ = ' ';
                ...
                if (spell_iswordp_nmw (s)) *t++ = *s;
                ++s;
            }
        }
        * t = '\000';
    }
    else ((__builtin_object_size ((char *) (word), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (word), (char *) (s), __builtin_object_size ((char *)
      (word), 1 > 1)) : __inline_strcpy_chk ((char *) (word), (char *) (s)));
    ...
    while ((c = word [i]) != '\000') {
        ...
        if (n >= 0) {
            for (; (s = smp [n].sm_lead) [0] == c; ++n) {
                k = smp [n].sm_leadlen;
                if (k > 1) {
                    if (word [i + 1] != s [1]) continue;
                    if (k > 2) {
                        for (j = 2; j < k; ++j) if (word [i + j] != s [j]) break;
                        if (j < k) continue;
                    }
                }
                if ((pf = smp [n].sm_oneof) != ((void *) 0)) {
                    while (*pf != '\000' && *pf != word [i + k]) ++pf;
                    if (*pf == '\000') continue;
                    ++k;
                }
                s = smp [n].sm_rules;
                ...
                if (*s == '^' && *(s+1) == '^') s ++;
                if (*s == '\000' || (*s == '^' && (i == 0 || !(word[i-1]==' '||spell_iswordp(word+i-1,curwin))) && (*(s+1) != '$' || 
                  (!spell_iswordp(word+i+k0,curwin)))) || (*s == '$' && i > 0 && spell_iswordp (word + i - 1, curwin) && (!spell_iswordp(word+i+k0,curwin)))) {
                    c0 = word [i + k - 1];
                    n0 = slang -> sl_sal_first [c0];
                    if (slang -> sl_followup && k > 1 && n0 >= 0 && p0 != '-' && word [i + k] != '\000') {
                        for (; (s = smp [n0].sm_lead) [0] == c0; ++n0) {
                            k0 = smp [n0].sm_leadlen;
                            if (k0 > 1) {
                                if (word [i + k] != s [1]) continue;
                                if (k0 > 2) {
                                    pf = word + i + k + 1;
                                    for (j = 2; j < k0; ++j) if (*pf++ != s [j]) break;
                                    if (j < k0) continue;
                                }
                            }
                            k0 += k - 1;
                            if ((pf = smp [n0].sm_oneof) != ((void *) 0)) {
                                while (*pf != '\000' && *pf != word [i + k0]) ++pf;
                                if (*pf == '\000') continue;
                                ++k0;
                            }
                            p0 = 5;
                            ...
                            if (*s == '\000' || (*s == '$' && !spell_iswordp(word+i+k0,curwin))) {
                                if (k0 == k) continue;
                                if (p0 < pri) continue;
                                break;
                            }
                        }
                        if (p0 >= pri && smp [n0].sm_lead [0] == c0) continue;
                    }
                    s = smp [n].sm_to;
                    ...
                    if (p0 == 1 && z == 0) {
                        if (reslen > 0 && *s != '\000' && (res [reslen - 1] == c || res [reslen - 1] == *s)) reslen --;
                        z0 = 1;
                        z = 1;
                        k0 = 0;
                        ...
                        if (k > k0) ((__builtin_object_size ((char *) ((word + i + k0)), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) ((word + i + k0
                          )), (char *) ((word + i + k)), strlen ((char *) (word + i + k)) + 1, __builtin_object_size ((char *) ((word + i + k0)), 0)) :
                          __inline_memmove_chk ((char *) ((word + i + k0)), (char *) ((word + i + k)), strlen ((char *) (word + i + k)) + 1));
                        c = word [i];
                    }
                    else {
                        i += k - 1;
                        ...
                        while (*s != '\000' && s [1] != '\000' && reslen < 250) {
                            if (reslen == 0 || res [reslen - 1] != *s) res [reslen ++] = *s;
                            s ++;
                        }
                        c = *s;
                        if (strstr ((char *) pf, "^^") != ((void *) 0)) {
                            if (c != '\000') res [reslen ++] = c;
                            ((__builtin_object_size ((char *) ((word)), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) ((word)), (char *) ((word + i +
                              1)), strlen ((char *) (word + i + 1)) + 1, __builtin_object_size ((char *) ((word)), 0)) : __inline_memmove_chk ((char *) ((word)
                              ), (char *) ((word + i + 1)), strlen ((char *) (word + i + 1)) + 1));
                            i = 0;
                            z0 = 1;
                        }
                    }
        ...
        if (z0 == 0) {
            if (k && !p0 && reslen < 250 && c != '\000' && (!slang->sl_collapse || reslen == 0 || res [reslen - 1] != c)) res [reslen ++] = c;
            i ++;
            z = 0;
            k = 0;
        }
    ...
    char_u * badstart;
{
    ...
    if ((*badsound == '*' || *goodsound == '*') && *badsound != *goodsound) {
        if ((badsound [0] == '\000' && goodsound [1] == '\000') || (goodsound [0] == '\000' && badsound [1] == '\000')) return 94;
        if (badsound [0] == '\000' || goodsound [0] == '\000') return 999999;
        if (badsound [1] == goodsound [1] || (badsound [1] != '\000' && goodsound [1] != '\000' && badsound [2] == goodsound [2])) {
        }
        else {
            score = 2 * 94 / 3;
            if (*badsound == '*') ++badsound;
            else ++goodsound;
        }
    }
    ...
    case 2 :
        ...
        if (strcmp ((char *) (pl + 1), (char *) (ps)) == 0) return score + 94 * 2;
        break;
    case -1 :
    case 1 :
        pl2 = pl + 1;
        ps2 = ps;
        while (*pl2 == *ps2) {
            if (*pl2 == '\000') return score + 94;
            ++pl2;
            ++ps2;
        }
        if (pl2 [0] == ps2 [1] && pl2 [1] == ps2 [0] && strcmp ((char *) (pl2 + 2), (char *) (ps2 + 2)) == 0) return score + 94 + 75;
        if (strcmp ((char *) (pl2 + 1), (char *) (ps2 + 1)) == 0) return score + 94 + 93;
        if (pl [0] == ps [1] && pl [1] == ps [0]) {
            ...
            if (strcmp ((char *) (pl2 + 1), (char *) (ps2)) == 0) return score + 75 + 94;
        }
        pl2 = pl + 1;
        ps2 = ps + 1;
        while (*pl2 == *ps2) {
        ...
        if (strcmp ((char *) (pl2 + 1), (char *) (ps2)) == 0) return score + 93 + 94;
        break;
    case 0 :
        if (*pl == '\000') return score;
        if (pl [0] == ps [1] && pl [1] == ps [0]) {
            pl2 = pl + 2;
            ps2 = ps + 2;
            while (*pl2 == *ps2) {
                if (*pl2 == '\000') return score + 75;
                ++pl2;
                ++ps2;
            }
            if (pl2 [0] == ps2 [1] && pl2 [1] == ps2 [0] && strcmp ((char *) (pl2 + 2), (char *) (ps2 + 2)) == 0) return score + 75 + 75;
            if (strcmp ((char *) (pl2 + 1), (char *) (ps2 + 1)) == 0) return score + 75 + 93;
        }
        pl2 = pl + 1;
        ps2 = ps + 1;
        while (*pl2 == *ps2) {
            if (*pl2 == '\000') return score + 93;
            ++pl2;
            ++ps2;
        }
        if (pl2 [0] == ps2 [1] && pl2 [1] == ps2 [0] && strcmp ((char *) (pl2 + 2), (char *) (ps2 + 2)) == 0) return score + 93 + 75;
        if (strcmp ((char *) (pl2 + 1), (char *) (ps2 + 1)) == 0) return score + 93 + 93;
        pl2 = pl;
        ps2 = ps + 1;
        while (*pl2 == *ps2) {
            ++pl2;
        ...
        if (strcmp ((char *) (pl2 + 1), (char *) (ps2)) == 0) return score + 96 + 94;
        pl2 = pl + 1;
        ps2 = ps;
        while (*pl2 == *ps2) {
            ++pl2;
        ...
        if (strcmp ((char *) (pl2), (char *) (ps2 + 1)) == 0) return score + 96 + 94;
        break;
    }
    return 999999;
}
    ...
    char_u * goodword;
{
    ...
    cnt = (int *) lalloc ((long_u) (sizeof (int) *(badlen+1) * (goodlen + 1)), 1);
    if (cnt == ((void *) 0)) return 0;
    cnt [(0) + (0) * (badlen + 1)] = 0;
    for (j = 1; j <= goodlen; ++j) cnt [(0) + (j) * (badlen + 1)] = cnt [(0) + (j - 1) * (badlen + 1)] + 96;
    for (i = 1; i <= badlen; ++i) {
        ...
        for (j = 1; j <= goodlen; ++j) {
            ...
            if (bc == gc) cnt [(i) + (j) * (badlen + 1)] = cnt [(i - 1) + (j - 1) * (badlen + 1)];
            else {
                if (((bc) < 256 ? (int) spelltab.st_fold [bc] : (bc)) == ((gc) < 256 ? (int) spelltab.st_fold [gc] : (gc))) cnt [(i) + (j) * (badlen + 1)] = 52
                  + cnt [(i - 1) + (j - 1) * (badlen + 1)];
                else {
                    if (slang != ((void *) 0) && slang -> sl_has_map && similar_chars (slang, gc, bc)) cnt [(i) + (j) * (badlen + 1)] = 33 + cnt [(i - 1) + (j -
                      1) * (badlen + 1)];
                    else cnt [(i) + (j) * (badlen + 1)] = 93 + cnt [(i - 1) + (j - 1) * (badlen + 1)];
                }
                if (i > 1 && j > 1) {
                    {
                        pbc = badword [i - 2];
                    ...
                    if (bc == pgc && pbc == gc) {
                        ...
                        if (t < cnt [(i) + (j) * (badlen + 1)]) cnt [(i) + (j) * (badlen + 1)] = t;
                    }
                }
    ...
    int limit;
{
    ...
    for (;;) {
        ...
        if (gc == '\000') {
            ...
            if (((bc) < 256 ? (int) spelltab.st_fold [bc] : (bc)) == ((gc) < 256 ? (int) spelltab.st_fold [gc] : (gc))) score += 52;
            else {
                if (slang != ((void *) 0) && slang -> sl_has_map && similar_chars (slang, gc, bc)) score += 33;
                else score += 93;
            }
            if (score < minscore) {
                ++gi;
    ...
    exarg_T * eap;
{
    ...
    if (!(curbuf->b_ml.ml_line_count==1&&*ml_get((linenr_T)1) == '\000') || !buf_valid(curbuf)) return;
    spell_dump_compl (((void *) 0), 0, ((void *) 0), eap -> forceit ? 2 : 0);
    if (curbuf -> b_ml.ml_line_count > 1) ml_delete (curbuf -> b_ml.ml_line_count, 0);
    redraw_later (40);
}
    ...
    int dumpflags_arg;
{
    ...
    if (pat != ((void *) 0)) {
        if (ic) dumpflags |= 4;
            ...
            if (n == 0x02) dumpflags |= 8;
            else if (n == 0x04 && (int) strlen ((char *) (pat)) > 1) dumpflags |= 16;
        }
    }
    for (lpi = 0; lpi < curwin -> w_s -> b_langp.ga_len; ++lpi) {
        lp = (((langp_T *) (curwin -> w_s -> b_langp).ga_data) + (lpi));
        ...
        if (p [0] != 0) {
            if (region_names == ((void *) 0)) region_names = p;
    ...
    for (lpi = 0; lpi < curwin -> w_s -> b_langp.ga_len; ++lpi) {
        lp = (((langp_T *) (curwin -> w_s -> b_langp).ga_data) + (lpi));
        slang = lp -> lp_slang;
        if (slang -> sl_fbyts == ((void *) 0)) continue;
        if (pat == ((void *) 0)) {
            vim_snprintf ((char *) IObuff, (1024 + 1), "# file: %s", slang -> sl_fname);
            ml_append (lnum ++, IObuff, (colnr_T) 0, 0);
        }
        if (pat != ((void *) 0) && slang -> sl_pbyts == ((void *) 0)) patlen = (int) strlen ((char *) (pat));
        else patlen = -1;
        for (round = 1; round <= 2; ++round) {
            if (round == 1) {
                dumpflags &= ~1;
            ...
            if (byts == ((void *) 0)) continue;
            depth = 0;
            arridx [0] = 0;
            curi [0] = 1;
            while (depth >= 0 && !got_int && (pat == ((void *) 0) || !compl_interrupted)) {
                if (curi [depth] > byts [arridx [depth]]) {
                    --depth;
                    line_breakcheck ();
                    ...
                    if (c == 0) {
                        flags = (int) idxs [n];
                        if ((round == 2 || (flags & 0x80) == 0) && (flags & 0x0200) == 0 && (do_region || (flags & 0x01) == 0 || (((unsigned) flags >> 16) & lp
                          -> lp_region) != 0)) {
                            word [depth] = '\000';
                            if (!do_region) flags &= ~0x01;
                            c = (unsigned) flags >> 24;
                            if (c == 0 || curi [depth] == 2) {
                                dump_word (slang, word, pat, dir, dumpflags, flags, lnum);
                                if (pat == ((void *) 0)) ++lnum;
                            }
                            if (c != 0) lnum = dump_prefixes (slang, word, pat, dir, dumpflags, flags, lnum);
                        }
                    }
                    else {
                        word [depth ++] = c;
                        arridx [depth] = idxs [n];
                        curi [depth] = 1;
                        if (depth <= patlen && strncasecmp ((char *) ((word)), (char *) ((pat)), (size_t) ((depth))) != 0) --depth;
                    }
                }
            }
        }
    ...
    linenr_T lnum;
{
    ...
    int i;
    int flags = wordflags;
    if (dumpflags & 8) flags |= 0x02;
    if (dumpflags & 16) flags |= 0x04;
    if ((dumpflags & 1) == 0 && (flags & (0x02 | 0x04 | 0x80 | 0x40)) != 0) {
        ...
        p = word;
        if ((dumpflags & 1) && ((captype (word, ((void *) 0)) & 0x80) == 0 || (flags & 0x40) != 0)) keepcap = 1;
    }
    tw = p;
    if (pat == ((void *) 0)) {
        if ((flags & (0x10 | 0x08 | 0x01)) || keepcap) {
            ((__builtin_object_size ((char *) (badword), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (badword), (char *) (p), __builtin_object_size (
              (char *) (badword), 1 > 1)) : __inline_strcpy_chk ((char *) (badword), (char *) (p)));
            ((__builtin_object_size ((char *) (badword), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (badword), (char *) ("/"), __builtin_object_size
              ((char *) (badword), 1 > 1)) : __inline_strcat_chk ((char *) (badword), (char *) ("/")));
            if (keepcap) ((__builtin_object_size ((char *) (badword), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (badword), (char *) ("="),
              __builtin_object_size ((char *) (badword), 1 > 1)) : __inline_strcat_chk ((char *) (badword), (char *) ("=")));
            if (flags & 0x10) ((__builtin_object_size ((char *) (badword), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (badword), (char *) ("!"),
              __builtin_object_size ((char *) (badword), 1 > 1)) : __inline_strcat_chk ((char *) (badword), (char *) ("!")));
            else if (flags & 0x08) ((__builtin_object_size ((char *) (badword), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (badword), (char *) ("?"
              ), __builtin_object_size ((char *) (badword), 1 > 1)) : __inline_strcat_chk ((char *) (badword), (char *) ("?")));
            if (flags & 0x01) for (i = 0; i < 7; ++i) if (flags & (0x10000 << i)) __builtin___sprintf_chk ((char *) badword + strlen ((char *) (badword)), 0,
              __builtin_object_size ((char *) badword + strlen ((char *) (badword)), 1 > 1), "%d", i + 1);
            p = badword;
        }
        if (dumpflags & 2) {
            hashitem_T * hi;
            ...
            if (!((hi)->hi_key==((void*)0)||(hi)->hi_key==&hash_removed)) {
                vim_snprintf ((char *) IObuff, (1024 + 1), "%s\t%d", tw, ((wordcount_T *) ((hi) -> hi_key - (unsigned) (dumwc.wc_word - (char_u *) &dumwc))) ->
                  wc_count);
                p = IObuff;
            }
        }
    ...
    else if (((dumpflags & 4) ? strncasecmp ((char *) ((p)), (char *) ((pat)), (size_t) ((strlen ((char *) (pat))))) == 0 : strncmp ((char *) (p), (char *) (
      pat), (size_t) (strlen ((char *) (pat)))) == 0) && ins_compl_add_infercase (p, (int) strlen ((char *) (p)), p_ic, ((void *) 0), *dir, 0) == 1) *dir = 1;
}

static linenr_T dump_prefixes (slang, word, pat, dir, dumpflags, flags, startlnum)
    slang_T * slang;
    char_u * word;
    char_u * pat;
    int * dir;
    ...
    if (byts != ((void *) 0)) {
        ...
        while (depth >= 0 && !got_int) {
            ...
            if (curi [depth] > len) {
                ...
                c = byts [n];
                if (c == 0) {
                    for (i = 1; i < len; ++i) if (byts [n + i] != 0) break;
                    curi [depth] += i - 1;
                    c = valid_word_prefix (i, n, flags, word, slang, 0);
                    if (c != 0) {
                        vim_strncpy (prefix + depth, word, 250 - depth - 1);
                        dump_word (slang, prefix, pat, dir, dumpflags, (c & (0x01 << 24)) ? (flags | 0x08) : flags, lnum);
                        if (lnum != 0) ++lnum;
                    }
                    if (has_word_up) {
                        c = valid_word_prefix (i, n, flags, word_up, slang, 1);
                        if (c != 0) {
                            vim_strncpy (prefix + depth, word_up, 250 - depth - 1);
                            dump_word (slang, prefix, pat, dir, dumpflags, (c & (0x01 << 24)) ? (flags | 0x08) : flags, lnum);
                            if (lnum != 0) ++lnum;
                        }
                    }
...
{
    char_u * p = start;
    while (*p != '\000' && spell_iswordp (p, win)) ++p;
    return p;
}
    ...
    int startcol;
{
    ...
    char_u * p;
    int col = 0;
    if (no_spell_checking (curwin)) return startcol;
    line = ml_get_curline ();
    for (p = line + startcol; p > line;) {
        --p;
        if (spell_iswordp_nmw (p)) break;
    }
    while (p > line) {
        col = (int) (p - line);
        --p;
        if (!spell_iswordp(p,curwin)) break;
        col = 0;
    }
    return col;
}
