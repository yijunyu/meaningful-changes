...
int get_indent_buf (buf, lnum)
    ...
    linenr_T lnum;
{
    return get_indent_str (ml_get_buf (buf, lnum, 0), (int) buf -> b_p_ts);
}
    ...
    int flags;
{
    ...
    if (!curbuf->b_p_et || (!(flags&2) && curbuf -> b_p_pi)) {
        if (!(flags&2) && curbuf -> b_p_pi) {
            ...
            if (curbuf -> b_p_et) orig_char_len = ind_len;
            tab_pad = (int) curbuf -> b_p_ts - (ind_done % (int) curbuf -> b_p_ts);
    ...
    if (!doit && !((*p) == ' ' || (*p) == '\t') && !(flags&2)) return 0;
    if (flags & 2) p = oldline;
    else p = skipwhite (p);
    line_len = (int) strlen ((char *) (p)) + 1;
    if (orig_char_len != -1) {
        newline = alloc (orig_char_len + size - ind_done + line_len);
        if (newline == ((void *) 0)) return 0;
        todo = size - ind_done;
    ...
    ((__builtin_object_size ((char *) (s), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (s), (char *) (p), (size_t) line_len, __builtin_object_size (
      (char *) (s), 0)) : __inline_memmove_chk ((char *) (s), (char *) (p), (size_t) line_len));
    if (!(flags&4) || u_savesub (curwin -> w_cursor.lnum) == 1) {
        ml_replace (curwin -> w_cursor.lnum, newline, 0);
        if (flags & 1) changed_bytes (curwin -> w_cursor.lnum, 0);
        if (saved_cursor.lnum == curwin -> w_cursor.lnum && saved_cursor.col >= (colnr_T) (p - oldline)) saved_cursor.col += ind_len - (colnr_T) (p - oldline);
        retval = 1;
    }
    ...
    char_u * src;
{
    ...
    for (round = 1; round <= 2; ++round) {
        ...
        if (p == ((void *) 0)) {
            line_len = (int) strlen ((char *) (ml_get_curline ())) + 1;
            line = alloc (ind_len + line_len);
            if (line == ((void *) 0)) return 0;
            p = line;
        }
    }
    ((__builtin_object_size ((char *) (p), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (p), (char *) (ml_get_curline ()), (size_t) line_len,
      __builtin_object_size ((char *) (p), 0)) : __inline_memmove_chk ((char *) (p), (char *) (ml_get_curline ()), (size_t) line_len));
    ...
    linenr_T lnum;
{
    ...
    if (lnum > curbuf -> b_ml.ml_line_count) return -1;
    pos.lnum = 0;
    regmatch.regprog = vim_regcomp (curbuf -> b_p_flp, 1);
    if (regmatch.regprog != ((void *) 0)) {
        regmatch.rmm_ic = 0;
        regmatch.rmm_maxcol = 0;
        if (vim_regexec_multi (&regmatch, curwin, curbuf, lnum, (colnr_T) 0, ((void *) 0))) {
    ...
    if (pos.lnum == 0 || *ml_get_pos(&pos) == '\000') return -1;
    ...
    char_u * line;
{
    ...
    int retval = 0;
    int len;
    cinw_len = (int) strlen ((char *) (curbuf -> b_p_cinw)) + 1;
    cinw_buf = alloc ((unsigned) cinw_len);
    if (cinw_buf != ((void *) 0)) {
        line = skipwhite (line);
        for (cinw = curbuf -> b_p_cinw; *cinw;) {
            len = copy_option_part (&cinw, cinw_buf, cinw_len, ",");
            if (strncmp ((char *) (line), (char *) (cinw_buf), (size_t) (len)) == 0 && (!vim_iswordc(line[len]) || !vim_iswordc(line[len-1]))) {
                retval = 1;
                break;
            }
        }
...
int open_line (dir, flags, old_indent)
    int dir;
    int flags;
    int old_indent;
{
    ...
    if (saved_line == ((void *) 0)) return 0;
    if (State & 0x80) {
        if (curwin -> w_cursor.lnum < orig_line_count) next_line = vim_strsave (ml_get (curwin -> w_cursor.lnum + 1));
        else next_line = vim_strsave ((char_u *) "");
        if (next_line == ((void *) 0)) goto theend;
        replace_push ('\000');
        replace_push ('\000');
    ...
    did_si = 0;
    ai_col = 0;
    if (dir == 1 && did_ai) trunc_line = 1;
    if (curbuf -> b_p_ai || do_si) {
        newindent = get_indent_str (saved_line, (int) curbuf -> b_p_ts);
        if (newindent == 0) newindent = old_indent;
        if (!trunc_line && do_si && *saved_line != '\000' && (p_extra == ((void *) 0) || first_char != '{')) {
            char_u * ptr;
            char_u last_char;
            old_cursor = curwin -> w_cursor;
            ptr = saved_line;
            if (flags & 2) lead_len = get_leader_len (ptr, ((void *) 0), 0);
            else lead_len = 0;
            if (dir == 1) {
                if (lead_len == 0 && ptr [0] == '#') {
                    while (ptr [0] == '#' && curwin -> w_cursor.lnum > 1) ptr = ml_get (--curwin->w_cursor.lnum);
                ...
                if (lead_len > 0) {
                    ...
                    else if (last_char != ';' && last_char != '}' && cin_is_cinword (ptr)) did_si = 1;
                }
            }
            else {
                if (lead_len == 0 && ptr [0] == '#') {
                    ...
                    while ((ptr [0] == '#' || was_backslashed) && curwin -> w_cursor.lnum < curbuf -> b_ml.ml_line_count) {
                        if (*ptr && ptr [strlen ((char *) (ptr)) - 1] == '\\') was_backslashed = 1;
                        else was_backslashed = 0;
                        ptr = ml_get (++curwin->w_cursor.lnum);
                    }
                    if (was_backslashed) newindent = 0;
    ...
    if (flags & 2) lead_len = get_leader_len (saved_line, &lead_flags, dir == (-1));
    else lead_len = 0;
    if (lead_len > 0) {
        ...
        for (p = lead_flags; *p && *p != ':'; ++p) {
            ...
            if (*p == 's' || *p == 'm') {
                ...
                while (*p && p [-1] != ':') {
                    if (*p == 'b') require_blank = 1;
                    ++p;
                }
                (void) copy_option_part (&p, lead_middle, 50, ",");
                while (*p && p [-1] != ':') {
                    if (*p == 'x') end_comment_pending = -1;
                    ++p;
                }
                n = copy_option_part (&p, lead_end, 50, ",");
                if (end_comment_pending == -1) end_comment_pending = lead_end [n - 1];
                if (dir == 1) {
                    for (p = saved_line + lead_len; *p; ++p) if (strncmp ((char *) (p), (char *) (lead_end), (size_t) (n)) == 0) {
                        comment_end = p;
                        lead_len = 0;
                        break;
                    }
                ...
                if (lead_len > 0) {
                    ...
                    if (!((saved_line[lead_len-1])==' '||(saved_line[lead_len-1])=='\t') && ((p_extra != ((void *) 0) && (int) curwin -> w_cursor.col ==
                      lead_len) || (p_extra == ((void *) 0) && saved_line [lead_len] == '\000') || require_blank)) extra_space = 1;
                }
                break;
            }
            if (*p == 'e') {
            ...
            if (leader == ((void *) 0)) lead_len = 0;
                ...
                if (lead_repl != ((void *) 0)) {
                    ...
                    for (p = lead_flags; *p != '\000' && *p != ':';) {
                        if (*p == 'r' || *p == 'l') c = *p++;
                        else if (((*p) >= '0' && (*p) <= '9') || *p == '-') off = getdigits (&p);
                        else ++p;
                    }
                    if (c == 'r') {
                        for (p = leader + lead_len - 1; p > leader && ((*p) == ' ' || (*p) == '\t'); --p);
                        ++p;
                        if (p < leader + lead_repl_len) p = leader;
                        else p -= lead_repl_len;
                        ((__builtin_object_size ((char *) (p), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (p), (char *) (lead_repl), (size_t)
                          lead_repl_len, __builtin_object_size ((char *) (p), 0)) : __inline_memmove_chk ((char *) (p), (char *) (lead_repl), (size_t)
                          lead_repl_len));
                        if (p + lead_repl_len > leader + lead_len) p [lead_repl_len] = '\000';
                        while (--p >= leader) {
                            if (!((*p) == ' ' || (*p) == '\t')) *p = ' ';
                        }
                    }
                        ...
                        ((__builtin_object_size ((char *) (p), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (p), (char *) (lead_repl), (size_t)
                          lead_repl_len, __builtin_object_size ((char *) (p), 0)) : __inline_memmove_chk ((char *) (p), (char *) (lead_repl), (size_t)
                          lead_repl_len));
                        for (p += lead_repl_len; p < leader + lead_len; ++p) if (!((*p) == ' ' || (*p) == '\t')) {
                            if (p + 1 < leader + lead_len && p [1] == '\011') {
                                --lead_len;
                                ((__builtin_object_size ((char *) (p), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (p), (char *) (p + 1), (leader +
                                  lead_len) - p, __builtin_object_size ((char *) (p), 0)) : __inline_memmove_chk ((char *) (p), (char *) (p + 1), (leader +
                                  lead_len) - p));
                            }
                            else {
                                * p = ' ';
                    ...
                    if (curbuf -> b_p_ai || do_si) newindent = get_indent_str (leader, (int) curbuf -> b_p_ts);
                    if (newindent + off < 0) {
                        off = -newindent;
                        newindent = 0;
                    }
                    ...
                    while (off > 0 && lead_len > 0 && leader [lead_len - 1] == ' ') {
                        if (vim_strchr (skipwhite (leader), '\t') != ((void *) 0)) break;
                        --lead_len;
                        --off;
                    }
                    if (lead_len > 0 && ((leader [lead_len - 1]) == ' ' || (leader [lead_len - 1]) == '\t')) extra_space = 0;
                    leader [lead_len] = '\000';
                }
                if (extra_space) {
                    leader [lead_len ++] = ' ';
    ...
    if (p_extra != ((void *) 0)) {
        * p_extra = saved_char;
        if ((((State) & 0x40) && !((State)&0x80))) replace_push ('\000');
        if (curbuf -> b_p_ai || (flags & 1)) {
            while ((*p_extra == ' ' || *p_extra == '\t')) {
                if ((((State) & 0x40) && !((State)&0x80))) replace_push (*p_extra);
                ++p_extra;
                ++less_cols_off;
            }
        }
        if (*p_extra != '\000') did_ai = 0;
        less_cols = (int) (p_extra - saved_line);
    }
    if (p_extra == ((void *) 0)) p_extra = (char_u *) "";
    if (lead_len) {
        ((__builtin_object_size ((char *) (leader), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (leader), (char *) (p_extra), __builtin_object_size (
          (char *) (leader), 1 > 1)) : __inline_strcat_chk ((char *) (leader), (char *) (p_extra)));
        p_extra = leader;
        did_ai = 1;
        less_cols -= lead_len;
    }
    else end_comment_pending = '\000';
    old_cursor = curwin -> w_cursor;
    if (dir == (-1)) --curwin->w_cursor.lnum;
    if (!(State&0x80) || old_cursor.lnum >= orig_line_count) {
        if (ml_append (curwin -> w_cursor.lnum, p_extra, (colnr_T) 0, 0) == 0) goto theend;
        mark_adjust (curwin -> w_cursor.lnum + 1, (linenr_T) (0x7fffffffL), 1L, 0L);
        did_append = 1;
    }
    else {
    ...
    if (newindent || did_si) {
        ++curwin->w_cursor.lnum;
        if (did_si) {
            if (p_sr) newindent -= newindent % (int) curbuf -> b_p_sw;
        ...
        else (void) set_indent (newindent, 2);
        less_cols -= curwin -> w_cursor.col;
        ai_col = curwin -> w_cursor.col;
        if ((((State) & 0x40) && !((State)&0x80))) for (n = 0; n < (int) curwin -> w_cursor.col; ++n) replace_push ('\000');
        newcol += curwin -> w_cursor.col;
        if (no_si) did_si = 0;
    }
    if ((((State) & 0x40) && !((State)&0x80))) while (lead_len -- > 0) replace_push ('\000');
    curwin -> w_cursor = old_cursor;
    if (dir == 1) {
        if (trunc_line || (State & 0x10)) {
            saved_line [curwin -> w_cursor.col] = '\000';
            if (trunc_line && !(flags&4)) truncate_spaces (saved_line);
            ...
            if (did_append) {
                ...
                did_append = 0;
                if (flags & 8) mark_col_adjust (curwin -> w_cursor.lnum, curwin -> w_cursor.col + less_cols_off, 1L, (long) -less_cols);
            }
            else changed_bytes (curwin -> w_cursor.lnum, curwin -> w_cursor.col);
        }
        curwin -> w_cursor.lnum = old_cursor.lnum + 1;
    }
    if (did_append) changed_lines (curwin -> w_cursor.lnum, 0, curwin -> w_cursor.lnum, 1L);
    ...
    if (!p_paste && leader == ((void *) 0) && curbuf -> b_p_lisp && curbuf -> b_p_ai) {
        fixthisline (get_lisp_indent);
        p = ml_get_curline ();
        ai_col = (colnr_T) (skipwhite (p) - p);
    }
    if (!p_paste && (curbuf -> b_p_cin || *curbuf->b_p_inde != '\000') && in_cinkeys (dir == 1 ? 0x101 : 0x102, ' ', linewhite (curwin -> w_cursor.lnum))) {
        do_c_expr_indent ();
        p = ml_get_curline ();
        ai_col = (colnr_T) (skipwhite (p) - p);
    }
...
int get_leader_len (line, flags, backward)
    char_u * line;
    char_u * * flags;
    int backward;
{
    ...
    while (line [i]) {
        ...
        for (list = curbuf -> b_p_com; *list;) {
            if (!got_com && flags != ((void *) 0)) *flags = list;
            (void) copy_option_part (&list, part_buf, 50, ",");
            string = vim_strchr (part_buf, ':');
            if (string == ((void *) 0)) continue;
            *string++ = '\000';
            if (got_com && vim_strchr (part_buf, 'n') == ((void *) 0)) continue;
            if (backward && vim_strchr (part_buf, 'O') != ((void *) 0)) continue;
            if (((string [0]) == ' ' || (string [0]) == '\t')) {
                if (i == 0 || !((line[i-1])==' '||(line[i-1])=='\t')) continue;
                while (((string [0]) == ' ' || (string [0]) == '\t')) ++string;
            }
            for (j = 0; string [j] != '\000' && string [j] == line [i + j]; ++j);
            if (string [j] != '\000') continue;
            if (vim_strchr (part_buf, 'b') != ((void *) 0) && !((line[i+j])==' '||(line[i+j])=='\t') && line [i + j] != '\000') continue;
            i += j;
            got_com = 1;
            found_one = 1;
            break;
        ...
        if (!found_one) break;
        while (((line [i]) == ' ' || (line [i]) == '\t')) ++i;
        if (vim_strchr (part_buf, 'n') == ((void *) 0)) break;
    }
    return (got_com ? i : 0);
}
...
{
    int lines;
    if (!wp->w_onebuf_opt.wo_wrap) return 1;
    if (wp -> w_width == 0) return 1;
    if (lineFolded (wp, lnum) == 1) return 1;
    lines = plines_win_nofold (wp, lnum);
    if (winheight > 0 && lines > wp -> w_height) return (int) wp -> w_height;
    return lines;
}
    ...
    linenr_T lnum;
{
    ...
    if (*s == '\000') return 1;
    col = win_linetabsize (wp, s, (colnr_T) (0x7fffffffL));
    if (wp -> w_onebuf_opt.wo_list && lcs_eol != '\000') col += 1;
    width = (wp -> w_width) - win_col_off (wp);
    if (width <= 0) return 32000;
    ...
    long column;
{
    ...
    if (!wp->w_onebuf_opt.wo_wrap) return lines + 1;
    if (wp -> w_width == 0) return lines + 1;
    s = ml_get_buf (wp -> w_buffer, lnum, 0);
    col = 0;
    while (*s != '\000' && --column >= 0) {
        col += win_lbr_chartabsize (wp, s, (colnr_T) col, ((void *) 0));
        ++s;
    }
    if (*s == '\011' && (State & 0x01) && (!wp->w_onebuf_opt.wo_list || lcs_tab1)) col += win_lbr_chartabsize (wp, s, (colnr_T) col, ((void *) 0)) - 1;
    width = (wp -> w_width) - win_col_off (wp);
    if (width <= 0) return 9999;
    lines += 1;
    if (col > width) lines += (col - width) / (width + win_col_off2 (wp)) + 1;
    return lines;
}
    ...
    linenr_T first, last;
{
    ...
    while (first <= last) {
        ...
        if (x > 0) {
            ...
            if (first == wp -> w_topline) count += plines_win_nofill (wp, first, 1) + wp -> w_topfill;
            else count += plines_win (wp, first, 1);
            ++first;
        }
    }
    return (count);
    ...
    int c;
{
    ...
    int i;
    if (virtual_active () && curwin -> w_cursor.coladd > 0) coladvance_force (getviscol ());
    ...
    if (State & 0x40) {
        if (State & 0x80) {
            ...
            if (old_list && vim_strchr (p_cpo, 'L') == ((void *) 0)) curwin -> w_onebuf_opt.wo_list = 0;
            ...
            while (oldp [col + oldlen] != '\000' && vcol < new_vcol) {
                ...
                if (vcol > new_vcol && oldp [col + oldlen] == '\011') break;
                ++oldlen;
                if (vcol > new_vcol) newlen += vcol - new_vcol;
            }
    ...
    if (newp == ((void *) 0)) return;
    if (col > 0) ((__builtin_object_size ((char *) (newp), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (newp), (char *) (oldp), (size_t) col,
      __builtin_object_size ((char *) (newp), 0)) : __inline_memmove_chk ((char *) (newp), (char *) (oldp), (size_t) col));
    p = newp + col;
    ((__builtin_object_size ((char *) (p + newlen), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (p + newlen), (char *) (oldp + col + oldlen), (
      size_t) (linelen - col - oldlen), __builtin_object_size ((char *) (p + newlen), 0)) : __inline_memmove_chk ((char *) (p + newlen), (char *) (oldp + col +
      oldlen), (size_t) (linelen - col - oldlen)));
    * p = c;
    i = 1;
    while (i < newlen) p [i ++] = ' ';
    ml_replace (lnum, newp, 0);
    changed_bytes (lnum, col);
    if (p_sm && (State & 0x10) && msg_silent == 0 && !ins_compl_active()) showmatch (c);
    {
        ++curwin->w_cursor.col;
    }
}
    ...
    char_u * s;
{
    ...
    if (col > 0) ((__builtin_object_size ((char *) (newp), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (newp), (char *) (oldp), (size_t) col,
      __builtin_object_size ((char *) (newp), 0)) : __inline_memmove_chk ((char *) (newp), (char *) (oldp), (size_t) col));
    ((__builtin_object_size ((char *) (newp + col), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (newp + col), (char *) (s), (size_t) newlen,
      __builtin_object_size ((char *) (newp + col), 0)) : __inline_memmove_chk ((char *) (newp + col), (char *) (s), (size_t) newlen));
    ((__builtin_object_size ((char *) (newp + col + newlen), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (newp + col + newlen), (char *) (oldp + col
      ), (size_t) (oldlen - col + 1), __builtin_object_size ((char *) (newp + col + newlen), 0)) : __inline_memmove_chk ((char *) (newp + col + newlen), (char *
      ) (oldp + col), (size_t) (oldlen - col + 1)));
    ml_replace (lnum, newp, 0);
    changed_bytes (lnum, col);
...
int del_bytes (count, fixpos_arg, use_delcombine)
    long count;
    int fixpos_arg;
    int use_delcombine __attribute__ ((unused));
{
    char_u * oldp, * newp;
    colnr_T oldlen;
    linenr_T lnum = curwin -> w_cursor.lnum;
    ...
    oldp = ml_get (lnum);
    oldlen = (int) strlen ((char *) (oldp));
    if (col >= oldlen) return 0;
    movelen = (long) oldlen - (long) col - count + 1;
    if (movelen <= 1) {
        if (col > 0 && fixpos && restart_edit == 0 && (ve_flags & 8) == 0) {
            --curwin->w_cursor.col;
            curwin -> w_cursor.coladd = 0;
        }
        count = oldlen - col;
    ...
    if (netbeans_active ()) was_alloced = 0;
    else was_alloced = ml_line_alloced ();
    if (was_alloced) newp = oldp;
    else {
        newp = alloc ((unsigned) (oldlen + 1 - count));
        if (newp == ((void *) 0)) return 0;
        ((__builtin_object_size ((char *) (newp), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (newp), (char *) (oldp), (size_t) col,
          __builtin_object_size ((char *) (newp), 0)) : __inline_memmove_chk ((char *) (newp), (char *) (oldp), (size_t) col));
    }
    ((__builtin_object_size ((char *) (newp + col), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (newp + col), (char *) (oldp + col + count), (
      size_t) movelen, __builtin_object_size ((char *) (newp + col), 0)) : __inline_memmove_chk ((char *) (newp + col), (char *) (oldp + col + count), (size_t)
      movelen));
    if (!was_alloced) ml_replace (lnum, newp, 0);
    ...
    int fixpos;
{
    ...
    linenr_T lnum = curwin -> w_cursor.lnum;
    colnr_T col = curwin -> w_cursor.col;
    if (col == 0) newp = vim_strsave ((char_u *) "");
    else newp = vim_strnsave (ml_get (lnum), col);
    if (newp == ((void *) 0)) return 0;
    ml_replace (lnum, newp, 0);
    changed_bytes (lnum, curwin -> w_cursor.col);
    if (fixpos && curwin -> w_cursor.col > 0) --curwin->w_cursor.col;
    return 1;
}
    ...
    int undo;
{
    ...
    if (nlines <= 0) return;
    if (undo && u_savedel (first, nlines) == 0) return;
    for (n = 0; n < nlines;) {
        if (curbuf -> b_ml.ml_flags & 1) break;
        ml_delete (first, 1);
        ++n;
        if (first > curbuf -> b_ml.ml_line_count) break;
    }
    curwin -> w_cursor.col = 0;
    check_cursor_lnum ();
...
void pchar_cursor (c)
    int c;
{
    *(ml_get_buf(curbuf,curwin->w_cursor.lnum,1)+curwin->w_cursor.col) = c;
}
    ...
    int extra;
{
    char_u * ptr;
    colnr_T col;
    for (col = 0, ptr = ml_get_curline (); ((*ptr) == ' ' || (*ptr) == '\t'); ++col) ++ptr;
    if (col >= curwin -> w_cursor.col + extra) return 1;
    else return 0;
}
    ...
    colnr_T col;
{
    ...
    if (curwin -> w_onebuf_opt.wo_diff) {
        win_T * wp;
        linenr_T wlnum;
        for (wp = firstwin; wp != ((void *) 0); wp = wp -> w_next) if (wp -> w_onebuf_opt.wo_diff && wp != curwin) {
...
void appended_lines_mark (lnum, count)
    ...
    long count;
{
    mark_adjust (lnum + 1, (linenr_T) (0x7fffffffL), count, 0L);
    changed_lines (lnum + 1, 0, lnum + 1, count);
}
...
void deleted_lines_mark (lnum, count)
    ...
    long count;
{
    mark_adjust (lnum, (linenr_T) (lnum + count - 1), (long) (0x7fffffffL), -count);
    changed_lines (lnum, 0, lnum + count, -count);
}
    ...
    long xtra;
{
    ...
    if (xtra == 0 && curwin -> w_onebuf_opt.wo_diff) {
        ...
        for (wp = firstwin; wp != ((void *) 0); wp = wp -> w_next) if (wp -> w_onebuf_opt.wo_diff && wp != curwin) {
            redraw_win_later (wp, 10);
            wlnum = diff_lnum_win (lnum, wp);
            if (wlnum > 0) changed_lines_buf (wp -> w_buffer, wlnum, lnume - lnum + wlnum, 0L);
        }
    }
    ...
    long xtra;
{
    if (buf -> b_mod_set) {
        ...
        if (lnum < buf -> b_mod_bot) {
            buf -> b_mod_bot += xtra;
            if (buf -> b_mod_bot < lnum) buf -> b_mod_bot = lnum;
        }
        if (lnume + xtra > buf -> b_mod_bot) buf -> b_mod_bot = lnume + xtra;
    ...
    long xtra;
{
    ...
    if (!cmdmod.keepjumps) {
        ...
        if (curbuf -> b_new_change || curbuf -> b_changelistlen == 0) {
            if (curbuf -> b_changelistlen == 0) add = 1;
            else {
                p = &curbuf->b_changelist[curbuf->b_changelistlen-1];
                if (p -> lnum != lnum) add = 1;
                else {
            ...
            if (add) {
                ...
                if (curbuf -> b_changelistlen == 100) {
                    ...
                    ((__builtin_object_size ((char *) (curbuf -> b_changelist), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (curbuf -> b_changelist
                      ), (char *) (curbuf -> b_changelist + 1), sizeof (pos_T) * (100 - 1), __builtin_object_size ((char *) (curbuf -> b_changelist), 0)) :
                      __inline_memmove_chk ((char *) (curbuf -> b_changelist), (char *) (curbuf -> b_changelist + 1), sizeof (pos_T) * (100 - 1)));
                    for ((tp) = first_tabpage; (tp) != ((void *) 0); (tp) = (tp) -> tp_next) for ((wp) = ((tp) == curtab) ? firstwin : (tp) -> tp_firstwin; (wp
                      ); (wp) = (wp) -> w_next) {
                        if (wp -> w_buffer == curbuf && wp -> w_changelistidx > 0) --wp->w_changelistidx;
                    }
                }
                for ((tp) = first_tabpage; (tp) != ((void *) 0); (tp) = (tp) -> tp_next) for ((wp) = ((tp) == curtab) ? firstwin : (tp) -> tp_firstwin; (wp); (
                  wp) = (wp) -> w_next) {
                    if (wp -> w_buffer == curbuf && wp -> w_changelistidx == curbuf -> b_changelistlen) ++wp->w_changelistidx;
                }
    ...
    for ((tp) = first_tabpage; (tp) != ((void *) 0); (tp) = (tp) -> tp_next) for ((wp) = ((tp) == curtab) ? firstwin : (tp) -> tp_firstwin; (wp); (wp) = (wp) ->
      w_next) {
        if (wp -> w_buffer == curbuf) {
            if (wp -> w_redr_type < 10) wp -> w_redr_type = 10;
            foldUpdate (wp, lnum, lnume + xtra - 1);
            i = hasFoldingWin (wp, lnum, &lnum, ((void *) 0), 0, ((void *) 0));
            if (wp -> w_cursor.lnum == lnum) wp -> w_cline_folded = i;
            i = hasFoldingWin (wp, lnume, ((void *) 0), &lnume, 0, ((void *) 0));
            if (wp -> w_cursor.lnum == lnume) wp -> w_cline_folded = i;
            if (wp -> w_cursor.lnum <= lnum) {
                i = find_wl_entry (wp, lnum);
                if (i >= 0 && wp -> w_cursor.lnum > wp -> w_lines [i].wl_lnum) changed_line_abv_curs_win (wp);
            }
            if (wp -> w_cursor.lnum > lnum) changed_line_abv_curs_win (wp);
            else if (wp -> w_cursor.lnum == lnum && wp -> w_cursor.col >= col) changed_cline_bef_curs_win (wp);
            if (wp -> w_botline >= lnum) {
                approximate_botline_win (wp);
            }
            for (i = 0; i < wp -> w_lines_valid; ++i) if (wp -> w_lines [i].wl_valid) {
            ...
            if (hasAnyFolding (wp)) set_topline (wp, wp -> w_topline);
        }
    }
    if (must_redraw < 10) must_redraw = 10;
    if (lnum <= curwin -> w_cursor.lnum && lnume + (xtra < 0 ? -xtra : xtra) > curwin -> w_cursor.lnum) last_cursormoved.lnum = 0;
}
...
{
    win_T * wp;
    for (wp = firstwin; wp != ((void *) 0); wp = wp -> w_next) if (wp -> w_buffer == buf && wp -> w_status_height) {
        wp -> w_redr_status = 1;
        if (must_redraw < 10) must_redraw = 10;
    }
}
    ...
    int col;
{
    ...
    if (curbuf -> b_did_warn == 0 && curbufIsChanged () == 0 && !autocmd_busy && curbuf -> b_p_ro) {
        ++curbuf_lock;
        apply_autocmds (EVENT_FILECHANGEDRO, ((void *) 0), ((void *) 0), 0, curbuf);
        --curbuf_lock;
        if (!curbuf->b_p_ro) return;
        msg_start ();
        if (msg_row == Rows - 1) msg_col = col;
        msg_source (highlight_attr [(int) (HLF_W)]);
        msg_puts_attr ((char_u *) (((char *) (w_readonly))), (highlight_attr [(int) (HLF_W)] | 0x1000));
        set_vim_var_string (4, (char_u *) ((char *) (w_readonly)), -1);
        msg_clr_eos ();
        (void) msg_end ();
...
int get_keystroke () {
    ...
    for (;;) {
        cursor_on ();
        out_flush ();
        n = ui_inchar (buf + len, (151 - 6 - len) / 3, len == 0 ? -1L : 100L, 0);
        if (n > 0) {
            n = fix_input_buffer (buf + len, n, 0);
            len += n;
            waited = 0;
        }
        else if (len > 0) ++waited;
        if ((n = check_termcode (1, buf, len)) < 0 && (!p_ttimeout || waited * 100L < (p_ttm < 0 ? p_tm : p_ttm))) continue;
        if (n > 0) len = n;
        if (len == 0) continue;
        n = buf [0];
        if (n == (0x80)) {
            n = ((buf [1]) == 254 ? (0x80) : (buf [1]) == 255 ? (-((255)+((int)(('X'))<<8))) : (-((buf[1])+((int)(buf[2])<<8))));
            if (buf [1] == 252 || n == (-((253)+((int)(KE_IGNORE)<<8))) || n == (-((253)+((int)(KE_LEFTMOUSE_NM)<<8))) || n == 
              (-((253)+((int)(KE_LEFTDRAG)<<8))) || n == (-((253)+((int)(KE_LEFTRELEASE)<<8))) || n == (-((253)+((int)(KE_LEFTRELEASE_NM)<<8))) || n == 
              (-((253)+((int)(KE_MIDDLEMOUSE)<<8))) || n == (-((253)+((int)(KE_MIDDLEDRAG)<<8))) || n == (-((253)+((int)(KE_MIDDLERELEASE)<<8))) || n == 
              (-((253)+((int)(KE_RIGHTMOUSE)<<8))) || n == (-((253)+((int)(KE_RIGHTDRAG)<<8))) || n == (-((253)+((int)(KE_RIGHTRELEASE)<<8))) || n == 
              (-((253)+((int)(KE_MOUSEDOWN)<<8))) || n == (-((253)+((int)(KE_MOUSEUP)<<8))) || n == (-((253)+((int)(KE_MOUSELEFT)<<8))) || n == 
              (-((253)+((int)(KE_MOUSERIGHT)<<8))) || n == (-((253)+((int)(KE_X1MOUSE)<<8))) || n == (-((253)+((int)(KE_X1DRAG)<<8))) || n == 
              (-((253)+((int)(KE_X1RELEASE)<<8))) || n == (-((253)+((int)(KE_X2MOUSE)<<8))) || n == (-((253)+((int)(KE_X2DRAG)<<8))) || n == 
              (-((253)+((int)(KE_X2RELEASE)<<8)))) {
                if (buf [1] == 252) mod_mask = buf [2];
                len -= 3;
                if (len > 0) ((__builtin_object_size ((char *) (buf), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (buf), (char *) (buf + 3), (
                  size_t) len, __builtin_object_size ((char *) (buf), 0)) : __inline_memmove_chk ((char *) (buf), (char *) (buf + 3), (size_t) len));
                continue;
            }
            break;
        }
        if (n == intr_char) n = '\033';
        break;
    }
    ...
    int * mouse_used;
{
    ...
    int c;
    int typed = 0;
    if (mouse_used != ((void *) 0)) *mouse_used = 0;
    ...
    for (;;) {
        ...
        if (((c) >= '0' && (c) <= '9')) {
            ...
            * mouse_used = 1;
            ...
            stuffcharReadbuff (':');
            if (!exmode_active) cmdline_row = msg_row;
            skip_redraw = 1;
            do_redraw = 0;
            break;
        }
        else if (c == '\015' || c == '\012' || c == 3 || c == '\033') break;
    }
    --no_mapping;
    --allow_keys;
    return n;
    ...
    int * mouse_used;
{
    ...
    int save_State;
    if (mouse_used != ((void *) 0)) msg_puts ((char_u *) (((char *) ("Type number and <Enter> or click with mouse (empty cancels): "))));
    else msg_puts ((char_u *) (((char *) ("Type number and <Enter> (empty cancels): "))));
...
{
    long pn;
    if (global_busy || !messaging()) return;
    if (keep_msg != ((void *) 0) && !keep_msg_more) return;
    if (n > 0) pn = n;
    else pn = -n;
    if (pn > p_report) {
        if (pn == 1) {
            if (n > 0) ((__builtin_object_size ((char *) (msg_buf), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (msg_buf), (char *) (((char *) (
              "1 more line"))), __builtin_object_size ((char *) (msg_buf), 1 > 1)) : __inline_strcpy_chk ((char *) (msg_buf), (char *) (((char *) (
              "1 more line")))));
            else ((__builtin_object_size ((char *) (msg_buf), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (msg_buf), (char *) (((char *) (
              "1 line less"))), __builtin_object_size ((char *) (msg_buf), 1 > 1)) : __inline_strcpy_chk ((char *) (msg_buf), (char *) (((char *) (
              "1 line less")))));
        }
        else {
            if (n > 0) __builtin___sprintf_chk ((char *) msg_buf, 0, __builtin_object_size ((char *) msg_buf, 1 > 1), ((char *) ("%ld more lines")), pn);
            else __builtin___sprintf_chk ((char *) msg_buf, 0, __builtin_object_size ((char *) msg_buf, 1 > 1), ((char *) ("%ld fewer lines")), pn);
        }
        if (got_int) ((__builtin_object_size ((char *) (msg_buf), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (msg_buf), (char *) (((char *) (
          " (Interrupted)"))), __builtin_object_size ((char *) (msg_buf), 1 > 1)) : __inline_strcat_chk ((char *) (msg_buf), (char *) (((char *) (
          " (Interrupted)")))));
        if (msg (msg_buf)) {
...
void vim_beep () {
    if (emsg_silent == 0) {
        ...
        if (vim_strchr (p_debug, 'e') != ((void *) 0)) {
            msg_source (highlight_attr [(int) (HLF_W)]);
            msg_attr ((char_u *) ((char *) ("Beep!")), highlight_attr [(int) (HLF_W)]);
        }
    }
}
...
void init_homedir () {
    ...
    if (var != ((void *) 0) && *var == '\000') var = ((void *) 0);
    if (var != ((void *) 0)) {
        if (mch_dirname (NameBuff, 1024) == 1 && mch_chdir ((char *) NameBuff) == 0) {
            if (!mch_chdir((char*)var) && mch_dirname (IObuff, (1024 + 1)) == 1) var = IObuff;
            if (mch_chdir ((char *) NameBuff) != 0) emsg ((char_u *) (((char *) (e_prev_dir))));
        }
    ...
    int one;
{
    char_u * p;
    p = alloc (1024);
    if (p != ((void *) 0)) expand_env_esc (src, p, 1024, 0, one, ((void *) 0));
    return p;
}
    ...
    char_u * startstr;
{
    ...
    int at_start = 1;
    int startstr_len = 0;
    if (startstr != ((void *) 0)) startstr_len = (int) strlen ((char *) (startstr));
    src = skipwhite (srcp);
    --dstlen;
    while (*src && dstlen > 0) {
        copy_char = 1;
        if ((*src == '$') || (*src == '~' && at_start)) {
            ...
            if (*src != '~') {
            ...
            else if (src [1] == '\000' || vim_ispathsep (src [1]) || vim_strchr ((char_u *) " ,\t\n", src [1]) != ((void *) 0)) {
                var = homedir;
                tail = src + 1;
            }
            else {
                ...
                var = dst;
                c = dstlen - 1;
                while (c -- > 0 && *tail && vim_isfilec (*tail) && !vim_ispathsep(*tail)) *var++ = *tail++;
                * var = '\000';
                {
                    struct passwd * pw;
                    pw = getpwnam ((char *) dst + 1);
                    if (pw != ((void *) 0)) var = (char_u *) pw -> pw_dir;
                    else var = ((void *) 0);
                }
                if (var == ((void *) 0)) {
                    expand_T xpc;
                    ExpandInit (&xpc);
                    xpc.xp_context = 2;
                    var = ExpandOne (&xpc, dst, ((void *) 0), 16 | 64, 2);
                    mustfree = 1;
                }
            }
            if (esc && var != ((void *) 0) && (char_u *) strpbrk ((char *) (var), (char *) ((char_u *) " \t")) != ((void *) 0)) {
                ...
                if (p != ((void *) 0)) {
                    if (mustfree) vim_free (var);
                    var = p;
                    mustfree = 1;
                }
            }
            if (var != ((void *) 0) && *var != '\000' && (strlen ((char *) (var)) + strlen ((char *) (tail)) + 1 < (unsigned) dstlen)) {
                ((__builtin_object_size ((char *) (dst), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (dst), (char *) (var), __builtin_object_size ((
                  char *) (dst), 1 > 1)) : __inline_strcpy_chk ((char *) (dst), (char *) (var)));
                dstlen -= (int) strlen ((char *) (var));
                c = (int) strlen ((char *) (var));
                if (*var != '\000' && vim_ispathsep (*((dst+c)-1)) && vim_ispathsep (*tail)) ++tail;
                dst += c;
                src = tail;
                copy_char = 0;
            }
        ...
        if (copy_char) {
            ...
            else if ((src [0] == ' ' || src [0] == ',') && !one) at_start = 1;
            *dst++ = *src++;
            --dstlen;
            if (startstr != ((void *) 0) && src - startstr_len >= srcp && strncmp ((char *) (src - startstr_len), (char *) (startstr), (size_t) (startstr_len))
              == 0) at_start = 1;
        }
    }
    * dst = '\000';
    ...
    int * mustfree;
{
    ...
    if (p != ((void *) 0) && *p == '\000') p = ((void *) 0);
    if (p != ((void *) 0)) {
        return p;
    }
    vimruntime = (strcmp ((char *) (name), (char *) ("VIMRUNTIME")) == 0);
    if (!vimruntime && strcmp ((char *) (name), (char *) ("VIM")) != 0) return ((void *) 0);
    if (vimruntime && *default_vimruntime_dir == '\000') {
        p = (char_u *) getenv ((char *) ((char_u *) "VIM"));
        if (p != ((void *) 0) && *p == '\000') p = ((void *) 0);
        if (p != ((void *) 0)) {
            p = vim_version_dir (p);
            if (p != ((void *) 0)) *mustfree = 1;
            else p = (char_u *) getenv ((char *) ((char_u *) "VIM"));
        }
    }
    if (p == ((void *) 0)) {
        if (p_hf != ((void *) 0) && vim_strchr (p_hf, '$') == ((void *) 0)) p = p_hf;
        else p = exe_name;
        if (p != ((void *) 0)) {
            pend = gettail (p);
            if (p == p_hf) pend = remove_tail (p, pend, (char_u *) "doc");
            if (p == exe_name) {
                char_u * pend1;
                char_u * pnew;
                ...
                if (pend1 != pend) {
                    ...
                    if (pnew != ((void *) 0)) {
                        ((__builtin_object_size ((char *) (pnew), 0) != (size_t) - 1) ? __builtin___strncpy_chk ((char *) (pnew), (char *) (p), (size_t) ((
                          pend1 - p)), __builtin_object_size ((char *) (pnew), 1 > 1)) : __inline_strncpy_chk ((char *) (pnew), (char *) (p), (size_t) ((pend1 -
                          p))));
                        ((__builtin_object_size ((char *) (pnew + (pend1 - p)), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (pnew + (pend1 - p)), (
                          char *) ("Resources/vim"), __builtin_object_size ((char *) (pnew + (pend1 - p)), 1 > 1)) : __inline_strcpy_chk ((char *) (pnew + (
                          pend1 - p)), (char *) ("Resources/vim")));
                        p = pnew;
                        pend = p + strlen ((char *) (p));
                    }
                }
            }
            if (p == exe_name) pend = remove_tail (p, pend, (char_u *) "src");
            if (!vimruntime) {
                pend = remove_tail (p, pend, (char_u *) "runtime");
                pend = remove_tail (p, pend, (char_u *) "vim73");
            }
            if (pend > p && vim_ispathsep (*((pend)-1))) --pend;
            if (p == exe_name || p == p_hf) p = vim_strnsave (p, (int) (pend - p));
            if (p != ((void *) 0) && !mch_isdir(p)) {
                vim_free (p);
                p = ((void *) 0);
            }
            else {
                if (vimruntime && (pend = vim_version_dir (p)) != ((void *) 0)) {
                    vim_free (p);
                    p = pend;
                }
                * mustfree = 1;
    ...
    if (p == ((void *) 0)) {
        if (vimruntime && *default_vimruntime_dir != '\000') {
            ...
            * mustfree = 0;
            ...
            if (vimruntime && (p = vim_version_dir (default_vim_dir)) != ((void *) 0)) *mustfree = 1;
            else {
                p = default_vim_dir;
                * mustfree = 0;
            }
...
{
    char_u * p;
    if (vimdir == ((void *) 0) || *vimdir == '\000') return ((void *) 0);
    p = concat_fnames (vimdir, (char_u *) "vim73", 1);
    if (p != ((void *) 0) && mch_isdir (p)) return p;
    vim_free (p);
    ...
    char_u * name;
{
    ...
    if (newend >= p && strncasecmp ((char *) (((newend))), (char *) (((name))), (size_t) (((len - 1)))) == 0 && (newend == p || vim_ispathsep (*((newend)-1))))
      return newend;
    return pend;
}
    ...
    int one;
{
    ...
    if (buf != ((void *) 0) && buf -> b_help) {
        ((__builtin_object_size ((char *) (dst), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (dst), (char *) (gettail (src)), __builtin_object_size (
          (char *) (dst), 1 > 1)) : __inline_strcpy_chk ((char *) (dst), (char *) (gettail (src))));
        return;
    }
    if (homedir != ((void *) 0)) dirlen = strlen ((char *) (homedir));
    homedir_env = (char_u *) getenv ((char *) ((char_u *) "HOME"));
    if (homedir_env != ((void *) 0) && *homedir_env == '\000') homedir_env = ((void *) 0);
    if (homedir_env != ((void *) 0)) envlen = strlen ((char *) (homedir_env));
    if (!one) src = skipwhite (src);
    while (*src && dstlen > 0) {
        p = homedir;
        len = dirlen;
        for (;;) {
            if (len && strncasecmp ((char *) (((src))), (char *) (((p))), (size_t) (((len)))) == 0 && (vim_ispathsep (src [len]) || (!one && (src [len] == ','
              || src [len] == ' ')) || src [len] == '\000')) {
                src += len;
                if (--dstlen > 0) *dst++ = '~';
                if (!vim_ispathsep(src[0]) && --dstlen > 0) *dst++ = '/';
                break;
            }
            if (p == homedir_env) break;
            p = homedir_env;
            len = envlen;
        }
        while (*src && (one || (*src != ',' && *src != ' ')) && --dstlen > 0) *dst++ = *src++;
        while ((*src == ' ' || *src == ',') && --dstlen > 0) *dst++ = *src++;
    }
    * dst = '\000';
}
    ...
    char_u * src;
{
    ...
    unsigned len;
    len = 3;
    if (src != ((void *) 0)) len += (unsigned) strlen ((char *) (src));
    dst = alloc (len);
    if (dst != ((void *) 0)) home_replace (buf, src, dst, len, 1);
    return dst;
}
    ...
    int checkname;
{
    ...
    if (r1 != 0 && r2 != 0) {
        if (checkname) {
            if (strcasecmp ((char *) (((exp1))), (char *) (((s2)))) == 0) return 7;
            r1 = vim_FullName (exp1, full1, 1024, 0);
            r2 = vim_FullName (s2, full2, 1024, 0);
            if (r1 == 1 && r2 == 1 && strcasecmp ((char *) (((full1))), (char *) (((full2)))) == 0) return 7;
        }
        return 4;
    }
    if (r1 != 0 || r2 != 0) return 6;
...
{
    char_u * p1, * p2;
    if (fname == ((void *) 0)) return (char_u *) "";
    for (p1 = p2 = fname; *p2;) {
        if (vim_ispathsep (*p2)) p1 = p2 + 1;
        ++p2;
    }
    return p1;
}
    ...
    char_u * fname;
{
    ...
    for (p = fname; *p != '\000';) {
        if (vim_ispathsep (*p)) {
            ...
            if (!look_for_sep) dir_end = next_dir_end;
            look_for_sep = 1;
        }
        ++p;
    }
...
char_u * getnextcomp (fname)
    char_u * fname;
{
    while (*fname && !vim_ispathsep(*fname)) ++fname;
    if (*fname) ++fname;
    return fname;
}
    ...
    char_u * path;
{
    char_u * retval;
    retval = path;
    while (vim_ispathsep (*retval)) ++retval;
    return retval;
}
    ...
    int sep;
{
    ...
    if (dest != ((void *) 0)) {
        ((__builtin_object_size ((char *) (dest), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (dest), (char *) (fname1), __builtin_object_size ((
          char *) (dest), 1 > 1)) : __inline_strcpy_chk ((char *) (dest), (char *) (fname1)));
        if (sep) add_pathsep (dest);
        ((__builtin_object_size ((char *) (dest), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (dest), (char *) (fname2), __builtin_object_size ((
          char *) (dest), 1 > 1)) : __inline_strcat_chk ((char *) (dest), (char *) (fname2)));
    }
    return dest;
}
    ...
    char_u * str2;
{
    ...
    dest = alloc ((unsigned) (l + strlen ((char *) (str2)) + 1L));
    if (dest != ((void *) 0)) {
        ((__builtin_object_size ((char *) (dest), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (dest), (char *) (str1), __builtin_object_size ((char *
          ) (dest), 1 > 1)) : __inline_strcpy_chk ((char *) (dest), (char *) (str1)));
        ((__builtin_object_size ((char *) (dest + l), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (dest + l), (char *) (str2), __builtin_object_size
          ((char *) (dest + l), 1 > 1)) : __inline_strcpy_chk ((char *) (dest + l), (char *) (str2)));
    }
    return dest;
}
...
void add_pathsep (p)
    char_u * p;
{
    if (*p != '\000' && !vim_ispathsep(*((p+strlen((char*)(p)))-1))) ((__builtin_object_size ((char *) (p), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char
      *) (p), (char *) ("/"), __builtin_object_size ((char *) (p), 1 > 1)) : __inline_strcat_chk ((char *) (p), (char *) ("/")));
}
...
{
    char_u * buf;
    char_u * new_fname = ((void *) 0);
    if (fname == ((void *) 0)) return ((void *) 0);
    buf = alloc ((unsigned) 1024);
    if (buf != ((void *) 0)) {
        if (vim_FullName (fname, buf, 1024, force) != 0) new_fname = vim_strsave (buf);
...
pos_T * find_start_comment (ind_maxcomment)
    int ind_maxcomment;
{
    pos_T * pos;
    char_u * line;
    ...
    for (;;) {
        ...
        for (p = line; *p && (colnr_T) (p - line) < pos -> col; ++p) p = skip_string (p);
        if ((colnr_T) (p - line) <= pos -> col) break;
...
void do_c_expr_indent () {
    if (*curbuf->b_p_inde != '\000') fixthisline (get_expr_indent);
    else fixthisline (get_c_indent);
}
...
static int cin_isfuncdecl (char_u * *, linenr_T);
...
static int cin_iswhileofdo_end (int terminated, int ind_maxparen, int ind_maxcomment);
static int cin_isbreak (char_u *);
static int cin_is_cpp_baseclass (colnr_T * col);
static int get_baseclass_amount (int col, int ind_maxparen, int ind_maxcomment, int ind_cpp_baseclass);
...
static int corr_ind_maxparen (int ind_maxparen, pos_T * startpos);
...
static pos_T * find_line_comment () {
    static pos_T pos;
    char_u * line;
    char_u * p;
    pos = curwin -> w_cursor;
...
static int cin_islabel_skip (s)
    char_u * * s;
{
    if (!vim_isIDc(**s)) return 0;
    while (vim_isIDc (**s)) (*s) ++;
    * s = cin_skipcomment (*s);
    return (**s == ':' && *++*s != ':');
}

int cin_islabel (ind_maxcomment)
    int ind_maxcomment;
{
    char_u * s;
    s = cin_skipcomment (ml_get_curline ());
    if (cin_isdefault (s)) return 0;
    if (cin_isscopedecl (s)) return 0;
    if (cin_islabel_skip (&s)) {
        ...
        while (curwin -> w_cursor.lnum > 1) {
            --curwin->w_cursor.lnum;
            curwin -> w_cursor.col = 0;
            if ((trypos = find_start_comment (ind_maxcomment)) != ((void *) 0)) curwin -> w_cursor = *trypos;
            line = ml_get_curline ();
            if (cin_ispreproc (line)) continue;
            if (*(line=cin_skipcomment(line)) == '\000') continue;
            curwin -> w_cursor = cursor_save;
            if (cin_isterminated (line, 1, 0) || cin_isscopedecl (line) || cin_iscase (line, 1) || (cin_islabel_skip (&line) && cin_nocode (line))) return 1;
            return 0;
        }
        curwin -> w_cursor = cursor_save;
        return 1;
...
static int cin_isinit (void) {
    char_u * s;
    s = cin_skipcomment (ml_get_curline ());
    if (strncmp ((char *) (s), (char *) ("typedef"), (size_t) (7)) == 0 && !vim_isIDc(s[7])) s = cin_skipcomment (s + 7);
    if (strncmp ((char *) (s), (char *) ("enum"), (size_t) (4)) == 0 && !vim_isIDc(s[4])) return 1;
    if (cin_ends_in (s, (char_u *) "=", (char_u *) "{")) return 1;
    return 0;
}
...
static int cin_isdefault (s)
    char_u * s;
{
    return (strncmp ((char *) (s), (char *) ("default"), (size_t) (7)) == 0 && *(s=cin_skipcomment(s+7)) == ':' && s [1] != ':');
}
    ...
    char_u * s;
{
    int i;
    s = cin_skipcomment (s);
    if (strncmp ((char *) (s), (char *) ("public"), (size_t) (6)) == 0) i = 6;
    else if (strncmp ((char *) (s), (char *) ("protected"), (size_t) (9)) == 0) i = 9;
    else if (strncmp ((char *) (s), (char *) ("private"), (size_t) (7)) == 0) i = 7;
    else return 0;
    ...
    char_u * l;
{
    for (; *l; ++l) {
        if (*l == ':') {
            if (l [1] == ':') ++l;
            else if (!cin_iscase(l+1,0)) break;
        }
        else if (*l == '\'' && l [1] && l [2] == '\'') l += 2;
    }
    if (*l == '\000') return ((void *) 0);
    l = cin_skipcomment (l + 1);
    if (*l == '\000') return ((void *) 0);
    return l;
}

static int get_indent_nolabel (lnum)
    linenr_T lnum;
{
    char_u * l;
    pos_T fp;
...
static int skip_label (lnum, pp, ind_maxcomment)
    linenr_T lnum;
    char_u * * pp;
    int ind_maxcomment;
{
    ...
    l = ml_get_curline ();
    if (cin_iscase (l, 0) || cin_isscopedecl (l) || cin_islabel (ind_maxcomment)) {
        amount = get_indent_nolabel (lnum);
        l = after_label (ml_get_curline ());
        if (l == ((void *) 0)) l = ml_get_curline ();
    }
    else {
        amount = get_indent ();
        l = ml_get_curline ();
...
static int cin_first_id_amount () {
    ...
    if (len == 6 && strncmp ((char *) (p), (char *) ("struct"), (size_t) (6)) == 0) p = skipwhite (p + 6);
    else if (len == 4 && strncmp ((char *) (p), (char *) ("enum"), (size_t) (4)) == 0) p = skipwhite (p + 4);
    else if ((len == 8 && strncmp ((char *) (p), (char *) ("unsigned"), (size_t) (8)) == 0) || (len == 6 && strncmp ((char *) (p), (char *) ("signed"), (size_t
      ) (6)) == 0)) {
        s = skipwhite (p + len);
        if ((strncmp ((char *) (s), (char *) ("int"), (size_t) (3)) == 0 && ((s [3]) == ' ' || (s [3]) == '\t')) || (strncmp ((char *) (s), (char *) ("long"), (
          size_t) (4)) == 0 && ((s [4]) == ' ' || (s [4]) == '\t')) || (strncmp ((char *) (s), (char *) ("short"), (size_t) (5)) == 0 && ((s [5]) == ' ' || (s [
          5]) == '\t')) || (strncmp ((char *) (s), (char *) ("char"), (size_t) (4)) == 0 && ((s [4]) == ' ' || (s [4]) == '\t'))) p = s;
    }
    for (len = 0; vim_isIDc (p [len]); ++len);
    if (len == 0 || !((p[len])==' '||(p[len])=='\t') || cin_nocode (p)) return 0;
    p = skipwhite (p + len);
    ...
    linenr_T lnum;
{
    ...
    if (lnum > 1) {
        line = ml_get (lnum - 1);
        if (*line != '\000' && line [strlen ((char *) (line)) - 1] == '\\') return -1;
    }
    line = s = ml_get (lnum);
    while (*s != '\000' && vim_strchr ((char_u *) "=;{}\"'", *s) == ((void *) 0)) {
        if (cin_iscomment (s)) s = cin_skipcomment (s);
        else ++s;
    }
    if (*s != '=') return 0;
    s = skipwhite (s + 1);
    if (cin_nocode (s)) return 0;
    if (*s == '"') ++s;
    fp.lnum = lnum;
    ...
    int incl_comma;
{
    ...
    s = cin_skipcomment (s);
    if (*s == '{' || (*s == '}' && !cin_iselse(s))) found_start = *s;
    while (*s) {
        s = skip_string (cin_skipcomment (s));
        if ((*s == ';' || (incl_open && *s == '{') || *s == '}' || (incl_comma && *s == ',')) && cin_nocode (s + 1)) return *s;
        if (*s) s ++;
    }
    return found_start;
}

static int cin_isfuncdecl (sp, first_lnum)
    char_u * * sp;
    linenr_T first_lnum;
{
    char_u * s;
    ...
    while (*s && *s != ';' && *s != '\'' && *s != '"') {
        if (*s == ')' && cin_nocode (s + 1)) {
            lnum = first_lnum - 1;
            s = ml_get (lnum);
            if (*s == '\000' || s [strlen ((char *) (s)) - 1] != '\\') retval = 1;
            goto done;
        }
        if (*s == ',' && cin_nocode (s + 1)) {
        ...
        else if (cin_iscomment (s)) s = cin_skipcomment (s);
        else ++s;
    }
    done : if (lnum != first_lnum && sp != ((void *) 0)) *sp = ml_get (first_lnum);
    return retval;
}
...
static int cin_iswhileofdo (p, lnum, ind_maxparen)
    char_u * p;
    linenr_T lnum;
    int ind_maxparen;
{
    ...
    if (strncmp ((char *) (p), (char *) ("while"), (size_t) (5)) == 0 && !vim_isIDc(p[5])) {
        ...
        if ((trypos = findmatchlimit (((void *) 0), 0, 0, ind_maxparen)) != ((void *) 0) && *cin_skipcomment(ml_get_pos(trypos)+1) == ';') retval = 1;
        curwin -> w_cursor = cursor_save;
    }
    return retval;
}

static int cin_iswhileofdo_end (terminated, ind_maxparen, ind_maxcomment)
    int terminated;
    int ind_maxparen;
    int ind_maxcomment;
{
        ...
        if (*p == ')') {
            ...
            if (*s == ';' && cin_nocode (s + 1)) {
                ...
                if (trypos != ((void *) 0)) {
                    ...
                    if (*s == '}') s = cin_skipcomment (s + 1);
                    if (strncmp ((char *) (s), (char *) ("while"), (size_t) (5)) == 0 && !vim_isIDc(s[5])) {
    ...
    colnr_T * col;
{
    ...
    * col = 0;
    ...
    for (;;) {
        if (*s == '\000') {
            if (lnum == curwin -> w_cursor.lnum) break;
            line = ml_get (++lnum);
            s = cin_skipcomment (line);
            if (*s == '\000') continue;
        }
        if (s [0] == ':') {
            ...
            else s = cin_skipcomment (s + 1);
        }
        else if ((strncmp ((char *) (s), (char *) ("class"), (size_t) (5)) == 0 && !vim_isIDc(s[5])) || (strncmp ((char *) (s), (char *) ("struct"), (size_t) (
          6)) == 0 && !vim_isIDc(s[6]))) {
            class_or_struct = 1;
            lookfor_ctor_init = 0;
            if (*s == 'c') s = cin_skipcomment (s + 5);
            else s = cin_skipcomment (s + 6);
        }
        else {
            if (s [0] == '{' || s [0] == '}' || s [0] == ';') {
                cpp_base_class = lookfor_ctor_init = class_or_struct = 0;
                ...
                lookfor_ctor_init = 0;
                if (cpp_base_class) *col = (colnr_T) (s - line);
            }
            if (lnum == curwin -> w_cursor.lnum && *s == ',' && cin_nocode (s + 1)) *col = 0;
            s = cin_skipcomment (s + 1);
        }
    }
...
static int get_baseclass_amount (col, ind_maxparen, ind_maxcomment, ind_cpp_baseclass)
    int col;
    int ind_maxparen;
    int ind_maxcomment;
    int ind_cpp_baseclass;
{
    ...
    if (col == 0) {
        amount = get_indent ();
        if (find_last_paren (ml_get_curline (), '(', ')') && (trypos = find_match_paren (ind_maxparen, ind_maxcomment)) != ((void *) 0)) amount =
          get_indent_lnum (trypos -> lnum);
        if (!cin_ends_in(ml_get_curline(),(char_u*)",",((void*)0))) amount += ind_cpp_baseclass;
    }
    else {
    ...
    if (amount < ind_cpp_baseclass) amount = ind_cpp_baseclass;
    return amount;
}
    ...
    char_u * ignore;
{
    ...
    while (*p != '\000') {
        ...
        if (strncmp ((char *) (p), (char *) (find), (size_t) (len)) == 0) {
            r = skipwhite (p + len);
            if (ignore != ((void *) 0) && strncmp ((char *) (r), (char *) (ignore), (size_t) (strlen ((char *) (ignore)))) == 0) r = skipwhite (r + strlen ((
              char *) (ignore)));
            if (cin_nocode (r)) return 1;
        }
        if (*p != '\000') ++p;
    }
    return 0;
    ...
    pos_T * trypos;
{
    ...
    while (*p && (colnr_T) (p - line) < trypos -> col) {
        if (cin_iscomment (p)) p = cin_skipcomment (p);
        else {
            p = skip_string (p);
            ++p;
        }
...
static pos_T * find_start_brace (ind_maxcomment)
    int ind_maxcomment;
{
    pos_T cursor_save;
    pos_T * trypos;
    ...
    while ((trypos = findmatchlimit (((void *) 0), '{', 0x04, 0)) != ((void *) 0)) {
        ...
        pos = ((void *) 0);
        if ((colnr_T) cin_skip2pos (trypos) == trypos -> col && (pos = find_start_comment (ind_maxcomment)) == ((void *) 0)) break;
        if (pos != ((void *) 0)) curwin -> w_cursor.lnum = pos -> lnum;
    }
    curwin -> w_cursor = cursor_save;
    return trypos;
}

static pos_T * find_match_paren (ind_maxparen, ind_maxcomment)
    int ind_maxparen;
    int ind_maxcomment;
{
    pos_T cursor_save;
    ...
    static pos_T pos_copy;
    cursor_save = curwin -> w_cursor;
    if ((trypos = findmatchlimit (((void *) 0), '(', 0, ind_maxparen)) != ((void *) 0)) {
        if ((colnr_T) cin_skip2pos (trypos) > trypos -> col) trypos = ((void *) 0);
        else {
            pos_copy = *trypos;
            trypos = &pos_copy;
            curwin -> w_cursor = *trypos;
            if (find_start_comment (ind_maxcomment) != ((void *) 0)) trypos = ((void *) 0);
        }
    }
    curwin -> w_cursor = cursor_save;
    return trypos;
...
static int corr_ind_maxparen (ind_maxparen, startpos)
    int ind_maxparen;
    pos_T * startpos;
{
    long n = (long) startpos -> lnum - (long) curwin -> w_cursor.lnum;
    if (n > 0 && n < ind_maxparen / 2) return ind_maxparen - (int) n;
    return ind_maxparen;
}
...
int get_c_indent () {
    ...
    for (options = curbuf -> b_p_cino; *options;) {
        ...
        if (*options == '.') {
            ...
            while (((*options) >= '0' && (*options) <= '9')) {
                ++options;
                if (divider) divider *= 10;
                else divider = 10;
            }
        }
        if (*options == 's') {
            if (n == 0 && fraction == 0) n = curbuf -> b_p_sw;
            else {
                n *= curbuf -> b_p_sw;
                if (divider) n += (curbuf -> b_p_sw * fraction + divider / 2) / divider;
            }
            ++options;
        }
        if (l [1] == '-') n = -n;
        ...
        case 'u' :
            ...
            break;
        case 'U' :
            ind_unclosed_noignore = n;
            break;
        case 'W' :
            ind_unclosed_wrapped = n;
            break;
        case 'w' :
            ind_unclosed_whiteok = n;
            break;
        case 'm' :
            ind_matching_paren = n;
            break;
        case 'M' :
            ind_paren_prev = n;
            break;
    ...
    if (cur_curpos.lnum == 1) return 0;
    linecopy = vim_strsave (ml_get (cur_curpos.lnum));
    if (linecopy == ((void *) 0)) return 0;
    if ((State & 0x10) && curwin -> w_cursor.col < (colnr_T) strlen ((char *) (linecopy)) && linecopy [curwin -> w_cursor.col] == ')') linecopy [curwin ->
      w_cursor.col] = '\000';
    theline = skipwhite (linecopy);
    curwin -> w_cursor.col = 0;
    original_line_islabel = cin_islabel (ind_maxcomment);
    if (*theline == '#' && (*linecopy == '#' || in_cinkeys ('#', ' ', 1))) {
    ...
    else if (cin_islinecomment (theline) && (trypos = find_line_comment ()) != ((void *) 0)) {
        getvcol (curwin, trypos, &col, ((void *) 0), ((void *) 0));
        amount = col;
    }
    else if (!cin_iscomment(theline) && (trypos = find_start_comment (ind_maxcomment)) != ((void *) 0)) {
        ...
        while (*p != '\000') {
            ...
            int what = 0;
            while (*p != '\000' && *p != ':') {
                if (*p == 's' || *p == 'e' || *p == 'm') what = *p++;
                else if (*p == 'l' || *p == 'r') align = *p++;
                else if (((*p) >= '0' && (*p) <= '9') || *p == '-') off = getdigits (&p);
                else ++p;
            }
            if (*p == ':') ++p;
            ...
            if (what == 's') {
                ((__builtin_object_size ((char *) (lead_start), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (lead_start), (char *) (lead_end),
                  __builtin_object_size ((char *) (lead_start), 1 > 1)) : __inline_strcpy_chk ((char *) (lead_start), (char *) (lead_end)));
                lead_start_len = (int) strlen ((char *) (lead_start));
                start_off = off;
                start_align = align;
            }
            else if (what == 'm') {
                ((__builtin_object_size ((char *) (lead_middle), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (lead_middle), (char *) (lead_end),
                  __builtin_object_size ((char *) (lead_middle), 1 > 1)) : __inline_strcpy_chk ((char *) (lead_middle), (char *) (lead_end)));
                lead_middle_len = (int) strlen ((char *) (lead_middle));
            }
            else if (what == 'e') {
                if (strncmp ((char *) (theline), (char *) (lead_middle), (size_t) (lead_middle_len)) == 0 && strncmp ((char *) (theline), (char *) (lead_end), (
                  size_t) (strlen ((char *) (lead_end)))) != 0) {
                    done = 1;
                    if (curwin -> w_cursor.lnum > 1) {
                        look = skipwhite (ml_get (curwin -> w_cursor.lnum - 1));
                        if (strncmp ((char *) (look), (char *) (lead_start), (size_t) (lead_start_len)) == 0) amount = get_indent_lnum (curwin -> w_cursor.lnum
                          - 1);
                        else if (strncmp ((char *) (look), (char *) (lead_middle), (size_t) (lead_middle_len)) == 0) {
                            amount = get_indent_lnum (curwin -> w_cursor.lnum - 1);
                            break;
                        }
                        else if (strncmp ((char *) (ml_get (trypos -> lnum) + trypos -> col), (char *) (lead_start), (size_t) (lead_start_len)) != 0) continue;
                    }
                    if (start_off != 0) amount += start_off;
                    else if (start_align == 'r') amount += vim_strsize (lead_start) - vim_strsize (lead_middle);
                    break;
                }
                if (strncmp ((char *) (theline), (char *) (lead_middle), (size_t) (lead_middle_len)) != 0 && strncmp ((char *) (theline), (char *) (lead_end), (
                  size_t) (strlen ((char *) (lead_end)))) == 0) {
                    amount = get_indent_lnum (curwin -> w_cursor.lnum - 1);
                    if (off != 0) amount += off;
                    else if (align == 'r') amount += vim_strsize (lead_start) - vim_strsize (lead_middle);
                    done = 1;
                    break;
                }
            }
        ...
        if (done);
        else if (theline [0] == '*') amount += 1;
        else {
            amount = -1;
            for (lnum = cur_curpos.lnum - 1; lnum > trypos -> lnum; --lnum) {
                if (linewhite (lnum)) continue;
            ...
            if (amount == -1) {
                if (!ind_in_comment2) {
                    ...
                    if (*look != '\000') trypos -> col = (colnr_T) (skipwhite (look) - start);
                }
                getvcol (curwin, trypos, &col, ((void *) 0), ((void *) 0));
                amount = col;
                if (ind_in_comment2 || *look == '\000') amount += ind_in_comment;
            }
        }
    }
    else if (((trypos = find_match_paren (ind_maxparen, ind_maxcomment)) != ((void *) 0) && ind_java == 0) || (tryposBrace = find_start_brace (ind_maxcomment))
      != ((void *) 0) || trypos != ((void *) 0)) {
        if (trypos != ((void *) 0) && tryposBrace != ((void *) 0)) {
            if (trypos -> lnum != tryposBrace -> lnum ? trypos -> lnum < tryposBrace -> lnum : trypos -> col < tryposBrace -> col) trypos = ((void *) 0);
            else tryposBrace = ((void *) 0);
        }
        if (trypos != ((void *) 0)) {
            if (theline [0] == ')' && ind_paren_prev) {
                ...
                for (lnum = cur_curpos.lnum - 1; lnum > our_paren_pos.lnum; --lnum) {
                    ...
                    if (cin_nocode (l)) continue;
                    if (cin_ispreproc_cont (&l, &lnum)) continue;
                    curwin -> w_cursor.lnum = lnum;
                    if ((trypos = find_start_comment (ind_maxcomment)) != ((void *) 0)) {
                        lnum = trypos -> lnum + 1;
                        continue;
                    }
                    if ((trypos = find_match_paren (corr_ind_maxparen (ind_maxparen, &cur_curpos), ind_maxcomment)) != ((void *) 0) && trypos -> lnum ==
                      our_paren_pos.lnum && trypos -> col == our_paren_pos.col) {
                        amount = get_indent_lnum (lnum);
                        if (theline [0] == ')') {
                            if (our_paren_pos.lnum != lnum && cur_amount > amount) cur_amount = amount;
                            amount = -1;
                        }
                        break;
                    }
            ...
            if (amount == -1) {
                ...
                if (*look == '(') {
                    ...
                    if ((trypos = findmatchlimit (((void *) 0), ')', 0, ind_maxparen)) != ((void *) 0) && trypos -> lnum == our_paren_pos.lnum && trypos -> col
                      < our_paren_pos.col) ignore_paren_col = trypos -> col + 1;
                    curwin -> w_cursor.lnum = save_lnum;
                    look = ml_get (our_paren_pos.lnum) + look_col;
                }
                if (theline [0] == ')' || ind_unclosed == 0 || (!ind_unclosed_noignore && *look == '(' && ignore_paren_col == 0)) {
                    if (theline [0] != ')') {
                        cur_amount = (0x7fffffffL);
                        l = ml_get (our_paren_pos.lnum);
                        if (ind_unclosed_wrapped && cin_ends_in (l, (char_u *) "(", ((void *) 0))) {
                            n = 1;
                        ...
                        else if (ind_unclosed_whiteok) our_paren_pos.col ++;
                        else {
                            col = our_paren_pos.col + 1;
                            while (((l [col]) == ' ' || (l [col]) == '\t')) col ++;
                            if (l [col] != '\000') our_paren_pos.col = col;
                            else our_paren_pos.col ++;
                        }
                    }
                    if (our_paren_pos.col > 0) {
                        getvcol (curwin, &our_paren_pos, &col, ((void *) 0), ((void *) 0));
                        if (cur_amount > (int) col) cur_amount = col;
                    }
                }
                if (theline [0] == ')' && ind_matching_paren) {
                }
                else if (ind_unclosed == 0 || (!ind_unclosed_noignore && *look == '(' && ignore_paren_col == 0)) {
                    if (cur_amount != (0x7fffffffL)) amount = cur_amount;
                }
                else {
                    ...
                    if (col == (0x7fffffffL)) amount += ind_unclosed;
                    else {
                        curwin -> w_cursor.lnum = our_paren_pos.lnum;
                        curwin -> w_cursor.col = col;
                        if ((trypos = find_match_paren (ind_maxparen, ind_maxcomment)) != ((void *) 0)) amount += ind_unclosed2;
                        else amount += ind_unclosed;
                    }
                    if (cur_amount < amount) amount = cur_amount;
                }
            }
            if (cin_iscomment (theline)) amount += ind_comment;
        }
        else {
            trypos = tryposBrace;
            ...
            if (*look == '{') {
                getvcol (curwin, trypos, &col, ((void *) 0), ((void *) 0));
                amount = col;
                if (*start == '{') start_brace = 1;
                else start_brace = 2;
            }
            else {
                curwin -> w_cursor.lnum = ourscope;
                lnum = ourscope;
                if (find_last_paren (start, '(', ')') && (trypos = find_match_paren (ind_maxparen, ind_maxcomment)) != ((void *) 0)) lnum = trypos -> lnum;
                if ((ind_keep_case_label && cin_iscase (skipwhite (ml_get_curline ()), 0))) amount = get_indent ();
                else amount = skip_label (lnum, &l, ind_maxcomment);
                start_brace = 3;
            }
            if (theline [0] == '}') {
                ...
                lookfor = 0;
                if (cin_iselse (theline)) lookfor = 1;
                else if (cin_iswhileofdo (theline, cur_curpos.lnum, ind_maxparen)) lookfor = 2;
                if (lookfor != 0) {
                    curwin -> w_cursor.lnum = cur_curpos.lnum;
                    if (find_match (lookfor, ourscope, ind_maxparen, ind_maxcomment) == 1) {
                        amount = get_indent ();
                        goto theend;
                    }
                }
                if (start_brace == 1) {
                    ...
                    if (start_brace == 3) amount += ind_open_imag;
                    else {
                ...
                if (cin_iscase (theline, 0)) {
                    ...
                    if (ind_case_break && cin_isbreak (theline)) lookfor_break = 1;
                    lookfor = 0;
                    amount += ind_level;
                }
                scope_amount = amount;
                ...
                for (;;) {
                    ...
                    if (curwin -> w_cursor.lnum <= ourscope) {
                        if (lookfor == 10) {
                            if (curwin -> w_cursor.lnum == 0 || curwin -> w_cursor.lnum < ourscope - ind_maxparen) {
                                if (cont_amount > 0) amount = cont_amount;
                                else if (!ind_js) amount += ind_continuation;
                                break;
                            }
                            l = ml_get_curline ();
                            ...
                            if (cin_ispreproc_cont (&l, &curwin->w_cursor.lnum)) continue;
                            if (cin_nocode (l)) continue;
                            terminated = cin_isterminated (l, 0, 1);
                            if (start_brace != 1 || !cin_isfuncdecl(&l, curwin -> w_cursor.lnum)) {
                                if (terminated == ',') break;
                                if (terminated != ';' && cin_isinit ()) break;
                                if (terminated == 0 || terminated == '{') continue;
                            }
                            if (terminated != ';') {
                                trypos = ((void *) 0);
                                if (find_last_paren (l, '(', ')')) trypos = find_match_paren (ind_maxparen, ind_maxcomment);
                                if (trypos == ((void *) 0) && find_last_paren (l, '{', '}')) trypos = find_start_brace (ind_maxcomment);
                                if (trypos != ((void *) 0)) {
                                    curwin -> w_cursor.lnum = trypos -> lnum + 1;
                                    curwin -> w_cursor.col = 0;
                                    continue;
                            ...
                            if (cont_amount > 0) amount = cont_amount;
                            else amount += ind_continuation;
                        }
                        else if (lookfor == 6) {
                            if (cont_amount > 0) amount = cont_amount;
                            else amount += ind_continuation;
                        }
                        else if (lookfor != 5 && lookfor != 9) {
                            amount = scope_amount;
                            if (theline [0] == '{') amount += ind_open_extra;
                        }
                        break;
                    }
                    if ((trypos = find_start_comment (ind_maxcomment)) != ((void *) 0)) {
                    ...
                    if (iscase || cin_isscopedecl (l)) {
                        if (lookfor == 9) break;
                        if (whilelevel > 0) continue;
                        if (lookfor == 6 || lookfor == 10) {
                            if (cont_amount > 0) amount = cont_amount;
                            else amount += ind_continuation;
                            break;
                        }
                        if ((iscase && lookfor == 3) || (iscase && lookfor_break) || (!iscase && lookfor == 7)) {
                            if ((trypos = find_start_brace (ind_maxcomment)) == ((void *) 0) || trypos -> lnum == ourscope) {
                                amount = get_indent ();
                                break;
                            }
                            continue;
                        ...
                        if (n) {
                            ...
                            if (l != ((void *) 0) && cin_is_cinword (l)) {
                                if (theline [0] == '{') amount += ind_open_extra;
                                else amount += ind_level + ind_no_brace;
                            }
                            break;
                        }
                        scope_amount = get_indent () + (iscase ? ind_case_code : ind_scopedecl_code);
                        lookfor = ind_case_break ? 8 : 4;
                        continue;
                    }
                    if (lookfor == 3 || lookfor == 7) {
                        if (find_last_paren (l, '{', '}') && (trypos = find_start_brace (ind_maxcomment)) != ((void *) 0)) {
                            curwin -> w_cursor.lnum = trypos -> lnum + 1;
                            curwin -> w_cursor.col = 0;
                        }
                        continue;
                    ...
                    if (!ind_js && cin_islabel (ind_maxcomment)) {
                        l = after_label (ml_get_curline ());
                        if (l == ((void *) 0) || cin_nocode (l)) continue;
                    }
                    l = ml_get_curline ();
                    if (cin_ispreproc_cont (&l, &curwin->w_cursor.lnum) || cin_nocode (l)) continue;
                    n = 0;
                    if (lookfor != 5 && ind_cpp_baseclass > 0) {
                    ...
                    if (n) {
                        ...
                        else amount = get_baseclass_amount (col, ind_maxparen, ind_maxcomment, ind_cpp_baseclass);
                        break;
                    }
                    else if (lookfor == 9) {
                        if (cin_isterminated (l, 1, 0)) break;
                        else continue;
                    }
                    terminated = cin_isterminated (l, 0, 1);
                    if (terminated == 0 || (lookfor != 6 && terminated == ',')) {
                        (void) find_last_paren (l, '(', ')');
                        trypos = find_match_paren (corr_ind_maxparen (ind_maxparen, &cur_curpos), ind_maxcomment);
                        if (trypos == ((void *) 0) && terminated == ',' && find_last_paren (l, '{', '}')) trypos = find_start_brace (ind_maxcomment);
                        if (trypos != ((void *) 0)) {
                            curwin -> w_cursor = *trypos;
                            l = ml_get_curline ();
                        ...
                        if (terminated == ',') {
                            while (curwin -> w_cursor.lnum > 1) {
                                ...
                                if (*l == '\000' || l [strlen ((char *) (l)) - 1] != '\\') break;
                                --curwin->w_cursor.lnum;
                                curwin -> w_cursor.col = 0;
                            }
                        }
                        if (!ind_js) cur_amount = skip_label (curwin -> w_cursor.lnum, &l, ind_maxcomment);
                        else cur_amount = get_indent ();
                        if (terminated != ',' && lookfor != 5 && theline [0] == '{') {
                            amount = cur_amount;
                            if (*skipwhite(l) != '{') amount += ind_open_extra;
                        ...
                        if (cin_is_cinword (l) || cin_iselse (skipwhite (l))) {
                            ...
                            if (cin_isdo (l)) {
                                if (whilelevel == 0) break;
                                --whilelevel;
                            }
                            if (cin_iselse (l) && whilelevel == 0 && ((trypos = find_start_brace (ind_maxcomment)) == ((void *) 0) || find_match (1, trypos ->
                              lnum, ind_maxparen, ind_maxcomment) == 0)) break;
                        }
                        else {
                            if (lookfor == 6) {
                                if (terminated == ',') amount += ind_continuation;
                                break;
                            }
                            if (lookfor == 10) {
                                if (terminated == ',') {
                                    if (ind_cpp_baseclass == 0) break;
                                    lookfor = 9;
                                    continue;
                                }
                                ...
                                if (lookfor == 0 && terminated == ',') {
                                    ...
                                    if (lookfor == 0 && *l != '\000' && l [strlen ((char *) (l)) - 1] == '\\') cont_amount = cin_get_equal_amount (curwin ->
                                      w_cursor.lnum);
                                    if (lookfor != 5) lookfor = 6;
                                }
                            }
                        }
                    }
                    else if (cin_iswhileofdo_end (terminated, ind_maxparen, ind_maxcomment)) {
                        if (lookfor == 6 || lookfor == 10) {
                            if (cont_amount > 0) amount = cont_amount;
                            else amount += ind_continuation;
                            break;
                        ...
                        if (lookfor == 8 && cin_isbreak (skipwhite (ml_get_curline ()))) {
                            lookfor = 4;
                            continue;
                        }
                        if (whilelevel > 0) {
                        ...
                        if (lookfor == 5) {
                            if (!lookfor_break && whilelevel == 0) break;
                        }
                        else {
                            term_again : l = ml_get_curline ();
                            if (find_last_paren (l, '(', ')') && (trypos = find_match_paren (ind_maxparen, ind_maxcomment)) != ((void *) 0)) {
                                curwin -> w_cursor = *trypos;
                                l = ml_get_curline ();
                                if (cin_iscase (l, 0) || cin_isscopedecl (l)) {
                                    ++curwin->w_cursor.lnum;
                            ...
                            iscase = (ind_keep_case_label && cin_iscase (l, 0));
                            amount = skip_label (curwin -> w_cursor.lnum, &l, ind_maxcomment);
                            if (theline [0] == '{') amount += ind_open_extra;
                            l = skipwhite (l);
                            if (*l == '{') amount -= ind_open_extra;
                            lookfor = iscase ? 4 : 5;
                            if (lookfor == 5 && *l != '}' && cin_iselse (l) && whilelevel == 0) {
                                if ((trypos = find_start_brace (ind_maxcomment)) == ((void *) 0) || find_match (1, trypos -> lnum, ind_maxparen, ind_maxcomment
                                  ) == 0) break;
                                continue;
                            }
                            curwin -> w_cursor.col = 0;
                            if (*cin_skipcomment(l) == '}' && (trypos = find_start_brace (ind_maxcomment)) != ((void *) 0)) {
                                curwin -> w_cursor = *trypos;
                                l = cin_skipcomment (ml_get_curline ());
                                if (*l == '}' || !cin_iselse(l)) goto term_again;
                                ++curwin->w_cursor.lnum;
                                curwin -> w_cursor.col = 0;
                            }
                        }
        ...
        if (cin_iscomment (theline)) amount += ind_comment;
        if (ind_jump_label > 0 && original_line_islabel) amount -= ind_jump_label;
    }
    else {
        if (theline [0] == '{') {
            amount = ind_first_open;
        }
        else if (cur_curpos.lnum < curbuf -> b_ml.ml_line_count && !cin_nocode(theline) && vim_strchr (theline, '{') == ((void *) 0) && vim_strchr (theline, '}'
          ) == ((void *) 0) && !cin_ends_in(theline,(char_u*)":",((void*)0)) && !cin_ends_in(theline,(char_u*)",",((void*)0)) && cin_isfuncdecl (((void *) 0),
          cur_curpos.lnum + 1) && !cin_isterminated(theline,0,1)) {
            ...
            while (curwin -> w_cursor.lnum > 1) {
                ...
                if (n) {
                    amount = get_baseclass_amount (col, ind_maxparen, ind_maxcomment, ind_cpp_baseclass);
                    break;
                }
                if (cin_ispreproc_cont (&l, &curwin->w_cursor.lnum)) continue;
                if (cin_nocode (l)) continue;
                n = 0;
                if (cin_ends_in (l, (char_u *) ",", ((void *) 0)) || (*l != '\000' && (n = l [strlen ((char *) (l)) - 1]) == '\\')) {
                    if (find_last_paren (l, '(', ')') && (trypos = find_match_paren (ind_maxparen, ind_maxcomment)) != ((void *) 0)) curwin -> w_cursor = 
                      *trypos;
                    ...
                    amount = get_indent ();
                    if (amount == 0) amount = cin_first_id_amount ();
                    if (amount == 0) amount = ind_continuation;
                    break;
                }
                if (cin_isfuncdecl (((void *) 0), cur_curpos.lnum)) break;
                l = ml_get_curline ();
                if (*skipwhite(l) == '}') break;
                if (cin_ends_in (l, (char_u *) "};", ((void *) 0))) break;
                ...
                if (cin_ends_in (l, (char_u *) ";", ((void *) 0))) {
                    l = ml_get (curwin -> w_cursor.lnum - 1);
                    if (cin_ends_in (l, (char_u *) ",", ((void *) 0)) || (*l != '\000' && l [strlen ((char *) (l)) - 1] == '\\')) break;
                    l = ml_get_curline ();
                }
                find_last_paren (l, '(', ')');
                if ((trypos = find_match_paren (ind_maxparen, ind_maxcomment)) != ((void *) 0)) curwin -> w_cursor = *trypos;
                amount = get_indent ();
                break;
            }
            if (cin_iscomment (theline)) amount += ind_comment;
            if (cur_curpos.lnum > 1) {
                ...
                if (*l != '\000' && l [strlen ((char *) (l)) - 1] == '\\') {
                    ...
                    if (cur_amount > 0) amount = cur_amount;
                    else if (cur_amount == 0) amount += ind_continuation;
                }
            }
        }
    }
    theend : curwin -> w_cursor = cur_curpos;
    vim_free (linecopy);
    if (amount < 0) return 0;
    return amount;
}

static int find_match (lookfor, ourscope, ind_maxparen, ind_maxcomment)
    int lookfor;
    linenr_T ourscope;
    int ind_maxparen;
    int ind_maxcomment;
{
    ...
    while (curwin -> w_cursor.lnum > ourscope + 1) {
        ...
        if (cin_iselse (look) || cin_isif (look) || cin_isdo (look) || cin_iswhileofdo (look, curwin -> w_cursor.lnum, ind_maxparen)) {
            theirscope = find_start_brace (ind_maxcomment);
            if (theirscope == ((void *) 0)) break;
            if (theirscope -> lnum < ourscope) break;
            if (theirscope -> lnum > ourscope) continue;
            look = cin_skipcomment (ml_get_curline ());
            if (cin_iselse (look)) {
                mightbeif = cin_skipcomment (look + 4);
                if (!cin_isif(mightbeif)) ++elselevel;
                continue;
            }
            ...
            if (cin_isif (look)) {
                elselevel --;
                if (elselevel == 0 && lookfor == 1) whilelevel = 0;
            }
            if (cin_isdo (look)) whilelevel --;
...
int get_expr_indent () {
    ...
    pos = curwin -> w_cursor;
    set_vim_var_nr (9, curwin -> w_cursor.lnum);
    ...
    char_u * p;
{
    ...
    while (*word != '\000') {
        ...
        if (strncmp ((char *) (buf), (char *) (p), (size_t) (len)) == 0 && p [len] == ' ') return 1;
    }
    return 0;
}
...
int get_lisp_indent () {
    ...
    curwin -> w_cursor.col = 0;
    if ((pos = findmatch (((void *) 0), '(')) == ((void *) 0)) pos = findmatch (((void *) 0), '[');
    else {
        paren = *pos;
        pos = findmatch (((void *) 0), '[');
        if (pos == ((void *) 0) || (((pos) -> lnum != (&paren) -> lnum) ? (pos) -> lnum < (&paren) -> lnum : (pos) -> col != (&paren) -> col ? (pos) -> col < (
          &paren) -> col : (pos) -> coladd < (&paren) -> coladd)) pos = &paren;
    }
    if (pos != ((void *) 0)) {
        amount = -1;
        parencount = 0;
        while (--curwin->w_cursor.lnum >= pos -> lnum) {
            if (linewhite (curwin -> w_cursor.lnum)) continue;
            for (that = ml_get_curline (); *that != '\000'; ++that) {
                if (*that == ';') {
                    while (*(that+1) != '\000') ++that;
                    continue;
                }
                if (*that == '\\') {
                    if (*(that+1) != '\000') ++that;
                ...
                if (*that == '(' || *that == '[') ++parencount;
                else if (*that == ')' || *that == ']') --parencount;
            }
            if (parencount == 0) {
                amount = get_indent ();
                break;
        ...
        if (amount == -1) {
            ...
            col = pos -> col;
            that = ml_get_curline ();
            if (vi_lisp && get_indent () == 0) amount = 2;
            else {
                amount = 0;
                while (*that && col) {
                ...
                if (!vi_lisp && (*that == '(' || *that == '[') && lisp_match (that + 1)) amount += 2;
                else {
                    that ++;
                    amount ++;
                    firsttry = amount;
                    ...
                    if (*that && *that != ';') {
                        if (!vi_lisp && *that != '(' && *that != '[') firsttry ++;
                        parencount = 0;
                        quotecount = 0;
                        if (vi_lisp || (*that != '"' && *that != '\'' && *that != '#' && (*that < '0' || *that > '9'))) {
                            while (*that && (!((*that) == ' ' || (*that) == '\t') || quotecount || parencount) && (!((*that == '(' || *that == '[') && 
                              !quotecount && !parencount && vi_lisp))) {
                                if (*that == '"') quotecount = !quotecount;
                                if ((*that == '(' || *that == '[') && !quotecount) ++parencount;
                                if ((*that == ')' || *that == ']') && !quotecount) --parencount;
                                if (*that == '\\' && *(that+1) != '\000') amount += lbr_chartabsize_adv (&that, (colnr_T) amount);
                                amount += lbr_chartabsize_adv (&that, (colnr_T) amount);
                            }
                        }
                        while (((*that) == ' ' || (*that) == '\t')) {
...
void preserve_exit () {
    ...
    for (buf = firstbuf; buf != ((void *) 0); buf = buf -> b_next) {
        if (buf -> b_ml.ml_mfp != ((void *) 0) && buf -> b_ml.ml_mfp -> mf_fname != ((void *) 0)) {
            out_str ((char_u *) (((char *) ("Vim: preserving files...\n"))));
            screen_start ();
            out_flush ();
            ml_sync_all (0, 0);
            break;
    ...
    ml_close_all (0);
    out_str ((char_u *) (((char *) ("Vim: Finished.\n"))));
    getout (1);
}
...
{
    struct stat st;
    if (stat (((char *) fname), (&st))) return 0;
    return 1;
}
    ...
    int flags;
{
    ...
    if (*exp_pat == '%' || *exp_pat == '#' || *exp_pat == '<') {
        ++emsg_off;
        eval_pat = eval_vars (exp_pat, exp_pat, &usedlen, ((void *) 0), &ignored_msg, ((void *) 0));
        --emsg_off;
        if (eval_pat != ((void *) 0)) exp_pat = concat_str (eval_pat, exp_pat + usedlen);
    }
    if (exp_pat != ((void *) 0)) ret = expand_wildcards (1, &exp_pat, num_file, file, flags);
    if (eval_pat != ((void *) 0)) {
        vim_free (exp_pat);
        vim_free (eval_pat);
    }
    ...
    int flags;
{
    ...
    if ((flags & 0x10) || retval == 0) return retval;
    if (*p_wig) {
        char_u * ffname;
        for (i = 0; i < *num_file; ++i) {
            ffname = FullName_save ((*file) [i], 0);
            if (ffname == ((void *) 0)) break;
            if (match_file_list (p_wig, (*file) [i], ffname)) {
                vim_free ((* file) [i]);
                for (j = i; j + 1 < *num_file; ++j) (*file) [j] = (*file) [j + 1];
                --*num_file;
                --i;
            }
            vim_free (ffname);
    ...
    if (*num_file > 1) {
        ...
        for (i = 0; i < *num_file; ++i) {
            if (!match_suffix((*file) [i])) {
                p = (*file) [i];
                for (j = i; j > non_suf_match; --j) (*file) [j] = (*file) [j - 1];
    ...
    char_u * fname;
{
    ...
    for (setsuf = p_su; *setsuf;) {
        ...
        if (setsuflen == 0) {
            ...
            if (fnamelen >= setsuflen && strncasecmp ((char *) (((suf_buf))), (char *) (((fname + fnamelen - setsuflen))), (size_t) ((((size_t) setsuflen)))) ==
              0) break;
            setsuflen = 0;
        }
    }
    ...
    int didstar;
{
    ...
    buf = alloc ((int) strlen ((char *) (path)) + (255 - 5) + 5);
    ...
    path_end = path;
    while (*path_end != '\000') {
        if (path_end >= path + wildoff && rem_backslash (path_end)) *p++ = *path_end++;
        else if (*path_end == '/') {
            if (e != ((void *) 0)) break;
            s = p + 1;
        }
        else if (path_end >= path + wildoff && vim_strchr ((char_u *) "*?[{~$", *path_end) != ((void *) 0)) e = p;
        *p++ = *path_end++;
    }
    e = p;
    * e = '\000';
    for (p = buf + wildoff; p < s; ++p) if (rem_backslash (p)) {
        ((__builtin_object_size ((char *) ((p)), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) ((p)), (char *) ((p + 1)), strlen ((char *) (p + 1)) +
          1, __builtin_object_size ((char *) ((p)), 0)) : __inline_memmove_chk ((char *) ((p)), (char *) ((p + 1)), strlen ((char *) (p + 1)) + 1));
        --e;
        --s;
    }
    for (p = s; p < e; ++p) if (p [0] == '*' && p [1] == '*') starstar = 1;
    starts_with_dot = (*s == '.');
    ...
    if (!didstar && stardepth < 100 && starstar && e - s == 2 && *path_end == '/') {
        ((__builtin_object_size ((char *) (s), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (s), (char *) (path_end + 1), __builtin_object_size ((
          char *) (s), 1 > 1)) : __inline_strcpy_chk ((char *) (s), (char *) (path_end + 1)));
        ++stardepth;
        (void) unix_expandpath (gap, buf, (int) (s - buf), flags, 1);
        --stardepth;
    }
    * s = '\000';
    dirp = opendir (*buf == '\000' ? "." : (char *) buf);
    if (dirp != ((void *) 0)) {
        for (;;) {
            dp = readdir (dirp);
            if (dp == ((void *) 0)) break;
            if ((dp -> d_name [0] != '.' || starts_with_dot) && vim_regexec (&regmatch, (char_u *) dp -> d_name, (colnr_T) 0)) {
                ((__builtin_object_size ((char *) (s), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (s), (char *) (dp -> d_name),
                  __builtin_object_size ((char *) (s), 1 > 1)) : __inline_strcpy_chk ((char *) (s), (char *) (dp -> d_name)));
                len = strlen ((char *) (buf));
                if (starstar && stardepth < 100) {
                    ((__builtin_object_size ((char *) (buf + len), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (buf + len), (char *) ("/**"),
                      __builtin_object_size ((char *) (buf + len), 1 > 1)) : __inline_strcpy_chk ((char *) (buf + len), (char *) ("/**")));
                    ((__builtin_object_size ((char *) (buf + len + 3), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (buf + len + 3), (char *) (
                      path_end), __builtin_object_size ((char *) (buf + len + 3), 1 > 1)) : __inline_strcpy_chk ((char *) (buf + len + 3), (char *) (path_end))
                      );
                    ++stardepth;
                    (void) unix_expandpath (gap, buf, len + 1, flags, 1);
                    --stardepth;
                }
                ((__builtin_object_size ((char *) (buf + len), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (buf + len), (char *) (path_end),
                  __builtin_object_size ((char *) (buf + len), 1 > 1)) : __inline_strcpy_chk ((char *) (buf + len), (char *) (path_end)));
                if (mch_has_exp_wildcard (path_end)) {
                    (void) unix_expandpath (gap, buf, len + 1, flags, 0);
                }
                else {
                    if (*path_end != '\000') backslash_halve (buf + len + 1);
    ...
    if (matches > 0) qsort (((char_u * *) gap -> ga_data) + start_len, matches, sizeof (char_u *), pstrcmp);
    return matches;
}
...
static int find_previous_pathsep (path, psep)
    ...
    char_u * * psep;
{
    if (*psep > path && vim_ispathsep (**psep)) --*psep;
    while (*psep > path) {
        if (vim_ispathsep (**psep)) return 1;
        --*psep;
    }
    return 0;
}
    ...
    int i;
{
    ...
    for (j = 0; j < gap -> ga_len; j ++) {
        ...
        if (other_path_len < candidate_len) continue;
        rival = other_paths [j] + other_path_len - candidate_len;
        if (strcasecmp ((char *) (((maybe_unique))), (char *) (((rival)))) == 0) return 0;
    }
    return 1;
}
    ...
    garray_T * gap;
{
    ...
    char_u * p;
    int len;
    if ((buf = alloc ((int) 1024)) == ((void *) 0)) return;
    while (*path_option != '\000') {
        copy_option_part (&path_option, buf, 1024, " ,");
        if (buf [0] == '.' && (buf [1] == '\000' || vim_ispathsep (buf [1]))) {
            if (curbuf -> b_ffname == ((void *) 0)) continue;
            p = gettail (curbuf -> b_ffname);
            len = (int) (p - curbuf -> b_ffname);
            if (len + (int) strlen ((char *) (buf)) >= 1024) continue;
            if (buf [1] == '\000') buf [len] = '\000';
            else ((__builtin_object_size ((char *) ((buf + len)), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) ((buf + len)), (char *) ((buf + 2)),
              strlen ((char *) (buf + 2)) + 1, __builtin_object_size ((char *) ((buf + len)), 0)) : __inline_memmove_chk ((char *) ((buf + len)), (char *) ((
              buf + 2)), strlen ((char *) (buf + 2)) + 1));
            ((__builtin_object_size ((char *) (buf), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (buf), (char *) (curbuf -> b_ffname), len,
              __builtin_object_size ((char *) (buf), 0)) : __inline_memmove_chk ((char *) (buf), (char *) (curbuf -> b_ffname), len));
            simplify_filename (buf);
        }
        else if (buf [0] == '\000') ((__builtin_object_size ((char *) (buf), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (buf), (char *) (curdir),
          __builtin_object_size ((char *) (buf), 1 > 1)) : __inline_strcpy_chk ((char *) (buf), (char *) (curdir)));
        else if (path_with_url (buf)) continue;
        else if (!mch_isFullName(buf)) {
            len = (int) strlen ((char *) (curdir));
            if (len + (int) strlen ((char *) (buf)) + 3 > 1024) continue;
            ((__builtin_object_size ((char *) ((buf + len + 1)), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) ((buf + len + 1)), (char *) ((buf)),
              strlen ((char *) (buf)) + 1, __builtin_object_size ((char *) ((buf + len + 1)), 0)) : __inline_memmove_chk ((char *) ((buf + len + 1)), (char *) (
              (buf)), strlen ((char *) (buf)) + 1));
            ((__builtin_object_size ((char *) (buf), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (buf), (char *) (curdir), __builtin_object_size ((
              char *) (buf), 1 > 1)) : __inline_strcpy_chk ((char *) (buf), (char *) (curdir)));
            buf [len] = '/';
            simplify_filename (buf);
        }
        if (ga_grow (gap, 1) == 0) break;
        p = vim_strsave (buf);
        if (p == ((void *) 0)) break;
    ...
    garray_T * gap;
{
    ...
    for (i = 0; i < gap -> ga_len; i ++) {
        int j = 0;
        while ((fname [j] == path_part [i] [j]) && fname [j] != '\000' && path_part [i] [j] != '\000') j ++;
        if (j > maxlen) {
            maxlen = j;
            cutoff = &fname[j];
        }
    }
    if (cutoff != ((void *) 0)) while (vim_ispathsep (*cutoff)) ++cutoff;
    return cutoff;
}
    ...
    char_u * pattern;
{
    ...
    if (file_pattern == ((void *) 0)) return;
    file_pattern [0] = '*';
    file_pattern [1] = '\000';
    ((__builtin_object_size ((char *) (file_pattern), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (file_pattern), (char *) (pattern),
      __builtin_object_size ((char *) (file_pattern), 1 > 1)) : __inline_strcat_chk ((char *) (file_pattern), (char *) (pattern)));
    pat = file_pat_to_reg_pat (file_pattern, ((void *) 0), ((void *) 0), 1);
    vim_free (file_pattern);
    if (pat == ((void *) 0)) return;
    regmatch.rm_ic = 1;
    regmatch.regprog = vim_regcomp (pat, 1 + 2);
    vim_free (pat);
    if (regmatch.regprog == ((void *) 0)) return;
    if ((curdir = alloc ((int) (1024))) == ((void *) 0)) goto theend;
    mch_dirname (curdir, 1024);
    expand_path_option (curdir, &path_ga);
    in_curdir = (char_u * *) alloc_clear (gap -> ga_len * sizeof (char_u *));
    if (in_curdir == ((void *) 0)) goto theend;
    for (i = 0; i < gap -> ga_len && !got_int; i ++) {
        ...
        is_in_curdir = strncasecmp ((char *) (((curdir))), (char *) (((path))), (size_t) (((dir_end - path)))) == 0 && curdir [dir_end - path] == '\000';
        if (is_in_curdir) in_curdir [i] = vim_strsave (path);
        path_cutoff = get_path_cutoff (path, &path_ga);
        pathsep_p = path + len - 1;
        while (find_previous_pathsep (path, &pathsep_p)) if (vim_regexec (&regmatch, pathsep_p + 1, (colnr_T) 0) && is_unique (pathsep_p + 1, gap, i) &&
          path_cutoff != ((void *) 0) && pathsep_p + 1 >= path_cutoff) {
            sort_again = 1;
            ((__builtin_object_size ((char *) (path), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (path), (char *) (pathsep_p + 1), strlen ((char *
              ) (pathsep_p)), __builtin_object_size ((char *) (path), 0)) : __inline_memmove_chk ((char *) (path), (char *) (pathsep_p + 1), strlen ((char *) (
              pathsep_p))));
            break;
        }
        if (mch_isFullName (path)) {
            ...
            if (short_name != ((void *) 0) && short_name > path + 1) {
                ((__builtin_object_size ((char *) (path), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (path), (char *) ("."), __builtin_object_size (
                  (char *) (path), 1 > 1)) : __inline_strcpy_chk ((char *) (path), (char *) (".")));
                add_pathsep (path);
                ((__builtin_object_size ((char *) ((path + strlen ((char *) (path)))), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) ((path + strlen (
                  (char *) (path)))), (char *) ((short_name)), strlen ((char *) (short_name)) + 1, __builtin_object_size ((char *) ((path + strlen ((char *) (
                  path)))), 0)) : __inline_memmove_chk ((char *) ((path + strlen ((char *) (path)))), (char *) ((short_name)), strlen ((char *) (short_name)) +
                  1));
            }
        }
        ui_breakcheck ();
    ...
    for (i = 0; i < gap -> ga_len && !got_int; i ++) {
        ...
        short_name = shorten_fname (path, curdir);
        if (short_name == ((void *) 0)) short_name = path;
        if (is_unique (short_name, gap, i)) {
            ((__builtin_object_size ((char *) (fnames [i]), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (fnames [i]), (char *) (short_name),
              __builtin_object_size ((char *) (fnames [i]), 1 > 1)) : __inline_strcpy_chk ((char *) (fnames [i]), (char *) (short_name)));
            continue;
        }
        rel_path = alloc ((int) (strlen ((char *) (short_name)) + strlen ((char *) ("/")) + 2));
        if (rel_path == ((void *) 0)) goto theend;
        ((__builtin_object_size ((char *) (rel_path), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (rel_path), (char *) ("."), __builtin_object_size (
          (char *) (rel_path), 1 > 1)) : __inline_strcpy_chk ((char *) (rel_path), (char *) (".")));
        add_pathsep (rel_path);
        ((__builtin_object_size ((char *) (rel_path), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (rel_path), (char *) (short_name),
          __builtin_object_size ((char *) (rel_path), 1 > 1)) : __inline_strcat_chk ((char *) (rel_path), (char *) (short_name)));
        vim_free (fnames [i]);
        fnames [i] = rel_path;
        sort_again = 1;
        ui_breakcheck ();
    }
    theend : vim_free (curdir);
    if (in_curdir != ((void *) 0)) {
        for (i = 0; i < gap -> ga_len; i ++) vim_free (in_curdir [i]);
        vim_free (in_curdir);
    }
    ga_clear_strings (&path_ga);
    vim_free (regmatch.regprog);
    if (sort_again) remove_duplicates (gap);
}
    ...
    int flags;
{
    ...
    char_u * e;
    char_u * paths = ((void *) 0);
    if ((curdir = alloc ((unsigned) 1024)) == ((void *) 0)) return 0;
    mch_dirname (curdir, 1024);
    ga_init2 (&path_ga, (int) sizeof (char_u *), 1);
    expand_path_option (curdir, &path_ga);
    vim_free (curdir);
    if (path_ga.ga_len == 0) return 0;
    paths = ga_concat_strings (&path_ga);
    ga_clear_strings (&path_ga);
    if (paths == ((void *) 0)) return 0;
    files = globpath (paths, pattern, 0);
    vim_free (paths);
    if (files == ((void *) 0)) return 0;
    s = e = files;
    while (*s != '\000') {
        while (*e != '\n' && *e != '\000') e ++;
        if (*e == '\000') {
    ...
    garray_T * gap;
{
    ...
    for (i = gap -> ga_len - 1; i > 0; --i) if (strcasecmp ((char *) (((fnames [i - 1]))), (char *) (((fnames [i])))) == 0) {
        vim_free (fnames [i]);
        for (j = i + 1; j < gap -> ga_len; ++j) fnames [j - 1] = fnames [j];
        --gap->ga_len;
    }
}
    ...
    int flags;
{
    ...
    if (recursive) return mch_expand_wildcards (num_pat, pat, num_file, file, flags);
    for (i = 0; i < num_pat; i ++) {
        if ((char_u *) strpbrk ((char *) (pat [i]), (char *) ((char_u *) "`'{")) != ((void *) 0) && !(vim_backtick(pat[i])&&pat[i][1]=='=')) return
    ...
    for (i = 0; i < num_pat; ++i) {
        add_pat = -1;
        p = pat [i];
        if (vim_backtick (p)) add_pat = expand_backtick (&ga, p, flags);
        else {
            if (vim_strchr (p, '$') != ((void *) 0) || *p == '~') {
                p = expand_env_save_opt (p, 1);
                if (p == ((void *) 0)) p = pat [i];
                else if (vim_strchr (p, '$') != ((void *) 0) || *p == '~') {
                    vim_free (p);
                    ga_clear_strings (&ga);
            ...
            if (mch_has_exp_wildcard (p)) {
                if ((flags & 0x80) && !mch_isFullName(p) && !(p[0]=='.'&&(vim_ispathsep(p[1])||(p[1]=='.'&&vim_ispathsep(p[2]))))) {
                    recursive = 0;
        ...
        if (add_pat == -1 || (add_pat == 0 && (flags & 0x04))) {
            ...
            if (flags & 0x04) addfile (&ga, t, flags | 0x01 | 0x02);
            else if (mch_getperm (t) >= 0) addfile (&ga, t, flags);
            vim_free (t);
        }
        if (did_expand_in_path && ga.ga_len > 0 && (flags & 0x80)) uniquefy_paths (&ga, p);
        if (p != pat [i]) vim_free (p);
    }
    * num_file = ga.ga_len;
    ...
    int flags;
{
    ...
    if (*cmd == '=') buffer = eval_to_string (cmd + 1, &p, 1);
    else buffer = get_cmd_output (cmd, ((void *) 0), (flags & 0x20) ? 16 : 0);
    vim_free (cmd);
    ...
    int flags;
{
    char_u * p;
    int isdir;
    if (!(flags&0x04) && mch_getperm (f) < 0) return;
    isdir = mch_isdir (f);
    if ((isdir && !(flags&0x01)) || (!isdir && !(flags&0x02))) return;
    if (!isdir && (flags & 0x40) && !mch_can_exe(f)) return;
    if (ga_grow (gap, 1) == 0) return;
    p = alloc ((unsigned) (strlen ((char *) (f)) + 1 + isdir));
    if (p == ((void *) 0)) return;
    ((__builtin_object_size ((char *) (p), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (p), (char *) (f), __builtin_object_size ((char *) (p), 1 > 1
      )) : __inline_strcpy_chk ((char *) (p), (char *) (f)));
    if (isdir && (flags & 0x08)) add_pathsep (p);
    ((char_u * *) gap -> ga_data) [gap -> ga_len ++] = p;
}
    ...
    int flags;
{
    ...
    int i = 0;
    FILE * fd;
    if (check_restricted () || check_secure ()) return ((void *) 0);
    if ((tempname = vim_tempname ('o')) == ((void *) 0)) {
        emsg ((char_u *) (((char *) (e_notmp))));
        return ((void *) 0);
    }
    command = make_filter_cmd (cmd, infile, tempname);
    if (command == ((void *) 0)) goto done;
    ++no_check_timestamps;
    ...
    if (fd == ((void *) 0)) {
        emsg2 ((char_u *) (((char *) (e_notopen))), (char_u *) (tempname));
        goto done;
    }
    fseek (fd, 0L, 2);
    len = ftell (fd);
    fseek (fd, 0L, 0);
    buffer = alloc (len + 1);
    if (buffer != ((void *) 0)) i = (int) fread ((char *) buffer, (size_t) 1, (size_t) len, fd);
    fclose (fd);
    unlink ((char *) (tempname));
    if (buffer == ((void *) 0)) goto done;
    if (i != len) {
        emsg2 ((char_u *) (((char *) (e_notread))), (char_u *) (tempname));
        vim_free (buffer);
        buffer = ((void *) 0);
    }
    else buffer [len] = '\000';
    done : vim_free (tempname);
    return buffer;
}
...
void FreeWild (count, files)
    ...
    char_u * * files;
{
    if (count <= 0 || files == ((void *) 0)) return;
    while (count --) vim_free (files [count]);
    vim_free (files);
}
