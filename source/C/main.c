...
int use_crypt_method = 0;
colnr_T dollar_vcol = 0;
int compl_length = 0;
...
VimClipboard clip_star;
int clip_unnamed = 0;
int clip_autoselect = 0;
int clip_autoselectml = 0;
int clip_html = 0;
regprog_T * clip_exclude_prog = ((void *) 0);
win_T * firstwin;
win_T * lastwin;
...
sigjmp_buf lc_jump_env;
volatile int lc_signal;
volatile int lc_active = 0;
int State = 0x01;
int finish_op = 0;
int opcount = 0;
int exmode_active = 0;
...
option_table_T printer_opts [14] =
{
    {"top", 1, 0, ((void *) 0), 0, 0},
...
char * netbeansArg = ((void *) 0);
int netbeansFireChanges = 1;
int netbeansForcedQuit = 0;
int netbeansReadFile = 1;
int netbeansSuppressNoLines = 0;
...
char_u e_font [] = "E235: Unknown font: %s";
...
char_u e_nogvim [] = "E25: GUI cannot be used: Not enabled at compile time";
char_u e_nohebrew [] = "E26: Hebrew cannot be used: Not enabled at compile time\n";
char_u e_nofarsi [] = "E27: Farsi cannot be used: Not enabled at compile time\n";
char_u e_noarabic [] = "E800: Arabic cannot be used: Not enabled at compile time\n";
...
char_u e_guarded [] = "E463: Region is guarded, cannot modify";
char_u e_nbreadonly [] = "E744: NetBeans does not allow changes in read-only files";
...
{"Unknown option argument", "Too many edit arguments", "Argument missing after", "Garbage after option argument",
      "Too many \"+command\", \"-c command\" or \"--cmd command\" arguments", "Invalid argument for",};
    ...
    char * * argv;
{
    ...
    int i;
    mch_early_init ();
    ((__builtin_object_size ((&params), 0) != (size_t) - 1) ? __builtin___memset_chk ((&params), (0), (sizeof (params)), __builtin_object_size ((&params), 0)) :
      __inline_memset_chk ((&params), (0), (sizeof (params))));
    params.argc = argc;
    params.argv = argv;
    params.want_full_screen = 1;
    params.use_debug_break_level = -1;
    params.window_count = -1;
    for (i = 1; i < argc; ++i) {
        if (strcasecmp ((char *) (argv [i]), (char *) ("--startuptime")) == 0 && i + 1 < argc) {
            time_fd = fopen ((argv [i + 1]), ("a"));
            {
                if (time_fd != ((void *) 0)) time_msg ("--- VIM STARTING ---", ((void *) 0));
            };
            break;
        }
    }
    ...
    eval_init ();
    init_normal_cmds ();
    if ((IObuff = alloc ((1024 + 1))) == ((void *) 0) || (NameBuff = alloc (1024)) == ((void *) 0)) mch_exit (0);
    {
        if (time_fd != ((void *) 0)) time_msg ("Allocated generic buffers", ((void *) 0));
    };
    init_locale ();
    {
        if (time_fd != ((void *) 0)) time_msg ("locale set", ((void *) 0));
    };
    early_arg_scan (&params);
    clip_init (0);
    {
        if (time_fd != ((void *) 0)) time_msg ("clipboard setup", ((void *) 0));
    };
    params.stdout_isatty = (mch_check_win (params.argc, params.argv) != 0);
    {
        if (time_fd != ((void *) 0)) time_msg ("window checked", ((void *) 0));
    };
    if (win_alloc_first () == 0) mch_exit (0);
    init_yank ();
    alist_init (&global_alist);
    init_homedir ();
    set_init_1 ();
    {
        if (time_fd != ((void *) 0)) time_msg ("inits 1", ((void *) 0));
    };
    set_lang_var ();
    parse_command_name (&params);
    command_line_scan (&params);
    {
        if (time_fd != ((void *) 0)) time_msg ("parsing arguments", ((void *) 0));
    };
    if ((global_alist.al_ga.ga_len) > 0) {
        fname = alist_name (&((aentry_T*)global_alist.al_ga.ga_data)[0]);
    }
    {
        if (time_fd != ((void *) 0)) time_msg ("expanding arguments", ((void *) 0));
    };
    if (params.diff_mode && params.window_count == -1) params.window_count = 0;
    ++RedrawingDisabled;
    if (recoverymode && fname == ((void *) 0)) params.want_full_screen = 0;
    mch_init ();
    {
        if (time_fd != ((void *) 0)) time_msg ("shell init", ((void *) 0));
    };
    check_tty (&params);
    if ((global_alist.al_ga.ga_len) > 1 && !silent_mode) printf (((char *) ("%d files to edit\n")), (global_alist.al_ga.ga_len));
    if (params.want_full_screen && !silent_mode) {
        termcapinit (params.term);
        screen_start ();
        {
            if (time_fd != ((void *) 0)) time_msg ("Termcap init", ((void *) 0));
        };
    }
    ui_get_shellsize ();
    win_init_size ();
    if (params.diff_mode) diff_win_options (firstwin, 0);
    cmdline_row = Rows - p_ch;
    msg_row = cmdline_row;
    screenalloc (0);
    set_init_2 ();
    {
        if (time_fd != ((void *) 0)) time_msg ("inits 2", ((void *) 0));
    };
    msg_scroll = 1;
    no_wait_return = 1;
    init_mappings ();
    init_highlight (1, 0);
    {
        if (time_fd != ((void *) 0)) time_msg ("init highlight", ((void *) 0));
    };
    ...
    if (p_lpl) {
        ...
        {
            if (time_fd != ((void *) 0)) time_msg ("loading plugins", ((void *) 0));
        };
    }
    if (params.diff_mode && params.window_layout == 0) {
        if (diffopt_horizontal ()) params.window_layout = 1;
        else params.window_layout = 2;
    }
    ...
    set_init_3 ();
    {
        if (time_fd != ((void *) 0)) time_msg ("inits 3", ((void *) 0));
    };
    if (params.no_swap_file) p_uc = 0;
    if (*p_viminfo != '\000') {
        read_viminfo (((void *) 0), 1 | 8);
        {
            if (time_fd != ((void *) 0)) time_msg ("reading viminfo", ((void *) 0));
        };
    }
    if (params.edit_type == 4) {
        if (params.use_ef != ((void *) 0)) set_string_option_direct ((char_u *) "ef", -1, params.use_ef, 1, -3);
        ...
        {
            if (time_fd != ((void *) 0)) time_msg ("reading errorfile", ((void *) 0));
        };
    }
    starting = 1;
    no_wait_return = 0;
    if (!exmode_active) msg_scroll = 0;
    clip_init (1);
    if (params.edit_type == 2 && !recoverymode) read_stdin ();
    if ((did_emsg || msg_didout) && *(term_strings[(int)(KS_TI)]) != '\000') newline_on_exit = 1;
    settmode (2);
    {
        if (time_fd != ((void *) 0)) time_msg ("setting raw mode", ((void *) 0));
    };
    if (need_wait_return || msg_didany) {
        wait_return (1);
        {
            if (time_fd != ((void *) 0)) time_msg ("waiting for return", ((void *) 0));
        };
    }
    starttermcap ();
    {
        if (time_fd != ((void *) 0)) time_msg ("start termcap", ((void *) 0));
    };
    setmouse ();
    if (scroll_region) scroll_region_reset ();
    scroll_start ();
    if (exmode_active) must_redraw = 50;
    else {
        screenclear ();
        {
            if (time_fd != ((void *) 0)) time_msg ("clearing screen", ((void *) 0));
        };
    }
    if (params.ask_for_key) {
        ...
        {
            if (time_fd != ((void *) 0)) time_msg ("getting crypt key", ((void *) 0));
        };
    }
    no_wait_return = 1;
    create_windows (&params);
    {
        if (time_fd != ((void *) 0)) time_msg ("opening buffers", ((void *) 0));
    };
    ...
    {
        if (time_fd != ((void *) 0)) time_msg ("BufEnter autocommands", ((void *) 0));
    };
    setpcmark ();
    if (params.edit_type == 4) {
        qf_jump (((void *) 0), 0, 0, 0);
        {
            if (time_fd != ((void *) 0)) time_msg ("jump to first error", ((void *) 0));
        };
    }
    edit_buffers (&params);
    if (params.diff_mode) {
        win_T * wp;
        for (wp = firstwin; wp != ((void *) 0); wp = wp -> w_next) diff_win_options (wp, 1);
    }
    shorten_fnames (0);
    if (params.tagname != ((void *) 0)) {
        swap_exists_did_quit = 0;
        vim_snprintf ((char *) IObuff, (1024 + 1), "ta %s", params.tagname);
        do_cmdline_cmd (IObuff);
        {
            if (time_fd != ((void *) 0)) time_msg ("jumping to tag", ((void *) 0));
        };
        if (swap_exists_did_quit) getout (1);
    }
    if (params.n_commands > 0) exe_commands (&params);
    RedrawingDisabled = 0;
    redraw_all_later (40);
    no_wait_return = 0;
    starting = 0;
    ...
    {
        if (time_fd != ((void *) 0)) time_msg ("VimEnter autocommands", ((void *) 0));
    };
    if (curwin -> w_onebuf_opt.wo_diff && curwin -> w_onebuf_opt.wo_scb) {
        update_topline ();
        check_scrollbind ((linenr_T) 0, 0L);
        {
            if (time_fd != ((void *) 0)) time_msg ("diff scrollbinding", ((void *) 0));
        };
    }
    if (restart_edit != 0) stuffcharReadbuff ((-((253)+((int)(KE_NOP)<<8))));
    if (netbeansArg != ((void *) 0) && strncmp ("-nb", netbeansArg, 3) == 0) {
        netbeans_open (netbeansArg + 3, 1);
    }
    {
        if (time_fd != ((void *) 0)) time_msg ("before starting main loop", ((void *) 0));
    };
    main_loop (0, 0);
    return 0;
}
    ...
    int noexmode;
{
    ...
    while (!cmdwin || cmdwin_result == 0) {
        if (stuff_empty ()) {
            ...
            if (need_check_timestamps) check_timestamps (0);
            if (need_wait_return) wait_return (0);
        ...
        if (got_int) {
            if (noexmode && global_busy && !exmode_active && previous_got_int) {
                ...
                if (!quit_more) (void) vgetc ();
                got_int = 0;
            }
            previous_got_int = 1;
        }
        else previous_got_int = 0;
        if (!exmode_active) msg_scroll = 0;
        quit_more = 0;
        if (skip_redraw || exmode_active) skip_redraw = 0;
        else if (do_redraw || stuff_empty ()) {
            if (!finish_op && (has_cursormoved ()) && !(((last_cursormoved).lnum==(curwin->w_cursor).lnum)&&((last_cursormoved).col==(curwin->w_cursor).col)&&(
              (last_cursormoved).coladd==(curwin->w_cursor).coladd))) {
                if (has_cursormoved ()) apply_autocmds (EVENT_CURSORMOVED, ((void *) 0), ((void *) 0), 0, curbuf);
            ...
            if (hasAnyFolding (curwin) && !char_avail()) {
                foldCheckClose ();
                if (fdo_flags & 0x001) foldOpenCursor ();
            }
            update_topline ();
            validate_cursor ();
            if (VIsual_active) update_curbuf (20);
            ...
            if (time_fd != ((void *) 0)) {
                {
                    if (time_fd != ((void *) 0)) time_msg ("first screen update", ((void *) 0));
                };
                {
                    if (time_fd != ((void *) 0)) time_msg ("--- VIM STARTED ---", ((void *) 0));
                };
                fclose (time_fd);
                time_fd = ((void *) 0);
            }
    ...
    int exitval;
{
    ...
    exiting = 1;
    if (exmode_active) exitval += ex_exitval;
    windgoto ((int) Rows - 1, 0);
    hash_debug_results ();
    if (get_vim_var_nr (28) <= 1) {
        for (tp = first_tabpage; tp != ((void *) 0); tp = next_tp) {
            next_tp = tp -> tp_next;
            for (wp = (tp == curtab) ? firstwin : tp -> tp_firstwin; wp != ((void *) 0); wp = wp -> w_next) {
                buf = wp -> w_buffer;
        ...
        for (buf = firstbuf; buf != ((void *) 0); buf = buf -> b_next) if (buf -> b_ml.ml_mfp != ((void *) 0)) {
            apply_autocmds (EVENT_BUFUNLOAD, buf -> b_fname, buf -> b_fname, 0, buf);
            if (!buf_valid(buf)) break;
        }
        apply_autocmds (EVENT_VIMLEAVEPRE, ((void *) 0), ((void *) 0), 0, curbuf);
    }
    if (*p_viminfo != '\000') write_viminfo (((void *) 0), 0);
    if (get_vim_var_nr (28) <= 1) apply_autocmds (EVENT_VIMLEAVE, ((void *) 0), ((void *) 0), 0, curbuf);
    if (did_emsg) {
        no_wait_return = 0;
        wait_return (0);
    }
    windgoto ((int) Rows - 1, 0);
    netbeans_end ();
    if (garbage_collect_at_exit) garbage_collect ();
    mch_exit (exitval);
}
    ...
    int def;
{
    if (vim_isdigit (p [*idx])) {
        def = atoi ((char *) &(p[*idx]));
        while (vim_isdigit (p [*idx])) *idx = *idx + 1;
    }
    return def;
}
    ...
    mparm_T * parmp;
{
    ...
    if (strcmp ((char *) (initstr), (char *) (parmp -> argv [0])) == 0) disallow_gui = 1;
    set_vim_var_string (26, initstr, -1);
    if ((((initstr [0]) < 'A' || (initstr [0]) > 'Z') ? (initstr [0]) : (initstr [0]) + ('a' - 'A')) == 'r') {
        restricted = 1;
        ++initstr;
    }
    if ((((initstr [0]) < 'A' || (initstr [0]) > 'Z') ? (initstr [0]) : (initstr [0]) + ('a' - 'A')) == 'e' && ((((initstr [1]) < 'A' || (initstr [1]) > 'Z') ?
      (initstr [1]) : (initstr [1]) + ('a' - 'A')) == 'v' || (((initstr [1]) < 'A' || (initstr [1]) > 'Z') ? (initstr [1]) : (initstr [1]) + ('a' - 'A')) == 'g'
      )) {
        parmp -> evim_mode = 1;
        ++initstr;
    }
    ...
    if (strncasecmp ((char *) (initstr), (char *) ("view"), (size_t) (4)) == 0) {
        readonlymode = 1;
        curbuf -> b_p_ro = 1;
        p_uc = 10000;
        initstr += 4;
    }
    else if (strncasecmp ((char *) (initstr), (char *) ("vim"), (size_t) (3)) == 0) initstr += 3;
    if (strcasecmp ((char *) (initstr), (char *) ("diff")) == 0) {
        parmp -> diff_mode = 1;
    }
    if (strncasecmp ((char *) (initstr), (char *) ("ex"), (size_t) (2)) == 0) {
        if (strncasecmp ((char *) (initstr + 2), (char *) ("im"), (size_t) (2)) == 0) exmode_active = 2;
...
static void early_arg_scan (parmp)
    mparm_T * parmp __attribute__ ((unused));
{
}
    ...
    mparm_T * parmp;
{
    ...
    while (argc > 0) {
        if (argv [0] [0] == '+' && !had_minmin) {
            if (parmp -> n_commands >= 10) mainerr (4, ((void *) 0));
            argv_idx = -1;
            if (argv [0] [1] == '\000') parmp -> commands [parmp -> n_commands ++] = (char_u *) "$";
            else parmp -> commands [parmp -> n_commands ++] = (char_u *) &(argv[0][1]);
        }
            ...
            switch (c) {
            case '\000' :
                if (exmode_active) silent_mode = 1;
                else {
                    if (parmp -> edit_type != 0) mainerr (1, (char_u *) argv [0]);
                    parmp -> edit_type = 2;
                    read_cmd_fd = 2;
                }
                argv_idx = -1;
                break;
            case '-' :
                if (strcasecmp ((char *) (argv [0] + argv_idx), (char *) ("help")) == 0) usage ();
                ...
                else if (strncasecmp ((char *) (argv [0] + argv_idx), (char *) ("nofork"), (size_t) (6)) == 0) {
                }
                else if (strncasecmp ((char *) (argv [0] + argv_idx), (char *) ("noplugin"), (size_t) (8)) == 0) p_lpl = 0;
                    ...
                    if (argv [0] [argv_idx]) mainerr (0, (char_u *) argv [0]);
                    had_minmin = 1;
                }
                if (!want_argument) argv_idx = -1;
                break;
            case 'A' :
                fprintf (__stderrp, "%s", (((char *) (e_noarabic))));
                mch_exit (2);
                break;
            case 'b' :
            ...
            case 'g' :
                ...
                break;
            case 'F' :
                fprintf (__stderrp, "%s", (((char *) (e_nofarsi))));
                mch_exit (2);
                break;
            case 'h' :
                usage ();
                break;
            case 'H' :
                fprintf (__stderrp, "%s", (((char *) (e_nohebrew))));
                mch_exit (2);
                break;
            case 'l' :
            ...
            case 'q' :
                ...
                else if (argc > 1) want_argument = 1;
                break;
            case 'R' :
                readonlymode = 1;
                curbuf -> b_p_ro = 1;
            ...
            if (want_argument) {
                if (argv [0] [argv_idx] != '\000') mainerr (3, (char_u *) argv [0]);
                --argc;
                if (argc < 1 && c != 'S') mainerr_arg_missing ((char_u *) argv [0]);
                ++argv;
                argv_idx = -1;
                switch (c) {
                case 'c' :
                case 'S' :
                    ...
                    if (c == 'S') {
                        ...
                        p = alloc ((unsigned) (strlen ((char *) (a)) + 4));
                        if (p == ((void *) 0)) mch_exit (2);
                        __builtin___sprintf_chk ((char *) p, 0, __builtin_object_size ((char *) p, 1 > 1), "so %s", a);
                        parmp -> cmds_tofree [parmp -> n_commands] = 1;
                        parmp -> commands [parmp -> n_commands ++] = p;
                    }
                    else parmp -> commands [parmp -> n_commands ++] = (char_u *) argv [0];
                    break;
                case '-' :
                    if (argv [-1] [2] == 'c') {
                        if (parmp -> n_pre_commands >= 10) mainerr (4, ((void *) 0));
                ...
                case 's' :
                    if (scriptin [0] != ((void *) 0)) {
                        scripterror : fprintf (__stderrp, "%s", (((char *) ("Attempt to open script file again: \""))));
                    ...
                    if ((scriptin [0] = fopen ((argv [0]), ("rb"))) == ((void *) 0)) {
                        fprintf (__stderrp, "%s", (((char *) ("Cannot open for reading: \""))));
                        fprintf (__stderrp, "%s", (argv [0]));
                        fprintf (__stderrp, "%s", ("\"\n"));
                        mch_exit (2);
                    }
                    if (save_typebuf () == 0) mch_exit (2);
                    break;
                case 't' :
                ...
                case 'W' :
                    if (scriptout != ((void *) 0)) goto scripterror;
                    if ((scriptout = fopen ((argv [0]), (c == 'w' ? "ab" : "wb"))) == ((void *) 0)) {
                        fprintf (__stderrp, "%s", (((char *) ("Cannot open for script output: \""))));
                        fprintf (__stderrp, "%s", (argv [0]));
                        fprintf (__stderrp, "%s", ("\"\n"));
                        mch_exit (2);
                    }
            ...
            argv_idx = -1;
            if (parmp -> edit_type != 0 && parmp -> edit_type != 1) mainerr (1, (char_u *) argv [0]);
            parmp -> edit_type = 1;
            if (ga_grow (&global_alist.al_ga, 1) == 0 || (p = vim_strsave ((char_u *) argv [0])) == ((void *) 0)) mch_exit (2);
            if (parmp -> diff_mode && mch_isdir (p) && (global_alist.al_ga.ga_len) > 0 && !mch_isdir(alist_name(&((aentry_T*)global_alist.al_ga.ga_data)[0]))) {
                char_u * r;
                r = concat_fnames (p, gettail (alist_name (&((aentry_T*)global_alist.al_ga.ga_data)[0])), 1);
                if (r != ((void *) 0)) {
                    vim_free (p);
                    p = r;
                }
    ...
    if (parmp -> n_commands > 0) {
        p = alloc ((unsigned) strlen ((char *) (parmp -> commands [0])) + 3);
        if (p != ((void *) 0)) {
            __builtin___sprintf_chk ((char *) p, 0, __builtin_object_size ((char *) p, 1 > 1), ":%s\r", parmp -> commands [0]);
    ...
    mparm_T * parmp;
{
    ...
    if (exmode_active) {
        if (!input_isatty) silent_mode = 1;
    }
    else if (parmp -> want_full_screen && (!parmp->stdout_isatty || !input_isatty)) {
        if (!parmp->stdout_isatty) fprintf (__stderrp, "%s", (((char *) ("Vim: Warning: Output is not to a terminal\n"))));
        if (!input_isatty) fprintf (__stderrp, "%s", (((char *) ("Vim: Warning: Input is not from a terminal\n"))));
        out_flush ();
        if (scriptin [0] == ((void *) 0)) ui_delay (2000L, 1);
        {
            if (time_fd != ((void *) 0)) time_msg ("Warning delay", ((void *) 0));
        };
    }
}
...
static void read_stdin () {
    ...
    msg_didany = i;
    {
        if (time_fd != ((void *) 0)) time_msg ("reading stdin", ((void *) 0));
    };
    ...
    mparm_T * parmp __attribute__ ((unused));
{
    int dorewind;
    int done = 0;
    if (parmp -> window_count == -1) parmp -> window_count = 1;
    if (parmp -> window_count == 0) parmp -> window_count = (global_alist.al_ga.ga_len);
    if (parmp -> window_count > 1) {
        if (parmp -> window_layout == 0) parmp -> window_layout = 1;
        if (parmp -> window_layout == 3) {
            parmp -> window_count = make_tabpages (parmp -> window_count);
            {
                if (time_fd != ((void *) 0)) time_msg ("making tab pages", ((void *) 0));
            };
        }
        else if (firstwin -> w_next == ((void *) 0)) {
            parmp -> window_count = make_windows (parmp -> window_count, parmp -> window_layout == 2);
            {
                if (time_fd != ((void *) 0)) time_msg ("making windows", ((void *) 0));
            };
        }
        else parmp -> window_count = win_count ();
    }
    else parmp -> window_count = 1;
    if (recoverymode) {
        msg_scroll = 1;
        ml_recover ();
        if (curbuf -> b_ml.ml_mfp == ((void *) 0)) getout (1);
        do_modelines (0);
    }
    else {
        ++autocmd_no_enter;
        ...
        while (done ++ < 1000) {
            if (dorewind) {
                if (parmp -> window_layout == 3) goto_tabpage (1);
                else curwin = firstwin;
            }
            else if (parmp -> window_layout == 3) {
                if (curtab -> tp_next == ((void *) 0)) break;
                goto_tabpage (0);
            }
            else {
                if (curwin -> w_next == ((void *) 0)) break;
                curwin = curwin -> w_next;
            }
            dorewind = 0;
            curbuf = curwin -> w_buffer;
            if (curbuf -> b_ml.ml_mfp == ((void *) 0)) {
                if (p_fdls >= 0) curwin -> w_onebuf_opt.wo_fdl = p_fdls;
                swap_exists_action = 1;
                set_buflisted (1);
                ...
                else handle_swap_exists (((void *) 0));
                dorewind = 1;
            }
            ui_breakcheck ();
            if (got_int) {
    ...
    mparm_T * parmp;
{
    ...
    for (i = 1; i < parmp -> window_count; ++i) {
        ...
        if (curbuf == firstwin -> w_buffer || curbuf -> b_ffname == ((void *) 0)) {
            ...
            swap_exists_did_quit = 0;
            (void) do_ecmd (0, arg_idx < (global_alist.al_ga.ga_len) ? alist_name (&((aentry_T*)global_alist.al_ga.ga_data)[arg_idx]) : ((void *) 0), ((void *)
              0), ((void *) 0), (linenr_T) 0, 0x01, curwin);
            ...
            if (arg_idx == (global_alist.al_ga.ga_len) - 1) arg_had_last = 1;
            ++arg_idx;
        }
        ui_breakcheck ();
        if (got_int) {
    ...
    --autocmd_no_leave;
    {
        if (time_fd != ((void *) 0)) time_msg ("editing files in windows", ((void *) 0));
    };
    if (parmp -> window_count > 1 && parmp -> window_layout != 3) win_equal (curwin, 0, 'b');
}
    ...
    mparm_T * parmp;
{
    ...
    if (cnt > 0) {
        ...
        sourcing_name = (char_u *) ((char *) ("pre-vimrc command line"));
        current_SID = -2;
        for (i = 0; i < cnt; ++i) do_cmdline_cmd (cmds [i]);
        sourcing_name = ((void *) 0);
        current_SID = 0;
        {
            if (time_fd != ((void *) 0)) time_msg ("--cmd commands", ((void *) 0));
        };
    }
}
    ...
    mparm_T * parmp;
{
    int i;
    msg_scroll = 1;
    if (parmp -> tagname == ((void *) 0) && curwin -> w_cursor.lnum <= 1) curwin -> w_cursor.lnum = 0;
    sourcing_name = (char_u *) "command line";
    current_SID = -3;
    for (i = 0; i < parmp -> n_commands; ++i) {
        do_cmdline_cmd (parmp -> commands [i]);
        if (parmp -> cmds_tofree [i]) vim_free (parmp -> commands [i]);
    }
    sourcing_name = ((void *) 0);
    current_SID = 0;
    if (curwin -> w_cursor.lnum == 0) curwin -> w_cursor.lnum = 1;
    if (!exmode_active) msg_scroll = 0;
    if (parmp -> edit_type == 4) qf_jump (((void *) 0), 0, 0, 0);
    {
        if (time_fd != ((void *) 0)) time_msg ("executing command arguments", ((void *) 0));
    };
}
    ...
    mparm_T * parmp;
{
    int i;
    if (parmp -> evim_mode) {
        (void) do_source ((char_u *) "$VIMRUNTIME/evim.vim", 0, 0);
        {
            if (time_fd != ((void *) 0)) time_msg ("source evim file", ((void *) 0));
        };
    }
    if (parmp -> use_vimrc != ((void *) 0)) {
        if (strcmp ((char *) (parmp -> use_vimrc), (char *) ("NONE")) == 0 || strcmp ((char *) (parmp -> use_vimrc), (char *) ("NORC")) == 0) {
            if (parmp -> use_vimrc [2] == 'N') p_lpl = 0;
        }
        else {
            if (do_source (parmp -> use_vimrc, 0, 0) != 1) emsg2 ((char_u *) (((char *) ("E282: Cannot read from \"%s\""))), (char_u *) (parmp -> use_vimrc));
        }
    }
    else if (!silent_mode) {
        (void) do_source ((char_u *) "$VIM/vimrc", 0, 0);
        (void) do_source ((char_u *) "$VIMRUNTIME/macmap.vim", 0, 0);
        if (process_env ((char_u *) "VIMINIT", 1) != 1) {
            if (do_source ((char_u *) "$HOME/.vimrc", 1, 1) == 0 && process_env ((char_u *) "EXINIT", 0) == 0 && do_source ((char_u *) "$HOME/.exrc", 0, 0) == 0
              ) {
            }
        }
        if (p_exrc) {
            if (!file_owned(".vimrc")) secure = p_secure;
            i = 0;
            if (fullpathcmp ((char_u *) "$HOME/.vimrc", (char_u *) ".vimrc", 0) != 1 && fullpathcmp ((char_u *) "$VIM/vimrc", (char_u *) ".vimrc", 0) != 1) i =
              do_source ((char_u *) ".vimrc", 1, 1);
            if (i == 0) {
                if (!file_owned(".exrc")) secure = p_secure;
                else secure = 0;
                if (fullpathcmp ((char_u *) "$HOME/.exrc", (char_u *) ".exrc", 0) != 1) (void) do_source ((char_u *) ".exrc", 0, 0);
            }
        }
    ...
    {
        if (time_fd != ((void *) 0)) time_msg ("sourcing vimrc file(s)", ((void *) 0));
    };
}

static void main_start_gui () {
    fprintf (__stderrp, "%s", (((char *) (e_nogvim))));
    fprintf (__stderrp, "%s", ("\n"));
    mch_exit (2);
}
    ...
    int is_viminit;
{
    ...
    scid_T save_sid;
    if ((initstr = (char_u *) getenv ((char *) (env))) != ((void *) 0) && *initstr != '\000') {
        if (is_viminit) vimrc_found (((void *) 0), ((void *) 0));
...
{
    reset_signals ();
    fprintf (__stderrp, "%s", (longVersion));
    fprintf (__stderrp, "%s", ("\n"));
    fprintf (__stderrp, "%s", (((char *) (main_errors [n]))));
    if (str != ((void *) 0)) {
        fprintf (__stderrp, "%s", (": \""));
        fprintf (__stderrp, "%s", ((char *) str));
        fprintf (__stderrp, "%s", ("\""));
    }
    fprintf (__stderrp, "%s", (((char *) ("\nMore info with: \"vim -h\"\n"))));
    mch_exit (1);
}
...
static void usage () {
    int i;
    static char * (use []) =
    {"[file ..]       edit specified file(s)", "-               read text from stdin", "-t tag          edit file where tag is defined",
          "-q [errorfile]  edit file with first error"};
    reset_signals ();
    printf ("%s", (longVersion));
    printf ("%s", (((char *) ("\n\nusage:"))));
    for (i = 0;; ++i) {
        printf ("%s", (((char *) (" vim [arguments] "))));
        printf ("%s", (((char *) (use [i]))));
        if (i == (sizeof (use) / sizeof (char_u *)) - 1) break;
        printf ("%s", (((char *) ("\n   or:"))));
    }
    printf ("%s", (((char *) ("\n\nArguments:\n"))));
    ...
    main_msg (((char *) ("-d\t\t\tDiff mode (like \"vimdiff\")")));
    ...
    main_msg (((char *) ("--startuptime <file>\tWrite startup timing messages to <file>")));
...
static void check_swap_exists_action () {
    if (swap_exists_action == 2) getout (1);
...
{
    *((structtimeval*)tv_rel) = prev_timeval;
    gettimeofday (&prev_timeval, ((void *) 0));
    ...
    void * tv_start;
{
    ...
    struct timeval now;
    if (time_fd != ((void *) 0)) {
        if (strstr (mesg, "STARTING") != ((void *) 0)) {
        ...
        time_diff (&start, &now);
        if (((struct timeval *) tv_start) != ((void *) 0)) {
            fprintf (time_fd, "  ");
