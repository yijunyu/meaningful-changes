    ...
    exarg_T * eap __attribute__ ((unused));
{
    ...
    {
        if (c == '\012') c = '\000';
        if (c == '\015' && get_fileformat (curbuf) == 2) cval = '\012';
        else cval = c;
        ...
        if (c >= 0x80) vim_snprintf (buf2, sizeof (buf2), "  <M-%s>", (char *) transchar (c & 0x7f));
        else buf2 [0] = '\000';
        vim_snprintf ((char *) IObuff, (1024 + 1), ((char *) ("<%s>%s%s  %d,  Hex %02x,  Octal %03o")), transchar (c), buf1, buf2, cval, cval, cval);
    }
    msg (IObuff);
}
    ...
    exarg_T * eap;
{
    ...
    if (eap -> cmdidx == CMD_left) {
        ...
        if (width <= 0) width = curbuf -> b_p_tw;
        if (width == 0 && curbuf -> b_p_wm > 0) width = (curwin -> w_width) - curbuf -> b_p_wm;
        if (width <= 0) width = 80;
    }
    if (u_save ((linenr_T) (eap -> line1 - 1), (linenr_T) (eap -> line2 + 1)) == 0) return;
    for (curwin -> w_cursor.lnum = eap -> line1; curwin -> w_cursor.lnum <= eap -> line2; ++curwin->w_cursor.lnum) {
        if (eap -> cmdidx == CMD_left) new_indent = indent;
        else {
            has_tab = 0;
            len = linelen (eap -> cmdidx == CMD_right ? &has_tab : ((void *) 0)) - get_indent ();
            if (len <= 0) continue;
            if (eap -> cmdidx == CMD_center) new_indent = (width - len) / 2;
            else {
                new_indent = width - len;
                if (has_tab) while (new_indent > 0) {
    ...
    int * has_tab;
{
    ...
    for (last = first + strlen ((char *) (first)); last > first && ((last [-1]) == ' ' || (last [-1]) == '\t'); --last);
    save = *last;
    * last = '\000';
    len = linetabsize (line);
    if (has_tab != ((void *) 0)) *has_tab = (vim_strrchr (first, '\011') != ((void *) 0));
    * last = save;
    return len;
}
...
static int sort_compare (s1, s2) const
    void * s1;
    const void * s2;
{
    ...
    int result = 0;
    if (sort_abort) return 0;
    fast_breakcheck ();
    if (got_int) sort_abort = 1;
    if (sort_nr) result = l1.start_col_nr - l2.start_col_nr;
    else {
        ((__builtin_object_size ((char *) (sortbuf1), 0) != (size_t) - 1) ? __builtin___strncpy_chk ((char *) (sortbuf1), (char *) (ml_get (l1.lnum) + l1.
          start_col_nr), (size_t) (l1.end_col_nr - l1.start_col_nr + 1), __builtin_object_size ((char *) (sortbuf1), 1 > 1)) : __inline_strncpy_chk ((char *) (
          sortbuf1), (char *) (ml_get (l1.lnum) + l1.start_col_nr), (size_t) (l1.end_col_nr - l1.start_col_nr + 1)));
        sortbuf1 [l1.end_col_nr - l1.start_col_nr] = 0;
        ((__builtin_object_size ((char *) (sortbuf2), 0) != (size_t) - 1) ? __builtin___strncpy_chk ((char *) (sortbuf2), (char *) (ml_get (l2.lnum) + l2.
          start_col_nr), (size_t) (l2.end_col_nr - l2.start_col_nr + 1), __builtin_object_size ((char *) (sortbuf2), 1 > 1)) : __inline_strncpy_chk ((char *) (
          sortbuf2), (char *) (ml_get (l2.lnum) + l2.start_col_nr), (size_t) (l2.end_col_nr - l2.start_col_nr + 1)));
        sortbuf2 [l2.end_col_nr - l2.start_col_nr] = 0;
        result = sort_ic ? strcasecmp ((char *) (sortbuf1), (char *) (sortbuf2)) : strcmp ((char *) (sortbuf1), (char *) (sortbuf2));
    }
    if (result == 0) return (int) (l1.lnum - l2.lnum);
    return result;
}
    ...
    exarg_T * eap;
{
    ...
    nrs = (sorti_T *) lalloc ((long_u) (count * sizeof (sorti_T)), 1);
    if (nrs == ((void *) 0)) goto sortend;
    sort_abort = sort_ic = sort_rx = sort_nr = sort_oct = sort_hex = 0;
    for (p = eap -> arg; *p != '\000'; ++p) {
        if (((*p) == ' ' || (*p) == '\t'));
        else if (*p == 'i') sort_ic = 1;
        else if (*p == 'r') sort_rx = 1;
        else if (*p == 'n') sort_nr = 2;
        else if (*p == 'o') sort_oct = 2;
        else if (*p == 'x') sort_hex = 2;
        ...
        else if (!((*p) < 0x7f && isalpha (*p)) && regmatch.regprog == ((void *) 0)) {
            s = skip_regexp (p + 1, *p, 1, ((void *) 0));
            if (*s != *p) {
                emsg ((char_u *) (((char *) (e_invalpat))));
                goto sortend;
            }
            * s = '\000';
            if (s == p + 1 && last_search_pat () != ((void *) 0)) regmatch.regprog = vim_regcomp (last_search_pat (), 1);
            else regmatch.regprog = vim_regcomp (p + 1, 1);
            if (regmatch.regprog == ((void *) 0)) goto sortend;
            p = s;
            regmatch.rm_ic = p_ic;
        }
        else {
    ...
    for (lnum = eap -> line1; lnum <= eap -> line2; ++lnum) {
        ...
        else if (regmatch.regprog != ((void *) 0)) end_col = 0;
        if (sort_nr) {
            s2 = s + end_col;
            c = *s2;
            (* s2) = 0;
            p = s + start_col;
            if (sort_hex) s = skiptohex (p);
            else s = skiptodigit (p);
            if (s > p && s [-1] == '-') --s;
            vim_str2nr (s, ((void *) 0), ((void *) 0), sort_oct, sort_hex, &nrs[lnum-eap->line1].start_col_nr, ((void *) 0));
            (* s2) = c;
        }
        else {
        ...
        if (regmatch.regprog != ((void *) 0)) fast_breakcheck ();
        if (got_int) goto sortend;
    }
    sortbuf1 = alloc ((unsigned) maxlen + 1);
    if (sortbuf1 == ((void *) 0)) goto sortend;
    sortbuf2 = alloc ((unsigned) maxlen + 1);
    if (sortbuf2 == ((void *) 0)) goto sortend;
    qsort ((void *) nrs, count, sizeof (sorti_T), sort_compare);
    if (sort_abort) goto sortend;
    lnum = eap -> line2;
    for (i = 0; i < count; ++i) {
        s = ml_get (nrs [eap -> forceit ? count - i - 1 : i].lnum);
        if (!unique || i == 0 || (sort_ic ? strcasecmp ((char *) (s), (char *) (sortbuf1)) : strcmp ((char *) (s), (char *) (sortbuf1))) != 0) {
            if (ml_append (lnum ++, s, (colnr_T) 0, 0) == 0) break;
            if (unique) ((__builtin_object_size ((char *) (sortbuf1), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (sortbuf1), (char *) (s),
              __builtin_object_size ((char *) (sortbuf1), 1 > 1)) : __inline_strcpy_chk ((char *) (sortbuf1), (char *) (s)));
        }
        fast_breakcheck ();
        if (got_int) goto sortend;
    }
    if (i == count) for (i = 0; i < count; ++i) ml_delete (eap -> line1, 0);
    else count = 0;
    deleted = (long) (count - (lnum - eap -> line2));
    if (deleted > 0) mark_adjust (eap -> line2 - deleted, eap -> line2, (long) (0x7fffffffL), -deleted);
    else if (deleted < 0) mark_adjust (eap -> line2, (0x7fffffffL), -deleted, 0L);
    changed_lines (eap -> line1, 0, eap -> line2 + 1, -deleted);
    curwin -> w_cursor.lnum = eap -> line1;
    beginline (1 | 4);
    sortend : vim_free (nrs);
    vim_free (sortbuf1);
    vim_free (sortbuf2);
    vim_free (regmatch.regprog);
    if (got_int) emsg ((char_u *) (((char *) (e_interr))));
}
    ...
    exarg_T * eap;
{
    ...
    if (new_ts < 0) {
        emsg ((char_u *) (((char *) (e_positive))));
        return;
    }
    if (new_ts == 0) new_ts = curbuf -> b_p_ts;
            ...
            if (((ptr [col]) == ' ' || (ptr [col]) == '\t')) {
                ...
                if (got_tab || (eap -> forceit && num_spaces > 1)) {
                    ...
                    if (curbuf -> b_p_et || got_tab || (num_spaces + num_tabs < len)) {
                        if (did_undo == 0) {
                            did_undo = 1;
                            if (u_save ((linenr_T) (lnum - 1), (linenr_T) (lnum + 1)) == 0) {
                                new_line = ((void *) 0);
                                break;
                            }
                        }
                        ...
                        new_line = lalloc (old_len - col + start_col + len + 1, 1);
                        if (new_line == ((void *) 0)) break;
                        if (start_col > 0) ((__builtin_object_size ((char *) (new_line), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (new_line), (
                          char *) (ptr), (size_t) start_col, __builtin_object_size ((char *) (new_line), 0)) : __inline_memmove_chk ((char *) (new_line), (char
                          *) (ptr), (size_t) start_col));
                        ((__builtin_object_size ((char *) (new_line + start_col + len), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (new_line +
                          start_col + len), (char *) (ptr + col), (size_t) (old_len - col + 1), __builtin_object_size ((char *) (new_line + start_col + len), 0
                          )) : __inline_memmove_chk ((char *) (new_line + start_col + len), (char *) (ptr + col), (size_t) (old_len - col + 1)));
                        ptr = new_line + start_col;
                        for (col = 0; col < len; col ++) ptr [col] = (col < num_tabs) ? '\t' : ' ';
    ...
    if (got_int) emsg ((char_u *) (((char *) (e_interr))));
    if (curbuf -> b_p_ts != new_ts) redraw_curbuf_later (40);
    if (first_line != 0) changed_lines (first_line, 0, last_line + 1, 0L);
    ...
    linenr_T dest;
{
    ...
    if (dest >= line1 && dest < line2) {
        emsg ((char_u *) (((char *) ("E134: Move lines into themselves"))));
        return 0;
    }
    num_lines = line2 - line1 + 1;
    if (u_save (dest, dest + 1) == 0) return 0;
    ...
    if (u_save (line1 + extra - 1, line2 + extra + 1) == 0) return 0;
    for (l = line1; l <= line2; l ++) ml_delete (line1 + extra, 1);
    if (!global_busy && num_lines > p_report) {
        if (num_lines == 1) msg ((char_u *) (((char *) ("1 line moved"))));
        else smsg ((char_u *) ((char *) ("%ld lines moved")), num_lines);
    }
    if (dest >= line1) curwin -> w_cursor.lnum = dest;
    else curwin -> w_cursor.lnum = dest + (line2 - line1) + 1;
    if (line1 < dest) changed_lines (line1, 0, dest + num_lines + 1, 0L);
    ...
    int do_in, do_out;
{
    ...
    do {
        ...
        if (newcmd != ((void *) 0)) len += (int) strlen ((char *) (newcmd));
        if (ins_prevcmd) {
            if (prevcmd == ((void *) 0)) {
                emsg ((char_u *) (((char *) (e_noprev))));
                vim_free (newcmd);
                return;
            }
            len += (int) strlen ((char *) (prevcmd));
        }
        if ((t = alloc ((unsigned) len)) == ((void *) 0)) {
            vim_free (newcmd);
            return;
        }
        * t = '\000';
        if (newcmd != ((void *) 0)) ((__builtin_object_size ((char *) (t), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (t), (char *) (newcmd),
          __builtin_object_size ((char *) (t), 1 > 1)) : __inline_strcat_chk ((char *) (t), (char *) (newcmd)));
        if (ins_prevcmd) ((__builtin_object_size ((char *) (t), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (t), (char *) (prevcmd),
          __builtin_object_size ((char *) (t), 1 > 1)) : __inline_strcat_chk ((char *) (t), (char *) (prevcmd)));
        p = t + strlen ((char *) (t));
        ((__builtin_object_size ((char *) (t), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (t), (char *) (trailarg), __builtin_object_size ((char *)
          (t), 1 > 1)) : __inline_strcat_chk ((char *) (t), (char *) (trailarg)));
        vim_free (newcmd);
        newcmd = t;
        trailarg = ((void *) 0);
        while (*p) {
            if (*p == '!') {
                if (p > newcmd && p [-1] == '\\') ((__builtin_object_size ((char *) ((p - 1)), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) ((p - 1)
                  ), (char *) ((p)), strlen ((char *) (p)) + 1, __builtin_object_size ((char *) ((p - 1)), 0)) : __inline_memmove_chk ((char *) ((p - 1)), (
                  char *) ((p)), strlen ((char *) (p)) + 1));
                else {
                    trailarg = p;
                    *trailarg++ = '\000';
                    ins_prevcmd = 1;
    ...
    if (*p_shq != '\000') {
        newcmd = alloc ((unsigned) (strlen ((char *) (prevcmd)) + 2 * strlen ((char *) (p_shq)) + 1));
        if (newcmd == ((void *) 0)) return;
        ((__builtin_object_size ((char *) (newcmd), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (newcmd), (char *) (p_shq), __builtin_object_size ((
          char *) (newcmd), 1 > 1)) : __inline_strcpy_chk ((char *) (newcmd), (char *) (p_shq)));
        ((__builtin_object_size ((char *) (newcmd), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (newcmd), (char *) (prevcmd), __builtin_object_size (
          (char *) (newcmd), 1 > 1)) : __inline_strcat_chk ((char *) (newcmd), (char *) (prevcmd)));
        ((__builtin_object_size ((char *) (newcmd), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (newcmd), (char *) (p_shq), __builtin_object_size ((
          char *) (newcmd), 1 > 1)) : __inline_strcat_chk ((char *) (newcmd), (char *) (p_shq)));
        free_newcmd = 1;
    }
    if (addr_count == 0) {
        msg_start ();
    ...
    if (free_newcmd) vim_free (newcmd);
}
    ...
    int do_in, do_out;
{
    ...
    else if ((do_in && (itmp = vim_tempname ('i')) == ((void *) 0)) || (do_out && (otmp = vim_tempname ('o')) == ((void *) 0))) {
        emsg ((char_u *) (((char *) (e_notmp))));
        goto filterend;
    }
    ++no_wait_return;
    if (itmp != ((void *) 0) && buf_write (curbuf, itmp, ((void *) 0), line1, line2, eap, 0, 0, 0, 1) == 0) {
        msg_putchar ('\n');
        --no_wait_return;
        if (!aborting()) (void) emsg2 ((char_u *) (((char *) (e_notcreate))), (char_u *) (itmp));
        goto filterend;
    }
    if (curbuf != old_curbuf) goto filterend;
    if (!do_out) msg_putchar ('\n');
    cmd_buf = make_filter_cmd (cmd, itmp, otmp);
    if (cmd_buf == ((void *) 0)) goto filterend;
    windgoto ((int) Rows - 1, 0);
    cursor_on ();
    if (!do_out || strcmp ((char *) (p_srr), (char *) (">")) == 0 || !do_in) redraw_later_clear ();
    if (do_out) {
    ...
    if (call_shell (cmd_buf, 1 | 4 | shell_flags)) {
        redraw_later_clear ();
        wait_return (0);
    }
    vim_free (cmd_buf);
    ...
    if (do_out) {
        if (otmp != ((void *) 0)) {
            if (readfile (otmp, ((void *) 0), line2, (linenr_T) 0, (linenr_T) (0x7fffffffL), eap, 0x02) == 0) {
                if (!aborting()) {
                    msg_putchar ('\n');
                    emsg2 ((char_u *) (((char *) (e_notread))), (char_u *) (otmp));
                }
                goto error;
            }
            if (curbuf != old_curbuf) goto filterend;
        ...
        if (do_in) {
            if (cmdmod.keepmarks || vim_strchr (p_cpo, 'R') == ((void *) 0)) {
                if (read_linecount >= linecount) mark_adjust (line1, line2, linecount, 0L);
                else {
                    mark_adjust (line1, line1 + read_linecount - 1, linecount, 0L);
                    mark_adjust (line1 + read_linecount, line2, (0x7fffffffL), 0L);
                }
            }
            curwin -> w_cursor.lnum = line1;
        ...
        if (linecount > p_report) {
            if (do_in) {
                vim_snprintf ((char *) msg_buf, sizeof (msg_buf), ((char *) ("%ld lines filtered")), (long) linecount);
                if (msg (msg_buf) && !msg_scroll) set_keep_msg (msg_buf, 0);
            }
            else msgmore ((long) linecount);
        }
    }
    else {
        error : curwin -> w_cursor = cursor_save;
        --no_wait_return;
        wait_return (0);
    }
    filterend : if (curbuf != old_curbuf) {
        --no_wait_return;
        emsg ((char_u *) (((char *) ("E135: *Filter* Autocommands must not change current buffer"))));
    }
    if (itmp != ((void *) 0)) unlink ((char *) (itmp));
    if (otmp != ((void *) 0)) unlink ((char *) (otmp));
    vim_free (itmp);
    vim_free (otmp);
}
    ...
    int flags;
{
    ...
    msg_putchar ('\n');
    if (p_warn && !autocmd_busy && msg_silent == 0) for (buf = firstbuf; buf; buf = buf -> b_next) if (bufIsChanged (buf)) {
        msg_puts ((char_u *) (((char *) ("[No write since last change]\n"))));
        break;
    }
    if (!swapping_screen()) windgoto (msg_row, msg_col);
    cursor_on ();
    (void) call_shell (cmd, 4 | flags);
    did_check_timestamps = 0;
    need_check_timestamps = 1;
    if (!swapping_screen()) {
        msg_row = Rows - 1;
    ...
    if (autocmd_busy) {
        if (msg_silent == 0) redraw_later_clear ();
    }
    else {
        if (cmd == ((void *) 0)) {
            if (msg_silent == 0) redraw_later_clear ();
            ...
            if (swapping_screen ()) no_wait_return = 0;
    ...
    char_u * otmp;
{
    ...
    if (itmp != ((void *) 0)) len += (long_u) strlen ((char *) (itmp)) + 9;
    if (otmp != ((void *) 0)) len += (long_u) strlen ((char *) (otmp)) + (long_u) strlen ((char *) (p_srr)) + 2;
    buf = lalloc (len, 1);
    if (buf == ((void *) 0)) return ((void *) 0);
    if (itmp != ((void *) 0) || otmp != ((void *) 0)) vim_snprintf ((char *) buf, len, "(%s)", (char *) cmd);
    else ((__builtin_object_size ((char *) (buf), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (buf), (char *) (cmd), __builtin_object_size ((char *)
      (buf), 1 > 1)) : __inline_strcpy_chk ((char *) (buf), (char *) (cmd)));
    if (itmp != ((void *) 0)) {
        ...
        ((__builtin_object_size ((char *) (buf), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (buf), (char *) (itmp), __builtin_object_size ((char *)
          (buf), 1 > 1)) : __inline_strcat_chk ((char *) (buf), (char *) (itmp)));
    }
    if (otmp != ((void *) 0)) append_redir (buf, (int) len, p_srr, otmp);
    return buf;
}
    ...
    char_u * fname;
{
    ...
    for (p = opt; (p = vim_strchr (p, '%')) != ((void *) 0); ++p) if (p [1] == 's') break;
    if (p != ((void *) 0)) {
        * end = ' ';
        vim_snprintf ((char *) end + 1, (size_t) (buflen - (end + 1 - buf)), (char *) opt, (char *) fname);
    }
    else vim_snprintf ((char *) end, (size_t) (buflen - (end - buf)), opt != p_sp ? " %s%s" : " %s %s", (char *) opt, (char *) fname);
}
...
int viminfo_error (errnum, message, line)
    ...
    char_u * line;
{
    vim_snprintf ((char *) IObuff, (1024 + 1), ((char *) ("%sviminfo: %s in line: ")), errnum, message);
    ((__builtin_object_size ((char *) (IObuff), 0) != (size_t) - 1) ? __builtin___strncat_chk ((char *) (IObuff), (char *) (line), (size_t) ((1024 + 1) - strlen
      ((char *) (IObuff)) - 1), __builtin_object_size ((char *) (IObuff), 1 > 1)) : __inline_strncat_chk ((char *) (IObuff), (char *) (line), (size_t) ((1024 +
      1) - strlen ((char *) (IObuff)) - 1)));
    if (IObuff [strlen ((char *) (IObuff)) - 1] == '\n') IObuff [strlen ((char *) (IObuff)) - 1] = '\000';
    emsg (IObuff);
    if (++viminfo_errcnt >= 10) {
        emsg ((char_u *) (((char *) ("E136: viminfo: Too many errors, skipping rest of file"))));
        return 1;
    }
    return 0;
}
    ...
    int flags;
{
    FILE * fp;
    char_u * fname;
    if (no_viminfo ()) return 0;
    fname = viminfo_filename (file);
    if (fname == ((void *) 0)) return 0;
    fp = fopen (((char *) fname), ("rb"));
    if (p_verbose > 0) {
        verbose_enter ();
        smsg ((char_u *) ((char *) ("Reading viminfo file \"%s\"%s%s%s")), fname, (flags & 1) ? ((char *) (" info")) : "", (flags & 2) ? ((char *) (" marks")) :
          "", (flags & 8) ? ((char *) (" oldfiles")) : "", fp == ((void *) 0) ? ((char *) (" FAILED")) : "");
        verbose_leave ();
    }
    vim_free (fname);
    ...
    int forceit;
{
    ...
    int shortname = 0;
    struct stat st_old;
    if (no_viminfo ()) return;
    fname = viminfo_filename (file);
    if (fname == ((void *) 0)) return;
    fp_in = fopen (((char *) fname), ("rb"));
    if (fp_in == ((void *) 0)) {
        if (stat (((char *) fname), (&st_new)) == 0) goto end;
        ...
        st_old.st_ino = 0;
        st_old.st_mode = 0600;
        if (stat (((char *) fname), (&st_old)) == 0 && getuid () != 0 && !(st_old.st_uid==getuid()?(st_old.st_mode&0200):(st_old.st_gid==getgid()?(st_old.
          st_mode&0020):(st_old.st_mode&0002)))) {
            int tt = msg_didany;
            emsg2 ((char_u *) (((char *) ("E137: Viminfo file is not writable: %s"))), (char_u *) (fname));
            msg_didany = tt;
            fclose (fp_in);
            goto end;
        }
        for (;;) {
            tempname = buf_modname (shortname, fname, (char_u *) ".tmp", 0);
            if (tempname == ((void *) 0)) break;
            if (stat (((char *) tempname), (&st_new)) == 0) {
                ...
                if (wp < gettail (tempname)) wp = gettail (tempname);
        ...
        if (tempname != ((void *) 0)) {
            int fd;
            umask_save = umask (0);
            fd = open (((char *) tempname), (0x0200 | 0 | 0x0800 | 0x0001 | 0x0100), ((int) ((st_old.st_mode & 0777) | 0600)));
            (void) umask (umask_save);
            if (fd < 0) fp_out = ((void *) 0);
            else fp_out = fdopen (fd, "wb");
            if (fp_out == ((void *) 0)) {
                vim_free (tempname);
                if ((tempname = vim_tempname ('o')) != ((void *) 0)) fp_out = fopen (((char *) tempname), ("wb"));
            }
            if (fp_out != ((void *) 0)) ignored = fchown (fileno (fp_out), st_old.st_uid, st_old.st_gid);
        }
    }
    if (fp_out == ((void *) 0)) {
        emsg2 ((char_u *) (((char *) ("E138: Can't write viminfo file %s!"))), (char_u *) ((fp_in == ((void *) 0) || tempname == ((void *) 0)) ? fname :
          tempname));
        if (fp_in != ((void *) 0)) fclose (fp_in);
        goto end;
    }
    if (p_verbose > 0) {
        verbose_enter ();
        smsg ((char_u *) ((char *) ("Writing viminfo file \"%s\"")), fname);
        verbose_leave ();
    }
    viminfo_errcnt = 0;
    ...
    if (fp_in != ((void *) 0)) {
        fclose (fp_in);
        if (viminfo_errcnt || vim_rename (tempname, fname) == -1) unlink ((char *) (tempname));
    }
    end : vim_free (fname);
    vim_free (tempname);
}
    ...
    char_u * file;
{
    if (file == ((void *) 0) || *file == '\000') {
        if (use_viminfo != ((void *) 0)) file = use_viminfo;
    ...
    int flags;
{
    ...
    int eof = 0;
    vir_T vir;
    if ((vir.vir_line = alloc (512)) == ((void *) 0)) return;
    vir.vir_fd = fp_in;
    if (fp_in != ((void *) 0)) {
        if (flags & 1) eof = read_viminfo_up_to_marks (&vir, flags & 4, fp_out != ((void *) 0));
        else while (!(eof=viminfo_readline(&vir)) && vir.vir_line [0] != '>');
    }
    if (fp_out != ((void *) 0)) {
        fprintf (fp_out, ((char *) ("# This viminfo file was generated by Vim %s.\n")), "7.3");
        fputs (((char *) ("# You may edit it if you're careful!\n\n")), fp_out);
    ...
    if (fp_in != ((void *) 0) && (flags & (2 | 8 | 4))) copy_viminfo_marks (&vir, fp_out, count, eof, flags);
    ...
    int writing;
{
    ...
    while (!eof && virp -> vir_line [0] != '>') {
        ...
        case '\'' :
            ...
            break;
        default :
            if (viminfo_error ("E575: ", ((char *) ("Illegal starting char")), virp -> vir_line)) eof = 1;
            else eof = viminfo_readline (virp);
            break;
        }
    }
    finish_viminfo_history ();
    for (buf = firstbuf; buf != ((void *) 0); buf = buf -> b_next) fmarks_check_names (buf);
    return eof;
}
...
char_u * viminfo_readstring (virp, off, convert)
    vir_T * virp;
    int off;
    int convert __attribute__ ((unused));
{
    char_u * retval;
    char_u * s, * d;
    long len;
    if (virp -> vir_line [off] == 22 && vim_isdigit (virp -> vir_line [off + 1])) {
    ...
    exarg_T * eap __attribute__ ((unused));
{
    ...
    add_termcode ((char_u *) "kD", p != ((void *) 0) && *p == 0x7f ? (char_u *) "\010" : (char_u *) "\177", 0);
}
    ...
    exarg_T * eap;
{
    ...
    buf_T * buf;
    if (eap -> addr_count > 0 && (*eap->arg != '\000' || eap -> line2 > 0 || eap -> addr_count > 1)) {
        emsg ((char_u *) (((char *) (e_invarg))));
        return;
    }
    if (*eap->arg != '\000' || eap -> addr_count == 1) {
        ...
        if (setfname (curbuf, eap -> arg, ((void *) 0), 1) == 0) {
        ...
        if (xfname != ((void *) 0) && *xfname != '\000') {
            ...
            if (buf != ((void *) 0) && !cmdmod.keepalt) curwin -> w_alt_fnum = buf -> b_fnum;
        }
        vim_free (fname);
        vim_free (sfname);
...
void ex_update (eap)
    exarg_T * eap;
{
    if (curbufIsChanged ()) (void) do_write (eap);
}

void ex_write (eap)
    exarg_T * eap;
{
    if (eap -> usefilter) do_bang (1, eap, 0, 1, 0);
    ...
    exarg_T * eap;
{
    ...
    if (*ffname == '\000') {
        ...
        if (free_fname != ((void *) 0)) ffname = free_fname;
        other = otherfile (ffname);
    }
    if (other) {
        if (vim_strchr (p_cpo, 'A') != ((void *) 0) || eap -> cmdidx == CMD_saveas) alt_buf = setaltfname (ffname, fname, (linenr_T) 1);
    ...
    if (!other && (bt_dontwrite_msg (curbuf) || check_fname () == 0 || check_readonly (&eap->forceit, curbuf))) goto theend;
    if (!other) {
        ffname = curbuf -> b_ffname;
        fname = curbuf -> b_fname;
        if ((eap -> line1 != 1 || eap -> line2 != curbuf -> b_ml.ml_line_count) && !eap->forceit && !eap->append && !p_wa) {
            if (p_confirm || cmdmod.confirm) {
                if (vim_dialog_yesno (4, ((void *) 0), (char_u *) ((char *) ("Write partial file?")), 2) != 2) goto theend;
                eap -> forceit = 1;
            }
            else {
                emsg ((char_u *) (((char *) ("E140: Use ! to write partial buffer"))));
                goto theend;
            }
        }
    }
    if (check_overwrite (eap, curbuf, fname, ffname, other) == 1) {
        if (eap -> cmdidx == CMD_saveas && alt_buf != ((void *) 0)) {
            ...
            if (*curbuf->b_p_ft == '\000') {
                if (au_has_group ((char_u *) "filetypedetect")) (void) do_doautocmd ((char_u *) "filetypedetect BufRead", 1);
                do_modelines (0);
            }
        }
        retval = buf_write (curbuf, ffname, fname, eap -> line1, eap -> line2, eap, eap -> append, eap -> forceit, 1, 0);
        if (eap -> cmdidx == CMD_saveas) {
            if (retval == 1) {
                curbuf -> b_p_ro = 0;
                redraw_tabline = 1;
    ...
    theend : vim_free (free_fname);
    return retval;
}

static int check_overwrite (eap, buf, fname, ffname, other)
    exarg_T * eap;
    buf_T * buf;
    char_u * fname;
    char_u * ffname;
    int other;
{
    if ((other || (buf -> b_flags & 0x08) || ((buf -> b_flags & 0x10) && vim_strchr (p_cpo, 'O') == ((void *) 0)) || (buf -> b_flags & 0x40)) && !p_wa && 
      !bt_nofile(buf) && vim_fexists (ffname)) {
        if (!eap->forceit && !eap->append) {
            if (mch_isdir (ffname)) {
                emsg2 ((char_u *) (((char *) (e_isadir2))), (char_u *) (ffname));
                return 0;
            }
            if (p_confirm || cmdmod.confirm) {
                char_u buff [(1024 + 1)];
                dialog_msg (buff, ((char *) ("Overwrite existing file \"%s\"?")), fname);
                if (vim_dialog_yesno (4, ((void *) 0), buff, 2) != 2) return 0;
                eap -> forceit = 1;
            }
            else {
        ...
        if (other && !emsg_silent) {
            ...
            int r;
            char_u * swapname;
            if (*p_dir == '\000') ((__builtin_object_size ((char *) (dir), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (dir), (char *) ("."),
              __builtin_object_size ((char *) (dir), 1 > 1)) : __inline_strcpy_chk ((char *) (dir), (char *) (".")));
            else {
                p = p_dir;
            ...
            if (r) {
                if (p_confirm || cmdmod.confirm) {
                    ...
                    emsg2 ((char_u *) (((char *) ("E768: Swap file exists: %s (:silent! overrides)"))), (char_u *) (swapname));
                    vim_free (swapname);
                    return 0;
                }
            }
...
{
    int i;
    if (eap -> cmd [1] == 'n') i = curwin -> w_arg_idx + (int) eap -> line2;
    else i = curwin -> w_arg_idx - (int) eap -> line2;
    eap -> line1 = 1;
    eap -> line2 = curbuf -> b_ml.ml_line_count;
    if (do_write (eap) != 0) do_argfile (eap, i);
}
    ...
    exarg_T * eap;
{
    ...
    if (eap -> cmdidx == CMD_xall || eap -> cmdidx == CMD_wqall) exiting = 1;
    for (buf = firstbuf; buf != ((void *) 0); buf = buf -> b_next) {
        if (bufIsChanged (buf)) {
            ...
            if (buf -> b_ffname == ((void *) 0)) {
                emsgn ((char_u *) (((char *) ("E141: No file name for buffer %ld"))), (long) ((long) buf -> b_fnum));
                ++error;
            }
            else if (check_readonly (&eap->forceit, buf) || check_overwrite (eap, buf, buf -> b_fname, buf -> b_ffname, 0) == 0) {
                ++error;
            }
            else {
                if (buf_write_all (buf, eap -> forceit) == 0) ++error;
                if (!buf_valid(buf)) buf = firstbuf;
            }
...
int not_writing () {
    if (p_write) return 0;
    emsg ((char_u *) (((char *) ("E142: File not written: Writing is disabled by 'write' option"))));
    return 1;
}
    ...
    buf_T * buf;
{
    ...
    if (!*forceit && (buf -> b_p_ro || (stat (((char *) buf -> b_ffname), (&st)) >= 0 && check_file_readonly (buf -> b_ffname, 0777)))) {
        if ((p_confirm || cmdmod.confirm) && buf -> b_fname != ((void *) 0)) {
            char_u buff [(1024 + 1)];
            if (buf -> b_p_ro) dialog_msg (buff, ((char *) ("'readonly' option is set for \"%s\".\nDo you wish to write anyway?")), buf -> b_fname);
            else dialog_msg (buff, ((char *) ("File permissions of \"%s\" are read-only.\nIt may still be possible to write it.\nDo you wish to try?")), buf ->
              b_fname);
            if (vim_dialog_yesno (4, ((void *) 0), buff, 2) == 2) {
                * forceit = 1;
                return 0;
        ...
        else if (buf -> b_p_ro) emsg ((char_u *) (((char *) (e_readonly))));
        else emsg2 ((char_u *) (((char *) ("E505: \"%s\" is read-only (add ! to override)"))), (char_u *) (buf -> b_fname));
        return 1;
    }
    return 0;
}
    ...
    int forceit;
{
    ...
    if (text_locked ()) return 1;
    if (curbuf_locked ()) return 1;
    if (fnum == 0) {
    ...
    if (other && !forceit && curbuf -> b_nwindows == 1 && !(buf_hide(curbuf)) && curbufIsChanged () && autowrite (curbuf, forceit) == 0) {
        if (p_confirm && p_write) dialog_changed (curbuf, 0);
        if (curbufIsChanged ()) {
            if (other) --no_wait_return;
    ...
    if (setpm) setpcmark ();
    if (!other) {
        if (lnum != 0) curwin -> w_cursor.lnum = lnum;
        check_cursor_lnum ();
        beginline (2 | 4);
        retval = 0;
    }
    else if (do_ecmd (fnum, ffname, sfname, ((void *) 0), lnum, ((buf_hide (curbuf)) ? 0x01 : 0) + (forceit ? 0x08 : 0), curwin) == 1) retval = -1;
    else retval = 1;
    theend : vim_free (free_me);
    return retval;
}
    ...
    win_T * oldwin;
{
    ...
    if (eap != ((void *) 0)) command = eap -> do_ecmd_cmd;
    if (fnum != 0) {
        if (fnum == curbuf -> b_fnum) return 1;
        other_file = 1;
    }
    else {
        if (sfname == ((void *) 0)) sfname = ffname;
        if (sfname != ((void *) 0)) fname_case (sfname, 0);
        if ((flags & 0x10) && (ffname == ((void *) 0) || *ffname == '\000')) goto theend;
        if (ffname == ((void *) 0)) other_file = 1;
        else if (*ffname == '\000' && curbuf -> b_ffname == ((void *) 0)) other_file = 0;
        else {
            if (*ffname == '\000') {
                ffname = curbuf -> b_ffname;
    ...
    if (((!other_file && !(flags&0x04)) || (curbuf -> b_nwindows == 1 && !(flags&(0x01|0x10)))) && check_changed (curbuf, p_awa, !other_file, (flags & 0x08), 0
      )) {
        if (fnum == 0 && other_file && ffname != ((void *) 0)) (void) setaltfname (ffname, sfname, newlnum < 0 ? 0 : newlnum);
        goto theend;
    }
    reset_VIsual ();
    if ((command != ((void *) 0) || newlnum > (linenr_T) 0) && *get_vim_var_str(47) == '\000') {
        int len;
        char_u * p;
        if (command != ((void *) 0)) len = (int) strlen ((char *) (command)) + 3;
        else len = 30;
        p = alloc ((unsigned) len);
        if (p != ((void *) 0)) {
            if (command != ((void *) 0)) vim_snprintf ((char *) p, len, ":%s\r", command);
            else vim_snprintf ((char *) p, len, "%ldG", (long) newlnum);
            set_vim_var_string (47, p, -1);
    ...
    if (other_file) {
        if (!(flags&0x10)) {
            if (!cmdmod.keepalt) curwin -> w_alt_fnum = curbuf -> b_fnum;
            if (oldwin != ((void *) 0)) buflist_altfpos (oldwin);
        }
        if (fnum) buf = buflist_findnr (fnum);
        else {
            if (flags & 0x10) {
                linenr_T tlnum = 1L;
                if (command != ((void *) 0)) {
                ...
                (void) buflist_new (ffname, sfname, tlnum, 2);
                goto theend;
            }
            buf = buflist_new (ffname, sfname, 0L, 1 | ((flags & 0x02) ? 0 : 2));
        }
        if (buf == ((void *) 0)) goto theend;
        if (buf -> b_ml.ml_mfp == ((void *) 0)) {
            ...
            if (!buf_valid(buf) || curbuf != old_curbuf) goto theend;
        ...
        if (buf != curbuf) {
            if (buf -> b_fname != ((void *) 0)) new_name = vim_strsave (buf -> b_fname);
            au_new_curbuf = buf;
            ...
            if (buf == curbuf) auto_buf = 1;
            else {
                if (curbuf == old_curbuf) buf_copy_options (buf, 1);
                u_sync (0);
                close_buffer (oldwin, curbuf, (flags & 0x01) ? 0 : 2);
                if (aborting ()) {
                    vim_free (new_name);
                    goto theend;
                }
                ...
                if (buf == curbuf) auto_buf = 1;
                    ...
                    if (curwin -> w_s == &(curwin->w_buffer->b_s)) curwin -> w_s = &(buf->b_s);
                    curwin -> w_buffer = buf;
                    curbuf = buf;
                    ++curbuf->b_nwindows;
                    if (*p_ffs && !oldbuf) set_fileformat (default_fileformat (), 4);
                }
                get_winopts (curbuf);
                did_get_winopts = 1;
            }
        ...
        else ++curbuf->b_nwindows;
        curwin -> w_pcmark.lnum = 1;
        curwin -> w_pcmark.col = 0;
    }
    else {
        if ((flags & 0x10) || check_fname () == 0) goto theend;
        oldbuf = (flags & 0x04);
    }
    if ((flags & 0x02) || keep_help_flag) {
        char_u * p;
        curbuf -> b_help = 1;
        set_string_option_direct ((char_u *) "buftype", -1, (char_u *) "help", 1 | 4, 0);
        p = (char_u *) "!-~,^*,^|,^\",192-255";
        ...
        buf = curbuf;
        if (!curbuf->b_help) set_buflisted (1);
    }
    if (buf != curbuf) goto theend;
    if (aborting ()) goto theend;
    did_filetype = 0;
    if (!other_file && !oldbuf) {
        ...
        buf = curbuf;
        if (buf -> b_fname != ((void *) 0)) new_name = vim_strsave (buf -> b_fname);
        else new_name = ((void *) 0);
        if (p_ur < 0 || curbuf -> b_ml.ml_line_count <= p_ur) {
            u_sync (0);
            if (u_savecommon (0, curbuf -> b_ml.ml_line_count + 1, 0, 1) == 0) goto theend;
            u_unchanged (curbuf);
            buf_freeall (curbuf, 4);
            readfile_flags = 0x20;
        }
    ...
    if (!auto_buf) {
        curwin_init ();
        {
            win_T * win;
            tabpage_T * tp;
            for ((tp) = first_tabpage; (tp) != ((void *) 0); (tp) = (tp) -> tp_next) for ((win) = ((tp) == curtab) ? firstwin : (tp) -> tp_firstwin; (win); (
              win) = (win) -> w_next) if (win -> w_buffer == curbuf) foldUpdateAll (win);
        }
        if (p_acd) do_autochdir ();
        ...
        if (!oldbuf) {
            ...
            curbuf -> b_flags |= 0x02;
            if (should_abort (open_buffer (0, eap, readfile_flags))) retval = 0;
            if (swap_exists_action == 2) retval = 0;
            handle_swap_exists (old_curbuf);
        }
        else {
            do_modelines (16);
            apply_autocmds_retval (EVENT_BUFENTER, ((void *) 0), ((void *) 0), 0, curbuf, &retval);
            apply_autocmds_retval (EVENT_BUFWINENTER, ((void *) 0), ((void *) 0), 0, curbuf, &retval);
        }
        check_arg_idx (curwin);
        ...
        if (curwin -> w_topline == topline) topline = 0;
        changed_line_abv_curs ();
        maketitle ();
    }
    ...
    if (did_get_winopts && curwin -> w_onebuf_opt.wo_spell && *curwin->w_s->b_p_spl != '\000') (void) did_set_spelllang (curwin);
    if (command == ((void *) 0)) {
        if (newcol >= 0) {
            ...
            if (exmode_active) curwin -> w_cursor.lnum = curbuf -> b_ml.ml_line_count;
            beginline (1 | 4);
        }
    }
    check_lnums (0);
    ...
    if (command != ((void *) 0)) do_cmdline (command, ((void *) 0), ((void *) 0), 0x01);
    --RedrawingDisabled;
    if (!skip_redraw) {
        n = p_so;
        if (topline == 0 && command == ((void *) 0)) p_so = 999;
        update_topline ();
        curwin -> w_scbind_pos = curwin -> w_topline;
        p_so = n;
        redraw_curbuf_later (40);
    }
    if (p_im) need_start_insertmode = 1;
    if (p_acd) do_autochdir ();
    if (curbuf -> b_ffname != ((void *) 0)) {
        if ((flags & 0x02) != 0x02) netbeans_file_opened (curbuf);
    }
    theend : if (did_set_swapcommand) set_vim_var_string (47, ((void *) 0), -1);
    vim_free (free_fname);
    return retval;
}
...
static void delbuf_msg (name)
    char_u * name;
{
    emsg2 ((char_u *) (((char *) ("E143: Autocommands unexpectedly deleted new buffer %s"))), (char_u *) (name == ((void *) 0) ? (char_u *) "" : name));
    vim_free (name);
    au_new_curbuf = ((void *) 0);
}
    ...
    exarg_T * eap;
{
    ...
    if (eap -> cmdidx != CMD_change && curbuf -> b_p_ai && lnum > 0) append_indent = get_indent_lnum (lnum);
    if (eap -> cmdidx != CMD_append) --lnum;
    if (empty && lnum == 1) lnum = 0;
    State = 0x10;
    if (curbuf -> b_p_iminsert == 1) State |= 0x20;
    for (;;) {
        msg_scroll = 1;
        need_wait_return = 0;
        if (curbuf -> b_p_ai) {
            ...
            else if (lnum > 0) indent = get_indent_lnum (lnum);
        }
        ex_keep_indent = 0;
        if (eap -> getline == ((void *) 0)) {
            if (eap -> nextcmd == ((void *) 0) || *eap->nextcmd == '\000') break;
            p = vim_strchr (eap -> nextcmd, '\012');
            if (p == ((void *) 0)) p = eap -> nextcmd + strlen ((char *) (eap -> nextcmd));
            theline = vim_strnsave (eap -> nextcmd, (int) (p - eap -> nextcmd));
            if (*p != '\000') ++p;
            eap -> nextcmd = p;
        }
        else theline = eap -> getline (eap -> cstack -> cs_looplevel > 0 ? -1 : '\000', eap -> cookie, indent);
        lines_left = Rows - 1;
        if (theline == ((void *) 0)) break;
        if (ex_keep_indent) append_indent = indent;
        vcol = 0;
        for (p = theline; indent > vcol; ++p) {
            if (*p == ' ') ++vcol;
            else if (*p == '\011') vcol += 8 - vcol % 8;
            else break;
        }
        if ((p [0] == '.' && p [1] == '\000') || (!did_undo && u_save (lnum, lnum + 1 + (empty ? 1 : 0)) == 0)) {
            vim_free (theline);
            break;
        }
        if (p [0] == '\000') theline [0] = '\000';
        did_undo = 1;
    ...
    if (eap -> cmdidx != CMD_append) --curbuf->b_op_start.lnum;
...
{
    linenr_T lnum;
    if (eap -> line2 >= eap -> line1 && u_save (eap -> line1 - 1, eap -> line2 + 1) == 0) return;
    if (eap -> forceit ? !curbuf->b_p_ai : curbuf -> b_p_ai) append_indent = get_indent_lnum (eap -> line1);
    ...
    exarg_T * eap;
{
    ...
    int j;
    linenr_T lnum = eap -> line2;
    if (eap -> forceit) bigness = curwin -> w_height;
    else if (firstwin == lastwin) bigness = curwin -> w_onebuf_opt.wo_scr * 2;
    else bigness = curwin -> w_height - 3;
    if (bigness < 1) bigness = 1;
    x = eap -> arg;
    kind = x;
    if (*kind == '-' || *kind == '+' || *kind == '=' || *kind == '^' || *kind == '.') ++x;
    while (*x == '-' || *x == '+') ++x;
    if (*x != 0) {
        if (!((*x) >= '0' && (*x) <= '9')) {
            emsg ((char_u *) (((char *) ("E144: non-numeric argument to :z"))));
            return;
        }
        else {
            bigness = atoi ((char *) x);
    ...
    if (*kind == '-' || *kind == '+') for (x = kind + 1; *x == *kind; ++x);
    switch (*kind) {
    case '-' :
    ...
    default :
        start = lnum;
        if (*kind == '+') start += bigness * (linenr_T) (x - kind - 1) + 1;
        else if (eap -> addr_count == 0) ++start;
        end = start + bigness - 1;
        curs = end;
        break;
    }
    if (start < 1) start = 1;
    if (end > curbuf -> b_ml.ml_line_count) end = curbuf -> b_ml.ml_line_count;
    if (curs > curbuf -> b_ml.ml_line_count) curs = curbuf -> b_ml.ml_line_count;
    for (i = start; i <= end; i ++) {
        if (minus && i == lnum) {
            msg_putchar ('\n');
            for (j = 1; j < Columns; j ++) msg_putchar ('-');
        }
...
int check_restricted () {
    if (restricted) {
        emsg ((char_u *) (((char *) ("E145: Shell commands not allowed in rvim"))));
        return 1;
    }
    return 0;
}
    ...
    exarg_T * eap;
{
    ...
    if (eap -> cmdidx == CMD_tilde) which_pat = 2;
    else which_pat = 1;
    if (eap -> cmd [0] == 's' && *cmd != '\000' && !((*cmd) == ' ' || (*cmd) == '\t') && vim_strchr ((char_u *) "0123456789cegriIp|\"", *cmd) == ((void *) 0)) {
        if (isalpha (*cmd)) {
            emsg ((char_u *) (((char *) ("E146: Regular expressions can't be delimited by letters"))));
            return;
        }
        if (*cmd == '\\') {
            ++cmd;
        ...
        while (cmd [0]) {
            ...
            if (cmd [0] == '\\' && cmd [1] != 0) ++cmd;
            ++cmd;
        }
        if (!eap->skip) {
            if (strcmp ((char *) (sub), (char *) ("%")) == 0 && vim_strchr (p_cpo, '/') != ((void *) 0)) {
                if (old_sub == ((void *) 0)) {
                    emsg ((char_u *) (((char *) (e_nopresub))));
                    return;
                }
                sub = old_sub;
            }
        ...
        pat = ((void *) 0);
        sub = old_sub;
        endcolumn = (curwin -> w_curswant == (0x7fffffffL));
    }
    if (*cmd == '&') ++cmd;
    else {
        if (!p_ed) {
    ...
    while (*cmd) {
        if (*cmd == 'g') do_all = !do_all;
        else if (*cmd == 'c') do_ask = !do_ask;
        else if (*cmd == 'n') do_count = 1;
        else if (*cmd == 'e') do_error = !do_error;
        else if (*cmd == 'r') which_pat = 2;
        else if (*cmd == 'p') do_print = 1;
        else if (*cmd == '#') {
            do_print = 1;
            do_number = 1;
        }
        ...
        else if (*cmd == 'i') do_ic = 'i';
        else if (*cmd == 'I') do_ic = 'I';
        else break;
        ++cmd;
    }
    if (do_count) do_ask = 0;
    cmd = skipwhite (cmd);
    if (((*cmd) >= '0' && (*cmd) <= '9')) {
        i = getdigits (&cmd);
        if (i <= 0 && !eap->skip && do_error) {
            emsg ((char_u *) (((char *) (e_zerocount))));
            return;
        }
        eap -> line1 = eap -> line2;
        eap -> line2 += i - 1;
        if (eap -> line2 > curbuf -> b_ml.ml_line_count) eap -> line2 = curbuf -> b_ml.ml_line_count;
    }
    cmd = skipwhite (cmd);
    if (*cmd && *cmd != '"') {
    ...
    if (search_regcomp (pat, 1, which_pat, 0x20, &regmatch) == 0) {
        if (do_error) emsg ((char_u *) (((char *) (e_invcmd))));
        return;
    }
    if (do_ic == 'i') regmatch.rmm_ic = 1;
    else if (do_ic == 'I') regmatch.rmm_ic = 0;
    sub_firstline = ((void *) 0);
    if (!(sub[0]=='\\'&&sub[1]=='=')) sub = regtilde (sub, p_magic);
    line2 = eap -> line2;
    for (lnum = eap -> line1; lnum <= line2 && !(got_quit||aborting()); ++lnum) {
        nmatch = vim_regexec_multi (&regmatch, curwin, curbuf, lnum, (colnr_T) 0, ((void *) 0));
        if (nmatch) {
            colnr_T copycol;
            colnr_T matchcol;
            colnr_T prev_matchcol = (0x7fffffffL);
            ...
            for (;;) {
                ...
                if (matchcol == prev_matchcol && regmatch.endpos [0].lnum == 0 && matchcol == regmatch.endpos [0].col) {
                    if (sub_firstline [matchcol] == '\000') skip_match = 1;
                    else ++matchcol;
                    goto skip;
                }
                ...
                if (do_ask) {
                    ...
                    if (vim_strchr (p_cpo, 'u') != ((void *) 0)) ++no_u_sync;
                    while (do_ask) {
                        if (exmode_active) {
                            char_u * resp;
                            colnr_T sc, ec;
                            print_line_no_prefix (lnum, 0, 0);
                            getvcol (curwin, &curwin->w_cursor, &sc, ((void *) 0), ((void *) 0));
                            curwin -> w_cursor.col = regmatch.endpos [0].col - 1;
                            getvcol (curwin, &curwin->w_cursor, ((void *) 0), ((void *) 0), &ec);
                            msg_start ();
                            for (i = 0; i < (long) sc; ++i) msg_putchar (' ');
                            for (; i <= (long) ec; ++i) msg_putchar ('^');
                            ...
                            if (msg_row == Rows - 1) msg_didout = 0;
                            msg_starthere ();
                            i = msg_scroll;
                            msg_scroll = 0;
                            msg_no_more = 1;
                            smsg_attr (highlight_attr [(int) (HLF_R)], (char_u *) ((char *) ("replace with %s (y/n/a/q/l/^E/^Y)?")), sub);
                            msg_no_more = 0;
                            msg_scroll = i;
                            showruler (1);
                            windgoto (msg_row, msg_col);
                        ...
                        need_wait_return = 0;
                        if (typed == 'q' || typed == '\033' || typed == 3 || typed == intr_char) {
                            got_quit = 1;
                            break;
                        }
                        if (typed == 'n') break;
                        ...
                        if (typed == 5) scrollup_clamp ();
                        else if (typed == 25) scrolldown_clamp ();
                    }
                    State = save_State;
                    setmouse ();
                    if (vim_strchr (p_cpo, 'u') != ((void *) 0)) --no_u_sync;
                ...
                curwin -> w_cursor.col = regmatch.startpos [0].col;
                sublen = vim_regsub_multi (&regmatch, sub_firstlnum - regmatch.startpos [0].lnum, sub, sub_firstline, 0, p_magic, 1);
                ...
                needed_len = copy_len + ((unsigned) strlen ((char *) (p1)) - regmatch.endpos [0].col) + sublen + 1;
                if (new_start == ((void *) 0)) {
                    new_start_len = needed_len + 50;
                    if ((new_start = alloc_check (new_start_len)) == ((void *) 0)) goto outofmem;
                    * new_start = '\000';
                    new_end = new_start;
                }
                else {
                    ...
                    if (needed_len > (int) new_start_len) {
                        new_start_len = needed_len + 50;
                        if ((p1 = alloc_check (new_start_len)) == ((void *) 0)) {
                            vim_free (new_start);
                            goto outofmem;
                        }
                        ((__builtin_object_size ((char *) (p1), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (p1), (char *) (new_start), (size_t) (
                          len + 1), __builtin_object_size ((char *) (p1), 0)) : __inline_memmove_chk ((char *) (p1), (char *) (new_start), (size_t) (len + 1)))
                          ;
                        vim_free (new_start);
                        new_start = p1;
                    }
                ...
                ((__builtin_object_size ((char *) (new_end), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (new_end), (char *) (sub_firstline +
                  copycol), (size_t) copy_len, __builtin_object_size ((char *) (new_end), 0)) : __inline_memmove_chk ((char *) (new_end), (char *) (
                  sub_firstline + copycol), (size_t) copy_len));
                new_end += copy_len;
                (void) vim_regsub_multi (&regmatch, sub_firstlnum - regmatch.startpos [0].lnum, sub, new_end, 1, p_magic, 1);
                sub_nsubs ++;
                did_sub = 1;
                curwin -> w_cursor.col = 0;
                if (nmatch > 1) {
                    ...
                    vim_free (sub_firstline);
                    sub_firstline = vim_strsave (ml_get (sub_firstlnum));
                    if (sub_firstlnum <= line2) do_again = 1;
                    else do_all = 0;
                }
                ...
                for (p1 = new_end; *p1; ++p1) {
                    if (p1 [0] == '\\' && p1 [1] != '\000') ((__builtin_object_size ((char *) ((p1)), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (
                      (p1)), (char *) ((p1 + 1)), strlen ((char *) (p1 + 1)) + 1, __builtin_object_size ((char *) ((p1)), 0)) : __inline_memmove_chk ((char *) (
                      (p1)), (char *) ((p1 + 1)), strlen ((char *) (p1 + 1)) + 1));
                    else if (*p1 == '\015') {
                        if (u_inssub (lnum) == 1) {
                            * p1 = '\000';
                            ml_append (lnum - 1, new_start, (colnr_T) (p1 - new_start + 1), 0);
                            mark_adjust (lnum + 1, (linenr_T) (0x7fffffffL), 1L, 0L);
                            if (do_ask) appended_lines (lnum - 1, 1L);
                            else {
                                if (first_line == 0) first_line = lnum;
                                last_line = lnum + 1;
                            }
                            ...
                            ++line2;
                            ++curwin->w_cursor.lnum;
                            ((__builtin_object_size ((char *) ((new_start)), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) ((new_start)), (char *) ((
                              p1 + 1)), strlen ((char *) (p1 + 1)) + 1, __builtin_object_size ((char *) ((new_start)), 0)) : __inline_memmove_chk ((char *) ((
                              new_start)), (char *) ((p1 + 1)), strlen ((char *) (p1 + 1)) + 1));
                            p1 = new_start - 1;
                        }
                    }
                }
                skip : lastone = (skip_match || got_int || got_quit || lnum > line2 || !(do_all||do_again) || (sub_firstline [matchcol] == '\000' && nmatch <= 1
                  && !re_multiline(regmatch.regprog)));
                nmatch = -1;
                if (lastone || (do_ask && !re_lookbehind(regmatch.regprog)) || nmatch_tl > 0 || (nmatch = vim_regexec_multi (&regmatch, curwin, curbuf,
                  sub_firstlnum, matchcol, ((void *) 0))) == 0 || regmatch.startpos [0].lnum > 0) {
                    if (new_start != ((void *) 0)) {
                        ((__builtin_object_size ((char *) (new_start), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (new_start), (char *) (
                          sub_firstline + copycol), __builtin_object_size ((char *) (new_start), 1 > 1)) : __inline_strcat_chk ((char *) (new_start), (char *) (
                          sub_firstline + copycol)));
                        matchcol = (colnr_T) strlen ((char *) (sub_firstline)) - matchcol;
                        prev_matchcol = (colnr_T) strlen ((char *) (sub_firstline)) - prev_matchcol;
                        if (u_savesub (lnum) != 1) break;
                        ml_replace (lnum, new_start, 1);
                        if (nmatch_tl > 0) {
                            ++lnum;
                            if (u_savedel (lnum, nmatch_tl) != 1) break;
                            for (i = 0; i < nmatch_tl; ++i) ml_delete (lnum, (int) 0);
                            mark_adjust (lnum, lnum + nmatch_tl - 1, (long) (0x7fffffffL), -nmatch_tl);
                            if (do_ask) deleted_lines (lnum, nmatch_tl);
                            --lnum;
                            line2 -= nmatch_tl;
                            nmatch_tl = 0;
                        }
                        if (do_ask) changed_bytes (lnum, 0);
                        else {
                            if (first_line == 0) first_line = lnum;
                            last_line = lnum + 1;
                        }
                    ...
                    if (nmatch == -1 && !lastone) nmatch = vim_regexec_multi (&regmatch, curwin, curbuf, sub_firstlnum, matchcol, ((void *) 0));
                    if (nmatch <= 0) {
                        if (nmatch == -1) lnum -= regmatch.startpos [0].lnum;
                        break;
                    }
                }
                line_breakcheck ();
            }
            if (did_sub) ++sub_nlines;
            vim_free (new_start);
            vim_free (sub_firstline);
            sub_firstline = ((void *) 0);
        }
    ...
    outofmem : vim_free (sub_firstline);
    if (do_count) curwin -> w_cursor = old_cursor;
    if (sub_nsubs > start_nsubs) {
        curbuf -> b_op_start.lnum = eap -> line1;
        curbuf -> b_op_end.lnum = line2;
        curbuf -> b_op_start.col = curbuf -> b_op_end.col = 0;
        if (!global_busy) {
            if (endcolumn) coladvance ((colnr_T) (0x7fffffffL));
            else beginline (1 | 4);
            if (!do_sub_msg(do_count) && do_ask) msg ((char_u *) (""));
        }
        else global_need_beginline = 1;
        if (do_print) print_line (curwin -> w_cursor.lnum, do_number, do_list);
    }
    else if (!global_busy) {
        if (got_int) emsg ((char_u *) (((char *) (e_interr))));
        else if (got_match) msg ((char_u *) (""));
        else if (do_error) emsg2 ((char_u *) (((char *) (e_patnotf2))), (char_u *) (get_search_pat ()));
    }
    vim_free (regmatch.regprog);
}
    ...
    int count_only;
{
    if (((sub_nsubs > p_report && (KeyTyped || sub_nlines > 1 || p_report < 1)) || count_only) && messaging ()) {
        if (got_int) ((__builtin_object_size ((char *) (msg_buf), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (msg_buf), (char *) (((char *) (
          "(Interrupted) "))), __builtin_object_size ((char *) (msg_buf), 1 > 1)) : __inline_strcpy_chk ((char *) (msg_buf), (char *) (((char *) (
          "(Interrupted) ")))));
        else *msg_buf = '\000';
        if (sub_nsubs == 1) vim_snprintf_add ((char *) msg_buf, sizeof (msg_buf), "%s", count_only ? ((char *) ("1 match")) : ((char *) ("1 substitution")));
        else vim_snprintf_add ((char *) msg_buf, sizeof (msg_buf), count_only ? ((char *) ("%ld matches")) : ((char *) ("%ld substitutions")), sub_nsubs);
        if (sub_nlines == 1) vim_snprintf_add ((char *) msg_buf, sizeof (msg_buf), "%s", ((char *) (" on 1 line")));
        else vim_snprintf_add ((char *) msg_buf, sizeof (msg_buf), ((char *) (" on %ld lines")), (long) sub_nlines);
        if (msg (msg_buf)) set_keep_msg (msg_buf, 0);
        return 1;
    }
    if (got_int) {
        emsg ((char_u *) (((char *) (e_interr))));
        return 1;
    }
    return 0;
}
    ...
    exarg_T * eap;
{
    ...
    int which_pat;
    if (global_busy) {
        emsg ((char_u *) (((char *) ("E147: Cannot do :global recursive"))));
        return;
    }
    if (eap -> forceit) type = 'v';
    else type = *eap->cmd;
    cmd = eap -> arg;
    which_pat = 2;
    if (*cmd == '\\') {
        ...
        emsg ((char_u *) (((char *) ("E148: Regular expression missing from global"))));
        return;
    }
    else {
        delim = *cmd;
        ...
        if (cmd [0] == delim) *cmd++ = '\000';
    }
    if (search_regcomp (pat, 2, which_pat, 0x20, &regmatch) == 0) {
        emsg ((char_u *) (((char *) (e_invcmd))));
        return;
    }
    for (lnum = eap -> line1; lnum <= eap -> line2 && !got_int; ++lnum) {
        match = vim_regexec_multi (&regmatch, curwin, curbuf, lnum, (colnr_T) 0, ((void *) 0));
    ...
    if (got_int) msg ((char_u *) (((char *) (e_interr))));
    else if (ndone == 0) {
        if (type == 'v') smsg ((char_u *) ((char *) ("Pattern found in every line: %s")), pat);
        else smsg ((char_u *) ((char *) (e_patnotf2)), pat);
    }
    else global_exe (cmd);
    ml_clearmarked ();
    vim_free (regmatch.regprog);
    ...
    char_u * cmd;
{
    ...
    while (!got_int && (lnum = ml_firstmarked ()) != 0 && global_busy == 1) {
        ...
        else do_cmdline (cmd, ((void *) 0), ((void *) 0), 0x02);
        ui_breakcheck ();
    }
    global_busy = 0;
    if (global_need_beginline) beginline (1 | 4);
    else check_cursor ();
    changed_line_abv_curs ();
    if (msg_col == 0 && msg_scrolled == 0) msg_didout = 0;
    if (!do_sub_msg(0)) msgmore (curbuf -> b_ml.ml_line_count - old_lcount);
}
...
int read_viminfo_sub_string (virp, force)
    ...
    int force;
{
    if (old_sub != ((void *) 0) && force) vim_free (old_sub);
    if (force || old_sub == ((void *) 0)) old_sub = viminfo_readstring (virp, 1, 1);
    return viminfo_readline (virp);
}
    ...
    int undo_sync;
{
    ...
    if (!curwin->w_onebuf_opt.wo_pvw) {
        for (wp = firstwin; wp != ((void *) 0); wp = wp -> w_next) if (wp -> w_onebuf_opt.wo_pvw) break;
        if (wp != ((void *) 0)) win_enter (wp, undo_sync);
        else {
            if (win_split (g_do_tagpreview > 0 ? g_do_tagpreview : 0, 0) == 0) return 0;
    ...
    exarg_T * eap;
{
    ...
    if (eap != ((void *) 0)) {
        ...
        arg = eap -> arg;
        if (eap -> forceit && *arg == '\000') {
            emsg ((char_u *) (((char *) ("E478: Don't panic!"))));
            return;
        }
        if (eap -> skip) return;
    }
    ...
    while (p > arg && ((*p) == ' ' || (*p) == '\t') && p [-1] != '\\') *p-- = '\000';
    lang = check_help_lang (arg);
    if (*arg == '\000') arg = (char_u *) "help.txt";
    n = find_help_tags (arg, &num_matches, &matches, eap != ((void *) 0) && eap -> forceit);
    i = 0;
    if (n != 0 && lang != ((void *) 0)) for (i = 0; i < num_matches; ++i) {
        len = (int) strlen ((char *) (matches [i]));
        if (len > 3 && matches [i] [len - 3] == '@' && strcasecmp ((char *) (matches [i] + len - 2), (char *) (lang)) == 0) break;
    }
    if (i >= num_matches || n == 0) {
        if (lang != ((void *) 0)) emsg3 ((char_u *) (((char *) ("E661: Sorry, no '%s' help for %s"))), (char_u *) (lang), (char_u *) (arg));
        else emsg2 ((char_u *) (((char *) ("E149: Sorry, no help for %s"))), (char_u *) (arg));
        if (n != 0) FreeWild (num_matches, matches);
        return;
    }
    tag = vim_strsave (matches [i]);
    FreeWild (num_matches, matches);
    if (!curwin->w_buffer->b_help || cmdmod.tab != 0) {
        if (cmdmod.tab != 0) wp = ((void *) 0);
        else for (wp = firstwin; wp != ((void *) 0); wp = wp -> w_next) if (wp -> w_buffer != ((void *) 0) && wp -> w_buffer -> b_help) break;
        if (wp != ((void *) 0) && wp -> w_buffer -> b_nwindows > 0) win_enter (wp, 1);
        else {
            if ((helpfd = fopen (((char *) p_hf), ("rb"))) == ((void *) 0)) {
                smsg ((char_u *) ((char *) ("Sorry, help file \"%s\" not found")), p_hf);
                goto erret;
            }
            fclose (helpfd);
            n = 16;
            if (cmdmod.split == 0 && curwin -> w_width != Columns && curwin -> w_width < 80) n |= 4;
            if (win_split (0, n) == 0) goto erret;
            if (curwin -> w_height < p_hh) win_setheight ((int) p_hh);
            alt_fnum = curbuf -> b_fnum;
            (void) do_ecmd (0, ((void *) 0), ((void *) 0), ((void *) 0), (linenr_T) 0, 0x01 + 0x02, ((void *) 0));
            if (!cmdmod.keepalt) curwin -> w_alt_fnum = alt_fnum;
            empty_fnum = curbuf -> b_fnum;
        }
    }
    if (!p_im) restart_edit = 0;
    if (tag != ((void *) 0)) do_tag (tag, 8, 1, 0, 1);
    if (empty_fnum != 0 && curbuf -> b_fnum != empty_fnum) {
        buf = buflist_findnr (empty_fnum);
        if (buf != ((void *) 0) && buf -> b_nwindows == 0) wipe_buffer (buf, 1);
    }
    if (alt_fnum != 0 && curwin -> w_alt_fnum == empty_fnum && !cmdmod.keepalt) curwin -> w_alt_fnum = alt_fnum;
    erret : vim_free (tag);
}
    ...
    char_u * arg;
{
    ...
    if (len >= 3 && arg [len - 3] == '@' && ((arg [len - 2]) < 0x7f && isalpha (arg [len - 2])) && ((arg [len - 1]) < 0x7f && isalpha (arg [len - 1]))) {
        arg [len - 3] = '\000';
        return arg + len - 2;
    }
    return ((void *) 0);
    ...
    int wrong_case;
{
    ...
    char_u * p;
    num_letters = 0;
    for (p = matched_string; *p; p ++) if (((*p) < 0x7f && isalnum (*p))) num_letters ++;
    if (((matched_string [offset]) < 0x7f && isalnum (matched_string [offset])) && offset > 0 && ((matched_string [offset - 1]) < 0x7f && isalnum (
      matched_string [offset - 1]))) offset += 10000;
    else if (offset > 2) offset *= 200;
    if (wrong_case) offset += 5000;
    if (matched_string [0] == '+' && matched_string [1] != '\000') offset += 100;
    return (int) (100 * num_letters + strlen ((char *) (matched_string)) + offset);
}

static int help_compare (s1, s2) const
    void * s1;
    const void * s2;
{
    char * p1;
    ...
    int keep_lang;
{
    ...
    static char * (mtable []) =
    {"*", "g*", "[*", "]*", ":*", "/*", "/\\*", "\"*", "**", "/\\(\\)", "?", ":?", "?<CR>", "g?", "g?g?", "g??", "z?", "/\\?", "/\\z(\\)", "\\=", ":s\\=",
          "[count]", "[quotex]", "[range]", "[pattern]", "\\|", "\\%$"};
    static char * (rtable []) =
    {"star", "gstar", "[star", "]star", ":star", "/star", "/\\\\star", "quotestar", "starstar", "/\\\\(\\\\)", "?", ":?", "?<CR>", "g?", "g?g?", "g??", "z?",
          "/\\\\?", "/\\\\z(\\\\)", "\\\\=", ":s\\\\=", "\\[count]", "\\[quotex]", "\\[range]", "\\[pattern]", "\\\\bar", "/\\\\%\\$"};
    int flags;
    d = IObuff;
    for (i = (int) (sizeof (mtable) / sizeof (char *)); --i >= 0;) if (strcmp ((char *) (arg), (char *) (mtable [i])) == 0) {
        ((__builtin_object_size ((char *) (d), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (d), (char *) (rtable [i]), __builtin_object_size ((char *
          ) (d), 1 > 1)) : __inline_strcpy_chk ((char *) (d), (char *) (rtable [i])));
        break;
    }
    if (i < 0) {
        if (arg [0] == '\\' && ((arg [1] != '\000' && arg [2] == '\000') || (vim_strchr ((char_u *) "%_z@", arg [1]) != ((void *) 0) && arg [2] != '\000'))) {
            ((__builtin_object_size ((char *) (d), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (d), (char *) ("/\\\\"), __builtin_object_size ((char
              *) (d), 1 > 1)) : __inline_strcpy_chk ((char *) (d), (char *) ("/\\\\")));
            ((__builtin_object_size ((char *) (d + 3), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (d + 3), (char *) (arg + 1), __builtin_object_size
              ((char *) (d + 3), 1 > 1)) : __inline_strcpy_chk ((char *) (d + 3), (char *) (arg + 1)));
            if (d [3] == '_' && d [4] == '$') ((__builtin_object_size ((char *) (d + 4), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (d + 4), (char *
              ) ("\\$"), __builtin_object_size ((char *) (d + 4), 1 > 1)) : __inline_strcpy_chk ((char *) (d + 4), (char *) ("\\$")));
        }
        else {
            if (arg [0] == '[' && (arg [1] == ':' || (arg [1] == '+' && arg [2] == '+'))) *d++ = '\\';
            for (s = arg; *s; ++s) {
                if (d - IObuff > (1024 + 1) - 10) break;
                switch (*s) {
                case '|' :
                    ((__builtin_object_size ((char *) (d), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (d), (char *) ("bar"), __builtin_object_size (
                      (char *) (d), 1 > 1)) : __inline_strcpy_chk ((char *) (d), (char *) ("bar")));
                    d += 3;
                    continue;
                case '"' :
                    ((__builtin_object_size ((char *) (d), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (d), (char *) ("quote"), __builtin_object_size
                      ((char *) (d), 1 > 1)) : __inline_strcpy_chk ((char *) (d), (char *) ("quote")));
                    d += 5;
                    continue;
                case '*' :
                    *d++ = '.';
                ...
                if (*s < ' ' || (*s == '^' && s [1] && (((s [1]) < 0x7f && isalpha (s [1])) || vim_strchr ((char_u *) "?@[\\]^", s [1]) != ((void *) 0)))) {
                    if (d > IObuff && d [-1] != '_') *d++ = '_';
                    ((__builtin_object_size ((char *) (d), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (d), (char *) ("CTRL-"), __builtin_object_size
                      ((char *) (d), 1 > 1)) : __inline_strcpy_chk ((char *) (d), (char *) ("CTRL-")));
                    d += 5;
                    if (*s < ' ') {
                        *d++ = *s + '@';
                        if (d [-1] == '\\') *d++ = '\\';
                    ...
                    if (s [1] != '\000' && s [1] != '_') *d++ = '_';
                    continue;
                }
                else if (*s == '^') *d++ = '\\';
                else if (s [0] == '\\' && s [1] != '\\' && *arg == '/' && s == arg + 1) *d++ = '\\';
                if (strncasecmp ((char *) (s), (char *) ("CTRL-\\_"), (size_t) (7)) == 0) {
                    ((__builtin_object_size ((char *) (d), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (d), (char *) ("CTRL-\\\\"),
                      __builtin_object_size ((char *) (d), 1 > 1)) : __inline_strcpy_chk ((char *) (d), (char *) ("CTRL-\\\\")));
                    d += 7;
                    s += 6;
                }
                *d++ = *s;
                if (*s == '\'' && s > arg && *arg == '\'') break;
            }
            * d = '\000';
        }
    }
    ...
    * num_matches = 0;
    flags = 1 | 4 | 2 | 32;
    if (keep_lang) flags |= 128;
    if (find_tags (IObuff, num_matches, matches, flags, (int) (0x7fffffffL), ((void *) 0)) == 1 && *num_matches > 0) {
        qsort ((void *) *matches, (size_t) * num_matches, sizeof (char_u *), help_compare);
        while (*num_matches > 300) vim_free ((*matches) [--*num_matches]);
    }
    return 1;
}
...
void fix_help_buffer () {
    ...
    if (strcasecmp ((char *) (((gettail (curbuf -> b_fname)))), (char *) ((("help.txt")))) == 0) {
        for (lnum = 1; lnum < curbuf -> b_ml.ml_line_count; ++lnum) {
            line = ml_get_buf (curbuf, lnum, 0);
            if (strstr ((char *) line, "*local-additions*") != ((void *) 0)) {
                ...
                while (*p != '\000') {
                    ...
                    if (fullpathcmp (rt, NameBuff, 0) != 1) {
                        int fcount;
                        char_u * * fnames;
                        FILE * fd;
                        char_u * s;
                        ...
                        ((__builtin_object_size ((char *) (NameBuff), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (NameBuff), (char *) ("doc/*.txt")
                          , __builtin_object_size ((char *) (NameBuff), 1 > 1)) : __inline_strcat_chk ((char *) (NameBuff), (char *) ("doc/*.txt")));
                        if (gen_expand_wildcards (1, &NameBuff, &fcount, &fnames, 0x02 | 0x20) == 1 && fcount > 0) {
                            for (fi = 0; fi < fcount; ++fi) {
                                fd = fopen (((char *) fnames [fi]), ("r"));
                                if (fd != ((void *) 0)) {
                                    vim_fgets (IObuff, (1024 + 1), fd);
                                    if (IObuff [0] == '*' && (s = vim_strchr (IObuff + 1, '*')) != ((void *) 0)) {
                                        IObuff [0] = '|';
                                        * s = '|';
                                        while (*s != '\000') {
                                            if (*s == '\r' || *s == '\n') *s = '\000';
    ...
    exarg_T * eap;
{
    ...
    char_u * * files;
    int add_help_tags = 0;
    if (strncmp ((char *) (eap -> arg), (char *) ("++t"), (size_t) (3)) == 0 && ((eap -> arg [3]) == ' ' || (eap -> arg [3]) == '\t')) {
        add_help_tags = 1;
        eap -> arg = skipwhite (eap -> arg + 3);
    }
    ExpandInit (&xpc);
    xpc.xp_context = 3;
    dirname = ExpandOne (&xpc, eap -> arg, ((void *) 0), 1 | 64, 2);
    if (dirname == ((void *) 0) || !mch_isdir(dirname)) {
        emsg2 ((char_u *) (((char *) ("E150: Not a directory: %s"))), (char_u *) (eap -> arg));
        return;
    }
    ((__builtin_object_size ((char *) (NameBuff), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (NameBuff), (char *) (dirname), __builtin_object_size (
      (char *) (NameBuff), 1 > 1)) : __inline_strcpy_chk ((char *) (NameBuff), (char *) (dirname)));
    add_pathsep (NameBuff);
    ((__builtin_object_size ((char *) (NameBuff), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (NameBuff), (char *) ("*"), __builtin_object_size ((
      char *) (NameBuff), 1 > 1)) : __inline_strcat_chk ((char *) (NameBuff), (char *) ("*")));
    if (gen_expand_wildcards (1, &NameBuff, &filecount, &files, 0x02 | 0x20) == 0 || filecount == 0) {
    ...
    for (i = 0; i < filecount; ++i) {
        ...
        if (len > 4) {
            ...
            else if (files [i] [len - 4] == '.' && ((files [i] [len - 3]) < 0x7f && isalpha (files [i] [len - 3])) && ((files [i] [len - 2]) < 0x7f && isalpha (
              files [i] [len - 2])) && (((files [i] [len - 1]) < 'A' || (files [i] [len - 1]) > 'Z') ? (files [i] [len - 1]) : (files [i] [len - 1]) + ('a' -
              'A')) == 'x') {
            ...
            for (j = 0; j < ga.ga_len; j += 2) if (strncmp ((char *) (lang), (char *) (((char_u *) ga.ga_data) + j), (size_t) (2)) == 0) break;
            if (j == ga.ga_len) {
                if (ga_grow (&ga, 2) == 0) break;
    ...
    for (j = 0; j < ga.ga_len; j += 2) {
        ((__builtin_object_size ((char *) (fname), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (fname), (char *) ("tags-xx"), __builtin_object_size (
          (char *) (fname), 1 > 1)) : __inline_strcpy_chk ((char *) (fname), (char *) ("tags-xx")));
        fname [5] = ((char_u *) ga.ga_data) [j];
        fname [6] = ((char_u *) ga.ga_data) [j + 1];
        if (fname [5] == 'e' && fname [6] == 'n') {
            fname [4] = '\000';
            ((__builtin_object_size ((char *) (ext), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (ext), (char *) (".txt"), __builtin_object_size ((
              char *) (ext), 1 > 1)) : __inline_strcpy_chk ((char *) (ext), (char *) (".txt")));
        }
        else {
            ((__builtin_object_size ((char *) (ext), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (ext), (char *) (".xxx"), __builtin_object_size ((
              char *) (ext), 1 > 1)) : __inline_strcpy_chk ((char *) (ext), (char *) (".xxx")));
            ext [1] = fname [5];
            ext [2] = fname [6];
        }
    ...
    int add_help_tags;
{
    ...
    int i;
    char_u * fname;
    ((__builtin_object_size ((char *) (NameBuff), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (NameBuff), (char *) (dir), __builtin_object_size ((
      char *) (NameBuff), 1 > 1)) : __inline_strcpy_chk ((char *) (NameBuff), (char *) (dir)));
    add_pathsep (NameBuff);
    ((__builtin_object_size ((char *) (NameBuff), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (NameBuff), (char *) ("*"), __builtin_object_size ((
      char *) (NameBuff), 1 > 1)) : __inline_strcat_chk ((char *) (NameBuff), (char *) ("*")));
    ((__builtin_object_size ((char *) (NameBuff), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (NameBuff), (char *) (ext), __builtin_object_size ((
      char *) (NameBuff), 1 > 1)) : __inline_strcat_chk ((char *) (NameBuff), (char *) (ext)));
    if (gen_expand_wildcards (1, &NameBuff, &filecount, &files, 0x02 | 0x20) == 0 || filecount == 0) {
        if (!got_int) emsg2 ((char_u *) ("E151: No match: %s"), (char_u *) (NameBuff));
        return;
    }
    ((__builtin_object_size ((char *) (NameBuff), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (NameBuff), (char *) (dir), __builtin_object_size ((
      char *) (NameBuff), 1 > 1)) : __inline_strcpy_chk ((char *) (NameBuff), (char *) (dir)));
    add_pathsep (NameBuff);
    ((__builtin_object_size ((char *) (NameBuff), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (NameBuff), (char *) (tagfname), __builtin_object_size
      ((char *) (NameBuff), 1 > 1)) : __inline_strcat_chk ((char *) (NameBuff), (char *) (tagfname)));
    fd_tags = fopen (((char *) NameBuff), ("w"));
    if (fd_tags == ((void *) 0)) {
        emsg2 ((char_u *) (((char *) ("E152: Cannot open %s for writing"))), (char_u *) (NameBuff));
        FreeWild (filecount, files);
        return;
    }
    ga_init2 (&ga, (int) sizeof (char_u *), 100);
    if (add_help_tags || fullpathcmp ((char_u *) "$VIMRUNTIME/doc", dir, 0) == 1) {
        if (ga_grow (&ga, 1) == 0) got_int = 1;
        else {
            s = alloc (18 + (unsigned) strlen ((char *) (tagfname)));
            if (s == ((void *) 0)) got_int = 1;
            else {
                __builtin___sprintf_chk ((char *) s, 0, __builtin_object_size ((char *) s, 1 > 1), "help-tags\t%s\t1\n", tagfname);
    ...
    for (fi = 0; fi < filecount && !got_int; ++fi) {
        ...
        if (fd == ((void *) 0)) {
            emsg2 ((char_u *) (((char *) ("E153: Unable to open %s for reading"))), (char_u *) (files [fi]));
            continue;
        }
        ...
        while (!vim_fgets(IObuff,(1024+1),fd) && !got_int) {
            ...
            while (p1 != ((void *) 0)) {
                ...
                if (p2 != ((void *) 0) && p2 > p1 + 1) {
                    for (s = p1 + 1; s < p2; ++s) if (*s == ' ' || *s == '\t' || *s == '|') break;
                    if (s == p2 && (p1 == IObuff || p1 [-1] == ' ' || p1 [-1] == '\t') && (vim_strchr ((char_u *) " \t\n\r", s [1]) != ((void *) 0) || s [1] ==
                      '\0')) {
                        * p2 = '\0';
                        ...
                        s = alloc ((unsigned) (p2 - p1 + strlen ((char *) (fname)) + 2));
                        if (s == ((void *) 0)) {
                            got_int = 1;
                            break;
                        }
                        ((char_u * *) ga.ga_data) [ga.ga_len] = s;
                        ++ga.ga_len;
                        __builtin___sprintf_chk ((char *) s, 0, __builtin_object_size ((char *) s, 1 > 1), "%s\t%s", p1, fname);
                        p2 = vim_strchr (p2 + 1, '*');
                    }
                }
                p1 = p2;
    ...
    if (!got_int) {
        ...
        for (i = 1; i < ga.ga_len; ++i) {
            ...
            while (*p1 == *p2) {
                if (*p2 == '\t') {
                    * p2 = '\000';
                    vim_snprintf ((char *) NameBuff, 1024, ((char *) ("E154: Duplicate tag \"%s\" in file %s/%s")), ((char_u * *) ga.ga_data) [i], dir, p2 + 1)
                      ;
        ...
        for (i = 0; i < ga.ga_len; ++i) {
            ...
            if (strncmp ((char *) (s), (char *) ("help-tags\t"), (size_t) (10)) == 0) fputs ((char *) s, fd_tags);
            else {
                fprintf (fd_tags, "%s\t/*", s);
                for (p1 = s; *p1 != '\t'; ++p1) {
                    if (*p1 == '\\' || *p1 == '/') putc ('\\', fd_tags);
                    putc (* p1, fd_tags);
                }
                fprintf (fd_tags, "*\n");
            }
    ...
    for (i = 0; i < ga.ga_len; ++i) vim_free (((char_u * *) ga.ga_data) [i]);
    ga_clear (&ga);
    fclose (fd_tags);
}
...
static int last_sign_typenr = 65535;
static int sign_cmd_idx (char_u * begin_cmd, char_u * end_cmd);
static void sign_list_defined (sign_T * sp);
static void sign_undefine (sign_T * sp, sign_T * sp_prev);
static char * cmds [] =
{"define", "undefine", "list", "place", "unplace", "jump", ((void *) 0)};

static int sign_cmd_idx (begin_cmd, end_cmd)
    char_u * begin_cmd;
    char_u * end_cmd;
{
    int idx;
    char save = *end_cmd;
    * end_cmd = '\000';
    for (idx = 0;; ++idx) if (cmds [idx] == ((void *) 0) || strcmp ((char *) (begin_cmd), (char *) (cmds [idx])) == 0) break;
    * end_cmd = save;
    return idx;
}
    ...
    exarg_T * eap;
{
    ...
    if (idx == 6) {
        emsg2 ((char_u *) (((char *) ("E160: Unknown sign command: %s"))), (char_u *) (arg));
        return;
    }
    arg = skipwhite (p);
    if (idx <= 2) {
        if (idx == 2 && *arg == '\000') {
            for (sp = first_sign; sp != ((void *) 0); sp = sp -> sn_next) sign_list_defined (sp);
        }
        else if (*arg == '\000') emsg ((char_u *) (((char *) ("E156: Missing sign name"))));
        else {
            p = skiptowhite (arg);
            if (*p != '\000') *p++ = '\000';
            sp_prev = ((void *) 0);
            for (sp = first_sign; sp != ((void *) 0); sp = sp -> sn_next) {
                if (strcmp ((char *) (sp -> sn_name), (char *) (arg)) == 0) break;
                sp_prev = sp;
            }
            if (idx == 0) {
                if (sp == ((void *) 0)) {
                    ...
                    if (sp == ((void *) 0)) return;
                    if (sp_prev == ((void *) 0)) first_sign = sp;
                    else sp_prev -> sn_next = sp;
                    sp -> sn_name = vim_strnsave (arg, (int) (p - arg));
                    if (((*arg) >= '0' && (*arg) <= '9')) sp -> sn_typenr = atoi ((char *) arg);
                    else {
                        sign_T * lp;
                        int start = last_sign_typenr;
                        for (lp = first_sign; lp != ((void *) 0); lp = lp -> sn_next) {
                            if (lp -> sn_typenr == last_sign_typenr) {
                                --last_sign_typenr;
                                if (last_sign_typenr == 0) last_sign_typenr = 65535;
                                if (last_sign_typenr == start) {
                                    emsg ((char_u *) (((char *) ("E612: Too many signs defined"))));
                                    return;
                                }
                                lp = first_sign;
                                continue;
                ...
                for (;;) {
                    ...
                    if (strncmp ((char *) (arg), (char *) ("icon="), (size_t) (5)) == 0) {
                        ...
                        arg += 5;
                        {
                            for (s = arg; s < p; ++s) if (!vim_isprintc(*s)) break;
                            cells = (int) (s - arg);
                        }
                        if (s != p || cells < 1 || cells > 2) {
                            * p = '\000';
                            emsg2 ((char_u *) (((char *) ("E239: Invalid sign text: %s"))), (char_u *) (arg));
                            return;
                        }
                        vim_free (sp -> sn_text);
                        len = (int) (p - arg + ((cells == 1) ? 1 : 0));
                        sp -> sn_text = vim_strnsave (arg, len);
                        if (sp -> sn_text != ((void *) 0) && cells == 1) ((__builtin_object_size ((char *) (sp -> sn_text + len - 1), 0) != (size_t) - 1) ?
                          __builtin___strcpy_chk ((char *) (sp -> sn_text + len - 1), (char *) (" "), __builtin_object_size ((char *) (sp -> sn_text + len - 1)
                          , 1 > 1)) : __inline_strcpy_chk ((char *) (sp -> sn_text + len - 1), (char *) (" ")));
                    }
            ...
            else if (sp == ((void *) 0)) emsg2 ((char_u *) (((char *) ("E155: Unknown sign: %s"))), (char_u *) (arg));
            else if (idx == 2) sign_list_defined (sp);
            else sign_undefine (sp, sp_prev);
        }
    }
    else {
        int id = -1;
        ...
        if (*arg == '\000') {
            if (idx == 3) {
                ...
                else emsg ((char_u *) (((char *) ("E159: Missing sign number"))));
            }
            else emsg ((char_u *) (((char *) (e_argreq))));
            return;
        }
        ...
        if (((*arg) >= '0' && (*arg) <= '9')) {
            ...
            if (!((*arg) == ' ' || (*arg) == '\t') && *arg != '\000') {
                ...
                arg = skipwhite (arg);
                if (idx == 4 && *arg == '\000') {
                    for (buf = firstbuf; buf != ((void *) 0); buf = buf -> b_next) if ((lnum = buf_delsign (buf, id)) != 0) update_debug_sign (buf, lnum);
                    return;
                }
            }
        }
        for (;;) {
            if (strncmp ((char *) (arg), (char *) ("line="), (size_t) (5)) == 0) {
                ...
                if (*skipwhite(arg) != '\000') emsg ((char_u *) (((char *) (e_trailing))));
                break;
            }
            else {
                emsg ((char_u *) (((char *) (e_invarg))));
        ...
        if (buf == ((void *) 0)) {
            emsg2 ((char_u *) (((char *) ("E158: Invalid buffer name: %s"))), (char_u *) (arg));
        }
        else if (id <= 0) {
            if (lnum >= 0 || sign_name != ((void *) 0)) emsg ((char_u *) (((char *) (e_invarg))));
            else sign_list_placed (buf);
        }
        else if (idx == 5) {
            if (lnum >= 0 || sign_name != ((void *) 0)) emsg ((char_u *) (((char *) (e_invarg))));
                ...
                if (buf_jump_open_win (buf) != ((void *) 0)) {
                    ...
                    char_u * cmd;
                    cmd = alloc ((unsigned) strlen ((char *) (buf -> b_fname)) + 25);
                    if (cmd == ((void *) 0)) return;
                    __builtin___sprintf_chk ((char *) cmd, 0, __builtin_object_size ((char *) cmd, 1 > 1), "e +%ld %s", (long) lnum, buf -> b_fname);
                    do_cmdline_cmd (cmd);
                    vim_free (cmd);
                }
                foldOpenCursor ();
            }
            else emsgn ((char_u *) (((char *) ("E157: Invalid sign ID: %ld"))), (long) (id));
        }
        else if (idx == 4) {
            if (lnum >= 0 || sign_name != ((void *) 0)) emsg ((char_u *) (((char *) (e_invarg))));
            else {
            ...
            for (sp = first_sign; sp != ((void *) 0); sp = sp -> sn_next) if (strcmp ((char *) (sp -> sn_name), (char *) (sign_name)) == 0) break;
            if (sp == ((void *) 0)) {
                emsg2 ((char_u *) (((char *) ("E155: Unknown sign: %s"))), (char_u *) (sign_name));
                return;
            }
            if (lnum > 0) buf_addsign (buf, id, lnum, sp -> sn_typenr);
            else lnum = buf_change_sign_type (buf, id, sp -> sn_typenr);
            update_debug_sign (buf, lnum);
        }
...
static void sign_list_defined (sp)
    sign_T * sp;
{
    char_u * p;
    ...
    if (sp -> sn_icon != ((void *) 0)) {
        ...
        msg_puts ((char_u *) (((char *) (" (not supported)"))));
    }
    ...
    if (sp -> sn_line_hl > 0) {
        ...
        if (p == ((void *) 0)) msg_puts ((char_u *) ("NONE"));
        else msg_puts (p);
    }
    if (sp -> sn_text_hl > 0) {
...
static void sign_undefine (sp, sp_prev)
    sign_T * sp;
    sign_T * sp_prev;
{
    vim_free (sp -> sn_name);
    vim_free (sp -> sn_icon);
    vim_free (sp -> sn_text);
    if (sp_prev == ((void *) 0)) first_sign = sp -> sn_next;
    else sp_prev -> sn_next = sp -> sn_next;
    vim_free (sp);
}

int sign_get_attr (typenr, line)
    int typenr;
    int line;
{
    sign_T * sp;
    for (sp = first_sign; sp != ((void *) 0); sp = sp -> sn_next) if (sp -> sn_typenr == typenr) {
        if (line) {
            if (sp -> sn_line_hl > 0) return syn_id2attr (sp -> sn_line_hl);
        }
        else {
            if (sp -> sn_text_hl > 0) return syn_id2attr (sp -> sn_text_hl);
        }
        break;
    }
    return 0;
...
char_u * sign_get_text (typenr)
    int typenr;
{
    sign_T * sp;
    for (sp = first_sign; sp != ((void *) 0); sp = sp -> sn_next) if (sp -> sn_typenr == typenr) return sp -> sn_text;
    return ((void *) 0);
}

char_u * sign_typenr2name (typenr)
    int typenr;
{
    sign_T * sp;
    for (sp = first_sign; sp != ((void *) 0); sp = sp -> sn_next) if (sp -> sn_typenr == typenr) return sp -> sn_name;
    return (char_u *) ((char *) ("[Deleted]"));
}

static enum {EXP_SUBCMD, EXP_DEFINE, EXP_PLACE, EXP_UNPLACE, EXP_SIGN_NAMES} expand_what;

char_u * get_sign_name (xp, idx)
    ...
    int idx;
{
    ...
    case EXP_SIGN_NAMES :
        current_idx = 0;
        for (sp = first_sign; sp != ((void *) 0); sp = sp -> sn_next) if (current_idx ++ == idx) return sp -> sn_name;
        return ((void *) 0);
    default :
        return ((void *) 0);
    }
...
void set_context_in_sign_cmd (xp, arg)
    expand_T * xp;
    char_u * arg;
{
    char_u * p;
    ...
    if (p == '\000') {
        ...
        switch (cmd_idx) {
        case 0 :
            if (strncmp ((char *) (last), (char *) ("texthl"), (size_t) (p - last)) == 0 || strncmp ((char *) (last), (char *) ("linehl"), (size_t) (p - last))
              == 0) xp -> xp_context = 13;
            else if (strncmp ((char *) (last), (char *) ("icon"), (size_t) (p - last)) == 0) xp -> xp_context = 2;
            else xp -> xp_context = 0;
            break;
        case 3 :
            if (strncmp ((char *) (last), (char *) ("name"), (size_t) (p - last)) == 0) expand_what = EXP_SIGN_NAMES;
            else xp -> xp_context = 0;
            break;
        default :
            xp -> xp_context = 0;
