    ...
    int len;
{
    ...
    int newlen;
    newlen = len;
    if (ta_str != ((void *) 0)) newlen += ta_len - ta_off;
    new = alloc (newlen);
    if (new != ((void *) 0)) {
        if (ta_str != ((void *) 0)) {
            ((__builtin_object_size ((char *) (new), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (new), (char *) (ta_str + ta_off), (size_t) (
              ta_len - ta_off), __builtin_object_size ((char *) (new), 0)) : __inline_memmove_chk ((char *) (new), (char *) (ta_str + ta_off), (size_t) (ta_len
              - ta_off)));
            ((__builtin_object_size ((char *) (new + ta_len - ta_off), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (new + ta_len - ta_off), (char *
              ) (s), (size_t) len, __builtin_object_size ((char *) (new + ta_len - ta_off), 0)) : __inline_memmove_chk ((char *) (new + ta_len - ta_off), (char
              *) (s), (size_t) len));
            vim_free (ta_str);
        }
        else ((__builtin_object_size ((char *) (new), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (new), (char *) (s), (size_t) len,
          __builtin_object_size ((char *) (new), 0)) : __inline_memmove_chk ((char *) (new), (char *) (s), (size_t) len));
        ta_str = new;
        ta_len = newlen;
        ta_off = 0;
    }
    ...
    int tb_change_cnt;
{
    ...
    if (wtime == -1 || wtime > 100L) {
        ...
        if (mapped_ctrl_c) ctrl_c_interrupts = 0;
    }
    {
        retval = mch_inchar (buf, maxlen, wtime, tb_change_cnt);
    }
    if (wtime == -1 || wtime > 100L) (void) vim_handle_signal (-1);
    ctrl_c_interrupts = 1;
    return retval;
}
...
void ui_set_shellsize (mustset)
    int mustset __attribute__ ((unused));
{
    mch_set_shellsize ();
}
...
void clip_update_selection () {
    ...
    if (!redo_VIsual_busy && VIsual_active && (State & 0x01)) {
        ...
        if (!(((clip_star.start).lnum==(start).lnum)&&((clip_star.start).col==(start).col)&&((clip_star.start).coladd==(start).coladd)) || 
          !(((clip_star.end).lnum==(end).lnum)&&((clip_star.end).col==(end).col)&&((clip_star.end).coladd==(end).coladd)) || clip_star.vmode != VIsual_mode) {
...
void clip_own_selection (cbd)
    VimClipboard * cbd;
{
    if (!cbd->owned && cbd -> available) cbd -> owned = (clip_gen_own_selection (cbd) == 1);
}

void clip_lose_selection (cbd)
    VimClipboard * cbd;
{
    int visual_selection = (cbd == &clip_star);
    clip_free_selection (cbd);
    cbd -> owned = 0;
    if (visual_selection) clip_clear_selection ();
    clip_gen_lose_selection (cbd);
}

void clip_copy_selection () {
    if (VIsual_active && (State & 0x01) && clip_star.available) {
        if (clip_isautosel ()) clip_update_selection ();
        clip_free_selection (&clip_star);
        clip_own_selection (&clip_star);
        if (clip_star.owned) clip_get_selection (&clip_star);
        clip_gen_set_selection (&clip_star);
    }
}

void clip_auto_select () {
    if (clip_isautosel ()) clip_copy_selection ();
}

int clip_isautosel () {
    return (clip_autoselect);
}
...
void clip_modeless (button, is_click, is_drag)
    int button;
    int is_click;
    int is_drag;
{
    int repeat;
    repeat = ((clip_star.mode == 0 || clip_star.mode == 2) && (mod_mask & 0x20)) || (clip_star.mode == 1 && (mod_mask & 0x40));
    if (is_click && button == 0x02) {
        if (clip_star.state == 0) clip_start_selection (mouse_col, mouse_row, 0);
        clip_process_selection (button, mouse_col, mouse_row, repeat);
    }
    else if (is_click) clip_start_selection (mouse_col, mouse_row, repeat);
    else if (is_drag) {
        if (clip_star.state != 0) clip_process_selection (button, mouse_col, mouse_row, repeat);
    }
    else clip_process_selection (0x03, mouse_col, mouse_row, 0);
}

static int clip_compare_pos (row1, col1, row2, col2)
    int row1;
    int col1;
    int row2;
    int col2;
...
void clip_start_selection (col, row, repeated_click)
    int col;
    int row;
    int repeated_click;
{
    VimClipboard * cb = &clip_star;
    if (cb -> state == 2) clip_clear_selection ();
    row = check_row (row);
    col = check_col (col);
    cb -> start.lnum = row;
    cb -> start.col = col;
    ...
    case 1 :
        ...
        clip_invert_area ((int) cb -> start.lnum, cb -> word_start_col, (int) cb -> end.lnum, cb -> word_end_col, 2);
        cb -> start.col = cb -> word_start_col;
        cb -> end.col = cb -> word_end_col;
        break;
    case 2 :
        clip_invert_area ((int) cb -> start.lnum, 0, (int) cb -> start.lnum, (int) Columns, 2);
        cb -> start.col = 0;
        cb -> end.col = Columns;
        break;
    }
...
void clip_process_selection (button, col, row, repeated_click)
    int button;
    int col;
    int row;
    int_u repeated_click;
{
    ...
    if (button == 0x03) {
        ...
        if (clip_isautosel () || (clip_autoselectml)) clip_copy_modeless_selection (0);
        cb -> state = 2;
        return;
    }
    row = check_row (row);
    col = check_col (col);
    if (col == (int) cb -> prev.col && row == cb -> prev.lnum && !repeated_click) return;
    if (cb -> state == 2 && button == 0x02) {
        if (clip_compare_pos (row, col, (int) cb -> start.lnum, cb -> start.col) < 0 || (clip_compare_pos (row, col, (int) cb -> end.lnum, cb -> end.col) < 0 &&
          (((cb -> start.lnum == cb -> end.lnum && cb -> end.col - col > col - cb -> start.col)) || ((diff = (cb -> end.lnum - row) - (row - cb -> start.lnum))
          > 0 || (diff == 0 && col < (int) (cb -> start.col + cb -> end.col) / 2))))) {
        ...
        if (cb -> mode == 1 && !repeated_click) cb -> mode = 0;
    }
    cb -> state = 1;
    if (repeated_click && ++cb->mode > 2) cb -> mode = 0;
    switch (cb -> mode) {
    case 0 :
        if (row != cb -> prev.lnum) cb -> word_end_col = clip_get_line_end (row);
        if (clip_compare_pos (row, col, cb -> origin_row, cb -> origin_start_col) >= 0) {
            if (col >= (int) cb -> word_end_col) clip_update_modeless_selection (cb, cb -> origin_row, cb -> origin_start_col, row, (int) Columns);
            else {
                clip_update_modeless_selection (cb, cb -> origin_row, cb -> origin_start_col, row, col + slen);
            }
        }
        else {
            if (col >= (int) cb -> word_end_col) clip_update_modeless_selection (cb, row, cb -> word_end_col, cb -> origin_row, cb -> origin_start_col + slen);
            else clip_update_modeless_selection (cb, row, col, cb -> origin_row, cb -> origin_start_col + slen);
        }
        break;
    case 1 :
        if (row == cb -> prev.lnum && col >= (int) cb -> word_start_col && col < (int) cb -> word_end_col && !repeated_click) return;
        clip_get_word_boundaries (cb, row, col);
        if (clip_compare_pos (row, col, cb -> origin_row, cb -> origin_start_col) >= 0) clip_update_modeless_selection (cb, cb -> origin_row, cb ->
          origin_start_col, row, cb -> word_end_col);
        else clip_update_modeless_selection (cb, row, cb -> word_start_col, cb -> origin_row, cb -> origin_end_col);
        break;
    case 2 :
        if (row == cb -> prev.lnum && !repeated_click) return;
        if (clip_compare_pos (row, col, cb -> origin_row, cb -> origin_start_col) >= 0) clip_update_modeless_selection (cb, cb -> origin_row, 0, row, (int)
          Columns);
        else clip_update_modeless_selection (cb, row, 0, cb -> origin_row, (int) Columns);
        break;
    }
    cb -> prev.lnum = row;
    cb -> prev.col = col;
...
void clip_clear_selection () {
    VimClipboard * cb = &clip_star;
    if (cb -> state == 0) return;
    clip_invert_area ((int) cb -> start.lnum, cb -> start.col, (int) cb -> end.lnum, cb -> end.col, 1);
    cb -> state = 0;
}

void clip_may_clear_selection (row1, row2)
    int row1, row2;
{
    if (clip_star.state == 2 && row2 >= clip_star.start.lnum && row1 <= clip_star.end.lnum) clip_clear_selection ();
}

void clip_scroll_selection (rows)
    int rows;
{
    int lnum;
    if (clip_star.state == 0) return;
    lnum = clip_star.start.lnum - rows;
    if (lnum <= 0) clip_star.start.lnum = 0;
    else if (lnum >= screen_Rows) clip_star.state = 0;
    else clip_star.start.lnum = lnum;
    lnum = clip_star.end.lnum - rows;
    if (lnum < 0) clip_star.state = 0;
    else if (lnum >= screen_Rows) clip_star.end.lnum = screen_Rows - 1;
    else clip_star.end.lnum = lnum;
}

static void clip_invert_area (row1, col1, row2, col2, how)
    int row1;
    int col1;
    int row2;
    int col2;
    int how;
{
    ...
    if (row1 == row2) {
        ...
        if (row2 >= row1) clip_invert_rectangle (row1, 0, row2 - row1 + 1, (int) Columns, invert);
    }
}

static void clip_invert_rectangle (row, col, height, width, invert)
    int row;
    int col;
    int height;
    int width;
...
void clip_copy_modeless_selection (both)
    int both __attribute__ ((unused));
{
    char_u * buffer;
    char_u * bufp;
    int row;
    ...
    buffer = lalloc ((long_u) len, 1);
    if (buffer == ((void *) 0)) return;
    for (bufp = buffer, row = row1; row <= row2; row ++) {
        if (row == row1) start_col = col1;
        else start_col = 0;
        if (row == row2) end_col = col2;
        else end_col = Columns;
        ...
        if (end_col >= Columns && (row < row2 || end_col > line_end_col)) {
            ...
            if (row == row2) add_newline_flag = 1;
        }
        if (row > row1 && !LineWraps[row-1]) *bufp++ = '\012';
        if (row < screen_Rows && end_col <= screen_Columns) {
            {
                ((__builtin_object_size ((char *) (bufp), 0) != (size_t) - 1) ? __builtin___strncpy_chk ((char *) (bufp), (char *) (ScreenLines + LineOffset [
                  row] + start_col), (size_t) (end_col - start_col), __builtin_object_size ((char *) (bufp), 1 > 1)) : __inline_strncpy_chk ((char *) (bufp), (
                  char *) (ScreenLines + LineOffset [row] + start_col), (size_t) (end_col - start_col)));
                bufp += end_col - start_col;
            }
        }
    }
    if (add_newline_flag) *bufp++ = '\012';
    clip_free_selection (&clip_star);
    clip_own_selection (&clip_star);
    clip_yank_selection (0, buffer, (long) (bufp - buffer), &clip_star);
    clip_gen_set_selection (&clip_star);
    vim_free (buffer);
}

static void clip_get_word_boundaries (cb, row, col)
    VimClipboard * cb;
    int row;
    int col;
{
    ...
    int temp_col;
    char_u * p;
    if (row >= screen_Rows || col >= screen_Columns || ScreenLines == ((void *) 0)) return;
    p = ScreenLines + LineOffset [row];
    start_class = (p [col] <= ' ' ? ' ' : vim_iswordc (p [col]));
    temp_col = col;
    for (; temp_col > 0; temp_col --) if ((p [temp_col - 1] <= ' ' ? ' ' : vim_iswordc (p [temp_col - 1])) != start_class) break;
    cb -> word_start_col = temp_col;
    temp_col = col;
    for (; temp_col < screen_Columns; temp_col ++) if ((p [temp_col] <= ' ' ? ' ' : vim_iswordc (p [temp_col])) != start_class) break;
    cb -> word_end_col = temp_col;
}

static int clip_get_line_end (row)
    int row;
{
    int i;
    if (row >= screen_Rows || ScreenLines == ((void *) 0)) return 0;
    for (i = screen_Columns; i > 0; i --) if (ScreenLines [LineOffset [row] + i - 1] != ' ') break;
    return i;
}

static void clip_update_modeless_selection (cb, row1, col1, row2, col2)
    VimClipboard * cb;
    int row1;
    int col1;
    int row2;
    int col2;
{
    if (row1 != cb -> start.lnum || col1 != (int) cb -> start.col) {
        clip_invert_area (row1, col1, (int) cb -> start.lnum, cb -> start.col, 3);
        cb -> start.lnum = row1;
        cb -> start.col = col1;
    }
    if (row2 != cb -> end.lnum || col2 != (int) cb -> end.col) {
        clip_invert_area ((int) cb -> end.lnum, cb -> end.col, row2, col2, 3);
        cb -> end.lnum = row2;
        cb -> end.col = col2;
    }
}

int clip_gen_own_selection (cbd)
    VimClipboard * cbd;
{
    return clip_mch_own_selection (cbd);
}

void clip_gen_lose_selection (cbd)
    VimClipboard * cbd;
{
    clip_mch_lose_selection (cbd);
}

void clip_gen_set_selection (cbd)
    VimClipboard * cbd;
{
    clip_mch_set_selection (cbd);
}

void clip_gen_request_selection (cbd)
    VimClipboard * cbd;
{
    clip_mch_request_selection (cbd);
}

static char_u inbuf [4096 + 6];
static int inbufcount = 0;

int vim_is_input_buf_full () {
    return (inbufcount >= 4096);
}
...
char_u * get_input_buf () {
    ...
    if (gap != ((void *) 0)) {
        ...
        if (gap -> ga_data != ((void *) 0)) ((__builtin_object_size ((char *) (gap -> ga_data), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (gap ->
          ga_data), (char *) (inbuf), (size_t) inbufcount, __builtin_object_size ((char *) (gap -> ga_data), 0)) : __inline_memmove_chk ((char *) (gap ->
          ga_data), (char *) (inbuf), (size_t) inbufcount));
    ...
    char_u * p;
{
    ...
    if (gap != ((void *) 0)) {
        if (gap -> ga_data != ((void *) 0)) {
            ((__builtin_object_size ((char *) (inbuf), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (inbuf), (char *) (gap -> ga_data), gap -> ga_len
              , __builtin_object_size ((char *) (inbuf), 0)) : __inline_memmove_chk ((char *) (inbuf), (char *) (gap -> ga_data), gap -> ga_len));
...
int read_from_input_buf (buf, maxlen)
    ...
    long maxlen;
{
    if (inbufcount == 0) fill_input_buf (1);
    if (maxlen > inbufcount) maxlen = inbufcount;
    ((__builtin_object_size ((char *) (buf), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (buf), (char *) (inbuf), (size_t) maxlen,
      __builtin_object_size ((char *) (buf), 0)) : __inline_memmove_chk ((char *) (buf), (char *) (inbuf), (size_t) maxlen));
    inbufcount -= maxlen;
    if (inbufcount) ((__builtin_object_size ((char *) (inbuf), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (inbuf), (char *) (inbuf + maxlen), (
      size_t) inbufcount, __builtin_object_size ((char *) (inbuf), 0)) : __inline_memmove_chk ((char *) (inbuf), (char *) (inbuf + maxlen), (size_t) inbufcount
      ));
    return (int) maxlen;
}

void fill_input_buf (exit_on_error)
    int exit_on_error __attribute__ ((unused));
{
    int len;
    int try;
    static int did_read_something = 0;
    if (vim_is_input_buf_full ()) return;
    len = 0;
    for (try = 0; try < 100; ++try) {
        len = read (read_cmd_fd, (char *) inbuf + inbufcount, (size_t) ((4096 - inbufcount)));
        if (len > 0 || got_int) break;
    ...
    if (len <= 0 && !got_int) read_error_exit ();
    if (len > 0) did_read_something = 1;
    if (got_int) {
        inbuf [0] = 3;
        inbufcount = 1;
    }
        ...
        while (len -- > 0) {
            if (inbuf [inbufcount] == 3 && ctrl_c_interrupts) {
                ((__builtin_object_size ((char *) (inbuf), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (inbuf), (char *) (inbuf + inbufcount), (
                  size_t) (len + 1), __builtin_object_size ((char *) (inbuf), 0)) : __inline_memmove_chk ((char *) (inbuf), (char *) (inbuf + inbufcount), (
                  size_t) (len + 1)));
                inbufcount = 0;
                got_int = 1;
            }
...
void read_error_exit () {
    if (silent_mode) getout (0);
    ((__builtin_object_size ((char *) (IObuff), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (IObuff), (char *) (((char *) (
      "Vim: Error reading input, exiting...\n"))), __builtin_object_size ((char *) (IObuff), 1 > 1)) : __inline_strcpy_chk ((char *) (IObuff), (char *) (((char
      *) ("Vim: Error reading input, exiting...\n")))));
    preserve_exit ();
}
...
{
    if (col < 0) return 0;
    if (col >= (int) screen_Columns) return (int) screen_Columns - 1;
    return col;
}
...
{
    if (row < 0) return 0;
    if (row >= (int) screen_Rows) return (int) screen_Rows - 1;
    return row;
}
    ...
    int which_button;
{
    ...
    mouse_past_eol = 0;
    if (flags & 0x20) {
        if (dragwin != ((void *) 0) && !did_drag) flags &= ~(0x01|0x04);
        dragwin = ((void *) 0);
        did_drag = 0;
    }
    if ((flags & 0x04) && prev_row == mouse_row && prev_col == mouse_col) {
        retnomove : if (on_status_line) return 2;
        if (on_sep_line) return 4;
        if (flags & 0x10) {
            end_visual_mode ();
            redraw_curbuf_later (20);
        }
        if (cmdwin_type != 0 && row < (curwin -> w_winrow)) return 8;
        return 1;
    }
    prev_row = mouse_row;
    prev_col = mouse_col;
    if (flags & 0x08) goto retnomove;
    if (row >= 0 && row < Rows && col >= 0 && col <= Columns && ScreenLines != ((void *) 0)) mouse_char = ScreenLines [LineOffset [row] + col];
    else mouse_char = ' ';
    old_curwin = curwin;
    ...
    if (!(flags&0x01)) {
        ...
        if (on_status_line && on_sep_line) {
            if (stl_connected (wp)) on_sep_line = 0;
            else on_status_line = 0;
        }
        if (VIsual_active && (wp -> w_buffer != curwin -> w_buffer || (!on_status_line && !on_sep_line && (col >= wp -> w_onebuf_opt.wo_fdc + (cmdwin_type == 0
          && wp == curwin ? 0 : 1)) && (flags & 0x10)))) {
            end_visual_mode ();
            redraw_curbuf_later (20);
        }
        if (cmdwin_type != 0 && wp != curwin) {
            on_sep_line = 0;
            if (on_status_line) return 2;
            return 8;
        }
        if (dragwin == ((void *) 0) || (flags & 0x20)) win_enter (wp, 1);
        if (curwin != old_curwin) set_mouse_topline (curwin);
        if (on_status_line) {
            if (curwin == old_curwin) return 2;
            else return 2 | 0x100;
        }
        if (on_sep_line) {
            if (curwin == old_curwin) return 4;
            else return 4 | 0x100;
        }
        ...
        if (row < 0) {
            ...
            for (first = 1; curwin -> w_topline > 1;) {
                if (curwin -> w_topfill < diff_check (curwin, curwin -> w_topline)) ++count;
                else count += plines (curwin -> w_topline - 1);
                if (!first && count > -row) break;
                first = 0;
                hasFolding (curwin -> w_topline, &curwin->w_topline, ((void *) 0));
                if (curwin -> w_topfill < diff_check (curwin, curwin -> w_topline)) ++curwin->w_topfill;
                else {
                    --curwin->w_topline;
                    curwin -> w_topfill = 0;
                }
            ...
            for (first = 1; curwin -> w_topline < curbuf -> b_ml.ml_line_count;) {
                if (curwin -> w_topfill > 0) ++count;
                else count += plines (curwin -> w_topline);
                if (!first && count > row - curwin -> w_height + 1) break;
                first = 0;
                if (hasFolding (curwin -> w_topline, ((void *) 0), &curwin->w_topline) && curwin -> w_topline == curbuf -> b_ml.ml_line_count) break;
                if (curwin -> w_topfill > 0) --curwin->w_topfill;
                else {
                    ++curwin->w_topline;
    ...
    if (col >= curwin -> w_onebuf_opt.wo_fdc + (cmdwin_type == 0 ? 0 : 1)) mouse_char = ' ';
    if (mouse_comp_pos (curwin, &row, &col, &curwin->w_cursor.lnum)) mouse_past_bottom = 1;
    ...
    if (coladvance (col) == 0) {
        if (inclusive != ((void *) 0)) *inclusive = 1;
        mouse_past_eol = 1;
    }
    else if (inclusive != ((void *) 0)) *inclusive = 0;
    count = 1;
    if (curwin != old_curwin || curwin -> w_cursor.lnum != old_cursor.lnum || curwin -> w_cursor.col != old_cursor.col) count |= 0x100;
    if (mouse_char == '+') count |= 0x400;
    else if (mouse_char != ' ') count |= 0x200;
    return count;
}
    ...
    linenr_T * lnump;
{
    ...
    lnum = win -> w_topline;
    while (row > 0) {
        if (win -> w_onebuf_opt.wo_diff && !hasFoldingWin(win,lnum,((void*)0),((void*)0),1,((void*)0))) {
            if (lnum == win -> w_topline) row -= win -> w_topfill;
            else row -= diff_check_fill (win, lnum);
            count = plines_win_nofill (win, lnum, 1);
        }
        else count = plines_win (win, lnum, 1);
        if (count > row) break;
    ...
    if (!win->w_onebuf_opt.wo_wrap) col += win -> w_leftcol;
    col -= win_col_off (win);
    if (col < 0) {
...
win_T * mouse_find_win (rowp, colp)
    int * rowp;
    int * colp __attribute__ ((unused));
{
    frame_T * fp;
    fp = topframe;
    *rowp -= firstwin -> w_winrow;
    for (;;) {
        if (fp -> fr_layout == 0) break;
        if (fp -> fr_layout == 1) {
            for (fp = fp -> fr_child; fp -> fr_next != ((void *) 0); fp = fp -> fr_next) {
                if (*colp < fp -> fr_width) break;
                *colp -= fp -> fr_width;
            }
        }
        else {
            for (fp = fp -> fr_child; fp -> fr_next != ((void *) 0); fp = fp -> fr_next) {
                if (*rowp < fp -> fr_height) break;
                *rowp -= fp -> fr_height;
            }
        }
    }
