...
int hasAnyFolding (win)
    win_T * win;
{
    return (win -> w_onebuf_opt.wo_fen && (!foldmethodIsManual(win) || win -> w_folds.ga_len > 0));
}
    ...
    foldinfo_T * infop;
{
    ...
    if (!hasAnyFolding(win)) {
        if (infop != ((void *) 0)) infop -> fi_level = 0;
        return 0;
    }
    if (cache) {
    ...
    if (first == 0) {
        gap = &win->w_folds;
        for (;;) {
            if (!foldFind(gap,lnum_rel,&fp)) break;
            if (lnum_rel == fp -> fd_top && low_level == 0) low_level = level + 1;
            first += fp -> fd_top;
            last += fp -> fd_top;
    ...
    if (lastp != ((void *) 0)) *lastp = last;
    if (firstp != ((void *) 0)) *firstp = first;
    if (infop != ((void *) 0)) {
...
int foldLevel (lnum)
    linenr_T lnum;
{
    if (invalid_top == (linenr_T) 0) checkupdate (curwin);
    else if (lnum == prev_lnum && prev_lnum_lvl >= 0) return prev_lnum_lvl;
    else if (lnum >= invalid_top && lnum <= invalid_bot) return -1;
    if (!hasAnyFolding(curwin)) return 0;
...
{
    linenr_T last;
    if (hasFoldingWin (win, lnum, ((void *) 0), &last, 0, infop)) return (long) (last - lnum + 1);
    return 0;
}
    ...
    int had_visual;
{
    ...
    for (lnum = first; lnum <= last; lnum = lnum_next + 1) {
        lnum_next = lnum;
        if (opening && !recurse) (void) hasFolding (lnum, ((void *) 0), &lnum_next);
        (void) setManualFold (lnum, opening, recurse, &done);
        if (!opening && !recurse) (void) hasFolding (lnum, ((void *) 0), &lnum_next);
    }
    if (done == 0) emsg ((char_u *) (((char *) (e_nofold))));
    if (had_visual) redraw_curbuf_later (20);
}
    ...
    win_T * wp;
{
    ...
    if (wp -> w_fold_manual) {
        ...
        for (i = 0; i < wp -> w_folds.ga_len; ++i) fp [i].fd_flags = 2;
        wp -> w_fold_manual = 0;
    }
...
void foldCheckClose () {
    if (*p_fcl != '\000') {
        checkupdate (curwin);
        if (checkCloseRec (&curwin->w_folds, curwin -> w_cursor.lnum, (int) curwin -> w_onebuf_opt.wo_fdl)) changed_window_setting ();
    }
}
    ...
    int level;
{
    ...
    for (i = 0; i < gap -> ga_len; ++i) {
        if (fp [i].fd_flags == 0) {
            ...
            else retval |= checkCloseRec (&fp[i].fd_nested, lnum - fp [i].fd_top, level - 1);
        }
    }
    return retval;
}
...
int foldManualAllowed (create)
    int create;
{
    if (foldmethodIsManual (curwin) || foldmethodIsMarker (curwin)) return 1;
    if (create) emsg ((char_u *) (((char *) ("E350: Cannot create fold with current 'foldmethod'"))));
    else emsg ((char_u *) (((char *) ("E351: Cannot delete fold with current 'foldmethod'"))));
    return 0;
}
    ...
    linenr_T end;
{
    ...
    gap = &curwin->w_folds;
    for (;;) {
        if (!foldFind(gap,start_rel,&fp)) break;
        if (fp -> fd_top + fp -> fd_len > end_rel) {
            ...
            if (use_level || fp -> fd_flags == 2) {
                use_level = 1;
                if (level >= curwin -> w_onebuf_opt.wo_fdl) closed = 1;
            }
            else if (fp -> fd_flags == 1) closed = 1;
            ++level;
        }
        else {
            break;
    ...
    if (ga_grow (gap, 1) == 1) {
        ...
        for (cont = 0; i + cont < gap -> ga_len; ++cont) if (fp [cont].fd_top > end_rel) break;
        if (cont > 0 && ga_grow (&fold_ga, cont) == 1) {
            if (start_rel > fp -> fd_top) start_rel = fp -> fd_top;
            if (end_rel < fp [cont - 1].fd_top + fp [cont - 1].fd_len - 1) end_rel = fp [cont - 1].fd_top + fp [cont - 1].fd_len - 1;
            ((__builtin_object_size ((char *) (fold_ga.ga_data), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (fold_ga.ga_data), (char *) (fp),
              sizeof (fold_T) * cont, __builtin_object_size ((char *) (fold_ga.ga_data), 0)) : __inline_memmove_chk ((char *) (fold_ga.ga_data), (char *) (fp),
              sizeof (fold_T) * cont));
            fold_ga.ga_len += cont;
            i += cont;
            for (j = 0; j < cont; ++j) ((fold_T *) fold_ga.ga_data) [j].fd_top -= start_rel;
        }
        if (i < gap -> ga_len) ((__builtin_object_size ((char *) (fp + 1), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (fp + 1), (char *) ((fold_T *
          ) gap -> ga_data + i), sizeof (fold_T) * (gap -> ga_len - i), __builtin_object_size ((char *) (fp + 1), 0)) : __inline_memmove_chk ((char *) (fp + 1)
          , (char *) ((fold_T *) gap -> ga_data + i), sizeof (fold_T) * (gap -> ga_len - i)));
        gap -> ga_len = gap -> ga_len + 1 - cont;
        fp -> fd_nested = fold_ga;
        fp -> fd_top = start_rel;
        fp -> fd_len = end_rel - start_rel + 1;
        if (use_level && !closed && level < curwin -> w_onebuf_opt.wo_fdl) closeFold (start, 1L);
        if (!use_level) curwin -> w_fold_manual = 1;
        fp -> fd_flags = 1;
        fp -> fd_small = 2;
    ...
    int had_visual;
{
    ...
    linenr_T lnum_off;
    int did_one = 0;
    linenr_T first_lnum = (0x7fffffffL);
    ...
    while (lnum <= end) {
        ...
        use_level = 0;
        for (;;) {
            if (!foldFind(gap,lnum-lnum_off,&fp)) break;
            found_ga = gap;
            found_fp = fp;
            found_off = lnum_off;
            if (check_closed (curwin, fp, &use_level, level, &maybe_small, lnum_off)) break;
            gap = &fp->fd_nested;
            lnum_off += fp -> fd_top;
            ++level;
        }
        if (found_ga == ((void *) 0)) {
            ...
            if (foldmethodIsManual (curwin)) deleteFoldEntry (found_ga, (int) (found_fp - (fold_T *) found_ga -> ga_data), recursive);
            else {
                if (first_lnum > found_fp -> fd_top + found_off) first_lnum = found_fp -> fd_top + found_off;
                if (last_lnum < lnum) last_lnum = lnum;
                if (!did_one) parseMarker (curwin);
    ...
    if (!did_one) {
        emsg ((char_u *) (((char *) (e_nofold))));
        if (had_visual) redraw_curbuf_later (20);
    }
    else check_cursor_col ();
    if (last_lnum > 0) changed_lines (first_lnum, (colnr_T) 0, last_lnum, 0L);
}
    ...
    linenr_T bot;
{
    ...
    while (fp < (fold_T *) curwin -> w_folds.ga_data + curwin -> w_folds.ga_len && fp -> fd_top < bot) {
        fp -> fd_small = 2;
        ++fp;
    }
    ...
    for (n = 0; n < count; ++n) {
        ...
        for (;;) {
            if (!foldFind(gap,curwin->w_cursor.lnum-lnum_off,&fp)) {
                if (!updown) break;
                if (dir == 1) {
                    if (fp - (fold_T *) gap -> ga_data >= gap -> ga_len) break;
                    --fp;
                }
                else {
                    if (fp == (fold_T *) gap -> ga_data) break;
                }
                last = 1;
            }
            if (!last) {
                if (check_closed (curwin, fp, &use_level, level, &maybe_small, lnum_off)) last = 1;
                if (last && !updown) break;
            }
            if (updown) {
                if (dir == 1) {
                    if (fp + 1 - (fold_T *) gap -> ga_data < gap -> ga_len) {
                        lnum = fp [1].fd_top + lnum_off;
                        if (lnum > curwin -> w_cursor.lnum) lnum_found = lnum;
                    }
                }
                else {
...
{
    int i;
    for (i = 0; i < win -> w_lines_valid; ++i) if (win -> w_lines [i].wl_valid) {
        if (lnum < win -> w_lines [i].wl_lnum) return -1;
        if (lnum <= win -> w_lines [i].wl_lastlnum) return i;
    }
    return -1;
}
...
void foldAdjustVisual () {
    pos_T * start, * end;
    char_u * ptr;
    if (!VIsual_active || !hasAnyFolding(curwin)) return;
    if (((((VIsual).lnum != (curwin -> w_cursor).lnum) ? (VIsual).lnum < (curwin -> w_cursor).lnum : (VIsual).col != (curwin -> w_cursor).col ? (VIsual).col < (
      curwin -> w_cursor).col : (VIsual).coladd < (curwin -> w_cursor).coladd) || (((VIsual).lnum == (curwin -> w_cursor).lnum) && ((VIsual).col == (curwin ->
      w_cursor).col) && ((VIsual).coladd == (curwin -> w_cursor).coladd)))) {
        start = &VIsual;
        end = &curwin->w_cursor;
    }
    else {
    ...
    if (hasFolding (start -> lnum, &start->lnum, ((void *) 0))) start -> col = 0;
    if (hasFolding (end -> lnum, ((void *) 0), &end->lnum)) {
        ptr = ml_get (end -> lnum);
        end -> col = (colnr_T) strlen ((char *) (ptr));
        if (end -> col > 0 && *p_sel == 'o') --end->col;
    }
}
    ...
    garray_T * to;
{
    ...
    if (from -> ga_len == 0 || ga_grow (to, from -> ga_len) == 0) return;
    ...
    fold_T * * fpp;
{
    ...
    while (low <= high) {
        i = (low + high) / 2;
        if (fp [i].fd_top > lnum) high = i - 1;
        else if (fp [i].fd_top + fp [i].fd_len <= lnum) low = i + 1;
        else {
            * fpp = fp + i;
            return 1;
        }
...
{
    if (wp -> w_foldinvalid) {
        foldUpdate (wp, (linenr_T) 1, (linenr_T) (0x7fffffffL));
        wp -> w_foldinvalid = 0;
    }
}
    ...
    int do_open;
{
    ...
    for (n = 0; n < count; ++n) {
        ...
        if (!(done&1)) {
            if (n == 0 && !(done&2)) emsg ((char_u *) (((char *) (e_nofold))));
            break;
        }
    }
}
    ...
    if (foldmethodIsDiff (curwin) && curwin -> w_onebuf_opt.wo_scb) {
        ...
        for (wp = firstwin; wp != ((void *) 0); wp = wp -> w_next) {
            if (wp != curwin && foldmethodIsDiff (wp) && wp -> w_onebuf_opt.wo_scb) {
                ...
                if (dlnum != 0) (void) setManualFoldWin (wp, dlnum, opening, recurse, ((void *) 0));
            }
        }
    }
    ...
    int * donep;
{
    ...
    for (;;) {
        if (!foldFind(gap,lnum,&fp)) {
            if (fp < (fold_T *) gap -> ga_data + gap -> ga_len) next = fp -> fd_top + off;
            break;
        }
        found_fold = 1;
        if (fp + 1 < (fold_T *) gap -> ga_data + gap -> ga_len) next = fp [1].fd_top + off;
        if (use_level || fp -> fd_flags == 2) {
            use_level = 1;
            if (level >= wp -> w_onebuf_opt.wo_fdl) fp -> fd_flags = 1;
            else fp -> fd_flags = 0;
            fp2 = (fold_T *) fp -> fd_nested.ga_data;
            for (j = 0; j < fp -> fd_nested.ga_len; ++j) fp2 [j].fd_flags = 2;
        }
        if (!opening && recurse) {
            if (fp -> fd_flags != 1) {
                done |= 1;
    ...
    if (found_fold) {
        ...
        wp -> w_fold_manual = 1;
        if (done & 1) changed_window_setting_win (wp);
        done |= 2;
    }
    else if (donep == ((void *) 0) && wp == curwin) emsg ((char_u *) (((char *) (e_nofold))));
    if (donep != ((void *) 0)) *donep |= done;
    return next;
}
    ...
    int recursive;
{
    ...
    if (recursive || fp -> fd_nested.ga_len == 0) {
        ...
        --gap->ga_len;
        if (idx < gap -> ga_len) ((__builtin_object_size ((char *) (fp), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (fp), (char *) (fp + 1), sizeof
          (fold_T) * (gap -> ga_len - idx), __builtin_object_size ((char *) (fp), 0)) : __inline_memmove_chk ((char *) (fp), (char *) (fp + 1), sizeof (fold_T)
          * (gap -> ga_len - idx)));
    }
    else {
        moved = fp -> fd_nested.ga_len;
        if (ga_grow (gap, (int) (moved - 1)) == 1) {
            nfp = (fold_T *) fp -> fd_nested.ga_data;
            for (i = 0; i < moved; ++i) {
                nfp [i].fd_top += fp -> fd_top;
                if (fp -> fd_flags == 2) nfp [i].fd_flags = 2;
                if (fp -> fd_small == 2) nfp [i].fd_small = 2;
            }
            if (idx < gap -> ga_len) ((__builtin_object_size ((char *) (fp + moved), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (fp + moved), (
              char *) (fp + 1), sizeof (fold_T) * (gap -> ga_len - idx), __builtin_object_size ((char *) (fp + moved), 0)) : __inline_memmove_chk ((char *) (fp
              + moved), (char *) (fp + 1), sizeof (fold_T) * (gap -> ga_len - idx)));
            ((__builtin_object_size ((char *) (fp), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (fp), (char *) (nfp), (size_t) (sizeof (fold_T) *
              moved), __builtin_object_size ((char *) (fp), 0)) : __inline_memmove_chk ((char *) (fp), (char *) (nfp), (size_t) (sizeof (fold_T) * moved)));
            vim_free (nfp);
            gap -> ga_len += moved - 1;
        }
    }
...
{
    int i;
    for (i = 0; i < gap -> ga_len; ++i) deleteFoldRecurse (&(((fold_T*)(gap->ga_data))[i].fd_nested));
    ga_clear (gap);
}
...
void foldMarkAdjust (wp, line1, line2, amount, amount_after)
    ...
    long amount_after;
{
    if (amount == (0x7fffffffL) && line2 >= line1 && line2 - line1 >= -amount_after) line2 = line1 - amount_after - 1;
    if ((State & 0x10) && amount == (linenr_T) 1 && line2 == (0x7fffffffL)) --line1;
    ...
    linenr_T lnum_off;
{
    ...
    if (*use_levelp || fp -> fd_flags == 2) {
        * use_levelp = 1;
        if (level >= win -> w_onebuf_opt.wo_fdl) closed = 1;
    }
    else if (fp -> fd_flags == 1) closed = 1;
    if (fp -> fd_small == 2) *maybe_smallp = 1;
    if (closed) {
        if (*maybe_smallp) fp -> fd_small = 2;
        checkSmall (win, fp, lnum_off);
        if (fp -> fd_small == 1) closed = 0;
    }
    return closed;
}
    ...
    linenr_T lnum_off;
{
    ...
    if (fp -> fd_small == 2) {
        ...
        if (fp -> fd_len > curwin -> w_onebuf_opt.wo_fml) fp -> fd_small = 0;
        else {
            count = 0;
    ...
    garray_T * gap;
{
    ...
    fold_T * fp;
    fp = (fold_T *) gap -> ga_data;
    for (i = 0; i < gap -> ga_len; ++i) fp [i].fd_small = 2;
}
    ...
    int markerlen;
{
    ...
    line = ml_get (lnum);
    line_len = (int) strlen ((char *) (line));
    if (u_save (lnum - 1, lnum + 1) == 1) {
        newline = alloc ((unsigned) (line_len + markerlen + strlen ((char *) (cms)) + 1));
        if (newline == ((void *) 0)) return;
        ((__builtin_object_size ((char *) (newline), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (newline), (char *) (line), __builtin_object_size (
          (char *) (newline), 1 > 1)) : __inline_strcpy_chk ((char *) (newline), (char *) (line)));
        if (p == ((void *) 0)) vim_strncpy (newline + line_len, marker, markerlen);
        else {
            ((__builtin_object_size ((char *) (newline + line_len), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (newline + line_len), (char *) (cms)
              , __builtin_object_size ((char *) (newline + line_len), 1 > 1)) : __inline_strcpy_chk ((char *) (newline + line_len), (char *) (cms)));
            ((__builtin_object_size ((char *) (newline + line_len + (p - cms)), 0) != (size_t) - 1) ? __builtin___strncpy_chk ((char *) (newline + line_len + (
              p - cms)), (char *) (marker), (size_t) (markerlen), __builtin_object_size ((char *) (newline + line_len + (p - cms)), 1 > 1)) :
              __inline_strncpy_chk ((char *) (newline + line_len + (p - cms)), (char *) (marker), (size_t) (markerlen)));
            ((__builtin_object_size ((char *) (newline + line_len + (p - cms) + markerlen), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (newline +
              line_len + (p - cms) + markerlen), (char *) (p + 2), __builtin_object_size ((char *) (newline + line_len + (p - cms) + markerlen), 1 > 1)) :
              __inline_strcpy_chk ((char *) (newline + line_len + (p - cms) + markerlen), (char *) (p + 2)));
        }
...
{
    int i;
    if (recursive) for (i = 0; i < fp -> fd_nested.ga_len; ++i) deleteFoldMarkers ((fold_T *) fp -> fd_nested.ga_data + i, 1, lnum_off + fp -> fd_top);
    ...
    int markerlen;
{
    ...
    char_u * cms2;
    line = ml_get (lnum);
    for (p = line; *p != '\000'; ++p) if (strncmp ((char *) (p), (char *) (marker), (size_t) (markerlen)) == 0) {
        len = markerlen;
        if (((p [len]) >= '0' && (p [len]) <= '9')) ++len;
        if (*cms != '\000') {
            cms2 = (char_u *) strstr ((char *) cms, "%s");
            if (p - line >= cms2 - cms && strncmp ((char *) (p - (cms2 - cms)), (char *) (cms), (size_t) (cms2 - cms)) == 0 && strncmp ((char *) (p + len), (
              char *) (cms2 + 2), (size_t) (strlen ((char *) (cms2 + 2)))) == 0) {
                p -= cms2 - cms;
                len += (int) strlen ((char *) (cms)) - 2;
            }
        }
        if (u_save (lnum - 1, lnum + 1) == 1) {
            newline = alloc ((unsigned) (strlen ((char *) (line)) - len + 1));
            if (newline != ((void *) 0)) {
                ((__builtin_object_size ((char *) (newline), 0) != (size_t) - 1) ? __builtin___strncpy_chk ((char *) (newline), (char *) (line), (size_t) (p -
                  line), __builtin_object_size ((char *) (newline), 1 > 1)) : __inline_strncpy_chk ((char *) (newline), (char *) (line), (size_t) (p - line)));
                ((__builtin_object_size ((char *) (newline + (p - line)), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (newline + (p - line)), (char *
                  ) (p + len), __builtin_object_size ((char *) (newline + (p - line)), 1 > 1)) : __inline_strcpy_chk ((char *) (newline + (p - line)), (char *)
                  (p + len)));
                ml_replace (lnum, newline, 0);
            }
        }
    ...
    char_u * buf;
{
    ...
    if (*wp->w_onebuf_opt.wo_fdt != '\000') {
        ...
        int level;
        char_u * p;
        set_vim_var_nr (22, lnum);
        set_vim_var_nr (23, lnume);
        level = foldinfo -> fi_level;
        if (level > (int) sizeof (dashes) - 1) level = (int) sizeof (dashes) - 1;
        ((__builtin_object_size ((dashes), 0) != (size_t) - 1) ? __builtin___memset_chk ((dashes), ('-'), ((size_t) level), __builtin_object_size ((dashes), 0)
          ) : __inline_memset_chk ((dashes), ('-'), ((size_t) level)));
        dashes [level] = '\000';
        set_vim_var_string (24, dashes, -1);
        set_vim_var_nr (25, (long) level);
        save_curwin = curwin;
        curwin = wp;
        curbuf = wp -> w_buffer;
        ++emsg_off;
        text = eval_to_string_safe (wp -> w_onebuf_opt.wo_fdt, ((void *) 0), was_set_insecurely ((char_u *) "foldtext", 4));
        --emsg_off;
        curwin = save_curwin;
        curbuf = curwin -> w_buffer;
        set_vim_var_string (24, ((void *) 0), -1);
        if (text != ((void *) 0)) {
            for (p = text; *p != '\000'; ++p) {
                if (*p == '\011') *p = ' ';
                else if (ptr2cells (p) > 1) break;
            }
            if (*p != '\000') {
                p = transstr (text);
                vim_free (text);
    ...
    if (text == ((void *) 0)) {
        __builtin___sprintf_chk ((char *) buf, 0, __builtin_object_size ((char *) buf, 1 > 1), ((char *) ("+--%3ld lines folded ")), (long) (lnume - lnum + 1))
          ;
        text = buf;
    }
    return text;
    ...
    char_u * str;
{
    ...
    cms_slen = (int) strlen ((char *) (cms_start));
    while (cms_slen > 0 && ((cms_start [cms_slen - 1]) == ' ' || (cms_start [cms_slen - 1]) == '\t')) --cms_slen;
    ...
    for (s = str; *s != '\000';) {
        len = 0;
        if (strncmp ((char *) (s), (char *) (curwin -> w_onebuf_opt.wo_fmr), (size_t) (foldstartmarkerlen)) == 0) len = foldstartmarkerlen;
        else if (strncmp ((char *) (s), (char *) (foldendmarker), (size_t) (foldendmarkerlen)) == 0) len = foldendmarkerlen;
        if (len > 0) {
            if (((s [len]) >= '0' && (s [len]) <= '9')) ++len;
            for (p = s; p > str && ((p [-1]) == ' ' || (p [-1]) == '\t'); --p);
            if (p >= str + cms_slen && strncmp ((char *) (p - cms_slen), (char *) (cms_start), (size_t) (cms_slen)) == 0) {
            ...
            else if (!did2 && cms_elen > 0 && strncmp ((char *) (s), (char *) (cms_end), (size_t) (cms_elen)) == 0) {
                len = cms_elen;
                did2 = 1;
            }
        }
        if (len != 0) {
            ...
            ((__builtin_object_size ((char *) ((s)), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) ((s)), (char *) ((s + len)), strlen ((char *) (s +
              len)) + 1, __builtin_object_size ((char *) ((s)), 0)) : __inline_memmove_chk ((char *) ((s)), (char *) ((s + len)), strlen ((char *) (s + len)) +
              1));
        }
        else {
            ++s;
    ...
    linenr_T bot;
{
    ...
    int level;
    fold_T * fp;
    if (invalid_top != (linenr_T) 0) return;
    if (wp -> w_foldinvalid) {
        top = 1;
    ...
    if (foldmethodIsDiff (wp)) {
        if (top > diff_context) top -= diff_context;
        else top = 1;
        bot += diff_context;
    }
    if (top > wp -> w_buffer -> b_ml.ml_line_count) top = wp -> w_buffer -> b_ml.ml_line_count;
    fold_changed = 0;
    fline.wp = wp;
    fline.off = 0;
    fline.lvl = 0;
    ...
    if (foldmethodIsMarker (wp)) {
        ...
        if (top > 1) {
            ...
            fline.lvl = level;
            getlevel (&fline);
            if (fline.lvl > level) fline.lvl = level - (fline.lvl - fline.lvl_next);
            else fline.lvl = fline.lvl_next;
        }
        fline.lnum = top;
        getlevel (&fline);
    }
        ...
        else if (foldmethodIsSyntax (wp)) getlevel = foldlevelSyntax;
        else if (foldmethodIsDiff (wp)) getlevel = foldlevelDiff;
        else getlevel = foldlevelIndent;
        fline.lvl = -1;
    ...
    if (foldlevelSyntax == getlevel) {
        ...
        while (current_fdl < fline.lvl) {
            if (!foldFind(gap,lnum_rel,&fpn)) break;
            ++current_fdl;
        ...
        if (fpn != ((void *) 0) && current_fdl == fline.lvl) {
            ...
            if (fold_end_lnum > bot) bot = fold_end_lnum;
        }
    }
    start = fline.lnum;
    end = bot;
    if (start > end && end < wp -> w_buffer -> b_ml.ml_line_count) end = start;
    while (!got_int) {
        if (fline.lnum > wp -> w_buffer -> b_ml.ml_line_count) break;
        if (fline.lnum > end) {
            if (getlevel != foldlevelMarker && getlevel != foldlevelSyntax && getlevel != foldlevelExpr) break;
            if ((start <= end && foldFind (&wp->w_folds, end, &fp) && fp -> fd_top + fp -> fd_len - 1 > end) || (fline.lvl == 0 && foldFind (&wp->w_folds, fline
              .lnum, &fp) && fp -> fd_top < fline.lnum)) end = fp -> fd_top + fp -> fd_len - 1;
            else if (getlevel == foldlevelSyntax && foldLevelWin (wp, fline.lnum) != fline.lvl) end = fline.lnum;
            else break;
        }
        if (fline.lvl > 0) {
            invalid_top = fline.lnum;
            invalid_bot = end;
            end = foldUpdateIEMSRecurse (&wp->w_folds, 1, start, &fline, getlevel, end, 2);
            start = fline.lnum;
        }
        else {
            if (fline.lnum == wp -> w_buffer -> b_ml.ml_line_count) break;
    ...
    if (fold_changed && wp -> w_onebuf_opt.wo_fen) changed_window_setting_win (wp);
    if (end != bot) {
        if (wp -> w_redraw_top == 0 || wp -> w_redraw_top > top) wp -> w_redraw_top = top;
        if (wp -> w_redraw_bot < end) wp -> w_redraw_bot = end;
    }
    invalid_top = (linenr_T) 0;
}

static linenr_T foldUpdateIEMSRecurse (gap, level, startlnum, flp, getlevel, bot, topflags)
    garray_T * gap;
    int level;
    linenr_T startlnum;
    fline_T * flp;
    ...
    int topflags;
{
    ...
    if (getlevel == foldlevelMarker && flp -> start <= flp -> lvl - level && flp -> lvl > 0) {
        ...
        if (fp >= ((fold_T *) gap -> ga_data) + gap -> ga_len || fp -> fd_top >= startlnum) fp = ((void *) 0);
    }
    ...
    while (!got_int) {
        ...
        lvl = flp -> lvl;
        if (lvl > 20) lvl = 20;
        if (flp -> lnum > firstlnum && (level > lvl - flp -> start || level >= flp -> had_end)) lvl = 0;
        if (flp -> lnum > bot && !finish && fp != ((void *) 0)) {
            if (getlevel != foldlevelMarker && getlevel != foldlevelExpr && getlevel != foldlevelSyntax) break;
            i = 0;
            fp2 = fp;
            if (lvl >= level) {
            ...
            if (lvl < level + i) {
                ...
                if (fp2 != ((void *) 0)) bot = fp2 -> fd_top + fp2 -> fd_len - 1 + fp -> fd_top;
            }
            else if (fp -> fd_top + fp -> fd_len <= flp -> lnum && lvl >= level) finish = 1;
            else break;
        }
        if (fp == ((void *) 0) && (lvl != level || flp -> lnum_save >= bot || flp -> start != 0 || flp -> had_end <= 20 || flp -> lnum == linecount)) {
            while (!got_int) {
                if (flp -> start != 0 || flp -> had_end <= 20) concat = 0;
                else concat = 1;
                if (foldFind (gap, startlnum, &fp) || (fp < ((fold_T *) gap -> ga_data) + gap -> ga_len && fp -> fd_top <= firstlnum) || foldFind (gap,
                  firstlnum - concat, &fp) || (fp < ((fold_T *) gap -> ga_data) + gap -> ga_len && ((lvl < level && fp -> fd_top < flp -> lnum) || (lvl >= level
                  && fp -> fd_top <= flp -> lnum_save)))) {
                    if (fp -> fd_top + fp -> fd_len + concat > firstlnum) {
                        if (fp -> fd_top >= startlnum && fp -> fd_top != firstlnum) {
                            if (fp -> fd_top > firstlnum) foldMarkAdjustRecurse (&fp->fd_nested, (linenr_T) 0, (linenr_T) (0x7fffffffL), (long) (fp -> fd_top -
                              firstlnum), 0L);
                            else foldMarkAdjustRecurse (&fp->fd_nested, (linenr_T) 0, (long) (firstlnum - fp -> fd_top - 1), (linenr_T) (0x7fffffffL), (long) (
                              fp -> fd_top - firstlnum));
                            fp -> fd_len += fp -> fd_top - firstlnum;
                            fp -> fd_top = firstlnum;
                            fold_changed = 1;
                        }
                        else if (flp -> start != 0 && lvl == level && fp -> fd_top != firstlnum) {
                            foldRemove (&fp->fd_nested, flp -> lnum - fp -> fd_top, flp -> lnum - fp -> fd_top);
                            i = (int) (fp - (fold_T *) gap -> ga_data);
                            foldSplit (gap, i, flp -> lnum, flp -> lnum - 1);
                            fp = (fold_T *) gap -> ga_data + i + 1;
                            if (getlevel == foldlevelMarker || getlevel == foldlevelExpr || getlevel == foldlevelSyntax) finish = 1;
                        }
                        break;
                    }
                    if (fp -> fd_top >= startlnum) {
                        ...
                        foldMarkAdjustRecurse (&fp->fd_nested, (linenr_T) fp -> fd_len, (linenr_T) (0x7fffffffL), (linenr_T) (0x7fffffffL), 0L);
                        fold_changed = 1;
                    }
                }
                else {
                    i = (int) (fp - (fold_T *) gap -> ga_data);
                    if (foldInsert (gap, i) != 1) return bot;
                    fp = (fold_T *) gap -> ga_data + i;
                    fp -> fd_top = firstlnum;
                    ...
                    if (topflags == 0) {
                        ...
                        if (topflags != 2) flp -> wp -> w_fold_manual = 1;
                    }
                    else fp -> fd_flags = (fp - 1) -> fd_flags;
                    fp -> fd_small = 2;
                    if (getlevel == foldlevelMarker || getlevel == foldlevelExpr || getlevel == foldlevelSyntax) finish = 1;
                    fold_changed = 1;
                    break;
        ...
        if (lvl > level && fp != ((void *) 0)) {
            ...
            while (!got_int) {
                ...
                if (++flp->lnum > linecount) break;
                flp -> lvl = flp -> lvl_next;
                getlevel (flp);
                if (flp -> lvl >= 0 || flp -> had_end <= 20) break;
            }
            prev_lnum = 0;
            if (flp -> lnum > linecount) break;
            flp -> lnum_save = flp -> lnum;
    ...
    if (lvl < level) {
        if (fp -> fd_len != flp -> lnum - fp -> fd_top) {
            ...
            else fp -> fd_len = flp -> lnum - fp -> fd_top;
            fold_changed = 1;
        }
    }
    for (;;) {
        fp2 = fp + 1;
        if (fp2 >= (fold_T *) gap -> ga_data + gap -> ga_len || fp2 -> fd_top > flp -> lnum) break;
        if (fp2 -> fd_top + fp2 -> fd_len > flp -> lnum) {
            if (fp2 -> fd_top < flp -> lnum) {
                foldMarkAdjustRecurse (&fp2->fd_nested, (linenr_T) 0, (long) (flp -> lnum - fp2 -> fd_top - 1), (linenr_T) (0x7fffffffL), (long) (fp2 -> fd_top
                  - flp -> lnum));
    ...
    if (bot < flp -> lnum - 1) bot = flp -> lnum - 1;
    return bot;
}
...
{
    fold_T * fp;
    if (ga_grow (gap, 1) != 1) return 0;
    fp = (fold_T *) gap -> ga_data + i;
    if (i < gap -> ga_len) ((__builtin_object_size ((char *) (fp + 1), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (fp + 1), (char *) (fp), sizeof (
      fold_T) * (gap -> ga_len - i), __builtin_object_size ((char *) (fp + 1), 0)) : __inline_memmove_chk ((char *) (fp + 1), (char *) (fp), sizeof (fold_T) * (
      gap -> ga_len - i)));
    ++gap->ga_len;
    ...
    linenr_T bot;
{
    ...
    int idx;
    int len;
    if (foldInsert (gap, i + 1) == 0) return;
    fp = (fold_T *) gap -> ga_data + i;
    fp [1].fd_top = bot + 1;
    ...
    for (;;) {
        ...
        if (fp -> fd_top >= top) {
            ...
            if (fp -> fd_top + fp -> fd_len - 1 > bot) {
                foldMarkAdjustRecurse (&fp->fd_nested, (linenr_T) 0, (long) (bot - fp -> fd_top), (linenr_T) (0x7fffffffL), (long) (fp -> fd_top - bot - 1));
    ...
    fold_T * fp2;
{
    ...
    if (foldFind (gap1, fp1 -> fd_len - 1L, &fp3) && foldFind (gap2, 0L, &fp4)) foldMerge (fp3, gap2, fp4);
    if (gap2 -> ga_len > 0 && ga_grow (gap1, gap2 -> ga_len) == 1) {
    ...
    fline_T * flp;
{
    ...
    if (*s == '\000' || vim_strchr (flp -> wp -> w_onebuf_opt.wo_fdi, *s) != ((void *) 0)) {
        if (lnum == 1 || lnum == buf -> b_ml.ml_line_count) flp -> lvl = 0;
        else flp -> lvl = -1;
    }
    else flp -> lvl = get_indent_buf (buf, lnum) / buf -> b_p_sw;
    if (flp -> lvl > flp -> wp -> w_onebuf_opt.wo_fdn) {
        flp -> lvl = flp -> wp -> w_onebuf_opt.wo_fdn;
        if (flp -> lvl < 0) flp -> lvl = 0;
    }
}
...
static void foldlevelDiff (flp)
    fline_T * flp;
{
    if (diff_infold (flp -> wp, flp -> lnum + flp -> off)) flp -> lvl = 1;
    else flp -> lvl = 0;
}
    ...
    fline_T * flp;
{
    ...
    case 's' :
        if (flp -> lvl >= 0) {
            if (n > flp -> lvl) flp -> lvl_next = 0;
            else flp -> lvl_next = flp -> lvl - n;
    ...
    if (flp -> lvl < 0) {
        ...
        if (lnum == curbuf -> b_ml.ml_line_count) flp -> lvl_next = 0;
    }
    curwin = win;
    curbuf = curwin -> w_buffer;
}
    ...
    win_T * wp;
{
    ...
    foldstartmarkerlen = (int) (foldendmarker ++ - wp -> w_onebuf_opt.wo_fmr);
    foldendmarkerlen = (int) strlen ((char *) (foldendmarker));
}
    ...
    fline_T * flp;
{
    ...
    while (*s) {
        if (*s == cstart && strncmp ((char *) (s + 1), (char *) (startmarker), (size_t) (foldstartmarkerlen - 1)) == 0) {
            ...
            if (((*s) >= '0' && (*s) <= '9')) {
                ...
                if (n > 0) {
                    flp -> lvl = n;
                    flp -> lvl_next = n;
                    if (n <= start_lvl) flp -> start = 1;
                    else flp -> start = n - start_lvl;
                }
            }
            else {
                ++flp->lvl;
            ...
            if (((*s) >= '0' && (*s) <= '9')) {
                ...
                if (n > 0) {
                    flp -> lvl = n;
                    flp -> lvl_next = n - 1;
                    if (flp -> lvl_next > start_lvl) flp -> lvl_next = start_lvl;
                }
            }
            else --flp->lvl_next;
        }
    ...
    if (flp -> lvl_next < 0) flp -> lvl_next = 0;
}
...
static int put_foldopen_recurse (FILE * fd, garray_T * gap, linenr_T off);
    ...
    win_T * wp;
{
    if (foldmethodIsManual (wp)) {
        if (put_line (fd, "silent! normal! zE") == 0 || put_folds_recurse (fd, &wp->w_folds, (linenr_T) 0) == 0) return 0;
    }
    if (wp -> w_fold_manual) return put_foldopen_recurse (fd, &wp->w_folds, (linenr_T) 0);
    return 1;
}
    ...
    linenr_T off;
{
    ...
    fp = (fold_T *) gap -> ga_data;
    for (i = 0; i < gap -> ga_len; i ++) {
        if (put_folds_recurse (fd, &fp->fd_nested, off + fp -> fd_top) == 0) return 0;
        if (fprintf (fd, "%ld,%ldfold", fp -> fd_top + off, fp -> fd_top + off + fp -> fd_len - 1) < 0 || put_eol (fd) == 0) return 0;
        ++fp;
    }
    return 1;
}

static int put_foldopen_recurse (fd, gap, off)
    FILE * fd;
    garray_T * gap;
    linenr_T off;
{
    ...
    for (i = 0; i < gap -> ga_len; i ++) {
        if (fp -> fd_flags != 2) {
            if (fp -> fd_nested.ga_len > 0) {
                if (fprintf (fd, "%ld", fp -> fd_top + off) < 0 || put_eol (fd) == 0 || put_line (fd, "normal zo") == 0) return 0;
                if (put_foldopen_recurse (fd, &fp->fd_nested, off + fp -> fd_top) == 0) return 0;
            }
            if (fprintf (fd, "%ld", fp -> fd_top + off) < 0 || put_eol (fd) == 0 || fprintf (fd, "normal z%c", fp -> fd_flags == 1 ? 'c' : 'o') < 0 || put_eol (
              fd) == 0) return 0;
        }
        ++fp;
    }
