...
{"-arabic", "+autocmd", "-balloon_eval", "-browse", "+builtin_terms", "+byte_offset", "+cindent", "-clientserver", "+clipboard", "+cmdline_compl",
      ...
      "-mouse_netterm", "-mouse_sysmouse", "+mouse_xterm", "-multi_byte", "+multi_lang", "-mzscheme", "+netbeans_intg", "-osfiletype", "+path_extra", "-perl",
      ...
      , "+vreplace", "+wildignore", "+wildmenu", "+windows", "+writebackup", "-X11", "-xfontset", "-xim", "-xsmp", "-xterm_clipboard", "-xterm_save", ((void *)
      0)};
...
int highest_patch () {
    int i;
    int h = 0;
    for (i = 0; included_patches [i] != 0; ++i) if (included_patches [i] > h) h = included_patches [i];
    return h;
}
...
{
    int i;
    for (i = 0; included_patches [i] != 0; ++i) if (included_patches [i] == n) return 1;
    return 0;
}
...
void list_version () {
    ...
    if (included_patches [0] != 0) {
        ...
        while (--i >= 0) {
            if (first < 0) first = included_patches [i];
    ...
    int colon;
{
    ...
    char * p;
    static char * (lines []) =
    {"VIM - Vi IMproved", "", "version ", "by Bram Moolenaar et al.", "Vim is open source and freely distributable", "", "Help poor children in Uganda!",
          "type  :help iccf<Enter>       for information ", "", "type  :q<Enter>               to exit         ",
          "type  :help<Enter>  or  <F1>  for on-line help", "type  :help version7<Enter>   for version info", ((void *) 0), "", "Running in Vi compatible mode"
          , "type  :set nocp<Enter>        for Vim defaults", "type  :help cp-default<Enter> for info on this",};
    blanklines = (int) Rows - ((sizeof (lines) / sizeof (char *)) - 1);
    if (!p_cp) blanklines += 4;
    if (p_ls > 1) blanklines -= Rows - topframe -> fr_height;
    if (blanklines < 0) blanklines = 0;
    ...
    if ((row >= 2 && Columns >= 50) || colon) {
        for (i = 0; i < (int) (sizeof (lines) / sizeof (char *)); ++i) {
            ...
            if (sponsor != 0) {
                if (strstr (p, "children") != ((void *) 0)) p = sponsor < 0 ? "Sponsor Vim development!" : "Become a registered Vim user!";
                else if (strstr (p, "iccf") != ((void *) 0)) p = sponsor < 0 ? "type  :help sponsor<Enter>    for information " :
                  "type  :help register<Enter>   for information ";
                else if (strstr (p, "Orphans") != ((void *) 0)) p = "menu  Help->Sponsor/Register  for information    ";
            }
            if (*p != '\000') do_intro_line (row, (char_u *) ((char *) (p)), i == 2, 0);
            ++row;
        }
    }
    ...
    int attr;
{
    ...
    if (add_version) {
        ((__builtin_object_size ((char *) (vers), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (vers), (char *) (mediumVersion), __builtin_object_size
          ((char *) (vers), 1 > 1)) : __inline_strcpy_chk ((char *) (vers), (char *) (mediumVersion)));
        if (highest_patch ()) {
            if (isalpha ((int) mediumVersion [3])) {
                if (isalpha ((int) mediumVersion [4])) __builtin___sprintf_chk ((char *) vers + 5, 0, __builtin_object_size ((char *) vers + 5, 1 > 1), ".%d%s"
                  , highest_patch (), mediumVersion + 5);
                else __builtin___sprintf_chk ((char *) vers + 4, 0, __builtin_object_size ((char *) vers + 4, 1 > 1), ".%d%s", highest_patch (), mediumVersion +
                  4);
            }
            else __builtin___sprintf_chk ((char *) vers + 3, 0, __builtin_object_size ((char *) vers + 3, 1 > 1), ".%d", highest_patch ());
        }
    ...
    for (p = mesg; *p != '\000'; p += l) {
        clen = 0;
        for (l = 0; p [l] != '\000' && (l == 0 || (p [l] != '<' && p [l - 1] != '>')); ++l) {
            clen += byte2cells (p [l]);
        }
        screen_puts_len (p, l, row, col, *p == '<' ? highlight_attr [(int) (HLF_8)] : attr);
        col += clen;
    }
    if (add_version) screen_puts (vers, row, col, 0);
}
