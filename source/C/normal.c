...
} nv_cmds [] =
{
    ...
    {(-(('F')+((int)('B')<<8))), nv_nbcmd, (0x04 | 0x01), 0},
    {(-((253)+((int)(KE_CURSORHOLD)<<8))), nv_cursorhold, 0x100, 0},};
static short nv_cmd_idx [(sizeof (nv_cmds) / sizeof (struct nv_cmd))];
static int nv_max_linear;

static int nv_compare (s1, s2) const
    void * s1;
    const void * s2;
{
    int c1, c2;
    c1 = nv_cmds [*(constshort*)s1].cmd_char;
    c2 = nv_cmds [*(constshort*)s2].cmd_char;
    if (c1 < 0) c1 = -c1;
    if (c2 < 0) c2 = -c2;
    return c1 - c2;
}
...
void init_normal_cmds () {
    int i;
    for (i = 0; i < (int) (sizeof (nv_cmds) / sizeof (struct nv_cmd)); ++i) nv_cmd_idx [i] = i;
    qsort ((void *) &nv_cmd_idx, (size_t) (sizeof (nv_cmds) / sizeof (struct nv_cmd)), sizeof (short), nv_compare);
    for (i = 0; i < (int) (sizeof (nv_cmds) / sizeof (struct nv_cmd)); ++i) if (i != nv_cmds [nv_cmd_idx [i]].cmd_char) break;
    nv_max_linear = i - 1;
}
    ...
    int cmdchar;
{
    ...
    int top, bot;
    int c;
    if (cmdchar < 0) cmdchar = -cmdchar;
    if (cmdchar <= nv_max_linear) return nv_cmd_idx [cmdchar];
    bot = nv_max_linear + 1;
    top = (sizeof (nv_cmds) / sizeof (struct nv_cmd)) - 1;
    idx = -1;
    while (bot <= top) {
        i = (top + bot) / 2;
...
void normal_cmd (oap, toplevel)
    oparg_T * oap;
    int toplevel __attribute__ ((unused));
{
    cmdarg_T ca;
    int c;
    int ctrl_w = 0;
    ...
    int idx;
    int set_prevcount = 0;
    ((__builtin_object_size ((&ca), 0) != (size_t) - 1) ? __builtin___memset_chk ((&ca), (0), (sizeof (ca)), __builtin_object_size ((&ca), 0)) :
      __inline_memset_chk ((&ca), (0), (sizeof (ca))));
    ca.oap = oap;
    ca.opcount = opcount;
    c = finish_op;
    ...
    c = safe_vgetc ();
    ;
    if (restart_edit == 0) old_mapped_len = 0;
    else if (old_mapped_len || (VIsual_active && mapped_len == 0 && typebuf_maplen () > 0)) old_mapped_len = typebuf_maplen ();
    if (c == '\000') c = (-((255)+((int)(('X'))<<8)));
    if (VIsual_active && VIsual_select && (vim_isprintc (c) || c == '\012' || c == '\015' || c == (-(('K')+((int)('A')<<8))))) {
        ins_char_typebuf (c);
    ...
    getcount : if (!(VIsual_active&&VIsual_select)) {
        while ((c >= '1' && c <= '9') || (ca.count0 != 0 && (c == (-(('k')+((int)('D')<<8))) || c == (-((253)+((int)(KE_KDEL)<<8))) || c == '0'))) {
            if (c == (-(('k')+((int)('D')<<8))) || c == (-((253)+((int)(KE_KDEL)<<8)))) {
                ca.count0 /= 10;
                del_from_showcmd (4);
            }
            else ca.count0 = ca.count0 * 10 + (c - '0');
            if (ca.count0 < 0) ca.count0 = 999999999L;
            if (toplevel && stuff_empty ()) {
                long count = ca.count0;
                if (ca.opcount != 0) count = ca.opcount * (count == 0 ? 1 : count);
    ...
    if (c == (-((253)+((int)(KE_CURSORHOLD)<<8)))) {
        ...
        if (ca.count0) ca.count0 *= ca.opcount;
        else ca.count0 = ca.opcount;
    }
    ca.opcount = ca.count0;
    ca.count1 = (ca.count0 == 0 ? 1 : ca.count0);
    if (toplevel && stuff_empty ()) set_vcount (ca.count0, ca.count1, set_prevcount);
    if (ctrl_w) {
        ca.nchar = c;
        ca.cmdchar = 23;
    }
    ...
    if ((nv_cmds [idx].cmd_flags & 0x200) && curbuf_locked ()) goto normal_end;
    if (VIsual_active) {
        if (km_stopsel && (nv_cmds [idx].cmd_flags & 0x40) && !(mod_mask&0x02)) {
            end_visual_mode ();
        ...
        if (km_startsel) {
            ...
            else if ((nv_cmds [idx].cmd_flags & 0x20) && (mod_mask & 0x02)) {
                mod_mask &= ~0x02;
            }
        }
    }
    if ((nv_cmds [idx].cmd_flags & 0x01) && (((nv_cmds [idx].cmd_flags & (0x02 | 0x01)) == (0x02 | 0x01) && oap -> op_type == 0) || (nv_cmds [idx].cmd_flags & (
      0x04 | 0x01)) == (0x04 | 0x01) || (ca.cmdchar == 'q' && oap -> op_type == 0 && !Recording && !Exec_reg) || ((ca.cmdchar == 'a' || ca.cmdchar == 'i') && (
      oap -> op_type != 0 || VIsual_active)))) {
        int * cp;
        int repl = 0;
        int lit = 0;
        int langmap_active = 0;
        int lang;
        ++no_mapping;
        ++allow_keys;
        did_cursorhold = 1;
        if (ca.cmdchar == 'g') {
            ...
            if (ca.nchar == 'r' || ca.nchar == '\'' || ca.nchar == '`' || ca.nchar == 28) {
                cp = &ca.extra_char;
                if (ca.nchar != 'r') lit = 1;
                else repl = 1;
            }
        ...
        if (cp != ((void *) 0)) {
            ...
            * cp = plain_vgetc ();
            ...
            if (!lit) {
                if (*cp == 11 && ((nv_cmds [idx].cmd_flags & 0x08) || cp == &ca.extra_char) && vim_strchr (p_cpo, 'D') == ((void *) 0)) {
                    c = get_digraph (0);
                    if (c > 0) {
                        * cp = c;
                        del_from_showcmd (3);
            ...
            if (cp == &ca.extra_char && ca.nchar == 28 && (ca.extra_char == 14 || ca.extra_char == 7)) {
                ca.cmdchar = 28;
    ...
    if (need_flushbuf) out_flush ();
    if (ca.cmdchar != (-((253)+((int)(KE_IGNORE)<<8)))) did_cursorhold = 0;
    State = 0x01;
    if (ca.nchar == '\033') {
        clearop (oap);
        if (restart_edit == 0 && goto_im ()) restart_edit = 'a';
        goto normal_end;
    }
    ...
    if (!finish_op && !oap->op_type && (idx < 0 || !(nv_cmds[idx].cmd_flags&0x100))) {
        clearop (oap);
        set_reg_var ('"');
    }
    if (old_mapped_len > 0) old_mapped_len = typebuf_maplen ();
    ...
    if (((p_smd && msg_silent == 0 && (restart_edit != 0 || (VIsual_active && old_pos.lnum == curwin -> w_cursor.lnum && old_pos.col == curwin -> w_cursor.col)
      ) && (clear_cmdline || redraw_cmdline) && (msg_didout || (msg_didany && msg_scroll)) && !msg_nowait && KeyTyped) || (restart_edit != 0 && !VIsual_active
      && (msg_scroll || emsg_on_display))) && oap -> regname == 0 && !(ca.retval&1) && stuff_empty () && typebuf_typed () && emsg_silent == 0 && 
      !did_wait_return && oap -> op_type == 0) {
        int save_State = State;
        if (restart_edit != 0) State = 0x10;
        ...
        cursor_on ();
        out_flush ();
        if (msg_scroll || emsg_on_display) ui_delay (1000L, 1);
        ui_delay (3000L, 0);
        State = save_State;
        msg_scroll = 0;
        emsg_on_display = 0;
    }
    normal_end : msg_nowait = 0;
    c = finish_op;
    finish_op = 0;
    if (c || ca.cmdchar == 'r') {
        ui_cursor_shape ();
    }
    if (oap -> op_type == 0 && oap -> regname == 0 && ca.cmdchar != (-((253)+((int)(KE_CURSORHOLD)<<8)))) clear_showcmd ();
    checkpcmark ();
    ...
    if (oap -> op_type == 0 && ((restart_edit != 0 && !VIsual_active && old_mapped_len == 0) || restart_VIsual_select == 1) && !(ca.retval&1) && stuff_empty ()
      && oap -> regname == 0) {
        ...
        if (restart_edit != 0 && !VIsual_active && old_mapped_len == 0) (void) edit (restart_edit, 0, 1L);
    }
    if (restart_VIsual_select == 2) restart_VIsual_select = 1;
    opcount = ca.opcount;
}
    ...
    int gui_yank;
{
    ...
    if (clip_star.available && oap -> op_type != 0 && !gui_yank && VIsual_active && !redo_VIsual_busy && oap -> regname == 0) clip_auto_select ();
    old_cursor = curwin -> w_cursor;
    if ((finish_op || VIsual_active) && oap -> op_type != 0) {
        oap -> is_VIsual = VIsual_active;
        if (oap -> motion_force == 'V') oap -> motion_type = 1;
        else if (oap -> motion_force == 'v') {
            if (oap -> motion_type == 1) oap -> inclusive = 0;
            else if (oap -> motion_type == 0) oap -> inclusive = !oap->inclusive;
            oap -> motion_type = 0;
        }
        ...
        if ((vim_strchr (p_cpo, 'y') != ((void *) 0) || oap -> op_type != 2) && (!VIsual_active || oap -> motion_force) && cap -> cmdchar != 'D' && oap ->
          op_type != 19 && oap -> op_type != 20 && oap -> op_type != 21 && oap -> op_type != 22 && oap -> op_type != 23 && oap -> op_type != 24 && oap ->
          op_type != 25) {
            prep_redo (oap -> regname, cap -> count0, get_op_char (oap -> op_type), get_extra_op_char (oap -> op_type), oap -> motion_force, cap -> cmdchar, cap
              -> nchar);
            if (cap -> cmdchar == '/' || cap -> cmdchar == '?') {
                if (vim_strchr (p_cpo, 'r') == ((void *) 0)) AppendToRedobuffLit (cap -> searchbuf, -1);
                AppendToRedobuff ((char_u *) "\012");
            }
            else if (cap -> cmdchar == ':') {
                if (repeat_cmdline == ((void *) 0)) ResetRedobuff ();
                else {
        ...
        if (redo_VIsual_busy) {
            ...
            if (curwin -> w_cursor.lnum > curbuf -> b_ml.ml_line_count) curwin -> w_cursor.lnum = curbuf -> b_ml.ml_line_count;
            VIsual_mode = redo_VIsual_mode;
            if (VIsual_mode == 'v') {
                if (redo_VIsual_line_count <= 1) curwin -> w_cursor.col += redo_VIsual_col - 1;
                else curwin -> w_cursor.col = redo_VIsual_col;
            }
            if (redo_VIsual_col == (0x7fffffffL)) {
                curwin -> w_curswant = (0x7fffffffL);
                coladvance ((colnr_T) (0x7fffffffL));
            }
            cap -> count0 = redo_VIsual_count;
            if (redo_VIsual_count != 0) cap -> count1 = redo_VIsual_count;
            else cap -> count1 = 1;
        }
            ...
            if (VIsual_select && VIsual_mode == 'V') {
                if ((((VIsual).lnum != (curwin -> w_cursor).lnum) ? (VIsual).lnum < (curwin -> w_cursor).lnum : (VIsual).col != (curwin -> w_cursor).col ? (
                  VIsual).col < (curwin -> w_cursor).col : (VIsual).coladd < (curwin -> w_cursor).coladd)) {
                    VIsual.col = 0;
                    curwin -> w_cursor.col = (colnr_T) strlen ((char *) (ml_get (curwin -> w_cursor.lnum)));
                }
                else {
            ...
            oap -> start = VIsual;
            if (VIsual_mode == 'V') oap -> start.col = 0;
        }
        if ((((oap -> start).lnum != (curwin -> w_cursor).lnum) ? (oap -> start).lnum < (curwin -> w_cursor).lnum : (oap -> start).col != (curwin -> w_cursor).
          col ? (oap -> start).col < (curwin -> w_cursor).col : (oap -> start).coladd < (curwin -> w_cursor).coladd)) {
            if (!VIsual_active) {
                if (hasFolding (oap -> start.lnum, &oap->start.lnum, ((void *) 0))) oap -> start.col = 0;
                if (hasFolding (curwin -> w_cursor.lnum, ((void *) 0), &curwin->w_cursor.lnum)) curwin -> w_cursor.col = (colnr_T) strlen ((char *) (
                  ml_get_curline ()));
            }
            ...
            if (!VIsual_active && oap -> motion_type == 1) {
                if (hasFolding (curwin -> w_cursor.lnum, &curwin->w_cursor.lnum, ((void *) 0))) curwin -> w_cursor.col = 0;
                if (hasFolding (oap -> start.lnum, ((void *) 0), &oap->start.lnum)) oap -> start.col = (colnr_T) strlen ((char *) (ml_get (oap -> start.lnum)))
                  ;
            }
            oap -> end = oap -> start;
            oap -> start = curwin -> w_cursor;
        ...
        if (VIsual_active || redo_VIsual_busy) {
            if (VIsual_mode == 22) {
                ...
                if (!redo_VIsual_busy) {
                    ...
                    if (start < oap -> start_vcol) oap -> start_vcol = start;
                    if (end > oap -> end_vcol) {
                        if (*p_sel == 'e' && start >= 1 && start - 1 >= oap -> end_vcol) oap -> end_vcol = start - 1;
                        else oap -> end_vcol = end;
                    }
                }
                if (curwin -> w_curswant == (0x7fffffffL)) {
                    ...
                    for (curwin -> w_cursor.lnum = oap -> start.lnum; curwin -> w_cursor.lnum <= oap -> end.lnum; ++curwin->w_cursor.lnum) {
                        ...
                        if (end > oap -> end_vcol) oap -> end_vcol = end;
                    }
                }
                else if (redo_VIsual_busy) oap -> end_vcol = oap -> start_vcol + redo_VIsual_col - 1;
            ...
            if (!redo_VIsual_busy && !gui_yank) {
                ...
                if (curwin -> w_curswant == (0x7fffffffL)) resel_VIsual_col = (0x7fffffffL);
                else if (VIsual_mode == 22) resel_VIsual_col = oap -> end_vcol - oap -> start_vcol + 1;
                else if (oap -> line_count > 1) resel_VIsual_col = oap -> end.col;
                else resel_VIsual_col = oap -> end.col - oap -> start.col + 1;
                resel_VIsual_line_count = oap -> line_count;
            }
            if ((vim_strchr (p_cpo, 'y') != ((void *) 0) || oap -> op_type != 2) && oap -> op_type != 10 && oap -> op_type != 19 && oap -> op_type != 20 && oap
              -> op_type != 21 && oap -> op_type != 22 && oap -> op_type != 23 && oap -> op_type != 24 && oap -> op_type != 25 && oap -> motion_force == '\000'
              ) {
                prep_redo (oap -> regname, 0L, '\000', 'v', get_op_char (oap -> op_type), get_extra_op_char (oap -> op_type), oap -> op_type == 16 ? cap ->
                  nchar : '\000');
                if (!redo_VIsual_busy) {
            ...
            if (oap -> motion_force == '\000' || oap -> motion_type == 1) oap -> inclusive = 1;
            if (VIsual_mode == 'V') oap -> motion_type = 1;
            else {
                oap -> motion_type = 0;
                if (VIsual_mode != 22 && *ml_get_pos(&(oap->end)) == '\000' && (include_line_break || !virtual_op)) {
                    oap -> inclusive = 0;
            ...
            if (!gui_yank) {
                ...
                setmouse ();
                mouse_dragging = 0;
                if (mode_displayed) clear_cmdline = 1;
                else clear_showcmd ();
                if ((oap -> op_type == 2 || oap -> op_type == 10 || oap -> op_type == 27 || oap -> op_type == 6) && oap -> motion_force == '\000')
                  redraw_curbuf_later (20);
            }
        }
        curwin -> w_set_curswant = 1;
        oap -> empty = (oap -> motion_type == 0 && (!oap->inclusive || (oap -> op_type == 2 && gchar_pos (&oap->end) == '\000')) && (((oap -> start).lnum == (
          oap -> end).lnum) && ((oap -> start).col == (oap -> end).col) && ((oap -> start).coladd == (oap -> end).coladd)) && 
          !(virtual_op&&oap->start.coladd!=oap->end.coladd));
        empty_region_error = (oap -> empty && vim_strchr (p_cpo, 'E') != ((void *) 0));
        if (oap -> is_VIsual && (oap -> empty || !curbuf->b_p_ma || oap -> op_type == 19)) redraw_curbuf_later (20);
        if (oap -> motion_type == 0 && oap -> inclusive == 0 && !(cap->retval&2) && oap -> end.col == 0 && (!oap->is_VIsual || *p_sel == 'o') && 
          !oap->block_mode && oap -> line_count > 1) {
            oap -> end_adjusted = 1;
            --oap->line_count;
            --oap->end.lnum;
            if (inindent (0)) oap -> motion_type = 1;
            else {
        ...
        else oap -> end_adjusted = 0;
        switch (oap -> op_type) {
        case 4 :
        case 5 :
            op_shift (oap, 1, oap -> is_VIsual ? (int) cap -> count1 : 1);
            auto_format (0, 1);
            break;
        case 14 :
        case 13 :
            if (oap -> line_count < 2) oap -> line_count = 2;
            if (curwin -> w_cursor.lnum + oap -> line_count - 1 > curbuf -> b_ml.ml_line_count) beep_flush ();
            else {
                (void) do_join (oap -> line_count, oap -> op_type == 13, 1);
                auto_format (0, 1);
            }
            break;
        case 1 :
            VIsual_reselect = 0;
            if (empty_region_error) vim_beep ();
            else {
                (void) op_delete (oap);
                if (oap -> motion_type == 1 && has_format_option ('a')) u_save_cursor ();
                auto_format (0, 1);
            }
            break;
        case 2 :
            if (empty_region_error) {
                if (!gui_yank) vim_beep ();
            }
            else (void) op_yank (oap, 0, !gui_yank);
            check_cursor_col ();
            break;
        case 3 :
            VIsual_reselect = 0;
            if (empty_region_error) vim_beep ();
                ...
                if (p_im || !KeyTyped) restart_edit_save = restart_edit;
                else restart_edit_save = 0;
                restart_edit = 0;
                finish_op = 0;
                if (op_change (oap)) cap -> retval |= 1;
                if (restart_edit == 0) restart_edit = restart_edit_save;
            }
            break;
        case 6 :
            if (vim_strchr (p_cpo, '!') != ((void *) 0)) AppendToRedobuff ((char_u *) "!\r");
            else bangredo = 1;
        case 8 :
        case 10 :
            if (oap -> op_type == 8 && *get_equalprg() == '\000') {
                ...
                op_reindent (oap, *curbuf->b_p_inde != '\000' ? get_expr_indent : get_c_indent);
                break;
            }
            op_colon (oap);
            break;
        ...
        case 15 :
            ...
            break;
        case 9 :
            if (*curbuf->b_p_fex != '\000') op_formatexpr (oap);
            else if (*p_fp != '\000') op_colon (oap);
            else op_format (oap, 0);
            break;
        case 26 :
            op_format (oap, 1);
            break;
        ...
        case 23 :
            VIsual_reselect = 0;
            opFoldRange (oap -> start.lnum, oap -> end.lnum, oap -> op_type == 20 || oap -> op_type == 21, oap -> op_type == 21 || oap -> op_type == 23, oap ->
              is_VIsual);
            break;
        case 24 :
        case 25 :
            VIsual_reselect = 0;
            deleteFold (oap -> start.lnum, oap -> end.lnum, oap -> op_type == 25, oap -> is_VIsual);
            break;
        default :
            clearopbeep (oap);
        }
        virtual_op = 2;
        if (!gui_yank) {
            if (!p_sol && oap -> motion_type == 1 && !oap->end_adjusted && (oap -> op_type == 4 || oap -> op_type == 5 || oap -> op_type == 1)) coladvance (
...
{
    stuffcharReadbuff (':');
    if (oap -> is_VIsual) stuffReadbuff ((char_u *) "'<,'>");
    else {
        if (oap -> start.lnum == curwin -> w_cursor.lnum) stuffcharReadbuff ('.');
        else stuffnumReadbuff ((long) oap -> start.lnum);
        if (oap -> end.lnum != oap -> start.lnum) {
            stuffcharReadbuff (',');
            if (oap -> end.lnum == curwin -> w_cursor.lnum) stuffcharReadbuff ('.');
            else if (oap -> end.lnum == curbuf -> b_ml.ml_line_count) stuffcharReadbuff ('$');
            else if (oap -> start.lnum == curwin -> w_cursor.lnum) {
                stuffReadbuff ((char_u *) ".+");
                stuffnumReadbuff ((long) oap -> line_count - 1);
            }
            else stuffnumReadbuff ((long) oap -> end.lnum);
        }
    }
    if (oap -> op_type != 10) stuffReadbuff ((char_u *) "!");
    if (oap -> op_type == 8) {
        stuffReadbuff (get_equalprg ());
        stuffReadbuff ((char_u *) "\n");
    }
    else if (oap -> op_type == 9) {
        if (*p_fp == '\000') stuffReadbuff ((char_u *) "fmt");
        else stuffReadbuff (p_fp);
        stuffReadbuff ((char_u *) "\n']");
    }
}

static void op_function (oap)
    oparg_T * oap __attribute__ ((unused));
{
    char_u * (argv [1]);
    if (*p_opfunc == '\000') emsg ((char_u *) (((char *) ("E774: 'operatorfunc' is empty"))));
    else {
        curbuf -> b_op_start = oap -> start;
        curbuf -> b_op_end = oap -> end;
        if (oap -> motion_type != 1 && !oap->inclusive) decl (&curbuf->b_op_end);
        if (oap -> block_mode) argv [0] = (char_u *) "block";
        else if (oap -> motion_type == 1) argv [0] = (char_u *) "line";
        else argv [0] = (char_u *) "char";
    ...
    int fixindent;
{
    ...
    if (do_always) do_always = 0;
    else {
        if (VIsual_active) {
            if (!mouse_has('v')) return 0;
        }
        else if (State == 0x01 && !mouse_has('n')) return 0;
    }
    which_button = get_mouse_button ((((unsigned) (-(c)) >> 8) & 0xff), &is_click, &is_drag);
    if (is_click) got_click = 1;
    else {
        if (!got_click) return 0;
        if (!is_drag) {
            got_click = 0;
            if (in_tab_line) {
                in_tab_line = 0;
    ...
    if (is_click && (mod_mask & 0x04) && which_button == 0x02) {
        if (State & 0x10) stuffcharReadbuff (15);
        if (count > 1) stuffnumReadbuff (count);
        stuffcharReadbuff (20);
        got_click = 0;
        return 0;
    }
    if ((mod_mask & 0x04) && which_button != 0x00) return 0;
    if ((mod_mask & (0x02 | 0x04 | 0x08 | 0x10)) && (!is_click || (mod_mask & (0x20 | 0x40 | 0x60)) || which_button == 0x01) && 
      !((mod_mask&(0x02|0x08))&&mouse_model_popup()&&which_button==0x00) && !((mod_mask&0x08)&&!mouse_model_popup() && which_button == 0x02)) return 0;
    if (!is_click && which_button == 0x01) return 0;
    if (oap != ((void *) 0)) regname = oap -> regname;
    else regname = 0;
    if (which_button == 0x01) {
        ...
        else if ((State & 0x10) == 0) return 0;
        if ((State & 0x10) || !mouse_has('n')) {
            if (regname == '.') insert_reg (regname, 1);
            else {
                if (clip_star.available && regname == 0) regname = '*';
                if ((State & 0x40) && !yank_register_mline(regname)) insert_reg (regname, 1);
                else {
                    do_put (regname, (-1), 1L, fixindent | 2);
    ...
    if (!is_click) jump_flags |= 0x01 | 0x04;
    start_visual.lnum = 0;
        ...
        if (is_click && cmdwin_type == 0 && mouse_col < Columns) {
            ...
            if (c1 >= 0) {
                if ((mod_mask & (0x20 | 0x40 | 0x60)) == 0x20) {
                    ...
                    goto_tabpage (c1);
                    if (curwin != old_curwin) end_visual_mode ();
                }
            }
            else if (c1 < 0) {
                tabpage_T * tp;
                if (c1 == -999) tp = curtab;
                else tp = find_tabpage (-c1);
                if (tp == curtab) {
                    if (first_tabpage -> tp_next != ((void *) 0)) tabpage_close (0);
                }
                else if (tp != ((void *) 0)) tabpage_close_other (tp, 0);
            }
        }
        return 1;
    }
    ...
    if (mouse_model_popup ()) {
        if (which_button == 0x02 && !(mod_mask&(0x02|0x04))) {
            return 0;
        }
        if (which_button == 0x00 && (mod_mask & (0x02 | 0x08))) {
            which_button = 0x02;
            mod_mask &= ~0x02;
        }
    }
    if ((State & (0x01 | 0x10)) && !(mod_mask&(0x02|0x04))) {
        if (which_button == 0x00) {
            if (is_click) {
                if (VIsual_active) jump_flags |= 0x10;
            }
            else if (mouse_has ('v')) jump_flags |= 0x02;
        }
        else if (which_button == 0x02) {
            if (is_click && VIsual_active) {
                if ((((curwin -> w_cursor).lnum != (VIsual).lnum) ? (curwin -> w_cursor).lnum < (VIsual).lnum : (curwin -> w_cursor).col != (VIsual).col ? (
                  curwin -> w_cursor).col < (VIsual).col : (curwin -> w_cursor).coladd < (VIsual).coladd)) {
            ...
            jump_flags |= 0x01;
            if (mouse_has ('v')) jump_flags |= 0x02;
        }
    }
    ...
    if (!is_click && !is_drag) jump_flags |= 0x20;
    jump_flags = jump_to_mouse (jump_flags, oap == ((void *) 0) ? ((void *) 0) : &(oap->inclusive), which_button);
    moved = (jump_flags & 0x100);
    in_status_line = (jump_flags & 2);
    in_sep_line = (jump_flags & 4);
    if (isNetbeansBuffer (curbuf) && !(jump_flags&(2|4))) {
        int key = (((unsigned) (-(c)) >> 8) & 0xff);
        if (key == (int) KE_LEFTRELEASE || key == (int) KE_MIDDLERELEASE || key == (int) KE_RIGHTRELEASE) netbeans_button_release (which_button);
    }
    if (curwin != old_curwin && oap != ((void *) 0) && oap -> op_type != 0) clearop (oap);
    if (mod_mask == 0 && !is_drag && (jump_flags & (0x200 | 0x400)) && which_button == 0x00) {
        if (jump_flags & 0x400) openFold (curwin -> w_cursor.lnum, 1L);
        else closeFold (curwin -> w_cursor.lnum, 1L);
        if (curwin == old_curwin) curwin -> w_cursor = save_cursor;
    }
    if ((jump_flags & 8) && !VIsual_active && clip_star.available) {
        clip_modeless (which_button, is_click, is_drag);
        return 0;
    }
    if (VIsual_active && is_drag && p_so) {
        if (mouse_row == 0) mouse_dragging = 2;
        else mouse_dragging = 1;
    }
    ...
    if (start_visual.lnum) {
        if (mod_mask & 0x08) VIsual_mode = 22;
        if (VIsual_mode == 22) {
            getvcols (curwin, &start_visual, &end_visual, &leftcol, &rightcol);
            if (curwin -> w_curswant > (leftcol + rightcol) / 2) end_visual.col = leftcol;
            else end_visual.col = rightcol;
            if (curwin -> w_cursor.lnum < (start_visual.lnum + end_visual.lnum) / 2) end_visual.lnum = end_visual.lnum;
            else end_visual.lnum = start_visual.lnum;
            start_visual = curwin -> w_cursor;
            ...
            if ((((curwin -> w_cursor).lnum != (start_visual).lnum) ? (curwin -> w_cursor).lnum < (start_visual).lnum : (curwin -> w_cursor).col != (
              start_visual).col ? (curwin -> w_cursor).col < (start_visual).col : (curwin -> w_cursor).coladd < (start_visual).coladd)) VIsual = end_visual;
            else if ((((end_visual).lnum != (curwin -> w_cursor).lnum) ? (end_visual).lnum < (curwin -> w_cursor).lnum : (end_visual).col != (curwin -> w_cursor
              ).col ? (end_visual).col < (curwin -> w_cursor).col : (end_visual).coladd < (curwin -> w_cursor).coladd)) VIsual = start_visual;
            else {
                if (end_visual.lnum == start_visual.lnum) {
                    if (curwin -> w_cursor.col - start_visual.col > end_visual.col - curwin -> w_cursor.col) VIsual = start_visual;
                    else VIsual = end_visual;
                }
                else {
                    diff = (curwin -> w_cursor.lnum - start_visual.lnum) - (end_visual.lnum - curwin -> w_cursor.lnum);
                    if (diff > 0) VIsual = start_visual;
                    else if (diff < 0) VIsual = end_visual;
                    else {
                        if (curwin -> w_cursor.col < (start_visual.col + end_visual.col) / 2) VIsual = end_visual;
                        else VIsual = start_visual;
                    }
                }
            }
        }
    }
    else if ((State & 0x10) && VIsual_active) stuffcharReadbuff (15);
    if (which_button == 0x01) {
        if (clip_star.available && regname == 0) regname = '*';
        if (yank_register_mline (regname)) {
            if (mouse_past_bottom) dir = 1;
        }
        else if (mouse_past_eol) dir = 1;
        if (fixindent) {
            c1 = (dir == (-1)) ? '[' : ']';
            c2 = 'p';
        }
        ...
        if (restart_edit != 0) where_paste_started = curwin -> w_cursor;
        do_put (regname, dir, count, fixindent | 2);
    }
    else if (((mod_mask & 0x04) || (mod_mask & (0x20 | 0x40 | 0x60)) == 0x20) && bt_quickfix (curbuf)) {
        if (State & 0x10) stuffcharReadbuff (15);
        if (curwin -> w_llist_ref == ((void *) 0)) stuffReadbuff ((char_u *) ":.cc\n");
        else stuffReadbuff ((char_u *) ":.ll\n");
        got_click = 0;
    }
    else if ((mod_mask & 0x04) || (curbuf -> b_help && (mod_mask & (0x20 | 0x40 | 0x60)) == 0x20)) {
        if (State & 0x10) stuffcharReadbuff (15);
        stuffcharReadbuff (29);
        got_click = 0;
    }
    else if ((mod_mask & 0x02)) {
        if (State & 0x10 || (VIsual_active && VIsual_select)) stuffcharReadbuff (15);
        if (which_button == 0x00) stuffcharReadbuff ('*');
        else stuffcharReadbuff ('#');
    }
    else if (in_status_line) {
    }
    else if (in_sep_line) {
    }
    else if ((mod_mask & (0x20 | 0x40 | 0x60)) && (State & (0x01 | 0x10)) && mouse_has ('v')) {
        if (is_click || !VIsual_active) {
            if (VIsual_active) orig_cursor = VIsual;
            else {
            ...
            else if ((mod_mask & (0x20 | 0x40 | 0x60)) == 0x40) VIsual_mode = 'V';
            else if ((mod_mask & (0x20 | 0x40 | 0x60)) == 0x60) VIsual_mode = 22;
            clip_star.vmode = '\000';
        }
        if ((mod_mask & (0x20 | 0x40 | 0x60)) == 0x20) {
            pos_T * pos = ((void *) 0);
            ...
            if (is_click) {
                end_visual = curwin -> w_cursor;
                while (gc = gchar_pos (&end_visual), ((gc) == ' ' || (gc) == '\t')) inc (&end_visual);
                if (oap != ((void *) 0)) oap -> motion_type = 0;
                if (oap != ((void *) 0) && VIsual_mode == 'v' && !vim_iswordc(gchar_pos(&end_visual)) && (((curwin -> w_cursor).lnum == (VIsual).lnum) && ((
                  curwin -> w_cursor).col == (VIsual).col) && ((curwin -> w_cursor).coladd == (VIsual).coladd)) && (pos = findmatch (oap, '\000')) != ((void *)
                  0)) {
                    curwin -> w_cursor = *pos;
                    if (oap -> motion_type == 1) VIsual_mode = 'V';
                    else if (*p_sel == 'e') {
                        if ((((curwin -> w_cursor).lnum != (VIsual).lnum) ? (curwin -> w_cursor).lnum < (VIsual).lnum : (curwin -> w_cursor).col != (VIsual).
                          col ? (curwin -> w_cursor).col < (VIsual).col : (curwin -> w_cursor).coladd < (VIsual).coladd)) ++VIsual.col;
                        else ++curwin->w_cursor.col;
                    }
                }
            }
            if (pos == ((void *) 0) && (is_click || is_drag)) {
                if ((((curwin -> w_cursor).lnum != (orig_cursor).lnum) ? (curwin -> w_cursor).lnum < (orig_cursor).lnum : (curwin -> w_cursor).col != (
                  orig_cursor).col ? (curwin -> w_cursor).col < (orig_cursor).col : (curwin -> w_cursor).coladd < (orig_cursor).coladd)) {
                    find_start_of_word (&curwin->w_cursor);
                    find_end_of_word (&VIsual);
                }
                else {
                    find_start_of_word (&VIsual);
                    if (*p_sel == 'e' && *ml_get_cursor() != '\000') ++curwin->w_cursor.col;
                    find_end_of_word (&curwin->w_cursor);
                }
            }
            curwin -> w_set_curswant = 1;
        }
        if (is_click) redraw_curbuf_later (20);
    }
    ...
    pos_T * pos;
{
    ...
    while (pos -> col > 0) {
        col = pos -> col - 1;
        if (get_mouse_class (line + col) != cclass) break;
        pos -> col = col;
    }
}
    ...
    char_u * p;
{
    ...
    c = *p;
    if (c == ' ' || c == '\t') return 0;
    if (vim_iswordc (c)) return 2;
    if (c != '\000' && vim_strchr ((char_u *) "-+*/%<>&|^!=", c) != ((void *) 0)) return 1;
    return c;
}
...
void check_visual_highlight () {
    ...
    if (full_screen) {
        if (!did_check && highlight_attr [(int) (HLF_V)] == 0) msg ((char_u *) (((char *) ("Warning: terminal cannot highlight"))));
        did_check = 1;
    }
}

void end_visual_mode () {
    if (clip_star.available && clip_star.owned) clip_auto_select ();
    VIsual_active = 0;
    setmouse ();
    mouse_dragging = 0;
    curbuf -> b_visual.vi_mode = VIsual_mode;
    ...
    int find_type;
{
    ...
    for (i = (find_type & 1) ? 0 : 1; i < 2; ++i) {
        col = startcol;
        while (ptr [col] != '\000' && (i == 0 ? !vim_iswordc(ptr[col]) : ((ptr [col]) == ' ' || (ptr [col]) == '\t'))) ++col;
        {
            while (col > 0 && ((i == 0 ? vim_iswordc (ptr [col - 1]) : (!((ptr[col-1])==' '||(ptr[col-1])=='\t') && (!(find_type&1) || !vim_iswordc(ptr[col-1])
              ))))) --col;
            if (!(find_type&2) || vim_iswordc (ptr [col])) break;
        }
    }
    if (ptr [col] == '\000' || (i == 0 && (!vim_iswordc(ptr[col])))) {
        if (find_type & 2) emsg ((char_u *) (((char *) ("E348: No string under cursor"))));
        else emsg ((char_u *) (((char *) (e_noident))));
        return 0;
    }
    ptr += col;
    * string = ptr;
    col = 0;
    while ((i == 0 ? vim_iswordc (ptr [col]) : (ptr [col] != '\000' && !((ptr[col])==' '||(ptr[col])=='\t')))) {
        ++col;
    }
    return col;
}
...
static void prep_redo_cmd (cap)
    cmdarg_T * cap;
{
    prep_redo (cap -> oap -> regname, cap -> count0, '\000', cap -> cmdchar, '\000', '\000', cap -> nchar);
}
    ...
    int cmd5;
{
    ...
    if (num) AppendNumberToRedobuff (num);
    if (cmd1 != '\000') AppendCharToRedobuff (cmd1);
    if (cmd2 != '\000') AppendCharToRedobuff (cmd2);
    if (cmd3 != '\000') AppendCharToRedobuff (cmd3);
    if (cmd4 != '\000') AppendCharToRedobuff (cmd4);
    if (cmd5 != '\000') AppendCharToRedobuff (cmd5);
}

static int checkclearop (oap)
    oparg_T * oap;
{
    if (oap -> op_type == 0) return 0;
    clearopbeep (oap);
    return 1;
}
...
static int checkclearopq (oap)
    oparg_T * oap;
{
    if (oap -> op_type == 0 && !VIsual_active) return 0;
    clearopbeep (oap);
    return 1;
}
...
static int showcmd_visual = 0;
static void display_showcmd (void);

void clear_showcmd () {
    if (!p_sc) return;
    if (VIsual_active && !char_avail()) {
        int cursor_bot = (((VIsual).lnum != (curwin -> w_cursor).lnum) ? (VIsual).lnum < (curwin -> w_cursor).lnum : (VIsual).col != (curwin -> w_cursor).col ?
          (VIsual).col < (curwin -> w_cursor).col : (VIsual).coladd < (curwin -> w_cursor).coladd);
        long lines;
        colnr_T leftcol, rightcol;
        linenr_T top, bot;
        ...
        if (VIsual_mode == 22) {
            ...
            p_sbr = saved_sbr;
            __builtin___sprintf_chk ((char *) showcmd_buf, 0, __builtin_object_size ((char *) showcmd_buf, 1 > 1), "%ldx%ld", lines, (long) (rightcol - leftcol
              + 1));
        }
        else if (VIsual_mode == 'V' || VIsual.lnum != curwin -> w_cursor.lnum) __builtin___sprintf_chk ((char *) showcmd_buf, 0, __builtin_object_size ((char *
          ) showcmd_buf, 1 > 1), "%ld", lines);
        else {
            char_u * s, * e;
            int l;
            int bytes = 0;
            ...
            if (bytes == chars) __builtin___sprintf_chk ((char *) showcmd_buf, 0, __builtin_object_size ((char *) showcmd_buf, 1 > 1), "%d", chars);
            else __builtin___sprintf_chk ((char *) showcmd_buf, 0, __builtin_object_size ((char *) showcmd_buf, 1 > 1), "%d-%d", chars, bytes);
        }
        showcmd_buf [10] = '\000';
        showcmd_visual = 1;
    }
    ...
    int c;
{
    ...
    int i;
    static int ignore [] =
    {(-((253)+((int)(KE_IGNORE)<<8))), (-((253)+((int)(KE_LEFTMOUSE)<<8))), (-((253)+((int)(KE_LEFTDRAG)<<8))), (-((253)+((int)(KE_LEFTRELEASE)<<8))), 
          (-((253)+((int)(KE_MIDDLEMOUSE)<<8))), (-((253)+((int)(KE_MIDDLEDRAG)<<8))), (-((253)+((int)(KE_MIDDLERELEASE)<<8))), 
          (-((253)+((int)(KE_RIGHTMOUSE)<<8))), (-((253)+((int)(KE_RIGHTDRAG)<<8))), (-((253)+((int)(KE_RIGHTRELEASE)<<8))), (-((253)+((int)(KE_MOUSEDOWN)<<8))
          ), (-((253)+((int)(KE_MOUSEUP)<<8))), (-((253)+((int)(KE_MOUSELEFT)<<8))), (-((253)+((int)(KE_MOUSERIGHT)<<8))), (-((253)+((int)(KE_X1MOUSE)<<8))), (
          -((253)+((int)(KE_X1DRAG)<<8))), (-((253)+((int)(KE_X1RELEASE)<<8))), (-((253)+((int)(KE_X2MOUSE)<<8))), (-((253)+((int)(KE_X2DRAG)<<8))), 
          (-((253)+((int)(KE_X2RELEASE)<<8))), (-((253)+((int)(KE_CURSORHOLD)<<8))), 0};
    if (!p_sc || msg_silent != 0) return 0;
    if (showcmd_visual) {
        showcmd_buf [0] = '\000';
        showcmd_visual = 0;
    }
    if (((c) < 0)) for (i = 0; ignore [i] != 0; ++i) if (ignore [i] == c) return 0;
    p = transchar (c);
    old_len = (int) strlen ((char *) (showcmd_buf));
    extra_len = (int) strlen ((char *) (p));
    overflow = old_len + extra_len - 10;
    if (overflow > 0) ((__builtin_object_size ((char *) (showcmd_buf), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (showcmd_buf), (char *) (
      showcmd_buf + overflow), old_len - overflow + 1, __builtin_object_size ((char *) (showcmd_buf), 0)) : __inline_memmove_chk ((char *) (showcmd_buf), (char
      *) (showcmd_buf + overflow), old_len - overflow + 1));
    ((__builtin_object_size ((char *) (showcmd_buf), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (showcmd_buf), (char *) (p), __builtin_object_size (
      (char *) (showcmd_buf), 1 > 1)) : __inline_strcat_chk ((char *) (showcmd_buf), (char *) (p)));
    if (char_avail ()) return 0;
    display_showcmd ();
    return 1;
}
    ...
    int len;
{
    ...
    if (!char_avail()) display_showcmd ();
}

void push_showcmd () {
    if (p_sc) ((__builtin_object_size ((char *) (old_showcmd_buf), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (old_showcmd_buf), (char *) (
      showcmd_buf), __builtin_object_size ((char *) (old_showcmd_buf), 1 > 1)) : __inline_strcpy_chk ((char *) (old_showcmd_buf), (char *) (showcmd_buf)));
}

void pop_showcmd () {
    if (!p_sc) return;
    ((__builtin_object_size ((char *) (showcmd_buf), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (showcmd_buf), (char *) (old_showcmd_buf),
      __builtin_object_size ((char *) (showcmd_buf), 1 > 1)) : __inline_strcpy_chk ((char *) (showcmd_buf), (char *) (old_showcmd_buf)));
    display_showcmd ();
}
    ...
    int check;
{
    ...
    if (check && curwin -> w_onebuf_opt.wo_scb) {
        if (did_syncbind) did_syncbind = 0;
        else if (curwin == old_curwin) {
            if ((curwin -> w_buffer == old_buf || curwin -> w_onebuf_opt.wo_diff) && (curwin -> w_topline != old_topline || curwin -> w_topfill != old_topfill
        ...
        if (curwin != old_curwin && curwin -> w_onebuf_opt.wo_scb) {
            if (want_ver) {
                if (old_curwin -> w_onebuf_opt.wo_diff && curwin -> w_onebuf_opt.wo_diff) {
                    ...
                    if (topline > curbuf -> b_ml.ml_line_count) topline = curbuf -> b_ml.ml_line_count;
...
static void nv_nop (cap)
    cmdarg_T * cap __attribute__ ((unused));
{
}
...
static void nv_help (cap)
    cmdarg_T * cap;
{
    if (!checkclearopq(cap->oap)) ex_help (((void *) 0));
}

static void nv_addsub (cap)
    cmdarg_T * cap;
{
    if (!checkclearopq(cap->oap) && do_addsub ((int) cap -> cmdchar, cap -> count1) == 1) prep_redo_cmd (cap);
}
    ...
    cmdarg_T * cap;
{
    if (!checkclearop(cap->oap)) {
        if (mod_mask & 0x04) {
            if (cap -> arg == (-1)) goto_tabpage (-(int)cap->count1);
    ...
    int thisblock;
{
    int len;
    char_u * ptr;
    if ((len = find_ident_under_cursor (&ptr, 1)) == 0 || find_decl (ptr, len, nchar == 'd', thisblock, 0) == 0) clearopbeep (oap);
    else if ((fdo_flags & 0x040) && KeyTyped && oap -> op_type == 0) foldOpenCursor ();
}
    ...
    int searchflags;
{
    ...
    int retval = 1;
    int incll;
    if ((pat = alloc (len + 7)) == ((void *) 0)) return 0;
    __builtin___sprintf_chk ((char *) pat, 0, __builtin_object_size ((char *) pat, 1 > 1), vim_iswordp (ptr) ? "\\V\\<%.*s\\>" : "\\V%.*s", len, ptr);
    old_pos = curwin -> w_cursor;
    save_p_ws = p_ws;
    save_p_scs = p_scs;
    p_ws = 0;
    ...
    if (!locally || !findpar(&incll, (-1), 1L, '{', 0)) {
        ...
        par_pos = curwin -> w_cursor;
        while (curwin -> w_cursor.lnum > 1 && *skipwhite(ml_get_curline()) != '\000') --curwin->w_cursor.lnum;
    }
    curwin -> w_cursor.col = 0;
    {
        (&found_pos) -> lnum = 0;
    ...
    for (;;) {
        t = searchit (curwin, curbuf, &curwin->w_cursor, 1, pat, 1L, searchflags, 2, (linenr_T) 0, ((void *) 0));
        if (curwin -> w_cursor.lnum >= old_pos.lnum) t = 0;
        if (thisblock && t != 0) {
            pos_T * pos;
            if ((pos = findmatchlimit (((void *) 0), '}', 0x02, (int) (old_pos.lnum - curwin -> w_cursor.lnum + 1))) != ((void *) 0) && pos -> lnum < old_pos.
              lnum) continue;
        }
        if (t == 0) {
            if (found_pos.lnum != 0) {
        ...
        if (get_leader_len (ml_get_curline (), ((void *) 0), 0) > 0) {
        ...
        if (curwin -> w_cursor.lnum >= par_pos.lnum) {
            if (found_pos.lnum != 0) curwin -> w_cursor = found_pos;
            break;
        }
    ...
    long dist;
{
    ...
    if (curwin -> w_width != 0) {
        if (curwin -> w_curswant == (0x7fffffffL)) {
            atend = 1;
            validate_virtcol ();
            if (width1 <= 0) curwin -> w_curswant = 0;
            else {
                curwin -> w_curswant = width1 - 1;
                if (curwin -> w_virtcol > curwin -> w_curswant) curwin -> w_curswant += ((curwin -> w_virtcol - curwin -> w_curswant - 1) / width2 + 1) * width2
                  ;
            }
        }
        else {
            if (linelen > width1) n = ((linelen - width1 - 1) / width2 + 1) * width2 + width1;
            else n = width1;
            if (curwin -> w_curswant > (colnr_T) n + 1) curwin -> w_curswant -= ((curwin -> w_curswant - n) / width2 + 1) * width2;
        }
        while (dist --) {
            if (dir == (-1)) {
                if ((long) curwin -> w_curswant >= width2) curwin -> w_curswant -= width2;
                else {
                    ...
                    --curwin->w_cursor.lnum;
                    if (!(fdo_flags&0x001)) (void) hasFolding (curwin -> w_cursor.lnum, &curwin->w_cursor.lnum, ((void *) 0));
                    linelen = linetabsize (ml_get_curline ());
                    if (linelen > width1) curwin -> w_curswant += (((linelen - width1 - 1) / width2) + 1) * width2;
                }
            }
            else {
                if (linelen > width1) n = ((linelen - width1 - 1) / width2 + 1) * width2 + width1;
                else n = width1;
                if (curwin -> w_curswant + width2 < (colnr_T) n) curwin -> w_curswant += width2;
                else {
                    (void) hasFolding (curwin -> w_cursor.lnum, ((void *) 0), &curwin->w_cursor.lnum);
    ...
    if (curwin -> w_cursor.col > 0 && curwin -> w_onebuf_opt.wo_wrap) {
        validate_virtcol ();
        if (curwin -> w_virtcol > curwin -> w_curswant && (curwin -> w_curswant < (colnr_T) width1 ? (curwin -> w_curswant > (colnr_T) width1 / 2) : ((curwin ->
          w_curswant - width1) % width2 > (colnr_T) width2 / 2))) --curwin->w_cursor.col;
    }
    if (atend) curwin -> w_curswant = (0x7fffffffL);
    return retval;
}
...
static void nv_scroll_line (cap)
    cmdarg_T * cap;
{
    if (!checkclearop(cap->oap)) scroll_redraw (cap -> arg, cap -> count1);
}
    ...
    long count;
{
    ...
    if (up) scrollup (count, 1);
    else scrolldown (count, 1);
    if (p_so) {
        cursor_correct ();
        ...
        while (curwin -> w_topline == prev_topline && curwin -> w_topfill == prev_topfill) {
            if (up) {
                if (curwin -> w_cursor.lnum > prev_lnum || cursor_down (1L, 0) == 0) break;
            }
            else {
                if (curwin -> w_cursor.lnum < prev_lnum || prev_topline == 1L || cursor_up (1L, 0) == 0) break;
            }
            check_cursor_moved (curwin);
            curwin -> w_valid |= 0x80;
        }
    }
    if (curwin -> w_cursor.lnum != prev_lnum) coladvance (curwin -> w_curswant);
    redraw_later (10);
}
    ...
    cmdarg_T * cap;
{
    ...
    int undo = 0;
    if (((nchar) >= '0' && (nchar) <= '9')) {
        if (checkclearop (cap -> oap)) return;
        n = nchar - '0';
        for (;;) {
            ++no_mapping;
            ++allow_keys;
            ...
            if (nchar == (-(('k')+((int)('D')<<8))) || nchar == (-((253)+((int)(KE_KDEL)<<8)))) n /= 10;
            else if (((nchar) >= '0' && (nchar) <= '9')) n = n * 10 + (nchar - '0');
            else if (nchar == '\015') {
                win_setheight ((int) n);
                break;
            }
    ...
    dozet : if (cap -> nchar != 'f' && cap -> nchar != 'F' && !(VIsual_active&&vim_strchr((char_u*)"dcCoO",cap->nchar)) && cap -> nchar != 'j' && cap -> nchar
      != 'k' && checkclearop (cap -> oap)) return;
    if ((vim_strchr ((char_u *) "+\r\nt.z^-b", nchar) != ((void *) 0)) && cap -> count0 && cap -> count0 != curwin -> w_cursor.lnum) {
        setpcmark ();
        if (cap -> count0 > curbuf -> b_ml.ml_line_count) curwin -> w_cursor.lnum = curbuf -> b_ml.ml_line_count;
    ...
    case '+' :
        if (cap -> count0 == 0) {
            validate_botline ();
            if (curwin -> w_botline > curbuf -> b_ml.ml_line_count) curwin -> w_cursor.lnum = curbuf -> b_ml.ml_line_count;
    ...
    case '^' :
        ...
        else if (curwin -> w_topline == 1) curwin -> w_cursor.lnum = 1;
    ...
    case (-(('k')+((int)('l')<<8))) :
        if (!curwin->w_onebuf_opt.wo_wrap) {
            if ((colnr_T) cap -> count1 > curwin -> w_leftcol) curwin -> w_leftcol = 0;
    ...
    case 's' :
        if (!curwin->w_onebuf_opt.wo_wrap) {
            if (hasFolding (curwin -> w_cursor.lnum, ((void *) 0), ((void *) 0))) col = 0;
            else getvcol (curwin, &curwin->w_cursor, &col, ((void *) 0), ((void *) 0));
            if ((long) col > p_siso) col -= p_siso;
            else col = 0;
    ...
    case 'e' :
        if (!curwin->w_onebuf_opt.wo_wrap) {
            if (hasFolding (curwin -> w_cursor.lnum, ((void *) 0), ((void *) 0))) col = 0;
            else getvcol (curwin, &curwin->w_cursor, ((void *) 0), ((void *) 0), &col);
            n = (curwin -> w_width) - curwin_col_off ();
            if ((long) col + p_siso < n) col = 0;
            else col = col + p_siso - n + 1;
            if (curwin -> w_leftcol != col) {
                curwin -> w_leftcol = col;
                redraw_later (40);
            }
    ...
    case 'd' :
    case 'D' :
        if (foldManualAllowed (0)) {
            if (VIsual_active) nv_operator (cap);
            else deleteFold (curwin -> w_cursor.lnum, curwin -> w_cursor.lnum, nchar == 'D', 0);
        }
        break;
    case 'E' :
        ...
        else if (foldmethodIsMarker (curwin)) deleteFold ((linenr_T) 1, curbuf -> b_ml.ml_line_count, 1, 0);
        else emsg ((char_u *) (((char *) ("E352: Cannot erase folds with current 'foldmethod'"))));
        break;
    case 'n' :
    ...
    case 'i' :
        ...
        break;
    case 'a' :
        if (hasFolding (curwin -> w_cursor.lnum, ((void *) 0), ((void *) 0))) openFold (curwin -> w_cursor.lnum, cap -> count1);
        else {
        ...
        break;
    case 'A' :
        if (hasFolding (curwin -> w_cursor.lnum, ((void *) 0), ((void *) 0))) openFoldRecurse (curwin -> w_cursor.lnum);
        else {
    ...
    case 'X' :
        ...
        break;
    case 'm' :
        if (curwin -> w_onebuf_opt.wo_fdl > 0) --curwin->w_onebuf_opt.wo_fdl;
        old_fdl = -1;
        curwin -> w_onebuf_opt.wo_fen = 1;
        break;
    case 'M' :
    ...
    case 'r' :
        ...
        break;
    case 'R' :
        curwin -> w_onebuf_opt.wo_fdl = getDeepestNesting ();
        old_fdl = -1;
        break;
    case 'j' :
    case 'k' :
        if (foldMoveTo (1, nchar == 'j' ? 1 : (-1), cap -> count1) == 0) clearopbeep (cap -> oap);
        break;
    case 'u' :
        ++no_mapping;
        ++allow_keys;
    ...
    case 'W' :
        {
            char_u * ptr = ((void *) 0);
            int len;
            if (checkclearop (cap -> oap)) break;
            if (VIsual_active && get_visual_text (cap, &ptr, &len) == 0) return;
            if (ptr == ((void *) 0)) {
                pos_T pos = curwin -> w_cursor;
                len = spell_move_to (curwin, 1, 1, 1, ((void *) 0));
                if (len != 0 && curwin -> w_cursor.col <= pos.col) ptr = ml_get_pos (&curwin->w_cursor);
                curwin -> w_cursor = pos;
            }
            if (ptr == ((void *) 0) && (len = find_ident_under_cursor (&ptr, 1)) == 0) return;
            spell_add_word (ptr, len, nchar == 'w' || nchar == 'W', (nchar == 'G' || nchar == 'W') ? 0 : (int) cap -> count1, undo);
        }
        break;
    case '=' :
        if (!checkclearop(cap->oap)) spell_suggest ((int) cap -> count0);
        break;
    default :
        clearopbeep (cap -> oap);
    }
    ...
    if (old_fdl != curwin -> w_onebuf_opt.wo_fdl) newFoldLevel ();
}

static void nv_exmode (cap)
    cmdarg_T * cap;
{
    if (VIsual_active) vim_beep ();
    else if (!checkclearop(cap->oap)) do_exmode (0);
}
    ...
    cmdarg_T * cap;
{
    ...
    if (VIsual_active) nv_operator (cap);
        ...
        if (KeyTyped) compute_cmdrow ();
        old_p_im = p_im;
        do_cmdline (((void *) 0), getexline, ((void *) 0), cap -> oap -> op_type != 0 ? 0x20 : 0);
        if (p_im != old_p_im) {
            if (p_im) restart_edit = 'i';
            else restart_edit = 0;
        }
        if (cap -> oap -> op_type != 0 && (cap -> oap -> start.lnum > curbuf -> b_ml.ml_line_count || cap -> oap -> start.col > (colnr_T) strlen ((char *) (
          ml_get (cap -> oap -> start.lnum))))) clearopbeep (cap -> oap);
    }
}
    ...
    cmdarg_T * cap;
{
    ...
    else if (!checkclearop(cap->oap)) fileinfo ((int) cap -> count0, 0, 1);
}
...
static void nv_hat (cap)
    cmdarg_T * cap;
{
    if (!checkclearopq(cap->oap)) (void) buflist_getfile ((int) cap -> count0, (linenr_T) 0, 0x01 | 0x02, 0);
}
    ...
    cmdarg_T * cap;
{
    ...
    if (cmdchar == 0xA3) cmdchar = '#';
    if (cmdchar == ']' || cmdchar == 29 || cmdchar == 'K') {
        if (VIsual_active && get_visual_text (cap, &ptr, &n) == 0) return;
        if (checkclearopq (cap -> oap)) return;
    }
    if (ptr == ((void *) 0) && (n = find_ident_under_cursor (&ptr, (cmdchar == '*' || cmdchar == '#') ? 1 | 2 : 1)) == 0) {
        clearop (cap -> oap);
        return;
    }
    kp = (*curbuf->b_p_kp == '\000' ? p_kp : curbuf -> b_p_kp);
    kp_help = (*kp == '\000' || strcmp ((char *) (kp), (char *) (":he")) == 0 || strcmp ((char *) (kp), (char *) (":help")) == 0);
    buf = alloc ((unsigned) (n * 2 + 30 + strlen ((char *) (kp))));
    ...
    case '#' :
        ...
        if (!g_cmd && vim_iswordp (ptr)) ((__builtin_object_size ((char *) (buf), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (buf), (char *) ("\\<"
          ), __builtin_object_size ((char *) (buf), 1 > 1)) : __inline_strcpy_chk ((char *) (buf), (char *) ("\\<")));
        no_smartcase = 1;
        break;
    case 'K' :
        if (kp_help) ((__builtin_object_size ((char *) (buf), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (buf), (char *) ("he! "),
          __builtin_object_size ((char *) (buf), 1 > 1)) : __inline_strcpy_chk ((char *) (buf), (char *) ("he! ")));
        else {
            while (*ptr == '-' && n > 0) {
                ++ptr;
                --n;
            ...
            if (n == 0) {
                emsg ((char_u *) (((char *) (e_noident))));
                vim_free (buf);
                return;
            }
            isman = (strcmp ((char *) (kp), (char *) ("man")) == 0);
            isman_s = (strcmp ((char *) (kp), (char *) ("man -s")) == 0);
            if (cap -> count0 != 0 && !(isman||isman_s)) __builtin___sprintf_chk ((char *) buf, 0, __builtin_object_size ((char *) buf, 1 > 1), ".,.+%ld", cap
              -> count0 - 1);
            ((__builtin_object_size ((char *) (buf), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (buf), (char *) ("! "), __builtin_object_size ((
              char *) (buf), 1 > 1)) : __inline_strcat_chk ((char *) (buf), (char *) ("! ")));
            if (cap -> count0 == 0 && isman_s) ((__builtin_object_size ((char *) (buf), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (buf), (char *) (
              "man"), __builtin_object_size ((char *) (buf), 1 > 1)) : __inline_strcat_chk ((char *) (buf), (char *) ("man")));
            else ((__builtin_object_size ((char *) (buf), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (buf), (char *) (kp), __builtin_object_size ((
              char *) (buf), 1 > 1)) : __inline_strcat_chk ((char *) (buf), (char *) (kp)));
            ((__builtin_object_size ((char *) (buf), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (buf), (char *) (" "), __builtin_object_size ((char
              *) (buf), 1 > 1)) : __inline_strcat_chk ((char *) (buf), (char *) (" ")));
            if (cap -> count0 != 0 && (isman || isman_s)) {
                __builtin___sprintf_chk ((char *) buf + strlen ((char *) (buf)), 0, __builtin_object_size ((char *) buf + strlen ((char *) (buf)), 1 > 1), "%ld"
                  , cap -> count0);
                ((__builtin_object_size ((char *) (buf), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (buf), (char *) (" "), __builtin_object_size ((
                  char *) (buf), 1 > 1)) : __inline_strcat_chk ((char *) (buf), (char *) (" ")));
            }
    ...
    case ']' :
        tag_cmd = 1;
        ((__builtin_object_size ((char *) (buf), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (buf), (char *) ("ts "), __builtin_object_size ((char *
          ) (buf), 1 > 1)) : __inline_strcpy_chk ((char *) (buf), (char *) ("ts ")));
        break;
    default :
        tag_cmd = 1;
        if (curbuf -> b_help) ((__builtin_object_size ((char *) (buf), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (buf), (char *) ("he! "),
          __builtin_object_size ((char *) (buf), 1 > 1)) : __inline_strcpy_chk ((char *) (buf), (char *) ("he! ")));
        else {
            if (g_cmd) ((__builtin_object_size ((char *) (buf), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (buf), (char *) ("tj "),
              __builtin_object_size ((char *) (buf), 1 > 1)) : __inline_strcpy_chk ((char *) (buf), (char *) ("tj ")));
            else __builtin___sprintf_chk ((char *) buf, 0, __builtin_object_size ((char *) buf, 1 > 1), "%ldta ", cap -> count0);
        }
    }
    if (cmdchar == 'K' && !kp_help) {
        ptr = vim_strnsave (ptr, n);
        p = vim_strsave_shellescape (ptr, 1);
        vim_free (ptr);
        if (p == ((void *) 0)) {
            vim_free (buf);
            return;
        }
        newbuf = (char_u *) realloc ((buf), (strlen ((char *) (buf)) + strlen ((char *) (p)) + 1));
        if (newbuf == ((void *) 0)) {
            vim_free (buf);
            vim_free (p);
            return;
        ...
        buf = newbuf;
        ((__builtin_object_size ((char *) (buf), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (buf), (char *) (p), __builtin_object_size ((char *) (
          buf), 1 > 1)) : __inline_strcat_chk ((char *) (buf), (char *) (p)));
        vim_free (p);
    }
    else {
        if (cmdchar == '*') aux_ptr = (char_u *) (p_magic ? "/.*~[^$\\" : "/^$\\");
        else if (cmdchar == '#') aux_ptr = (char_u *) (p_magic ? "/?.*~[^$\\" : "/?^$\\");
        else if (tag_cmd) aux_ptr = (char_u *) "\\|\"\n[";
        else aux_ptr = (char_u *) "\\|\"\n*?[";
        p = buf + strlen ((char *) (buf));
        while (n -- > 0) {
            if (vim_strchr (aux_ptr, *ptr) != ((void *) 0)) *p++ = '\\';
            *p++ = *ptr++;
        }
        * p = '\000';
    }
    if (cmdchar == '*' || cmdchar == '#') {
        if (!g_cmd && (vim_iswordc (ptr [-1]))) ((__builtin_object_size ((char *) (buf), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (buf), (char *)
          ("\\>"), __builtin_object_size ((char *) (buf), 1 > 1)) : __inline_strcat_chk ((char *) (buf), (char *) ("\\>")));
        init_history ();
        add_to_history (1, buf, 1, '\000');
        normal_search (cap, cmdchar == '*' ? '/' : '?', buf, 0);
    }
    else do_cmdline_cmd (buf);
    vim_free (buf);
}
...
int get_visual_text (cap, pp, lenp)
    ...
    int * lenp;
{
    if (VIsual_mode != 'V') unadjust_for_sel ();
    if (VIsual.lnum != curwin -> w_cursor.lnum) {
        if (cap != ((void *) 0)) clearopbeep (cap -> oap);
        return 0;
    }
    if (VIsual_mode == 'V') {
        * pp = ml_get_curline ();
        * lenp = (int) strlen ((char *) (*pp));
    }
    else {
        if ((((curwin -> w_cursor).lnum != (VIsual).lnum) ? (curwin -> w_cursor).lnum < (VIsual).lnum : (curwin -> w_cursor).col != (VIsual).col ? (curwin ->
          w_cursor).col < (VIsual).col : (curwin -> w_cursor).coladd < (VIsual).coladd)) {
...
static void nv_tagpop (cap)
    cmdarg_T * cap;
{
    if (!checkclearopq(cap->oap)) do_tag ((char_u *) "", 2, (int) cap -> count1, 0, 1);
}
    ...
    cmdarg_T * cap;
{
    ...
    if (cap -> cmdchar == 'L') {
        ...
        if (cap -> count1 - 1 >= curwin -> w_cursor.lnum) curwin -> w_cursor.lnum = 1;
        else {
        ...
        if (cap -> cmdchar == 'M') {
            ...
            for (n = 0; curwin -> w_topline + n < curbuf -> b_ml.ml_line_count; ++n) {
                ...
                if (used >= half) break;
                if (hasFolding (curwin -> w_topline + n, ((void *) 0), &lnum)) n = lnum - curwin -> w_topline;
            }
            if (n > 0 && used > curwin -> w_height) --n;
        }
        else {
            n = cap -> count1 - 1;
            if (hasAnyFolding (curwin)) {
    ...
    cmdarg_T * cap;
{
    ...
    if (virtual_active ()) PAST_LINE = 0;
    for (n = cap -> count1; n > 0; --n) {
        if ((!PAST_LINE && oneright () == 0) || (PAST_LINE && *ml_get_cursor() == '\000')) {
            if (((cap -> cmdchar == ' ' && vim_strchr (p_ww, 's') != ((void *) 0)) || (cap -> cmdchar == 'l' && vim_strchr (p_ww, 'l') != ((void *) 0)) || (cap
              -> cmdchar == (-(('k')+((int)('r')<<8))) && vim_strchr (p_ww, '>') != ((void *) 0))) && curwin -> w_cursor.lnum < curbuf -> b_ml.ml_line_count) {
                if (cap -> oap -> op_type != 0 && !cap->oap->inclusive && !(*ml_get(curwin->w_cursor.lnum) == '\000')) cap -> oap -> inclusive = 1;
                else {
                    ++curwin->w_cursor.lnum;
                    curwin -> w_cursor.col = 0;
            ...
            if (cap -> oap -> op_type == 0) {
                if (n == cap -> count1) beep_flush ();
            }
            else {
                if (!(*ml_get(curwin->w_cursor.lnum) == '\000')) cap -> oap -> inclusive = 1;
            }
            break;
        }
        else if (PAST_LINE) {
            curwin -> w_set_curswant = 1;
            if (virtual_active ()) oneright ();
            else {
                ++curwin->w_cursor.col;
            }
        }
    }
    if (n != cap -> count1 && (fdo_flags & 0x004) && KeyTyped && cap -> oap -> op_type == 0) foldOpenCursor ();
}
    ...
    cmdarg_T * cap;
{
    ...
    for (n = cap -> count1; n > 0; --n) {
        if (oneleft () == 0) {
            if ((((cap -> cmdchar == (-(('k')+((int)('b')<<8))) || cap -> cmdchar == 8) && vim_strchr (p_ww, 'b') != ((void *) 0)) || (cap -> cmdchar == 'h' &&
              vim_strchr (p_ww, 'h') != ((void *) 0)) || (cap -> cmdchar == (-(('k')+((int)('l')<<8))) && vim_strchr (p_ww, '<') != ((void *) 0))) && curwin ->
              w_cursor.lnum > 1) {
                --(curwin->w_cursor.lnum);
                coladvance ((colnr_T) (0x7fffffffL));
                curwin -> w_set_curswant = 1;
                if ((cap -> oap -> op_type == 1 || cap -> oap -> op_type == 3) && !(*ml_get(curwin->w_cursor.lnum) == '\000')) {
                    if (*ml_get_cursor() != '\000') ++curwin->w_cursor.col;
                    cap -> retval |= 2;
                }
                continue;
            }
            else if (cap -> oap -> op_type == 0 && n == cap -> count1) beep_flush ();
            break;
        }
    }
    if (n != cap -> count1 && (fdo_flags & 0x004) && KeyTyped && cap -> oap -> op_type == 0) foldOpenCursor ();
    ...
    cmdarg_T * cap;
{
    if (mod_mask & 0x02) {
        ...
        cap -> oap -> motion_type = 1;
        if (cursor_up (cap -> count1, cap -> oap -> op_type == 0) == 0) clearopbeep (cap -> oap);
        else if (cap -> arg) beginline (1 | 4);
    }
}
    ...
    cmdarg_T * cap;
{
    ...
    else if (bt_quickfix (curbuf) && cap -> cmdchar == '\015') if (curwin -> w_llist_ref == ((void *) 0)) do_cmdline_cmd ((char_u *) ".cc");
    else do_cmdline_cmd ((char_u *) ".ll");
    else {
        if (cmdwin_type != 0 && cap -> cmdchar == '\015') cmdwin_result = '\015';
        else {
            cap -> oap -> motion_type = 1;
            if (cursor_down (cap -> count1, cap -> oap -> op_type == 0) == 0) clearopbeep (cap -> oap);
            else if (cap -> arg) beginline (1 | 4);
        }
    }
}
    ...
    cmdarg_T * cap;
{
    ...
    if (ptr != ((void *) 0)) {
        if (curbufIsChanged () && curbuf -> b_nwindows <= 1 && !(buf_hide(curbuf))) autowrite (curbuf, 0);
        setpcmark ();
        (void) do_ecmd (0, ptr, ((void *) 0), ((void *) 0), (linenr_T) - 1, (buf_hide (curbuf)) ? 0x01 : 0, curwin);
    ...
    cmdarg_T * cap;
{
    cap -> oap -> motion_type = 0;
    cap -> oap -> inclusive = 1;
    if (!virtual_active() || gchar_cursor () != '\000' || cap -> oap -> op_type == 0) curwin -> w_curswant = (0x7fffffffL);
    if (cursor_down ((long) (cap -> count1 - 1), cap -> oap -> op_type == 0) == 0) clearopbeep (cap -> oap);
    else if ((fdo_flags & 0x004) && KeyTyped && cap -> oap -> op_type == 0) foldOpenCursor ();
}
    ...
    cmdarg_T * cap;
{
    ...
    normal_search (cap, cap -> cmdchar, cap -> searchbuf, (cap -> arg ? 0 : 0x200));
}

static void nv_next (cap)
    cmdarg_T * cap;
{
    normal_search (cap, 0, ((void *) 0), 0x200 | cap -> arg);
}
    ...
    int opt;
{
    ...
    curwin -> w_set_curswant = 1;
    i = do_search (cap -> oap, dir, pat, cap -> count1, opt | 0x10 | 0x02 | 0x0c, ((void *) 0));
    if (i == 0) clearop (cap -> oap);
    else {
        if (i == 2) cap -> oap -> motion_type = 1;
        curwin -> w_cursor.coladd = 0;
        if (cap -> oap -> op_type == 0 && (fdo_flags & 0x040) && KeyTyped) foldOpenCursor ();
    }
    check_cursor ();
}
...
{
    int t_cmd;
    if (cap -> cmdchar == 't' || cap -> cmdchar == 'T') t_cmd = 1;
    else t_cmd = 0;
    cap -> oap -> motion_type = 0;
    if (((cap -> nchar) < 0) || searchc (cap, t_cmd) == 0) clearopbeep (cap -> oap);
    else {
        curwin -> w_set_curswant = 1;
        if (gchar_cursor () == '\011' && virtual_active () && cap -> arg == 1 && (t_cmd || cap -> oap -> op_type != 0)) {
            colnr_T scol, ecol;
        ...
        adjust_for_sel (cap);
        if ((fdo_flags & 0x004) && KeyTyped && cap -> oap -> op_type == 0) foldOpenCursor ();
    }
}
    ...
    cmdarg_T * cap;
{
    ...
    old_pos = curwin -> w_cursor;
    curwin -> w_cursor.coladd = 0;
    if (cap -> nchar == 'f') nv_gotofile (cap);
    else if (vim_strchr ((char_u *) "iI\011dD\004", cap -> nchar) != ((void *) 0)) {
        char_u * ptr;
        int len;
        if ((len = find_ident_under_cursor (&ptr, 1)) == 0) clearop (cap -> oap);
        else {
            find_pattern_in_path (ptr, 0, len, 1, cap -> count0 == 0 ? !isupper(cap->nchar) : 0, ((cap -> nchar & 0xf) == ('d' & 0xf)) ? 2 : 1, cap -> count1,
              isupper (cap -> nchar) ? 4 : islower (cap -> nchar) ? 1 : 2, cap -> cmdchar == ']' ? curwin -> w_cursor.lnum + 1 : (linenr_T) 1, (linenr_T) (
              0x7fffffffL));
            curwin -> w_set_curswant = 1;
        }
    }
    else if ((cap -> cmdchar == '[' && vim_strchr ((char_u *) "{(*/#mM", cap -> nchar) != ((void *) 0)) || (cap -> cmdchar == ']' && vim_strchr ((char_u *)
      "})*/#mM", cap -> nchar) != ((void *) 0))) {
        if (cap -> nchar == '*') cap -> nchar = '/';
        prev_pos.lnum = 0;
        if (cap -> nchar == 'm' || cap -> nchar == 'M') {
            if (cap -> cmdchar == '[') findc = '{';
            else findc = '}';
            n = 9999;
        }
        else {
        ...
        for (; n > 0; --n) {
            if ((pos = findmatchlimit (cap -> oap, findc, (cap -> cmdchar == '[') ? 0x01 : 0x02, 0)) == ((void *) 0)) {
                if (new_pos.lnum == 0) {
                    if (cap -> nchar != 'm' && cap -> nchar != 'M') clearopbeep (cap -> oap);
                }
                else pos = &new_pos;
                break;
            }
        ...
        if (cap -> nchar == 'm' || cap -> nchar == 'M') {
            ...
            while (n > 0) {
                for (;;) {
                    if ((findc == '{' ? dec_cursor () : inc_cursor ()) < 0) {
                        if (pos == ((void *) 0)) clearopbeep (cap -> oap);
                        n = 0;
                        break;
                    }
                    c = gchar_cursor ();
                    if (c == '{' || c == '}') {
                        ...
                        else if ((pos = findmatchlimit (cap -> oap, findc, (cap -> cmdchar == '[') ? 0x01 : 0x02, 0)) == ((void *) 0)) n = 0;
            ...
            curwin -> w_cursor = old_pos;
            if (pos == ((void *) 0) && new_pos.lnum != 0) clearopbeep (cap -> oap);
        }
        if (pos != ((void *) 0)) {
            setpcmark ();
            curwin -> w_cursor = *pos;
            curwin -> w_set_curswant = 1;
            if ((fdo_flags & 0x002) && KeyTyped && cap -> oap -> op_type == 0) foldOpenCursor ();
        }
    }
        ...
        curwin -> w_set_curswant = 1;
        if (!findpar(&cap->oap->inclusive, cap -> arg, cap -> count1, flag, (cap -> oap -> op_type != 0 && cap -> arg == 1 && flag == '{'))) clearopbeep (cap ->
          oap);
        else {
            if (cap -> oap -> op_type == 0) beginline (1 | 4);
            if ((fdo_flags & 0x002) && KeyTyped && cap -> oap -> op_type == 0) foldOpenCursor ();
        }
    }
        ...
        if (!checkclearop(cap->oap)) {
            prep_redo_cmd (cap);
            do_put (cap -> oap -> regname, (cap -> cmdchar == ']' && cap -> nchar == 'p') ? 1 : (-1), cap -> count1, 1);
        }
    }
    else if (cap -> nchar == '\'' || cap -> nchar == '`') {
        pos = &curwin->w_cursor;
        for (n = cap -> count1; n > 0; --n) {
            prev_pos = *pos;
            pos = getnextmark (pos, cap -> cmdchar == '[' ? (-1) : 1, cap -> nchar == '\'');
            if (pos == ((void *) 0)) break;
        }
        if (pos == ((void *) 0)) pos = &prev_pos;
        nv_cursormark (cap, cap -> nchar == '\'', pos);
    }
    else if (cap -> nchar >= (-((253)+((int)(KE_RIGHTRELEASE)<<8))) && cap -> nchar <= (-((253)+((int)(KE_LEFTMOUSE)<<8)))) {
        (void) do_mouse (cap -> oap, cap -> nchar, (cap -> cmdchar == ']') ? 1 : (-1), cap -> count1, 1);
    }
    else if (cap -> nchar == 'z') {
        if (foldMoveTo (0, cap -> cmdchar == ']' ? 1 : (-1), cap -> count1) == 0) clearopbeep (cap -> oap);
    }
    else if (cap -> nchar == 'c') {
        if (diff_move_to (cap -> cmdchar == ']' ? 1 : (-1), cap -> count1) == 0) clearopbeep (cap -> oap);
    }
    else if (cap -> nchar == 's' || cap -> nchar == 'S') {
        setpcmark ();
        for (n = 0; n < cap -> count1; ++n) if (spell_move_to (curwin, cap -> cmdchar == ']' ? 1 : (-1), cap -> nchar == 's' ? 1 : 0, 0, ((void *) 0)) == 0) {
            clearopbeep (cap -> oap);
            break;
        }
        if (cap -> oap -> op_type == 0 && (fdo_flags & 0x040) && KeyTyped) foldOpenCursor ();
    ...
    cmdarg_T * cap;
{
    ...
    cap -> oap -> inclusive = 1;
    if (cap -> count0) {
        if (cap -> count0 > 100) clearopbeep (cap -> oap);
        else {
            cap -> oap -> motion_type = 1;
            setpcmark ();
            if (curbuf -> b_ml.ml_line_count > 1000000) curwin -> w_cursor.lnum = (curbuf -> b_ml.ml_line_count + 99L) / 100L * cap -> count0;
        ...
        cap -> oap -> motion_type = 0;
        cap -> oap -> use_reg_one = 1;
        if ((pos = findmatch (cap -> oap, '\000')) == ((void *) 0)) clearopbeep (cap -> oap);
        else {
            setpcmark ();
            curwin -> w_cursor = *pos;
            curwin -> w_set_curswant = 1;
    ...
    if (cap -> oap -> op_type == 0 && lnum != curwin -> w_cursor.lnum && (fdo_flags & 0x010) && KeyTyped) foldOpenCursor ();
}
    ...
    cmdarg_T * cap;
{
    ...
    cap -> oap -> inclusive = 0;
    curwin -> w_set_curswant = 1;
    if (findsent (cap -> arg, cap -> count1) == 0) clearopbeep (cap -> oap);
    else {
...
{
    if (!checkclearop(cap->oap)) {
        if (setmark (cap -> nchar) == 0) clearopbeep (cap -> oap);
    }
}
    ...
    cmdarg_T * cap;
{
    ...
    cap -> oap -> use_reg_one = 1;
    curwin -> w_set_curswant = 1;
    if (!findpar(&cap->oap->inclusive, cap -> arg, cap -> count1, '\000', 0)) clearopbeep (cap -> oap);
    else {
        curwin -> w_cursor.coladd = 0;
        if ((fdo_flags & 0x002) && KeyTyped && cap -> oap -> op_type == 0) foldOpenCursor ();
    }
...
static void nv_undo (cap)
    cmdarg_T * cap;
{
    if (cap -> oap -> op_type == 12 || VIsual_active) {
        cap -> cmdchar = 'g';
        cap -> nchar = 'u';
        nv_operator (cap);
    }
    ...
    cmdarg_T * cap;
{
    ...
    if (cap -> nchar == 22) {
        ...
        if (cap -> nchar > 0x7f) had_ctrl_v = '\000';
    }
    else had_ctrl_v = '\000';
    if (((cap -> nchar) < 0)) {
        clearopbeep (cap -> oap);
    ...
    if (VIsual_active) {
        if (got_int) reset_VIsual ();
        nv_operator (cap);
        return;
    }
    if (virtual_active ()) {
        if (u_save_cursor () == 0) return;
        if (gchar_cursor () == '\000') {
            coladvance_force ((colnr_T) (getviscol () + cap -> count1));
            curwin -> w_cursor.col -= cap -> count1;
        }
        else if (gchar_cursor () == '\011') coladvance_force (getviscol ());
    }
    ptr = ml_get_cursor ();
    ...
    if (had_ctrl_v != 22 && (cap -> nchar == '\r' || cap -> nchar == '\n')) {
        (void) del_bytes (cap -> count1, 0, 0);
        stuffcharReadbuff ('\r');
        stuffcharReadbuff ('\033');
        invoke_edit (cap, 1, 'r', 0);
    }
    else {
        prep_redo (cap -> oap -> regname, cap -> count1, '\000', 'r', '\000', had_ctrl_v, cap -> nchar);
        curbuf -> b_op_start = curwin -> w_cursor;
        {
            for (n = cap -> count1; n > 0; --n) {
                ptr = ml_get_buf (curbuf, curwin -> w_cursor.lnum, 1);
                ptr [curwin -> w_cursor.col] = cap -> nchar;
                if (p_sm && msg_silent == 0) showmatch (cap -> nchar);
                ++curwin->w_cursor.col;
            }
            if (netbeans_active ()) {
    ...
    int cmdchar;
{
    ...
    if (cmdchar == 'O' && VIsual_mode == 22) {
        ...
        if (old_cursor.lnum >= VIsual.lnum && *p_sel == 'e') ++curwin->w_curswant;
        coladvance (curwin -> w_curswant);
        if (curwin -> w_cursor.col == old_cursor.col && (!virtual_active() || curwin -> w_cursor.coladd == old_cursor.coladd)) {
            curwin -> w_cursor.lnum = VIsual.lnum;
            if (old_cursor.lnum <= VIsual.lnum && *p_sel == 'e') ++right;
            coladvance (right);
            VIsual = curwin -> w_cursor;
            curwin -> w_cursor.lnum = old_cursor.lnum;
            coladvance (left);
    ...
    cmdarg_T * cap;
{
    if (VIsual_active) {
        ...
        if (!curbuf->b_p_ma) emsg ((char_u *) (((char *) (e_modifiable))));
        else {
            if (virtual_active ()) coladvance (getviscol ());
    ...
    cmdarg_T * cap;
{
    if (VIsual_active) {
        ...
        if (!curbuf->b_p_ma) emsg ((char_u *) (((char *) (e_modifiable))));
            ...
            if (cap -> extra_char == 22) cap -> extra_char = get_literal ();
    ...
    cmdarg_T * cap;
{
    ...
    for (n = cap -> count1; n > 0; --n) {
        ...
        inc_cursor ();
        if (gchar_cursor () == '\000') {
            if (vim_strchr (p_ww, '~') != ((void *) 0) && curwin -> w_cursor.lnum < curbuf -> b_ml.ml_line_count) {
                if (netbeans_active ()) {
                    if (did_change) {
                        ptr = ml_get (pos.lnum);
                        count = (int) strlen ((char *) (ptr)) - pos.col;
                ...
                curwin -> w_cursor.col = 0;
                if (n > 1) {
                    if (u_savesub (curwin -> w_cursor.lnum) == 0) break;
                    u_clearline ();
                }
            }
            else break;
...
static void nv_cursormark (cap, flag, pos)
    ...
    pos_T * pos;
{
    if (check_mark (pos) == 0) clearop (cap -> oap);
    else {
        if (cap -> cmdchar == '\'' || cap -> cmdchar == '`' || cap -> cmdchar == '[' || cap -> cmdchar == ']') setpcmark ();
        curwin -> w_cursor = *pos;
        if (flag) beginline (1 | 4);
        else check_cursor ();
    }
    cap -> oap -> motion_type = flag ? 1 : 0;
    if (cap -> cmdchar == '`') cap -> oap -> use_reg_one = 1;
    cap -> oap -> inclusive = 0;
    curwin -> w_set_curswant = 1;
}
    ...
    cmdarg_T * cap;
{
    ...
    if (isupper (cap -> cmdchar)) {
        if (VIsual_mode != 22) VIsual_mode = 'V';
        else if (cap -> cmdchar == 'C' || cap -> cmdchar == 'D') curwin -> w_curswant = (0x7fffffffL);
    }
...
{
    if (VIsual_active) {
        if (cap -> cmdchar == 'S') VIsual_mode = 'V';
        cap -> cmdchar = 'c';
        nv_operator (cap);
    }
    else nv_optrans (cap);
...
static void nv_abbrev (cap)
    cmdarg_T * cap;
{
    if (cap -> cmdchar == (-(('k')+((int)('D')<<8))) || cap -> cmdchar == (-((253)+((int)(KE_KDEL)<<8)))) cap -> cmdchar = 'x';
    if (VIsual_active) v_visop (cap);
    else nv_optrans (cap);
}
...
{
    static char_u * (ar [8]) =
    {(char_u *) "dl", (char_u *) "dh", (char_u *) "d$", (char_u *) "c$", (char_u *) "cl", (char_u *) "cc", (char_u *) "yy", (char_u *) ":s\r"};
    ...
    if (!checkclearopq(cap->oap)) {
        if (cap -> cmdchar == 'D' && vim_strchr (p_cpo, '#') != ((void *) 0)) {
            ...
            if (cap -> count0) stuffnumReadbuff (cap -> count0);
    ...
    cmdarg_T * cap;
{
    ...
    if (cap -> cmdchar == 'g') c = cap -> extra_char;
    else c = cap -> nchar;
    ...
    if (!virtual_active()) curwin -> w_cursor.coladd = 0;
    if (cap -> oap -> op_type == 0 && (pos == (pos_T *) -1 || lnum != curwin -> w_cursor.lnum) && (fdo_flags & 0x008) && old_KeyTyped) foldOpenCursor ();
}
    ...
    cmdarg_T * cap;
{
    ...
    int old_KeyTyped = KeyTyped;
    if (!checkclearopq(cap->oap)) {
        if (cap -> cmdchar == 'g') pos = movechangelist ((int) cap -> count1);
        else pos = movemark ((int) cap -> count1);
        if (pos == (pos_T *) -1) {
            curwin -> w_set_curswant = 1;
            check_cursor ();
        }
        else if (pos != ((void *) 0)) nv_cursormark (cap, 0, pos);
        else if (cap -> cmdchar == 'g') {
            if (curbuf -> b_changelistlen == 0) emsg ((char_u *) (((char *) ("E664: changelist is empty"))));
            else if (cap -> count1 < 0) emsg ((char_u *) (((char *) ("E662: At start of changelist"))));
            else emsg ((char_u *) (((char *) ("E663: At end of changelist"))));
        }
        else clearopbeep (cap -> oap);
        if (cap -> oap -> op_type == 0 && (pos == (pos_T *) -1 || lnum != curwin -> w_cursor.lnum) && (fdo_flags & 0x008) && old_KeyTyped) foldOpenCursor ();
    }
...
{
    if (checkclearop (cap -> oap)) return;
    if (cap -> nchar == '=') cap -> nchar = get_expr_register ();
...
static void nv_visual (cap)
    cmdarg_T * cap;
{
    if (cap -> cmdchar == 17) cap -> cmdchar = 22;
    if (cap -> oap -> op_type != 0) {
    ...
    if (VIsual_active) {
        if (VIsual_mode == cap -> cmdchar) end_visual_mode ();
        else {
        ...
        if (cap -> count0) {
            ...
            VIsual_active = 1;
            VIsual_reselect = 1;
            if (!cap->arg) may_start_select ('c');
            setmouse ();
            if (p_smd && msg_silent == 0) redraw_cmdline = 1;
            ...
            if (VIsual_mode == 'v') {
                if (resel_VIsual_line_count <= 1) curwin -> w_cursor.col += resel_VIsual_col * cap -> count0 - 1;
                else curwin -> w_cursor.col = resel_VIsual_col;
                check_cursor_col ();
            }
            if (resel_VIsual_col == (0x7fffffffL)) {
                curwin -> w_curswant = (0x7fffffffL);
                coladvance ((colnr_T) (0x7fffffffL));
            }
            else if (VIsual_mode == 22) {
                validate_virtcol ();
                curwin -> w_curswant = curwin -> w_virtcol + resel_VIsual_col * cap -> count0 - 1;
                coladvance (curwin -> w_curswant);
            }
...
void may_start_select (c)
    int c;
{
    VIsual_select = (stuff_empty () && typebuf_typed () && (vim_strchr (p_slm, c) != ((void *) 0)));
}
    ...
    int c;
{
    ...
    VIsual_active = 1;
    VIsual_reselect = 1;
    if (c == 22 && (ve_flags & 5) && gchar_cursor () == '\011') coladvance (curwin -> w_virtcol);
    VIsual = curwin -> w_cursor;
    foldAdjustVisual ();
    setmouse ();
    if (p_smd && msg_silent == 0) redraw_cmdline = 1;
...
static void nv_window (cap)
    cmdarg_T * cap;
{
    if (!checkclearop(cap->oap)) do_window (cap -> nchar, cap -> count0, '\000');
}
...
{
    clearop (cap -> oap);
    if (VIsual_active) end_visual_mode ();
    ...
    cmdarg_T * cap;
{
    ...
    case 'j' :
    case (-(('k')+((int)('d')<<8))) :
        if (!curwin->w_onebuf_opt.wo_wrap || hasFolding (curwin -> w_cursor.lnum, ((void *) 0), ((void *) 0))) {
            oap -> motion_type = 1;
            i = cursor_down (cap -> count1, oap -> op_type == 0);
        }
        else i = nv_screengo (oap, 1, cap -> count1);
        if (i == 0) clearopbeep (oap);
        break;
    case 'k' :
    case (-(('k')+((int)('u')<<8))) :
        if (!curwin->w_onebuf_opt.wo_wrap || hasFolding (curwin -> w_cursor.lnum, ((void *) 0), ((void *) 0))) {
    ...
    case (-(('K')+((int)('1')<<8))) :
        ...
        if (curwin -> w_onebuf_opt.wo_wrap && curwin -> w_width != 0) {
            ...
            validate_virtcol ();
            i = 0;
            if (curwin -> w_virtcol >= (colnr_T) width1 && width2 > 0) i = (curwin -> w_virtcol - width1) / width2 * width2 + width1;
        }
        else i = curwin -> w_leftcol;
        if (cap -> nchar == 'm') i += ((curwin -> w_width) - curwin_col_off () + ((curwin -> w_onebuf_opt.wo_wrap && i > 0) ? curwin_col_off2 () : 0)) / 2;
        coladvance ((colnr_T) i);
        if (flag) {
            do i = gchar_cursor ();
            while (((i) == ' ' || (i) == '\t') && oneright () == 1);
        }
        curwin -> w_set_curswant = 1;
        break;
    case '_' :
        ...
        if (cursor_down ((long) (cap -> count1 - 1), cap -> oap -> op_type == 0) == 0) clearopbeep (cap -> oap);
            ...
            if (curwin -> w_cursor.col > 0 && ptr [curwin -> w_cursor.col] == '\000') --curwin->w_cursor.col;
            while (curwin -> w_cursor.col > 0 && ((ptr [curwin -> w_cursor.col]) == ' ' || (ptr [curwin -> w_cursor.col]) == '\t')) --curwin->w_cursor.col;
            curwin -> w_set_curswant = 1;
            adjust_for_sel (cap);
        }
        break;
    ...
    case (-(('K')+((int)('4')<<8))) :
        {
            ...
            if (curwin -> w_onebuf_opt.wo_wrap && curwin -> w_width != 0) {
                ...
                if (cap -> count1 == 1) {
                    ...
                    validate_virtcol ();
                    i = width1 - 1;
                    if (curwin -> w_virtcol >= (colnr_T) width1) i += ((curwin -> w_virtcol - width1) / width2 + 1) * width2;
                    coladvance ((colnr_T) i);
                    if (curwin -> w_cursor.col > 0 && curwin -> w_onebuf_opt.wo_wrap) {
                        validate_virtcol ();
                        if (curwin -> w_virtcol > (colnr_T) i) --curwin->w_cursor.col;
                    }
                }
                else if (nv_screengo (oap, 1, cap -> count1 - 1) == 0) clearopbeep (oap);
            }
            else {
    ...
    case 'E' :
        ...
        curwin -> w_set_curswant = 1;
        oap -> inclusive = 1;
        if (bckend_word (cap -> count1, cap -> nchar == 'E', 0) == 0) clearopbeep (oap);
        break;
    case 7 :
        cursor_pos_info ();
        break;
    case 'i' :
        if (curbuf -> b_last_insert.lnum != 0) {
            ...
            check_cursor_lnum ();
            i = (int) strlen ((char *) (ml_get_curline ()));
            if (curwin -> w_cursor.col > (colnr_T) i) {
                if (virtual_active ()) curwin -> w_cursor.coladd += curwin -> w_cursor.col - i;
    ...
    case 'I' :
        beginline (0);
        if (!checkclearopq(oap)) invoke_edit (cap, 0, 'g', 0);
        break;
    case 'f' :
    case 'F' :
        nv_gotofile (cap);
    ...
    case 'Q' :
        ...
        if (!checkclearopq(oap)) do_exmode (1);
        break;
    case ',' :
        nv_pcmark (cap);
        break;
    ...
    case '+' :
    case '-' :
        if (!checkclearopq(oap)) undo_time (cap -> nchar == '-' ? -cap->count1 : cap -> count1, 0, 0, 0);
        break;
    default :
        clearopbeep (oap);
        break;
...
{
    if (!checkclearopq(cap->oap)) {
        if (cap -> cmdchar == 'O') (void) hasFolding (curwin -> w_cursor.lnum, &curwin->w_cursor.lnum, ((void *) 0));
        else (void) hasFolding (curwin -> w_cursor.lnum, ((void *) 0), &curwin->w_cursor.lnum);
        if (u_save ((linenr_T) (curwin -> w_cursor.lnum - (cap -> cmdchar == 'O' ? 1 : 0)), (linenr_T) (curwin -> w_cursor.lnum + (cap -> cmdchar == 'o' ? 1 : 0
          ))) == 1 && open_line (cap -> cmdchar == 'O' ? (-1) : 1, has_format_option ('o') ? 2 : 0, 0)) {
            if (vim_strchr (p_cpo, '#') != ((void *) 0)) cap -> count1 = 1;
...
{
    if (!checkclearopq(cap->oap)) {
        if (start_redo (cap -> count0, restart_edit != 0 && !arrow_used) == 0) clearopbeep (cap -> oap);
    }
}
...
static void nv_tilde (cap)
    cmdarg_T * cap;
{
    if (!p_to && !VIsual_active && cap -> oap -> op_type != 7) n_swapchar (cap);
    ...
    cmdarg_T * cap;
{
    ...
    if (op_type == cap -> oap -> op_type) nv_lineop (cap);
...
{
    char_u opchars [3];
    if (optype == 0) set_vim_var_string (52, ((void *) 0), 0);
    else {
        opchars [0] = get_op_char (optype);
        opchars [1] = get_extra_op_char (optype);
        opchars [2] = '\000';
        set_vim_var_string (52, opchars, -1);
    }
}
...
{
    cap -> oap -> motion_type = 1;
    if (cursor_down (cap -> count1 - 1L, cap -> oap -> op_type == 0) == 0) clearopbeep (cap -> oap);
    else if (cap -> oap -> op_type == 1 || cap -> oap -> op_type == 4 || cap -> oap -> op_type == 5) beginline (2 | 4);
    else if (cap -> oap -> op_type != 2) beginline (1 | 4);
}

static void nv_home (cap)
    cmdarg_T * cap;
{
    if (mod_mask & 0x04) nv_goto (cap);
    else {
        cap -> count0 = 1;
        nv_pipe (cap);
    }
    ...
    cmdarg_T * cap;
{
    ...
    cap -> oap -> inclusive = 0;
    curwin -> w_set_curswant = 1;
    if (bck_word (cap -> count1, cap -> arg, 0) == 0) clearopbeep (cap -> oap);
    else if ((fdo_flags & 0x004) && KeyTyped && cap -> oap -> op_type == 0) foldOpenCursor ();
}
    ...
    cmdarg_T * cap;
{
    ...
    if (cap -> cmdchar == 'e' || cap -> cmdchar == 'E') word_end = 1;
    else word_end = 0;
    ...
    if (!word_end && cap -> oap -> op_type == 3) {
        ...
        if (n != '\000') {
            if (((n) == ' ' || (n) == '\t')) {
                if (cap -> count1 == 1 && vim_strchr (p_cpo, 'w') != ((void *) 0)) {
                    cap -> oap -> inclusive = 1;
                    cap -> oap -> motion_type = 0;
                    return;
                }
    ...
    cap -> oap -> motion_type = 0;
    curwin -> w_set_curswant = 1;
    if (word_end) n = end_word (cap -> count1, cap -> arg, flag, 0);
    else n = fwd_word (cap -> count1, cap -> arg, cap -> oap -> op_type != 0);
    if ((((startpos).lnum != (curwin -> w_cursor).lnum) ? (startpos).lnum < (curwin -> w_cursor).lnum : (startpos).col != (curwin -> w_cursor).col ? (startpos)
      .col < (curwin -> w_cursor).col : (startpos).coladd < (curwin -> w_cursor).coladd)) adjust_cursor (cap -> oap);
    if (n == 0 && cap -> oap -> op_type == 0) clearopbeep (cap -> oap);
    else {
        adjust_for_sel (cap);
        if ((fdo_flags & 0x004) && KeyTyped && cap -> oap -> op_type == 0) foldOpenCursor ();
    }
...
static void adjust_cursor (oap)
    oparg_T * oap;
{
    if (curwin -> w_cursor.col > 0 && gchar_cursor () == '\000' && (!VIsual_active || *p_sel == 'o') && !virtual_active() && (ve_flags & 8) == 0) {
        --curwin->w_cursor.col;
        oap -> inclusive = 1;
    }
}
...
static void adjust_for_sel (cap)
    cmdarg_T * cap;
{
    if (VIsual_active && cap -> oap -> inclusive && *p_sel == 'e' && gchar_cursor () != '\000' && (((VIsual).lnum != (curwin -> w_cursor).lnum) ? (VIsual).lnum
      < (curwin -> w_cursor).lnum : (VIsual).col != (curwin -> w_cursor).col ? (VIsual).col < (curwin -> w_cursor).col : (VIsual).coladd < (curwin -> w_cursor)
      .coladd)) {
        ++curwin->w_cursor.col;
        cap -> oap -> inclusive = 0;
    }
...
static int unadjust_for_sel () {
    ...
    if (*p_sel == 'e' && !(((VIsual).lnum==(curwin->w_cursor).lnum)&&((VIsual).col==(curwin->w_cursor).col)&&((VIsual).coladd==(curwin->w_cursor).coladd))) {
        if ((((VIsual).lnum != (curwin -> w_cursor).lnum) ? (VIsual).lnum < (curwin -> w_cursor).lnum : (VIsual).col != (curwin -> w_cursor).col ? (VIsual).col
          < (curwin -> w_cursor).col : (VIsual).coladd < (curwin -> w_cursor).coladd)) pp = &curwin->w_cursor;
        else pp = &VIsual;
        if (pp -> coladd > 0) --pp->coladd;
        else if (pp -> col > 0) {
            --pp->col;
        }
        else if (pp -> lnum > 1) {
...
static void nv_select (cap)
    cmdarg_T * cap;
{
    if (VIsual_active) VIsual_select = 1;
...
{
    linenr_T lnum;
    if (cap -> arg) lnum = curbuf -> b_ml.ml_line_count;
    else lnum = 1L;
    cap -> oap -> motion_type = 1;
    setpcmark ();
    if (cap -> count0 != 0) lnum = cap -> count0;
    if (lnum < 1L) lnum = 1L;
    else if (lnum > curbuf -> b_ml.ml_line_count) lnum = curbuf -> b_ml.ml_line_count;
    curwin -> w_cursor.lnum = lnum;
    beginline (2 | 4);
    if ((fdo_flags & 0x400) && KeyTyped && cap -> oap -> op_type == 0) foldOpenCursor ();
}
    ...
    cmdarg_T * cap;
{
    if (cap -> nchar == 14 || cap -> nchar == 7) {
        ...
        restart_edit = 0;
        if (cmdwin_type != 0) cmdwin_result = 3;
        if (VIsual_active) {
            end_visual_mode ();
            redraw_curbuf_later (20);
        }
        if (cap -> nchar == 7 && p_im) restart_edit = 'a';
    }
    else clearopbeep (cap -> oap);
}
...
{
    int no_reason;
    no_reason = (cap -> oap -> op_type == 0 && cap -> opcount == 0 && cap -> count0 == 0 && cap -> oap -> regname == 0 && !p_im);
    if (cap -> arg) {
        if (restart_edit == 0 && cmdwin_type == 0 && !VIsual_active && no_reason) msg ((char_u *) (((char *) ("Type  :quit<Enter>  to exit Vim"))));
    ...
    else if (no_reason) vim_beep ();
    clearop (cap -> oap);
    if (restart_edit == 0 && goto_im () && ex_normal_busy == 0) restart_edit = 'a';
}

static void nv_edit (cap)
    cmdarg_T * cap;
{
    if (cap -> cmdchar == (-(('k')+((int)('I')<<8))) || cap -> cmdchar == (-((253)+((int)(KE_KINS)<<8)))) cap -> cmdchar = 'i';
    if (VIsual_active && (cap -> cmdchar == 'A' || cap -> cmdchar == 'I')) v_visop (cap);
    else if ((cap -> cmdchar == 'a' || cap -> cmdchar == 'i') && (cap -> oap -> op_type != 0 || VIsual_active)) {
        nv_object (cap);
    }
        ...
        case 'A' :
            ...
            else curwin -> w_cursor.col += (colnr_T) strlen ((char *) (ml_get_cursor ()));
            break;
        case 'I' :
            if (vim_strchr (p_cpo, 'H') == ((void *) 0)) beginline (1);
            else beginline (1 | 4);
            break;
        case 'a' :
            if (virtual_active () && (curwin -> w_cursor.coladd > 0 || *ml_get_cursor() == '\000' || *ml_get_cursor() == '\011')) curwin -> w_cursor.coladd ++;
            else if (*ml_get_cursor() != '\000') inc_cursor ();
            break;
        }
    ...
    int startln;
{
    ...
    if (repl || !stuff_empty()) restart_edit_save = restart_edit;
    else restart_edit_save = 0;
    restart_edit = 0;
    if (edit (cmd, startln, cap -> count1)) cap -> retval |= 1;
    if (restart_edit == 0) restart_edit = restart_edit_save;
}
    ...
    cmdarg_T * cap;
{
    if (cap -> oap -> op_type == 9) {
        ...
        else if (!Exec_reg && do_record (cap -> nchar) == 0) clearopbeep (cap -> oap);
    }
}
    ...
    cmdarg_T * cap;
{
    if (checkclearop (cap -> oap)) return;
    if (cap -> nchar == '=') {
        if (get_expr_register () == '\000') return;
    }
...
static void nv_halfpage (cap)
    cmdarg_T * cap;
{
    if ((cap -> cmdchar == 21 && curwin -> w_cursor.lnum == 1) || (cap -> cmdchar == 4 && curwin -> w_cursor.lnum == curbuf -> b_ml.ml_line_count)) clearopbeep
      (cap -> oap);
    else if (!checkclearop(cap->oap)) halfpage (cap -> cmdchar == 4, cap -> count0);
}
    ...
    cmdarg_T * cap;
{
    if (VIsual_active) nv_operator (cap);
        ...
        if (cap -> count0 <= 1) cap -> count0 = 2;
        if (curwin -> w_cursor.lnum + cap -> count0 - 1 > curbuf -> b_ml.ml_line_count) clearopbeep (cap -> oap);
        else {
            prep_redo (cap -> oap -> regname, cap -> count0, '\000', cap -> cmdchar, '\000', '\000', cap -> nchar);
            (void) do_join (cap -> count0, cap -> nchar == '\000', 1);
        }
    }
}
    ...
    cmdarg_T * cap;
{
    ...
    if (cap -> oap -> op_type != 0) {
        ...
        dir = (cap -> cmdchar == 'P' || (cap -> cmdchar == 'g' && cap -> nchar == 'P')) ? (-1) : 1;
        prep_redo_cmd (cap);
        if (cap -> cmdchar == 'g') flags |= 2;
        if (VIsual_active) {
            was_visual = 1;
            regname = cap -> oap -> regname;
            adjust_clip_reg (&regname);
            if (regname == 0 || ((regname) >= '0' && (regname) <= '9') || (clip_unnamed && (regname == '*' || regname == '+'))) {
                reg1 = get_register (regname, 1);
            }
            cap -> cmdchar = 'd';
            cap -> nchar = '\000';
            cap -> oap -> regname = '\000';
            ...
            if (reg1 != ((void *) 0)) {
                reg2 = get_register (regname, 0);
                put_register (regname, reg1);
            }
            if (VIsual_mode == 'V') flags |= 8;
            else if (VIsual_mode == 'v') flags |= 16;
            if (VIsual_mode == 22 && dir == 1) flags |= 32;
            dir = (-1);
            if ((VIsual_mode != 'V' && curwin -> w_cursor.col < curbuf -> b_op_start.col) || (VIsual_mode == 'V' && curwin -> w_cursor.lnum < curbuf ->
              b_op_start.lnum)) dir = 1;
        }
        do_put (cap -> oap -> regname, dir, cap -> count1, flags);
        if (reg2 != ((void *) 0)) put_register (regname, reg2);
        if (was_visual) {
    ...
    cmdarg_T * cap;
{
    ...
    else if (VIsual_active) v_swap_corners (cap -> cmdchar);
    else n_opencmd (cap);
}

static void nv_nbcmd (cap)
    cmdarg_T * cap;
{
