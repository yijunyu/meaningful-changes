...
static char * crypt_magic [] =
{"VimCrypt~01!", "VimCrypt~02!"};
static char crypt_magic_head [] = "VimCrypt~";
static int crypt_salt_len [] =
{0, 8};
static int crypt_seed_len [] =
{0, 8};
...
{
    int msg_scroll_save;
    if (msg_silent != 0) return;
    msg_add_fname (buf, name);
    if (strlen ((char *) (IObuff)) > (1024 + 1) - 80) IObuff [(1024 + 1) - 80] = '\000';
    ((__builtin_object_size ((char *) (IObuff), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (IObuff), (char *) (s), __builtin_object_size ((char *) (
      IObuff), 1 > 1)) : __inline_strcat_chk ((char *) (IObuff), (char *) (s)));
    msg_scroll_save = msg_scroll;
    if (shortmess ('O') && !exiting && p_verbose == 0) msg_scroll = 0;
    if (!msg_scroll) check_for_delay (0);
    msg_start ();
    ...
    int flags;
{
    ...
    int set_options = newfile || read_buffer || (eap != ((void *) 0) && eap -> read_edit);
    ...
    int using_b_fname;
    write_no_eol_lnum = 0;
    if (curbuf -> b_ffname == ((void *) 0) && !filtering && fname != ((void *) 0) && vim_strchr (p_cpo, 'f') != ((void *) 0) && !(flags&0x10)) {
        if (set_rw_fname (fname, sfname) == 0) return 0;
    }
    old_curbuf = curbuf;
    ...
    if (!filtering && !read_stdin && !read_buffer) {
        ...
        if (newfile) {
            if (apply_autocmds_exarg (EVENT_BUFREADCMD, ((void *) 0), sfname, 0, curbuf, eap)) return aborting () ? 0 : 1;
        }
        else if (apply_autocmds_exarg (EVENT_FILEREADCMD, sfname, sfname, 0, ((void *) 0), eap)) return aborting () ? 0 : 1;
        curbuf -> b_op_start = pos;
    }
    if ((shortmess ('o') || curbuf -> b_help) && p_verbose == 0) msg_scroll = 0;
    else msg_scroll = 1;
    if (fname != ((void *) 0) && *fname != '\000') {
        p = fname + strlen ((char *) (fname));
        if (vim_ispathsep (*((p)-1)) || strlen ((char *) (fname)) >= 1024) {
            filemess (curbuf, fname, (char_u *) ((char *) ("Illegal file name")), 0);
            msg_end ();
            msg_scroll = msg_save;
            return 0;
        }
    ...
    if (!read_stdin && !read_buffer) {
        ...
        if (perm >= 0 && !(((perm)&0170000)==0100000) && !(((perm)&0170000)==0010000) && !(((perm)&0170000)==0140000)) {
            if ((((perm) & 0170000) == 0040000)) filemess (curbuf, fname, (char_u *) ((char *) ("is a directory")), 0);
            else filemess (curbuf, fname, (char_u *) ((char *) ("is not a file")), 0);
            msg_end ();
            msg_scroll = msg_save;
            return 0;
        }
    ...
    if (set_options) {
        if (eap != ((void *) 0) && eap -> force_ff != 0) set_fileformat (get_fileformat_force (curbuf, eap), 4);
        else if (*p_ffs != '\000') set_fileformat (default_fileformat (), 4);
    }
    ...
    if (check_readonly && !readonlymode) curbuf -> b_p_ro = 0;
    ...
    if (read_stdin) {
        ...
        if (!(perm&0222) || access (((char *) fname), ((1 << 1)))) file_readonly = 1;
        fd = open (((char *) fname), (0x0000 | 0), (0));
    }
    if (fd < 0) {
        msg_scroll = msg_save;
        if (newfile) {
            if (perm < 0 && (*__error()) == 2) {
                ...
                if (!bt_dontwrite(curbuf)) {
                    ...
                    if (curbuf != old_curbuf || (using_b_ffname && (old_b_ffname != curbuf -> b_ffname)) || (using_b_fname && (old_b_fname != curbuf -> b_fname
                        ...
                        emsg ((char_u *) (((char *) (e_auchangedbuf))));
                        return 0;
                    }
                }
                if (dir_of_file_exists (fname)) filemess (curbuf, sfname, (char_u *) ((char *) ("[New File]")), 0);
                else filemess (curbuf, sfname, (char_u *) ((char *) ("[New DIRECTORY]")), 0);
                check_marks_read ();
                apply_autocmds_exarg (EVENT_BUFNEWFILE, sfname, sfname, 0, curbuf, eap);
                save_file_ff (curbuf);
                if (aborting ()) return 0;
                return 1;
            }
            else {
                filemess (curbuf, sfname, (char_u *) (((*__error()) == 27) ? ((char *) ("[File too big]")) : ((*__error()) == 84) ? ((char *) ("[File too big]"
    ...
    if ((check_readonly && file_readonly) || curbuf -> b_help) curbuf -> b_p_ro = 1;
    if (set_options) {
        if (!read_buffer) {
            curbuf -> b_p_eol = 1;
    ...
    if (!bt_dontwrite(curbuf)) {
        check_need_swap (newfile);
        if (!read_stdin && (curbuf != old_curbuf || (using_b_ffname && (old_b_ffname != curbuf -> b_ffname)) || (using_b_fname && (old_b_fname != curbuf ->
          b_fname)))) {
            emsg ((char_u *) (((char *) (e_auchangedbuf))));
            if (!read_buffer) close (fd);
            return 0;
        }
        if (swap_mode > 0 && curbuf -> b_ml.ml_mfp != ((void *) 0) && curbuf -> b_ml.ml_mfp -> mf_fname != ((void *) 0)) (void) mch_setperm (curbuf -> b_ml.
          ml_mfp -> mf_fname, (long) swap_mode);
    }
    if (swap_exists_action == 2) {
        if (!read_buffer && !read_stdin) close (fd);
        return 0;
    }
    ++no_wait_return;
    curbuf -> b_op_start.lnum = ((from == 0) ? 1 : from);
    ...
    if (!read_buffer) {
        int m = msg_scroll;
        int n = msg_scrolled;
        if (!read_stdin) close (fd);
        msg_scroll = 1;
        if (filtering) apply_autocmds_exarg (EVENT_FILTERREADPRE, ((void *) 0), sfname, 0, curbuf, eap);
        else if (read_stdin) apply_autocmds_exarg (EVENT_STDINREADPRE, ((void *) 0), sfname, 0, curbuf, eap);
        else if (newfile) apply_autocmds_exarg (EVENT_BUFREADPRE, ((void *) 0), sfname, 0, curbuf, eap);
        else apply_autocmds_exarg (EVENT_FILEREADPRE, sfname, sfname, 0, ((void *) 0), eap);
        if (msg_scrolled == n) msg_scroll = m;
        if (aborting ()) {
            --no_wait_return;
            msg_scroll = msg_save;
            curbuf -> b_p_ro = 1;
        ...
        if (!read_stdin && (curbuf != old_curbuf || (using_b_ffname && (old_b_ffname != curbuf -> b_ffname)) || (using_b_fname && (old_b_fname != curbuf ->
          b_fname)) || (fd = open (((char *) fname), (0x0000 | 0), (0))) < 0)) {
            --no_wait_return;
            msg_scroll = msg_save;
            if (fd < 0) emsg ((char_u *) (((char *) ("E200: *ReadPre autocommands made the file unreadable"))));
            else emsg ((char_u *) (((char *) ("E201: *ReadPre autocommands must not change current buffer"))));
            curbuf -> b_p_ro = 1;
            return 0;
        }
    }
    ...
    if (!recoverymode && !filtering && !(flags&0x10)) {
        if (read_stdin) {
            printf ("%s", (((char *) ("Vim: Reading from stdin...\n"))));
        }
        else if (!read_buffer) filemess (curbuf, sfname, (char_u *) "", 0);
    }
    msg_scroll = 0;
    linecnt = curbuf -> b_ml.ml_line_count;
    retry : if (file_rewind) {
        if (read_buffer) {
            read_buf_lnum = 1;
            read_buf_col = 0;
        }
        ...
        while (lnum > from) ml_delete (lnum --, 0);
        file_rewind = 0;
    }
    if (cryptkey != ((void *) 0)) crypt_pop_state ();
    if (keep_fileformat) keep_fileformat = 0;
    else {
        if (eap != ((void *) 0) && eap -> force_ff != 0) {
            fileformat = get_fileformat_force (curbuf, eap);
            try_unix = try_dos = try_mac = 0;
        }
        else if (curbuf -> b_p_bin) fileformat = 0;
        else if (*p_ffs == '\000') fileformat = get_fileformat (curbuf);
        else fileformat = -1;
    }
    if (!skip_read) {
        linerest = 0;
        ...
        read_undo_file = (newfile && (flags & 0x20) == 0 && curbuf -> b_ffname != ((void *) 0) && curbuf -> b_p_udf && !filtering && !read_stdin && 
          !read_buffer);
        if (read_undo_file) sha256_start (&sha_ctx);
    }
    while (!error && !got_int) {
        {
            if (!skip_read) {
                ...
                for (; size >= 10; size = (long) ((long_u) size >> 1)) {
                    if ((new_buffer = lalloc ((long_u) (size + linerest + 1), 0)) != ((void *) 0)) break;
                }
                ...
                if (linerest) ((__builtin_object_size ((char *) (new_buffer), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (new_buffer), (char *) (
                  ptr - linerest), (size_t) linerest, __builtin_object_size ((char *) (new_buffer), 0)) : __inline_memmove_chk ((char *) (new_buffer), (char *)
                  (ptr - linerest), (size_t) linerest));
                vim_free (buffer);
                buffer = new_buffer;
                ptr = buffer + linerest;
                line_start = buffer;
                if (read_buffer) {
                    if (read_buf_lnum > from) size = 0;
                    else {
                        int n, ni;
                        long tlen;
                        tlen = 0;
                ...
                if (filesize == 0) cryptkey = check_for_cryptkey (cryptkey, ptr, &size, &filesize, newfile, sfname, &did_ask_for_key);
                if (cryptkey != ((void *) 0) && size > 0) crypt_decode (ptr, size);
            }
            skip_read = 0;
            if (size <= 0) break;
            filesize += size;
            if (fileformat == -1) {
                if (try_dos || try_unix) {
                    for (p = ptr; p < ptr + size; ++p) {
                        if (*p == '\012') {
                            if (!try_unix || (try_dos && p > ptr && p [-1] == '\015')) fileformat = 1;
                            else fileformat = 0;
                            break;
                        }
                    }
                    if (fileformat == 0 && try_mac) {
                        ...
                        if (p >= ptr) {
                            ...
                            if (try_mac > try_unix) fileformat = 2;
                        }
                    }
                }
                if (fileformat == -1 && try_mac) fileformat = 2;
                if (fileformat == -1) fileformat = default_fileformat ();
                if (set_options) set_fileformat (fileformat, 4);
            }
        }
        if (fileformat == 2) {
            --ptr;
            while (++ptr, --size >= 0) {
                if ((c = *ptr) != '\000' && c != '\015' && c != '\012') continue;
                if (c == '\000') *ptr = '\012';
                else if (c == '\012') *ptr = '\015';
                else {
                    if (skip_count == 0) {
                        * ptr = '\000';
                        ...
                        if (read_undo_file) sha256_update (&sha_ctx, line_start, len);
                        ++lnum;
                        if (--read_count == 0) {
                            error = 1;
                            line_start = ptr;
                        ...
                        * ptr = '\000';
                        ...
                        if (fileformat == 1) {
                            if (ptr [-1] == '\015') {
                                ...
                                if (try_unix && !read_stdin && (read_buffer || lseek (fd, (off_t) 0L, 0) == 0)) {
                                    fileformat = 0;
                                    if (set_options) set_fileformat (0, 4);
                                    file_rewind = 1;
                                    keep_fileformat = 1;
                                    goto retry;
                                }
    ...
    failed : if (error && read_count == 0) error = 0;
    if (!error && !got_int && linerest != 0 && !(!curbuf->b_p_bin && fileformat == 1 && *line_start == 26 && ptr == line_start + 1)) {
        if (set_options) curbuf -> b_p_eol = 0;
        * ptr = '\000';
        len = (colnr_T) (ptr - line_start + 1);
        if (ml_append (lnum, line_start, len, newfile) == 0) error = 1;
        else {
            if (read_undo_file) sha256_update (&sha_ctx, line_start, len);
            read_no_eol_lnum = ++lnum;
        }
    }
    if (set_options) save_file_ff (curbuf);
    if (cryptkey != ((void *) 0)) {
        crypt_pop_state ();
        if (cryptkey != curbuf -> b_p_key) free_crypt_key (cryptkey);
    }
    if (!read_buffer && !read_stdin) close (fd);
    else {
        int fdflags = fcntl (fd, 1);
        if (fdflags >= 0 && (fdflags & 1) == 0) fcntl (fd, 2, fdflags | 1);
    }
    vim_free (buffer);
    if (read_stdin) {
        close (0);
    ...
    if (!recoverymode) {
        ...
        else if (linecnt) appended_lines_mark (from, linecnt);
        if (read_stdin) {
            settmode (2);
            starttermcap ();
            screenclear ();
        ...
        if (got_int) {
            if (!(flags&0x10)) {
                filemess (curbuf, sfname, (char_u *) ((char *) (e_interr)), 0);
                if (newfile) curbuf -> b_p_ro = 1;
            }
            msg_scroll = msg_save;
            check_marks_read ();
            return 1;
        ...
        if (!filtering && !(flags&0x10)) {
            ...
            if ((((perm) & 0170000) == 0010000)) {
                ((__builtin_object_size ((char *) (IObuff), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (IObuff), (char *) (((char *) (
                  "[fifo/socket]"))), __builtin_object_size ((char *) (IObuff), 1 > 1)) : __inline_strcat_chk ((char *) (IObuff), (char *) (((char *) (
                  "[fifo/socket]")))));
                c = 1;
            }
            if (curbuf -> b_p_ro) {
                ((__builtin_object_size ((char *) (IObuff), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (IObuff), (char *) (shortmess ('r') ? ((char
                  *) ("[RO]")) : ((char *) ("[readonly]"))), __builtin_object_size ((char *) (IObuff), 1 > 1)) : __inline_strcat_chk ((char *) (IObuff), (char *
                  ) (shortmess ('r') ? ((char *) ("[RO]")) : ((char *) ("[readonly]")))));
                c = 1;
            }
            if (read_no_eol_lnum) {
                msg_add_eol ();
            ...
            if (ff_error == 1) {
                ((__builtin_object_size ((char *) (IObuff), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (IObuff), (char *) (((char *) (
                  "[CR missing]"))), __builtin_object_size ((char *) (IObuff), 1 > 1)) : __inline_strcat_chk ((char *) (IObuff), (char *) (((char *) (
                  "[CR missing]")))));
                c = 1;
            }
            if (split) {
                ((__builtin_object_size ((char *) (IObuff), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (IObuff), (char *) (((char *) (
                  "[long lines split]"))), __builtin_object_size ((char *) (IObuff), 1 > 1)) : __inline_strcat_chk ((char *) (IObuff), (char *) (((char *) (
                  "[long lines split]")))));
                c = 1;
            }
            if (cryptkey != ((void *) 0)) {
                ((__builtin_object_size ((char *) (IObuff), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (IObuff), (char *) (((char *) ("[crypted]"))
                  ), __builtin_object_size ((char *) (IObuff), 1 > 1)) : __inline_strcat_chk ((char *) (IObuff), (char *) (((char *) ("[crypted]")))));
                c = 1;
            }
            if (error) {
                ((__builtin_object_size ((char *) (IObuff), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (IObuff), (char *) (((char *) (
                  "[READ ERRORS]"))), __builtin_object_size ((char *) (IObuff), 1 > 1)) : __inline_strcat_chk ((char *) (IObuff), (char *) (((char *) (
                  "[READ ERRORS]")))));
                c = 1;
            }
            if (msg_add_fileformat (fileformat)) c = 1;
            if (cryptkey != ((void *) 0)) msg_add_lines (c, (long) linecnt, filesize - 12 - crypt_salt_len [use_crypt_method] - crypt_seed_len [
              use_crypt_method]);
            else msg_add_lines (c, (long) linecnt, filesize);
            vim_free (keep_msg);
            keep_msg = ((void *) 0);
            msg_scrolled_ign = 1;
            p = msg_trunc_attr (IObuff, 0, 0);
            if (read_stdin || read_buffer || restart_edit != 0 || (msg_scrolled != 0 && !need_wait_return)) set_keep_msg (p, 0);
            msg_scrolled_ign = 0;
        }
        if (newfile && (error)) curbuf -> b_p_ro = 1;
        u_clearline ();
        if (exmode_active) curwin -> w_cursor.lnum = from + linecnt;
        else curwin -> w_cursor.lnum = from + 1;
        check_cursor_lnum ();
        beginline (1 | 4);
    ...
    if (flags & 0x20) u_find_first_changed ();
    if (read_undo_file) {
        char_u hash [32];
    ...
    if (!read_stdin && !read_buffer) {
        ...
        if (set_options) save_file_ff (curbuf);
        msg_scroll = 1;
        if (filtering) apply_autocmds_exarg (EVENT_FILTERREADPOST, ((void *) 0), sfname, 0, curbuf, eap);
        else if (newfile) apply_autocmds_exarg (EVENT_BUFREADPOST, ((void *) 0), sfname, 0, curbuf, eap);
        else apply_autocmds_exarg (EVENT_FILEREADPOST, sfname, sfname, 0, ((void *) 0), eap);
        if (msg_scrolled == n) msg_scroll = m;
        if (aborting ()) return 0;
    }
    if (recoverymode && error) return 0;
    return 1;
}
...
int prep_exarg (eap, buf)
    ...
    buf_T * buf;
{
    eap -> cmd = alloc ((unsigned) (strlen ((char *) (buf -> b_p_ff)) + 15));
    if (eap -> cmd == ((void *) 0)) return 0;
    __builtin___sprintf_chk ((char *) eap -> cmd, 0, __builtin_object_size ((char *) eap -> cmd, 1 > 1), "e ++ff=%s", buf -> b_p_ff);
    eap -> force_ff = 7;
...
static void check_marks_read () {
    if (!curbuf->b_marks_read && get_viminfo_parameter ('\'') > 0 && curbuf -> b_ffname != ((void *) 0)) read_viminfo (((void *) 0), 2);
    ...
    int len;
{
    ...
    for (i = 0; i < (int) (sizeof (crypt_magic) / sizeof (crypt_magic [0])); i ++) {
        if (len < (12 + crypt_salt_len [i] + crypt_seed_len [i])) continue;
        if (memcmp (ptr, crypt_magic [i], 12) == 0) return i;
    }
    i = (int) strlen ((char *) (crypt_magic_head));
    if (len >= i && memcmp (ptr, crypt_magic_head, i) == 0) emsg ((char_u *) (((char *) ("E821: File is encrypted with unknown method"))));
    return -1;
}

static char_u * check_for_cryptkey (cryptkey, ptr, sizep, filesizep, newfile, fname, did_ask)
    char_u * cryptkey;
    char_u * ptr;
    long * sizep;
    off_t * filesizep;
    ...
    int * did_ask;
{
    ...
    if (method >= 0) {
        ...
        if (method > 0) (void) blowfish_self_test ();
        if (cryptkey == ((void *) 0) && !*did_ask) {
            if (*curbuf->b_p_key) cryptkey = curbuf -> b_p_key;
            else {
                smsg ((char_u *) ((char *) (need_key_msg)), fname);
                msg_scroll = 1;
                cryptkey = get_crypt_key (newfile, 0);
                * did_ask = 1;
                if (cryptkey != ((void *) 0) && *cryptkey == '\000') {
                    if (cryptkey != curbuf -> b_p_key) vim_free (cryptkey);
                    cryptkey = ((void *) 0);
                }
            }
        }
        if (cryptkey != ((void *) 0)) {
            ...
            if (method == 0) crypt_init_keys (cryptkey);
            else {
                bf_key_init (cryptkey, ptr + 12, salt_len);
                bf_ofb_init (ptr + 12 + salt_len, seed_len);
            }
            *filesizep += 12 + salt_len + seed_len;
            *sizep -= 12 + salt_len + seed_len;
            ((__builtin_object_size ((char *) (ptr), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (ptr), (char *) (ptr + 12 + salt_len + seed_len), (
              size_t) * sizep, __builtin_object_size ((char *) (ptr), 0)) : __inline_memmove_chk ((char *) (ptr), (char *) (ptr + 12 + salt_len + seed_len), (
              size_t) * sizep));
        }
    }
    else if (newfile && *curbuf->b_p_key != '\000' && !starting) set_option_value ((char_u *) "key", 0L, (char_u *) "", 4);
    return cryptkey;
}
...
{
    int method;
    char_u buffer [12 + 8 + 8 + 2];
    if (fread (buffer, 12, 1, fp) != 1) return 0;
    method = crypt_method_from_magic ((char *) buffer, 12 + 8 + 8);
    if (method < 0 || method != get_crypt_method (curbuf)) return 0;
    crypt_push_state ();
    if (method == 0) crypt_init_keys (curbuf -> b_p_key);
    else {
        int salt_len = crypt_salt_len [method];
        int seed_len = crypt_seed_len [method];
        if (fread (buffer, salt_len + seed_len, 1, fp) != 1) return 0;
    ...
    int * lenp;
{
    ...
    char_u * salt;
    char_u * seed;
    header = alloc_clear (12 + 8 + 8 + 2);
    if (header != ((void *) 0)) {
        crypt_push_state ();
        use_crypt_method = get_crypt_method (buf);
        vim_strncpy (header, (char_u *) crypt_magic [use_crypt_method], 12);
        if (use_crypt_method == 0) crypt_init_keys (buf -> b_p_key);
        else {
            salt = header + 12;
            seed = salt + salt_len;
...
int check_file_readonly (fname, perm)
    ...
    int perm;
{
    return ((perm & 0222) == 0 || access (((char *) fname), ((1 << 1))));
}

int buf_write (buf, fname, sfname, start, end, eap, append, forceit, reset_changed, filtering)
    buf_T * buf;
    char_u * fname;
    char_u * sfname;
    linenr_T start, end;
    ...
    int filtering;
{
    ...
    context_sha256_T sha_ctx;
    if (fname == ((void *) 0) || *fname == '\000') return 0;
    if (buf -> b_ml.ml_mfp == ((void *) 0)) {
        emsg ((char_u *) (((char *) (e_emptybuf))));
        return 0;
    }
    if (check_secure ()) return 0;
    if (strlen ((char *) (fname)) >= 1024) {
        emsg ((char_u *) (((char *) (e_longname))));
        return 0;
    }
    ex_no_reprint = 1;
    if (buf -> b_ffname == ((void *) 0) && reset_changed && whole && buf == curbuf && !bt_nofile(buf) && !filtering && (!append || vim_strchr (p_cpo, 'P') != (
      (void *) 0)) && vim_strchr (p_cpo, 'F') != ((void *) 0)) {
        if (set_rw_fname (fname, sfname) == 0) return 0;
        buf = curbuf;
    }
    if (sfname == ((void *) 0)) sfname = fname;
    ffname = fname;
    fname = sfname;
    if (buf -> b_ffname != ((void *) 0) && strcasecmp ((char *) (((ffname))), (char *) (((buf -> b_ffname)))) == 0) overwriting = 1;
    else overwriting = 0;
    if (exiting) settmode (0);
    ++no_wait_return;
    ...
    buf -> b_op_end.col = 0;
    {
        ...
        if (ffname == buf -> b_ffname) buf_ffname = 1;
        if (sfname == buf -> b_sfname) buf_sfname = 1;
        if (fname == buf -> b_ffname) buf_fname_f = 1;
        if (fname == buf -> b_sfname) buf_fname_s = 1;
        aucmd_prepbuf (&aco, buf);
        if (append) {
            if (!(did_cmd=apply_autocmds_exarg(EVENT_FILEAPPENDCMD,sfname,sfname,0,curbuf,eap))) {
                if (overwriting && bt_nofile (curbuf)) nofile_err = 1;
                else apply_autocmds_exarg (EVENT_FILEAPPENDPRE, sfname, sfname, 0, curbuf, eap);
            }
        }
        else if (filtering) {
            apply_autocmds_exarg (EVENT_FILTERWRITEPRE, ((void *) 0), sfname, 0, curbuf, eap);
        }
        else if (reset_changed && whole) {
            if (!(did_cmd=apply_autocmds_exarg(EVENT_BUFWRITECMD,sfname,sfname,0,curbuf,eap))) {
                if (overwriting && bt_nofile (curbuf)) nofile_err = 1;
                else apply_autocmds_exarg (EVENT_BUFWRITEPRE, sfname, sfname, 0, curbuf, eap);
            }
        }
        else {
            if (!(did_cmd=apply_autocmds_exarg(EVENT_FILEWRITECMD,sfname,sfname,0,curbuf,eap))) {
                if (overwriting && bt_nofile (curbuf)) nofile_err = 1;
                else apply_autocmds_exarg (EVENT_FILEWRITEPRE, sfname, sfname, 0, curbuf, eap);
            }
        }
        aucmd_restbuf (&aco);
        if (!buf_valid(buf)) buf = ((void *) 0);
        if (buf == ((void *) 0) || (buf -> b_ml.ml_mfp == ((void *) 0) && !empty_memline) || did_cmd || nofile_err || aborting ()) {
            --no_wait_return;
            msg_scroll = msg_save;
            if (nofile_err) emsg ((char_u *) (((char *) ("E676: No matching autocommands for acwrite buffer"))));
            if (nofile_err || aborting ()) return 0;
            if (did_cmd) {
                if (buf == ((void *) 0)) return 1;
                if (overwriting) {
                    ml_timestamp (buf);
                    if (append) buf -> b_flags &= ~0x10;
                    else buf -> b_flags &= ~(0x08+0x10+0x40);
                }
                if (reset_changed && buf -> b_changed && !append && (overwriting || vim_strchr (p_cpo, '+') != ((void *) 0))) return 0;
                return 1;
            }
            if (!aborting()) emsg ((char_u *) (((char *) ("E203: Autocommands deleted or unloaded buffer to be written"))));
            return 0;
        }
        if (buf -> b_ml.ml_line_count != old_line_count) {
            if (whole) end = buf -> b_ml.ml_line_count;
            else if (buf -> b_ml.ml_line_count > old_line_count) end += buf -> b_ml.ml_line_count - old_line_count;
            else {
                end -= old_line_count - buf -> b_ml.ml_line_count;
                if (end < start) {
                    --no_wait_return;
                    msg_scroll = msg_save;
                    emsg ((char_u *) (((char *) ("E204: Autocommand changed number of lines in unexpected way"))));
                    return 0;
                }
            }
        }
        if (buf_ffname) ffname = buf -> b_ffname;
        if (buf_sfname) sfname = buf -> b_sfname;
        if (buf_fname_f) fname = buf -> b_ffname;
        if (buf_fname_s) fname = buf -> b_sfname;
    }
    if (netbeans_active () && isNetbeansBuffer (buf)) {
        if (whole) {
            if (buf -> b_changed || isNetbeansModified (buf)) {
                ...
                errnum = (char_u *) "E656: ";
                errmsg = (char_u *) ((char *) ("NetBeans disallows writes of unmodified buffers"));
                buffer = ((void *) 0);
                goto fail;
            }
        }
            ...
            errnum = (char_u *) "E657: ";
            errmsg = (char_u *) ((char *) ("Partial writes disallowed for NetBeans buffers"));
            buffer = ((void *) 0);
            goto fail;
        }
    }
    if (shortmess ('o') && !exiting) msg_scroll = 0;
    else msg_scroll = 1;
    if (!filtering) filemess (buf, fname, (char_u *) "", 0);
    msg_scroll = 0;
    buffer = alloc (8192);
    if (buffer == ((void *) 0)) {
        buffer = smallbuf;
    ...
    st_old.st_ino = 0;
    perm = -1;
    if (stat (((char *) fname), (&st_old)) < 0) newfile = 1;
    else {
        perm = st_old.st_mode;
        if (!(((st_old.st_mode)&0170000)==0100000)) {
            if ((((st_old.st_mode) & 0170000) == 0040000)) {
                errnum = (char_u *) "E502: ";
                errmsg = (char_u *) ((char *) ("is a directory"));
                goto fail;
            }
            if (mch_nodetype (fname) != 1) {
                errnum = (char_u *) "E503: ";
                errmsg = (char_u *) ((char *) ("is not a file or writable device"));
                goto fail;
            }
            device = 1;
            newfile = 1;
    ...
    if (!device && !newfile) {
        ...
        if (!forceit && file_readonly) {
            if (vim_strchr (p_cpo, 'W') != ((void *) 0)) {
                errnum = (char_u *) "E504: ";
                errmsg = (char_u *) ((char *) (err_readonly));
            }
            else {
                errnum = (char_u *) "E505: ";
                errmsg = (char_u *) ((char *) ("is read-only (add ! to override)"));
            }
            goto fail;
        }
        if (overwriting) {
    ...
    if (!newfile) acl = mch_get_acl (fname);
    dobackup = (p_wb || p_bk || *p_pm != '\000');
    if (dobackup && *p_bsk != '\000' && match_file_list (p_bsk, sfname, ffname)) dobackup = 0;
    prev_got_int = got_int;
    got_int = 0;
    buf -> b_saving = 1;
    if (!(append&&*p_pm == '\000') && !filtering && perm >= 0 && dobackup) {
        struct stat st;
        if ((bkc_flags & 0x001) || append) backup_copy = 1;
        else if ((bkc_flags & 0x002)) {
            int i;
            if (st_old.st_nlink > 1 || lstat (((char *) fname), (&st)) < 0 || st.st_dev != st_old.st_dev || st.st_ino != st_old.st_ino) backup_copy = 1;
            else {
                ((__builtin_object_size ((char *) (IObuff), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (IObuff), (char *) (fname),
                  __builtin_object_size ((char *) (IObuff), 1 > 1)) : __inline_strcpy_chk ((char *) (IObuff), (char *) (fname)));
                for (i = 4913;; i += 123) {
                    __builtin___sprintf_chk ((char *) gettail (IObuff), 0, __builtin_object_size ((char *) gettail (IObuff), 1 > 1), "%d", i);
                    if (lstat (((char *) IObuff), (&st)) < 0) break;
                }
                fd = open (((char *) IObuff), (0x0200 | 0x0001 | 0x0800 | 0x0100), (perm));
                if (fd < 0) backup_copy = 1;
                else {
                    ignored = fchown (fd, st_old.st_uid, st_old.st_gid);
                    if (stat (((char *) IObuff), (&st)) < 0 || st.st_uid != st_old.st_uid || st.st_gid != st_old.st_gid || (long) st.st_mode != perm)
                      backup_copy = 1;
                    close (fd);
                    unlink ((char *) (IObuff));
                }
        ...
        if ((bkc_flags & 0x008) || (bkc_flags & 0x010)) {
            int lstat_res;
            lstat_res = lstat (((char *) fname), (&st));
            if ((bkc_flags & 0x008) && lstat_res == 0 && st.st_ino != st_old.st_ino) backup_copy = 0;
            if ((bkc_flags & 0x010) && st_old.st_nlink > 1 && (lstat_res != 0 || st.st_ino == st_old.st_ino)) backup_copy = 0;
        }
        if (*p_bex == '\000') {
            backup_ext = (char_u *) ".bak";
        }
        else backup_ext = p_bex;
        if (backup_copy && (fd = open (((char *) fname), (0x0000 | 0), (0))) >= 0) {
            int bfd;
            char_u * copybuf, * wp;
            int some_error = 0;
            struct stat st_new;
            ...
            while (*dirp) {
                ...
                did_set_shortname = 0;
                for (;;) {
                    backup = buf_modname ((buf -> b_p_sn || buf -> b_shortname), rootname, backup_ext, 0);
                    if (backup == ((void *) 0)) {
                        vim_free (rootname);
                        some_error = 1;
                        goto nobackup;
                    ...
                    if (stat (((char *) backup), (&st_new)) >= 0) {
                        if (st_new.st_dev == st_old.st_dev && st_new.st_ino == st_old.st_ino) {
                            ...
                            if (did_set_shortname) buf -> b_shortname = 0;
                            break;
                        }
                        if (!p_bk) {
                            wp = backup + strlen ((char *) (backup)) - 1 - strlen ((char *) (backup_ext));
                            if (wp < backup) wp = backup;
                            * wp = 'z';
                            while (*wp > 'a' && stat (((char *) backup), (&st_new)) >= 0) --*wp;
                            if (*wp == 'a') {
                                vim_free (backup);
                                backup = ((void *) 0);
                            }
                ...
                if (backup != ((void *) 0)) {
                    unlink ((char *) (backup));
                    bfd = open (((char *) backup), (0x0001 | 0x0200 | 0 | 0x0800 | 0x0100), (perm & 0777));
                    if (bfd < 0) {
                        vim_free (backup);
                        backup = ((void *) 0);
                    }
                        ...
                        if (st_new.st_gid != st_old.st_gid && fchown (bfd, (uid_t) - 1, st_old.st_gid) != 0) mch_setperm (backup, (perm & 0707) | ((perm & 07)
                          << 3));
                        write_info.bw_fd = bfd;
                        write_info.bw_buf = copybuf;
                        write_info.bw_flags = 0x2000;
                        while ((write_info.bw_len = read ((fd), (char *) (copybuf), (size_t) (8192))) > 0) {
                            if (buf_write_bytes (&write_info) == 0) {
                                errmsg = (char_u *) ((char *) ("E506: Can't write to backup file (add ! to override)"));
                                break;
                            }
                            ui_breakcheck ();
                            if (got_int) {
                                errmsg = (char_u *) ((char *) (e_interr));
                                break;
                            }
                        }
                        if (close (bfd) < 0 && errmsg == ((void *) 0)) errmsg = (char_u *) ((char *) ("E507: Close error for backup file (add ! to override)"))
                          ;
                        if (write_info.bw_len < 0) errmsg = (char_u *) ((char *) ("E508: Can't read file for backup (add ! to override)"));
                        set_file_time (backup, st_old.st_atimespec.tv_sec, st_old.st_mtimespec.tv_sec);
            ...
            nobackup : close (fd);
            vim_free (copybuf);
            if (backup == ((void *) 0) && errmsg == ((void *) 0)) errmsg = (char_u *) ((char *) ("E509: Cannot create backup file (add ! to override)"));
            ...
            while (*dirp) {
                ...
                if (rootname == ((void *) 0)) backup = ((void *) 0);
                else {
                    backup = buf_modname ((buf -> b_p_sn || buf -> b_shortname), rootname, backup_ext, 0);
                    vim_free (rootname);
                }
                if (backup != ((void *) 0)) {
                    if (!p_bk && mch_getperm (backup) >= 0) {
                        ...
                        if (p < backup) p = backup;
                        * p = 'z';
                        while (*p > 'a' && mch_getperm (backup) >= 0) --*p;
                        if (*p == 'a') {
                            vim_free (backup);
                            backup = ((void *) 0);
                        }
                ...
                if (backup != ((void *) 0)) {
                    if (vim_rename (fname, backup) == 0) break;
                    vim_free (backup);
                    backup = ((void *) 0);
                }
            }
            if (backup == ((void *) 0) && !forceit) {
                errmsg = (char_u *) ((char *) ("E510: Can't make backup file (add ! to override)"));
                goto fail;
            }
        }
    }
    if (forceit && perm >= 0 && !(perm&0200) && st_old.st_uid == getuid () && vim_strchr (p_cpo, 'W') == ((void *) 0)) {
        perm |= 0200;
    ...
    if (buf -> b_ml.ml_flags & 1) start = end + 1;
    if (reset_changed && !newfile && overwriting && !(exiting&&backup!=((void*)0))) {
        ml_preserve (buf, 0);
        if (got_int) {
            errmsg = (char_u *) ((char *) (e_interr));
            goto restore_backup;
    ...
    wfname = fname;
    while ((fd = open (((char *) wfname), (0x0001 | 0 | (append ? (forceit ? (0x0008 | 0x0200) : 0x0008) : (0x0200 | 0x0400))), (perm < 0 ? 0666 : (perm & 0777
      )))) < 0) {
        if (errmsg == ((void *) 0)) {
            struct stat st;
            if ((!newfile && st_old.st_nlink > 1) || (lstat (((char *) fname), (&st)) == 0 && (st.st_dev != st_old.st_dev || st.st_ino != st_old.st_ino)))
              errmsg = (char_u *) ((char *) ("E166: Can't open linked file for writing"));
            else {
                errmsg = (char_u *) ((char *) ("E212: Can't open file for writing"));
                if (forceit && vim_strchr (p_cpo, 'W') == ((void *) 0) && perm >= 0) {
                    if (!(perm&0200)) made_writable = 1;
                    perm |= 0200;
                    if (st_old.st_uid != getuid () || st_old.st_gid != getgid ()) perm &= 0777;
                    if (!append) unlink ((char *) (wfname));
                    continue;
                }
            }
        }
        restore_backup : {
            ...
            if (backup != ((void *) 0) && wfname == fname) {
                if (backup_copy) {
                    if (stat (((char *) fname), (&st)) < 0) vim_rename (backup, fname);
                    if (stat (((char *) fname), (&st)) >= 0) unlink ((char *) (backup));
                }
                else {
                    vim_rename (backup, fname);
                }
            }
            if (!newfile && stat (((char *) fname), (&st)) < 0) end = 0;
        }
        goto fail;
    }
    errmsg = ((void *) 0);
    write_info.bw_fd = fd;
    if (*buf->b_p_key != '\000' && !filtering) {
        char_u * header;
        int header_len;
        header = prepare_crypt_write (buf, &header_len);
        if (header == ((void *) 0)) end = 0;
        else {
            write_info.bw_buf = header;
            write_info.bw_len = header_len;
            write_info.bw_flags = 0x2000;
            if (buf_write_bytes (&write_info) == 0) end = 0;
            wb_flags |= 0x1000;
            vim_free (header);
        }
    }
    ...
    nchars = 0;
    if (eap != ((void *) 0) && eap -> force_bin != 0) write_bin = (eap -> force_bin == 1);
    else write_bin = buf -> b_p_bin;
    write_undo_file = (buf -> b_p_udf && overwriting && !append && !filtering && reset_changed);
    if (write_undo_file) sha256_start (&sha_ctx);
    ...
    for (lnum = start; lnum <= end; ++lnum) {
        ...
        if (write_undo_file) sha256_update (&sha_ctx, ptr + 1, (UINT32_T) (strlen ((char *) (ptr + 1)) + 1));
        while ((c = *++ptr) != '\000') {
            if (c == '\012') *s = '\000';
            else if (c == '\015' && fileformat == 2) *s = '\012';
            else *s = c;
            ++s;
        ...
        if (end == 0 || (lnum == end && write_bin && (lnum == write_no_eol_lnum || (lnum == buf -> b_ml.ml_line_count && !buf->b_p_eol)))) {
            ++lnum;
            no_eol = 1;
            break;
        }
        if (fileformat == 0) *s++ = '\012';
        else {
            *s++ = '\015';
            if (fileformat == 1) {
                if (++len == bufsize) {
    ...
    if (p_fs && fsync (fd) != 0 && !device) {
        errmsg = (char_u *) ((char *) ("E667: Fsync failed"));
        end = 0;
    }
    if (backup != ((void *) 0) && !backup_copy) {
        ...
        if (stat (((char *) wfname), (&st)) < 0 || st.st_uid != st_old.st_uid || st.st_gid != st_old.st_gid) {
            ignored = fchown (fd, st_old.st_uid, st_old.st_gid);
            if (perm >= 0) (void) mch_setperm (wfname, perm);
        }
        buf_setino (buf);
    }
    else if (!buf->b_dev_valid) buf_setino (buf);
    if (close (fd) != 0) {
        errmsg = (char_u *) ((char *) ("E512: Close failed"));
        end = 0;
    }
    if (made_writable) perm &= ~0200;
    if (perm >= 0) (void) mch_setperm (wfname, perm);
    if (!backup_copy) mch_set_acl (wfname, acl);
    if (wb_flags & 0x1000) crypt_pop_state ();
    if (end == 0) {
        if (errmsg == ((void *) 0)) {
            if (got_int) errmsg = (char_u *) ((char *) (e_interr));
            else errmsg = (char_u *) ((char *) ("E514: write error (file system full?)"));
        }
        if (backup != ((void *) 0)) {
            if (backup_copy) {
                if (got_int) {
                    msg ((char_u *) (((char *) (e_interr))));
                    out_flush ();
                }
                if ((fd = open (((char *) backup), (0x0000 | 0), (0))) >= 0) {
                    if ((write_info.bw_fd = open (((char *) fname), (0x0001 | 0x0200 | 0x0400 | 0), (perm & 0777))) >= 0) {
                        write_info.bw_buf = smallbuf;
                        write_info.bw_flags = 0x2000;
                        while ((write_info.bw_len = read ((fd), (char *) (smallbuf), (size_t) (256))) > 0) if (buf_write_bytes (&write_info) == 0) break;
                        if (close (write_info.bw_fd) >= 0 && write_info.bw_len == 0) end = 1;
                    }
                    close (fd);
                }
            }
            else {
                if (vim_rename (backup, fname) == 0) end = 1;
            }
        }
        goto fail;
    }
    ...
    if (!filtering) {
        ...
        c = 0;
        if (device) {
            ((__builtin_object_size ((char *) (IObuff), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (IObuff), (char *) (((char *) ("[Device]"))),
              __builtin_object_size ((char *) (IObuff), 1 > 1)) : __inline_strcat_chk ((char *) (IObuff), (char *) (((char *) ("[Device]")))));
            c = 1;
        }
        else if (newfile) {
            ((__builtin_object_size ((char *) (IObuff), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (IObuff), (char *) (shortmess ('n') ? ((char *) (
              "[New]")) : ((char *) ("[New File]"))), __builtin_object_size ((char *) (IObuff), 1 > 1)) : __inline_strcat_chk ((char *) (IObuff), (char *) (
              shortmess ('n') ? ((char *) ("[New]")) : ((char *) ("[New File]")))));
            c = 1;
        }
        if (no_eol) {
            msg_add_eol ();
        ...
        if (!shortmess('W')) {
            if (append) ((__builtin_object_size ((char *) (IObuff), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (IObuff), (char *) (shortmess ('w') ?
              ((char *) (" [a]")) : ((char *) (" appended"))), __builtin_object_size ((char *) (IObuff), 1 > 1)) : __inline_strcat_chk ((char *) (IObuff), (
              char *) (shortmess ('w') ? ((char *) (" [a]")) : ((char *) (" appended")))));
            else ((__builtin_object_size ((char *) (IObuff), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (IObuff), (char *) (shortmess ('w') ? ((
              char *) (" [w]")) : ((char *) (" written"))), __builtin_object_size ((char *) (IObuff), 1 > 1)) : __inline_strcat_chk ((char *) (IObuff), (char *
              ) (shortmess ('w') ? ((char *) (" [w]")) : ((char *) (" written")))));
        }
        set_keep_msg (msg_trunc_attr (IObuff, 0, 0), 0);
    }
    if (reset_changed && whole && !append && (overwriting || vim_strchr (p_cpo, '+') != ((void *) 0))) {
        unchanged (buf, 1);
        u_unchanged (buf);
    ...
    if (*p_pm && dobackup) {
        char * org = (char *) buf_modname ((buf -> b_p_sn || buf -> b_shortname), fname, p_pm, 0);
        if (backup != ((void *) 0)) {
            struct stat st;
            if (org == ((void *) 0)) emsg ((char_u *) (((char *) ("E205: Patchmode: can't save original file"))));
            else if (stat ((org), (&st)) < 0) {
                vim_rename (backup, (char_u *) org);
                vim_free (backup);
                backup = ((void *) 0);
                set_file_time ((char_u *) org, st_old.st_atimespec.tv_sec, st_old.st_mtimespec.tv_sec);
            }
        }
        else {
            int empty_fd;
            if (org == ((void *) 0) || (empty_fd = open ((org), (0x0200 | 0 | 0x0800 | 0x0100), (perm < 0 ? 0666 : (perm & 0777)))) < 0) emsg ((char_u *) (((
    ...
    if (!p_bk && backup != ((void *) 0) && unlink ((char *) (backup)) != 0) emsg ((char_u *) (((char *) ("E207: Can't delete backup file"))));
    goto nofail;
    fail : --no_wait_return;
    nofail : buf -> b_saving = 0;
    vim_free (backup);
    if (buffer != smallbuf) vim_free (buffer);
    mch_free_acl (acl);
    if (errmsg != ((void *) 0)) {
        int numlen = errnum != ((void *) 0) ? (int) strlen ((char *) (errnum)) : 0;
        attr = highlight_attr [(int) (HLF_E)];
        msg_add_fname (buf, fname);
        if (strlen ((char *) (IObuff)) + strlen ((char *) (errmsg)) + numlen >= (1024 + 1)) IObuff [(1024 + 1) - strlen ((char *) (errmsg)) - numlen - 1] =
          '\000';
        if (errnum != ((void *) 0)) {
            ((__builtin_object_size ((char *) ((IObuff + numlen)), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) ((IObuff + numlen)), (char *) ((
              IObuff)), strlen ((char *) (IObuff)) + 1, __builtin_object_size ((char *) ((IObuff + numlen)), 0)) : __inline_memmove_chk ((char *) ((IObuff +
              numlen)), (char *) ((IObuff)), strlen ((char *) (IObuff)) + 1));
            ((__builtin_object_size ((char *) (IObuff), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (IObuff), (char *) (errnum), (size_t) numlen,
              __builtin_object_size ((char *) (IObuff), 0)) : __inline_memmove_chk ((char *) (IObuff), (char *) (errnum), (size_t) numlen));
        }
        ((__builtin_object_size ((char *) (IObuff), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (IObuff), (char *) (errmsg), __builtin_object_size (
          (char *) (IObuff), 1 > 1)) : __inline_strcat_chk ((char *) (IObuff), (char *) (errmsg)));
        emsg (IObuff);
        if (errmsg_allocated) vim_free (errmsg);
        retval = 0;
        if (end == 0) {
            msg_puts_attr ((char_u *) (((char *) ("\nWARNING: Original file may be lost or damaged\n"))), (attr | 0x1000));
            msg_puts_attr ((char_u *) (((char *) ("don't quit the editor until the file is successfully written!"))), (attr | 0x1000));
    ...
    if (!should_abort(retval)) {
        ...
        write_no_eol_lnum = 0;
        aucmd_prepbuf (&aco, buf);
        if (append) apply_autocmds_exarg (EVENT_FILEAPPENDPOST, fname, fname, 0, curbuf, eap);
        else if (filtering) apply_autocmds_exarg (EVENT_FILTERWRITEPOST, ((void *) 0), fname, 0, curbuf, eap);
        else if (reset_changed && whole) apply_autocmds_exarg (EVENT_BUFWRITEPOST, fname, fname, 0, curbuf, eap);
        else apply_autocmds_exarg (EVENT_FILEWRITEPOST, fname, fname, 0, curbuf, eap);
        aucmd_restbuf (&aco);
        if (aborting ()) retval = 0;
    }
...
{
    buf_T * buf = curbuf;
    if (curbuf -> b_p_bl) apply_autocmds (EVENT_BUFDELETE, ((void *) 0), ((void *) 0), 0, curbuf);
    ...
    if (setfname (curbuf, fname, sfname, 0) == 1) curbuf -> b_flags |= 0x08;
    apply_autocmds (EVENT_BUFNEW, ((void *) 0), ((void *) 0), 0, curbuf);
    if (curbuf -> b_p_bl) apply_autocmds (EVENT_BUFADD, ((void *) 0), ((void *) 0), 0, curbuf);
    if (aborting ()) return 0;
    if (*curbuf->b_p_ft == '\000') {
        if (au_has_group ((char_u *) "filetypedetect")) (void) do_doautocmd ((char_u *) "filetypedetect BufRead", 0);
        do_modelines (0);
    }
    return 1;
}
...
void msg_add_fname (buf, fname)
    ...
    char_u * fname;
{
    if (fname == ((void *) 0)) fname = (char_u *) "-stdin-";
    home_replace (buf, fname, IObuff + 1, (1024 + 1) - 4, 1);
    IObuff [0] = '"';
    ((__builtin_object_size ((char *) (IObuff), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (IObuff), (char *) ("\" "), __builtin_object_size ((char
      *) (IObuff), 1 > 1)) : __inline_strcat_chk ((char *) (IObuff), (char *) ("\" ")));
}
...
{
    if (eol_type == 1) {
        ((__builtin_object_size ((char *) (IObuff), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (IObuff), (char *) (shortmess ('x') ? ((char *) (
          "[dos]")) : ((char *) ("[dos format]"))), __builtin_object_size ((char *) (IObuff), 1 > 1)) : __inline_strcat_chk ((char *) (IObuff), (char *) (
          shortmess ('x') ? ((char *) ("[dos]")) : ((char *) ("[dos format]")))));
        return 1;
    }
    if (eol_type == 2) {
        ((__builtin_object_size ((char *) (IObuff), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (IObuff), (char *) (shortmess ('x') ? ((char *) (
          "[mac]")) : ((char *) ("[mac format]"))), __builtin_object_size ((char *) (IObuff), 1 > 1)) : __inline_strcat_chk ((char *) (IObuff), (char *) (
          shortmess ('x') ? ((char *) ("[mac]")) : ((char *) ("[mac format]")))));
        return 1;
    }
    return 0;
}
    ...
    off_t nchars;
{
    ...
    if (insert_space) *p++ = ' ';
    if (shortmess ('l')) __builtin___sprintf_chk ((char *) p, 0, __builtin_object_size ((char *) p, 1 > 1), "%ldL, %ldC", lnum, (long) nchars);
    else {
        if (lnum == 1) ((__builtin_object_size ((char *) (p), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (p), (char *) (((char *) ("1 line, "))),
          __builtin_object_size ((char *) (p), 1 > 1)) : __inline_strcpy_chk ((char *) (p), (char *) (((char *) ("1 line, ")))));
        else __builtin___sprintf_chk ((char *) p, 0, __builtin_object_size ((char *) p, 1 > 1), ((char *) ("%ld lines, ")), lnum);
        p += strlen ((char *) (p));
        if (nchars == 1) ((__builtin_object_size ((char *) (p), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (p), (char *) (((char *) ("1 character")
          )), __builtin_object_size ((char *) (p), 1 > 1)) : __inline_strcpy_chk ((char *) (p), (char *) (((char *) ("1 character")))));
        else __builtin___sprintf_chk ((char *) p, 0, __builtin_object_size ((char *) p, 1 > 1), ((char *) ("%ld characters")), (long) nchars);
    }
}

static void msg_add_eol () {
    ((__builtin_object_size ((char *) (IObuff), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (IObuff), (char *) (shortmess ('i') ? ((char *) (
      "[noeol]")) : ((char *) ("[Incomplete last line]"))), __builtin_object_size ((char *) (IObuff), 1 > 1)) : __inline_strcat_chk ((char *) (IObuff), (char *
      ) (shortmess ('i') ? ((char *) ("[noeol]")) : ((char *) ("[Incomplete last line]")))));
}
...
static int check_mtime (buf, st)
    ...
    struct stat * st;
{
    if (buf -> b_mtime_read != 0 && time_differs ((long) st -> st_mtimespec.tv_sec, buf -> b_mtime_read)) {
        msg_scroll = 1;
        msg_silent = 0;
        msg_attr ((char_u *) (((char *) ("WARNING: The file has been changed since reading it!!!"))), (highlight_attr [(int) (HLF_E)]));
        if (ask_yesno ((char_u *) ((char *) ("Do you really want to write to it")), 1) == 'n') return 0;
        msg_scroll = 0;
    }
    return 1;
}
    ...
    struct bw_info * ip;
{
    ...
    if (flags & 0x1000) crypt_encode (buf, len, buf);
    while (len > 0) {
    ...
    char_u * full_path;
{
    char_u dirname [1024];
    char_u * p = full_path;
    if (mch_dirname (dirname, 1024) == 1) {
        p = shorten_fname (full_path, dirname);
        if (p == ((void *) 0) || *p == '\000') p = full_path;
    }
    return p;
}
    ...
    char_u * dir_name;
{
    int len;
    char_u * p;
    if (full_path == ((void *) 0)) return ((void *) 0);
    len = (int) strlen ((char *) (dir_name));
    if (strncasecmp ((char *) (((dir_name))), (char *) (((full_path))), (size_t) (((len)))) == 0) {
        p = full_path + len;
        {
            if (vim_ispathsep (*p)) ++p;
            else p = ((void *) 0);
        }
    }
    else p = ((void *) 0);
    ...
    int force;
{
    ...
    for (buf = firstbuf; buf != ((void *) 0); buf = buf -> b_next) {
        if (buf -> b_fname != ((void *) 0) && !bt_nofile(buf) && !path_with_url(buf->b_fname) && (force || buf -> b_sfname == ((void *) 0) || mch_isFullName (
          buf -> b_sfname))) {
            ...
            if (p == ((void *) 0) || buf -> b_fname == ((void *) 0)) buf -> b_fname = buf -> b_ffname;
        }
    ...
    int prepend_dot;
{
    ...
    if (fname == ((void *) 0) || *fname == '\000') {
        retval = alloc ((unsigned) (1024 + extlen + 3));
        if (retval == ((void *) 0)) return ((void *) 0);
        if (mch_dirname (retval, 1024) == 0 || (fnamelen = (int) strlen ((char *) (retval))) == 0) {
            vim_free (retval);
            return ((void *) 0);
        }
        ...
        if (retval == ((void *) 0)) return ((void *) 0);
        ((__builtin_object_size ((char *) (retval), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (retval), (char *) (fname), __builtin_object_size ((
          char *) (retval), 1 > 1)) : __inline_strcpy_chk ((char *) (retval), (char *) (fname)));
    }
    for (ptr = retval + fnamelen; ptr > retval; --ptr) {
        if (*ext == '.' && shortname) if (*ptr == '.') *ptr = '_';
        if (vim_ispathsep (*ptr)) {
            ++ptr;
            break;
        }
    }
    if (strlen ((char *) (ptr)) > (unsigned) (255 - 5)) ptr [(255 - 5)] = '\0';
    s = ptr + strlen ((char *) (ptr));
    if (shortname) {
        if (fname == ((void *) 0) || *fname == '\000' || vim_ispathsep (fname [strlen ((char *) (fname)) - 1])) {
            if (*ext == '.') *s++ = '_';
        }
        else if (*ext == '.') {
        ...
        else if ((e = vim_strchr (ptr, '.')) == ((void *) 0)) *s++ = '.';
        else if ((int) strlen ((char *) (e)) + extlen > 4) s = e + 4 - extlen;
    }
    ((__builtin_object_size ((char *) (s), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (s), (char *) (ext), __builtin_object_size ((char *) (s), 1 >
      1)) : __inline_strcpy_chk ((char *) (s), (char *) (ext)));
    if (prepend_dot && !shortname && *(e=gettail(retval)) != '.') {
        ((__builtin_object_size ((char *) ((e + 1)), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) ((e + 1)), (char *) ((e)), strlen ((char *) (e)) +
          1, __builtin_object_size ((char *) ((e + 1)), 0)) : __inline_memmove_chk ((char *) ((e + 1)), (char *) ((e)), strlen ((char *) (e)) + 1));
        * e = '.';
    }
    if (fname != ((void *) 0) && strcmp ((char *) (fname), (char *) (retval)) == 0) {
        while (--s >= ptr) {
            if (*s != '_') {
                * s = '_';
                break;
    ...
    char_u * to;
{
    ...
    vim_acl_T acl;
    int use_tmp_file = 0;
    if (strcasecmp ((char *) (((from))), (char *) (((to)))) == 0) {
        if (strcmp ((char *) (gettail (from)), (char *) (gettail (to))) != 0) use_tmp_file = 1;
        else return 0;
    }
    if (stat (((char *) from), (&st)) < 0) return -1;
    {
        struct stat st_to;
        if (stat (((char *) to), (&st_to)) >= 0 && st.st_dev == st_to.st_dev && st.st_ino == st_to.st_ino) use_tmp_file = 1;
    }
    if (use_tmp_file) {
        char tempname [1024 + 1];
        if (strlen ((char *) (from)) >= 1024 - 5) return -1;
        ((__builtin_object_size ((char *) (tempname), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (tempname), (char *) (from), __builtin_object_size
          ((char *) (tempname), 1 > 1)) : __inline_strcpy_chk ((char *) (tempname), (char *) (from)));
        for (n = 123; n < 99999; ++n) {
            __builtin___sprintf_chk ((char *) gettail ((char_u *) tempname), 0, __builtin_object_size ((char *) gettail ((char_u *) tempname), 1 > 1), "%d", n)
              ;
            if (stat ((tempname), (&st)) < 0) {
                if (rename ((char *) from, tempname) == 0) {
                    if (rename (tempname, (char *) to) == 0) return 0;
    ...
    unlink ((char *) (to));
    if (rename ((char *) from, (char *) to) == 0) return 0;
    perm = mch_getperm (from);
    acl = mch_get_acl (from);
    fd_in = open (((char *) from), (0x0000 | 0), (0));
    if (fd_in == -1) {
        mch_free_acl (acl);
        return -1;
    }
    fd_out = open (((char *) to), (0x0200 | 0x0800 | 0x0001 | 0 | 0x0100), ((int) perm));
    if (fd_out == -1) {
        close (fd_in);
        mch_free_acl (acl);
        return -1;
    }
    buffer = (char *) alloc (8192);
    if (buffer == ((void *) 0)) {
        close (fd_out);
        close (fd_in);
        mch_free_acl (acl);
    ...
    while ((n = read ((fd_in), (char *) (buffer), (size_t) (8192))) > 0) if (write ((fd_out), (char *) (buffer), (size_t) (n)) != n) {
        errmsg = ((char *) ("E208: Error writing to \"%s\""));
        break;
    }
    vim_free (buffer);
    close (fd_in);
    if (close (fd_out) < 0) errmsg = ((char *) ("E209: Error closing \"%s\""));
    if (n < 0) {
    ...
    int focus;
{
    ...
    int didit = 0;
    int n;
    if (no_check_timestamps > 0) return 0;
    if (focus && did_check_timestamps) {
        need_check_timestamps = 1;
        return 0;
    }
    if (!stuff_empty() || global_busy || !typebuf_typed() || autocmd_busy || curbuf_lock > 0 || allbuf_lock > 0) need_check_timestamps = 1;
    else {
        ++no_wait_return;
    ...
    buf_T * tobuf;
{
    ...
    for (lnum = 1; lnum <= frombuf -> b_ml.ml_line_count; ++lnum) {
        ...
        if (p == ((void *) 0) || ml_append (lnum - 1, p, 0, 0) == 0) {
            vim_free (p);
            retval = 0;
            break;
        }
    ...
    if (retval != 0) {
        curbuf = frombuf;
        for (lnum = curbuf -> b_ml.ml_line_count; lnum > 0; --lnum) if (ml_delete (lnum, 0) == 0) {
            retval = 0;
            break;
        }
    }
...
int buf_check_timestamp (buf, focus)
    buf_T * buf;
    int focus __attribute__ ((unused));
{
    struct stat st;
    int stat_res;
    int retval = 0;
    ...
    char_u * s;
    char * reason;
    if (buf -> b_ffname == ((void *) 0) || buf -> b_ml.ml_mfp == ((void *) 0) || *buf->b_p_bt != '\000' || buf -> b_saving || busy || isNetbeansBuffer (buf))
      return 0;
    if (!(buf->b_flags&0x08) && buf -> b_mtime != 0 && ((stat_res = stat (((char *) buf -> b_ffname), (&st))) < 0 || time_differs ((long) st.st_mtimespec.
      tv_sec, buf -> b_mtime) || (int) st.st_mode != buf -> b_orig_mode)) {
        retval = 1;
        if (stat_res < 0) {
            buf -> b_mtime = 0;
            buf -> b_orig_size = 0;
        ...
        else buf_store_time (buf, &st, buf -> b_ffname);
        if (mch_isdir (buf -> b_fname));
        else if ((buf -> b_p_ar >= 0 ? buf -> b_p_ar : p_ar) && !bufIsChanged(buf) && stat_res >= 0) reload = 1;
        else {
            if (stat_res < 0) reason = "deleted";
            else if (bufIsChanged (buf)) reason = "conflict";
            else if (orig_size != buf -> b_orig_size || buf_contents_changed (buf)) reason = "changed";
            else if (orig_mode != buf -> b_orig_mode) reason = "mode";
            else reason = "time";
            busy = 1;
            set_vim_var_string (37, (char_u *) reason, -1);
            set_vim_var_string (38, (char_u *) "", -1);
            ++allbuf_lock;
            n = apply_autocmds (EVENT_FILECHANGEDSHELL, buf -> b_fname, buf -> b_fname, 0, buf);
            --allbuf_lock;
            busy = 0;
            if (n) {
                if (!buf_valid(buf)) emsg ((char_u *) (((char *) ("E246: FileChangedShell autocommand deleted buffer"))));
                s = get_vim_var_str (38);
                if (strcmp ((char *) (s), (char *) ("reload")) == 0 && *reason != 'd') reload = 1;
                else if (strcmp ((char *) (s), (char *) ("ask")) == 0) n = 0;
                else return 2;
            }
            if (!n) {
                if (*reason == 'd') mesg = ((char *) ("E211: File \"%s\" no longer available"));
                else {
                    helpmesg = 1;
                    can_reload = 1;
                    if (reason [2] == 'n') {
                    ...
                    else buf -> b_mtime_read = buf -> b_mtime;
                }
            }
        }
    }
    else if ((buf -> b_flags & 0x10) && !(buf->b_flags&0x20) && vim_fexists (buf -> b_ffname)) {
        retval = 1;
    ...
    if (mesg != ((void *) 0)) {
        ...
        if (path != ((void *) 0)) {
            if (!helpmesg) mesg2 = "";
            tbuf = alloc ((unsigned) (strlen ((char *) (path)) + strlen ((char *) (mesg)) + strlen ((char *) (mesg2)) + 2));
            __builtin___sprintf_chk ((char *) tbuf, 0, __builtin_object_size ((char *) tbuf, 1 > 1), mesg, path);
            set_vim_var_string (4, tbuf, -1);
            if (can_reload) {
                if (*mesg2 != '\000') {
                    ((__builtin_object_size ((char *) (tbuf), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (tbuf), (char *) ("\n"),
                      __builtin_object_size ((char *) (tbuf), 1 > 1)) : __inline_strcat_chk ((char *) (tbuf), (char *) ("\n")));
                    ((__builtin_object_size ((char *) (tbuf), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (tbuf), (char *) (mesg2),
                      __builtin_object_size ((char *) (tbuf), 1 > 1)) : __inline_strcat_chk ((char *) (tbuf), (char *) (mesg2)));
                }
                if (do_dialog (2, (char_u *) ((char *) ("Warning")), tbuf, (char_u *) ((char *) ("&OK\n&Load File")), 1, ((void *) 0)) == 2) reload = 1;
            }
                ...
                if (!autocmd_busy) {
                    ...
                    if (*mesg2 != '\000') msg_puts_attr ((char_u *) mesg2, highlight_attr [(int) (HLF_W)] + 0x1000);
                    msg_clr_eos ();
                    (void) msg_end ();
                    if (emsg_silent == 0) {
                        out_flush ();
    ...
    if (reload) buf_reload (buf, orig_mode);
    if (buf_valid (buf) && retval != 0) (void) apply_autocmds (EVENT_FILECHANGEDSHELLPOST, buf -> b_fname, buf -> b_fname, 0, buf);
    return retval;
}
    ...
    int orig_mode;
{
    ...
    if (prep_exarg (&ea, buf) == 1) {
        ...
        if ((curbuf -> b_ml.ml_line_count == 1 && *ml_get((linenr_T)1) == '\000') || saved == 0) savebuf = ((void *) 0);
        else {
            savebuf = buflist_new (((void *) 0), ((void *) 0), (linenr_T) 1, 4);
            ...
            if (savebuf == ((void *) 0) || saved == 0 || buf != curbuf || move_lines (buf, savebuf) == 0) {
                emsg2 ((char_u *) (((char *) ("E462: Could not prepare for reloading \"%s\""))), (char_u *) (buf -> b_fname));
                saved = 0;
            }
        }
        if (saved == 1) {
            curbuf -> b_flags |= 0x02;
            keep_filetype = 1;
            if (readfile (buf -> b_ffname, buf -> b_fname, (linenr_T) 0, (linenr_T) 0, (linenr_T) (0x7fffffffL), &ea, flags) == 0) {
                if (!aborting()) emsg2 ((char_u *) (((char *) ("E321: Could not reload \"%s\""))), (char_u *) (buf -> b_fname));
                if (savebuf != ((void *) 0) && buf_valid (savebuf) && buf == curbuf) {
                    while (!(curbuf->b_ml.ml_line_count==1&&*ml_get((linenr_T)1) == '\000')) if (ml_delete (buf -> b_ml.ml_line_count, 0) == 0) break;
        ...
        vim_free (ea.cmd);
        if (savebuf != ((void *) 0) && buf_valid (savebuf)) wipe_buffer (savebuf, 0);
        diff_invalidate (curbuf);
        if (old_topline > curbuf -> b_ml.ml_line_count) curwin -> w_topline = curbuf -> b_ml.ml_line_count;
        else curwin -> w_topline = old_topline;
        ...
        keep_filetype = 0;
        {
            ...
            for ((tp) = first_tabpage; (tp) != ((void *) 0); (tp) = (tp) -> tp_next) for ((wp) = ((tp) == curtab) ? firstwin : (tp) -> tp_firstwin; (wp); (wp) =
              (wp) -> w_next) if (wp -> w_buffer == curwin -> w_buffer && !foldmethodIsManual(wp)) foldUpdateAll (wp);
        }
        if (orig_mode == curbuf -> b_orig_mode) curbuf -> b_p_ro |= old_ro;
    }
    aucmd_restbuf (&aco);
}
...
void buf_store_time (buf, st, fname)
    buf_T * buf;
    struct stat * st;
    char_u * fname __attribute__ ((unused));
{
    buf -> b_mtime = (long) st -> st_mtimespec.tv_sec;
...
void write_lnum_adjust (offset)
    linenr_T offset;
{
    if (write_no_eol_lnum != 0) write_no_eol_lnum += offset;
}
...
void vim_deltempdir () {
    ...
    if (vim_tempdir != ((void *) 0)) {
        __builtin___sprintf_chk ((char *) NameBuff, 0, __builtin_object_size ((char *) NameBuff, 1 > 1), "%s*", vim_tempdir);
        if (gen_expand_wildcards (1, &NameBuff, &file_count, &files, 0x01 | 0x02 | 0x20) == 1) {
            for (i = 0; i < file_count; ++i) unlink ((char *) (files [i]));
            FreeWild (file_count, files);
        }
        gettail (NameBuff) [-1] = '\000';
...
{
    char_u * buf;
    buf = alloc ((unsigned) 1024 + 2);
    if (buf != ((void *) 0)) {
        if (vim_FullName (tempdir, buf, 1024, 0) == 0) ((__builtin_object_size ((char *) (buf), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (buf), (
          char *) (tempdir), __builtin_object_size ((char *) (buf), 1 > 1)) : __inline_strcpy_chk ((char *) (buf), (char *) (tempdir)));
        add_pathsep (buf);
...
char_u * vim_tempname (extra_char)
    int extra_char __attribute__ ((unused));
{
    char_u itmp [256];
    ...
    if (vim_tempdir == ((void *) 0)) {
        for (i = 0; i < (int) (sizeof (tempdirs) / sizeof (char *)); ++i) {
            expand_env ((char_u *) tempdirs [i], itmp, 256 - 20);
            if (mch_isdir (itmp)) {
                add_pathsep (itmp);
                ((__builtin_object_size ((char *) (itmp), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (itmp), (char *) ("vXXXXXX"),
                  __builtin_object_size ((char *) (itmp), 1 > 1)) : __inline_strcat_chk ((char *) (itmp), (char *) ("vXXXXXX")));
                if (mkdtemp ((char *) itmp) != ((void *) 0)) vim_settempdir (itmp);
                if (vim_tempdir != ((void *) 0)) break;
            }
        }
    }
    if (vim_tempdir != ((void *) 0)) {
        __builtin___sprintf_chk ((char *) itmp, 0, __builtin_object_size ((char *) itmp, 1 > 1), "%s%ld", vim_tempdir, temp_count ++);
...
static garray_T augroups =
{0, 0, sizeof (char_u *), 10, ((void *) 0)};
    ...
    event_T event;
{
    AutoCmd * ac;
    if (got_int) return;
    if (ap -> pat == ((void *) 0)) return;
    msg_putchar ('\n');
    if (got_int) return;
    if (event != last_event || ap -> group != last_group) {
        if (ap -> group != -1) {
            if ((((char_u * *) augroups.ga_data) [ap -> group]) == ((void *) 0)) msg_puts_attr ((char_u *) ((char *) ("--Deleted--")), highlight_attr [(int) (
              HLF_E)]);
            else msg_puts_attr ((((char_u * *) augroups.ga_data) [ap -> group]), highlight_attr [(int) (HLF_T)]);
            msg_puts ((char_u *) "  ");
        }
    ...
    for (ac = ap -> cmds; ac != ((void *) 0); ac = ac -> next) {
        if (ac -> cmd != ((void *) 0)) {
            if (msg_col >= 14) msg_putchar ('\n');
            msg_col = 14;
            if (got_int) return;
            msg_outtrans (ac -> cmd);
            if (p_verbose > 0) last_set_msg (ac -> scriptID);
            if (got_int) return;
            if (ac -> next != ((void *) 0)) {
                msg_putchar ('\n');
                if (got_int) return;
...
static void au_cleanup () {
    ...
    AutoCmd * ac, * * prev_ac;
    event_T event;
    if (autocmd_busy || !au_need_clean) return;
    ...
    buf_T * buf;
{
    ...
    event_T event;
    AutoPatCmd * apc;
    for (apc = active_apc_list; apc; apc = apc -> next) if (buf -> b_fnum == apc -> arg_bufnr) apc -> arg_bufnr = 0;
    for (event = (event_T) 0; (int) event < (int) NUM_EVENTS; event = (event_T) ((int) event + 1)) for (ap = first_autopat [(int) event]; ap != ((void *) 0); ap
      = ap -> next) if (ap -> buflocal_nr == buf -> b_fnum) {
        au_remove_pat (ap);
        if (p_verbose >= 6) {
            verbose_enter ();
            smsg ((char_u *) ((char *) ("auto-removing autocommand: %s <buffer=%d>")), event_nr2name (event), buf -> b_fnum);
            verbose_leave ();
        }
    }
    au_cleanup ();
    ...
    char_u * name;
{
    ...
    if (i == -2) {
        for (i = 0; i < augroups.ga_len; ++i) if ((((char_u * *) augroups.ga_data) [i]) == ((void *) 0)) break;
        if (i == augroups.ga_len && ga_grow (&augroups, 1) == 0) return -2;
        (((char_u * *) augroups.ga_data) [i]) = vim_strsave (name);
        if ((((char_u * *) augroups.ga_data) [i]) == ((void *) 0)) return -2;
        if (i == augroups.ga_len) ++augroups.ga_len;
    }
    return i;
}
    ...
    char_u * name;
{
    int i;
    i = au_find_group (name);
    if (i == -2) emsg2 ((char_u *) (((char *) ("E367: No such group: \"%s\""))), (char_u *) (name));
    else {
...
{
    int i;
    for (i = 0; i < augroups.ga_len; ++i) if ((((char_u * *) augroups.ga_data) [i]) != ((void *) 0) && strcmp ((char *) ((((char_u * *) augroups.ga_data) [i]))
      , (char *) (name)) == 0) return i;
    return -2;
}
    ...
    int del_group;
{
    int i;
    if (del_group) {
        if (*arg == '\000') emsg ((char_u *) (((char *) (e_argreq))));
        else au_del_group (arg);
    }
    else if (strcasecmp ((char *) (arg), (char *) ("end")) == 0) current_augroup = -1;
    else if (*arg) {
        i = au_new_group (arg);
        if (i != -2) current_augroup = i;
    }
    else {
        msg_start ();
    ...
    char_u * * end;
{
    ...
    for (i = 0; event_names [i].name != ((void *) 0); ++i) {
        ...
        if (len == p - start && strncasecmp ((char *) (event_names [i].name), (char *) (start), (size_t) (len)) == 0) break;
    }
    if (*p == ',') ++p;
    * end = p;
    if (event_names [i].name == ((void *) 0)) return NUM_EVENTS;
...
{
    int i;
    for (i = 0; event_names [i].name != ((void *) 0); ++i) if (event_names [i].event == event) return (char_u *) event_names [i].name;
    ...
    int have_group;
{
    ...
    if (*arg == '*') {
        if (arg [1] && !((arg[1])==' '||(arg[1])=='\t')) {
            emsg2 ((char_u *) (((char *) ("E215: Illegal character after *: %s"))), (char_u *) (arg));
            return ((void *) 0);
        }
        pat = arg + 1;
    }
    else {
        for (pat = arg; *pat && !((*pat) == ' ' || (*pat) == '\t'); pat = p) {
            if ((int) event_name2nr (pat, &p) >= (int) NUM_EVENTS) {
                if (have_group) emsg2 ((char_u *) (((char *) ("E216: No such event: %s"))), (char_u *) (pat));
                else emsg2 ((char_u *) (((char *) ("E216: No such group or event: %s"))), (char_u *) (pat));
                return ((void *) 0);
            }
        }
    }
    ...
    event_T event;
{
    char_u * p = p_ei;
    while (*p != '\000') {
        if (strncasecmp ((char *) (p), (char *) ("all"), (size_t) (3)) == 0 && (p [3] == '\000' || p [3] == ',')) return 1;
        if (event_name2nr (p, &p) == event) return 1;
    }
    return 0;
}
...
int check_ei () {
    ...
    while (*p) {
        ...
        else if (event_name2nr (p, &p) == NUM_EVENTS) return 0;
    }
    return 1;
}
    ...
    char * what;
{
    ...
    if (save_ei != ((void *) 0)) {
        ...
        if (new_ei != ((void *) 0)) {
            if (*what == ',' && *p_ei == '\000') ((__builtin_object_size ((char *) (new_ei), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (new_ei), (
              char *) (what + 1), __builtin_object_size ((char *) (new_ei), 1 > 1)) : __inline_strcpy_chk ((char *) (new_ei), (char *) (what + 1)));
            else ((__builtin_object_size ((char *) (new_ei), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (new_ei), (char *) (what),
              __builtin_object_size ((char *) (new_ei), 1 > 1)) : __inline_strcat_chk ((char *) (new_ei), (char *) (what)));
...
{
    if (old_ei != ((void *) 0)) {
        set_string_option_direct ((char_u *) "ei", -1, old_ei, 1, -6);
        vim_free (old_ei);
    }
}
    ...
    int forceit;
{
    ...
    pat = skipwhite (pat);
    cmd = pat;
    while (*cmd && (!((*cmd) == ' ' || (*cmd) == '\t') || cmd [-1] == '\\')) cmd ++;
    if (*cmd) *cmd++ = '\000';
    if (vim_strchr (pat, '$') != ((void *) 0) || vim_strchr (pat, '~') != ((void *) 0)) {
        envpat = expand_env_save (pat);
        if (envpat != ((void *) 0)) pat = envpat;
    }
    cmd = skipwhite (cmd);
    if (*cmd != '\000' && strncmp ((char *) (cmd), (char *) ("nested"), (size_t) (6)) == 0 && ((cmd [6]) == ' ' || (cmd [6]) == '\t')) {
        nested = 1;
        cmd = skipwhite (cmd + 6);
    }
    if (*cmd != '\000') {
    ...
    last_group = -2;
    if (*arg == '*' || *arg == '\000') {
        for (event = (event_T) 0; (int) event < (int) NUM_EVENTS; event = (event_T) ((int) event + 1)) if (do_autocmd_event (event, pat, nested, cmd, forceit,
          group) == 0) break;
    }
    else {
        while (*arg && !((*arg) == ' ' || (*arg) == '\t')) if (do_autocmd_event (event_name2nr (arg, &arg), pat, nested, cmd, forceit, group) == 0) break;
    }
    if (need_free) vim_free (cmd);
    vim_free (envpat);
}
    ...
    char_u * * argp;
{
    ...
    if (p > arg) {
        ...
        if (group_name == ((void *) 0)) return -2;
        group = au_find_group (group_name);
        if (group == -2) group = -3;
        else *argp = skipwhite (p);
        vim_free (group_name);
    }
    return group;
}
    ...
    int group;
{
    ...
    if (group == -3) findgroup = current_augroup;
    else findgroup = group;
    allgroups = (group == -3 && !forceit && *cmd == '\000');
    if (*pat == '\000') {
        for (ap = first_autopat [(int) event]; ap != ((void *) 0); ap = ap -> next) {
            if (forceit) {
                if (ap -> group == findgroup) au_remove_pat (ap);
            }
            else if (group == -3 || ap -> group == group) show_autocmd (ap, event);
        }
    }
    for (; *pat; pat = (*endpat == ',' ? endpat + 1 : endpat)) {
        brace_level = 0;
        for (endpat = pat; *endpat && (*endpat != ',' || brace_level || endpat [-1] == '\\'); ++endpat) {
            if (*endpat == '{') brace_level ++;
        ...
        is_buflocal = 0;
        buflocal_nr = 0;
        if (patlen >= 7 && strncmp ((char *) (pat), (char *) ("<buffer"), (size_t) (7)) == 0 && pat [patlen - 1] == '>') {
            is_buflocal = 1;
            if (patlen == 8) buflocal_nr = curbuf -> b_fnum;
            else if (patlen > 9 && pat [7] == '=') {
                if (patlen == 13 && strncasecmp ((char *) (pat), (char *) ("<buffer=abuf>"), (size_t) (13))) buflocal_nr = autocmd_bufnr;
                else if (skipdigits (pat + 8) == pat + patlen - 1) buflocal_nr = atoi ((char *) pat + 8);
            }
        }
        if (is_buflocal) {
            __builtin___sprintf_chk ((char *) buflocal_pat, 0, __builtin_object_size ((char *) buflocal_pat, 1 > 1), "<buffer=%d>", buflocal_nr);
            pat = buflocal_pat;
        ...
        while ((ap = *prev_ap) != ((void *) 0)) {
            if (ap -> pat != ((void *) 0)) {
                if ((allgroups || ap -> group == findgroup) && ap -> patlen == patlen && strncmp ((char *) (pat), (char *) (ap -> pat), (size_t) (patlen)) == 0
                  ) {
                    if (forceit) {
                    ...
                    else if (*cmd == '\000') show_autocmd (ap, event);
                    else if (ap -> next == ((void *) 0)) break;
                }
            }
            prev_ap = &ap->next;
        }
        if (*cmd != '\000') {
            if (ap == ((void *) 0)) {
                if (is_buflocal && (buflocal_nr == 0 || buflist_findnr (buflocal_nr) == ((void *) 0))) {
                    emsgn ((char_u *) (((char *) ("E680: <buffer=%d>: invalid buffer number "))), (long) (buflocal_nr));
                    return 0;
                }
                ap = (AutoPat *) alloc ((unsigned) sizeof (AutoPat));
                if (ap == ((void *) 0)) return 0;
                ...
                if (is_buflocal) {
                    ...
                    if (reg_pat != ((void *) 0)) ap -> reg_prog = vim_regcomp (reg_pat, 1);
                    vim_free (reg_pat);
                ...
                * prev_ap = ap;
                ap -> next = ((void *) 0);
                if (group == -3) ap -> group = current_augroup;
                else ap -> group = group;
            }
            prev_ac = &(ap->cmds);
            while ((ac = *prev_ac) != ((void *) 0)) prev_ac = &ac->next;
            ac = (AutoCmd *) alloc ((unsigned) sizeof (AutoCmd));
            if (ac == ((void *) 0)) return 0;
    ...
    int do_msg;
{
    ...
    if (*arg == '*') {
        emsg ((char_u *) (((char *) ("E217: Can't execute autocommands for ALL events"))));
        return 0;
    }
    fname = find_end_event (arg, group != -3);
    if (fname == ((void *) 0)) return 0;
    fname = skipwhite (fname);
    while (*arg && !((*arg) == ' ' || (*arg) == '\t')) if (apply_autocmds_group (event_name2nr (arg, &arg), fname, ((void *) 0), 1, group, curbuf, ((void *) 0)
      )) nothing_done = 0;
    if (nothing_done && do_msg) msg ((char_u *) (((char *) ("No matching autocommands"))));
    return aborting () ? 0 : 1;
}
    ...
    exarg_T * eap;
{
    ...
    for (buf = firstbuf; buf != ((void *) 0); buf = buf -> b_next) {
        if (buf -> b_ml.ml_mfp != ((void *) 0)) {
            aucmd_prepbuf (&aco, buf);
            retval = do_doautocmd (eap -> arg, 0);
            do_modelines (curwin == aucmd_win ? 32 : 0);
            aucmd_restbuf (&aco);
            if (retval == 0 || !buf_valid(buf)) break;
        }
    }
    check_cursor ();
}
    ...
    buf_T * buf;
{
    ...
    else for (win = firstwin; win != ((void *) 0); win = win -> w_next) if (win -> w_buffer == buf) break;
    if (win == ((void *) 0) && aucmd_win == ((void *) 0)) {
        win_alloc_aucmd_win ();
        if (aucmd_win == ((void *) 0)) win = curwin;
    }
    if (win == ((void *) 0) && aucmd_win_used) win = curwin;
    ...
    if (aco -> use_aucmd_win) {
        ...
        if (curwin != aucmd_win) {
            ...
            for ((tp) = first_tabpage; (tp) != ((void *) 0); (tp) = (tp) -> tp_next) for ((wp) = ((tp) == curtab) ? firstwin : (tp) -> tp_firstwin; (wp); (wp) =
              (wp) -> w_next) {
                if (wp == aucmd_win) {
                    if (tp != curtab) goto_tabpage_tp (tp);
                    win_goto (aucmd_win);
                    break;
                }
            }
        ...
        (void) win_comp_pos ();
        unblock_autocmds ();
        if (win_valid (aco -> save_curwin)) curwin = aco -> save_curwin;
        else curwin = firstwin;
        ...
        if (win_valid (aco -> save_curwin)) {
            if (curwin == aco -> new_curwin && curbuf != aco -> new_curbuf && buf_valid (aco -> new_curbuf) && aco -> new_curbuf -> b_ml.ml_mfp != ((void *) 0)
              ) {
                --curbuf->b_nwindows;
...
int apply_autocmds (event, fname, fname_io, force, buf)
    ...
    buf_T * buf;
{
    return apply_autocmds_group (event, fname, fname_io, force, -3, buf, ((void *) 0));
}
...
{
    int did_cmd;
    if (should_abort (*retval)) return 0;
    did_cmd = apply_autocmds_group (event, fname, fname_io, force, -3, buf, ((void *) 0));
    if (did_cmd && aborting ()) *retval = 0;
    return did_cmd;
}

int has_cursorhold () {
    return (first_autopat [(int) (get_real_state () == (0x100 + 0x01) ? EVENT_CURSORHOLD : EVENT_CURSORHOLDI)] != ((void *) 0));
}

int trigger_cursorhold () {
    int state;
    if (!did_cursorhold && has_cursorhold () && !Recording && !ins_compl_active()) {
        state = get_real_state ();
        if (state == (0x100 + 0x01) || (state & 0x10) != 0) return 1;
    }
    return 0;
}
    ...
    exarg_T * eap;
{
    ...
    if (first_autopat [(int) event] == ((void *) 0) || autocmd_blocked > 0) goto BYPASS_AU;
    if (autocmd_busy && !(force||autocmd_nested)) goto BYPASS_AU;
    if (aborting ()) goto BYPASS_AU;
    if (filechangeshell_busy && (event == EVENT_FILECHANGEDSHELL || event == EVENT_FILECHANGEDSHELLPOST)) goto BYPASS_AU;
    if (event_ignored (event)) goto BYPASS_AU;
    if (nesting == 10) {
        emsg ((char_u *) (((char *) ("E218: autocommand nesting too deep"))));
        goto BYPASS_AU;
    }
    ...
    old_curbuf = curbuf;
    if (fname_io == ((void *) 0)) {
        if (fname != ((void *) 0) && *fname != '\000') autocmd_fname = fname;
        else if (buf != ((void *) 0)) autocmd_fname = buf -> b_ffname;
        else autocmd_fname = ((void *) 0);
    }
    else autocmd_fname = fname_io;
    if (autocmd_fname != ((void *) 0)) autocmd_fname = vim_strsave (autocmd_fname);
    autocmd_fname_full = 0;
    if (buf == ((void *) 0)) autocmd_bufnr = 0;
    else autocmd_bufnr = buf -> b_fnum;
    if (fname == ((void *) 0) || *fname == '\000') {
        if (buf == ((void *) 0)) fname = ((void *) 0);
        else {
            if (event == EVENT_SYNTAX) fname = buf -> b_p_syn;
            else if (event == EVENT_FILETYPE) fname = buf -> b_p_ft;
            else {
                if (buf -> b_sfname != ((void *) 0)) sfname = vim_strsave (buf -> b_sfname);
                fname = buf -> b_ffname;
            }
        }
        if (fname == ((void *) 0)) fname = (char_u *) "";
        fname = vim_strsave (fname);
    }
    else {
        sfname = vim_strsave (fname);
        if (event == EVENT_FILETYPE || event == EVENT_SYNTAX || event == EVENT_FUNCUNDEFINED || event == EVENT_REMOTEREPLY || event == EVENT_SPELLFILEMISSING ||
          event == EVENT_QUICKFIXCMDPRE || event == EVENT_QUICKFIXCMDPOST) fname = vim_strsave (fname);
        else fname = FullName_save (fname, 0);
    }
    if (fname == ((void *) 0)) {
        vim_free (sfname);
        retval = 0;
    ...
    ++nesting;
    if (event == EVENT_FILETYPE) did_filetype = 1;
    tail = gettail (fname);
    ...
    if (patcmd.curpat != ((void *) 0)) {
        ...
        save_cmdbang = get_vim_var_nr (32);
        if (eap != ((void *) 0)) {
            save_cmdarg = set_cmdarg (eap, ((void *) 0));
            set_vim_var_nr (32, (long) eap -> forceit);
        }
        else save_cmdarg = ((void *) 0);
        retval = 1;
        for (ap = patcmd.curpat; ap -> next != ((void *) 0); ap = ap -> next) ap -> last = 0;
        ap -> last = 1;
        check_lnums (1);
        do_cmdline (((void *) 0), getnextac, (void *) &patcmd, 0x02 | 0x01 | 0x04);
        if (eap != ((void *) 0)) {
    ...
    if (curbuf == old_curbuf && (event == EVENT_BUFREADPOST || event == EVENT_BUFWRITEPOST || event == EVENT_FILEAPPENDPOST || event == EVENT_VIMLEAVE || event
      == EVENT_VIMLEAVEPRE)) {
        if (curbuf -> b_changed != save_changed) need_maketitle = 1;
        curbuf -> b_changed = save_changed;
    }
    au_cleanup ();
    BYPASS_AU : if (event == EVENT_BUFWIPEOUT && buf != ((void *) 0)) aubuflocal_remove (buf);
    return retval;
}
...
void block_autocmds () {
    if (autocmd_blocked == 0) old_termresponse = get_vim_var_str (10);
    ++autocmd_blocked;
}

void unblock_autocmds () {
    --autocmd_blocked;
    if (autocmd_blocked == 0 && get_vim_var_str (10) != old_termresponse) apply_autocmds (EVENT_TERMRESPONSE, ((void *) 0), ((void *) 0), 0, curbuf);
}
    ...
    int stop_at_last;
{
    ...
    for (ap = apc -> curpat; ap != ((void *) 0) && !got_int; ap = ap -> next) {
        apc -> curpat = ((void *) 0);
        if (ap -> pat != ((void *) 0) && ap -> cmds != ((void *) 0) && (apc -> group == -3 || apc -> group == ap -> group)) {
            if (ap -> buflocal_nr == 0 ? (match_file_pat (((void *) 0), ap -> reg_prog, apc -> fname, apc -> sfname, apc -> tail, ap -> allow_dirs)) : ap ->
              buflocal_nr == apc -> arg_bufnr) {
                name = event_nr2name (apc -> event);
                s = ((char *) ("%s Auto commands for \"%s\""));
                sourcing_name = alloc ((unsigned) (strlen ((char *) (s)) + strlen ((char *) (name)) + ap -> patlen + 1));
                if (sourcing_name != ((void *) 0)) {
                    __builtin___sprintf_chk ((char *) sourcing_name, 0, __builtin_object_size ((char *) sourcing_name, 1 > 1), s, (char *) name, (char *) ap ->
                      pat);
                    if (p_verbose >= 8) {
                        verbose_enter ();
                        smsg ((char_u *) ((char *) ("Executing %s")), sourcing_name);
                        verbose_leave ();
                    }
                }
                apc -> curpat = ap;
                apc -> nextcmd = ap -> cmds;
                for (cp = ap -> cmds; cp -> next != ((void *) 0); cp = cp -> next) cp -> last = 0;
                cp -> last = 1;
            }
            line_breakcheck ();
            if (apc -> curpat != ((void *) 0)) break;
        }
        if (stop_at_last && ap -> last) break;
    }
}

static char_u * getnextac (c, cookie, indent)
    ...
    int indent __attribute__ ((unused));
{
    ...
    char_u * retval;
    AutoCmd * ac;
    if (acp -> curpat == ((void *) 0)) return ((void *) 0);
    for (;;) {
        while (acp -> nextcmd != ((void *) 0) && acp -> nextcmd -> cmd == ((void *) 0)) if (acp -> nextcmd -> last) acp -> nextcmd = ((void *) 0);
        else acp -> nextcmd = acp -> nextcmd -> next;
        if (acp -> nextcmd != ((void *) 0)) break;
        if (acp -> curpat -> last) acp -> curpat = ((void *) 0);
        else acp -> curpat = acp -> curpat -> next;
        if (acp -> curpat != ((void *) 0)) auto_next_pat (acp, 1);
        if (acp -> curpat == ((void *) 0)) return ((void *) 0);
    }
    ac = acp -> nextcmd;
    if (p_verbose >= 9) {
        verbose_enter_scroll ();
        smsg ((char_u *) ((char *) ("autocommand %s")), ac -> cmd);
        msg_puts ((char_u *) "\n");
    ...
    if (ac -> last) acp -> nextcmd = ((void *) 0);
    else acp -> nextcmd = ac -> next;
    return retval;
}
    ...
    buf_T * buf;
{
    ...
    if (fname == ((void *) 0)) return 0;
    for (ap = first_autopat [(int) event]; ap != ((void *) 0); ap = ap -> next) if (ap -> pat != ((void *) 0) && ap -> cmds != ((void *) 0) && (ap ->
      buflocal_nr == 0 ? match_file_pat (((void *) 0), ap -> reg_prog, fname, sfname, tail, ap -> allow_dirs) : buf != ((void *) 0) && ap -> buflocal_nr == buf
      -> b_fnum)) {
        retval = 1;
        break;
    }
    ...
    int idx;
{
    ...
    if (idx >= augroups.ga_len) return ((void *) 0);
    if ((((char_u * *) augroups.ga_data) [idx]) == ((void *) 0)) return (char_u *) "";
    ...
    int doautocmd;
{
    ...
    p = arg;
    group = au_get_grouparg (&arg);
    if (group == -2) return ((void *) 0);
    if (*arg == '\000' && group != -3 && !((arg[-1]) == ' ' || (arg [-1]) == '\t')) {
        arg = p;
        group = -3;
    }
    for (p = arg; *p != '\000' && !((*p) == ' ' || (*p) == '\t'); ++p) if (*p == ',') arg = p + 1;
    if (*p == '\000') {
        if (group == -3) include_groups = 1;
        xp -> xp_context = 10;
        xp -> xp_pattern = arg;
        return ((void *) 0);
    }
    arg = skipwhite (p);
    while (*arg && (!((*arg) == ' ' || (*arg) == '\t') || arg [-1] == '\\')) arg ++;
    if (*arg) return arg;
    if (doautocmd) xp -> xp_context = 2;
    ...
    int idx;
{
    if (idx < augroups.ga_len) {
        if (!include_groups || (((char_u * *) augroups.ga_data) [idx]) == ((void *) 0)) return (char_u *) "";
        return (((char_u * *) augroups.ga_data) [idx]);
    }
    ...
    char_u * arg;
{
    ...
    if (event == NUM_EVENTS) goto theend;
    ap = first_autopat [(int) event];
    if (ap == ((void *) 0)) goto theend;
    if (pattern != ((void *) 0) && strcasecmp ((char *) (pattern), (char *) ("<buffer>")) == 0) buflocal_buf = curbuf;
    for (; ap != ((void *) 0); ap = ap -> next) if (ap -> pat != ((void *) 0) && ap -> cmds != ((void *) 0) && (group == -3 || ap -> group == group) && (
      pattern == ((void *) 0) || (buflocal_buf == ((void *) 0) ? strcasecmp ((char *) (((ap -> pat))), (char *) (((pattern)))) == 0 : ap -> buflocal_nr ==
      buflocal_buf -> b_fnum))) {
        retval = 1;
        break;
    }
    theend : vim_free (arg_save);
    return retval;
}
    ...
    int allow_dirs;
{
    ...
    regmatch.rm_ic = 1;
    {
        if (prog != ((void *) 0)) regmatch.regprog = prog;
        else regmatch.regprog = vim_regcomp (pattern, 1);
    }
    if ((regmatch.regprog != ((void *) 0) && ((allow_dirs && (vim_regexec (&regmatch, fname, (colnr_T) 0) || (sfname != ((void *) 0) && vim_regexec (&regmatch,
      sfname, (colnr_T) 0)))) || (!allow_dirs && vim_regexec (&regmatch, tail, (colnr_T) 0))))) result = 1;
    if (prog == ((void *) 0)) vim_free (regmatch.regprog);
    return result;
}
...
char_u * file_pat_to_reg_pat (pat, pat_end, allow_dirs, no_bslash)
    ...
    char_u * pat_end;
    char * allow_dirs;
    int no_bslash __attribute__ ((unused));
{
    int size;
    char_u * endp;
    char_u * reg_pat;
    ...
    int nested = 0;
    int add_dollar = 1;
    if (allow_dirs != ((void *) 0)) *allow_dirs = 0;
    if (pat_end == ((void *) 0)) pat_end = pat + strlen ((char *) (pat));
    size = 2;
    ...
    if (reg_pat == ((void *) 0)) return ((void *) 0);
    i = 0;
    if (pat [0] == '*') while (pat [0] == '*' && pat < pat_end - 1) pat ++;
    else reg_pat [i ++] = '^';
    endp = pat_end - 1;
    if (*endp == '*') {
        while (endp - pat > 0 && *endp == '*') endp --;
        add_dollar = 0;
    }
    for (p = pat; *p && nested >= 0 && p <= endp; p ++) {
        ...
        case '\\' :
            if (p [1] == '\000') break;
            if (*++p == '?') reg_pat [i ++] = '?';
            else if (*p == ',' || *p == '%' || *p == '#' || *p == ' ') reg_pat [i ++] = *p;
            else {
                if (allow_dirs != ((void *) 0) && vim_ispathsep (*p)) *allow_dirs = 1;
                reg_pat [i ++] = '\\';
                reg_pat [i ++] = *p;
            }
            break;
    ...
    if (add_dollar) reg_pat [i ++] = '$';
    reg_pat [i] = '\000';
    if (nested != 0) {
        if (nested < 0) emsg ((char_u *) (((char *) ("E219: Missing {."))));
        else emsg ((char_u *) (((char *) ("E220: Missing }."))));
        vim_free (reg_pat);
        reg_pat = ((void *) 0);
    }
    return reg_pat;
