...
void hash_init (ht)
    hashtab_T * ht;
{
    ((__builtin_object_size ((ht), 0) != (size_t) - 1) ? __builtin___memset_chk ((ht), (0), (sizeof (hashtab_T)), __builtin_object_size ((ht), 0)) :
      __inline_memset_chk ((ht), (0), (sizeof (hashtab_T))));
...
void hash_clear (ht)
    hashtab_T * ht;
{
    if (ht -> ht_array != ht -> ht_smallarray) vim_free (ht -> ht_array);
}
    ...
    hash_T hash;
{
    ...
    hi = &ht->ht_array[idx];
    if (hi -> hi_key == ((void *) 0)) return hi;
    if (hi -> hi_key == &hash_removed) freeitem = hi;
    else if (hi -> hi_hash == hash && strcmp ((char *) (hi -> hi_key), (char *) (key)) == 0) return hi;
    else freeitem = ((void *) 0);
    for (perturb = hash;; perturb >>= 5) {
        idx = (int) ((idx << 2) + idx + perturb + 1);
        hi = &ht->ht_array[idx&ht->ht_mask];
        if (hi -> hi_key == ((void *) 0)) return freeitem == ((void *) 0) ? hi : freeitem;
        if (hi -> hi_hash == hash && hi -> hi_key != &hash_removed && strcmp ((char *) (hi -> hi_key), (char *) (key)) == 0) return hi;
        if (hi -> hi_key == &hash_removed && freeitem == ((void *) 0)) freeitem = hi;
    }
}
...
int hash_add_item (ht, hi, key, hash)
    ...
    hash_T hash;
{
    if (ht -> ht_error && hash_may_resize (ht, 0) == 0) return 0;
    ++ht->ht_used;
    if (hi -> hi_key == ((void *) 0)) ++ht->ht_filled;
    hi -> hi_key = key;
    hi -> hi_hash = hash;
    ...
    int minitems;
{
    ...
    long_u newmask;
    hash_T perturb;
    if (ht -> ht_locked > 0) return 1;
    if (minitems == 0) {
        if (ht -> ht_filled < 16 - 1 && ht -> ht_array == ht -> ht_smallarray) return 1;
        oldsize = ht -> ht_mask + 1;
        if (ht -> ht_filled * 3 < oldsize * 2 && ht -> ht_used > oldsize / 5) return 1;
        if (ht -> ht_used > 1000) minsize = ht -> ht_used * 2;
        else minsize = ht -> ht_used * 4;
    }
    else {
        if ((long_u) minitems < ht -> ht_used) minitems = (int) ht -> ht_used;
    ...
    if (newsize == 16) {
        ...
        if (ht -> ht_array == newarray) {
            ((__builtin_object_size ((char *) (temparray), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (temparray), (char *) (newarray), sizeof (
              temparray), __builtin_object_size ((char *) (temparray), 0)) : __inline_memmove_chk ((char *) (temparray), (char *) (newarray), sizeof (temparray
              )));
            oldarray = temparray;
        }
        else oldarray = ht -> ht_array;
    }
    else {
        newarray = (hashitem_T *) alloc ((unsigned) (sizeof (hashitem_T) * newsize));
        if (newarray == ((void *) 0)) {
            if (ht -> ht_filled < ht -> ht_mask) return 1;
            ht -> ht_error = 1;
            return 0;
        }
        oldarray = ht -> ht_array;
    }
    ((__builtin_object_size ((newarray), 0) != (size_t) - 1) ? __builtin___memset_chk ((newarray), (0), ((size_t) (sizeof (hashitem_T) * newsize)),
      __builtin_object_size ((newarray), 0)) : __inline_memset_chk ((newarray), (0), ((size_t) (sizeof (hashitem_T) * newsize))));
    newmask = newsize - 1;
    todo = (int) ht -> ht_used;
    for (olditem = oldarray; todo > 0; ++olditem) if (!((olditem)->hi_key==((void*)0)||(olditem)->hi_key==&hash_removed)) {
        newi = (int) (olditem -> hi_hash & newmask);
        newitem = &newarray[newi];
        if (newitem -> hi_key != ((void *) 0)) for (perturb = olditem -> hi_hash;; perturb >>= 5) {
            newi = (int) ((newi << 2) + newi + perturb + 1);
            newitem = &newarray[newi&newmask];
            if (newitem -> hi_key == ((void *) 0)) break;
        }
        * newitem = *olditem;
        --todo;
    }
    ...
    char_u * key;
{
    hash_T hash;
    char_u * p;
    if ((hash = *key) == 0) return (hash_T) 0;
    p = key + 1;
    while (*p != '\000') hash = hash * 101 + *p++;
    return hash;
}
