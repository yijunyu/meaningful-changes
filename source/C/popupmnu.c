    ...
    int selected;
{
    ...
    if (firstwin -> w_onebuf_opt.wo_pvw) top_clear = firstwin -> w_height;
    else top_clear = 0;
    if (lastwin -> w_onebuf_opt.wo_pvw) above_row -= lastwin -> w_height + lastwin -> w_status_height + 1;
    if (size < 10) pum_height = size;
    else pum_height = 10;
    if (p_ph > 0 && pum_height > p_ph) pum_height = p_ph;
    if (row + 2 >= above_row - pum_height && row > (above_row - top_clear) / 2) {
        if (curwin -> w_wrow - curwin -> w_cline_row >= 2) context_lines = 2;
        else context_lines = curwin -> w_wrow - curwin -> w_cline_row;
        ...
        if (curwin -> w_cline_row + curwin -> w_cline_height - curwin -> w_wrow >= 3) context_lines = 3;
        else context_lines = curwin -> w_cline_row + curwin -> w_cline_height - curwin -> w_wrow;
        pum_row = row + context_lines;
        if (size > above_row - pum_row) pum_height = above_row - pum_row;
        else pum_height = size;
        if (p_ph > 0 && pum_height > p_ph) pum_height = p_ph;
    }
    if (pum_height < 1 || (pum_height == 1 && size > 1)) return;
    if (firstwin -> w_onebuf_opt.wo_pvw && pum_row < firstwin -> w_height && pum_height > firstwin -> w_height + 4) {
    ...
    pum_array = array;
    pum_size = size;
    if (pum_set_selected (selected, redo_count) && ++redo_count <= 2) goto redo;
}
...
void pum_redraw () {
    ...
    for (i = 0; i < pum_height; ++i) {
        ...
        for (round = 1; round <= 3; ++round) {
            ...
            if (round > 1) n = pum_kind_width + 1;
            else n = 1;
            if (round == 3 || (round == 2 && pum_array [idx].pum_extra == ((void *) 0)) || (round == 1 && pum_array [idx].pum_kind == ((void *) 0) && pum_array
              [idx].pum_extra == ((void *) 0)) || pum_base_width + n >= pum_width) break;
            {
    ...
    int repeat;
{
    ...
    if (pum_selected >= 0 && pum_selected < pum_size) {
        if (pum_first > pum_selected - 4) {
            if (pum_first > pum_selected - 2) {
                ...
                if (pum_first < 0) pum_first = 0;
                else if (pum_first > pum_selected) pum_first = pum_selected;
            }
            else pum_first = pum_selected;
        }
        else if (pum_first < pum_selected - pum_height + 5) {
            if (pum_first < pum_selected - pum_height + 1 + 2) {
                pum_first += pum_height - 2;
                if (pum_first < pum_selected - pum_height + 1) pum_first = pum_selected - pum_height + 1;
            }
            else pum_first = pum_selected - pum_height + 1;
        }
        ...
        if (pum_array [pum_selected].pum_info != ((void *) 0) && Rows > 10 && repeat <= 1 && vim_strchr (p_cot, 'p') != ((void *) 0)) {
            ...
            if (curwin -> w_onebuf_opt.wo_pvw) {
                if (curbuf -> b_fname == ((void *) 0) && curbuf -> b_p_bt [0] == 'n' && curbuf -> b_p_bt [2] == 'f' && curbuf -> b_p_bh [0] == 'w') {
                    while (!(curbuf->b_ml.ml_line_count==1&&*ml_get((linenr_T)1) == '\000')) ml_delete ((linenr_T) 1, 0);
                }
                else {
                    ++no_u_sync;
                    ...
                    if (res == 1) {
                        ...
                        set_option_value ((char_u *) "diff", 0L, ((void *) 0), 4);
                    }
                }
                if (res == 1) {
                    char_u * p, * e;
                    ...
                    if (curwin != curwin_save && win_valid (curwin_save)) {
                        ...
                        update_screen (0);
                        pum_do_redraw = 0;
                        if (!resized && win_valid (curwin_save)) win_enter (curwin_save, 1);
                        pum_do_redraw = 1;
                        update_screen (0);
                        pum_do_redraw = 0;
                    }
    ...
    if (pum_first > pum_size - pum_height) pum_first = pum_size - pum_height;
    if (!resized) pum_redraw ();
    return resized;
}
