    ...
    regmmatch_T * regmatch;
{
    ...
    if (pat == ((void *) 0) || *pat == '\000') {
        if (pat_use == 2) i = last_idx;
        else i = pat_use;
        if (spats [i].pat == ((void *) 0)) {
            if (pat_use == 1) emsg ((char_u *) (((char *) (e_nopresub))));
            else emsg ((char_u *) (((char *) (e_noprevre))));
            rc_did_emsg = 1;
            return 0;
        }
        pat = spats [i].pat;
    ...
    else if (options & 0x20) add_to_history (1, pat, 1, '\000');
    mr_pattern = pat;
    if (!(options&0x400)) {
        if (pat_save == 0 || pat_save == 2) save_re_pat (0, pat, magic);
        if (pat_save == 1 || pat_save == 2) save_re_pat (1, pat, magic);
    }
...
void save_search_patterns () {
    if (save_level ++ == 0) {
        ...
        if (spats [0].pat != ((void *) 0)) saved_spats [0].pat = vim_strsave (spats [0].pat);
        saved_spats [1] = spats [1];
        if (spats [1].pat != ((void *) 0)) saved_spats [1].pat = vim_strsave (spats [1].pat);
...
{
    int ic = p_ic;
    if (ic && !no_smartcase && p_scs && !(ctrl_x_mode&&curbuf->b_p_inf)) ic = !pat_has_uppercase(pat);
    no_smartcase = 0;
    return ic;
}
    ...
    char_u * pat;
{
    ...
    while (*p != '\000') {
        if (*p == '\\') {
            if (p [1] == '_' && p [2] != '\000') p += 3;
            else if (p [1] == '%' && p [2] != '\000') p += 3;
            else if (p [1] != '\000') p += 2;
            else p += 1;
        }
        else if (isupper (*p)) return 1;
        else ++p;
    }
    return 0;
}
    ...
    int setlast;
{
    ...
    spats [idx].off.end = 0;
    spats [idx].off.off = 0;
    if (setlast) last_idx = idx;
    if (save_level) {
        vim_free (saved_spats [idx].pat);
        saved_spats [idx] = spats [0];
        if (spats [idx].pat == ((void *) 0)) saved_spats [idx].pat = ((void *) 0);
        else saved_spats [idx].pat = vim_strsave (spats [idx].pat);
        saved_last_idx = last_idx;
    }
    if (p_hls && idx == last_idx && !no_hlsearch) redraw_all_later (35);
}
...
int searchit (win, buf, pos, dir, pat, count, options, pat_use, stop_lnum, tm)
    ...
    int pat_use;
    linenr_T stop_lnum;
    proftime_T * tm __attribute__ ((unused));
{
    int found;
    linenr_T lnum;
    regmmatch_T regmatch;
    ...
    int break_loop = 0;
    if (search_regcomp (pat, 0, pat_use, (options & (0x20 + 0x400)), &regmatch) == 0) {
        if ((options & 0x0c) && !rc_did_emsg) emsg2 ((char_u *) (((char *) ("E383: Invalid search string: %s"))), (char_u *) (mr_pattern));
        return 0;
    }
    if ((options & 0x100) || pos -> col == (0x7fffffffL)) extra_col = 0;
    else extra_col = 1;
    called_emsg = 0;
    do {
        start_pos = *pos;
        ...
        if (dir == (-1) && start_pos.col == 0 && (options & 0x100) == 0) {
            lnum = pos -> lnum - 1;
            at_first_line = 0;
        }
        else lnum = pos -> lnum;
        for (loop = 0; loop <= 1; ++loop) {
            for (; lnum > 0 && lnum <= buf -> b_ml.ml_line_count; lnum += dir, at_first_line = 0) {
                if (stop_lnum != 0 && (dir == 1 ? lnum > stop_lnum : lnum < stop_lnum)) break;
                if (tm != ((void *) 0) && profile_passed_limit (tm)) break;
                nmatched = vim_regexec_multi (&regmatch, win, buf, lnum, (colnr_T) 0, tm);
                if (called_emsg) break;
                if (nmatched > 0) {
                    matchpos = regmatch.startpos [0];
                    endpos = regmatch.endpos [0];
                    submatch = first_submatch (&regmatch);
                    if (lnum + matchpos.lnum > buf -> b_ml.ml_line_count) ptr = (char_u *) "";
                    else ptr = ml_get_buf (buf, lnum + matchpos.lnum, 0);
                    if (dir == 1 && at_first_line) {
                        match_ok = 1;
                        while (matchpos.lnum == 0 && ((options & 0x40) ? (nmatched == 1 && (int) endpos.col - 1 < (int) start_pos.col + extra_col) : ((int)
                          matchpos.col - (ptr [matchpos.col] == '\000') < (int) start_pos.col + extra_col))) {
                            ...
                            if (ptr [matchcol] == '\000' || (nmatched = vim_regexec_multi (&regmatch, win, buf, lnum + matchpos.lnum, matchcol, tm)) == 0) {
                                match_ok = 0;
                                break;
                            }
                            matchpos = regmatch.startpos [0];
                    ...
                    if (dir == (-1)) {
                        match_ok = 0;
                        for (;;) {
                            if (loop || ((options & 0x40) ? (lnum + regmatch.endpos [0].lnum < start_pos.lnum || (lnum + regmatch.endpos [0].lnum == start_pos.
                              lnum && (int) regmatch.endpos [0].col - 1 + extra_col <= (int) start_pos.col)) : (lnum + regmatch.startpos [0].lnum < start_pos.
                              lnum || (lnum + regmatch.startpos [0].lnum == start_pos.lnum && (int) regmatch.startpos [0].col + extra_col <= (int) start_pos.
                              col)))) {
                                match_ok = 1;
                                matchpos = regmatch.startpos [0];
                                endpos = regmatch.endpos [0];
                            ...
                            if (vim_strchr (p_cpo, 'c') != ((void *) 0)) {
                                ...
                                if (matchpos.lnum > 0) break;
                                matchcol = matchpos.col;
                                if (ptr [matchcol] != '\000') {
                                    ++matchcol;
                                }
                            }
                            if (ptr [matchcol] == '\000' || (nmatched = vim_regexec_multi (&regmatch, win, buf, lnum + matchpos.lnum, matchcol, tm)) == 0) break
                              ;
                            ptr = ml_get_buf (buf, lnum + matchpos.lnum, 0);
                        }
                        if (!match_ok) continue;
                    }
                    if (options & 0x40 && !(options&0x80) && !(matchpos.lnum==endpos.lnum&&matchpos.col==endpos.col)) {
                ...
                line_breakcheck ();
                if (got_int) break;
                if ((options & 0x800) && ((lnum - pos -> lnum) & 0x3f) == 0 && char_avail ()) {
                    break_loop = 1;
                    break;
                }
                if (loop && lnum == start_pos.lnum) break;
            }
            at_first_line = 0;
            if (!p_ws || stop_lnum != 0 || got_int || called_emsg || break_loop || found || loop) break;
            if (dir == (-1)) lnum = buf -> b_ml.ml_line_count;
            else lnum = 1;
            if (!shortmess('s') && (options & 0x0c)) give_warning ((char_u *) ((char *) (dir == (-1) ? top_bot_msg : bot_top_msg)), 1);
        }
        if (got_int || called_emsg || break_loop) break;
    }
    while (--count > 0 && found);
    ...
    if (!found) {
        if (got_int) emsg ((char_u *) (((char *) (e_interr))));
            ...
            if (p_ws) emsg2 ((char_u *) (((char *) (e_patnotf2))), (char_u *) (mr_pattern));
            else if (lnum == 0) emsg2 ((char_u *) (((char *) ("E384: search hit TOP without match for: %s"))), (char_u *) (mr_pattern));
            else emsg2 ((char_u *) (((char *) ("E385: search hit BOTTOM without match for: %s"))), (char_u *) (mr_pattern));
        }
        return 0;
    }
...
static void set_vv_searchforward () {
    set_vim_var_nr (53, (long) (spats [0].off.dir == '/'));
}
    ...
    regmmatch_T * rp;
{
    ...
    for (submatch = 1;; ++submatch) {
        if (rp -> startpos [submatch].lnum >= 0) break;
        if (submatch == 9) {
            submatch = 0;
            break;
        }
    ...
    proftime_T * tm;
{
    ...
    old_off = spats [0].off;
    pos = curwin -> w_cursor;
    if (dirc == 0) dirc = spats [0].off.dir;
    else {
    ...
    if (dirc == '/') {
        if (hasFolding (pos.lnum, ((void *) 0), &pos.lnum)) pos.col = (0x7fffffffL) - 2;
    }
    else {
        if (hasFolding (pos.lnum, &pos.lnum, ((void *) 0))) pos.col = 0;
    }
    ...
    for (;;) {
        ...
        if (pat != ((void *) 0) && *pat != '\000') {
            ...
            spats [0].off.end = 0;
            spats [0].off.off = 0;
            if (*p == '+' || *p == '-' || ((*p) >= '0' && (*p) <= '9')) spats [0].off.line = 1;
            else if ((options & 0x10) && (*p == 'e' || *p == 's' || *p == 'b')) {
                if (*p == 'e') spats [0].off.end = 0x40;
                ++p;
            }
            if (((*p) >= '0' && (*p) <= '9') || *p == '+' || *p == '-') {
                if (((*p) >= '0' && (*p) <= '9') || ((*(p+1)) >= '0' && (*(p+1)) <= '9')) spats [0].off.off = atol ((char *) p);
                else if (*p == '-') spats [0].off.off = -1;
        ...
        if ((options & 0x02) && messaging () && !cmd_silent && msg_silent == 0) {
            char_u * msgbuf;
            char_u * trunc;
            if (*searchstr == '\000') p = spats [last_idx].pat;
            else p = searchstr;
            msgbuf = alloc ((unsigned) (strlen ((char *) (p)) + 40));
            if (msgbuf != ((void *) 0)) {
                msgbuf [0] = dirc;
                ((__builtin_object_size ((char *) (msgbuf + 1), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (msgbuf + 1), (char *) (p),
                  __builtin_object_size ((char *) (msgbuf + 1), 1 > 1)) : __inline_strcpy_chk ((char *) (msgbuf + 1), (char *) (p)));
                if (spats [0].off.line || spats [0].off.end || spats [0].off.off) {
                    p = msgbuf + strlen ((char *) (msgbuf));
                    *p++ = dirc;
                    if (spats [0].off.end) *p++ = 'e';
                    else if (!spats[0].off.line) *p++ = 's';
                    if (spats [0].off.off > 0 || spats [0].off.line) *p++ = '+';
                    if (spats [0].off.off != 0 || spats [0].off.line) __builtin___sprintf_chk ((char *) p, 0, __builtin_object_size ((char *) p, 1 > 1), "%ld",
                      spats [0].off.off);
                    else *p = '\000';
                }
                msg_start ();
        ...
        if (!spats[0].off.line && spats [0].off.off && pos.col < (0x7fffffffL) - 2) {
            if (spats [0].off.off > 0) {
                for (c = spats [0].off.off; c; --c) if (decl (&pos) == -1) break;
                if (c) {
                    pos.lnum = 0;
                    pos.col = (0x7fffffffL);
                }
                ...
                for (c = spats [0].off.off; c; ++c) if (incl (&pos) == -1) break;
                if (c) {
        ...
        c = searchit (curwin, curbuf, &pos, dirc == '/' ? 1 : (-1), searchstr, count, spats [0].off.end + (options & (0x400 + 0x800 + 0x20 + 0x0c + 0x100 + ((
          pat != ((void *) 0) && *pat == ';') ? 0 : 0x80))), 2, (linenr_T) 0, tm);
        if (dircp != ((void *) 0)) *dircp = dirc;
        if (c == 0) {
            retval = 0;
            goto end_do_search;
        }
        if (spats [0].off.end && oap != ((void *) 0)) oap -> inclusive = 1;
        retval = 1;
        if (!(options&0x80) || (pat != ((void *) 0) && *pat == ';')) {
            if (spats [0].off.line) {
                c = pos.lnum + spats [0].off.off;
                if (c < 1) pos.lnum = 1;
                else if (c > curbuf -> b_ml.ml_line_count) pos.lnum = curbuf -> b_ml.ml_line_count;
                else pos.lnum = c;
                pos.col = 0;
                retval = 2;
            }
            else if (pos.col < (0x7fffffffL) - 2) {
                c = spats [0].off.off;
                if (c > 0) {
                    while (c -- > 0) if (incl (&pos) == -1) break;
                }
                else {
                    while (c ++ < 0) if (decl (&pos) == -1) break;
                }
            }
        }
        if (!(options&0x10) || pat == ((void *) 0) || *pat != ';') break;
        dirc = *++pat;
        if (dirc != '?' && dirc != '/') {
            retval = 0;
            emsg ((char_u *) (((char *) ("E386: Expected '?' or '/'  after ';'"))));
            goto end_do_search;
        }
        ++pat;
    }
    if (options & 0x200) setpcmark ();
    curwin -> w_cursor = pos;
    curwin -> w_set_curswant = 1;
    end_do_search : if (options & 0x400) spats [0].off = old_off;
    vim_free (strcopy);
    return retval;
}
    ...
    char_u * pat;
{
    ...
    char_u * ptr;
    char_u * p;
    if (buf -> b_ml.ml_line_count == 0) return 0;
    for (;;) {
        pos -> lnum += dir;
        if (pos -> lnum < 1) {
            if (p_ws) {
        ...
        if (pos -> lnum == start) break;
        if (start == 0) start = pos -> lnum;
        ptr = ml_get_buf (buf, pos -> lnum, 0);
        p = skipwhite (ptr);
        pos -> col = (colnr_T) (p - ptr);
        if ((compl_cont_status & 1) && !(compl_cont_status&16)) {
            if ((p_ic ? strcasecmp ((char *) ((p)), (char *) ((pat))) : strcmp ((char *) (p), (char *) (pat))) == 0) return 1;
        }
        else if (*p != '\000') {
            if ((p_ic ? strncasecmp ((char *) ((p)), (char *) ((pat)), (size_t) ((compl_length))) : strncmp ((char *) (p), (char *) (pat), (size_t) (
              compl_length))) == 0) return 1;
        }
    }
    return 0;
}
    ...
    int t_cmd;
{
    ...
    if (c != '\000') {
        ...
        if (lastc == '\000') return 0;
        if (dir) dir = -lastcdir;
        else dir = lastcdir;
        t_cmd = last_t_cmd;
        c = lastc;
    }
    if (dir == (-1)) cap -> oap -> inclusive = 0;
    else cap -> oap -> inclusive = 1;
    p = ml_get_curline ();
    ...
    int maxtravel;
{
    ...
    if (flags & 0x01) dir = (-1);
    else if (flags & 0x02) dir = 1;
    else dir = 0;
    if (initc == '/' || initc == '*') {
        comment_dir = dir;
        if (initc == '/') ignore_cend = 1;
        backwards = (dir == 1) ? 0 : 1;
        initc = '\000';
    }
        ...
        if (initc == '#') {
            ...
            if (!cpo_match) {
                ...
                if (*ptr == '#' && pos.col <= (colnr_T) (ptr - linep)) {
                    ptr = skipwhite (ptr + 1);
                    if (strncmp ((char *) (ptr), (char *) ("if"), (size_t) (2)) == 0 || strncmp ((char *) (ptr), (char *) ("endif"), (size_t) (5)) == 0 ||
                      strncmp ((char *) (ptr), (char *) ("el"), (size_t) (2)) == 0) hash_dir = 1;
                }
            ...
            if (!hash_dir && !comment_dir) {
                if (linep [pos.col] == '\000' && pos.col) --pos.col;
                for (;;) {
                    initc = linep [pos.col];
                    if (initc == '\000') break;
                ...
                if (!findc) {
                    if (!cpo_match && *skipwhite(linep) == '#') hash_dir = 1;
                    else return ((void *) 0);
                }
                else if (!cpo_bsl) {
                    int col, bslcnt = 0;
                    for (col = pos.col; check_prevcol (linep, col, '\\', &col);) bslcnt ++;
                    match_escaped = (bslcnt & 1);
                }
            }
        }
        if (hash_dir) {
            if (oap != ((void *) 0)) oap -> motion_type = 1;
            if (initc != '#') {
                ptr = skipwhite (skipwhite (linep) + 1);
                if (strncmp ((char *) (ptr), (char *) ("if"), (size_t) (2)) == 0 || strncmp ((char *) (ptr), (char *) ("el"), (size_t) (2)) == 0) hash_dir = 1;
                else if (strncmp ((char *) (ptr), (char *) ("endif"), (size_t) (5)) == 0) hash_dir = -1;
                else return ((void *) 0);
            }
            pos.col = 0;
            while (!got_int) {
                if (hash_dir > 0) {
                    if (pos.lnum == curbuf -> b_ml.ml_line_count) break;
                }
                else if (pos.lnum == 1) break;
                pos.lnum += hash_dir;
                linep = ml_get (pos.lnum);
                line_breakcheck ();
                ptr = skipwhite (linep);
                ...
                ptr = skipwhite (ptr + 1);
                if (hash_dir > 0) {
                    if (strncmp ((char *) (ptr), (char *) ("if"), (size_t) (2)) == 0) count ++;
                    ...
                    else if (strncmp ((char *) (ptr), (char *) ("endif"), (size_t) (5)) == 0) count ++;
                }
            }
            return ((void *) 0);
        }
    ...
    if ((backwards && comment_dir) || lisp) comment_col = check_linecomment (linep);
    if (lisp && comment_col != (0x7fffffffL) && pos.col > (colnr_T) comment_col) lispcomm = 1;
    while (!got_int) {
        if (backwards) {
            if (lispcomm && pos.col < (colnr_T) comment_col) break;
            if (pos.col == 0) {
                if (pos.lnum == 1) break;
                --pos.lnum;
                if (maxtravel > 0 && ++traveled > maxtravel) break;
                linep = ml_get (pos.lnum);
                pos.col = (colnr_T) strlen ((char *) (linep));
                do_quotes = -1;
                line_breakcheck ();
                if (comment_dir || lisp) comment_col = check_linecomment (linep);
                if (lisp && comment_col != (0x7fffffffL)) pos.col = comment_col;
            }
            else {
                --pos.col;
            }
            ...
            if (linep [pos.col] == '\000' || (lisp && comment_col != (0x7fffffffL) && pos.col == (colnr_T) comment_col)) {
                if (pos.lnum == curbuf -> b_ml.ml_line_count || lispcomm) break;
                ++pos.lnum;
                if (maxtravel && traveled ++ > maxtravel) break;
                linep = ml_get (pos.lnum);
                pos.col = 0;
                do_quotes = -1;
                line_breakcheck ();
                if (lisp) comment_col = check_linecomment (linep);
            }
            else {
                ++pos.col;
            }
        }
        if (pos.col == 0 && (flags & 0x04) && (linep [0] == '{' || linep [0] == '}')) {
            if (linep [0] == findc && count == 0) return &pos;
            break;
        }
        if (comment_dir) {
            if (comment_dir == 1) {
                ...
                else if (linep [pos.col - 1] == '/' && linep [pos.col] == '*' && (int) pos.col < comment_col) {
                    count ++;
                    match_pos = pos;
                    match_pos.col --;
                }
                else if (linep [pos.col - 1] == '*' && linep [pos.col] == '/') {
                    if (count > 0) pos = match_pos;
                    else if (pos.col > 1 && linep [pos.col - 2] == '/' && (int) pos.col <= comment_col) pos.col -= 2;
                    else if (ignore_cend) continue;
                    else return ((void *) 0);
                    return &pos;
                }
            }
        ...
        if (cpo_match) do_quotes = 0;
        else if (do_quotes == -1) {
            at_start = do_quotes;
            for (ptr = linep; *ptr; ++ptr) {
                if (ptr == linep + pos.col + backwards) at_start = (do_quotes & 1);
                if (*ptr == '"' && (ptr == linep || ptr [-1] != '\'' || ptr [1] != '\'')) ++do_quotes;
                if (*ptr == '\\' && ptr [1] != '\000') ++ptr;
            }
            do_quotes &= 1;
            if (!do_quotes) {
                inquote = 0;
        ...
        if (start_in_quotes == 2) start_in_quotes = 0;
        ...
        case '"' :
            if (do_quotes) {
                int col;
                for (col = pos.col - 1; col >= 0; --col) if (linep [col] != '\\') break;
        ...
        default :
            if (curbuf -> b_p_lisp && vim_strchr ((char_u *) "(){}[]", c) != ((void *) 0) && pos.col > 1 && check_prevcol (linep, pos.col, '\\', ((void *) 0))
              && check_prevcol (linep, pos.col - 1, '#', ((void *) 0))) break;
            if ((!inquote || start_in_quotes == 1) && (c == initc || c == findc)) {
                int col, bslcnt = 0;
                if (!cpo_bsl) {
                    for (col = pos.col; check_prevcol (linep, col, '\\', &col);) bslcnt ++;
                }
        ...
        if (vim_strchr (p, ';') != ((void *) 0)) {
            ...
            while ((p = (char_u *) strpbrk ((char *) (p), (char *) ((char_u *) "\";"))) != ((void *) 0)) {
                if (*p == '"') {
                    ...
                    else if (p == line || ((p - line) >= 2 && *(p-1) != '\\' && *(p-2) != '#')) instr = 1;
                }
                else if (!instr && ((p - line) < 2 || (*(p-1) != '\\' && *(p-2) != '#'))) break;
                ++p;
            }
        }
        else p = ((void *) 0);
    }
    else while ((p = vim_strchr (p, '/')) != ((void *) 0)) {
        if (p [1] == '/' && (p == line || p [-1] != '*' || p [2] != '*')) break;
        ++p;
    }
    if (p == ((void *) 0)) return (0x7fffffffL);
    return (int) (p - line);
}
    ...
    int c;
{
    ...
    char_u * p;
    for (p = curbuf -> b_p_mps; *p != '\000'; p += 2) {
        p += 2;
        if (*p == c) break;
        if (p [1] != ',') return;
    }
    if ((lpos = findmatch (((void *) 0), '\000')) == ((void *) 0)) vim_beep ();
    else if (lpos -> lnum >= curwin -> w_topline) {
        if (!curwin->w_onebuf_opt.wo_wrap) getvcol (curwin, lpos, ((void *) 0), &vcol, ((void *) 0));
        if (curwin -> w_onebuf_opt.wo_wrap || (vcol >= curwin -> w_leftcol && vcol < curwin -> w_leftcol + (curwin -> w_width))) {
            mpos = *lpos;
            save_cursor = curwin -> w_cursor;
            save_so = p_so;
            save_siso = p_siso;
            if (dollar_vcol > 0 && dollar_vcol == curwin -> w_virtcol) dollar_vcol = 0;
            ++curwin->w_virtcol;
            update_screen (10);
            ...
            if (vim_strchr (p_cpo, 'm') != ((void *) 0)) ui_delay (p_mat * 100L, 1);
            else if (!char_avail()) ui_delay (p_mat * 100L, 0);
    ...
    long count;
{
    ...
    while (count --) {
        if (gchar_pos (&pos) == '\000') {
            do if ((*func) (&pos) == -1) break;
            while (gchar_pos (&pos) == '\000');
            if (dir == 1) goto found;
        }
        else if (dir == 1 && pos.col == 0 && startPS (pos.lnum, '\000', 0)) {
            if (pos.lnum == curbuf -> b_ml.ml_line_count) return 0;
            ++pos.lnum;
            goto found;
        }
        else if (dir == (-1)) decl (&pos);
        found_dot = 0;
        while ((c = gchar_pos (&pos)) == ' ' || c == '\t' || (dir == (-1) && vim_strchr ((char_u *) ".!?)]\"'", c) != ((void *) 0))) {
        ...
        for (;;) {
            ...
            if (c == '\000' || (pos.col == 0 && startPS (pos.lnum, '\000', 0))) {
                if (dir == (-1) && pos.lnum != startlnum) ++pos.lnum;
                break;
            }
            if (c == '.' || c == '!' || c == '?') {
                tpos = pos;
                do if ((c = inc (&tpos)) == -1) break;
                while (vim_strchr ((char_u *) ")]\"'", c = gchar_pos (&tpos)) != ((void *) 0));
                if (c == -1 || (!cpo_J && (c == ' ' || c == '\t')) || c == '\000' || (cpo_J && (c == ' ' && inc (&tpos) >= 0 && gchar_pos (&tpos) == ' '))) {
                    pos = tpos;
                    if (gchar_pos (&pos) == '\000') inc (&pos);
                    break;
                }
            }
            if ((*func) (&pos) == -1) {
        ...
        found : while (!noskip && ((c = gchar_pos (&pos)) == ' ' || c == '\t')) if (incl (&pos) == -1) break;
    }
    setpcmark ();
    curwin -> w_cursor = pos;
    return 1;
    ...
    int both;
{
    ...
    while (count --) {
        did_skip = 0;
        for (first = 1;; first = 0) {
            if (*ml_get(curr) != '\000') did_skip = 1;
            fold_skipped = 0;
            if (first && hasFolding (curr, &fold_first, &fold_last)) {
                curr = ((dir > 0) ? fold_last : fold_first) + dir;
                fold_skipped = 1;
            }
            if (!first && did_skip && (startPS (curr, what, both) || (posix && what == '\000' && *ml_get(curr) == '{'))) break;
            if (fold_skipped) curr -= dir;
    ...
    char_u * s;
{
    ...
    for (macro = opt; macro [0]; ++macro) {
        if ((macro [0] == s [0] || (macro [0] == ' ' && (s [0] == '\000' || s [0] == ' '))) && (macro [1] == s [1] || ((macro [1] == '\000' || macro [1] == ' '
          ) && (s [0] == '\000' || s [1] == '\000' || s [1] == ' ')))) break;
        ++macro;
        if (macro [0] == '\000') break;
    }
    return (macro [0] != '\000');
}
    ...
    int both;
{
    char_u * s;
    s = ml_get (lnum);
    if (*s == para || *s == '\f' || (both && *s == '}')) return 1;
    if (*s == '.' && (inmacro (p_sections, s + 1) || (!para && inmacro (p_para, s + 1)))) return 1;
    return 0;
}
...
static int cls () {
    int c;
    c = gchar_cursor ();
    if (c == ' ' || c == '\t' || c == '\000') return 0;
    if (cls_bigword) return 1;
    ...
    int eol;
{
    ...
    cls_bigword = bigword;
    while (--count >= 0) {
        if (hasFolding (curwin -> w_cursor.lnum, ((void *) 0), &curwin->w_cursor.lnum)) coladvance ((colnr_T) (0x7fffffffL));
        sclass = cls ();
        last_line = (curwin -> w_cursor.lnum == curbuf -> b_ml.ml_line_count);
        i = inc_cursor ();
        if (i == -1 || (i >= 1 && last_line)) return 0;
        if (i >= 1 && eol && count == 0) return 1;
        if (sclass != 0) while (cls () == sclass) {
            i = inc_cursor ();
            if (i == -1 || (i >= 1 && eol && count == 0)) return 1;
        }
        while (cls () == 0) {
            if (curwin -> w_cursor.col == 0 && *ml_get_curline() == '\000') break;
            i = inc_cursor ();
            if (i == -1 || (i >= 1 && eol && count == 0)) return 1;
        }
    }
    ...
    int stop;
{
    ...
    while (--count >= 0) {
        ...
        sclass = cls ();
        if (dec_cursor () == -1) return 0;
        if (!stop || sclass == cls () || sclass == 0) {
            while (cls () == 0) {
                if (curwin -> w_cursor.col == 0 && (*ml_get(curwin->w_cursor.lnum) == '\000')) goto finished;
                if (dec_cursor () == -1) return 1;
            }
            if (skip_chars (cls (), (-1))) return 1;
        }
        inc_cursor ();
        finished : stop = 0;
    }
    ...
    int empty;
{
    ...
    while (--count >= 0) {
        if (hasFolding (curwin -> w_cursor.lnum, ((void *) 0), &curwin->w_cursor.lnum)) coladvance ((colnr_T) (0x7fffffffL));
        sclass = cls ();
        if (inc_cursor () == -1) return 0;
        if (cls () == sclass && sclass != 0) {
            if (skip_chars (sclass, 1)) return 0;
        }
        else if (!stop || sclass == 0) {
            while (cls () == 0) {
                if (empty && curwin -> w_cursor.col == 0 && (*ml_get(curwin->w_cursor.lnum) == '\000')) goto finished;
                if (inc_cursor () == -1) return 0;
            }
            if (skip_chars (cls (), 1)) return 0;
        }
        dec_cursor ();
        finished : stop = 0;
    }
    ...
    int eol;
{
    ...
    while (--count >= 0) {
        sclass = cls ();
        if ((i = dec_cursor ()) == -1) return 0;
        if (eol && i == 1) return 1;
        if (sclass != 0) {
            while (cls () == sclass) if ((i = dec_cursor ()) == -1 || (eol && i == 1)) return 1;
        }
        while (cls () == 0) {
            if (curwin -> w_cursor.col == 0 && (*ml_get(curwin->w_cursor.lnum) == '\000')) break;
            if ((i = dec_cursor ()) == -1 || (eol && i == 1)) return 1;
        }
    }
    return 1;
}
...
static int skip_chars (cclass, dir)
    ...
    int dir;
{
    while (cls () == cclass) if ((dir == 1 ? inc_cursor () : dec_cursor ()) == -1) return 1;
    return 0;
}
    ...
    int at_start_sent;
{
    while (count --) {
        findsent (1, 1L);
        if (at_start_sent) find_first_blank (&curwin->w_cursor);
        if (count == 0 || at_start_sent) decl (&curwin->w_cursor);
    ...
    int bigword;
{
    ...
    if (VIsual_active && *p_sel == 'e' && (((VIsual).lnum != (curwin -> w_cursor).lnum) ? (VIsual).lnum < (curwin -> w_cursor).lnum : (VIsual).col != (curwin ->
      w_cursor).col ? (VIsual).col < (curwin -> w_cursor).col : (VIsual).coladd < (curwin -> w_cursor).coladd)) dec_cursor ();
    if (!VIsual_active || (((curwin -> w_cursor).lnum == (VIsual).lnum) && ((curwin -> w_cursor).col == (VIsual).col) && ((curwin -> w_cursor).coladd == (
      VIsual).coladd))) {
        back_in_line ();
        start_pos = curwin -> w_cursor;
        if ((cls () == 0) == include) {
            if (end_word (1L, bigword, 1, 1) == 0) return 0;
        }
        else {
            fwd_word (1L, bigword, 1);
            if (curwin -> w_cursor.col == 0) decl (&curwin->w_cursor);
            else oneleft ();
            if (include) include_white = 1;
        }
        if (VIsual_active) {
            VIsual = start_pos;
    ...
    while (count > 0) {
        inclusive = 1;
        if (VIsual_active && (((curwin -> w_cursor).lnum != (VIsual).lnum) ? (curwin -> w_cursor).lnum < (VIsual).lnum : (curwin -> w_cursor).col != (VIsual).
          col ? (curwin -> w_cursor).col < (VIsual).col : (curwin -> w_cursor).coladd < (VIsual).coladd)) {
            if (decl (&curwin->w_cursor) == -1) return 0;
            if (include != (cls () != 0)) {
                if (bck_word (1L, bigword, 1) == 0) return 0;
            }
            else {
                if (bckend_word (1L, bigword, 1) == 0) return 0;
                (void) incl (&curwin->w_cursor);
            }
        }
        else {
            if (incl (&curwin->w_cursor) == -1) return 0;
            if (include != (cls () == 0)) {
                if (fwd_word (1L, bigword, 1) == 0 && count > 1) return 0;
                if (oneleft () == 0) inclusive = 0;
            }
            else {
                if (end_word (1L, bigword, 1, 1) == 0) return 0;
            }
    ...
    if (include_white && (cls () != 0 || (curwin -> w_cursor.col == 0 && !inclusive))) {
        ...
        if (oneleft () == 1) {
            ...
            if (cls () == 0 && curwin -> w_cursor.col > 0) {
                if (VIsual_active) VIsual = curwin -> w_cursor;
                else oap -> start = curwin -> w_cursor;
            }
        }
        curwin -> w_cursor = pos;
    }
    if (VIsual_active) {
        if (*p_sel == 'e' && inclusive && ((((VIsual).lnum != (curwin -> w_cursor).lnum) ? (VIsual).lnum < (curwin -> w_cursor).lnum : (VIsual).col != (curwin
          -> w_cursor).col ? (VIsual).col < (curwin -> w_cursor).col : (VIsual).coladd < (curwin -> w_cursor).coladd) || (((VIsual).lnum == (curwin -> w_cursor
          ).lnum) && ((VIsual).col == (curwin -> w_cursor).col) && ((VIsual).coladd == (curwin -> w_cursor).coladd)))) inc_cursor ();
        if (VIsual_mode == 'V') {
            VIsual_mode = 'v';
            redraw_cmdline = 1;
        }
    }
    else oap -> inclusive = inclusive;
    return 1;
}
    ...
    int include;
{
    ...
    if (VIsual_active && !(((start_pos).lnum==(VIsual).lnum)&&((start_pos).col==(VIsual).col)&&((start_pos).coladd==(VIsual).coladd))) {
        extend : if ((((start_pos).lnum != (VIsual).lnum) ? (start_pos).lnum < (VIsual).lnum : (start_pos).col != (VIsual).col ? (start_pos).col < (VIsual).col
          : (start_pos).coladd < (VIsual).coladd)) {
            at_start_sent = 1;
            decl (&pos);
            while ((((pos).lnum != (curwin -> w_cursor).lnum) ? (pos).lnum < (curwin -> w_cursor).lnum : (pos).col != (curwin -> w_cursor).col ? (pos).col < (
              curwin -> w_cursor).col : (pos).coladd < (curwin -> w_cursor).coladd)) {
                c = gchar_pos (&pos);
                if (!((c)==' '||(c)=='\t')) {
                    at_start_sent = 0;
                    break;
            ...
            if (!at_start_sent) {
                findsent ((-1), 1L);
                if ((((curwin -> w_cursor).lnum == (start_pos).lnum) && ((curwin -> w_cursor).col == (start_pos).col) && ((curwin -> w_cursor).coladd == (
                  start_pos).coladd))) at_start_sent = 1;
                else findsent (1, 1L);
            }
            if (include) count *= 2;
            while (count --) {
                if (at_start_sent) find_first_blank (&curwin->w_cursor);
                c = gchar_cursor ();
                if (!at_start_sent || (!include && !((c)==' '||(c)=='\t'))) findsent ((-1), 1L);
                at_start_sent = !at_start_sent;
            }
        }
        else {
            ...
            if (!(((pos).lnum==(curwin->w_cursor).lnum)&&((pos).col==(curwin->w_cursor).col)&&((pos).coladd==(curwin->w_cursor).coladd))) {
                ...
                if (at_start_sent) findsent ((-1), 1L);
                else curwin -> w_cursor = start_pos;
            }
            if (include) count *= 2;
            findsent_forward (count, at_start_sent);
            if (*p_sel == 'e') ++curwin->w_cursor.col;
        }
        return 1;
    }
    while (c = gchar_pos (&pos), ((c) == ' ' || (c) == '\t')) incl (&pos);
    if ((((pos).lnum == (curwin -> w_cursor).lnum) && ((pos).col == (curwin -> w_cursor).col) && ((pos).coladd == (curwin -> w_cursor).coladd))) {
        start_blank = 1;
    ...
    if (include) ncount = count * 2;
    else {
        ncount = count;
        if (start_blank) --ncount;
    }
    if (ncount > 0) findsent_forward (ncount, 1);
    else decl (&curwin->w_cursor);
    if (include) {
        if (start_blank) {
            find_first_blank (&curwin->w_cursor);
            c = gchar_pos (&curwin->w_cursor);
            if (((c) == ' ' || (c) == '\t')) decl (&curwin->w_cursor);
        }
        else if (c = gchar_cursor (), !((c)==' '||(c)=='\t')) find_first_blank (&start_pos);
    }
    if (VIsual_active) {
        if ((((start_pos).lnum == (curwin -> w_cursor).lnum) && ((start_pos).col == (curwin -> w_cursor).col) && ((start_pos).coladd == (curwin -> w_cursor).
          coladd))) goto extend;
        if (*p_sel == 'e') ++curwin->w_cursor.col;
        VIsual = start_pos;
        VIsual_mode = 'v';
        ...
        if (incl (&curwin->w_cursor) == -1) oap -> inclusive = 1;
        else oap -> inclusive = 0;
        oap -> start = start_pos;
        oap -> motion_type = 0;
    }
    return 1;
    ...
    int other;
{
    ...
    old_start = old_end;
    if (!VIsual_active || (((VIsual).lnum == (curwin -> w_cursor).lnum) && ((VIsual).col == (curwin -> w_cursor).col) && ((VIsual).coladd == (curwin -> w_cursor
      ).coladd))) {
        setpcmark ();
        if (what == '{') while (inindent (1)) if (inc_cursor () != 0) break;
        if (gchar_cursor () == what) ++curwin->w_cursor.col;
    }
    else if ((((VIsual).lnum != (curwin -> w_cursor).lnum) ? (VIsual).lnum < (curwin -> w_cursor).lnum : (VIsual).col != (curwin -> w_cursor).col ? (VIsual).
    ...
    p_cpo = (char_u *) "%";
    while (count -- > 0) {
        if ((pos = findmatch (((void *) 0), what)) == ((void *) 0)) break;
        curwin -> w_cursor = *pos;
        start_pos = *pos;
    }
    p_cpo = save_cpo;
    ...
    while (!include) {
        ...
        if (what == '{') while (inindent (1)) {
            sol = 1;
            if (decl (&curwin->w_cursor) != 0) break;
        }
        if (!(((start_pos).lnum!=(old_start).lnum)?(start_pos).lnum<(old_start).lnum:(start_pos).col!=(old_start).col?(start_pos).col<(old_start).col:(
          start_pos).coladd<(old_start).coladd) && !(((old_end).lnum!=(curwin->w_cursor).lnum)?(old_end).lnum<(curwin->w_cursor).lnum:(old_end).col!=(curwin->
          w_cursor).col?(old_end).col<(curwin->w_cursor).col:(old_end).coladd<(curwin->w_cursor).coladd) && VIsual_active) {
    ...
    if (VIsual_active) {
        if (*p_sel == 'e') ++curwin->w_cursor.col;
        if (sol && gchar_cursor () != '\000') inc (&curwin->w_cursor);
        VIsual = start_pos;
        VIsual_mode = 'v';
        redraw_curbuf_later (20);
        showmode ();
        ...
        oap -> motion_type = 0;
        oap -> inclusive = 0;
        if (sol) incl (&curwin->w_cursor);
        else if (((((start_pos).lnum != (curwin -> w_cursor).lnum) ? (start_pos).lnum < (curwin -> w_cursor).lnum : (start_pos).col != (curwin -> w_cursor).col
          ? (start_pos).col < (curwin -> w_cursor).col : (start_pos).coladd < (curwin -> w_cursor).coladd) || (((start_pos).lnum == (curwin -> w_cursor).lnum)
          && ((start_pos).col == (curwin -> w_cursor).col) && ((start_pos).coladd == (curwin -> w_cursor).coladd)))) oap -> inclusive = 1;
        else curwin -> w_cursor = start_pos;
    }
    return 1;
}
    ...
    int include;
{
    ...
    old_end = curwin -> w_cursor;
    old_start = old_end;
    if (!VIsual_active || *p_sel == 'e') decl (&old_end);
    if (!VIsual_active || (((VIsual).lnum == (curwin -> w_cursor).lnum) && ((VIsual).col == (curwin -> w_cursor).col) && ((VIsual).coladd == (curwin -> w_cursor
      ).coladd))) {
        setpcmark ();
        while (inindent (1)) if (inc_cursor () != 0) break;
        if (in_html_tag (0)) {
            while (*ml_get_cursor() != '>') if (inc_cursor () < 0) break;
        }
        else if (in_html_tag (1)) {
            while (*ml_get_cursor() != '<') if (dec_cursor () < 0) break;
            dec_cursor ();
            old_end = curwin -> w_cursor;
        }
    }
    ...
    again : for (n = 0; n < count; ++n) {
        if (do_searchpair ((char_u *) "<[^ \t>/!]\\+\\%(\\_s\\_[^>]\\{-}[^/]>\\|$\\|\\_s\\=>\\)", (char_u *) "", (char_u *) "</[^>]*>", (-1), (char_u *) "", 0,
    ...
    spat = alloc (len + 29);
    epat = alloc (len + 9);
    if (spat == ((void *) 0) || epat == ((void *) 0)) {
        vim_free (spat);
        vim_free (epat);
        curwin -> w_cursor = old_pos;
        goto theend;
    }
    __builtin___sprintf_chk ((char *) spat, 0, __builtin_object_size ((char *) spat, 1 > 1), "<%.*s\\%%(\\_[^>]\\{-}[^/]>\\|>\\)\\c", len, p);
    __builtin___sprintf_chk ((char *) epat, 0, __builtin_object_size ((char *) epat, 1 > 1), "</%.*s>\\c", len, p);
    r = do_searchpair (spat, (char_u *) "", epat, 1, (char_u *) "", 0, ((void *) 0), (linenr_T) 0, 0L);
    vim_free (spat);
    vim_free (epat);
    if (r < 1 || (((curwin -> w_cursor).lnum != (old_end).lnum) ? (curwin -> w_cursor).lnum < (old_end).lnum : (curwin -> w_cursor).col != (old_end).col ? (
      curwin -> w_cursor).col < (old_end).col : (curwin -> w_cursor).coladd < (old_end).coladd)) {
        count = 1;
        curwin -> w_cursor = start_pos;
        goto again;
    }
    if (do_include || r < 1) {
        ...
        if (*ml_get_cursor() == '<') dec_cursor ();
    }
    end_pos = curwin -> w_cursor;
    if (!do_include) {
        curwin -> w_cursor = start_pos;
        while (inc_cursor () >= 0) if (*ml_get_cursor() == '>') {
            inc_cursor ();
            start_pos = curwin -> w_cursor;
            break;
        }
        curwin -> w_cursor = end_pos;
        if ((((start_pos).lnum == (old_start).lnum) && ((start_pos).col == (old_start).col) && ((start_pos).coladd == (old_start).coladd)) && (((end_pos).lnum
          == (old_end).lnum) && ((end_pos).col == (old_end).col) && ((end_pos).coladd == (old_end).coladd))) {
            do_include = 1;
            curwin -> w_cursor = old_start;
            count = count_arg;
            goto again;
    ...
    if (VIsual_active) {
        if ((((end_pos).lnum != (start_pos).lnum) ? (end_pos).lnum < (start_pos).lnum : (end_pos).col != (start_pos).col ? (end_pos).col < (start_pos).col : (
          end_pos).coladd < (start_pos).coladd)) curwin -> w_cursor = start_pos;
        else if (*p_sel == 'e') ++curwin->w_cursor.col;
        VIsual = start_pos;
        VIsual_mode = 'v';
        redraw_curbuf_later (20);
        showmode ();
        ...
        oap -> motion_type = 0;
        if ((((end_pos).lnum != (start_pos).lnum) ? (end_pos).lnum < (start_pos).lnum : (end_pos).col != (start_pos).col ? (end_pos).col < (start_pos).col : (
          end_pos).coladd < (start_pos).coladd)) {
            curwin -> w_cursor = start_pos;
            oap -> inclusive = 0;
        }
        else oap -> inclusive = 1;
    }
    retval = 1;
    ...
    int type;
{
    ...
    if (VIsual_active && start_lnum != VIsual.lnum) {
        extend : if (start_lnum < VIsual.lnum) dir = (-1);
        else dir = 1;
        for (i = count; --i >= 0;) {
            ...
            for (t = 0; t < 2; ++t) {
                ...
                for (;;) {
                    if (start_lnum == (dir == (-1) ? 1 : curbuf -> b_ml.ml_line_count)) break;
                    if (start_is_white != linewhite (start_lnum + dir) || (!start_is_white && startPS (start_lnum + (dir > 0 ? 1 : 0), 0, 0))) break;
                    start_lnum += dir;
                }
                if (!include) break;
                if (start_lnum == (dir == (-1) ? 1 : curbuf -> b_ml.ml_line_count)) break;
    ...
    while (start_lnum > 1) {
        if (white_in_front) {
            if (!linewhite(start_lnum-1)) break;
        }
        else {
            if (linewhite (start_lnum - 1) || startPS (start_lnum, 0, 0)) break;
        }
        --start_lnum;
    }
    end_lnum = start_lnum;
    while (end_lnum <= curbuf -> b_ml.ml_line_count && linewhite (end_lnum)) ++end_lnum;
    --end_lnum;
    i = count;
    if (!include && white_in_front) --i;
    while (i --) {
        if (end_lnum == curbuf -> b_ml.ml_line_count) return 0;
        if (!include) do_white = linewhite (end_lnum + 1);
        if (include || !do_white) {
            ++end_lnum;
            while (end_lnum < curbuf -> b_ml.ml_line_count && !linewhite(end_lnum+1) && !startPS(end_lnum+1,0,0)) ++end_lnum;
        }
        if (i == 0 && white_in_front && include) break;
        if (include || do_white) while (end_lnum < curbuf -> b_ml.ml_line_count && linewhite (end_lnum + 1)) ++end_lnum;
    }
    if (!white_in_front && !linewhite(end_lnum) && include) while (start_lnum > 1 && linewhite (start_lnum - 1)) --start_lnum;
    if (VIsual_active) {
        if (VIsual_mode == 'V' && start_lnum == curwin -> w_cursor.lnum) goto extend;
    ...
    char_u * escape;
{
    ...
    for (;;) {
        c = line [col];
        if (c == '\000') return -1;
        else if (escape != ((void *) 0) && vim_strchr (escape, c)) ++col;
    ...
    char_u * escape;
{
    ...
    while (col_start > 0) {
        --col_start;
        n = 0;
        if (escape != ((void *) 0)) while (col_start - n > 0 && vim_strchr (escape, line [col_start - n - 1]) != ((void *) 0)) ++n;
        if (n & 1) col_start -= n;
        else if (line [col_start] == quotechar) break;
    }
    return col_start;
}
    ...
    int quotechar;
{
    ...
    int i;
    if (VIsual_active) {
        vis_bef_curs = (((VIsual).lnum != (curwin -> w_cursor).lnum) ? (VIsual).lnum < (curwin -> w_cursor).lnum : (VIsual).col != (curwin -> w_cursor).col ? (
          VIsual).col < (curwin -> w_cursor).col : (VIsual).coladd < (curwin -> w_cursor).coladd);
        if (*p_sel == 'e' && vis_bef_curs) dec_cursor ();
        vis_empty = (((VIsual).lnum == (curwin -> w_cursor).lnum) && ((VIsual).col == (curwin -> w_cursor).col) && ((VIsual).coladd == (curwin -> w_cursor).
          coladd));
    }
    if (!vis_empty) {
        if (vis_bef_curs) {
            inside_quotes = VIsual.col > 0 && line [VIsual.col - 1] == quotechar && line [curwin -> w_cursor.col] != '\000' && line [curwin -> w_cursor.col + 1
              ] == quotechar;
            i = VIsual.col;
            col_end = curwin -> w_cursor.col;
        }
        else {
            inside_quotes = curwin -> w_cursor.col > 0 && line [curwin -> w_cursor.col - 1] == quotechar && line [VIsual.col] != '\000' && line [VIsual.col + 1
              ] == quotechar;
            i = curwin -> w_cursor.col;
            col_end = VIsual.col;
        }
        while (i <= col_end) if (line [i ++] == quotechar) {
            selected_quote = 1;
            break;
        }
    }
    if (!vis_empty && line [col_start] == quotechar) {
        if (vis_bef_curs) {
            ...
            if (line [col_end] != quotechar) return 0;
        ...
        if (!vis_empty) {
            if (vis_bef_curs) first_col = find_next_quote (line, col_start, quotechar, ((void *) 0));
            else first_col = find_prev_quote (line, col_start, quotechar, ((void *) 0));
        }
        col_start = 0;
        for (;;) {
            col_start = find_next_quote (line, col_start, quotechar, ((void *) 0));
            if (col_start < 0 || col_start > first_col) return 0;
            col_end = find_next_quote (line, col_start + 1, quotechar, curbuf -> b_p_qe);
            if (col_end < 0) return 0;
            if (col_start <= first_col && first_col <= col_end) break;
            col_start = col_end + 1;
        }
    }
    else {
    ...
    if (include) {
        if (((line [col_end + 1]) == ' ' || (line [col_end + 1]) == '\t')) while (((line [col_end + 1]) == ' ' || (line [col_end + 1]) == '\t')) ++col_end;
        else while (col_start > 0 && ((line [col_start - 1]) == ' ' || (line [col_start - 1]) == '\t')) --col_start;
    }
    if (!include && count < 2 && (vis_empty || !inside_quotes)) ++col_start;
    curwin -> w_cursor.col = col_start;
    if (VIsual_active) {
        if (vis_empty || (vis_bef_curs && !selected_quote && (inside_quotes || (line [VIsual.col] != quotechar && (VIsual.col == 0 || line [VIsual.col - 1] !=
          quotechar))))) {
    ...
    if ((include || count > 1 || (!vis_empty && inside_quotes)) && inc_cursor () == 2) inclusive = 1;
    if (VIsual_active) {
        if (vis_empty || vis_bef_curs) {
            if (*p_sel != 'e') dec_cursor ();
        }
        else {
            if (inside_quotes || (!selected_quote && line [VIsual.col] != quotechar && (line [VIsual.col] == '\000' || line [VIsual.col + 1] != quotechar))) {
                dec_cursor ();
                VIsual = curwin -> w_cursor;
            }
            curwin -> w_cursor.col = col_start;
...
void find_pattern_in_path (ptr, dir, len, whole, skip_comments, type, count, action, start_lnum, end_lnum)
    char_u * ptr;
    ...
    linenr_T end_lnum;
{
    ...
    file_line = alloc (512);
    if (file_line == ((void *) 0)) return;
    if (type != 3 && type != 2 && !(compl_cont_status&16)) {
        pat = alloc (len + 5);
        if (pat == ((void *) 0)) goto fpip_end;
        __builtin___sprintf_chk ((char *) pat, 0, __builtin_object_size ((char *) pat, 1 > 1), whole ? "\\<%.*s\\>" : "%.*s", len, ptr);
        regmatch.rm_ic = ignorecase (pat);
        regmatch.regprog = vim_regcomp (pat, p_magic ? 1 : 0);
        vim_free (pat);
        if (regmatch.regprog == ((void *) 0)) goto fpip_end;
    }
    inc_opt = (*curbuf->b_p_inc == '\000') ? p_inc : curbuf -> b_p_inc;
    if (*inc_opt != '\000') {
        incl_regmatch.regprog = vim_regcomp (inc_opt, p_magic ? 1 : 0);
        if (incl_regmatch.regprog == ((void *) 0)) goto fpip_end;
        incl_regmatch.rm_ic = 0;
    }
    if (type == 2 && (*curbuf->b_p_def != '\000' || *p_def != '\000')) {
        def_regmatch.regprog = vim_regcomp (*curbuf->b_p_def == '\000' ? p_def : curbuf -> b_p_def, p_magic ? 1 : 0);
        if (def_regmatch.regprog == ((void *) 0)) goto fpip_end;
        def_regmatch.rm_ic = 0;
    }
    files = (SearchedFile *) lalloc_clear ((long_u) (max_path_depth * sizeof (SearchedFile)), 1);
    if (files == ((void *) 0)) goto fpip_end;
    old_files = max_path_depth;
    depth = depth_displayed = -1;
    lnum = start_lnum;
    if (end_lnum > curbuf -> b_ml.ml_line_count) end_lnum = curbuf -> b_ml.ml_line_count;
    if (lnum > end_lnum) lnum = end_lnum;
    line = ml_get (lnum);
    for (;;) {
        if (incl_regmatch.regprog != ((void *) 0) && vim_regexec (&incl_regmatch, line, (colnr_T) 0)) {
            char_u * p_fname = (curr_fname == curbuf -> b_fname) ? curbuf -> b_ffname : curr_fname;
            if (inc_opt != ((void *) 0) && strstr ((char *) inc_opt, "\\zs") != ((void *) 0)) new_fname = find_file_name_in_path (incl_regmatch.startp [0], (
              int) (incl_regmatch.endp [0] - incl_regmatch.startp [0]), 2 | 8 | 16, 1L, p_fname);
            else new_fname = file_name_in_line (incl_regmatch.endp [0], 0, 2 | 8 | 16, 1L, p_fname, ((void *) 0));
            already_searched = 0;
            if (new_fname != ((void *) 0)) {
                for (i = 0;; i ++) {
                    if (i == depth + 1) i = old_files;
                    if (i == max_path_depth) break;
                    if (fullpathcmp (new_fname, files [i].name, 1) & 1) {
                        if (type != 3 && action == 4 && files [i].matched) {
                            msg_putchar ('\n');
                            if (!got_int) {
                                msg_home_replace_hl (new_fname);
                                msg_puts ((char_u *) (((char *) (" (includes previously listed match)"))));
                                prev_fname = ((void *) 0);
                            }
                        }
                        vim_free (new_fname);
            ...
            if (type == 3 && (action == 4 || (new_fname == ((void *) 0) && !already_searched))) {
                if (did_show) msg_putchar ('\n');
                else {
                    gotocmdline (1);
                    msg_puts_title ((char_u *) (((char *) ("--- Included files "))));
                    if (action != 4) msg_puts_title ((char_u *) (((char *) ("not found "))));
                    msg_puts_title ((char_u *) (((char *) ("in path ---\n"))));
                }
                did_show = 1;
                while (depth_displayed < depth && !got_int) {
                    ++depth_displayed;
                    for (i = 0; i < depth_displayed; i ++) msg_puts ((char_u *) ("  "));
                ...
                if (!got_int) {
                    ...
                    if (new_fname == ((void *) 0) && action == 4) {
                        if (already_searched) msg_puts ((char_u *) (((char *) ("  (Already listed)"))));
                        else msg_puts ((char_u *) (((char *) ("  NOT FOUND"))));
                    }
                }
                out_flush ();
            }
            if (new_fname != ((void *) 0)) {
                if (depth + 1 == old_files) {
                    bigger = (SearchedFile *) lalloc ((long_u) (max_path_depth * 2 * sizeof (SearchedFile)), 1);
                    if (bigger != ((void *) 0)) {
                        for (i = 0; i <= depth; i ++) bigger [i] = files [i];
                        ...
                        for (i = old_files; i < max_path_depth; i ++) bigger [i + max_path_depth] = files [i];
                        old_files += max_path_depth;
                        max_path_depth *= 2;
                        vim_free (files);
                        files = bigger;
                ...
                if ((files [depth + 1].fp = fopen (((char *) new_fname), ("r"))) == ((void *) 0)) vim_free (new_fname);
                else {
                    if (++depth == old_files) {
                    ...
                    if (action == 5) {
                        msg_hist_off = 1;
                        vim_snprintf ((char *) IObuff, (1024 + 1), ((char *) ("Scanning included file: %s")), (char *) new_fname);
                        msg_trunc_attr (IObuff, 1, highlight_attr [(int) (HLF_R)]);
                    }
                    else if (p_verbose >= 5) {
                        verbose_enter ();
                        smsg ((char_u *) ((char *) ("Searching included file %s")), (char *) new_fname);
                        verbose_leave ();
                    }
                }
            }
            ...
            p = line;
            search_line : define_matched = 0;
            if (def_regmatch.regprog != ((void *) 0) && vim_regexec (&def_regmatch, line, (colnr_T) 0)) {
                p = def_regmatch.endp [0];
                while (*p && !vim_iswordc(*p)) p ++;
                define_matched = 1;
            }
            if (def_regmatch.regprog == ((void *) 0) || define_matched) {
                if (define_matched || (compl_cont_status & 16)) {
                    startp = skipwhite (p);
                    if (p_ic) matched = !strncasecmp((char*)((startp)),(char*)((ptr)),(size_t)((len)));
                    else matched = !strncmp((char*)(startp),(char*)(ptr),(size_t)(len));
                    if (matched && define_matched && whole && vim_iswordc (startp [len])) matched = 0;
                }
                else if (regmatch.regprog != ((void *) 0) && vim_regexec (&regmatch, line, (colnr_T) (p - line))) {
                    matched = 1;
                    startp = regmatch.startp [0];
                    if (!define_matched && skip_comments) {
                        if ((*line != '#' || strncmp ((char *) (skipwhite (line + 1)), (char *) ("define"), (size_t) (6)) != 0) && get_leader_len (line, ((void
                          *) 0), 0)) matched = 0;
                        p = skipwhite (line);
                        if (matched || (p [0] == '/' && p [1] == '*') || p [0] == '*') for (p = line; *p && p < startp; ++p) {
        ...
        if (matched) {
            if (action == 5) {
                ...
                int add_r;
                char_u * aux;
                if (depth == -1 && lnum == curwin -> w_cursor.lnum) break;
                found = 1;
                aux = p = startp;
                if (compl_cont_status & 1) {
                    p += compl_length;
                    if (vim_iswordp (p)) goto exit_matched;
                    p = find_word_start (p);
                }
                p = find_word_end (p);
                i = (int) (p - aux);
                if ((compl_cont_status & 1) && i == compl_length) {
                    ((__builtin_object_size ((char *) (IObuff), 0) != (size_t) - 1) ? __builtin___strncpy_chk ((char *) (IObuff), (char *) (aux), (size_t) (i),
                      __builtin_object_size ((char *) (IObuff), 1 > 1)) : __inline_strncpy_chk ((char *) (IObuff), (char *) (aux), (size_t) (i)));
                    if (depth < 0) {
                        if (lnum >= end_lnum) goto exit_matched;
                        line = ml_get (++lnum);
                    }
                    else if (vim_fgets (line = file_line, 512, files [depth].fp)) goto exit_matched;
                    already = aux = p = skipwhite (line);
                    p = find_word_start (p);
                    p = find_word_end (p);
                    if (p > aux) {
                        if (*aux != ')' && IObuff [i - 1] != '\011') {
                            if (IObuff [i - 1] != ' ') IObuff [i ++] = ' ';
                            if (p_js && (IObuff [i - 2] == '.' || (vim_strchr (p_cpo, 'j') == ((void *) 0) && (IObuff [i - 2] == '?' || IObuff [i - 2] == '!'))
                              )) IObuff [i ++] = ' ';
                        }
                        if (p - aux >= (1024 + 1) - i) p = aux + (1024 + 1) - i - 1;
                        ((__builtin_object_size ((char *) (IObuff + i), 0) != (size_t) - 1) ? __builtin___strncpy_chk ((char *) (IObuff + i), (char *) (aux), (
                          size_t) (p - aux), __builtin_object_size ((char *) (IObuff + i), 1 > 1)) : __inline_strncpy_chk ((char *) (IObuff + i), (char *) (aux
                          ), (size_t) (p - aux)));
                        i += (int) (p - aux);
                        reuse |= 8;
                    }
                    IObuff [i] = '\000';
                    aux = IObuff;
                    if (i == compl_length) goto exit_matched;
                }
                add_r = ins_compl_add_infercase (aux, i, p_ic, curr_fname == curbuf -> b_fname ? ((void *) 0) : curr_fname, dir, reuse);
                if (add_r == 1) dir = 1;
                else if (add_r == 0) break;
            }
            else if (action == 4) {
                found = 1;
                if (!did_show) gotocmdline (1);
                if (curr_fname != prev_fname) {
                    if (did_show) msg_putchar ('\n');
                    if (!got_int) msg_home_replace_hl (curr_fname);
                    prev_fname = curr_fname;
                }
                did_show = 1;
                if (!got_int) show_pat_in_path (line, type, 1, action, (depth == -1) ? ((void *) 0) : files [depth].fp, (depth == -1) ? &lnum : 
                  &files[depth].lnum, match_count ++);
                for (i = 0; i <= depth; ++i) files [i].matched = 1;
            }
            else if (--count <= 0) {
                found = 1;
                if (depth == -1 && lnum == curwin -> w_cursor.lnum && g_do_tagpreview == 0) emsg ((char_u *) (((char *) ("E387: Match is on current line"))));
                else if (action == 1) {
                    show_pat_in_path (line, type, did_show, action, (depth == -1) ? ((void *) 0) : files [depth].fp, (depth == -1) ? &lnum : &files[depth].lnum
                      , 1L);
                    did_show = 1;
                }
                else {
                    ...
                    if (action == 3) {
                        if (win_split (0, 0) == 0) break;
                        curwin -> w_onebuf_opt.wo_scb = 0;
                    }
                    if (depth == -1) {
                        if (g_do_tagpreview != 0) {
                            if (getfile (0, curwin_save -> w_buffer -> b_fname, ((void *) 0), 1, lnum, 0) > 0) break;
                        }
                        else setpcmark ();
                        curwin -> w_cursor.lnum = lnum;
                    }
                    else {
                        if (getfile (0, files [depth].name, ((void *) 0), 1, files [depth].lnum, 0) > 0) break;
            ...
            exit_matched : matched = 0;
            if (def_regmatch.regprog == ((void *) 0) && action == 5 && !(compl_cont_status&16) && *startp != '\000' && *(p=startp+1) != '\000') goto search_line
              ;
        }
        line_breakcheck ();
        if (action == 5) ins_compl_check_keys (30);
        if (got_int || compl_interrupted) break;
        while (depth >= 0 && !already && vim_fgets (line = file_line, 512, files [depth].fp)) {
            ...
            if (depth < depth_displayed) depth_displayed = depth;
        }
        if (depth >= 0) files [depth].lnum ++;
        else if (!already) {
            if (++lnum > end_lnum) break;
            line = ml_get (lnum);
        }
        already = ((void *) 0);
    }
    ...
    for (i = old_files; i < max_path_depth; i ++) vim_free (files [i].name);
    vim_free (files);
    if (type == 3) {
        if (!did_show) {
            if (action != 4) msg ((char_u *) (((char *) ("All included files were found"))));
            else msg ((char_u *) (((char *) ("No included files"))));
        }
    }
    else if (!found && action != 5) {
        if (got_int || compl_interrupted) emsg ((char_u *) (((char *) (e_interr))));
        else if (type == 2) emsg ((char_u *) (((char *) ("E388: Couldn't find definition"))));
        else emsg ((char_u *) (((char *) ("E389: Couldn't find pattern"))));
    }
    if (action == 1 || action == 4) msg_end ();
    fpip_end : vim_free (file_line);
    vim_free (regmatch.regprog);
    ...
    long count;
{
    char_u * p;
    if (did_show) msg_putchar ('\n');
    else if (!msg_silent) gotocmdline (1);
    if (got_int) return;
    for (;;) {
        ...
        if (action == 4) {
            __builtin___sprintf_chk ((char *) IObuff, 0, __builtin_object_size ((char *) IObuff, 1 > 1), "%3ld: ", count);
            msg_puts (IObuff);
            __builtin___sprintf_chk ((char *) IObuff, 0, __builtin_object_size ((char *) IObuff, 1 > 1), "%4ld", *lnum);
            msg_puts_attr (IObuff, highlight_attr [(int) (HLF_N)]);
            msg_puts ((char_u *) (" "));
        }
        msg_prt_line (line, 0);
        out_flush ();
        if (got_int || type != 2 || p < line || *p != '\\') break;
        if (fp != ((void *) 0)) {
            if (vim_fgets (line, 512, fp)) break;
            ++*lnum;
        }
        else {
    ...
    int force;
{
    ...
    if (lp [0] == '~' && (lp [1] == 'm' || lp [1] == 'M')) {
        if (lp [1] == 'M') magic = 1;
        if (lp [2] == 's') no_scs = 1;
        if (lp [3] == 'L') off_line = 1;
        if (lp [4] == 'E') off_end = 0x40;
        lp += 5;
        off = getdigits (&lp);
    }
    if (lp [0] == '~') {
    ...
    else if (lp [0] == 'h') hlsearch_on = 0;
    else if (lp [0] == 'H') hlsearch_on = 1;
    if (idx >= 0) {
        if (force || spats [idx].pat == ((void *) 0)) {
            ...
            if (val != ((void *) 0)) {
                ...
                if (setlast) no_hlsearch = !hlsearch_on;
            }
        }
    }
    ...
    FILE * fp;
{
    if (get_viminfo_parameter ('/') != 0) {
        fprintf (fp, "\n# hlsearch on (H) or off (h):\n~%c", (no_hlsearch || find_viminfo_parameter ('h') != ((void *) 0)) ? 'h' : 'H');
        wvsp_one (fp, 0, "", '/');
        wvsp_one (fp, 1, ((char *) ("Substitute ")), '&');
    }
}
    ...
    int sc;
{
    if (spats [idx].pat != ((void *) 0)) {
        ...
        fprintf (fp, "%c%c%c%c%ld%s%c", spats [idx].magic ? 'M' : 'm', spats [idx].no_scs ? 's' : 'S', spats [idx].off.line ? 'L' : 'l', spats [idx].off.end ?
          'E' : 'e', spats [idx].off.off, last_idx == idx ? "~" : "", sc);
