    ...
    int flags;
{
    ...
    off_t size;
    struct stat stf;
    if ((mfp = (memfile_T *) alloc ((unsigned) sizeof (memfile_T))) == ((void *) 0)) return ((void *) 0);
    if (fname == ((void *) 0)) {
        mfp -> mf_fname = ((void *) 0);
    ...
    mfp -> mf_used_count = 0;
    for (i = 0; i < 64; ++i) {
        mfp -> mf_hash [i] = ((void *) 0);
        mfp -> mf_trans [i] = ((void *) 0);
    }
    mfp -> mf_page_size = 4096;
    mfp -> mf_old_key = ((void *) 0);
    if (mfp -> mf_fd >= 0 && fstat (((mfp -> mf_fd)), ((&stf))) == 0 && stf.st_blksize >= 1048 && stf.st_blksize <= 50000) mfp -> mf_page_size = stf.st_blksize
      ;
    if (mfp -> mf_fd < 0 || (flags & (0x0400 | 0x0800)) || (size = lseek (mfp -> mf_fd, (off_t) 0L, 2)) <= 0) mfp -> mf_blocknr_max = 0;
    else mfp -> mf_blocknr_max = (blocknr_T) ((size + mfp -> mf_page_size - 1) / mfp -> mf_page_size);
    ...
    mfp -> mf_infile_count = mfp -> mf_blocknr_max;
    {
        ...
        if (mfp -> mf_used_count_max < 10) mfp -> mf_used_count_max = 10;
    }
    return mfp;
}
    ...
    int del_file;
{
    ...
    if (mfp -> mf_fd >= 0) {
        if (close (mfp -> mf_fd) < 0) emsg ((char_u *) (((char *) (e_swapclose))));
    }
    if (del_file && mfp -> mf_fname != ((void *) 0)) unlink ((char *) (mfp -> mf_fname));
    for (hp = mfp -> mf_used_first; hp != ((void *) 0); hp = nextp) {
        total_mem_used -= hp -> bh_page_count * mfp -> mf_page_size;
        nextp = hp -> bh_next;
        mf_free_bhdr (hp);
    }
    while (mfp -> mf_free_first != ((void *) 0)) vim_free (mf_rem_free (mfp));
    for (i = 0; i < 64; ++i) for (tp = mfp -> mf_trans [i]; tp != ((void *) 0); tp = tpnext) {
        tpnext = tp -> nt_next;
        vim_free (tp);
    }
    ...
    int getlines;
{
    ...
    linenr_T lnum;
    mfp = buf -> b_ml.ml_mfp;
    if (mfp == ((void *) 0) || mfp -> mf_fd < 0) return;
    if (getlines) {
        mf_dont_release = 1;
        for (lnum = 1; lnum <= buf -> b_ml.ml_line_count; ++lnum) (void) ml_get_buf (buf, lnum, 0);
        mf_dont_release = 0;
    }
    if (close (mfp -> mf_fd) < 0) emsg ((char_u *) (((char *) (e_swapclose))));
    mfp -> mf_fd = -1;
    ...
    int page_count;
{
    ...
    if (!negative && freep != ((void *) 0) && freep -> bh_page_count >= page_count) {
        if (freep -> bh_page_count > page_count) {
            if (hp == ((void *) 0) && (hp = mf_alloc_bhdr (mfp, page_count)) == ((void *) 0)) return ((void *) 0);
            hp -> bh_bnum = freep -> bh_bnum;
            freep -> bh_bnum += page_count;
            freep -> bh_page_count -= page_count;
        }
        else if (hp == ((void *) 0)) {
            if ((p = (char_u *) alloc (mfp -> mf_page_size * page_count)) == ((void *) 0)) return ((void *) 0);
            hp = mf_rem_free (mfp);
            hp -> bh_data = p;
        }
        else {
    ...
    mf_ins_used (mfp, hp);
    mf_ins_hash (mfp, hp);
    (void) ((__builtin_object_size (((char *) (hp -> bh_data)), 0) != (size_t) - 1) ? __builtin___memset_chk (((char *) (hp -> bh_data)), (0), ((size_t) mfp ->
      mf_page_size * page_count), __builtin_object_size (((char *) (hp -> bh_data)), 0)) : __inline_memset_chk (((char *) (hp -> bh_data)), (0), ((size_t) mfp
      -> mf_page_size * page_count)));
    return hp;
}
...
{
    bhdr_T * hp;
    if (nr >= mfp -> mf_blocknr_max || nr <= mfp -> mf_blocknr_min) return ((void *) 0);
    hp = mf_find_hash (mfp, nr);
    if (hp == ((void *) 0)) {
        if (nr < 0 || nr >= mfp -> mf_infile_count) return ((void *) 0);
        hp = mf_release (mfp, page_count);
        if (hp == ((void *) 0) && (hp = mf_alloc_bhdr (mfp, page_count)) == ((void *) 0)) return ((void *) 0);
        hp -> bh_bnum = nr;
        hp -> bh_flags = 0;
    ...
    int infile;
{
    int flags;
    flags = hp -> bh_flags;
    if ((flags & 2) == 0) emsg ((char_u *) (((char *) ("E293: block was not locked"))));
    flags &= ~2;
    if (dirty) {
        flags |= 1;
        mfp -> mf_dirty = 1;
    ...
    int flags;
{
    ...
    got_int = 0;
    status = 1;
    for (hp = mfp -> mf_used_last; hp != ((void *) 0); hp = hp -> bh_prev) if (((flags & 1) || hp -> bh_bnum >= 0) && (hp -> bh_flags & 1) && (status == 1 || (
      hp -> bh_bnum >= 0 && hp -> bh_bnum < mfp -> mf_infile_count))) {
        if ((flags & 8) && hp -> bh_bnum != 0) continue;
        if (mf_write (mfp, hp) == 0) {
            if (status == 0) break;
            status = 0;
        }
        if (flags & 2) {
            if (ui_char_avail ()) break;
        }
        else ui_breakcheck ();
        if (got_int) break;
    }
    if (hp == ((void *) 0) || status == 0) mfp -> mf_dirty = 0;
    if ((flags & 4) && *p_sws != '\000') {
        if (strcmp ((char *) (p_sws), (char *) ("fsync")) == 0) {
            if (fsync (mfp -> mf_fd)) status = 0;
        }
        else sync ();
    }
...
{
    bhdr_T * hp;
    for (hp = mfp -> mf_used_last; hp != ((void *) 0); hp = hp -> bh_prev) if (hp -> bh_bnum > 0) hp -> bh_flags |= 1;
    mfp -> mf_dirty = 1;
}
    ...
    bhdr_T * hp;
{
    bhdr_T * hhp;
    int hash;
    hash = ((hp -> bh_bnum) & (64 - 1));
    hhp = mfp -> mf_hash [hash];
    hp -> bh_hash_next = hhp;
    hp -> bh_hash_prev = ((void *) 0);
    if (hhp != ((void *) 0)) hhp -> bh_hash_prev = hp;
    mfp -> mf_hash [hash] = hp;
}
...
static void mf_rem_hash (mfp, hp)
    ...
    bhdr_T * hp;
{
    if (hp -> bh_hash_prev == ((void *) 0)) mfp -> mf_hash [((hp -> bh_bnum) & (64 - 1))] = hp -> bh_hash_next;
    else hp -> bh_hash_prev -> bh_hash_next = hp -> bh_hash_next;
    if (hp -> bh_hash_next) hp -> bh_hash_next -> bh_hash_prev = hp -> bh_hash_prev;
}
...
{
    bhdr_T * hp;
    for (hp = mfp -> mf_hash [((nr) & (64 - 1))]; hp != ((void *) 0); hp = hp -> bh_hash_next) if (hp -> bh_bnum == nr) break;
    return hp;
}
    ...
    bhdr_T * hp;
{
    ...
    hp -> bh_prev = ((void *) 0);
    if (hp -> bh_next == ((void *) 0)) mfp -> mf_used_last = hp;
    else hp -> bh_next -> bh_prev = hp;
...
static void mf_rem_used (mfp, hp)
    ...
    bhdr_T * hp;
{
    if (hp -> bh_next == ((void *) 0)) mfp -> mf_used_last = hp -> bh_prev;
    else hp -> bh_next -> bh_prev = hp -> bh_prev;
    if (hp -> bh_prev == ((void *) 0)) mfp -> mf_used_first = hp -> bh_next;
    else hp -> bh_prev -> bh_next = hp -> bh_next;
    mfp -> mf_used_count -= hp -> bh_page_count;
    total_mem_used -= hp -> bh_page_count * mfp -> mf_page_size;
}
    ...
    int page_count;
{
    ...
    int need_release;
    buf_T * buf;
    if (mf_dont_release) return ((void *) 0);
    need_release = ((mfp -> mf_used_count >= mfp -> mf_used_count_max) || (total_mem_used >> 10) >= (long_u) p_mmt);
    if (mfp -> mf_fd < 0 && need_release && p_uc) {
        for (buf = firstbuf; buf != ((void *) 0); buf = buf -> b_next) if (buf -> b_ml.ml_mfp == mfp) break;
        if (buf != ((void *) 0) && buf -> b_may_swap) ml_open_file (buf);
    }
    if (mfp -> mf_fd < 0 || !need_release) return ((void *) 0);
    for (hp = mfp -> mf_used_last; hp != ((void *) 0); hp = hp -> bh_prev) if (!(hp->bh_flags&2)) break;
    if (hp == ((void *) 0)) return ((void *) 0);
    if ((hp -> bh_flags & 1) && mf_write (mfp, hp) == 0) return ((void *) 0);
    mf_rem_used (mfp, hp);
    mf_rem_hash (mfp, hp);
    if (hp -> bh_page_count != page_count) {
        vim_free (hp -> bh_data);
        if ((hp -> bh_data = alloc (mfp -> mf_page_size * page_count)) == ((void *) 0)) {
            vim_free (hp);
            return ((void *) 0);
        }
        hp -> bh_page_count = page_count;
...
int mf_release_all () {
    ...
    for (buf = firstbuf; buf != ((void *) 0); buf = buf -> b_next) {
        mfp = buf -> b_ml.ml_mfp;
        if (mfp != ((void *) 0)) {
            if (mfp -> mf_fd < 0 && buf -> b_may_swap) ml_open_file (buf);
            if (mfp -> mf_fd >= 0) {
                for (hp = mfp -> mf_used_last; hp != ((void *) 0);) {
                    if (!(hp->bh_flags&2) && (!(hp->bh_flags&1) || mf_write (mfp, hp) != 0)) {
                        mf_rem_used (mfp, hp);
                        mf_rem_hash (mfp, hp);
                        mf_free_bhdr (hp);
                        hp = mfp -> mf_used_last;
...
{
    bhdr_T * hp;
    if ((hp = (bhdr_T *) alloc ((unsigned) sizeof (bhdr_T))) != ((void *) 0)) {
        if ((hp -> bh_data = (char_u *) alloc (mfp -> mf_page_size * page_count)) == ((void *) 0)) {
            vim_free (hp);
            return ((void *) 0);
        }
        hp -> bh_page_count = page_count;
    ...
    bhdr_T * hp;
{
    ...
    if (lseek (mfp -> mf_fd, offset, 0) != offset) {
        (void) emsg3 ((char_u *) "%s: %s", (char_u *) ((char *) ("E294: Seek error in swap file read")), (char_u *) strerror ((*__error())));
        return 0;
    }
    if ((unsigned) read ((mfp -> mf_fd), (char *) (hp -> bh_data), (size_t) (size)) != size) {
        (void) emsg3 ((char_u *) "%s: %s", (char_u *) ((char *) ("E295: Read error in swap file")), (char_u *) strerror ((*__error())));
        return 0;
    }
    if (*mfp->mf_buffer->b_p_key != '\000') ml_decrypt_data (mfp, hp -> bh_data, offset, size);
    return 1;
}
    ...
    bhdr_T * hp;
{
    ...
    unsigned size;
    if (mfp -> mf_fd < 0) return 0;
    if (hp -> bh_bnum < 0) if (mf_trans_add (mfp, hp) == 0) return 0;
    page_size = mfp -> mf_page_size;
    for (;;) {
        nr = hp -> bh_bnum;
        ...
        if (lseek (mfp -> mf_fd, offset, 0) != offset) {
            (void) emsg3 ((char_u *) "%s: %s", (char_u *) ((char *) ("E296: Seek error in swap file write")), (char_u *) strerror ((*__error())));
            return 0;
        }
        if (hp2 == ((void *) 0)) page_count = 1;
        else page_count = hp2 -> bh_page_count;
        size = page_size * page_count;
        if (mf_write_block (mfp, hp2 == ((void *) 0) ? hp : hp2, offset, size) == 0) {
            if (!did_swapwrite_msg) emsg ((char_u *) (((char *) ("E297: Write error in swap file"))));
            did_swapwrite_msg = 1;
            return 0;
        }
        did_swapwrite_msg = 0;
        if (hp2 != ((void *) 0)) hp2 -> bh_flags &= ~1;
        if (nr + (blocknr_T) page_count > mfp -> mf_infile_count) mfp -> mf_infile_count = nr + page_count;
    ...
    unsigned size;
{
    ...
    int result = 1;
    if (*mfp->mf_buffer->b_p_key != '\000') {
        data = ml_encrypt_data (mfp, data, offset, size);
        if (data == ((void *) 0)) return 0;
    }
    if ((unsigned) write ((mfp -> mf_fd), (char *) (data), (size_t) (size)) != size) result = 0;
    if (data != hp -> bh_data) vim_free (data);
    return result;
}
    ...
    bhdr_T * hp;
{
    ...
    NR_TRANS * np;
    int page_count;
    if (hp -> bh_bnum >= 0) return 1;
    if ((np = (NR_TRANS *) alloc ((unsigned) sizeof (NR_TRANS))) == ((void *) 0)) return 0;
    freep = mfp -> mf_free_first;
    ...
    hp -> bh_bnum = new_bnum;
    mf_ins_hash (mfp, hp);
    hash = ((np -> nt_old_bnum) & (64 - 1));
    np -> nt_next = mfp -> mf_trans [hash];
    mfp -> mf_trans [hash] = np;
    if (np -> nt_next != ((void *) 0)) np -> nt_next -> nt_prev = np;
    np -> nt_prev = ((void *) 0);
    return 1;
}
    ...
    blocknr_T old_nr;
{
    ...
    blocknr_T new_bnum;
    hash = ((old_nr) & (64 - 1));
    for (np = mfp -> mf_trans [hash]; np != ((void *) 0); np = np -> nt_next) if (np -> nt_old_bnum == old_nr) break;
    if (np == ((void *) 0)) return old_nr;
    mfp -> mf_neg_count --;
    new_bnum = np -> nt_new_bnum;
    if (np -> nt_prev != ((void *) 0)) np -> nt_prev -> nt_next = np -> nt_next;
    else mfp -> mf_trans [hash] = np -> nt_next;
    if (np -> nt_next != ((void *) 0)) np -> nt_next -> nt_prev = np -> nt_prev;
    vim_free (np);
    return new_bnum;
}
    ...
    int flags;
{
    ...
    mfp -> mf_fname = fname;
    mf_set_ffname (mfp);
    if ((flags & 0x0200) && lstat (((char *) mfp -> mf_fname), (&sb)) >= 0) {
        mfp -> mf_fd = -1;
        emsg ((char_u *) (((char *) ("E300: Swap file already exists (symlink attack?)"))));
    }
    else {
        flags |= 0 | 0x0100;
