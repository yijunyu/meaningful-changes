...
int u_save (top, bot)
    linenr_T top, bot;
{
    if (undo_off) return 1;
    if (top > curbuf -> b_ml.ml_line_count || top >= bot || bot > curbuf -> b_ml.ml_line_count + 1) return 0;
    if (top + 2 == bot) u_saveline ((linenr_T) (top + 1));
...
{
    if (undo_off) return 1;
    return (u_savecommon (lnum - 1, lnum + nlines, nlines == curbuf -> b_ml.ml_line_count ? 2 : lnum, 0));
}
    ...
    int reload;
{
    ...
    if (curbuf -> b_u_synced) {
        ...
        while (curbuf -> b_u_numhead > p_ul && curbuf -> b_u_oldhead != ((void *) 0)) {
            ...
            if (uhfree == old_curhead) u_freebranch (curbuf, uhfree, &old_curhead);
            else if (uhfree -> uh_alt_next.ptr == ((void *) 0)) u_freeheader (curbuf, uhfree, &old_curhead);
            else {
                while (uhfree -> uh_alt_next.ptr != ((void *) 0)) uhfree = uhfree -> uh_alt_next.ptr;
                u_freebranch (curbuf, uhfree, &old_curhead);
            }
        }
        if (uhp == ((void *) 0)) {
            if (old_curhead != ((void *) 0)) u_freebranch (curbuf, old_curhead, ((void *) 0));
            curbuf -> b_u_synced = 0;
            return 1;
        }
        ...
        if (old_curhead != ((void *) 0)) {
            ...
            if (uhp -> uh_alt_prev.ptr != ((void *) 0)) uhp -> uh_alt_prev.ptr -> uh_alt_next.ptr = uhp;
            old_curhead -> uh_alt_prev.ptr = uhp;
            if (curbuf -> b_u_oldhead == old_curhead) curbuf -> b_u_oldhead = uhp;
        }
        else uhp -> uh_alt_prev.ptr = ((void *) 0);
        if (curbuf -> b_u_newhead != ((void *) 0)) curbuf -> b_u_newhead -> uh_prev.ptr = uhp;
        ...
        if (virtual_active () && curwin -> w_cursor.coladd > 0) uhp -> uh_cursor_vcol = getviscol ();
        else uhp -> uh_cursor_vcol = -1;
        uhp -> uh_flags = (curbuf -> b_changed ? 0x01 : 0) + ((curbuf -> b_ml.ml_flags & 1) ? 0x02 : 0);
        ((__builtin_object_size ((char *) (uhp -> uh_namedm), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (uhp -> uh_namedm), (char *) (curbuf ->
          b_namedm), sizeof (pos_T) * ('z' - 'a' + 1), __builtin_object_size ((char *) (uhp -> uh_namedm), 0)) : __inline_memmove_chk ((char *) (uhp ->
          uh_namedm), (char *) (curbuf -> b_namedm), sizeof (pos_T) * ('z' - 'a' + 1)));
        uhp -> uh_visual = curbuf -> b_visual;
        curbuf -> b_u_newhead = uhp;
        if (curbuf -> b_u_oldhead == ((void *) 0)) curbuf -> b_u_oldhead = uhp;
        ++curbuf->b_u_numhead;
    }
    else {
        if (p_ul < 0) return 1;
        if (size == 1) {
            ...
            for (i = 0; i < 10; ++i) {
                if (uep == ((void *) 0)) break;
                if ((curbuf -> b_u_newhead -> uh_getbot_entry != uep ? (uep -> ue_top + uep -> ue_size + 1 != (uep -> ue_bot == 0 ? curbuf -> b_ml.
                  ml_line_count + 1 : uep -> ue_bot)) : uep -> ue_lcount != curbuf -> b_ml.ml_line_count) || (uep -> ue_size > 1 && top >= uep -> ue_top && top
                  + 2 <= uep -> ue_top + uep -> ue_size + 1)) break;
                if (uep -> ue_size == 1 && uep -> ue_top == top) {
                    ...
                    if (newbot != 0) uep -> ue_bot = newbot;
                    else if (bot > curbuf -> b_ml.ml_line_count) uep -> ue_bot = 0;
                    else {
    ...
    if (uep == ((void *) 0)) goto nomem;
    ((__builtin_object_size ((uep), 0) != (size_t) - 1) ? __builtin___memset_chk ((uep), (0), (sizeof (u_entry_T)), __builtin_object_size ((uep), 0)) :
      __inline_memset_chk ((uep), (0), (sizeof (u_entry_T))));
    uep -> ue_size = size;
    uep -> ue_top = top;
    if (newbot != 0) uep -> ue_bot = newbot;
    else if (bot > curbuf -> b_ml.ml_line_count) uep -> ue_bot = 0;
    ...
    return 1;
    nomem : msg_silent = 0;
    if (ask_yesno ((char_u *) ((char *) ("No undo possible; continue anyway")), 1) == 'y') {
        undo_off = 1;
        return 1;
    }
    ...
    int reading;
{
    ...
    if (ffname == ((void *) 0)) return ((void *) 0);
    if (resolve_symlink (ffname, fname_buf) == 1) ffname = fname_buf;
    dirp = p_udir;
    while (*dirp != '\000') {
        dir_len = copy_option_part (&dirp, dir_name, (1024 + 1), ",");
        if (dir_len == 1 && dir_name [0] == '.') {
            undo_file_name = vim_strnsave (ffname, (int) (strlen ((char *) (ffname)) + 5));
            if (undo_file_name == ((void *) 0)) break;
            p = gettail (undo_file_name);
            ((__builtin_object_size ((char *) (p + 1), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (p + 1), (char *) (p), strlen ((char *) (p)) + 1
              , __builtin_object_size ((char *) (p + 1), 0)) : __inline_memmove_chk ((char *) (p + 1), (char *) (p), strlen ((char *) (p)) + 1));
            * p = '.';
            ((__builtin_object_size ((char *) (p), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (p), (char *) (".un~"), __builtin_object_size ((char *
              ) (p), 1 > 1)) : __inline_strcat_chk ((char *) (p), (char *) (".un~")));
        }
        else {
            ...
            if (mch_isdir (dir_name)) {
                if (munged_name == ((void *) 0)) {
                    ...
                    if (munged_name == ((void *) 0)) return ((void *) 0);
                    for (p = munged_name; *p != '\000'; ++p) if (vim_ispathsep (*p)) *p = '%';
                }
                undo_file_name = concat_fnames (dir_name, munged_name, 1);
            }
        }
        if (undo_file_name != ((void *) 0) && (!reading || stat (((char *) undo_file_name), (&st)) >= 0)) break;
    ...
    FILE * fp;
{
    ...
    size_t i;
    if (*buf->b_p_key == '\000') return fwrite (ptr, len, (size_t) 1, fp);
    if (len < 100) copy = small_buf;
    else {
        copy = lalloc (len, 0);
        if (copy == ((void *) 0)) return 0;
    }
    crypt_encode (ptr, len, copy);
    i = fwrite (copy, len, (size_t) 1, fp);
    if (copy != small_buf) vim_free (copy);
    return i;
}
    ...
    int len;
{
    ...
    if (ptr != ((void *) 0) && *buf->b_p_key != '\000') crypt_decode (ptr, len);
    return ptr;
}
...
{
    int len;
    if (fwrite ("Vim\237UnDo\345", (size_t) 9, (size_t) 1, fp) != 1) return 0;
    if (*buf->b_p_key != '\000') {
        char_u * header;
        int header_len;
    ...
    else put_bytes (fp, (long_u) 2, 2);
    if (fwrite (hash, (size_t) 32, (size_t) 1, fp) != 1) return 0;
    put_bytes (fp, (long_u) buf -> b_ml.ml_line_count, 4);
    len = buf -> b_u_line_ptr != ((void *) 0) ? (int) strlen ((char *) (buf -> b_u_line_ptr)) : 0;
    put_bytes (fp, (long_u) len, 4);
    if (len > 0 && fwrite_crypt (buf, buf -> b_u_line_ptr, (size_t) len, fp) != 1) return 0;
    ...
    u_header_T * uhp;
{
    int i;
    u_entry_T * uep;
    if (put_bytes (fp, (long_u) 0x5fd0, 2) == 0) return 0;
    ...
    for (i = 0; i < ('z' - 'a' + 1); ++i) serialize_pos (uhp -> uh_namedm [i], fp);
    ...
    for (uep = uhp -> uh_entry; uep != ((void *) 0); uep = uep -> ue_next) {
        ...
        if (serialize_uep (fp, buf, uep) == 0) return 0;
    }
    ...
    char_u * file_name;
{
    ...
    int error;
    uhp = (u_header_T *) lalloc ((long_u) (sizeof (u_header_T)), 0);
    if (uhp == ((void *) 0)) return ((void *) 0);
    ((__builtin_object_size ((uhp), 0) != (size_t) - 1) ? __builtin___memset_chk ((uhp), (0), (sizeof (u_header_T)), __builtin_object_size ((uhp), 0)) :
      __inline_memset_chk ((uhp), (0), (sizeof (u_header_T))));
    ...
    for (i = 0; i < ('z' - 'a' + 1); ++i) unserialize_pos (&uhp->uh_namedm[i], fp);
    ...
    while ((c = get2c (fp)) == 0xf518) {
        ...
        if (last_uep == ((void *) 0)) uhp -> uh_entry = uep;
    ...
    u_entry_T * uep;
{
    ...
    for (i = 0; i < uep -> ue_size; ++i) {
        ...
        if (put_bytes (fp, (long_u) len, 4) == 0) return 0;
        if (len > 0 && fwrite_crypt (buf, uep -> ue_array [i], len, fp) != 1) return 0;
    }
    return 1;
}
    ...
    char_u * file_name;
{
    ...
    if (uep -> ue_size > 0) {
        ...
        ((__builtin_object_size ((array), 0) != (size_t) - 1) ? __builtin___memset_chk ((array), (0), (sizeof (char_u *) *uep->ue_size), __builtin_object_size (
          (array), 0)) : __inline_memset_chk ((array), (0), (sizeof (char_u *) *uep->ue_size)));
    }
    else array = ((void *) 0);
    uep -> ue_array = array;
    for (i = 0; i < uep -> ue_size; ++i) {
        line_len = get4c (fp);
        if (line_len >= 0) line = read_string_decrypt (curbuf, fp, line_len);
        else {
            line = ((void *) 0);
...
{
    pos -> lnum = get4c (fp);
    if (pos -> lnum < 0) pos -> lnum = 0;
    pos -> col = get4c (fp);
    if (pos -> col < 0) pos -> col = 0;
    pos -> coladd = get4c (fp);
    if (pos -> coladd < 0) pos -> coladd = 0;
}
    ...
    char_u * hash;
{
    ...
    if (name == ((void *) 0)) {
        ...
        if (file_name == ((void *) 0)) {
            if (p_verbose > 0) {
                verbose_enter ();
                smsg ((char_u *) ((char *) ("Cannot write undo file in any directory in 'undodir'")));
                verbose_leave ();
            }
            return;
        }
        ...
        if (name == ((void *) 0) || !forceit) {
            ...
            if (fd < 0) {
                if (name != ((void *) 0) || p_verbose > 0) {
                    if (name == ((void *) 0)) verbose_enter ();
                    smsg ((char_u *) ((char *) ("Will not overwrite with undo file, cannot read: %s")), file_name);
                    if (name == ((void *) 0)) verbose_leave ();
                }
                goto theend;
            }
            else {
                ...
                if (len < 9 || memcmp (mbuf, "Vim\237UnDo\345", 9) != 0) {
                    if (name != ((void *) 0) || p_verbose > 0) {
                        if (name == ((void *) 0)) verbose_enter ();
                        smsg ((char_u *) ((char *) ("Will not overwrite, this is not an undo file: %s")), file_name);
                        if (name == ((void *) 0)) verbose_leave ();
                    }
                    goto theend;
                }
    ...
    if (buf -> b_u_numhead == 0 && buf -> b_u_line_ptr == ((void *) 0)) {
        if (p_verbose > 0) verb_msg ((char_u *) ((char *) ("Skipping undo file write, nothing to undo")));
        goto theend;
    }
    ...
    if (st_old_valid && stat (((char *) file_name), (&st_new)) >= 0 && st_new.st_gid != st_old.st_gid && fchown (fd, (uid_t) - 1, st_old.st_gid) != 0)
    ...
    u_sync (1);
    if (serialize_header (fp, buf, hash) == 0) goto write_error;
    if (*buf->b_p_key != '\000') do_crypt = 1;
    mark = ++lastmark;
    uhp = buf -> b_u_oldhead;
    while (uhp != ((void *) 0)) {
        if (uhp -> uh_walk != mark) {
            uhp -> uh_walk = mark;
            if (serialize_uhp (fp, buf, uhp) == 0) goto write_error;
        }
        if (uhp -> uh_prev.ptr != ((void *) 0) && uhp -> uh_prev.ptr -> uh_walk != mark) uhp = uhp -> uh_prev.ptr;
        else if (uhp -> uh_alt_next.ptr != ((void *) 0) && uhp -> uh_alt_next.ptr -> uh_walk != mark) uhp = uhp -> uh_alt_next.ptr;
        else if (uhp -> uh_next.ptr != ((void *) 0) && uhp -> uh_alt_prev.ptr == ((void *) 0) && uhp -> uh_next.ptr -> uh_walk != mark) uhp = uhp -> uh_next.
          ptr;
        else if (uhp -> uh_alt_prev.ptr != ((void *) 0)) uhp = uhp -> uh_alt_prev.ptr;
        else uhp = uhp -> uh_next.ptr;
    }
    if (put_bytes (fp, (long_u) 0xe7aa, 2) == 1) write_ok = 1;
    write_error : fclose (fp);
    if (!write_ok) emsg2 ((char_u *) (((char *) ("E829: write error in undo file: %s"))), (char_u *) (file_name));
    if (buf -> b_ffname != ((void *) 0)) {
        vim_acl_T acl;
        acl = mch_get_acl (buf -> b_ffname);
        mch_set_acl (file_name, acl);
    }
    theend : if (do_crypt) crypt_pop_state ();
    if (file_name != name) vim_free (file_name);
}
    ...
    char_u * orig_name;
{
    ...
    if (name == ((void *) 0)) {
        ...
        if (file_name == ((void *) 0)) return;
        if (stat (((char *) orig_name), (&st_orig)) >= 0 && stat (((char *) file_name), (&st_undo)) >= 0 && st_orig.st_uid != st_undo.st_uid) {
            if (p_verbose > 0) {
                verbose_enter ();
                smsg ((char_u *) ((char *) ("Not reading undo file, owner differs: %s")), file_name);
                verbose_leave ();
            }
            return;
        }
    ...
    if (fp == ((void *) 0)) {
        if (name != ((void *) 0) || p_verbose > 0) emsg2 ((char_u *) (((char *) ("E822: Cannot open undo file for reading: %s"))), (char_u *) (file_name));
        goto error;
    }
    if (fread (magic_buf, 9, 1, fp) != 1 || memcmp (magic_buf, "Vim\237UnDo\345", 9) != 0) {
    ...
    if (memcmp (hash, read_hash, 32) != 0 || line_count != curbuf -> b_ml.ml_line_count) {
        if (p_verbose > 0 || name != ((void *) 0)) {
            if (name == ((void *) 0)) verbose_enter ();
            give_warning ((char_u *) ((char *) ("File contents changed, cannot use undo info")), 1);
            if (name == ((void *) 0)) verbose_leave ();
        }
        goto error;
    }
    str_len = get4c (fp);
    if (str_len < 0) goto error;
    if (str_len > 0) line_ptr = read_string_decrypt (curbuf, fp, str_len);
    ...
    if (num_head > 0) {
        ...
        if (uhp_table == ((void *) 0)) goto error;
    }
    ...
    for (i = 0; i < num_head; i ++) {
        uhp = uhp_table [i];
        if (uhp == ((void *) 0)) continue;
        for (j = 0; j < num_head; j ++) if (uhp_table [j] != ((void *) 0) && i != j && uhp_table [i] -> uh_seq == uhp_table [j] -> uh_seq) {
            corruption_error ("duplicate uh_seq", file_name);
            goto error;
        }
        for (j = 0; j < num_head; j ++) if (uhp_table [j] != ((void *) 0) && uhp_table [j] -> uh_seq == uhp -> uh_next.seq) {
            uhp -> uh_next.ptr = uhp_table [j];
            ;
            break;
        }
        for (j = 0; j < num_head; j ++) if (uhp_table [j] != ((void *) 0) && uhp_table [j] -> uh_seq == uhp -> uh_prev.seq) {
            uhp -> uh_prev.ptr = uhp_table [j];
            ;
            break;
        }
        for (j = 0; j < num_head; j ++) if (uhp_table [j] != ((void *) 0) && uhp_table [j] -> uh_seq == uhp -> uh_alt_next.seq) {
            uhp -> uh_alt_next.ptr = uhp_table [j];
            ;
            break;
        }
        for (j = 0; j < num_head; j ++) if (uhp_table [j] != ((void *) 0) && uhp_table [j] -> uh_seq == uhp -> uh_alt_prev.seq) {
    ...
    curbuf -> b_u_synced = 1;
    vim_free (uhp_table);
    if (name != ((void *) 0)) smsg ((char_u *) ((char *) ("Finished reading undo file %s")), file_name);
    goto theend;
    error : vim_free (line_ptr);
    if (uhp_table != ((void *) 0)) {
        for (i = 0; i < num_read_uhps; i ++) if (uhp_table [i] != ((void *) 0)) u_free_uhp (uhp_table [i]);
        vim_free (uhp_table);
    }
    theend : if (do_decrypt) crypt_pop_state ();
    ...
    int count;
{
    ...
    if (vim_strchr (p_cpo, 'u') == ((void *) 0)) undo_undoes = 1;
...
void u_redo (count)
    int count;
{
    if (vim_strchr (p_cpo, 'u') == ((void *) 0)) undo_undoes = 0;
    u_doit (count);
}
    ...
    int startcount;
{
    ...
    u_newcount = 0;
    u_oldcount = 0;
    if (curbuf -> b_ml.ml_flags & 1) u_oldcount = -1;
    while (count --) {
        change_warning (0);
        if (undo_undoes) {
            if (curbuf -> b_u_curhead == ((void *) 0)) curbuf -> b_u_curhead = curbuf -> b_u_newhead;
            else if (p_ul > 0) curbuf -> b_u_curhead = curbuf -> b_u_curhead -> uh_next.ptr;
            ...
            u_undoredo (0);
            if (curbuf -> b_u_curhead -> uh_prev.ptr == ((void *) 0)) curbuf -> b_u_newhead = curbuf -> b_u_curhead;
    ...
    int absolute;
{
    ...
    int above = 0;
    int did_undo = 1;
    if (curbuf -> b_u_synced == 0) u_sync (1);
    u_newcount = 0;
    u_oldcount = 0;
    if (curbuf -> b_ml.ml_flags & 1) u_oldcount = -1;
    if (absolute) {
        ...
        if (dosec) target = (long) (curbuf -> b_u_time_cur - starttime) + step;
        else if (dofile) {
            if (step < 0) {
                uhp = curbuf -> b_u_curhead;
                if (uhp != ((void *) 0)) uhp = uhp -> uh_next.ptr;
                else uhp = curbuf -> b_u_newhead;
                if (uhp != ((void *) 0) && uhp -> uh_save_nr != 0) target = curbuf -> b_u_save_nr_cur + step;
                else target = curbuf -> b_u_save_nr_cur + step + 1;
                if (target <= 0) dofile = 0;
            }
            else {
        ...
        if (step < 0) {
            ...
            if (dosec) closest = (long) (time (((void *) 0)) - starttime + 1);
            else if (dofile) closest = curbuf -> b_u_save_nr_last + 2;
            else closest = curbuf -> b_u_seq_last + 2;
            if (target >= closest) target = closest - 1;
        }
    }
    ...
    for (round = 1; round <= 2; ++round) {
        mark = ++lastmark;
        nomark = ++lastmark;
        if (curbuf -> b_u_curhead == ((void *) 0)) uhp = curbuf -> b_u_newhead;
        else uhp = curbuf -> b_u_curhead;
        while (uhp != ((void *) 0)) {
            uhp -> uh_walk = mark;
            if (dosec) val = (long) (uhp -> uh_time - starttime);
            else if (dofile) val = uhp -> uh_save_nr;
            else val = uhp -> uh_seq;
            if (round == 1 && !(dofile&&val==0)) {
                if ((step < 0 ? uhp -> uh_seq <= curbuf -> b_u_seq_cur : uhp -> uh_seq > curbuf -> b_u_seq_cur) && ((dosec && val == closest) ? (step < 0 ? uhp
                  -> uh_seq < closest_seq : uhp -> uh_seq > closest_seq) : closest == closest_start || (val > target ? (closest > target ? val - target <=
                  closest - target : val - target <= target - closest) : (closest > target ? target - val <= closest - target : target - val <= target - closest
                  )))) {
                    closest = val;
            ...
            if (uhp -> uh_prev.ptr != ((void *) 0) && uhp -> uh_prev.ptr -> uh_walk != nomark && uhp -> uh_prev.ptr -> uh_walk != mark) uhp = uhp -> uh_prev.
              ptr;
            else if (uhp -> uh_alt_next.ptr != ((void *) 0) && uhp -> uh_alt_next.ptr -> uh_walk != nomark && uhp -> uh_alt_next.ptr -> uh_walk != mark) uhp =
              uhp -> uh_alt_next.ptr;
            else if (uhp -> uh_next.ptr != ((void *) 0) && uhp -> uh_alt_prev.ptr == ((void *) 0) && uhp -> uh_next.ptr -> uh_walk != nomark && uhp -> uh_next.
              ptr -> uh_walk != mark) {
                if (uhp == curbuf -> b_u_curhead) uhp -> uh_walk = nomark;
                uhp = uhp -> uh_next.ptr;
            }
            else {
                uhp -> uh_walk = nomark;
                if (uhp -> uh_alt_prev.ptr != ((void *) 0)) uhp = uhp -> uh_alt_prev.ptr;
                else uhp = uhp -> uh_next.ptr;
            }
        }
        if (uhp != ((void *) 0)) break;
        ...
        if (closest == closest_start) {
            if (step < 0) msg ((char_u *) (((char *) ("Already at oldest change"))));
            else msg ((char_u *) (((char *) ("Already at newest change"))));
            return;
        }
        target = closest_seq;
        dosec = 0;
    ...
    if (uhp != ((void *) 0)) {
        while (!got_int) {
            change_warning (0);
            uhp = curbuf -> b_u_curhead;
            if (uhp == ((void *) 0)) uhp = curbuf -> b_u_newhead;
            else uhp = uhp -> uh_next.ptr;
            if (uhp == ((void *) 0) || uhp -> uh_walk != mark || (uhp -> uh_seq == target && !above)) break;
        ...
        while (!got_int) {
            ...
            uhp = curbuf -> b_u_curhead;
            if (uhp == ((void *) 0)) break;
            while (uhp -> uh_alt_prev.ptr != ((void *) 0) && uhp -> uh_alt_prev.ptr -> uh_walk == mark) uhp = uhp -> uh_alt_prev.ptr;
            last = uhp;
            while (last -> uh_alt_next.ptr != ((void *) 0) && last -> uh_alt_next.ptr -> uh_walk == mark) last = last -> uh_alt_next.ptr;
            if (last != uhp) {
                while (uhp -> uh_alt_prev.ptr != ((void *) 0)) uhp = uhp -> uh_alt_prev.ptr;
                if (last -> uh_alt_next.ptr != ((void *) 0)) last -> uh_alt_next.ptr -> uh_alt_prev.ptr = last -> uh_alt_prev.ptr;
                ...
                if (curbuf -> b_u_oldhead == uhp) curbuf -> b_u_oldhead = last;
                uhp = last;
                if (uhp -> uh_next.ptr != ((void *) 0)) uhp -> uh_next.ptr -> uh_prev.ptr = uhp;
            }
            curbuf -> b_u_curhead = uhp;
            if (uhp -> uh_walk != mark) break;
            if (uhp -> uh_seq == target && above) {
                curbuf -> b_u_seq_cur = target - 1;
                break;
            }
            u_undoredo (0);
            if (uhp -> uh_prev.ptr == ((void *) 0)) curbuf -> b_u_newhead = uhp;
            curbuf -> b_u_curhead = uhp -> uh_prev.ptr;
            did_undo = 0;
            if (uhp -> uh_seq == target) break;
            uhp = uhp -> uh_prev.ptr;
    ...
    int undo;
{
    ...
    new_flags = (curbuf -> b_changed ? 0x01 : 0) + ((curbuf -> b_ml.ml_flags & 1) ? 0x02 : 0);
    setpcmark ();
    ((__builtin_object_size ((char *) (namedm), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (namedm), (char *) (curbuf -> b_namedm), sizeof (pos_T)
      * ('z' - 'a' + 1), __builtin_object_size ((char *) (namedm), 0)) : __inline_memmove_chk ((char *) (namedm), (char *) (curbuf -> b_namedm), sizeof (pos_T)
      * ('z' - 'a' + 1)));
    ...
    for (uep = curhead -> uh_entry; uep != ((void *) 0); uep = nuep) {
        top = uep -> ue_top;
        bot = uep -> ue_bot;
        if (bot == 0) bot = curbuf -> b_ml.ml_line_count + 1;
        ...
        if (top < newlnum) {
            ...
            if (lnum >= top && lnum <= top + newsize + 1) {
                ...
                for (i = 0; i < newsize && i < oldsize; ++i) if (strcmp ((char *) (uep -> ue_array [i]), (char *) (ml_get (top + 1 + i))) != 0) break;
        ...
        if (oldsize > 0) {
            ...
            for (lnum = bot - 1, i = oldsize; --i >= 0; --lnum) {
                if ((newarray [i] = u_save_line (lnum)) == ((void *) 0)) do_outofmem_msg ((long_u) 0);
                if (curbuf -> b_ml.ml_line_count == 1) empty_buffer = 1;
                ml_delete (lnum, 0);
            }
        }
        else newarray = ((void *) 0);
        if (newsize) {
            for (lnum = top, i = 0; i < newsize; ++i, ++lnum) {
                if (empty_buffer && lnum == 0) ml_replace ((linenr_T) 1, uep -> ue_array [i], 1);
                else ml_append (lnum, uep -> ue_array [i], (colnr_T) 0, 0);
                vim_free (uep -> ue_array [i]);
            }
            vim_free ((char_u *) uep -> ue_array);
        }
        if (oldsize != newsize) {
            mark_adjust (top + 1, top + oldsize, (long) (0x7fffffffL), (long) newsize - (long) oldsize);
            if (curbuf -> b_op_start.lnum > top + oldsize) curbuf -> b_op_start.lnum += newsize - oldsize;
            if (curbuf -> b_op_end.lnum > top + oldsize) curbuf -> b_op_end.lnum += newsize - oldsize;
        }
        changed_lines (top + 1, 0, bot, newsize - oldsize);
        if (top + 1 < curbuf -> b_op_start.lnum) curbuf -> b_op_start.lnum = top + 1;
        if (newsize == 0 && top + 1 > curbuf -> b_op_end.lnum) curbuf -> b_op_end.lnum = top + 1;
        else if (top + newsize > curbuf -> b_op_end.lnum) curbuf -> b_op_end.lnum = top + newsize;
        u_newcount += newsize;
        u_oldcount += oldsize;
        uep -> ue_size = oldsize;
    ...
    if ((old_flags & 0x02) && (curbuf -> b_ml.ml_line_count == 1 && *ml_get((linenr_T)1) == '\000')) curbuf -> b_ml.ml_flags |= 1;
    if (old_flags & 0x01) changed ();
    else if (!isNetbeansModified(curbuf)) unchanged (curbuf, 0);
    for (i = 0; i < ('z' - 'a' + 1); ++i) if (curhead -> uh_namedm [i].lnum != 0) {
    ...
    if (curhead -> uh_cursor.lnum + 1 == curwin -> w_cursor.lnum && curwin -> w_cursor.lnum > 1) --curwin->w_cursor.lnum;
    if (curwin -> w_cursor.lnum <= curbuf -> b_ml.ml_line_count) {
        if (curhead -> uh_cursor.lnum == curwin -> w_cursor.lnum) {
            curwin -> w_cursor.col = curhead -> uh_cursor.col;
            if (virtual_active () && curhead -> uh_cursor_vcol >= 0) coladvance ((colnr_T) curhead -> uh_cursor_vcol);
    ...
    if (undo) --curbuf->b_u_seq_cur;
    if (curhead -> uh_save_nr != 0) {
        if (undo) curbuf -> b_u_save_nr_cur = curhead -> uh_save_nr - 1;
    ...
    int absolute;
{
    ...
    u_header_T * uhp;
    char_u msgbuf [80];
    if ((fdo_flags & 0x200) && KeyTyped) foldOpenCursor ();
    if (global_busy || !messaging()) return;
    if (curbuf -> b_ml.ml_flags & 1) --u_newcount;
    u_oldcount -= u_newcount;
    if (u_oldcount == -1) msgstr = "more line";
    else if (u_oldcount < 0) msgstr = "more lines";
    else if (u_oldcount == 1) msgstr = "line less";
    else if (u_oldcount > 1) msgstr = "fewer lines";
    else {
        u_oldcount = u_newcount;
        if (u_newcount == 1) msgstr = "change";
        else msgstr = "changes";
    }
    if (curbuf -> b_u_curhead != ((void *) 0)) {
        ...
        else if (did_undo) uhp = curbuf -> b_u_curhead;
        else uhp = curbuf -> b_u_curhead -> uh_next.ptr;
    }
    else uhp = curbuf -> b_u_newhead;
    if (uhp == ((void *) 0)) *msgbuf = '\000';
    else u_add_time (msgbuf, sizeof (msgbuf), uhp -> uh_time);
    smsg ((char_u *) ((char *) ("%ld %s; %s #%ld  %s")), u_oldcount < 0 ? -u_oldcount : u_oldcount, ((char *) (msgstr)), did_undo ? ((char *) ("before")) : ((
      char *) ("after")), uhp == ((void *) 0) ? 0L : uhp -> uh_seq, msgbuf);
}

void u_sync (force)
    int force;
{
    if (curbuf -> b_u_synced || (!force && no_u_sync > 0)) return;
    if (p_ul < 0) curbuf -> b_u_synced = 1;
    else {
        u_getbot ();
        curbuf -> b_u_curhead = ((void *) 0);
    }
    ...
    exarg_T * eap __attribute__ ((unused));
{
    ...
    while (uhp != ((void *) 0)) {
        if (uhp -> uh_prev.ptr == ((void *) 0) && uhp -> uh_walk != nomark && uhp -> uh_walk != mark) {
            if (ga_grow (&ga, 1) == 0) break;
            vim_snprintf ((char *) IObuff, (1024 + 1), "%6ld %7ld  ", uhp -> uh_seq, changes);
            u_add_time (IObuff + strlen ((char *) (IObuff)), (1024 + 1) - strlen ((char *) (IObuff)), uhp -> uh_time);
            if (uhp -> uh_save_nr > 0) {
                while (strlen ((char *) (IObuff)) < 32) ((__builtin_object_size ((char *) (IObuff), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (
        ...
        uhp -> uh_walk = mark;
        if (uhp -> uh_prev.ptr != ((void *) 0) && uhp -> uh_prev.ptr -> uh_walk != nomark && uhp -> uh_prev.ptr -> uh_walk != mark) {
            uhp = uhp -> uh_prev.ptr;
            ++changes;
        }
        else if (uhp -> uh_alt_next.ptr != ((void *) 0) && uhp -> uh_alt_next.ptr -> uh_walk != nomark && uhp -> uh_alt_next.ptr -> uh_walk != mark) uhp = uhp
          -> uh_alt_next.ptr;
        else if (uhp -> uh_next.ptr != ((void *) 0) && uhp -> uh_alt_prev.ptr == ((void *) 0) && uhp -> uh_next.ptr -> uh_walk != nomark && uhp -> uh_next.ptr
          -> uh_walk != mark) {
            uhp = uhp -> uh_next.ptr;
            --changes;
        }
    ...
    if (ga.ga_len == 0) msg ((char_u *) (((char *) ("Nothing to undo"))));
    else {
        sort_strings ((char_u * *) ga.ga_data, ga.ga_len);
        msg_start ();
        msg_puts_attr ((char_u *) ((char *) ("number changes  time            saved")), highlight_attr [(int) (HLF_T)]);
    ...
    time_t tt;
{
    ...
    else vim_snprintf ((char *) buf, buflen, ((char *) ("%ld seconds ago")), (long) (time (((void *) 0)) - tt));
}

void ex_undojoin (eap)
    exarg_T * eap __attribute__ ((unused));
{
    if (curbuf -> b_u_newhead == ((void *) 0)) return;
    if (curbuf -> b_u_curhead != ((void *) 0)) {
        emsg ((char_u *) (((char *) ("E790: undojoin is not allowed after undo"))));
        return;
    }
    if (!curbuf->b_u_synced) return;
    if (p_ul < 0) return;
    else {
...
void u_find_first_changed () {
    ...
    u_entry_T * uep;
    linenr_T lnum;
    if (curbuf -> b_u_curhead != ((void *) 0) || uhp == ((void *) 0)) return;
    uep = uhp -> uh_entry;
    if (uep -> ue_top != 0 || uep -> ue_bot != 0) return;
    for (lnum = 1; lnum < curbuf -> b_ml.ml_line_count && lnum <= uep -> ue_size; ++lnum) if (strcmp ((char *) (ml_get_buf (curbuf, lnum, 0)), (char *) (uep ->
      ue_array [lnum - 1])) != 0) {
        {
    ...
    buf_T * buf;
{
    ...
    if (uhp != ((void *) 0)) uhp -> uh_save_nr = buf -> b_u_save_nr_last;
}
    ...
    u_header_T * uhp;
{
    ...
    for (uh = uhp; uh != ((void *) 0); uh = uh -> uh_prev.ptr) {
        uh -> uh_flags |= 0x01;
        if (uh -> uh_alt_next.ptr != ((void *) 0)) u_unch_branch (uh -> uh_alt_next.ptr);
    }
}
...
{
    u_header_T * uhap;
    if (uhp -> uh_alt_next.ptr != ((void *) 0)) u_freebranch (buf, uhp -> uh_alt_next.ptr, uhpp);
    if (uhp -> uh_alt_prev.ptr != ((void *) 0)) uhp -> uh_alt_prev.ptr -> uh_alt_next.ptr = ((void *) 0);
    if (uhp -> uh_next.ptr == ((void *) 0)) buf -> b_u_oldhead = uhp -> uh_prev.ptr;
    else uhp -> uh_next.ptr -> uh_prev.ptr = uhp -> uh_prev.ptr;
    if (uhp -> uh_prev.ptr == ((void *) 0)) buf -> b_u_newhead = uhp -> uh_next.ptr;
    else for (uhap = uhp -> uh_prev.ptr; uhap != ((void *) 0); uhap = uhap -> uh_alt_next.ptr) uhap -> uh_next.ptr = uhp -> uh_next.ptr;
    ...
    u_header_T * * uhpp;
{
    ...
    while (next != ((void *) 0)) {
        tofree = next;
        if (tofree -> uh_alt_next.ptr != ((void *) 0)) u_freebranch (buf, tofree -> uh_alt_next.ptr, uhpp);
...
{
    u_entry_T * uep, * nuep;
    if (buf -> b_u_curhead == uhp) buf -> b_u_curhead = ((void *) 0);
    if (buf -> b_u_newhead == uhp) buf -> b_u_newhead = ((void *) 0);
    if (uhpp != ((void *) 0) && uhp == *uhpp) *uhpp = ((void *) 0);
...
static void u_freeentry (uep, n)
    ...
    long n;
{
    while (n > 0) vim_free (uep -> ue_array [--n]);
    vim_free ((char_u *) uep -> ue_array);
    vim_free ((char_u *) uep);
}
...
void u_saveline (lnum)
    linenr_T lnum;
{
    if (lnum == curbuf -> b_u_line_lnum) return;
    if (lnum < 1 || lnum > curbuf -> b_ml.ml_line_count) return;
    u_clearline ();
    curbuf -> b_u_line_lnum = lnum;
    if (curwin -> w_cursor.lnum == lnum) curbuf -> b_u_line_colnr = curwin -> w_cursor.col;
    else curbuf -> b_u_line_colnr = 0;
    if ((curbuf -> b_u_line_ptr = u_save_line (lnum)) == ((void *) 0)) do_outofmem_msg ((long_u) 0);
}
...
void u_undoline () {
    ...
    char_u * oldp;
    if (undo_off) return;
    if (curbuf -> b_u_line_ptr == ((void *) 0) || curbuf -> b_u_line_lnum > curbuf -> b_ml.ml_line_count) {
        beep_flush ();
        return;
    }
    if (u_savecommon (curbuf -> b_u_line_lnum - 1, curbuf -> b_u_line_lnum + 1, (linenr_T) 0, 0) == 0) return;
    ...
    if (curwin -> w_cursor.lnum == curbuf -> b_u_line_lnum) curbuf -> b_u_line_colnr = curwin -> w_cursor.col;
...
void u_blockfree (buf)
    buf_T * buf;
{
    while (buf -> b_u_oldhead != ((void *) 0)) u_freeheader (buf, buf -> b_u_oldhead, ((void *) 0));
...
int bufIsChanged (buf)
    buf_T * buf;
{
    return !bt_dontwrite(buf) && (buf -> b_changed || file_ff_differs (buf));
}

int curbufIsChanged () {
    return !bt_dontwrite(curbuf) && (curbuf -> b_changed || file_ff_differs (curbuf));
}
    ...
    list_T * list;
{
    ...
    while (uhp != ((void *) 0)) {
        ...
        if (uhp == curbuf -> b_u_newhead) dict_add_nr_str (dict, "newhead", 1, ((void *) 0));
        if (uhp == curbuf -> b_u_curhead) dict_add_nr_str (dict, "curhead", 1, ((void *) 0));
        if (uhp -> uh_save_nr > 0) dict_add_nr_str (dict, "save", uhp -> uh_save_nr, ((void *) 0));
