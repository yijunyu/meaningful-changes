...
static struct buffheader redobuff =
{
    {((void *) 0),
        {'\000'}}, ((void *) 0), 0, 0};
static struct buffheader old_redobuff =
{
    {((void *) 0),
        {'\000'}}, ((void *) 0), 0, 0};
static struct buffheader save_redobuff =
{
    {((void *) 0),
        {'\000'}}, ((void *) 0), 0, 0};
static struct buffheader save_old_redobuff =
{
    {((void *) 0),
        {'\000'}}, ((void *) 0), 0, 0};
static struct buffheader recordbuff =
{
    {((void *) 0),
        {'\000'}}, ((void *) 0), 0, 0};
    ...
    int dozero;
{
    ...
    char_u * str;
    struct buffblock * bp;
    for (bp = buffer -> bh_first.b_next; bp != ((void *) 0); bp = bp -> b_next) count += (long_u) strlen ((char *) (bp -> b_str));
    if ((count || dozero) && (p = lalloc (count + 1, 1)) != ((void *) 0)) {
        p2 = p;
        for (bp = buffer -> bh_first.b_next; bp != ((void *) 0); bp = bp -> b_next) for (str = bp -> b_str; *str;) *p2++ = *str++;
        * p2 = '\000';
    }
    return (p);
}
...
char_u * get_recorded () {
    ...
    if (len > 0 && restart_edit != 0 && p [len - 1] == 15) p [len - 1] = '\000';
    return (p);
}
    ...
    long slen;
{
    struct buffblock * p;
    long_u len;
    if (slen < 0) slen = (long) strlen ((char *) (s));
    if (slen == 0) return;
    if (buf -> bh_first.b_next == ((void *) 0)) {
        ...
        emsg ((char_u *) (((char *) ("E222: Add to read buffer"))));
        return;
    }
    else if (buf -> bh_index != 0) ((__builtin_object_size ((char *) (buf -> bh_first.b_next -> b_str), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *)
      (buf -> bh_first.b_next -> b_str), (char *) (buf -> bh_first.b_next -> b_str + buf -> bh_index), strlen ((char *) (buf -> bh_first.b_next -> b_str + buf
      -> bh_index)) + 1, __builtin_object_size ((char *) (buf -> bh_first.b_next -> b_str), 0)) : __inline_memmove_chk ((char *) (buf -> bh_first.b_next ->
      b_str), (char *) (buf -> bh_first.b_next -> b_str + buf -> bh_index), strlen ((char *) (buf -> bh_first.b_next -> b_str + buf -> bh_index)) + 1));
    buf -> bh_index = 0;
    if (buf -> bh_space >= (int) slen) {
        len = (long_u) strlen ((char *) (buf -> bh_curr -> b_str));
        ...
        p = (struct buffblock *) lalloc ((long_u) (sizeof (struct buffblock) +len), 1);
        if (p == ((void *) 0)) return;
...
{
    char_u number [32];
    __builtin___sprintf_chk ((char *) number, 0, __builtin_object_size ((char *) number, 1 > 1), "%ld", n);
    ...
    int advance;
{
    ...
    if (stuffbuff.bh_first.b_next == ((void *) 0)) return '\000';
    curr = stuffbuff.bh_first.b_next;
    c = curr -> b_str [stuffbuff.bh_index];
    if (advance) {
        if (curr -> b_str [++stuffbuff.bh_index] == '\000') {
...
void flush_buffers (typeahead)
    int typeahead;
{
    init_typebuf ();
    start_stuff ();
    while (read_stuff (1) != '\000');
    if (typeahead) {
...
void AppendToRedobuff (s)
    char_u * s;
{
    if (!block_redo) add_buff (&redobuff, s, -1L);
}
    ...
    int len;
{
    ...
    int c;
    char_u * start;
    if (block_redo) return;
    while (len < 0 ? *s != '\000' : s - str < len) {
        start = s;
        while (*s >= ' ' && *s < 0x7f && (len < 0 || s - str < len)) ++s;
        if (*s == '\000' && (s [-1] == '0' || s [-1] == '^')) --s;
        if (s > start) add_buff (&redobuff, start, (long) (s - start));
        if (*s == '\000' || (len >= 0 && s - str >= len)) break;
        c = *s++;
        if (c < ' ' || c == 0x7f || (*s == '\000' && (c == '0' || c == '^'))) add_char_buff (&redobuff, 22);
        if (*s == '\000' && c == '0') add_buff (&redobuff, (char_u *) "048", 3L);
        else add_char_buff (&redobuff, c);
    }
}
...
void AppendCharToRedobuff (c)
    int c;
{
    if (!block_redo) add_char_buff (&redobuff, c);
}

void AppendNumberToRedobuff (n)
    long n;
{
    if (!block_redo) add_num_buff (&redobuff, n);
}
    ...
    int old_redo;
{
    ...
    int c;
    if (init) {
        if (old_redo) bp = old_redobuff.bh_first.b_next;
        else bp = redobuff.bh_first.b_next;
        if (bp == ((void *) 0)) return 0;
        p = bp -> b_str;
        return 1;
    }
    if ((c = *p) != '\000') {
...
{
    int c;
    while ((c = read_redo (0, old_redo)) != '\000') stuffcharReadbuff (c);
}
...
{
    int c;
    if (read_redo (1, old_redo) == 0) return 0;
    ...
    if (count) {
        while (((c) >= '0' && (c) <= '9')) c = read_redo (0, old_redo);
    ...
    int silent;
{
    ...
    if (offset == 0 && addlen <= typebuf.tb_off) {
        ...
        ((__builtin_object_size ((char *) (typebuf.tb_buf + typebuf.tb_off), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (typebuf.tb_buf + typebuf.
          tb_off), (char *) (str), (size_t) addlen, __builtin_object_size ((char *) (typebuf.tb_buf + typebuf.tb_off), 0)) : __inline_memmove_chk ((char *) (
          typebuf.tb_buf + typebuf.tb_off), (char *) (str), (size_t) addlen));
    }
    else {
        newoff = 50 + 4;
        newlen = typebuf.tb_len + addlen + newoff + 4 * (50 + 4);
        if (newlen < 0) {
            emsg ((char_u *) (((char *) (e_toocompl))));
            setcursor ();
            return 0;
        }
        s1 = alloc (newlen);
        if (s1 == ((void *) 0)) return 0;
        s2 = alloc (newlen);
        if (s2 == ((void *) 0)) {
            vim_free (s1);
            return 0;
        ...
        ((__builtin_object_size ((char *) (s1 + newoff), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (s1 + newoff), (char *) (typebuf.tb_buf +
          typebuf.tb_off), (size_t) offset, __builtin_object_size ((char *) (s1 + newoff), 0)) : __inline_memmove_chk ((char *) (s1 + newoff), (char *) (
          typebuf.tb_buf + typebuf.tb_off), (size_t) offset));
        ((__builtin_object_size ((char *) (s1 + newoff + offset), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (s1 + newoff + offset), (char *) (str
          ), (size_t) addlen, __builtin_object_size ((char *) (s1 + newoff + offset), 0)) : __inline_memmove_chk ((char *) (s1 + newoff + offset), (char *) (
          str), (size_t) addlen));
        ((__builtin_object_size ((char *) (s1 + newoff + offset + addlen), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (s1 + newoff + offset +
          addlen), (char *) (typebuf.tb_buf + typebuf.tb_off + offset), (size_t) (typebuf.tb_len - offset + 1), __builtin_object_size ((char *) (s1 + newoff +
          offset + addlen), 0)) : __inline_memmove_chk ((char *) (s1 + newoff + offset + addlen), (char *) (typebuf.tb_buf + typebuf.tb_off + offset), (size_t)
          (typebuf.tb_len - offset + 1)));
        if (typebuf.tb_buf != typebuf_init) vim_free (typebuf.tb_buf);
        typebuf.tb_buf = s1;
        ((__builtin_object_size ((char *) (s2 + newoff), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (s2 + newoff), (char *) (typebuf.tb_noremap +
          typebuf.tb_off), (size_t) offset, __builtin_object_size ((char *) (s2 + newoff), 0)) : __inline_memmove_chk ((char *) (s2 + newoff), (char *) (
          typebuf.tb_noremap + typebuf.tb_off), (size_t) offset));
        ((__builtin_object_size ((char *) (s2 + newoff + offset + addlen), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (s2 + newoff + offset +
          addlen), (char *) (typebuf.tb_noremap + typebuf.tb_off + offset), (size_t) (typebuf.tb_len - offset), __builtin_object_size ((char *) (s2 + newoff +
          offset + addlen), 0)) : __inline_memmove_chk ((char *) (s2 + newoff + offset + addlen), (char *) (typebuf.tb_noremap + typebuf.tb_off + offset), (
          size_t) (typebuf.tb_len - offset)));
        if (typebuf.tb_noremap != noremapbuf_init) vim_free (typebuf.tb_noremap);
        typebuf.tb_noremap = s2;
        typebuf.tb_off = newoff;
    }
    typebuf.tb_len += addlen;
    if (noremap == -2) val = 2;
    else if (noremap == -3) val = 4;
    else val = 1;
    if (noremap == -3) nrm = 1;
    else if (noremap < 0) nrm = addlen;
    else nrm = noremap;
    for (i = 0; i < addlen; ++i) typebuf.tb_noremap [typebuf.tb_off + i + offset] = (--nrm >= 0) ? val : 0;
    if (nottyped || typebuf.tb_maplen > offset) typebuf.tb_maplen += addlen;
    if (silent || typebuf.tb_silent > offset) {
        typebuf.tb_silent += addlen;
        cmd_silent = 1;
    }
    if (typebuf.tb_no_abbr_cnt && offset == 0) typebuf.tb_no_abbr_cnt += addlen;
    return 1;
}
    ...
    int offset;
{
    ...
    if (len == 0) return;
    typebuf.tb_len -= len;
    if (offset == 0 && typebuf.tb_buflen - (typebuf.tb_off + len) >= 3 * 50 + 3) typebuf.tb_off += len;
    else {
        i = typebuf.tb_off + offset;
        if (typebuf.tb_off > 50) {
            ((__builtin_object_size ((char *) (typebuf.tb_buf + 50), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (typebuf.tb_buf + 50), (char *) (
              typebuf.tb_buf + typebuf.tb_off), (size_t) offset, __builtin_object_size ((char *) (typebuf.tb_buf + 50), 0)) : __inline_memmove_chk ((char *) (
              typebuf.tb_buf + 50), (char *) (typebuf.tb_buf + typebuf.tb_off), (size_t) offset));
            ((__builtin_object_size ((char *) (typebuf.tb_noremap + 50), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (typebuf.tb_noremap + 50), (
              char *) (typebuf.tb_noremap + typebuf.tb_off), (size_t) offset, __builtin_object_size ((char *) (typebuf.tb_noremap + 50), 0)) :
              __inline_memmove_chk ((char *) (typebuf.tb_noremap + 50), (char *) (typebuf.tb_noremap + typebuf.tb_off), (size_t) offset));
            typebuf.tb_off = 50;
        }
        ((__builtin_object_size ((char *) (typebuf.tb_buf + typebuf.tb_off + offset), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (typebuf.tb_buf +
          typebuf.tb_off + offset), (char *) (typebuf.tb_buf + i + len), (size_t) (typebuf.tb_len - offset + 1), __builtin_object_size ((char *) (typebuf.
          tb_buf + typebuf.tb_off + offset), 0)) : __inline_memmove_chk ((char *) (typebuf.tb_buf + typebuf.tb_off + offset), (char *) (typebuf.tb_buf + i + len
          ), (size_t) (typebuf.tb_len - offset + 1)));
        ((__builtin_object_size ((char *) (typebuf.tb_noremap + typebuf.tb_off + offset), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (typebuf.
          tb_noremap + typebuf.tb_off + offset), (char *) (typebuf.tb_noremap + i + len), (size_t) (typebuf.tb_len - offset), __builtin_object_size ((char *) (
          typebuf.tb_noremap + typebuf.tb_off + offset), 0)) : __inline_memmove_chk ((char *) (typebuf.tb_noremap + typebuf.tb_off + offset), (char *) (typebuf
          .tb_noremap + i + len), (size_t) (typebuf.tb_len - offset)));
    }
    if (typebuf.tb_maplen > offset) {
        if (typebuf.tb_maplen < offset + len) typebuf.tb_maplen = offset;
        else typebuf.tb_maplen -= len;
    }
    if (typebuf.tb_silent > offset) {
        if (typebuf.tb_silent < offset + len) typebuf.tb_silent = offset;
        else typebuf.tb_silent -= len;
    }
    if (typebuf.tb_no_abbr_cnt > offset) {
        if (typebuf.tb_no_abbr_cnt < offset + len) typebuf.tb_no_abbr_cnt = offset;
        else typebuf.tb_no_abbr_cnt -= len;
    }
    ...
    int len;
{
    ...
    char_u buf [2];
    int todo = len;
    if (Recording) last_recorded_len += len;
    buf [1] = '\000';
    while (todo --) {
        c = *s++;
        updatescript (c);
...
static void may_sync_undo () {
    if ((!(State&(0x10+0x08)) || arrow_used) && scriptin [curscript] == ((void *) 0)) u_sync (0);
}

int alloc_typebuf () {
    typebuf.tb_buf = alloc ((5 * (50 + 3)));
    typebuf.tb_noremap = alloc ((5 * (50 + 3)));
...
void free_typebuf () {
    if (typebuf.tb_buf == typebuf_init) emsg2 ((char_u *) (((char *) (e_intern2))), (char_u *) ("Free typebuf 1"));
    else vim_free (typebuf.tb_buf);
    if (typebuf.tb_noremap == noremapbuf_init) emsg2 ((char_u *) (((char *) (e_intern2))), (char_u *) ("Free typebuf 2"));
    else vim_free (typebuf.tb_noremap);
}
    ...
    tasave_T * tp;
{
    ...
    tp -> typebuf_valid = (alloc_typebuf () == 1);
    if (!tp->typebuf_valid) typebuf = tp -> save_typebuf;
    tp -> old_char = old_char;
    tp -> old_mod_mask = old_mod_mask;
    old_char = -1;
    tp -> save_stuffbuff = stuffbuff;
    stuffbuff.bh_first.b_next = ((void *) 0);
...
{
    if (curscript + 1 == 15) {
        emsg ((char_u *) (((char *) (e_nesting))));
        return;
    }
    if (ignore_script) return;
    if (scriptin [curscript] != ((void *) 0)) ++curscript;
    expand_env (name, NameBuff, 1024);
    if ((scriptin [curscript] = fopen (((char *) NameBuff), ("rb"))) == ((void *) 0)) {
        emsg2 ((char_u *) (((char *) (e_notopen))), (char_u *) (name));
        if (curscript) --curscript;
        return;
    }
    if (save_typebuf () == 0) return;
    if (directly) {
        oparg_T oa;
        int oldcurscript;
...
void before_blocking () {
    updatescript (0);
    if (may_garbage_collect) garbage_collect ();
}
...
{
    static int count = 0;
    if (c && scriptout) putc (c, scriptout);
...
int vgetc () {
    int c, c2;
    if (may_garbage_collect && want_garbage_collect) garbage_collect ();
    if (old_char != -1) {
        c = old_char;
        old_char = -1;
...
int vpeekc_any () {
    int c;
    c = vpeekc ();
    if (c == '\000' && typebuf.tb_len > 0) c = '\033';
    return c;
}
    ...
    int advance;
{
    ...
    int wait_tb_len;
    if (vgetc_busy > 0 && ex_normal_busy == 0) return '\000';
    local_State = get_real_state ();
    ++vgetc_busy;
    if (advance) KeyStuffed = 0;
    init_typebuf ();
    start_stuff ();
    if (advance && typebuf.tb_maplen == 0) Exec_reg = 0;
    do {
        if (typeahead_char != 0) {
            c = typeahead_char;
            if (advance) typeahead_char = 0;
        }
        else c = read_stuff (advance);
        if (c != '\000' && !got_int) {
            if (advance) {
                KeyStuffed = 1;
            }
            if (typebuf.tb_no_abbr_cnt == 0) typebuf.tb_no_abbr_cnt = 1;
        }
        else {
            for (;;) {
                if (typebuf.tb_maplen) line_breakcheck ();
                else ui_breakcheck ();
                keylen = 0;
                if (got_int) {
                    c = inchar (typebuf.tb_buf, typebuf.tb_buflen - 1, 0L, typebuf.tb_change_cnt);
                    if ((c || typebuf.tb_maplen) && (State & (0x10 + 0x08))) c = '\033';
                    else c = 3;
                    flush_buffers (1);
                    if (advance) {
                        *typebuf.tb_buf = c;
                    ...
                    if (no_mapping == 0 && maphash_valid && (no_zero_mapping == 0 || c1 != '0') && (typebuf.tb_maplen == 0 || (p_remap && (typebuf.tb_noremap [
                      typebuf.tb_off] & (1 | 4)) == 0)) && !(p_paste&&(State&(0x10+0x08))) && !(State==(0x200+0x01)&&(c1=='\015'||c1==' ')) && State != 0x300 &&
                      State != 0x800 && !((ctrl_x_mode!=0&&vim_is_ctrl_x_key(c1))||((compl_cont_status&32)&&(c1==14||c1==16)))) {
                        ...
                        for (; mp != ((void *) 0); mp -> m_next == ((void *) 0) ? (mp = mp2, mp2 = ((void *) 0)) : (mp = mp -> m_next)) {
                            if (mp -> m_keys [0] == c1 && (mp -> m_mode & local_State) && ((mp -> m_mode & 0x20) == 0 || typebuf.tb_maplen == 0)) {
                                for (mlen = 1; mlen < typebuf.tb_len; ++mlen) {
                                    if (mp -> m_keys [mlen] != typebuf.tb_buf [typebuf.tb_off + mlen]) break;
                                }
                                keylen = mp -> m_keylen;
                                if (mlen == keylen || (mlen == typebuf.tb_len && typebuf.tb_len < keylen)) {
                                    s = typebuf.tb_noremap + typebuf.tb_off;
                                    if (*s == 2 && (mp -> m_keys [0] != (0x80) || mp -> m_keys [1] != 253 || mp -> m_keys [2] != (int) KE_SNR)) continue;
                                    for (n = mlen; --n >= 0;) if (*s++ & (1 | 4)) break;
                                    if (n >= 0) continue;
                                ...
                                else if (max_mlen < mlen) max_mlen = mlen;
                            }
                        }
                        if (keylen != -2) {
                            mp = mp_match;
                    ...
                    if (*p_pt != '\000' && mp == ((void *) 0) && (State & (0x10 | 0x01))) {
                        for (mlen = 0; mlen < typebuf.tb_len && p_pt [mlen]; ++mlen) if (p_pt [mlen] != typebuf.tb_buf [typebuf.tb_off + mlen]) break;
                        if (p_pt [mlen] == '\000') {
                            if (mlen > typebuf.tb_maplen) gotchars (typebuf.tb_buf + typebuf.tb_off + typebuf.tb_maplen, mlen - typebuf.tb_maplen);
                            del_typebuf (mlen, 0);
                            set_option_value ((char_u *) "paste", (long) !p_paste, ((void *) 0), 0);
                            if (!(State&0x10)) {
                                msg_col = 0;
                                msg_row = Rows - 1;
                                msg_clr_eos ();
                        ...
                        if (mlen == typebuf.tb_len) keylen = -1;
                        else if (max_mlen < mlen) max_mlen = mlen + 1;
                    }
                    if ((mp == ((void *) 0) || max_mlen >= mp_match_len) && keylen != -2) {
                        int save_keylen = keylen;
                        if ((no_mapping == 0 || allow_keys != 0) && (typebuf.tb_maplen == 0 || (p_remap && typebuf.tb_noremap [typebuf.tb_off] == 0)) && 
                          !timedout) {
                            keylen = check_termcode (max_mlen + 1, ((void *) 0), 0);
                            if (keylen == 0 && save_keylen == -1) keylen = -1;
                            if (keylen < 0 && typebuf.tb_len == typebuf.tb_maplen) keylen = 0;
                        }
                        else keylen = 0;
                        if (keylen == 0) {
                            if (mp == ((void *) 0)) {
                                ...
                                if (advance) {
                                    ...
                                    if (typebuf.tb_maplen > 0) KeyTyped = 0;
                                    else {
                                        KeyTyped = 1;
                        ...
                        if (mp == ((void *) 0) || keylen < 0) keylen = -1;
                        else keylen = mp_match_len;
                    }
                    if (keylen >= 0 && keylen <= typebuf.tb_len) {
                        ...
                        char_u * save_m_keys;
                        char_u * save_m_str;
                        if (keylen > typebuf.tb_maplen) gotchars (typebuf.tb_buf + typebuf.tb_off + typebuf.tb_maplen, keylen - typebuf.tb_maplen);
                        cmd_silent = (typebuf.tb_silent > 0);
                        del_typebuf (keylen, 0);
                        if (++mapdepth >= p_mmd) {
                            emsg ((char_u *) (((char *) ("E223: recursive mapping"))));
                            if (State & 0x08) redrawcmdline ();
                            else setcursor ();
                            flush_buffers (0);
                            mapdepth = 0;
                            c = -1;
                        ...
                        if (VIsual_active && VIsual_select && (mp -> m_mode & 0x02)) {
                            VIsual_select = 0;
                            (void) ins_typebuf ((char_u *) "\200\365X", -1, 0, 1, 0);
                        }
                        ...
                        if (s == ((void *) 0)) i = 0;
                            ...
                            int noremap;
                            if (save_m_noremap != 0) noremap = save_m_noremap;
                            else if (strncmp ((char *) (s), (char *) (save_m_keys != ((void *) 0) ? save_m_keys : mp -> m_keys), (size_t) ((size_t) keylen)) !=
                              0) noremap = 0;
                            else noremap = -3;
                            i = ins_typebuf (s, noremap, 0, 1, cmd_silent || save_m_silent);
                            if (save_m_expr) vim_free (s);
                        }
                ...
                if (advance && typebuf.tb_len == 1 && typebuf.tb_buf [typebuf.tb_off] == '\033' && !no_mapping && ex_normal_busy == 0 && typebuf.tb_maplen == 0
                  && (State & 0x10) && (p_timeout || (keylen == -1 && p_ttimeout)) && (c = inchar (typebuf.tb_buf + typebuf.tb_off + typebuf.tb_len, 3, 25L,
                        ...
                        if (curwin -> w_wcol > 0) {
                            if (did_ai) {
                                ...
                                while (col < curwin -> w_cursor.col) {
                                    if (!((ptr[col])==' '||(ptr[col])=='\t')) curwin -> w_wcol = vcol;
                                    vcol += lbr_chartabsize (ptr + col, (colnr_T) vcol);
                                    ++col;
                                }
                ...
                if (ex_normal_busy > 0) {
                    ...
                    if (p_im && (State & 0x10)) c = 12;
                    else if ((State & 0x08) || (cmdwin_type > 0 && tc == '\033')) c = 3;
                    else c = '\033';
                    tc = c;
                    break;
                }
                if ((State & 0x10) && advance && must_redraw != 0) {
                    update_screen (0);
                    setcursor ();
                }
                i = 0;
                ...
                if (typebuf.tb_len > 0 && advance && !exmode_active) {
                    if (((State & (0x01 | 0x10)) || State == 0x20) && State != (0x200 + 0x01)) {
                        if (State & 0x10 && ptr2cells (typebuf.tb_buf + typebuf.tb_off + typebuf.tb_len - 1) == 1) {
                        ...
                        push_showcmd ();
                        if (typebuf.tb_len > 10) i = typebuf.tb_len - 10;
                        while (i < typebuf.tb_len) (void) add_to_showcmd (typebuf.tb_buf [typebuf.tb_off + i ++]);
                        curwin -> w_wcol = old_wcol;
                        curwin -> w_wrow = old_wrow;
                    }
                    if ((State & 0x08) && cmdline_star == 0 && ptr2cells (typebuf.tb_buf + typebuf.tb_off + typebuf.tb_len - 1) == 1) {
                        putcmdline (typebuf.tb_buf [typebuf.tb_off + typebuf.tb_len - 1], 0);
                        c1 = 1;
                    }
                }
                wait_tb_len = typebuf.tb_len;
                c = inchar (typebuf.tb_buf + typebuf.tb_off + typebuf.tb_len, typebuf.tb_buflen - typebuf.tb_off - typebuf.tb_len - 1, !advance ? 0 : ((typebuf
                  .tb_len == 0 || !(p_timeout||(p_ttimeout&&keylen==-1))) ? -1L : ((keylen == -1 && p_ttm >= 0) ? p_ttm : p_tm)), typebuf.tb_change_cnt);
                if (i != 0) pop_showcmd ();
                if (c1 == 1) {
                    if (State & 0x10) edit_unputchar ();
                    if (State & 0x08) unputcmdline ();
                    setcursor ();
                }
                if (c < 0) continue;
                if (c == '\000') {
                    ...
                    while (typebuf.tb_buf [typebuf.tb_off + typebuf.tb_len] != '\000') typebuf.tb_noremap [typebuf.tb_off + typebuf.tb_len ++] = 0;
                }
            }
        }
    }
    ...
    if (advance && p_smd && msg_silent == 0 && (State & 0x10)) {
        if (c == '\033' && !mode_deleted && !no_mapping && mode_displayed) {
            if (typebuf.tb_len && !KeyTyped) redraw_cmdline = 1;
            else unshowmode (0);
        }
    ...
    int tb_change_cnt;
{
    ...
    undo_off = 0;
    script_char = -1;
    while (scriptin [curscript] != ((void *) 0) && script_char < 0 && !ignore_script) {
    ...
    if (script_char < 0) {
        if (got_int) {
            char_u dum [50 * 3 + 3 + 1];
            for (;;) {
                len = ui_inchar (dum, 50 * 3 + 3, 0L, 0);
                if (len == 0 || (len == 1 && dum [0] == 3)) break;
            }
            return retesc;
        }
        out_flush ();
    ...
    int script;
{
    ...
    char_u * p = buf;
    for (i = len; --i >= 0; ++p) {
        if (p [0] == '\000' || (p [0] == (0x80) && !script && (i < 2 || p [1] != 253 || p [2] != (int) KE_CURSORHOLD))) {
            ((__builtin_object_size ((char *) (p + 3), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (p + 3), (char *) (p + 1), (size_t) i,
              __builtin_object_size ((char *) (p + 3), 0)) : __inline_memmove_chk ((char *) (p + 3), (char *) (p + 1), (size_t) i));
    ...
    int abbrev;
{
    ...
    while (*p && (maptype == 1 || !((*p) == ' ' || (*p) == '\t'))) {
        if ((p [0] == 22 || (do_backslash && p [0] == '\\')) && p [1] != '\000') ++p;
        ++p;
    }
    if (*p != '\000') *p++ = '\000';
    p = skipwhite (p);
    ...
    if (haskey) keys = replace_termcodes (keys, &keys_buf, 1, 1, special);
    if (hasarg) {
        if (strcasecmp ((char *) (rhs), (char *) ("<nop>")) == 0) rhs = (char_u *) "";
        else rhs = replace_termcodes (rhs, &arg_buf, 0, 1, special);
    }
    if (haskey) {
        ...
        if (abbrev && maptype != 1) {
            if (vim_iswordc (keys [len - 1])) for (n = 0; n < len - 2; ++n) if (vim_iswordc (keys [n]) != vim_iswordc (keys [len - 2])) {
                retval = 1;
                goto theend;
            }
            for (n = 0; n < len; ++n) if (((keys [n]) == ' ' || (keys [n]) == '\t')) {
                retval = 1;
                goto theend;
            }
        }
    }
    if (haskey && hasarg && abbrev) no_abbr = 0;
    if (!haskey || (maptype != 1 && !hasarg)) msg_start ();
    if (map_table == curbuf -> b_maphash && haskey && hasarg && maptype != 1) {
        for (hash = 0; hash < 256 && !got_int; ++hash) {
            ...
            else mp = maphash [hash];
            for (; mp != ((void *) 0) && !got_int; mp = mp -> m_next) {
                if ((mp -> m_mode & mode) != 0 && mp -> m_keylen == len && unique && strncmp ((char *) (mp -> m_keys), (char *) (keys), (size_t) ((size_t) len)
                  ) == 0) {
                    if (abbrev) emsg2 ((char_u *) (((char *) ("E224: global abbreviation already exists for %s"))), (char_u *) (mp -> m_keys));
                    else emsg2 ((char_u *) (((char *) ("E225: global mapping already exists for %s"))), (char_u *) (mp -> m_keys));
                    retval = 5;
                    goto theend;
                }
            }
            ...
            for (mp = *mpp; mp != ((void *) 0) && !got_int; mp = *mpp) {
                ...
                if (!haskey) {
                    ...
                    if (strncmp ((char *) (p), (char *) (keys), (size_t) ((size_t) (n < len ? n : len))) == 0) {
                        if (maptype == 1) {
                            if (n != len && (!abbrev || round || n > len || *skipwhite(keys+n) != '\000')) {
                                mpp = &(mp->m_next);
                                continue;
                            }
                            mp -> m_mode &= ~mode;
                            ...
                            if (abbrev) emsg2 ((char_u *) (((char *) ("E226: abbreviation already exists for %s"))), (char_u *) (p));
                            else emsg2 ((char_u *) (((char *) ("E227: mapping already exists for %s"))), (char_u *) (p));
                            retval = 5;
                            goto theend;
                        }
                        else {
    ...
    if (!haskey || !hasarg) {
        if (!did_it && !did_local) {
            if (abbrev) msg ((char_u *) (((char *) ("No abbreviation found"))));
            else msg ((char_u *) (((char *) ("No mapping found"))));
        }
        goto theend;
    }
    if (did_it) goto theend;
    mp = (mapblock_T *) alloc ((unsigned) sizeof (mapblock_T));
    if (mp == ((void *) 0)) {
        retval = 4;
        goto theend;
    }
    if (*keys == 3) mapped_ctrl_c = 1;
    mp -> m_keys = vim_strsave (keys);
    mp -> m_str = vim_strsave (rhs);
    if (mp -> m_keys == ((void *) 0) || mp -> m_str == ((void *) 0)) {
        vim_free (mp -> m_keys);
        vim_free (mp -> m_str);
        vim_free (mp);
        retval = 4;
...
static void validate_maphash () {
    if (!maphash_valid) {
        ((__builtin_object_size ((maphash), 0) != (size_t) - 1) ? __builtin___memset_chk ((maphash), (0), (sizeof (maphash)), __builtin_object_size ((maphash),
          0)) : __inline_memset_chk ((maphash), (0), (sizeof (maphash))));
        maphash_valid = 1;
    }
}
    ...
    int forceit;
{
    ...
    p = *cmdp;
    modec = *p++;
    if (modec == 'i') mode = 0x10;
    else if (modec == 'l') mode = 0x20;
    else if (modec == 'c') mode = 0x08;
    else if (modec == 'n' && *p != 'o') mode = 0x01;
    else if (modec == 'v') mode = 0x02 + 0x1000;
    else if (modec == 'x') mode = 0x02;
    else if (modec == 's') mode = 0x1000;
    else if (modec == 'o') mode = 0x04;
    else {
        --p;
        if (forceit) mode = 0x10 + 0x08;
        else mode = 0x02 + 0x1000 + 0x01 + 0x04;
    }
    * cmdp = p;
    return mode;
}
...
void map_clear (cmdp, arg, forceit, abbr)
    char_u * cmdp;
    char_u * arg __attribute__ ((unused));
    int forceit;
    int abbr;
{
    int mode;
    ...
    int abbr;
{
    ...
    for (hash = 0; hash < 256; ++hash) {
        if (abbr) {
            if (hash > 0) break;
            if (local) mpp = &buf->b_first_abbr;
            else mpp = &first_abbr;
        }
        else {
            if (local) mpp = &buf->b_maphash[hash];
            else mpp = &maphash[hash];
        }
        while (*mpp != ((void *) 0)) {
            mp = *mpp;
            if (mp -> m_mode & mode) {
    ...
    int local;
{
    ...
    if ((mp -> m_mode & (0x10 + 0x08)) == 0x10 + 0x08) msg_putchar ('!');
    else if (mp -> m_mode & 0x10) msg_putchar ('i');
    else if (mp -> m_mode & 0x20) msg_putchar ('l');
    else if (mp -> m_mode & 0x08) msg_putchar ('c');
    else if ((mp -> m_mode & (0x01 + 0x02 + 0x1000 + 0x04)) == 0x01 + 0x02 + 0x1000 + 0x04) msg_putchar (' ');
    else {
        len = 0;
        if (mp -> m_mode & 0x01) {
            msg_putchar ('n');
        ...
        if ((mp -> m_mode & (0x02 + 0x1000)) == 0x02 + 0x1000) {
            msg_putchar ('v');
            ++len;
        }
        else {
    ...
    while (len < 12);
    if (mp -> m_noremap == -1) msg_puts_attr ((char_u *) "*", highlight_attr [(int) (HLF_8)]);
    else if (mp -> m_noremap == -2) msg_puts_attr ((char_u *) "&", highlight_attr [(int) (HLF_8)]);
    else msg_putchar (' ');
    if (local) msg_putchar ('@');
    else msg_putchar (' ');
    if (*mp->m_str == '\000') msg_puts_attr ((char_u *) "<Nop>", highlight_attr [(int) (HLF_8)]);
    else msg_outtrans_special (mp -> m_str, 0);
    if (p_verbose > 0) last_set_msg (mp -> m_script_ID);
    out_flush ();
}
    ...
    int abbr;
{
    ...
    if (vim_strchr (modechars, 'n') != ((void *) 0)) mode |= 0x01;
    if (vim_strchr (modechars, 'v') != ((void *) 0)) mode |= 0x02 + 0x1000;
    if (vim_strchr (modechars, 'x') != ((void *) 0)) mode |= 0x02;
    if (vim_strchr (modechars, 's') != ((void *) 0)) mode |= 0x1000;
    if (vim_strchr (modechars, 'o') != ((void *) 0)) mode |= 0x04;
    if (vim_strchr (modechars, 'i') != ((void *) 0)) mode |= 0x10;
    if (vim_strchr (modechars, 'l') != ((void *) 0)) mode |= 0x20;
    if (vim_strchr (modechars, 'c') != ((void *) 0)) mode |= 0x08;
    ...
    int abbr;
{
    ...
    for (;;) {
        for (hash = 0; hash < 256; ++hash) {
            if (abbr) {
                if (hash > 0) break;
                if (expand_buffer) mp = curbuf -> b_first_abbr;
                else mp = first_abbr;
            }
            else if (expand_buffer) mp = curbuf -> b_maphash [hash];
            else mp = maphash [hash];
            for (; mp; mp = mp -> m_next) {
                if ((mp -> m_mode & mode) && strstr ((char *) mp -> m_str, (char *) rhs) != ((void *) 0)) return 1;
            }
        }
        if (expand_buffer) break;
        expand_buffer = 1;
    }
    return 0;
}
...
char_u * set_context_in_map_cmd (xp, cmd, arg, forceit, isabbrev, isunmap, cmdidx)
    expand_T * xp;
    char_u * cmd;
    char_u * arg;
    int forceit;
    ...
    cmdidx_T cmdidx;
{
    if (forceit && cmdidx != CMD_map && cmdidx != CMD_unmap) xp -> xp_context = 0;
    else {
        if (isunmap) expand_mapmodes = get_map_mode (&cmd, forceit || isabbrev);
        else {
            expand_mapmodes = 0x10 + 0x08;
            if (!isabbrev) expand_mapmodes += 0x02 + 0x1000 + 0x01 + 0x04;
        }
    ...
    char_u * * * file;
{
    ...
    * file = ((void *) 0);
    for (round = 1; round <= 2; ++round) {
        count = 0;
        for (i = 0; i < 5; ++i) {
            if (i == 0) p = (char_u *) "<silent>";
            else if (i == 1) p = (char_u *) "<unique>";
            else if (i == 2) p = (char_u *) "<script>";
            else if (i == 3) p = (char_u *) "<expr>";
            else if (i == 4 && !expand_buffer) p = (char_u *) "<buffer>";
            else continue;
            if (vim_regexec (regmatch, p, (colnr_T) 0)) {
                if (round == 1) ++count;
                else (*file) [count ++] = vim_strsave (p);
            }
        }
        for (hash = 0; hash < 256; ++hash) {
            if (expand_isabbrev) {
    ...
    if (count > 1) {
        ...
        ptr3 = ptr1 + count;
        while (ptr2 < ptr3) {
            if (strcmp ((char *) (*ptr1), (char *) (*ptr2))) *++ptr1 = *ptr2++;
            else {
                vim_free (*ptr2++);
                count --;
            }
    ...
    int mincol;
{
    ...
    int is_id = 1;
    int vim_abbr;
    if (typebuf.tb_no_abbr_cnt) return 0;
    if ((KeyNoremap & (1 | 2)) != 0) return 0;
    if (col == 0) return 0;
    {
        if (!vim_iswordc(ptr[col-1])) vim_abbr = 1;
        else {
            vim_abbr = 0;
            if (col > 1) is_id = vim_iswordc (ptr [col - 2]);
        }
        for (scol = col - 1; scol > 0 && !vim_isspace(ptr[scol-1]) && (vim_abbr || is_id == vim_iswordc (ptr [scol - 1])); --scol);
    }
    if (scol < mincol) scol = mincol;
    if (scol < col) {
        ptr += scol;
        len = col - scol;
        mp = curbuf -> b_first_abbr;
        ...
        for (; mp; mp -> m_next == ((void *) 0) ? (mp = mp2, mp2 = ((void *) 0)) : (mp = mp -> m_next)) {
            if ((mp -> m_mode & State) && mp -> m_keylen == len && !strncmp((char*)(mp->m_keys),(char*)(ptr),(size_t)((size_t)len))) break;
        }
        if (mp != ((void *) 0)) {
            j = 0;
            if (c != 29) {
                if (((c) < 0) || c == (0x80)) {
                    ...
                    if (c < 0x100 && (c < ' ' || c > '~')) tb [j ++] = 22;
                    tb [j ++] = c;
                }
                tb [j] = '\000';
                (void) ins_typebuf (tb, 1, 0, 1, mp -> m_silent);
            }
            if (mp -> m_expr) s = eval_map_expr (mp -> m_str, c);
            else s = mp -> m_str;
            if (s != ((void *) 0)) {
                (void) ins_typebuf (s, mp -> m_noremap, 0, 1, mp -> m_silent);
                typebuf.tb_no_abbr_cnt += (int) strlen ((char *) (s)) + j + 1;
                if (mp -> m_expr) vim_free (s);
            }
            tb [0] = 8;
            tb [1] = '\000';
            while (len -- > 0) (void) ins_typebuf (tb, 1, 0, 1, mp -> m_silent);
            return 1;
        }
    }
    return 0;
    ...
    int c;
{
    ...
    if (save_cmd == ((void *) 0)) return ((void *) 0);
    ++textlock;
    ++ex_normal_lock;
    set_vim_var_char (c);
    save_cursor = curwin -> w_cursor;
    p = eval_to_string (str, ((void *) 0), 0);
    --textlock;
    --ex_normal_lock;
    ...
    char_u * p;
{
    char_u * res;
    char_u * s, * d;
    res = alloc ((unsigned) (strlen ((char *) (p)) * 3) + 1);
    if (res != ((void *) 0)) {
        d = res;
        for (s = p; *s != '\000';) {
    ...
    char_u * p;
{
    ...
    while (*s != '\000') {
        ...
        else if ((s [0] == (0x80) || s [0] == 0x9b) && s [1] == 253 && s [2] == (int) KE_CSI) {
            *d++ = 0x9b;
            s += 3;
        }
        else *d++ = *s++;
    ...
    buf_T * buf;
{
    ...
    int i;
    validate_maphash ();
    for (abbr = 0; abbr < 2; ++abbr) for (hash = 0; hash < 256; ++hash) {
        if (abbr) {
            if (hash > 0) break;
            if (buf != ((void *) 0)) mp = buf -> b_first_abbr;
            else mp = first_abbr;
        }
        else {
            if (buf != ((void *) 0)) mp = buf -> b_maphash [hash];
            else mp = maphash [hash];
        }
        for (; mp; mp = mp -> m_next) {
            if (mp -> m_noremap == -2) continue;
            for (p = mp -> m_str; *p != '\000'; ++p) if (p [0] == (0x80) && p [1] == 253 && p [2] == (int) KE_SNR) break;
            if (*p != '\000') continue;
            c1 = '\000';
            c2 = '\000';
            c3 = '\000';
            ...
            case 0x20 :
                ...
                break;
            default :
                emsg ((char_u *) (((char *) ("E228: makemap: Illegal mode"))));
                return 0;
            }
            do {
                if (!did_cpo) {
                    if (*mp->m_str == '\000') did_cpo = 1;
                    else for (i = 0; i < 2; ++i) for (p = (i ? mp -> m_str : mp -> m_keys); *p; ++p) if (*p == (0x80) || *p == '\012') did_cpo = 1;
                    if (did_cpo) {
                        if (fprintf (fd, "let s:cpo_save=&cpo") < 0 || put_eol (fd) < 0 || fprintf (fd, "set cpo&vim") < 0 || put_eol (fd) < 0) return 0;
                    }
                }
                if (c1 && putc (c1, fd) < 0) return 0;
                if (mp -> m_noremap != 0 && fprintf (fd, "nore") < 0) return 0;
                if (fputs (cmd, fd) < 0) return 0;
                if (buf != ((void *) 0) && fputs (" <buffer>", fd) < 0) return 0;
                if (mp -> m_silent && fputs (" <silent>", fd) < 0) return 0;
                if (mp -> m_noremap == -2 && fputs ("<script>", fd) < 0) return 0;
                if (mp -> m_expr && fputs (" <expr>", fd) < 0) return 0;
                if (putc (' ', fd) < 0 || put_escstr (fd, mp -> m_keys, 0) == 0 || putc (' ', fd) < 0 || put_escstr (fd, mp -> m_str, 1) == 0 || put_eol (fd) <
                  0) return 0;
                c1 = c2;
                c2 = c3;
                c3 = '\000';
    ...
    if (did_cpo) if (fprintf (fd, "let &cpo=s:cpo_save") < 0 || put_eol (fd) < 0 || fprintf (fd, "unlet s:cpo_save") < 0 || put_eol (fd) < 0) return 0;
    return 1;
}
    ...
    int what;
{
    ...
    for (; *str != '\000'; ++str) {
        ...
        if (c == (0x80) && what != 2) {
            ...
            if (((c) < 0) || modifiers) {
                if (fputs ((char *) get_special_key_name (c, modifiers), fd) < 0) return 0;
                continue;
            }
        }
        if (c == '\012') {
            if (what == 2) {
                ...
                if (fprintf (fd, "<NL>") < 0) return 0;
            }
            continue;
        }
        if (what == 2 && (((c) == ' ' || (c) == '\t') || c == '"' || c == '\\')) {
            if (putc ('\\', fd) < 0) return 0;
        }
        else if (c < ' ' || c > '~' || c == '|' || (what == 0 && c == ' ') || (what == 1 && str == strstart && c == ' ') || (what != 2 && c == '<')) {
            if (putc (22, fd) < 0) return 0;
        }
        if (putc (c, fd) < 0) return 0;
    }
    return 1;
}
...
void check_map_keycodes () {
    ...
    for (bp = firstbuf;; bp = bp -> b_next) {
        for (abbr = 0; abbr <= 1; ++abbr) for (hash = 0; hash < 256; ++hash) {
            if (abbr) {
                if (hash) break;
                if (bp != ((void *) 0)) mp = bp -> b_first_abbr;
                else mp = first_abbr;
            }
            else {
                if (bp != ((void *) 0)) mp = bp -> b_maphash [hash];
                else mp = maphash [hash];
            }
...
char_u * check_map (keys, mode, exact, ign_mod, abbr)
    char_u * keys;
    int mode;
    int exact;
    int ign_mod;
    int abbr;
{
    ...
    validate_maphash ();
    len = (int) strlen ((char *) (keys));
    for (local = 1; local >= 0; --local) for (hash = 0; hash < 256; ++hash) {
        if (abbr) {
            if (hash > 0) break;
            if (local) mp = curbuf -> b_first_abbr;
            else mp = first_abbr;
        }
        else if (local) mp = curbuf -> b_maphash [hash];
        else mp = maphash [hash];
        for (; mp != ((void *) 0); mp = mp -> m_next) {
            if ((mp -> m_mode & mode) && (!exact || mp -> m_keylen == len)) {
                if (len > mp -> m_keylen) minlen = mp -> m_keylen;
                else minlen = len;
                s = mp -> m_keys;
                if (ign_mod && s [0] == (0x80) && s [1] == 252 && s [2] != '\000') {
                    s += 3;
                    if (len > mp -> m_keylen - 3) minlen = mp -> m_keylen - 3;
                }
                if (strncmp ((char *) (s), (char *) (keys), (size_t) (minlen)) == 0) return mp -> m_str;
            }
        }
    }
    return ((void *) 0);
...
{
    {(char_u *) "<D-v> \"*P", 0x01},
    {(char_u *) "<D-v> \"-d\"*P", (0x02 + 0x1000)},
    {(char_u *) "<D-v> <C-R>*", 0x10 + 0x08},
    {(char_u *) "<D-c> \"*y", (0x02 + 0x1000)},
    {(char_u *) "<D-x> \"*d", (0x02 + 0x1000)},
    {(char_u *) "<Backspace> \"-d", (0x02 + 0x1000)},};

void init_mappings () {
    int i;
    for (i = 0; i < sizeof (initmappings) / sizeof (struct initmap); ++i) add_map (initmappings [i].arg, initmappings [i].mode);
}
