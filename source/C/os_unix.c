...
static char_u * oldtitle = ((void *) 0);
static int did_set_title = 0;
static char_u * oldicon = ((void *) 0);
...
static volatile int do_resize = 0;
static char_u * extra_shell_arg = ((void *) 0);
static int show_shell_mess = 1;
static volatile int deadly_signal = 0;
static volatile int in_mch_delay = 0;
    ...
    int len;
{
    ...
    if (p_wd) RealWaitForChar (read_cmd_fd, p_wd, ((void *) 0));
}
    ...
    int tb_change_cnt;
{
    ...
    while (do_resize) handle_resize ();
    if (wtime >= 0) {
        while (WaitForChar (wtime) == 0) {
            if (!do_resize) return 0;
            handle_resize ();
            netbeans_parse_messages ();
        }
    }
...
int kevent64 (int kq, const struct kevent64_s * changelist, int nchanges, struct kevent64_s * eventlist, int nevents, unsigned int flags, const struct timespec
  * timeout);
struct session;
struct pgrp;
struct proc;
...
long_u mch_total_mem (special)
    int special __attribute__ ((unused));
{
    long_u mem = 0;
    ...
    mib [1] = 6;
    len = sizeof (physmem);
    if (sysctl (mib, 2, &physmem, &len, ((void *) 0), 0) == 0) mem = (long_u) physmem;
    {
        struct rlimit rlp;
        if (getrlimit (2, &rlp) == 0 && rlp.rlim_cur < ((rlim_t) 1 << (sizeof (long_u) * 8 - 1)) && rlp.rlim_cur != (((__uint64_t) 1 << 63) - 1) && ((long_u)
          rlp.rlim_cur >> 10) < (mem >> shiftright)) {
            mem = (long_u) rlp.rlim_cur;
            shiftright = 10;
        }
    }
    if (mem > 0) return mem >> shiftright;
...
static void init_signal_stack () {
    if (signal_stack != ((void *) 0)) {
        sigstk.ss_sp = signal_stack;
        sigstk.ss_size = 131072;
        sigstk.ss_flags = 0;
        (void) sigaltstack (&sigstk, ((void *) 0));
    }
}

static void sig_winch (sigarg)
    int sigarg __attribute__ ((unused));
{
    sigset (28, (void (*) ()) sig_winch);
    do_resize = 1;
    return;
}

static void catch_sigint (sigarg)
    int sigarg __attribute__ ((unused));
{
    sigset (2, (void (*) ()) catch_sigint);
    got_int = 1;
    return;
}
    ...
    int sigarg __attribute__ ((unused));
{
    ...
    if (lc_active) {
        lc_signal = sigarg;
        lc_active = 0;
        siglongjmp (lc_jump_env, 1);
    }
    if (in_mch_delay && sigarg == 3) return;
    if (entered == 0 && (0 || sigarg == 1 || sigarg == 3 || sigarg == 15 || sigarg == 30 || sigarg == 31) && !vim_handle_signal(sigarg)) return;
    ++entered;
    set_vim_var_nr (28, (long) entered);
    for (i = 0; signal_info [i].sig != -1; i ++) if (sigarg == signal_info [i].sig) break;
    deadly_signal = sigarg;
    full_screen = 0;
    if (entered >= 3) {
        reset_signals ();
        may_core_dump ();
        if (entered >= 4) _exit (8);
        exit (7);
    }
    if (entered == 2) {
        out_str ((char_u *) (((char *) ("Vim: Double signal, exiting\n"))));
        out_flush ();
        getout (1);
    }
    __builtin___sprintf_chk ((char *) IObuff, 0, __builtin_object_size ((char *) IObuff, 1 > 1), ((char *) ("Vim: Caught deadly signal %s\n")), signal_info [i]
      .name);
    preserve_exit ();
    return;
}
...
static void set_signals () {
    sigset (28, (void (*) ()) sig_winch);
    sigset (18, restricted ? (void (*) (int)) 1 : (void (*) (int)) 0);
    sigset (13, (void (*) (int)) 1);
    catch_int_signal ();
    sigset (14, (void (*) (int)) 1);
    catch_signals (deathtrap, ((void (*) (int)) -1));
}
...
void reset_signals () {
    catch_signals ((void (*) (int)) 0, (void (*) (int)) 0);
}
...
{
    int i;
    for (i = 0; signal_info [i].sig != -1; i ++) if (signal_info [i].deadly) {
        struct sigaction sa;
        sa.__sigaction_u.__sa_handler = func_deadly;
        (*(&sa.sa_mask) = 0, 0);
        sa.sa_flags = 0x0001;
        sigaction (signal_info [i].sig, &sa, ((void *) 0));
    }
    else if (func_other != ((void (*) (int)) -1)) sigset (signal_info [i].sig, func_other);
}
...
int mch_input_isatty () {
    if (isatty (read_cmd_fd)) return 1;
    return 0;
}

static int get_x11_title (test_only)
    int test_only __attribute__ ((unused));
{
    return 0;
}
...
{
    if (!test_only) {
        if (strncmp ((char *) ((term_strings [(int) (KS_NAME)])), (char *) ("builtin_"), (size_t) (8)) == 0) oldicon = vim_strsave ((term_strings [(int) (
          KS_NAME)]) + 8);
        else oldicon = vim_strsave ((term_strings [(int) (KS_NAME)]));
    }
    return 0;
}

int mch_can_restore_title () {
    return get_x11_title (1);
}

int mch_can_restore_icon () {
    return get_x11_icon (1);
}
    ...
    char_u * icon;
{
    ...
    if ((term_strings [(int) (KS_NAME)]) == ((void *) 0)) return;
    if (title == ((void *) 0) && icon == ((void *) 0)) return;
    if (recursive) return;
    ++recursive;
    if ((type || *(term_strings[(int)(KS_TS)]) != '\000') && title != ((void *) 0)) {
        if (oldtitle == ((void *) 0)) (void) get_x11_title (0);
        if (*(term_strings[(int)(KS_TS)]) != '\000') term_settitle (title);
        did_set_title = 1;
    }
    if ((type || *(term_strings[(int)(KS_CIS)]) != '\000') && icon != ((void *) 0)) {
        if (oldicon == ((void *) 0)) get_x11_icon (0);
        if (*(term_strings[(int)(KS_CIS)]) != '\000') {
...
void mch_restore_title (which)
    int which;
{
    mch_settitle (((which & 1) && did_set_title) ? (oldtitle ? oldtitle : p_titleold) : ((void *) 0), ((which & 2) && did_set_icon) ? oldicon : ((void *) 0));
}
    ...
    char_u * name;
{
    ...
    return (strncasecmp ((char *) (name), (char *) ("xterm"), (size_t) (5)) == 0 || strncasecmp ((char *) (name), (char *) ("nxterm"), (size_t) (6)) == 0 ||
      strncasecmp ((char *) (name), (char *) ("kterm"), (size_t) (5)) == 0 || strncasecmp ((char *) (name), (char *) ("mlterm"), (size_t) (6)) == 0 ||
      strncasecmp ((char *) (name), (char *) ("rxvt"), (size_t) (4)) == 0 || strcmp ((char *) (name), (char *) ("builtin_xterm")) == 0);
}

int use_xterm_like_mouse (name)
    char_u * name;
{
    return (name != ((void *) 0) && (term_is_xterm || strncasecmp ((char *) (name), (char *) ("screen"), (size_t) (6)) == 0));
}

int use_xterm_mouse () {
    if (ttym_flags == 0x02) return 2;
    if (ttym_flags == 0x01) return 1;
    return 0;
}
...
{
    if (name == ((void *) 0)) return 0;
    return (strncasecmp ((char *) (name), (char *) ("iris-ansi"), (size_t) (9)) == 0 || strcmp ((char *) (name), (char *) ("builtin_iris-ansi")) == 0);
}
...
{
    if (name == ((void *) 0)) return 0;
    return ((strncasecmp ((char *) (name), (char *) ("vt"), (size_t) (2)) == 0 && vim_strchr ((char_u *) "12345", name [2]) != ((void *) 0)) || strcmp ((char *
      ) (name), (char *) ("builtin_vt320")) == 0);
}
...
{
    if (name == ((void *) 0)) return 0;
    if (vim_is_xterm (name) || vim_is_vt300 (name) || vim_is_iris (name)) return 1;
    return (strncasecmp ((char *) (name), (char *) ("hpterm"), (size_t) (6)) == 0 || strncasecmp ((char *) (name), (char *) ("sun-cmd"), (size_t) (7)) == 0 ||
      strncasecmp ((char *) (name), (char *) ("screen"), (size_t) (6)) == 0 || strncasecmp ((char *) (name), (char *) ("dtterm"), (size_t) (6)) == 0);
}
...
{
    struct passwd * pw;
    if ((pw = getpwuid (uid)) != ((void *) 0) && pw -> pw_name != ((void *) 0) && *(pw->pw_name) != '\000') {
        vim_strncpy (s, (char_u *) pw -> pw_name, len - 1);
        return 1;
    }
    __builtin___sprintf_chk ((char *) s, 0, __builtin_object_size ((char *) s, 1 > 1), "%d", (int) uid);
    return 0;
}
    ...
    int len;
{
    ...
    if (uname (&vutsname) < 0) *s = '\000';
    else vim_strncpy (s, (char_u *) vutsname.nodename, len - 1);
}
...
int mch_dirname (buf, len)
    ...
    int len;
{
    if (getcwd ((char *) buf, len) == ((void *) 0)) {
        ((__builtin_object_size ((char *) (buf), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (buf), (char *) (strerror ((*__error()))),
          __builtin_object_size ((char *) (buf), 1 > 1)) : __inline_strcpy_chk ((char *) (buf), (char *) (strerror ((*__error())))));
        return 0;
    }
    return 1;
}
    ...
    int force;
{
    ...
    if (force || !mch_isFullName(fname)) {
        if ((p = vim_strrchr (fname, '/')) != ((void *) 0)) {
            ...
            if (fd < 0 && (mch_dirname (olddir, 1024) == 0 || mch_chdir ((char *) olddir) != 0)) {
                p = ((void *) 0);
                retval = 0;
            }
            else {
                if (p - fname >= len) retval = 0;
                else {
                    vim_strncpy (buf, fname, p - fname);
                    if (mch_chdir ((char *) buf)) retval = 0;
                    else fname = p + 1;
                    * buf = '\000';
                }
            }
        }
        if (mch_dirname (buf, len) == 0) {
            retval = 0;
            * buf = '\000';
        }
        if (p != ((void *) 0)) {
            if (fd >= 0) {
                if (p_verbose >= 5) {
                    verbose_enter ();
                    msg ((char_u *) ("fchdir() to previous dir"));
                    verbose_leave ();
                }
                l = fchdir (fd);
                close (fd);
            }
            else l = mch_chdir ((char *) olddir);
            if (l != 0) emsg ((char_u *) (((char *) (e_prev_dir))));
        }
        l = strlen ((char *) (buf));
        if (l >= len) retval = 0;
        else {
            if (l > 0 && buf [l - 1] != '/' && *fname != '\000' && strcmp ((char *) (fname), (char *) (".")) != 0) ((__builtin_object_size ((char *) (buf), 0)
              != (size_t) - 1) ? __builtin___strcat_chk ((char *) (buf), (char *) ("/"), __builtin_object_size ((char *) (buf), 1 > 1)) : __inline_strcat_chk (
              (char *) (buf), (char *) ("/")));
        }
    }
    if (retval == 0 || (int) (strlen ((char *) (buf)) + strlen ((char *) (fname))) >= len) return 0;
    if (strcmp ((char *) (fname), (char *) (".")) != 0) ((__builtin_object_size ((char *) (buf), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (buf), (
      char *) (fname), __builtin_object_size ((char *) (buf), 1 > 1)) : __inline_strcat_chk ((char *) (buf), (char *) (fname)));
    return 1;
}
    ...
    if (lstat ((char *) name, &st) >= 0) {
        ...
        if (dirp != ((void *) 0)) {
            while ((dp = readdir (dirp)) != ((void *) 0)) {
                if (strcasecmp ((char *) (tail), (char *) (dp -> d_name)) == 0 && strlen ((char *) (tail)) == strlen ((char *) (dp -> d_name))) {
                    char_u newname [1024 + 1];
                    struct stat st2;
                    vim_strncpy (newname, name, 1024);
                    vim_strncpy (newname + (tail - name), (char_u *) dp -> d_name, 1024 - (tail - name));
                    if (lstat ((char *) newname, &st2) >= 0 && st.st_ino == st2.st_ino && st.st_dev == st2.st_dev) {
                        ((__builtin_object_size ((char *) (tail), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (tail), (char *) (dp -> d_name),
                          __builtin_object_size ((char *) (tail), 1 > 1)) : __inline_strcpy_chk ((char *) (tail), (char *) (dp -> d_name)));
                        break;
                    }
                }
            }
...
{
    struct stat statb;
    if (stat ((char *) name, &statb)) return -1;
...
int mch_setperm (name, perm)
    ...
    long perm;
{
    return (chmod ((char *) name, (mode_t) perm) == 0 ? 1 : 0);
}
...
typedef enum {ACL_READ_DATA = (1 << 1), ACL_LIST_DIRECTORY = (1 << 1), ACL_WRITE_DATA = (1 << 2), ACL_ADD_FILE = (1 << 2), ACL_EXECUTE = (1 << 3), ACL_SEARCH =
  (1 << 3), ACL_DELETE = (1 << 4), ACL_APPEND_DATA = (1 << 5), ACL_ADD_SUBDIRECTORY = (1 << 5), ACL_DELETE_CHILD = (1 << 6), ACL_READ_ATTRIBUTES = (1 << 7),
  ACL_WRITE_ATTRIBUTES = (1 << 8), ACL_READ_EXTATTRIBUTES = (1 << 9), ACL_WRITE_EXTATTRIBUTES = (1 << 10), ACL_READ_SECURITY = (1 << 11), ACL_WRITE_SECURITY = (
  1 << 12), ACL_CHANGE_OWNER = (1 << 13)} acl_perm_t;
typedef enum {ACL_UNDEFINED_TAG = 0, ACL_EXTENDED_ALLOW = 1, ACL_EXTENDED_DENY = 2} acl_tag_t;
typedef enum {ACL_TYPE_EXTENDED = 0x00000100, ACL_TYPE_ACCESS = 0x00000000, ACL_TYPE_DEFAULT = 0x00000001, ACL_TYPE_AFS = 0x00000002, ACL_TYPE_CODA = 0x00000003
  , ACL_TYPE_NTFS = 0x00000004, ACL_TYPE_NWFS = 0x00000005} acl_type_t;
typedef enum {ACL_FIRST_ENTRY = 0, ACL_NEXT_ENTRY = -1, ACL_LAST_ENTRY = -2} acl_entry_id_t;
typedef enum {ACL_FLAG_DEFER_INHERIT = (1 << 0), ACL_ENTRY_INHERITED = (1 << 4), ACL_ENTRY_FILE_INHERIT = (1 << 5), ACL_ENTRY_DIRECTORY_INHERIT = (1 << 6),
  ACL_ENTRY_LIMIT_INHERIT = (1 << 7), ACL_ENTRY_ONLY_INHERIT = (1 << 8)} acl_flag_t;
struct _acl;
struct _acl_entry;
struct _acl_permset;
struct _acl_flagset;
...
void mch_hide (name)
    char_u * name __attribute__ ((unused));
{
}
    ...
    char_u * name;
{
    ...
    if (stat ((char *) name, &statb)) return 0;
    return ((statb.st_mode & 0170000) == 0040000 ? 1 : 0);
}
...
{
    struct stat st;
    if (stat ((char *) name, &st)) return 0;
    return (((st.st_mode) & 0170000) == 0100000) && access (((char *) name), ((1 << 0))) == 0;
}
    ...
    char_u * name;
{
    ...
    char_u * p, * e;
    int retval;
    if (mch_isFullName (name) || (name [0] == '.' && (name [1] == '/' || (name [1] == '.' && name [2] == '/')))) return executable_file (name);
    p = (char_u *) getenv ("PATH");
    if (p == ((void *) 0) || *p == '\000') return -1;
    buf = alloc ((unsigned) (strlen ((char *) (name)) + strlen ((char *) (p)) + 2));
    if (buf == ((void *) 0)) return -1;
    for (;;) {
        e = (char_u *) strchr ((char *) p, ':');
        if (e == ((void *) 0)) e = p + strlen ((char *) (p));
        if (e - p <= 1) ((__builtin_object_size ((char *) (buf), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (buf), (char *) ("./"),
          __builtin_object_size ((char *) (buf), 1 > 1)) : __inline_strcpy_chk ((char *) (buf), (char *) ("./")));
        else {
            vim_strncpy (buf, p, e - p);
            add_pathsep (buf);
        }
        ((__builtin_object_size ((char *) (buf), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (buf), (char *) (name), __builtin_object_size ((char *)
          (buf), 1 > 1)) : __inline_strcat_chk ((char *) (buf), (char *) (name)));
    ...
    char_u * name;
{
    struct stat st;
    if (stat ((char *) name, &st)) return 0;
    if ((((st.st_mode) & 0170000) == 0100000) || (((st.st_mode) & 0170000) == 0040000)) return 0;
    if ((((st.st_mode) & 0170000) == 0060000)) return 2;
    return 1;
}

void mch_early_init () {
    signal_stack = (char *) alloc (131072);
    init_signal_stack ();
}
...
static void exit_scroll () {
    ...
    if (newline_on_exit || msg_didout) {
        if (msg_use_printf ()) {
            if (info_message) printf ("%s", ("\n"));
            else fprintf (__stderrp, "%s", ("\r\n"));
        }
        else out_char ('\n');
    }
    else {
    ...
    int r;
{
    exiting = 1;
    {
        settmode (0);
        mch_restore_title (3);
        if (swapping_screen () && !newline_on_exit) exit_scroll ();
        stoptermcap ();
        if (!swapping_screen() || newline_on_exit) exit_scroll ();
    ...
    int tmode;
{
    ...
    tnew = told;
    if (tmode == 2) {
        tnew.c_iflag &= ~0x00000100;
        tnew.c_lflag &= ~(0x00000100|0x00000008|0x00000080|0x00000002|0x00000400);
        tnew.c_oflag &= ~0x00000002;
        tnew.c_cc [16] = 1;
        tnew.c_cc [17] = 0;
    }
    else if (tmode == 1) tnew.c_lflag &= ~(0x00000008);
    {
        int n = 10;
        while (tcsetattr (read_cmd_fd, 0, &tnew) == -1 && (*__error()) == 4 && n > 0) --n;
    }
    curr_tmode = tmode;
}
...
void get_stty () {
    ...
    if (tcgetattr (read_cmd_fd, &keys) != -1) {
        buf [0] = keys.c_cc [3];
        intr_char = keys.c_cc [8];
        buf [1] = '\000';
        add_termcode ((char_u *) "kb", buf, 0);
        p = find_termcode ((char_u *) "kD");
        if (p != ((void *) 0) && p [0] == buf [0] && p [1] == buf [1]) do_fixdel (((void *) 0));
    }
}
    ...
    int on;
{
    ...
    if (xterm_mouse_vers > 0) {
        if (on) out_str_nf ((char_u *) (xterm_mouse_vers > 1 ? "\033[?1002h" : "\033[?1000h"));
        else out_str_nf ((char_u *) (xterm_mouse_vers > 1 ? "\033[?1002l" : "\033[?1000l"));
        ison = on;
    }
}
...
void check_mouse_termcode () {
    if (use_xterm_mouse ()) {
        set_mouse_termcode (251, (char_u *) (term_is_8bit ((term_strings [(int) (KS_NAME)])) ? "\233M" : "\033[M"));
        if (*p_mouse != '\000') {
            mch_setmouse (0);
            setmouse ();
        }
    }
    else del_mouse_termcode (251);
}

int mch_screenmode (arg)
    char_u * arg __attribute__ ((unused));
{
    emsg ((char_u *) (((char *) (e_screenmode))));
    return 0;
}
...
int mch_get_shellsize () {
    ...
    char_u * p;
    {
        struct winsize ws;
        int fd = 1;
        if (!isatty(fd) && isatty (read_cmd_fd)) fd = read_cmd_fd;
        if (ioctl (fd, ((__uint32_t) 0x40000000 | ((sizeof (struct winsize) &0x1fff) << 16) | ((('t')) << 8) | ((104))), &ws) == 0) {
            columns = ws.ws_col;
            rows = ws.ws_row;
        }
    }
    if (columns == 0 || rows == 0 || vim_strchr (p_cpo, '|') != ((void *) 0)) {
        if ((p = (char_u *) getenv ("LINES"))) rows = atoi ((char *) p);
        if ((p = (char_u *) getenv ("COLUMNS"))) columns = atoi ((char *) p);
    }
    if (columns == 0 || rows == 0) getlinecol (&columns, &rows);
...
static void append_ga_line (garray_T * gap);

static void append_ga_line (gap)
    garray_T * gap;
{
    if (gap -> ga_len > 0 && !curbuf->b_p_bin && ((char_u *) gap -> ga_data) [gap -> ga_len - 1] == '\015') --gap->ga_len;
    ga_append (gap, '\000');
    ml_append (curwin -> w_cursor.lnum ++, gap -> ga_data, 0, 0);
    gap -> ga_len = 0;
}
    ...
    int options;
{
    ...
    int did_settmode = 0;
    out_flush ();
    if (options & 4) settmode (0);
    newcmd = vim_strsave (p_sh);
    if (newcmd == ((void *) 0)) goto error;
    for (i = 0; i < 2; ++i) {
        p = newcmd;
        inquote = 0;
        argc = 0;
        for (;;) {
            if (i == 1) argv [argc] = (char *) p;
            ++argc;
        ...
        if (argv == ((void *) 0)) {
            ...
            if (argv == ((void *) 0)) goto error;
        }
    }
    if (cmd != ((void *) 0)) {
        if (extra_shell_arg != ((void *) 0)) argv [argc ++] = (char *) extra_shell_arg;
    ...
    if (!pipe_error) {
        if ((pid = fork ()) == -1) {
            ...
            reset_signals ();
            if (!show_shell_mess || (options & 2)) {
                int fd;
                fd = open ("/dev/null", 0x0002 | 0, 0);
                fclose (__stdinp);
                fclose (__stdoutp);
                fclose (__stderrp);
                if (fd >= 0) {
            ...
            else if ((options & (32 | 64))) {
                if (p_stmp) {
                    (void) setsid ();
                    sigset (1, (void (*) (int)) 1);
                }
                setenv ("TERM", "dumb", 1);
                __builtin___sprintf_chk ((char *) envbuf, 0, __builtin_object_size ((char *) envbuf, 1 > 1), "%ld", Rows);
                setenv ("ROWS", (char *) envbuf, 1);
                __builtin___sprintf_chk ((char *) envbuf, 0, __builtin_object_size ((char *) envbuf, 1 > 1), "%ld", Rows);
                setenv ("LINES", (char *) envbuf, 1);
                __builtin___sprintf_chk ((char *) envbuf, 0, __builtin_object_size ((char *) envbuf, 1 > 1), "%ld", Columns);
            ...
            catch_signals ((void (*) (int)) 1, ((void (*) (int)) -1));
            catch_int_signal ();
            if ((options & (32 | 64))) {
                char_u buffer [100 + 1];
                char_u ta_buf [100 + 1];
                ...
                if ((options & 64) && toshell_fd >= 0) {
                    if ((wpid = fork ()) == -1) {
                        ...
                        for (;;) {
                            ...
                            if (l == 0) len = 0;
                            else if (lp [written] == '\012') len = write (toshell_fd, "", (size_t) 1);
                            else {
                                s = vim_strchr (lp + written, '\012');
                                len = write (toshell_fd, (char *) lp + written, s == ((void *) 0) ? l : (size_t) (s - (lp + written)));
                            }
                            if (len == (int) l) {
                                if (lnum != curbuf -> b_op_end.lnum || !curbuf->b_p_bin || (lnum != write_no_eol_lnum && (lnum != curbuf -> b_ml.ml_line_count
                                  || curbuf -> b_p_eol))) ignored = write (toshell_fd, "\n", (size_t) 1);
                                ++lnum;
                ...
                if (options & 32) ga_init2 (&ga, 1, 100);
                noread_cnt = 0;
                gettimeofday (&start_tv, ((void *) 0));
                for (;;) {
                    len = 0;
                    if (!(options&2) && ((options & (32 | 64 | 4)) != (32 | 64 | 4)) && wait_pid == 0 && (ta_len > 0 || noread_cnt > 4)) {
                        if (ta_len == 0) {
                            noread_cnt = 0;
                            gettimeofday (&start_tv, ((void *) 0));
                            len = ui_inchar (ta_buf, 100, 10L, 0);
                        }
                        if (ta_len > 0 || len > 0) {
                            if (len == 1 && (pty_master_fd < 0 || cmd != ((void *) 0))) {
                                if (ta_buf [ta_len] == 3 || ta_buf [ta_len] == intr_char) {
                                    kill (-pid, 2);
                                    if (wpid > 0) kill (wpid, 2);
                                }
                                if (pty_master_fd < 0 && toshell_fd >= 0 && ta_buf [ta_len] == 4) {
                                    close (toshell_fd);
                                    toshell_fd = -1;
                                }
                            }
                            for (i = ta_len; i < ta_len + len; ++i) {
                                if (ta_buf [i] == 0x9b && len - i > 2) {
                                    c = (-((ta_buf[i+1])+((int)(ta_buf[i+2])<<8)));
                                    if (c == (-(('k')+((int)('D')<<8))) || c == (-((253)+((int)(KE_KDEL)<<8))) || c == (-(('k')+((int)('b')<<8)))) {
                                        ((__builtin_object_size ((char *) (ta_buf + i + 1), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (ta_buf + i
                                          + 1), (char *) (ta_buf + i + 3), (size_t) (len - i - 2), __builtin_object_size ((char *) (ta_buf + i + 1), 0)) :
                                          __inline_memmove_chk ((char *) (ta_buf + i + 1), (char *) (ta_buf + i + 3), (size_t) (len - i - 2)));
                                        if (c == (-(('k')+((int)('D')<<8))) || c == (-((253)+((int)(KE_KDEL)<<8)))) ta_buf [i] = 0x7f;
                                        else ta_buf [i] = 8;
                                        len -= 2;
                                    }
                                }
                                else if (ta_buf [i] == '\r') ta_buf [i] = '\n';
                            }
                            if (pty_master_fd < 0) {
                                for (i = ta_len; i < ta_len + len; ++i) {
                                    if (ta_buf [i] == '\n' || ta_buf [i] == '\b') msg_putchar (ta_buf [i]);
                                    else msg_outtrans_len (ta_buf + i, 1);
                                }
                                windgoto (msg_row, msg_col);
                                out_flush ();
                            }
                            ta_len += len;
                            if (options & 64) ta_len = 0;
                            else if (toshell_fd >= 0) {
                                len = write (toshell_fd, (char *) ta_buf, (size_t) 1);
                                if (len > 0) {
                                    ta_len -= len;
                                    ((__builtin_object_size ((char *) (ta_buf), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (ta_buf), (char *) (
                                      ta_buf + len), ta_len, __builtin_object_size ((char *) (ta_buf), 0)) : __inline_memmove_chk ((char *) (ta_buf), (char *) (
                                      ta_buf + len), ta_len));
                                }
                            }
                        }
                    ...
                    ++noread_cnt;
                    while (RealWaitForChar (fromshell_fd, 10L, ((void *) 0))) {
                        len = read (fromshell_fd, (char *) buffer, (size_t) 100);
                        if (len <= 0) goto finished;
                        noread_cnt = 0;
                        if (options & 32) {
                            for (i = 0; i < len; ++i) {
                                if (buffer [i] == '\012') append_ga_line (&ga);
                                else if (buffer [i] == '\000') ga_append (&ga, '\012');
                                else ga_append (&ga, buffer [i]);
                            }
                        }
                        else {
                            buffer [len] = '\000';
                        ...
                        if (got_int) break;
                        {
                            ...
                            long msec;
                            gettimeofday (&now_tv, ((void *) 0));
                            msec = (now_tv.tv_sec - start_tv.tv_sec) * 1000L + (now_tv.tv_usec - start_tv.tv_usec) / 1000L;
                            if (msec > 2000) {
                                noread_cnt = 5;
                                break;
                            }
                    ...
                    if (wait_pid == pid) break;
                    wait_pid = waitpid (pid, &status, 0x00000001);
                    if ((wait_pid == (pid_t) - 1 && (*__error()) == 10) || (wait_pid == pid && (((*(int*)&(status)) & 0177) == 0))) {
                        wait_pid = pid;
                    }
                    else wait_pid = 0;
                }
                finished : p_more = p_more_save;
                if (options & 32) {
                    if (ga.ga_len > 0) {
                        append_ga_line (&ga);
                        write_no_eol_lnum = curwin -> w_cursor.lnum;
                    }
                    else write_no_eol_lnum = 0;
                    ga_clear (&ga);
                }
                if (ta_len) ui_inchar_undo (ta_buf, ta_len);
                State = old_State;
                if (toshell_fd >= 0) close (toshell_fd);
                close (fromshell_fd);
            }
            while (wait_pid != pid) {
                wait_pid = wait (&status);
                if (wait_pid <= 0 && (*__error()) == 10) break;
            }
            if (wpid > 0) kill (wpid, 9);
            if (tmode == 2) settmode (2);
            did_settmode = 1;
            set_signals ();
            if ((((*(int*)&(status)) & 0177) == 0)) {
                retval = (((*(int*)&(status)) >> 8) & 0x000000ff);
    ...
    vim_free (argv);
    error : if (!did_settmode) if (tmode == 2) settmode (2);
    resettitle ();
    vim_free (newcmd);
    return retval;
}
...
void mch_breakcheck () {
    if (curr_tmode == 2 && RealWaitForChar (read_cmd_fd, 0L, ((void *) 0))) fill_input_buf (0);
}
    ...
    long msec;
{
    ...
    avail = RealWaitForChar (read_cmd_fd, msec, ((void *) 0));
    return avail;
}
...
static int RealWaitForChar (fd, msec, check_for_gpm)
    int fd;
    long msec;
    int * check_for_gpm __attribute__ ((unused));
{
    int ret;
    int nb_fd = netbeans_filedesc ();
    {
        ...
        __builtin_bzero (&efds, sizeof (*(&efds)));
        do {
            int __fd = (fd);
        ...
        do {
            int __fd = (fd);
            ((&efds) -> fds_bits [__fd / (sizeof (__int32_t) * 8)] |= (1 << (__fd % (sizeof (__int32_t) * 8))));
        }
        while (0);
        maxfd = fd;
        if (nb_fd != -1) {
            ...
            while (0);
            if (maxfd < nb_fd) maxfd = nb_fd;
        }
        ret = select (maxfd + 1, &rfds, ((void *) 0), &efds, tvp);
        if (ret > 0 && nb_fd != -1 && __darwin_fd_isset ((nb_fd), (&rfds))) {
            netbeans_read ();
            --ret;
        }
    }
...
int mch_expandpath (gap, path, flags)
    ...
    int flags;
{
    return unix_expandpath (gap, path, 0, flags, 0);
}
    ...
    int flags;
{
    ...
    * num_file = 0;
    * file = ((void *) 0);
    if (!have_wildcard(num_pat,pat)) return save_patterns (num_pat, pat, num_file, file);
    if (sandbox != 0 && check_secure ()) return 0;
    if (secure || restricted) for (i = 0; i < num_pat; ++i) if (vim_strchr (pat [i], '`') != ((void *) 0) && (check_restricted () || check_secure ())) return 0
      ;
    if ((tempname = vim_tempname ('o')) == ((void *) 0)) {
        emsg ((char_u *) (((char *) (e_notmp))));
        return 0;
    }
    if (num_pat == 1 && *pat[0] == '`' && (len = strlen ((char *) (pat [0]))) > 2 && *(pat[0]+len-1) == '`') shell_style = 4;
    else if ((len = strlen ((char *) (p_sh))) >= 3) {
        if (strcmp ((char *) (p_sh + len - 3), (char *) ("csh")) == 0) shell_style = 1;
        else if (strcmp ((char *) (p_sh + len - 3), (char *) ("zsh")) == 0) shell_style = 3;
    }
    if (shell_style == 0 && strstr ((char *) gettail (p_sh), "sh") != ((void *) 0)) shell_style = 2;
    len = strlen ((char *) (tempname)) + 29;
    if (shell_style == 2) len += strlen ((char *) (sh_vimglob_func));
    for (i = 0; i < num_pat; ++i) {
        ++len;
        for (j = 0; pat [i] [j] != '\000'; ++j) {
            if (vim_strchr ((char_u *) "\t \"&'$;<>()\\|", pat [i] [j]) != ((void *) 0)) ++len;
            ++len;
        }
    }
    command = alloc (len);
    ...
    if (shell_style == 4) {
        ((__builtin_object_size ((char *) (command), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (command), (char *) ("("), __builtin_object_size ((
          char *) (command), 1 > 1)) : __inline_strcpy_chk ((char *) (command), (char *) ("(")));
        ((__builtin_object_size ((char *) (command), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (command), (char *) (pat [0] + 1),
          __builtin_object_size ((char *) (command), 1 > 1)) : __inline_strcat_chk ((char *) (command), (char *) (pat [0] + 1)));
        p = command + strlen ((char *) (command)) - 1;
        *p-- = ')';
        while (p > command && ((*p) == ' ' || (*p) == '\t')) --p;
        if (*p == '&') {
            ampersent = 1;
            * p = ' ';
        }
        ((__builtin_object_size ((char *) (command), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (command), (char *) (">"), __builtin_object_size ((
          char *) (command), 1 > 1)) : __inline_strcat_chk ((char *) (command), (char *) (">")));
    }
    else {
        if (flags & 0x04) ((__builtin_object_size ((char *) (command), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (command), (char *) (
          "set nonomatch; "), __builtin_object_size ((char *) (command), 1 > 1)) : __inline_strcpy_chk ((char *) (command), (char *) ("set nonomatch; ")));
        else ((__builtin_object_size ((char *) (command), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (command), (char *) ("unset nonomatch; "),
          __builtin_object_size ((char *) (command), 1 > 1)) : __inline_strcpy_chk ((char *) (command), (char *) ("unset nonomatch; ")));
        if (shell_style == 1) ((__builtin_object_size ((char *) (command), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (command), (char *) ("glob >"
          ), __builtin_object_size ((char *) (command), 1 > 1)) : __inline_strcat_chk ((char *) (command), (char *) ("glob >")));
        else if (shell_style == 3) ((__builtin_object_size ((char *) (command), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (command), (char *) (
          "print -N >"), __builtin_object_size ((char *) (command), 1 > 1)) : __inline_strcat_chk ((char *) (command), (char *) ("print -N >")));
        else if (shell_style == 2) ((__builtin_object_size ((char *) (command), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (command), (char *) (
          sh_vimglob_func), __builtin_object_size ((char *) (command), 1 > 1)) : __inline_strcat_chk ((char *) (command), (char *) (sh_vimglob_func)));
        else ((__builtin_object_size ((char *) (command), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (command), (char *) ("echo >"),
          __builtin_object_size ((char *) (command), 1 > 1)) : __inline_strcat_chk ((char *) (command), (char *) ("echo >")));
    }
    ((__builtin_object_size ((char *) (command), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (command), (char *) (tempname), __builtin_object_size (
      (char *) (command), 1 > 1)) : __inline_strcat_chk ((char *) (command), (char *) (tempname)));
    if (shell_style != 4) for (i = 0; i < num_pat; ++i) {
        int intick = 0;
        p = command + strlen ((char *) (command));
        *p++ = ' ';
        for (j = 0; pat [i] [j] != '\000'; ++j) {
            if (pat [i] [j] == '`') intick = !intick;
            else if (pat [i] [j] == '\\' && pat [i] [j + 1] != '\000') {
                if (intick || vim_strchr ((char_u *) "\t \"&'$;<>()\\|", pat [i] [j + 1]) != ((void *) 0) || pat [i] [j + 1] == '`') *p++ = '\\';
                ++j;
            }
            else if (!intick && vim_strchr ((char_u *) "\t \"&'$;<>()\\|", pat [i] [j]) != ((void *) 0)) *p++ = '\\';
            *p++ = pat [i] [j];
        }
        * p = '\000';
    }
    if (flags & 0x20) show_shell_mess = 0;
    if (ampersent) ((__builtin_object_size ((char *) (command), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (command), (char *) ("&"),
      __builtin_object_size ((char *) (command), 1 > 1)) : __inline_strcat_chk ((char *) (command), (char *) ("&")));
    if (shell_style == 3) extra_shell_arg = (char_u *) "-G";
    else if (shell_style == 1 && !have_dollars(num_pat,pat)) extra_shell_arg = (char_u *) "-f";
    i = call_shell (command, 2 | 16);
    if (ampersent) mch_delay (10L, 1);
    extra_shell_arg = ((void *) 0);
    show_shell_mess = 1;
    vim_free (command);
    if (i != 0) {
        ...
        if (!(flags&0x20)) {
            ...
            cmdline_row = Rows - 1;
            {
                msg ((char_u *) (((char *) (e_wildexpand))));
                msg_start ();
            }
        }
        if (shell_style == 4) return 0;
        goto notfound;
    }
    ...
    if (shell_style == 0) {
        ...
        if (shell_style == 3 && !did_find_nul) {
            if (len && (int) strlen ((char *) (buffer)) < (int) len - 1) did_find_nul = 1;
            else check_spaces = 1;
        }
        if (len && buffer [len - 1] == '\000') --len;
        else buffer [len] = '\000';
        i = 0;
        for (p = buffer; p < buffer + len; ++p) if (*p == '\000' || (*p == ' ' && check_spaces)) {
            ++i;
            * p = '\000';
        }
        if (len) ++i;
    ...
    * file = (char_u * *) alloc (sizeof (char_u *) *i);
    ...
    for (i = 0; i < *num_file; ++i) {
        (* file) [i] = p;
        if (shell_style == 0 || shell_style == 4 || shell_style == 2) {
            while (!(shell_style==0&&*p == ' ') && *p != '\n' && *p != '\000') ++p;
            if (p == buffer + len) *p = '\000';
            else {
                *p++ = '\000';
                p = skipwhite (p);
            }
    ...
    for (j = 0, i = 0; i < *num_file; ++i) {
        if (!(flags&0x04) && mch_getperm ((*file) [i]) < 0) continue;
        dir = (mch_isdir ((*file) [i]));
        if ((dir && !(flags&0x01)) || (!dir && !(flags&0x02))) continue;
        if (!dir && (flags & 0x40) && !mch_can_exe((*file) [i])) continue;
        p = alloc ((unsigned) (strlen ((char *) ((*file) [i])) + 1 + dir));
        if (p) {
            ((__builtin_object_size ((char *) (p), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (p), (char *) ((*file) [i]), __builtin_object_size ((
              char *) (p), 1 > 1)) : __inline_strcpy_chk ((char *) (p), (char *) ((*file) [i])));
            if (dir) add_pathsep (p);
            (* file) [j ++] = p;
        }
    }
    vim_free (buffer);
    * num_file = j;
    ...
    return 1;
    notfound : if (flags & 0x04) return save_patterns (num_pat, pat, num_file, file);
    return 0;
}
    ...
    char_u * p;
{
    for (; *p; ++p) {
        if (*p == '\\' && p [1] != '\000') ++p;
        else if (vim_strchr ((char_u *) "*?[{'", *p) != ((void *) 0)) return 1;
    }
    return 0;
}
    ...
    char_u * p;
{
    for (; *p; ++p) {
        if (*p == '\\' && p [1] != '\000') ++p;
        else if (vim_strchr ((char_u *) "*?[{`'$", *p) != ((void *) 0) || (*p == '~' && p [1] != '\000')) return 1;
    }
    return 0;
}
...
{
    int i;
    for (i = 0; i < num; i ++) if (mch_has_wildcard (file [i])) return 1;
    return 0;
}
...
{
    int i;
    for (i = 0; i < num; i ++) if (vim_strchr (file [i], '$') != ((void *) 0)) return 1;
    return 0;
}
...
int mch_libcall (libname, funcname, argstring, argint, string_result, number_result)
    char_u * libname;
    char_u * funcname;
    char_u * argstring;
    int argint;
    ...
    int * number_result;
{
    ...
    STRPROCSTR ProcAdd;
    INTPROCSTR ProcAddI;
    char_u * retval_str = ((void *) 0);
    int retval_int = 0;
    int success = 0;
    (void) dlerror ();
    hinstLib = dlopen ((char *) libname, 0x1 | 0x4);
    if (hinstLib == ((void *) 0)) {
        dlerr = (char *) dlerror ();
        if (dlerr != ((void *) 0)) emsg2 ((char_u *) (((char *) ("dlerror = \"%s\""))), (char_u *) (dlerr));
    }
    if (hinstLib != ((void *) 0)) {
        mch_startjmp ();
        if (sigsetjmp ((lc_jump_env), 1) != 0) {
            success = 0;
            dlerr = ((void *) 0);
            mch_didjmp ();
        }
            ...
            if (argstring != ((void *) 0)) {
                ...
                dlerr = (char *) dlerror ();
                if ((success = (ProcAdd != ((void *) 0) && dlerr == ((void *) 0)))) {
                    if (string_result == ((void *) 0)) retval_int = ((STRPROCINT) ProcAdd) (argstring);
                    else retval_str = (ProcAdd) (argstring);
                }
            }
            else {
                ProcAddI = (INTPROCSTR) dlsym (hinstLib, (const char *) funcname);
                dlerr = (char *) dlerror ();
                if ((success = (ProcAddI != ((void *) 0) && dlerr == ((void *) 0)))) {
                    if (string_result == ((void *) 0)) retval_int = ((INTPROCINT) ProcAddI) (argint);
                    else retval_str = (ProcAddI) (argint);
                }
            }
            if (string_result == ((void *) 0)) *number_result = retval_int;
            else if (retval_str != ((void *) 0) && retval_str != (char_u *) 1 && retval_str != (char_u *) -1) *string_result = vim_strsave (retval_str);
        }
        mch_endjmp ();
        if (lc_signal != 0) {
            int i;
            for (i = 0; signal_info [i].sig != -1; i ++) if (lc_signal == signal_info [i].sig) break;
            emsg2 ((char_u *) ("E368: got SIG%s in libcall()"), (char_u *) (signal_info [i].name));
        }
        if (dlerr != ((void *) 0)) emsg2 ((char_u *) (((char *) ("dlerror = \"%s\""))), (char_u *) (dlerr));
        (void) dlclose (hinstLib);
    }
    if (!success) {
        emsg2 ((char_u *) (((char *) (e_libcall))), (char_u *) (funcname));
        return 0;
    }
    return 1;
}
