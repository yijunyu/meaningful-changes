...
static char_u * username = ((void *) 0);
static char_u * ff_expand_buffer = ((void *) 0);
static int coladvance2 (pos_T * pos, int addspaces, int finetune, colnr_T wcol);

int virtual_active () {
    if (virtual_op != 2) return virtual_op;
    return (ve_flags == 4 || ((ve_flags & 5) && VIsual_active && VIsual_mode == 22) || ((ve_flags & 6) && (State & 0x10)));
}
    ...
    colnr_T wcol;
{
    ...
    if (wcol == (0x7fffffffL)) curwin -> w_valid &= ~0x04;
    else {
        curwin -> w_valid |= 0x04;
    ...
    colnr_T wcol;
{
    ...
    if (wcol == (0x7fffffffL) || rc == 0) curwin -> w_valid &= ~0x04;
    else if (*ml_get_cursor() != '\011') {
        curwin -> w_valid |= 0x04;
        curwin -> w_virtcol = wcol;
    }
    return rc;
    ...
    colnr_T wcol;
{
    ...
    int one_more;
    int head = 0;
    one_more = (State & 0x10) || restart_edit != '\000' || (VIsual_active && *p_sel != 'o') || ((ve_flags & 8) && wcol < (0x7fffffffL));
    ...
    if (wcol >= (0x7fffffffL)) {
        ...
        if ((addspaces || finetune) && !VIsual_active) {
            ...
            if (curwin -> w_curswant > 0) --curwin->w_curswant;
        }
    }
    else {
        int width = (curwin -> w_width) - win_col_off (curwin);
        if (finetune && curwin -> w_onebuf_opt.wo_wrap && curwin -> w_width != 0 && wcol >= (colnr_T) width) {
            csize = linetabsize (line);
            if (csize > 0) csize --;
            if (wcol / width > (colnr_T) csize / width && ((State & 0x10) == 0 || (int) wcol > csize + 1)) {
        ...
        if (virtual_active () && addspaces && ((col != wcol && col != wcol + 1) || csize > 1)) {
            if (line [idx] == '\000') {
                ...
                for (t = 0; t < idx; ++t) newline [t] = line [t];
                for (t = 0; t < correct; ++t) newline [t + idx] = ' ';
                newline [idx + correct] = '\000';
                ...
                int t, s = 0;
                int v;
                if (-correct > csize) return 0;
                newline = alloc (linelen + csize);
                if (newline == ((void *) 0)) return 0;
                for (t = 0; t < linelen; t ++) {
                    if (t != idx) newline [s ++] = line [t];
                    else for (v = 0; v < csize; v ++) newline [s ++] = ' ';
                }
                newline [linelen + csize - 1] = '\000';
                ml_replace (pos -> lnum, newline, 0);
    ...
    if (finetune) {
        if (wcol == (0x7fffffffL)) {
            ...
            if (b > 0 && b < ((0x7fffffffL) - 2 * (curwin -> w_width))) pos -> coladd = b;
            col += b;
        }
    }
    if (col < wcol) return 0;
    return 1;
}
...
void check_cursor_lnum () {
    if (curwin -> w_cursor.lnum > curbuf -> b_ml.ml_line_count) {
        if (!hasFolding(curbuf->b_ml.ml_line_count,&curwin->w_cursor.lnum, ((void *) 0))) curwin -> w_cursor.lnum = curbuf -> b_ml.ml_line_count;
    }
    if (curwin -> w_cursor.lnum <= 0) curwin -> w_cursor.lnum = 1;
}
...
void check_cursor_col () {
    ...
    if (len == 0) curwin -> w_cursor.col = 0;
    else if (curwin -> w_cursor.col >= len) {
        if ((State & 0x10) || restart_edit || (VIsual_active && *p_sel != 'o') || (ve_flags & 8) || virtual_active ()) curwin -> w_cursor.col = len;
        else {
            curwin -> w_cursor.col = len - 1;
        }
    }
    else if (curwin -> w_cursor.col < 0) curwin -> w_cursor.col = 0;
    if (oldcol == (0x7fffffffL)) curwin -> w_cursor.coladd = 0;
    else if (ve_flags == 4) {
        if (oldcoladd > curwin -> w_cursor.col) curwin -> w_cursor.coladd = oldcoladd - curwin -> w_cursor.col;
...
void adjust_cursor_col () {
    if (curwin -> w_cursor.col > 0 && (!VIsual_active || *p_sel == 'o') && gchar_cursor () == '\000') --curwin->w_cursor.col;
}
...
int leftcol_changed () {
    ...
    if (retval) curwin -> w_set_curswant = 1;
    redraw_later (40);
    return retval;
}
...
{
    char_u * p;
    p = lalloc ((long_u) size, 1);
    if (p != ((void *) 0)) (void) ((__builtin_object_size ((p), 0) != (size_t) - 1) ? __builtin___memset_chk ((p), (0), ((size_t) size), __builtin_object_size (
      (p), 0)) : __inline_memset_chk ((p), (0), ((size_t) size)));
    return p;
}
    ...
    int message;
{
    ...
    if (size == 0) {
        emsg_silent = 0;
        emsgn ((char_u *) (((char *) ("E341: Internal error: lalloc(%ld, )"))), (long) (size));
        return ((void *) 0);
    }
    for (;;) {
        if ((p = (char_u *) malloc ((size_t) size)) != ((void *) 0)) {
            goto theend;
        }
        if (releasing) break;
        releasing = 1;
    ...
    if (message && p == ((void *) 0)) do_outofmem_msg (size);
    theend : return p;
}
    ...
    long_u size;
{
    if (!did_outofmem_msg) {
        emsg_silent = 0;
        emsgn ((char_u *) (((char *) ("E342: Out of memory!  (allocating %lu bytes)"))), (long) (size));
        did_outofmem_msg = 1;
    }
}
    ...
    char_u * string;
{
    char_u * p;
    unsigned len;
    len = (unsigned) strlen ((char *) (string)) + 1;
    p = alloc (len);
    if (p != ((void *) 0)) ((__builtin_object_size ((char *) (p), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (p), (char *) (string), (size_t) len,
      __builtin_object_size ((char *) (p), 0)) : __inline_memmove_chk ((char *) (p), (char *) (string), (size_t) len));
    return p;
}
    ...
    int len;
{
    ...
    if (p != ((void *) 0)) {
        ((__builtin_object_size ((char *) (p), 0) != (size_t) - 1) ? __builtin___strncpy_chk ((char *) (p), (char *) (string), (size_t) (len),
          __builtin_object_size ((char *) (p), 1 > 1)) : __inline_strncpy_chk ((char *) (p), (char *) (string), (size_t) (len)));
        p [len] = '\000';
    }
    return p;
}
    ...
    int bsl;
{
    ...
    length = 1;
    for (p = string; *p; p ++) {
        if (vim_strchr (esc_chars, *p) != ((void *) 0) || (bsl && rem_backslash (p))) ++length;
        ++length;
    }
    escaped_string = alloc (length);
    if (escaped_string != ((void *) 0)) {
        p2 = escaped_string;
        for (p = string; *p; p ++) {
            if (vim_strchr (esc_chars, *p) != ((void *) 0) || (bsl && rem_backslash (p))) *p2++ = cc;
            *p2++ = *p;
        }
        * p2 = '\000';
    }
    ...
    char_u * p;
{
    ...
    if (p != ((void *) 0)) {
        p2 = p;
        while ((c = *p2) != '\000') *p2++ = (c < 'a' || c > 'z') ? c : (c - 0x20);
    }
}
    ...
    char_u * ptr;
{
    ...
    while (--q > ptr && ((q [0]) == ' ' || (q [0]) == '\t') && q [-1] != '\\' && q [-1] != 22) *q = '\000';
}
...
void vim_strncpy (to, from, len)
    ...
    size_t len;
{
    ((__builtin_object_size ((char *) (to), 0) != (size_t) - 1) ? __builtin___strncpy_chk ((char *) (to), (char *) (from), (size_t) (len), __builtin_object_size
      ((char *) (to), 1 > 1)) : __inline_strncpy_chk ((char *) (to), (char *) (from), (size_t) (len)));
    to [len] = '\000';
}
    ...
    char * sep_chars;
{
    ...
    while (*p != '\000' && vim_strchr ((char_u *) sep_chars, *p) == ((void *) 0)) {
        if (p [0] == '\\' && vim_strchr ((char_u *) sep_chars, p [1]) != ((void *) 0)) ++p;
        if (len < maxlen - 1) buf [len ++] = *p;
        ++p;
    }
    buf [len] = '\000';
    if (*p != '\000' && *p != ',') ++p;
...
{
    int i;
    for (i = 0; i < gap -> ga_len; ++i) vim_free (((char_u * *) (gap -> ga_data)) [i]);
    ga_clear (gap);
}
    ...
    int n;
{
    ...
    if (gap -> ga_maxlen - gap -> ga_len < n) {
        if (n < gap -> ga_growsize) n = gap -> ga_growsize;
        len = gap -> ga_itemsize * (gap -> ga_len + n);
        pp = alloc_clear ((unsigned) len);
        if (pp == ((void *) 0)) return 0;
        gap -> ga_maxlen = gap -> ga_len + n;
        if (gap -> ga_data != ((void *) 0)) {
            ((__builtin_object_size ((char *) (pp), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) (pp), (char *) (gap -> ga_data), (size_t) (gap ->
              ga_itemsize * gap -> ga_len), __builtin_object_size ((char *) (pp), 0)) : __inline_memmove_chk ((char *) (pp), (char *) (gap -> ga_data), (size_t
              ) (gap -> ga_itemsize * gap -> ga_len)));
            vim_free (gap -> ga_data);
        }
        gap -> ga_data = pp;
    }
    ...
    garray_T * gap;
{
    ...
    int len = 0;
    char_u * s;
    for (i = 0; i < gap -> ga_len; ++i) len += (int) strlen ((char *) (((char_u * *) (gap -> ga_data)) [i])) + 1;
    s = alloc (len + 1);
    if (s != ((void *) 0)) {
        * s = '\000';
        for (i = 0; i < gap -> ga_len; ++i) {
            if (*s != '\000') ((__builtin_object_size ((char *) (s), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (s), (char *) (","),
              __builtin_object_size ((char *) (s), 1 > 1)) : __inline_strcat_chk ((char *) (s), (char *) (",")));
            ((__builtin_object_size ((char *) (s), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (s), (char *) (((char_u * *) (gap -> ga_data)) [i]),
              __builtin_object_size ((char *) (s), 1 > 1)) : __inline_strcat_chk ((char *) (s), (char *) (((char_u * *) (gap -> ga_data)) [i])));
        }
    }
    return s;
}
    ...
    char_u * s;
{
    ...
    if (ga_grow (gap, len) == 1) {
        ((__builtin_object_size ((char *) ((char *) gap -> ga_data + gap -> ga_len), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) ((char *) gap ->
          ga_data + gap -> ga_len), (char *) (s), (size_t) len, __builtin_object_size ((char *) ((char *) gap -> ga_data + gap -> ga_len), 0)) :
          __inline_memmove_chk ((char *) ((char *) gap -> ga_data + gap -> ga_len), (char *) (s), (size_t) len));
        gap -> ga_len += len;
    }
}
...
{0x02, '&', '9', '@', '1', 0x02, '&', '0', '@', '2', 0x02, '*', '1', '@', '4', 0x02, '*', '2', '@', '5', 0x02, '*', '3', '@', '6', 0x02, '*', '4', 'k', 'D',
      0x02, '*', '5', 'k', 'L', 0x02, '*', '7', '@', '7', 0x04, 253, (int) KE_C_END, '@', '7', 0x02, '*', '9', '@', '9', 0x02, '*', '0', '@', '0', 0x02, '#',
      '1', '%', '1', 0x02, '#', '2', 'k', 'h', 0x04, 253, (int) KE_C_HOME, 'k', 'h', 0x02, '#', '3', 'k', 'I', 0x02, '#', '4', 'k', 'l', 0x04, 253, (int)
      KE_C_LEFT, 'k', 'l', 0x02, '%', 'a', '%', '3', 0x02, '%', 'b', '%', '4', 0x02, '%', 'c', '%', '5', 0x02, '%', 'd', '%', '7', 0x02, '%', 'e', '%', '8',
      0x02, '%', 'f', '%', '9', 0x02, '%', 'g', '%', '0', 0x02, '%', 'h', '&', '3', 0x02, '%', 'i', 'k', 'r', 0x04, 253, (int) KE_C_RIGHT, 'k', 'r', 0x02, '%',
      'j', '&', '5', 0x02, '!', '1', '&', '6', 0x02, '!', '2', '&', '7', 0x02, '!', '3', '&', '8', 0x02, 253, (int) KE_S_UP, 'k', 'u', 0x02, 253, (int)
      KE_S_DOWN, 'k', 'd', 0x02, 253, (int) KE_S_XF1, 253, (int) KE_XF1, 0x02, 253, (int) KE_S_XF2, 253, (int) KE_XF2, 0x02, 253, (int) KE_S_XF3, 253, (int)
      KE_XF3, 0x02, 253, (int) KE_S_XF4, 253, (int) KE_XF4, 0x02, 253, (int) KE_S_F1, 'k', '1', 0x02, 253, (int) KE_S_F2, 'k', '2', 0x02, 253, (int) KE_S_F3,
      'k', '3', 0x02, 253, (int) KE_S_F4, 'k', '4', 0x02, 253, (int) KE_S_F5, 'k', '5', 0x02, 253, (int) KE_S_F6, 'k', '6', 0x02, 253, (int) KE_S_F7, 'k', '7',
      0x02, 253, (int) KE_S_F8, 'k', '8', 0x02, 253, (int) KE_S_F9, 'k', '9', 0x02, 253, (int) KE_S_F10, 'k', ';', 0x02, 253, (int) KE_S_F11, 'F', '1', 0x02,
      253, (int) KE_S_F12, 'F', '2', 0x02, 253, (int) KE_S_F13, 'F', '3', 0x02, 253, (int) KE_S_F14, 'F', '4', 0x02, 253, (int) KE_S_F15, 'F', '5', 0x02, 253, (
      int) KE_S_F16, 'F', '6', 0x02, 253, (int) KE_S_F17, 'F', '7', 0x02, 253, (int) KE_S_F18, 'F', '8', 0x02, 253, (int) KE_S_F19, 'F', '9', 0x02, 253, (int)
      KE_S_F20, 'F', 'A', 0x02, 253, (int) KE_S_F21, 'F', 'B', 0x02, 253, (int) KE_S_F22, 'F', 'C', 0x02, 253, (int) KE_S_F23, 'F', 'D', 0x02, 253, (int)
      KE_S_F24, 'F', 'E', 0x02, 253, (int) KE_S_F25, 'F', 'F', 0x02, 253, (int) KE_S_F26, 'F', 'G', 0x02, 253, (int) KE_S_F27, 'F', 'H', 0x02, 253, (int)
      KE_S_F28, 'F', 'I', 0x02, 253, (int) KE_S_F29, 'F', 'J', 0x02, 253, (int) KE_S_F30, 'F', 'K', 0x02, 253, (int) KE_S_F31, 'F', 'L', 0x02, 253, (int)
      KE_S_F32, 'F', 'M', 0x02, 253, (int) KE_S_F33, 'F', 'N', 0x02, 253, (int) KE_S_F34, 'F', 'O', 0x02, 253, (int) KE_S_F35, 'F', 'P', 0x02, 253, (int)
      KE_S_F36, 'F', 'Q', 0x02, 253, (int) KE_S_F37, 'F', 'R', 0x02, 'k', 'B', 253, (int) KE_TAB, '\000'};
...
} key_names_table [] =
{
    ...
    {(-((247)+((int)(('X'))<<8))), (char_u *) "NetMouse"},
    {(-((246)+((int)(('X'))<<8))), (char_u *) "DecMouse"},
    {(-((243)+((int)(('X'))<<8))), (char_u *) "JsbMouse"},
    {(-((241)+((int)(('X'))<<8))), (char_u *) "PtermMouse"},
    ...
    int c;
{
    int i;
    c = (((c) < 'a' || (c) > 'z') ? (c) : (c) - ('a' - 'A'));
    for (i = 0; mod_mask_table [i].mod_mask != 0; i ++) if (c == mod_mask_table [i].name) return mod_mask_table [i].mod_flag;
    return 0;
}
    ...
    int * modifiers;
{
    ...
    if (*modifiers & (0x02 | 0x04 | 0x08)) {
        ...
        for (i = 0; modifier_keys_table [i] != '\000'; i += 5) if (key0 == modifier_keys_table [i + 3] && key1 == modifier_keys_table [i + 4] && (*modifiers &
          modifier_keys_table [i])) {
    ...
    int modifiers;
{
    ...
    string [0] = '<';
    idx = 1;
    if (((c) < 0) && ((-(c)) & 0xff) == 242) c = (((unsigned) (-(c)) >> 8) & 0xff);
    if (((c) < 0)) {
        for (i = 0; modifier_keys_table [i] != 0; i += 5) if (((-(c)) & 0xff) == (int) modifier_keys_table [i + 1] && (int) (((unsigned) (-(c)) >> 8) & 0xff) ==
          (int) modifier_keys_table [i + 2]) {
            modifiers |= modifier_keys_table [i];
    ...
    for (i = 0; mod_mask_table [i].name != 'A'; i ++) if ((modifiers & mod_mask_table [i].mod_mask) == mod_mask_table [i].mod_flag) {
    ...
    if (table_idx < 0) {
        if (((c) < 0)) {
            ...
            if (vim_isprintc (c)) string [idx ++] = c;
            else {
                s = transchar (c);
                while (*s) string [idx ++] = *s++;
            }
        }
    }
    else {
        ((__builtin_object_size ((char *) (string + idx), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (string + idx), (char *) (key_names_table [
          table_idx].name), __builtin_object_size ((char *) (string + idx), 1 > 1)) : __inline_strcpy_chk ((char *) (string + idx), (char *) (key_names_table [
          table_idx].name)));
    ...
    int keycode;
{
    ...
    else if (keycode) dlen = (int) (add_char2buf (key, dst + dlen) - dst);
    else dst [dlen ++] = key;
    return dlen;
}
    ...
    int keep_x_key;
{
    ...
    for (bp = src + 1; *bp == '-' || vim_isIDc (*bp); bp ++) {
        ...
        if (bp [0] == 't' && bp [1] == '_' && bp [2] && bp [3]) bp += 3;
    }
    if (*bp == '>') {
        end_of_name = bp + 1;
        if (strncasecmp ((char *) (src + 1), (char *) ("char-"), (size_t) (5)) == 0 && ((src [6]) >= '0' && (src [6]) <= '9')) {
            vim_str2nr (src + 6, ((void *) 0), ((void *) 0), 1, 1, ((void *) 0), &n);
            * modp = 0;
            * srcp = end_of_name;
            return (int) n;
        }
        ...
        if (bp >= last_dash) {
            if (modifiers != 0 && last_dash [2] == '>') key = last_dash [1];
            else {
                key = get_special_key_code (last_dash + 1);
                if (!keep_x_key) key = handle_x_keys (key);
            }
            if (key != '\000') {
                key = simplify_key (key, &modifiers);
                if (!keycode) {
                    if (key == (-(('k')+((int)('b')<<8)))) key = '\010';
                    else if (key == (-(('k')+((int)('D')<<8))) || key == (-((253)+((int)(KE_KDEL)<<8)))) key = 0x7f;
                }
                if (!((key)<0)) key = extract_modifiers (key, &modifiers);
                * modp = modifiers;
                * srcp = end_of_name;
                return key;
            }
...
{
    int modifiers = *modp;
    if (!(modifiers&0x80)) if ((modifiers & 0x02) && ((key) < 0x7f && isalpha (key))) {
        key = (((key) < 'a' || (key) > 'z') ? (key) : (key) - ('a' - 'A'));
        modifiers &= ~0x02;
    }
    if ((modifiers & 0x04) && ((key >= '?' && key <= '_') || ((key) < 0x7f && isalpha (key)))) {
        key = ((((key) < 'a' || (key) > 'z') ? (key) : (key) - ('a' - 'A')) ^ 0x40);
        modifiers &= ~0x04;
        if (key == 0) key = (-((255)+((int)(('X'))<<8)));
    }
    if (!(modifiers&0x80)) if ((modifiers & 0x08) && key < 0x80) {
        key |= 0x80;
        modifiers &= ~0x08;
    }
    * modp = modifiers;
...
{
    int i;
    for (i = 0; key_names_table [i].name != ((void *) 0); i ++) if (c == key_names_table [i].key) break;
    if (key_names_table [i].name == ((void *) 0)) i = -1;
    return i;
}
    ...
    char_u * name;
{
    ...
    if (name [0] == 't' && name [1] == '_' && name [2] != '\000' && name [3] != '\000') {
        ...
        string [1] = name [3];
        string [2] = '\000';
        if (add_termcap_entry (string, 0) == 1) return (-((name[2])+((int)(name[3])<<8)));
    }
    else for (i = 0; key_names_table [i].name != ((void *) 0); i ++) {
        table_name = key_names_table [i].name;
        for (j = 0; vim_isIDc (name [j]) && table_name [j] != '\000'; j ++) if ((((table_name [j]) < 'A' || (table_name [j]) > 'Z') ? (table_name [j]) : (
          table_name [j]) + ('a' - 'A')) != (((name [j]) < 'A' || (name [j]) > 'Z') ? (name [j]) : (name [j]) + ('a' - 'A'))) break;
        if (!vim_isIDc(name[j]) && table_name [j] == '\000') return key_names_table [i].key;
    }
    return 0;
}
...
char_u * get_key_name (i)
    int i;
{
    if (i >= (int) (sizeof (key_names_table) / sizeof (struct key_name_entry))) return ((void *) 0);
...
{
    int i;
    for (i = 0; mouse_table [i].pseudo_code; i ++) if (code == mouse_table [i].pseudo_code) {
...
{
    int i;
    for (i = 0; mouse_table [i].pseudo_code; i ++) if (button == mouse_table [i].button && is_click == mouse_table [i].is_click && is_drag == mouse_table [i].
      is_drag) {
...
{
    int c;
    if (eap != ((void *) 0) && eap -> force_ff != 0) c = eap -> cmd [eap -> force_ff];
    else {
        if ((eap != ((void *) 0) && eap -> force_bin != 0) ? (eap -> force_bin == 1) : buf -> b_p_bin) return 0;
        c = *buf->b_p_ff;
    }
    if (c == 'u') return 0;
    if (c == 'm') return 2;
    ...
    int opt_flags;
{
    ...
    if (p != ((void *) 0)) set_string_option_direct ((char_u *) "ff", -1, (char_u *) p, 1 | opt_flags, 0);
    check_status (curbuf);
    redraw_tabline = 1;
    need_maketitle = 1;
}
    ...
    int opt;
{
    ...
    if (p_verbose > 3) {
        verbose_enter ();
        smsg ((char_u *) ((char *) ("Calling shell to execute: \"%s\"")), cmd == ((void *) 0) ? p_sh : cmd);
        out_char ('\n');
        cursor_on ();
        verbose_leave ();
    }
    if (*p_sh == '\000') {
        emsg ((char_u *) (((char *) (e_shellempty))));
        retval = -1;
    }
    else {
        tag_freematch ();
        if (cmd == ((void *) 0) || *p_sxq == '\000') retval = mch_call_shell (cmd, opt);
        else {
            ncmd = alloc ((unsigned) (strlen ((char *) (cmd)) + strlen ((char *) (p_sxq)) * 2 + 1));
            if (ncmd != ((void *) 0)) {
                ((__builtin_object_size ((char *) (ncmd), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (ncmd), (char *) (p_sxq), __builtin_object_size
                  ((char *) (ncmd), 1 > 1)) : __inline_strcpy_chk ((char *) (ncmd), (char *) (p_sxq)));
                ((__builtin_object_size ((char *) (ncmd), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (ncmd), (char *) (cmd), __builtin_object_size (
                  (char *) (ncmd), 1 > 1)) : __inline_strcat_chk ((char *) (ncmd), (char *) (cmd)));
                ((__builtin_object_size ((char *) (ncmd), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (ncmd), (char *) (p_sxq), __builtin_object_size
                  ((char *) (ncmd), 1 > 1)) : __inline_strcat_chk ((char *) (ncmd), (char *) (p_sxq)));
...
int get_real_state () {
    if (State & 0x01) {
        if (VIsual_active) {
            if (VIsual_select) return 0x1000;
            return 0x02;
        }
        else if (finish_op) return 0x04;
    }
    return State;
}
    ...
    char_u * f2;
{
    ...
    char_u * t1;
    char_u * t2;
    if (f1 == ((void *) 0) || f2 == ((void *) 0)) return 0;
    (void) vim_FullName (f1, ffname, 1024, 0);
...
{
    char_u dir [1024];
    vim_strncpy (dir, fname, 1024 - 1);
    * gettail_sep (dir) = '\000';
    return mch_chdir ((char *) dir) == 0 ? 1 : 0;
}

cursorentry_T shape_table [17] =
{
    ...
    int what;
{
    ...
    for (round = 1; round <= 2; ++round) {
        ...
        while (*modep != '\000') {
            ...
            if (colonp == ((void *) 0)) return (char_u *) "E545: Missing colon";
            if (colonp == modep) return (char_u *) "E546: Illegal mode";
            commap = vim_strchr (modep, ',');
            all_idx = -1;
            while (modep < colonp || all_idx >= 0) {
                if (all_idx < 0) {
                    if (modep [1] == '-' || modep [1] == ':') len = 1;
                    else len = 2;
                    if (len == 1 && (((modep [0]) < 'A' || (modep [0]) > 'Z') ? (modep [0]) : (modep [0]) + ('a' - 'A')) == 'a') all_idx = 17 - 1;
                    else {
                        for (idx = 0; idx < 17; ++idx) if (strncasecmp ((char *) (modep), (char *) (shape_table [idx].name), (size_t) (len)) == 0) break;
                        if (idx == 17 || (shape_table [idx].used_for & what) == 0) return (char_u *) "E546: Illegal mode";
                        if (len == 2 && modep [0] == 'v' && modep [1] == 'e') found_ve = 1;
                    }
                    modep += len + 1;
                }
                if (all_idx >= 0) idx = all_idx --;
                else if (round == 2) {
                    {
                ...
                for (p = colonp + 1; *p && *p != ',';) {
                    {
                        i = *p;
                        len = 0;
                        if (strncasecmp ((char *) (p), (char *) ("ver"), (size_t) (3)) == 0) len = 3;
                        else if (strncasecmp ((char *) (p), (char *) ("hor"), (size_t) (3)) == 0) len = 3;
                        else if (strncasecmp ((char *) (p), (char *) ("blinkwait"), (size_t) (9)) == 0) len = 9;
                        else if (strncasecmp ((char *) (p), (char *) ("blinkon"), (size_t) (7)) == 0) len = 7;
                        else if (strncasecmp ((char *) (p), (char *) ("blinkoff"), (size_t) (8)) == 0) len = 8;
                        if (len != 0) {
                            p += len;
                            if (!((*p) >= '0' && (*p) <= '9')) return (char_u *) "E548: digit expected";
                            n = getdigits (&p);
                            if (len == 3) {
                                if (n == 0) return (char_u *) "E549: Illegal percentage";
                                if (round == 2) {
                                    if ((((i) < 'A' || (i) > 'Z') ? (i) : (i) + ('a' - 'A')) == 'v') shape_table [idx].shape = 2;
                                    else shape_table [idx].shape = 1;
                                    shape_table [idx].percentage = n;
                                }
                            }
                            else if (round == 2) {
                                if (len == 9) shape_table [idx].blinkwait = n;
                                else if (len == 7) shape_table [idx].blinkon = n;
                                else shape_table [idx].blinkoff = n;
                            }
                        }
                        else if (strncasecmp ((char *) (p), (char *) ("block"), (size_t) (5)) == 0) {
                            if (round == 2) shape_table [idx].shape = 0;
                            p += 5;
                        }
                        else {
                            endp = vim_strchr (p, '-');
                            if (commap == ((void *) 0)) {
                                if (endp == ((void *) 0)) endp = p + strlen ((char *) (p));
                            }
                            else if (endp > commap || endp == ((void *) 0)) endp = commap;
                            slashp = vim_strchr (p, '/');
                            ...
                            if (round == 2) {
                                ...
                                if (slashp != ((void *) 0) && slashp < endp) shape_table [idx].id = i;
                            }
                            p = endp;
                        }
                    }
    ...
    if (!found_ve) {
        {
            ...
            shape_table [8].percentage = shape_table [1].percentage;
            shape_table [8].blinkwait = shape_table [1].blinkwait;
            shape_table [8].blinkon = shape_table [1].blinkon;
            shape_table [8].blinkoff = shape_table [1].blinkoff;
...
int get_crypt_method (buf)
    buf_T * buf;
{
    return crypt_method_from_string (*buf->b_p_cm == '\000' ? p_cm : buf -> b_p_cm);
}
...
void crypt_push_state () {
    ...
    else if (crypt_busy > 1) emsg2 ((char_u *) (((char *) (e_intern2))), (char_u *) ("crypt_push_state()"));
    ++crypt_busy;
}
    ...
    char_u * to;
{
    size_t i;
    int ztemp, t;
    if (use_crypt_method == 0) for (i = 0; i < len; ++i) {
        ztemp = from [i];
        {
            ush temp;
            temp = (ush) keys [2] | 2;
        ...
        {
            keys [0] = (crc_32_tab [((int) (keys [0]) ^ ((ztemp))) & 0xff] ^ ((keys [0]) >> 8));
            keys [1] += keys [0] & 0xff;
            keys [1] = keys [1] * 134775813L + 1;
            keys [2] = (crc_32_tab [((int) (keys [2]) ^ ((int) (keys [1] >> 24))) & 0xff] ^ ((keys [2]) >> 8));
        };
        to [i] = t ^ ztemp;
    }
    else bf_crypt_encode (from, len, to);
}
...
{
    char_u * p;
    if (use_crypt_method == 0) for (p = ptr; p < ptr + len; ++p) {
        ush temp;
        temp = (ush) keys [2] | 2;
        temp = (int) (((unsigned) (temp * (temp ^ 1)) >> 8) & 0xff);
        {
            keys [0] = (crc_32_tab [((int) (keys [0]) ^ ((*p ^= temp))) & 0xff] ^ ((keys [0]) >> 8));
            keys [1] += keys [0] & 0xff;
            keys [1] = keys [1] * 134775813L + 1;
            keys [2] = (crc_32_tab [((int) (keys [2]) ^ ((int) (keys [1] >> 24))) & 0xff] ^ ((keys [2]) >> 8));
        };
    }
    else bf_crypt_decode (ptr, len);
}

void crypt_init_keys (passwd)
    char_u * passwd;
{
    if (passwd != ((void *) 0) && *passwd != '\000') {
        if (use_crypt_method == 0) {
            char_u * p;
            make_crc_tab ();
            keys [0] = 305419896L;
            keys [1] = 591751049L;
            keys [2] = 878082192L;
            for (p = passwd; *p != '\000'; ++p) {
                {
                    keys [0] = (crc_32_tab [((int) (keys [0]) ^ (((int) *p))) & 0xff] ^ ((keys [0]) >> 8));
                    keys [1] += keys [0] & 0xff;
                    keys [1] = keys [1] * 134775813L + 1;
                    keys [2] = (crc_32_tab [((int) (keys [2]) ^ ((int) (keys [1] >> 24))) & 0xff] ^ ((keys [2]) >> 8));
                };
        ...
        else bf_crypt_init_keys (passwd);
    }
}
    ...
    int twice;
{
    ...
    for (round = 0;; ++round) {
        cmdline_star = 1;
        cmdline_row = msg_row;
        p1 = getcmdline_prompt ('\000', round == 0 ? (char_u *) ((char *) ("Enter encryption key: ")) : (char_u *) ((char *) ("Enter same key again: ")), 0, 0,
          ((void *) 0));
        cmdline_star = 0;
        if (p1 == ((void *) 0)) break;
        if (round == twice) {
            if (p2 != ((void *) 0) && strcmp ((char *) (p1), (char *) (p2)) != 0) {
                msg ((char_u *) (((char *) ("Keys don't match!"))));
                free_crypt_key (p1);
                free_crypt_key (p2);
                p2 = ((void *) 0);
                round = -1;
            ...
            if (store) {
                set_option_value ((char_u *) "key", 0L, p1, 4);
                free_crypt_key (p1);
                p1 = curbuf -> b_p_key;
            }
            break;
...
void * vim_findfile_init (path, filename, stopdirs, level, free_visited, find_what, search_ctx_arg, tagfile, rel_fname)
    char_u * path;
    char_u * filename;
    char_u * stopdirs __attribute__ ((unused));
    int level;
    int free_visited;
    int find_what;
    void * search_ctx_arg;
    ...
    char_u * rel_fname;
{
    ...
    if (search_ctx_arg != ((void *) 0)) search_ctx = search_ctx_arg;
    else {
        search_ctx = (ff_search_ctx_T *) alloc ((unsigned) sizeof (ff_search_ctx_T));
        if (search_ctx == ((void *) 0)) goto error_return;
        ((__builtin_object_size ((search_ctx), 0) != (size_t) - 1) ? __builtin___memset_chk ((search_ctx), (0), (sizeof (ff_search_ctx_T)),
          __builtin_object_size ((search_ctx), 0)) : __inline_memset_chk ((search_ctx), (0), (sizeof (ff_search_ctx_T))));
    }
    search_ctx -> ffsc_find_what = find_what;
    search_ctx -> ffsc_tagfile = tagfile;
    ff_clear (search_ctx);
    if (free_visited == 1) vim_findfile_free_visited (search_ctx);
    else {
        search_ctx -> ffsc_visited_list = ff_get_visited_list (filename, &search_ctx->ffsc_visited_lists_list);
        if (search_ctx -> ffsc_visited_list == ((void *) 0)) goto error_return;
        search_ctx -> ffsc_dir_visited_list = ff_get_visited_list (filename, &search_ctx->ffsc_dir_visited_lists_list);
        if (search_ctx -> ffsc_dir_visited_list == ((void *) 0)) goto error_return;
    }
    if (ff_expand_buffer == ((void *) 0)) {
        ff_expand_buffer = (char_u *) alloc (1024);
        if (ff_expand_buffer == ((void *) 0)) goto error_return;
    }
    if (path [0] == '.' && (vim_ispathsep (path [1]) || path [1] == '\000') && (!tagfile || vim_strchr (p_cpo, 'd') == ((void *) 0)) && rel_fname != ((void *) 0
      )) {
        ...
        if (search_ctx -> ffsc_start_dir == ((void *) 0)) goto error_return;
        if (*++path != '\000') ++path;
    }
    else if (*path == '\000' || !vim_isAbsName(path)) {
        if (mch_dirname (ff_expand_buffer, 1024) == 0) goto error_return;
    ...
    if (stopdirs != ((void *) 0)) {
        ...
        if (search_ctx -> ffsc_stopdirs_v != ((void *) 0)) {
            do {
                ...
                void * ptr;
                helper = walker;
                ptr = realloc ((search_ctx -> ffsc_stopdirs_v), ((dircount + 1) * sizeof (char_u *)));
                if (ptr) search_ctx -> ffsc_stopdirs_v = ptr;
                else break;
    ...
    if (wc_part != ((void *) 0)) {
        ...
        while (*wc_part != '\000') {
            if (strncmp ((char *) (wc_part), (char *) ("**"), (size_t) (2)) == 0) {
                ...
                if ((char_u *) errpt != wc_part && llevel > 0 && llevel < 255) ff_expand_buffer [len ++] = llevel;
                else if ((char_u *) errpt != wc_part && llevel == 0) len -= 2;
                else ff_expand_buffer [len ++] = ((char_u) 30);
                wc_part = (char_u *) errpt;
                if (*wc_part != '\000' && !vim_ispathsep(*wc_part)) {
                    emsg2 ((char_u *) (((char *) ("E343: Invalid path: '**[number]' must be at the end of the path or be followed by '%s'."))), (char_u *) ("/"
                      ));
                    goto error_return;
                }
            }
        ...
        if (search_ctx -> ffsc_wc_path == ((void *) 0)) goto error_return;
    }
    ...
    ((__builtin_object_size ((char *) (ff_expand_buffer), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (ff_expand_buffer), (char *) (search_ctx ->
      ffsc_start_dir), __builtin_object_size ((char *) (ff_expand_buffer), 1 > 1)) : __inline_strcpy_chk ((char *) (ff_expand_buffer), (char *) (search_ctx ->
      ffsc_start_dir)));
    add_pathsep (ff_expand_buffer);
    ((__builtin_object_size ((char *) (ff_expand_buffer), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (ff_expand_buffer), (char *) (search_ctx ->
      ffsc_fix_path), __builtin_object_size ((char *) (ff_expand_buffer), 1 > 1)) : __inline_strcat_chk ((char *) (ff_expand_buffer), (char *) (search_ctx ->
      ffsc_fix_path)));
    add_pathsep (ff_expand_buffer);
    sptr = ff_create_stack_element (ff_expand_buffer, search_ctx -> ffsc_wc_path, level, 0);
    if (sptr == ((void *) 0)) goto error_return;
    ff_push (search_ctx, sptr);
    search_ctx -> ffsc_file_to_search = vim_strsave (filename);
    if (search_ctx -> ffsc_file_to_search == ((void *) 0)) goto error_return;
    return search_ctx;
    error_return : vim_findfile_cleanup (search_ctx);
    return ((void *) 0);
}
    ...
    char_u * buf;
{
    ...
    while (*r_ptr != '\000' && *r_ptr != ';') {
        if (r_ptr [0] == '\\' && r_ptr [1] == ';') {
            ((__builtin_object_size ((char *) ((r_ptr)), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) ((r_ptr)), (char *) ((r_ptr + 1)), strlen ((
              char *) (r_ptr + 1)) + 1, __builtin_object_size ((char *) ((r_ptr)), 0)) : __inline_memmove_chk ((char *) ((r_ptr)), (char *) ((r_ptr + 1)),
              strlen ((char *) (r_ptr + 1)) + 1));
            r_ptr ++;
        }
        r_ptr ++;
    }
    ...
    else if (*r_ptr == '\000') r_ptr = ((void *) 0);
    return r_ptr;
}
    ...
    void * search_ctx_arg;
{
    ...
    char_u * suf;
    ff_search_ctx_T * search_ctx;
    if (search_ctx_arg == ((void *) 0)) return ((void *) 0);
    search_ctx = (ff_search_ctx_T *) search_ctx_arg;
    if ((file_path = alloc ((int) 1024)) == ((void *) 0)) return ((void *) 0);
    if (search_ctx -> ffsc_start_dir != ((void *) 0)) path_end = &search_ctx->ffsc_start_dir[strlen((char*)(search_ctx->ffsc_start_dir))];
    for (;;) {
        for (;;) {
            ui_breakcheck ();
            if (got_int) break;
            stackp = ff_pop (search_ctx);
            if (stackp == ((void *) 0)) break;
            if (stackp -> ffs_filearray == ((void *) 0) && ff_check_visited (&search_ctx->ffsc_dir_visited_list->ffvl_visited_list, stackp -> ffs_fix_path,
              stackp -> ffs_wc_path) == 0) {
            ...
            if (stackp -> ffs_filearray == ((void *) 0)) {
                ...
                if (!vim_isAbsName(stackp->ffs_fix_path) && search_ctx -> ffsc_start_dir) {
                    ((__builtin_object_size ((char *) (file_path), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (file_path), (char *) (search_ctx ->
                      ffsc_start_dir), __builtin_object_size ((char *) (file_path), 1 > 1)) : __inline_strcpy_chk ((char *) (file_path), (char *) (search_ctx ->
                      ffsc_start_dir)));
                    add_pathsep (file_path);
                }
                ((__builtin_object_size ((char *) (file_path), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (file_path), (char *) (stackp ->
                  ffs_fix_path), __builtin_object_size ((char *) (file_path), 1 > 1)) : __inline_strcat_chk ((char *) (file_path), (char *) (stackp ->
                  ffs_fix_path)));
                add_pathsep (file_path);
                rest_of_wildcards = stackp -> ffs_wc_path;
                if (*rest_of_wildcards != '\000') {
                    len = (int) strlen ((char *) (file_path));
                    if (strncmp ((char *) (rest_of_wildcards), (char *) ("**"), (size_t) (2)) == 0) {
                        ...
                        if (*p == 0) {
                            ((__builtin_object_size ((char *) ((rest_of_wildcards)), 0) != (size_t) - 1) ? __builtin___memmove_chk ((char *) ((
                              rest_of_wildcards)), (char *) ((rest_of_wildcards + 3)), strlen ((char *) (rest_of_wildcards + 3)) + 1, __builtin_object_size ((
                              char *) ((rest_of_wildcards)), 0)) : __inline_memmove_chk ((char *) ((rest_of_wildcards)), (char *) ((rest_of_wildcards + 3)),
                              strlen ((char *) (rest_of_wildcards + 3)) + 1));
                        }
                    ...
                    while (*rest_of_wildcards && !vim_ispathsep(*rest_of_wildcards)) file_path [len ++] = *rest_of_wildcards++;
                    file_path [len] = '\000';
                    if (vim_ispathsep (*rest_of_wildcards)) rest_of_wildcards ++;
                }
                if (path_with_url (dirptrs [0])) {
                    stackp -> ffs_filearray = (char_u * *) alloc ((unsigned) sizeof (char *));
                    if (stackp -> ffs_filearray != ((void *) 0) && (stackp -> ffs_filearray [0] = vim_strsave (dirptrs [0])) != ((void *) 0)) stackp ->
                      ffs_filearray_size = 1;
                    else stackp -> ffs_filearray_size = 0;
                }
                else expand_wildcards ((dirptrs [1] == ((void *) 0)) ? 1 : 2, dirptrs, &stackp->ffs_filearray_size, &stackp->ffs_filearray, 0x01 | 0x08 | 0x20)
                  ;
                stackp -> ffs_filearray_cur = 0;
                stackp -> ffs_stage = 0;
            }
            else rest_of_wildcards = &stackp->ffs_wc_path[strlen((char*)(stackp->ffs_wc_path))];
            if (stackp -> ffs_stage == 0) {
                if (*rest_of_wildcards == '\000') {
                    for (i = stackp -> ffs_filearray_cur; i < stackp -> ffs_filearray_size; ++i) {
                        if (!path_with_url(stackp->ffs_filearray[i]) && !mch_isdir(stackp->ffs_filearray[i])) continue;
                        ((__builtin_object_size ((char *) (file_path), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (file_path), (char *) (stackp ->
                          ffs_filearray [i]), __builtin_object_size ((char *) (file_path), 1 > 1)) : __inline_strcpy_chk ((char *) (file_path), (char *) (
                          stackp -> ffs_filearray [i])));
                        add_pathsep (file_path);
                        ((__builtin_object_size ((char *) (file_path), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (file_path), (char *) (search_ctx
                          -> ffsc_file_to_search), __builtin_object_size ((char *) (file_path), 1 > 1)) : __inline_strcat_chk ((char *) (file_path), (char *) (
                          search_ctx -> ffsc_file_to_search)));
                        len = (int) strlen ((char *) (file_path));
                        if (search_ctx -> ffsc_tagfile) suf = (char_u *) "";
                        else suf = curbuf -> b_p_sua;
                        for (;;) {
                            if ((path_with_url (file_path) || (mch_getperm (file_path) >= 0 && (search_ctx -> ffsc_find_what == 2 || ((search_ctx ->
                              ffsc_find_what == 1) == mch_isdir (file_path))))) && (ff_check_visited (&search_ctx->ffsc_visited_list->ffvl_visited_list,
                              file_path, (char_u *) "") == 1)) {
                                stackp -> ffs_filearray_cur = i + 1;
                                ff_push (search_ctx, stackp);
                                if (!path_with_url(file_path)) simplify_filename (file_path);
                                if (mch_dirname (ff_expand_buffer, 1024) == 1) {
                                    p = shorten_fname (file_path, ff_expand_buffer);
                                    if (p != ((void *) 0)) ((__builtin_object_size ((char *) ((file_path)), 0) != (size_t) - 1) ? __builtin___memmove_chk ((
                                      char *) ((file_path)), (char *) ((p)), strlen ((char *) (p)) + 1, __builtin_object_size ((char *) ((file_path)), 0)) :
                                      __inline_memmove_chk ((char *) ((file_path)), (char *) ((p)), strlen ((char *) (p)) + 1));
                                }
                                return file_path;
                            }
                            if (*suf == '\000') break;
                            copy_option_part (&suf, file_path + len, 1024 - len, ",");
                        }
                    }
                }
                else {
                    for (i = stackp -> ffs_filearray_cur; i < stackp -> ffs_filearray_size; ++i) {
                        if (!mch_isdir(stackp->ffs_filearray[i])) continue;
            ...
            if (strncmp ((char *) (stackp -> ffs_wc_path), (char *) ("**"), (size_t) (2)) == 0) {
                for (i = stackp -> ffs_filearray_cur; i < stackp -> ffs_filearray_size; ++i) {
                    if (strcasecmp ((char *) (((stackp -> ffs_filearray [i]))), (char *) (((stackp -> ffs_fix_path)))) == 0) continue;
                    if (!mch_isdir(stackp->ffs_filearray[i])) continue;
        ...
        if (search_ctx -> ffsc_start_dir && search_ctx -> ffsc_stopdirs_v != ((void *) 0) && !got_int) {
            ff_stack_T * sptr;
            if (ff_path_in_stoplist (search_ctx -> ffsc_start_dir, (int) (path_end - search_ctx -> ffsc_start_dir), search_ctx -> ffsc_stopdirs_v) == 1) break;
            while (path_end > search_ctx -> ffsc_start_dir && vim_ispathsep (*path_end)) path_end --;
            while (path_end > search_ctx -> ffsc_start_dir && !vim_ispathsep(path_end[-1])) path_end --;
            * path_end = 0;
            path_end --;
            if (*search_ctx->ffsc_start_dir == 0) break;
            ((__builtin_object_size ((char *) (file_path), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (file_path), (char *) (search_ctx ->
              ffsc_start_dir), __builtin_object_size ((char *) (file_path), 1 > 1)) : __inline_strcpy_chk ((char *) (file_path), (char *) (search_ctx ->
              ffsc_start_dir)));
            add_pathsep (file_path);
            ((__builtin_object_size ((char *) (file_path), 0) != (size_t) - 1) ? __builtin___strcat_chk ((char *) (file_path), (char *) (search_ctx ->
              ffsc_fix_path), __builtin_object_size ((char *) (file_path), 1 > 1)) : __inline_strcat_chk ((char *) (file_path), (char *) (search_ctx ->
              ffsc_fix_path)));
            sptr = ff_create_stack_element (file_path, search_ctx -> ffsc_wc_path, search_ctx -> ffsc_level, 0);
            if (sptr == ((void *) 0)) break;
            ff_push (search_ctx, sptr);
        }
        else break;
    }
...
{
    ff_search_ctx_T * search_ctx;
    if (search_ctx_arg == ((void *) 0)) return;
    search_ctx = (ff_search_ctx_T *) search_ctx_arg;
    ...
    ff_visited_list_hdr_T * * list_headp;
{
    ...
    if (*list_headp != ((void *) 0)) {
        retptr = *list_headp;
        while (retptr != ((void *) 0)) {
            if (strcasecmp ((char *) (((filename))), (char *) (((retptr -> ffvl_filename)))) == 0) {
                return retptr;
            }
    ...
    if (retptr == ((void *) 0)) return ((void *) 0);
    retptr -> ffvl_visited_list = ((void *) 0);
    retptr -> ffvl_filename = vim_strsave (filename);
    if (retptr -> ffvl_filename == ((void *) 0)) {
        vim_free (retptr);
        return ((void *) 0);
    }
    ...
    char_u * s2;
{
    ...
    if (strlen ((char *) (s1)) != strlen ((char *) (s2))) return 0;
    for (i = 0; s1 [i] != '\000' && s2 [i] != '\000'; i ++) {
        if (s1 [i] != s2 [i] && toupper (s1 [i]) != toupper (s2 [i])) {
            if (i >= 2) if (s1 [i - 1] == '*' && s1 [i - 2] == '*') continue;
            else return 0;
            else return 0;
        }
    }
...
static int ff_check_visited (visited_list, fname, wc_path)
    ...
    char_u * wc_path;
{
    ...
    if (path_with_url (fname)) {
        vim_strncpy (ff_expand_buffer, fname, 1024 - 1);
        url = 1;
    }
    else {
        ff_expand_buffer [0] = '\000';
        if (stat (((char *) fname), (&st)) < 0) return 0;
    }
    for (vp = *visited_list; vp != ((void *) 0); vp = vp -> ffv_next) {
        if (!url ? (vp -> ffv_dev_valid && vp -> ffv_dev == st.st_dev && vp -> ffv_ino == st.st_ino) : strcasecmp ((char *) (((vp -> ffv_fname))), (char *) (((
          ff_expand_buffer)))) == 0) {
            if (ff_wc_equal (vp -> ffv_wc_path, wc_path) == 1) return 0;
        }
    }
    vp = (ff_visited_T *) alloc ((unsigned) (sizeof (ff_visited_T) + strlen ((char *) (ff_expand_buffer))));
    if (vp != ((void *) 0)) {
        if (!url) {
            vp -> ffv_dev_valid = 1;
            ...
            vp -> ffv_dev_valid = 0;
            ((__builtin_object_size ((char *) (vp -> ffv_fname), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (vp -> ffv_fname), (char *) (
              ff_expand_buffer), __builtin_object_size ((char *) (vp -> ffv_fname), 1 > 1)) : __inline_strcpy_chk ((char *) (vp -> ffv_fname), (char *) (
              ff_expand_buffer)));
        }
        if (wc_path != ((void *) 0)) vp -> ffv_wc_path = vim_strsave (wc_path);
        else vp -> ffv_wc_path = ((void *) 0);
...
static ff_stack_T * ff_create_stack_element (fix_part, wc_part, level, star_star_empty)
    char_u * fix_part;
    char_u * wc_part;
    int level;
    int star_star_empty;
{
    ...
    ;
    if (fix_part == ((void *) 0)) fix_part = (char_u *) "";
    new -> ffs_fix_path = vim_strsave (fix_part);
    if (wc_part == ((void *) 0)) wc_part = (char_u *) "";
    new -> ffs_wc_path = vim_strsave (wc_part);
    if (new -> ffs_fix_path == ((void *) 0) || new -> ffs_wc_path == ((void *) 0)) {
    ...
    ff_search_ctx_T * search_ctx;
{
    ...
    if (search_ctx -> ffsc_stack_ptr != ((void *) 0)) search_ctx -> ffsc_stack_ptr = search_ctx -> ffsc_stack_ptr -> ffs_prev;
    return sptr;
}
    ...
    ff_stack_T * stack_ptr;
{
    ...
    if (stack_ptr -> ffs_filearray != ((void *) 0)) FreeWild (stack_ptr -> ffs_filearray_size, stack_ptr -> ffs_filearray);
    vim_free (stack_ptr);
}
...
{
    ff_stack_T * sptr;
    while ((sptr = ff_pop (search_ctx)) != ((void *) 0)) ff_free_stack_element (sptr);
...
{
    int i = 0;
    while (path_len > 1 && vim_ispathsep (path [path_len - 1])) path_len --;
    if (path_len == 0) return 1;
    for (i = 0; stopdirs_v [i] != ((void *) 0); i ++) {
        if ((int) strlen ((char *) (stopdirs_v [i])) > path_len) {
            if (strncasecmp ((char *) (((stopdirs_v [i]))), (char *) (((path))), (size_t) (((path_len)))) == 0 && vim_ispathsep (stopdirs_v [i] [path_len]))
              return 1;
        }
        else {
            if (strcasecmp ((char *) (((stopdirs_v [i]))), (char *) (((path)))) == 0) return 1;
        }
    }
    return 0;
}
...
char_u * find_file_in_path (ptr, len, options, first, rel_fname)
    ...
    char_u * rel_fname;
{
    return find_file_in_path_option (ptr, len, options, first, *curbuf->b_p_path == '\000' ? p_path : curbuf -> b_p_path, 2, rel_fname, curbuf -> b_p_sua);
}
...
char_u * find_directory_in_path (ptr, len, options, rel_fname)
    ...
    char_u * rel_fname;
{
    return find_file_in_path_option (ptr, len, options, 1, p_cdpath, 1, rel_fname, (char_u *) "");
}

char_u * find_file_in_path_option (ptr, len, options, first, path_option, find_what, rel_fname, suffixes)
    char_u * ptr;
    int len;
    int options;
    int first;
    ...
    char_u * suffixes;
{
    ...
    if (first == 1) {
        ...
        if (ff_file_to_find == ((void *) 0)) {
            file_name = ((void *) 0);
            goto theend;
        }
    }
    rel_to_curdir = (ff_file_to_find [0] == '.' && (ff_file_to_find [1] == '\000' || vim_ispathsep (ff_file_to_find [1]) || (ff_file_to_find [1] == '.' && (
      ff_file_to_find [2] == '\000' || vim_ispathsep (ff_file_to_find [2])))));
    if (vim_isAbsName (ff_file_to_find) || rel_to_curdir) {
        if (first == 1) {
            ...
            for (run = 1; run <= 2; ++run) {
                l = (int) strlen ((char *) (ff_file_to_find));
                if (run == 1 && rel_to_curdir && (options & 16) && rel_fname != ((void *) 0) && strlen ((char *) (rel_fname)) + l < 1024) {
                    ((__builtin_object_size ((char *) (NameBuff), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (NameBuff), (char *) (rel_fname),
                      __builtin_object_size ((char *) (NameBuff), 1 > 1)) : __inline_strcpy_chk ((char *) (NameBuff), (char *) (rel_fname)));
                    ((__builtin_object_size ((char *) (gettail (NameBuff)), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (gettail (NameBuff)), (char *
                      ) (ff_file_to_find), __builtin_object_size ((char *) (gettail (NameBuff)), 1 > 1)) : __inline_strcpy_chk ((char *) (gettail (NameBuff)), (
                      char *) (ff_file_to_find)));
                    l = (int) strlen ((char *) (NameBuff));
                }
                else {
                    ((__builtin_object_size ((char *) (NameBuff), 0) != (size_t) - 1) ? __builtin___strcpy_chk ((char *) (NameBuff), (char *) (ff_file_to_find)
                      , __builtin_object_size ((char *) (NameBuff), 1 > 1)) : __inline_strcpy_chk ((char *) (NameBuff), (char *) (ff_file_to_find)));
                    run = 2;
                }
                buf = suffixes;
                for (;;) {
                    if ((mch_getperm (NameBuff) >= 0 && (find_what == 2 || ((find_what == 1) == mch_isdir (NameBuff))))) {
        ...
        for (;;) {
            if (did_findfile_init) {
                ...
                if ((buf = alloc ((int) (1024))) == ((void *) 0)) break;
                buf [0] = 0;
                copy_option_part (&dir, buf, 1024, " ,");
                r_ptr = vim_findfile_stopdir (buf);
                fdip_search_ctx = vim_findfile_init (buf, ff_file_to_find, r_ptr, 100, 0, find_what, fdip_search_ctx, 0, rel_fname);
                if (fdip_search_ctx != ((void *) 0)) did_findfile_init = 1;
                vim_free (buf);
            }
        }
    }
    if (file_name == ((void *) 0) && (options & 1)) {
        if (first == 1) {
            if (find_what == 1) emsg2 ((char_u *) (((char *) ("E344: Can't find directory \"%s\" in cdpath"))), (char_u *) (ff_file_to_find));
            else emsg2 ((char_u *) (((char *) ("E345: Can't find file \"%s\" in path"))), (char_u *) (ff_file_to_find));
        }
        else {
            if (find_what == 1) emsg2 ((char_u *) (((char *) ("E346: No more directory \"%s\" found in cdpath"))), (char_u *) (ff_file_to_find));
            else emsg2 ((char_u *) (((char *) ("E347: No more file \"%s\" found in path"))), (char_u *) (ff_file_to_find));
        }
    }
    theend : return file_name;
}
    ...
    char_u * new_dir;
{
    char_u * dir_name;
    int r;
    dir_name = find_directory_in_path (new_dir, (int) strlen ((char *) (new_dir)), 1, curbuf -> b_ffname);
    if (dir_name == ((void *) 0)) return -1;
...
{
    if (username == ((void *) 0)) {
        if (mch_get_user_name (buf, len) == 0) return 0;
    ...
    int maxlen;
{
    ...
    for (i = 0; maxlen < 0 || i < maxlen; ++i) {
        ...
        if (toupper (p [i]) != toupper (q [i])) {
            if (vim_ispathsep (p [i])) return -1;
            if (vim_ispathsep (q [i])) return 1;
            return ((char_u *) p) [i] - ((char_u *) q) [i];
        }
    }
    if (s == ((void *) 0)) return 0;
    if (s [i + 1] == '\000' && i > 0 && !vim_ispathsep(*(((char_u*)s+i)-1)) && s [i] == '/') return 0;
    if (s == q) return -1;
    return 1;
}
    ...
    char_u * fname;
{
    ...
    if ((perm & 0222) && access (((char *) fname), ((1 << 1))) == 0) {
        ++retval;
        if (mch_isdir (fname)) ++retval;
    }
    return retval;
}
...
{
    int i;
    for (i = len - 1; i >= 0; --i) if (putc ((int) (nr >> (i * 8)), fd) == (-1)) return 0;
    return 1;
}
    ...
    time_t the_time;
{
    ...
    for (i = 7; i >= 0; --i) {
        if (i + 1 > (int) sizeof (time_t)) putc (0, fd);
        else {
            c = (int) (wtime >> (i * 8));
            putc (c, fd);
        }
