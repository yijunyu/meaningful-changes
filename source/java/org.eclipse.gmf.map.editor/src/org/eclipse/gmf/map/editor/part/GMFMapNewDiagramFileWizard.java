 ...
 * @generated
 */
public class GMFMapNewDiagramFileWizard extends Wizard {
	 ...
	 * @generated
	 */
	public GMFMapNewDiagramFileWizard(IFile selectedModelFile, IWorkbenchPage workbenchPage, IStructuredSelection selection, EObject diagramRoot, TransactionalEditingDomain editingDomain) {
		assert selectedModelFile != null : "Null selectedModelFile in GMFMapNewDiagramFileWizard constructor"; //$NON-NLS-1$
		assert workbenchPage != null : "Null workbenchPage in GMFMapNewDiagramFileWizard constructor"; //$NON-NLS-1$
		assert selection != null : "Null selection in GMFMapNewDiagramFileWizard constructor"; //$NON-NLS-1$
		assert diagramRoot != null : "Null diagramRoot in GMFMapNewDiagramFileWizard constructor"; //$NON-NLS-1$
		assert editingDomain != null : "Null editingDomain in GMFMapNewDiagramFileWizard constructor"; //$NON-NLS-1$
	...
	public void addPages() {
		myFileCreationPage = new WizardNewFileCreationPage("Initialize new Ecore diagram file", mySelection) {
			...
			public void createControl(Composite parent) {
				...
				String fileExtension = ".gmfmap_diagram"; //$NON-NLS-1$
		...
		myFileCreationPage.setDescription("Create new diagram based on " + MappingEditPart.MODEL_ID + " model content");
	...
	public boolean performFinish() {
		...
		AbstractTransactionalCommand command = new AbstractTransactionalCommand(myEditingDomain, "Initializing diagram contents", affectedFiles) { //$NON-NLS-1$
			...
			protected CommandResult doExecuteWithResult(IProgressMonitor monitor, IAdaptable info) throws ExecutionException {
				int diagramVID = GMFMapVisualIDRegistry.getDiagramVisualID(myDiagramRoot);
				if (diagramVID != MappingEditPart.VISUAL_ID) {
					return CommandResult.newErrorCommandResult("Incorrect model object stored as a root resource object"); //$NON-NLS-1$
				}
				Diagram diagram = ViewService.createDiagram(myDiagramRoot, MappingEditPart.MODEL_ID, GMFMapDiagramEditorPlugin.DIAGRAM_PREFERENCES_HINT);
		...
		try {
			...
			GMFMapDiagramEditorUtil.openDiagramEditor(myWorkbenchPage, diagramFile);
		} catch (ExecutionException e) {
			GMFMapDiagramEditorPlugin.getInstance().logError("Unable to create model and diagram", e); //$NON-NLS-1$
		} catch (IOException ex) {
			GMFMapDiagramEditorPlugin.getInstance().logError("Save operation failed for: " + diagramFile.getFullPath().toString(), ex); //$NON-NLS-1$
		} catch (PartInitException ex) {
			GMFMapDiagramEditorPlugin.getInstance().logError("Unable to open editor", ex); //$NON-NLS-1$
		}
		return true;
	}
	...
	private class RootElementSelectorPage extends WizardPage implements ISelectionChangedListener {
		...
		private void createModelBrowser(Composite parent) {
			...
			treeViewer.setContentProvider(new AdapterFactoryContentProvider(GMFMapDiagramEditorPlugin.getInstance().getItemProvidersAdapterFactory()));
			treeViewer.setLabelProvider(new AdapterFactoryLabelProvider(GMFMapDiagramEditorPlugin.getInstance().getItemProvidersAdapterFactory()));
		...
		private boolean validatePage() {
			...
			boolean result = ViewService.getInstance().provides(
					new CreateDiagramViewOperation(new EObjectAdapter(myDiagramRoot), MappingEditPart.MODEL_ID, GMFMapDiagramEditorPlugin.DIAGRAM_PREFERENCES_HINT));
